...
bool CAIEditor::AddNewObject(const SMapObjectInfo &object, IRefCount **pObject)
{
  ...
  *pObject = pAILogic->AddObject(object, pIDB, nullptr, false, true, nullptr);

  // CRAP { Misha needs a piece of the trench to be immediately added to Yura
  if (pIDB->GetDesc(object.szName.c_str())->eGameType == SGVOGT_ENTRENCHMENT) updater.Update(ACTION_NOTIFY_NEW_ST_OBJ, static_cast<IUpdatableObj *>(*pObject));
  // CRAP}
...
{
  if (auto pStaticObj = dynamic_cast<CStaticObject *>(pObj)) pStaticObj->SetPlayerForEditor(nPlayer);
  else if (auto pUnit = dynamic_cast<CCommonUnit *>(pObj)) pUnit->SetPlayerForEditor(nPlayer);
}

void CAIEditor::SetDiplomacies(const std::vector<BYTE> &playerParty) { theDipl.SetDiplomaciesForEditor(playerParty); }
// //////////////////////////////////////////////////////////// 
...
void CAIEditor::LoadEntrenchments(const std::vector<SEntrenchmentInfo> &entrenchments) { pAILogic->LoadEntrenchments(entrenchments); }

void CAIEditor::RecalcPassabilityForPlayer(CArray2D<BYTE> *array, const int nPlayer) { theStatObjs.RecalcPassabilityForPlayer(array, nPlayer); }
...
{
  if (auto pUnit = dynamic_cast<CAIUnit *>(pObject))
  {
  ...
  else if (auto pFormation = dynamic_cast<CFormation *>(pObject))
  {
  ...
  else if (auto pStObject = dynamic_cast<CExistingObject *>(pObject)) { if (pStObject->CanBeMovedTo(CVec2(x, y))) pStObject->SetNewPlacement(CVec2(x, y), pStObject->GetDir()); }
  else
...
{
  if (CCommonUnit *pUnit = dynamic_cast<CAIUnit *>(pObject)) pUnit->Disappear();
  else if (auto pStObject = dynamic_cast<CExistingObject *>(pObject)) pStObject->DeleteForEditor();
  else
    NI_ASSERT_T(false, "Unknown object");
}
...
{
  if (auto pFormation = dynamic_cast<CFormation *>(pObject))
  {
  ...
  else if (auto pUnit = dynamic_cast<CAIUnit *>(pObject))
  {
    if (pUnit->CanSetNewDir(GetVectorByDirection(wDir)))
    {
      SRect rect;
      GetUnitRectByStats(pUnit->GetStats(), pUnit->GetCenter(), wDir, &rect);
      if (IsRectInsideOfMap(rect)) pUnit->UpdateDirectionForEditor(GetVectorByDirection(wDir));
    }
  }
  else if (auto pStObject = dynamic_cast<CExistingObject *>(pObject)) pStObject->SetNewPlacement(pStObject->GetCenter(), wDir);
...
{
  if (auto pUnit = dynamic_cast<CAIUnit *>(pObject)) pUnit->TakeEditorDamage(fHP);
  else if (auto pFormation = dynamic_cast<CFormation *>(pObject)) { for (int i = 0; i < pFormation->Size(); ++i) DamageObject((*pFormation)[i], fHP); }
  else if (auto pStObject = dynamic_cast<CStaticObject *>(pObject)) pStObject->TakeEditorDamage(fHP);
  else
...
{
  if (auto pUnit = dynamic_cast<CAIUnit *>(pObject)) return pUnit->GetHitPoints();
  else if (auto pStObject = dynamic_cast<CStaticObject *>(pObject)) return pStObject->GetHitPoints();
  else
    NI_ASSERT_T(false, "Unknown object");
...
void CAIEditor::HandOutLinks()
{
  ...
  for (CGlobalIter iter(0, ANY_PARTY); !iter.IsFinished(); iter.Iterate())
  {
    ...
    if ((*iter)->IsInFormation()) (*iter)->GetFormation()->SetLink(id++);
  }

  for (CStObjGlobalIter<false> iter; !iter.IsFinished(); iter.Iterate()) (*iter)->SetLink(id++);
}
...
{
  if (auto pUnit = dynamic_cast<CAIUnit *>(pObject)) return pUnit->GetFormation();
  return nullptr;
}

bool CAIEditor::IsFormation(IRefCount *pObject) const { return dynamic_cast<CFormation *>(pObject); }
...
void CAIEditor::GetUnitsInFormation(IRefCount *pObject, IRefCount ***pUnits, int *pnLen)
{
  ...
  auto pFormation = static_cast<CFormation *>(pObject);

  *pUnits = GetTempBuffer<IRefCount *>(pFormation->Size());
  *pnLen = pFormation->Size();

  for (int i = 0; i < *pnLen; ++i) (*pUnits)[i] = (*pFormation)[i];
}

const CVec2 &CAIEditor::GetCenter(IRefCount *pObj) const
{
  if (auto pUnit = dynamic_cast<CCommonUnit *>(pObj)) return pUnit->GetCenter();
  else if (auto pObject = dynamic_cast<CStaticObject *>(pObj)) return pObject->GetCenter();
  else
...
{
  if (auto pUnit = dynamic_cast<CCommonUnit *>(pObj)) return pUnit->GetFrontDir();
  else if (auto pObject = dynamic_cast<CStaticObject *>(pObj)) return 0;
  else
    NI_ASSERT_T(false, "Wrong object passed");
...
bool CheckStaticObject(const SMapObjectInfo &object, IObjectsDB *pIDB, const SGDBObjectDesc *pDesc, const T &checkFunc)
{
  ...
  if (object.vPos.x - vOrigin.x + SConsts::TILE_SIZE / 2 < 0.0f) return false;
  if (object.vPos.y - vOrigin.y + SConsts::TILE_SIZE / 2 < 0.0f) return false;
  ...
  for (int y = 0; y < passability.GetSizeY(); ++y)
  {
    for (int x = 0; x < passability.GetSizeX(); ++x)
    {
      ...
      if (!checkFunc(passability[y][x], tile)) return false;
    }
  }
...
{
  if (!IsObjectInsideOfMap(object)) return false;
  else
  {
    CPtr<IObjectsDB> pIDB = GetSingleton<IObjectsDB>();
    CGDBPtr<SGDBObjectDesc> pDesc = pIDB->GetDesc(object.szName.c_str());
...
void CAIEditor::Clear() { pAILogic->Clear(); }

void CAIEditor::ApplyPattern(const struct SVAPattern &rPattern) { theStaticMap.ApplyPattern(rPattern); }

void CAIEditor::UpdateAllHeights() { theStaticMap.UpdateAllHeights(); }

bool CAIEditor::ToggleShow(const int nShowType) { return pAILogic->ToggleShow(nShowType); }
...
void CAIEditor::UpdateTerrain(const CTRect<int> &rect, const STerrainInfo &terrainInfo)
{
  ...
  for (CUnitsIter<0, 2> iter(0, ANY_PARTY, vCenter, Max(vAABBHalfSize.x, vAABBHalfSize.y)); !iter.IsFinished(); iter.Iterate())
  {
    ...
    nMinX = Min(nMinX, Max(tile.x - SConsts::MAX_UNIT_RADIUS - 1, 0));
    nMinY = Min(nMinY, Max(tile.y - SConsts::MAX_UNIT_RADIUS - 1, 0));
    nMaxX = Max(nMaxX, Min(tile.x + SConsts::MAX_UNIT_RADIUS + 1, theStaticMap.GetSizeX() - 1));
    nMaxY = Max(nMaxY, Min(tile.y + SConsts::MAX_UNIT_RADIUS + 1, theStaticMap.GetSizeY() - 1));
  }
  ...
  for (auto iter = terrainInfo.rivers.begin(); iter != terrainInfo.rivers.end(); ++iter) theStaticMap.UpdateRiverPassability(*iter, false, false);

  // remove old terrain
  for (int x = nMinX; x <= nMaxX; ++x) { for (int y = nMinY; y <= nMaxY; ++y) theStaticMap.RemoveTerrainPassability(x, y); }
  ...
  for (auto iter = terrainInfo.rivers.begin(); iter != terrainInfo.rivers.end(); ++iter) theStaticMap.UpdateRiverPassability(*iter, true, false);

  // return static 
  for (CStObjCircleIter<false> iter(vCenter, Max(vAABBHalfSize.x, vAABBHalfSize.y)); !iter.IsFinished(); iter.Iterate())
  {
    CExistingObject *pObj = *iter;
    if (pObj->GetObjectType() != ESOT_BRIDGE_SPAN) pObj->LockTiles(true);
  }
  ...
  for (CUnitsIter<0, 2> iter(0, ANY_PARTY, vCenter, Max(vAABBHalfSize.x, vAABBHalfSize.y)); !iter.IsFinished(); iter.Iterate())
  {
    ...
    if (!pUnit->GetStats()->IsInfantry()) pUnit->ForceLockingTiles(false);
  }

  theStaticMap.UpdateMaxesForAddedRect(nMinX, nMinY, nMaxX, nMaxY);
}

void CAIEditor::DeleteRiver(const SVectorStripeObject &river) { theStaticMap.UpdateRiverPassability(river, false, true); }

void CAIEditor::AddRiver(const SVectorStripeObject &river) { theStaticMap.UpdateRiverPassability(river, true, true); }