...
const WORD GetDirectionByVector(float x, float y)
{
  ...
  if (x + y != 0) return 16384.0f * y / (x + y) + add;
  else return 0;
}

// //////////////////////////////////////////////////////////// 
const WORD GetDirectionByVector(CVec2 vec) { return GetDirectionByVector(vec.x, vec.y); }
// //////////////////////////////////////////////////////////// 
const CVec2 GetVectorByDirection(const WORD dir)
{
  const float fDir = static_cast<float>(dir % 16384) / 16384.0f;
...
const int DifferenceSign(const WORD dir1, const WORD dir2)
{
  ...
  return Sign(static_cast<int>(antiClockWise) - static_cast<int>(clockWise));
}
...
{
  return
      static_cast<WORD>(dir - startAngleDir) + static_cast<WORD>(finishAngleDir - dir) == static_cast<WORD>(finishAngleDir - startAngleDir);
}
...
{
  return
      static_cast<int>(DirsDifference(dir, dir1)) + static_cast<int>(DirsDifference(dir, dir2)) == DirsDifference(dir1, dir2);
}

// //////////////////////////////////////////////////////////// 
const WORD GetZDirectionBy3DVector(const float x, const float y, const float z) { return GetDirectionByVector(fabs(x, y), z); }
// //////////////////////////////////////////////////////////// 
const WORD GetZDirectionBy3DVector(const CVec2 &vec, const float z) { return GetZDirectionBy3DVector(vec.x, vec.y, z); }
// //////////////////////////////////////////////////////////// 
...
// //////////////////////////////////////////////////////////// 
const WORD GetZAngle(const CVec2 &vec, const float z) { return GetZAngle(vec.x, vec.y, z); }
// //////////////////////////////////////////////////////////// 
const float GetDistanceToSegment(const CVec2 &vSegmentStart, const CVec2 &vSegmentEnd, const CVec2 &vPoint)
{
  ...
  if (fDiff3 < fDiff2 + fDiff1)// the normal from the point does not fall on the segment
  {
...
bool SRect::IsPointInside(const CVec2 &point) const
{
  ...
  if (rightSign == 0) return fabs2(point - center) < 0.001f;
...
{
  if (IsPointInside(circleCenter)) return true;
  ...
  if (vLocalCoordCenter.x < -lengthBack) fDist += sqr(vLocalCoordCenter.x - (-lengthBack));
  else if (vLocalCoordCenter.x > lengthAhead) fDist += sqr(vLocalCoordCenter.x - lengthAhead);

  if (vLocalCoordCenter.y < -width) fDist += sqr(vLocalCoordCenter.y - (-width));
  else if (vLocalCoordCenter.y > width) fDist += sqr(vLocalCoordCenter.y - width);
...
{
  // modulo difference 65536
  const WORD diff = dirFromRectCenter - GetDirectionByVector(dir);

  if (diff <= 8192) return RPG_FRONT;
  if (diff > 8192 && diff <= 24576) return RPG_LEFT;
  if (diff > 24576 && diff <= 40960) return RPG_BACK;
  if (diff > 40960 && diff <= 57344) return RPG_RIGHT;
  return RPG_FRONT;
}

// //////////////////////////////////////////////////////////// 
const int SRect::GetSide(const CVec2 &point) const { return GetSide(GetDirectionByVector(point - center)); }
// //////////////////////////////////////////////////////////// 
...
const float fabs(const SRect rect1, const SRect rect2)
{
  ...
  // do not intersect
  if (segm1Max < segm2Min || segm2Max < segm1Min) return Min(fabs(segm1Max - segm2Min), fabs(segm2Max - segm1Min));
  return 0;
}
...
const int CBSpline::N_ITERS_TO_FORWARD = DELTA_FORWARD / DELTA;
// //////////////////////////////////////////////////////////// 
