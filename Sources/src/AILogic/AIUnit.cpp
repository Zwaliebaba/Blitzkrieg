...
{
  // hit the battalion system
  if (combatRect.IsPointInside(explCoord))
  {
    // struck
    if (nRandPiercing >= nRandArmor && !IsSavedByCover()) return SAINotifyHitInfo::EHT_HIT;
...
const int CAIUnit::GetRandArmorByDir(const int nArmorDir, const WORD wAttackDir, const SRect &unitRect)
{
  ...
  {
    case 0:
      return GetRandomArmor(unitRect.GetSide(wAttackDir));
    case 1:
      return GetRandomArmor(RPG_BOTTOM);
    case 2:
      return GetRandomArmor(RPG_TOP);
...
bool CAIUnit::ProcessCumulativeExpl(CExplosion *pExpl, const int nArmorDir, const bool bFromExpl)
{
  if (!IsInSolidPlace())
  {
    ...
    if (fabs(GetZ() - pExpl->GetExplZ()) <= GetStats()->vAABBHalfSize.y && unitRect.IsPointInside(vExplCoord))
    {
      ...
      // if it hits from below, then do not squeeze
      if (nArmorDir != 1) unitRect.Compress(GetRemissiveCoeff());

      const SAINotifyHitInfo::EHitType eHitType = ProcessExactHit(unitRect, vExplCoord, pExpl->GetRandomPiercing(), nRandArmor);

      if (eHitType == SAINotifyHitInfo::EHT_HIT || theCheats.GetFirstShoot(pExpl->GetPartyOfShoot()) == 1) TakeDamage(pExpl->GetRandomDamage(), &pExpl->GetShellStats(), pExpl->GetPlayerOfShoot(), pExpl->GetWhoFire());

      if (!GetStats()->IsInfantry()) pExpl->AddHitToSend(new CHitInfo(pExpl, this, eHitType, CVec3(vExplCoord, pExpl->GetExplZ())));
      else
      {
        auto pSoldier = static_cast<CSoldier *>(this);
        if (!pSoldier->IsInBuilding()) pExpl->AddHitToSend(new CHitInfo(pExpl, this, eHitType, CVec3(vExplCoord, pExpl->GetExplZ())));
      }
...
void CAIUnit::IncreaseHitPoints(const float fInc)
{
  ...
  if (fHitPoints != fMaxHP)
  {
    ...
    if (fFormerHP / fMaxHP <= SConsts::LOW_HP_PERCENTAGE && fHitPoints / fMaxHP > SConsts::LOW_HP_PERCENTAGE) UnRegisterAsBored(ACK_BORED_LOW_HIT_POINTS);
  }
}

void CAIUnit::TakeEditorDamage(const float fDamage)
{
  if (fDamage < 0) IncreaseHitPoints(fabs(fDamage));
  else
  {
    updater.Update(ACTION_NOTIFY_RPG_CHANGED, this);

    if ((fHitPoints -= fDamage) <= 0) theGroupLogic.UnitCommand(SAIUnitCmd(ACTION_COMMAND_DIE, false), this, false);
  }
}
...
void CAIUnit::Init(const CVec2 &center, const int z, const float fHP, const WORD dir, const BYTE _player, const WORD _id, EObjVisType eVisType, const int dbID)
{
  ...
  //
  pPathUnit = GetStats()->IsTrain() ? CreateObject<CPathUnit>(AI_CARRIAGE_PATH_UNIT) : CreateObject<CPathUnit>(AI_SIMPLE_PATH_UNIT);
  pPathUnit->Init(this, center, z, dir, id);

  pAnimUnit = GetStats()->IsInfantry() ? CreateObject<IAnimUnit>(AI_ANIM_UNIT_SOLDIER) : CreateObject<IAnimUnit>(AI_ANIM_UNIT_MECH);
  pAnimUnit->Init(this);
  ...
  if (CBasicGun *pGun = GetFirstArtilleryGun())
  {
    ...
    GetSingleton<IScene>()->GetStatSystem()->UpdateEntry("Artillery guns:", NStr::Format("%d %g", nCntGuns, fRange / static_cast<float>(nCntGuns)));
  }
}
...
void CAIUnit::SetScenarioStats()
{
  ...
  if (IScenarioUnit *pScenarioUnit = GetScenarioUnit())
  {
    ...
    while (nLevel > 0 && fExperience < pExpLevels->levels[nLevel].nExp) --nLevel;
  }
  else
  {
    fExperience = 0;
...
void CAIUnit::PrepareToDelete()
{
  if (bAlive)
  {
    ...
    if (pAntiArtillery != nullptr) theAAManager.RemoveAA(pAntiArtillery);

    theGroupLogic.DelUnitFromGroup(this);
    theGroupLogic.DelUnitFromSpecialGroup(this);
    DelCmdQueue(GetID());

    pAntiArtillery = nullptr;
...
void CAIUnit::Disappear()
{
  ...
  theGraveyard.PushToKilled(SKilledUnit(nullptr, 0, 0, curTime + SConsts::DEAD_SEE_TIME), this);
...
void CAIUnit::DieTrain(const float fDamage)
{
  if (bAlive)
  {
    ...
    if (GetStats()->type == RPG_TYPE_TRAIN_LOCOMOTIVE) StopUnit();
    GetState()->TryInterruptState(nullptr);
...
void CAIUnit::Die(const bool fromExplosion, const float fDamage)
{
  if (bAlive)
  {
    if (GetStats()->IsTrain()) DieTrain(fDamage);
    else
    {
      if (!theCheats.IsHistoryPlaying() && theDipl.GetMyParty() == GetParty())
      {
        if (GetScenarioUnit()) updater.AddFeedBack(SAIFeedBack(EFB_SCENARIO_UNIT_DEAD, MAKELONG(GetCenter().x, GetCenter().y)));
        if (GetStats()->type == RPG_TYPE_SNIPER && GetPlayer() == theDipl.GetMyNumber()) updater.AddFeedBack(SAIFeedBack(EFB_SNIPER_DEAD, MAKELONG(GetCenter().x, GetCenter().y)));
      }
      ...
      if (fromExplosion && GetStats()->IsInfantry() && IsFree()) timeToDeath = curTime + Random(SConsts::AI_SEGMENT_DURATION) * 20;
      else timeToDeath = curTime;
    }
...
const bool CAIUnit::IsVisible(const BYTE cParty) const { return visible4Party[cParty]; }

bool CAIUnit::CalculateUnitVisibility4Party(const BYTE party) const
{
  if (bAlwaysVisible) return true;
  if (theDipl.GetNeutralParty() == party) return false;
...
const DWORD CAIUnit::GetNormale(const CVec2 &vCenter) const { return theStaticMap.GetNormal(vCenter.x, vCenter.y); }

const DWORD CAIUnit::GetNormale() const { return CAIUnit::GetNormale(GetCenter()); }
  ...
  if (!theDipl.IsNetGame() && nextRevealCheck <= curTime)
  {
    ...
    if (bQueredToReveal)
    {
      ...
      if (!bRevealed)
      {
        ...
        if (bRevealed) timeOfReveal = curTime;
      }
...
void CAIUnit::FreezeSegment()
{
  if (!theDipl.IsNetGame())
  {
    ...
    {
      if (bVisibleByPlayer && GetSpeed() != VNULL2) theCombatEstimator.AddUnit(this);
      else theCombatEstimator.DelUnit(this);
    }
  ...
  // TankPit processing
  if (!IsValidObj(pTankPit)) pTankPit = nullptr;

  if (creationTime + 1000 < curTime) CCommonUnit::FreezeSegment();

  if (GetState() && (IsRestState(GetState()->GetName()) || GetState()->GetName() == EUSN_USE_SPYGLASS)) AnalyzeCamouflage();
...
const bool CAIUnit::IsVisibleByPlayer() { return bVisibleByPlayer; }
...
void CAIUnit::CalcVisibility()
{
  if (curTime >= creationTime + SConsts::AI_SEGMENT_DURATION * SConsts::SHOW_ALL_TIME_COEFF)
  {
    ...
    {
      // is visible or a sufficiently long time has passed since going into invisibility (so as not to blink)
      if (bVisibility || lastTimeOfVis + SConsts::RESIDUAL_VISIBILITY_TIME < curTime)
      {
        ...
        if (IsInTankPit()) UpdateTankPitVisibility();
        if (GetStats()->IsArtillery()) (static_cast<CArtillery *>(this))->UpdateAmmoBoxVisibility();
      }
    }
    ...
    for (int i = 0; i < theDipl.GetNPlayers(); ++i)
    {
      ...
      if (static_cast<bool>(wWisibility & (1 << i)) != bVisibility)
      {
        theSupremeBeing.SetUnitVisible(this, i, bVisibility);
        wWisibility ^= (1 << i);

        if (theDipl.GetNeutralPlayer() != GetPlayer() && !theDipl.IsAIPlayer(GetPlayer()) && theDipl.IsAIPlayer(i)) pUnitInfoForGeneral->UpdateVisibility(bVisibility);

        if (bVisibility && GetParty() == theDipl.GetNeutralParty()) UpdateUnitsRequestsForResupply();
      }
    }

    const int nUnitParty = GetParty();
    for (int i = 0; i < 3; ++i) visible4Party[i] = CalculateUnitVisibility4Party(i);
  }
}
...
void CAIUnit::GetRPGStats(SAINotifyRPGStats *pStats)
{
  ...
  {
    if (GetCommonGunStats(i).bPrimary) pStats->nMainAmmo += GetNAmmo(i);
    else pStats->nSecondaryAmmo += GetNAmmo(i);
  }

  pStats->time = curTime;
}

bool CAIUnit::CanCommandBeExecuted(CAICommand *pCommand) { return GetStats()->HasCommand(pCommand->ToUnitCmd().cmdType); }

bool CAIUnit::CanCommandBeExecutedByStats(CAICommand *pCommand) { return CanCommandBeExecutedByStats(pCommand->ToUnitCmd().cmdType); }

bool CAIUnit::CanCommandBeExecutedByStats(int nCmd) const { return GetStats()->HasCommand(nCmd); }
  ...
  return
      Max(0.0f,
          (GetStats()->fSight * GetSightMultiplier() * GetExpLevel().fBonusSight +
           floor(theStaticMap.GetTileHeight(GetTile()) / SConsts::HEIGHT_FOR_VIS_RADIUS_INC)) * SConsts::TILE_SIZE
          );
}
...
void CAIUnit::TakeDamage(const float fDamage, const SWeaponRPGStats::SShell *pShell, const int nPlayerOfShoot, CAIUnit *pShotUnit)
{
  if (fHitPoints > 0)
  {
    ...
    {
      if (pShotUnit && pShotUnit->IsAlive()) pShotUnit->EnemyKilled(this);
      SendNTotalKilledUnits(nPlayerOfShoot);

      // kill or remove from the map
      ...
      if (!GetStats()->IsAviation() && theStaticMap.IsBridge(GetTile()))
      {
        ...
        if (bDisappear) theGroupLogic.UnitCommand(SAIUnitCmd(ACTION_COMMAND_DISAPPEAR), this, false);
        if (GetStats()->IsInfantry()) theGraveyard.AddBridgeKilledSoldier(GetTile(), this);
      ...
      if (!bDisappear)
      {
        const bool bFromExpl = (pShell != nullptr && pShell->fArea2 != 0);
        theGroupLogic.UnitCommand(SAIUnitCmd(ACTION_COMMAND_DIE, fDamage, bFromExpl), this, false);
      }
    }
    else// the unit didn't die
    {
      const SUnitBaseRPGStats *pStats = GetStats();
      if (!pStats->IsAviation())
      {
        const float fMaxHP = pStats->fMaxHP;
        if (fFormerHP / fMaxHP > SConsts::LOW_HP_PERCENTAGE && fHitPoints / fMaxHP <= SConsts::LOW_HP_PERCENTAGE) RegisterAsBored(ACK_BORED_LOW_HIT_POINTS);

        if (fFormerHP == fMaxHP && fHitPoints != fMaxHP) theSupremeBeing.UnitAskedForResupply(this, pStats->IsInfantry() ? ERT_MEDICINE : ERT_REPAIR, true);
      }
    }
  }
}
...
{
  if (pAntiArtillery != nullptr && fGunRadius != 0.0f) pAntiArtillery->Fired(fGunRadius, GetCenter());

  if (!GetStats()->IsAviation())
  {
    const int nAmmo = GetNAmmo(nGun);
    const int nMaxAmmo = GetCommonGunStats(nGun).nAmmo;
    if (nAmmo < nMaxAmmo / 3 && (nAmmo + 1) >= nMaxAmmo / 3) theSupremeBeing.UnitAskedForResupply(this, ERT_RESUPPLY, true);
  }
}
...
void CAIUnit::UpdateUnitsRequestsForResupply()
{
  if (!theDipl.IsNetGame())
  {
    ...
    if (theDipl.GetNParty(GetPlayer()) == theDipl.GetNeutralParty()) { if (GetStats()->IsArtillery()) theSupremeBeing.UnitAskedForResupply(this, ERT_HUMAN_RESUPPLY, true); }
    else
    {
      const SUnitBaseRPGStats *pStats = GetStats();
      if (!pStats->IsAviation())
      ...
      {
        if (pStats->IsInfantry()) theSupremeBeing.UnitAskedForResupply(this, ERT_MEDICINE, true);
        else if (!pStats->IsAviation()) theSupremeBeing.UnitAskedForResupply(this, ERT_REPAIR, true);
      }
    }
  }
}
...
{
  if (GetParty() != theDipl.GetNParty(cPlayer) && !theDipl.IsNetGame()) { theSupremeBeing.UnitChangedParty(this, theDipl.GetNParty(cPlayer)); }
  ...
  {
    theGroupLogic.DelUnitFromGroup(this);
    if (theGroupLogic.BeginGroup(nGroup) == theGroupLogic.EndGroup()) theGroupLogic.UnregisterGroup(nGroup);
  }
  ...
  for (int i = 0; i < GetNGuns(); ++i)
  {
    ...
    if (GetGun(i)->GetOwner() == this) GetGun(i)->SetOwner(this);
  }

  if (pAntiArtillery) pAntiArtillery->SetParty(GetParty());

  if (GetFormation()) GetFormation()->ChangePlayer(cPlayer);
}

void CAIUnit::SetPlayerForEditor(const int nPlayer) { player = nPlayer; }
// //////////////////////////////////////////////////////////// 
float CAIUnit::GetRemissiveCoeff() const { return GetStats()->fSmallAABBCoeff * theDifficultyLevel.GetSmallAABBCoeff(GetParty()); }

const NTimer::STime CAIUnit::GetTimeToCamouflage() const
{
  if (GetStats()->type == RPG_TYPE_SNIPER) return SConsts::TIME_BEFORE_SNIPER_CAMOUFLAGE;
...
{
  float fSpeed = 0;
  for (int i = 0; i < sizeof(dwGuns) * 8 && i < GetNGuns(); ++i) { if ((dwGuns & i << i)) fSpeed += GetKillSpeed(pEnemy, GetGun(i)); }
...
{
  float fSpeed = 0;
  for (int i = 0; i < sizeof(dwGuns) * 8 && i < GetNGuns(); ++i) { if ((dwGuns & i << i)) fSpeed += GetKillSpeed(pStats, vCenter, GetGun(i)); }
...
const float CAIUnit::GetKillSpeed(const SHPObjectRPGStats *pStats, const CVec2 &vCenter, CBasicGun *pGun) const
{
  ...
  static constexpr float weights[4] = {1.2f, 0.6f, 0.3f, 0.6f};
  ...
  if (fPiercingProbability == 0.0f) return 0;
  else
  {
    const float fMaxHP = pStats->fMaxHP;
...
{
  int i = 0;
  while (i < GetNGuns() && !GetGun(i)->CanShootToUnit(pEnemy)) ++i;

  if (i >= GetNGuns()) return 0;
...
{
  // NI_ASSERT_T( fTakenDamagePower + fUpdate >= -0.0001f, "Wrong damage power taken" );
  if (fTakenDamagePower + fUpdate >= -0.0001f)
  {
    fTakenDamagePower += fUpdate;
    if (fTakenDamagePower < 0 && fTakenDamagePower >= -0.0001f) fTakenDamagePower = 0.0f;
...
{
  // long-range AI weapon
  if (theDipl.IsAIPlayer(GetPlayer()) && GetFirstArtilleryGun() != nullptr && !DoesReservePosExist()) return GetFirstArtilleryGun()->GetFireRange(0);
  if (GetStats()->type == RPG_TYPE_OFFICER) return GetGun(0)->GetFireRange(0) * SConsts::OFFICER_COEFFICIENT_FOR_SCAN;
  if (GetStats()->IsArtillery()) return Min(GetMaxFireRange(), SConsts::MAX_FIRE_RANGE_TO_SHOOT_BY_LINE);
...
void CAIUnit::LookForTarget(CAIUnit *pCurTarget, const bool bDamageUpdated, CAIUnit **pBestTarget, CBasicGun **pGun)
{
  if (CanShoot())
  {
         ...
         theScanLimiter.CanScan() && !iter.IsFinished(); iter.Iterate())
    {
      ...
      {
        // for enemy artillery add all artillerymen
        if (pTarget->GetStats()->IsArtillery())
        {
          auto pArt = checked_cast<CArtillery *>(pTarget);
          if (pArt->HasServeCrew())
          {
            ...
            for (int i = 0; i < pCrew->Size(); ++i)
            {
              ...
              if (pSoldier->IsNoticableByUnit(this, r)) AddUnitToShootEstimator(pSoldier);
            }
          }
        }
         ...
         theScanLimiter.CanScan() && !iter.IsFinished(); iter.Iterate())
    {
      ...
      for (int i = 0; i < pTarget->GetNDefenders(); ++i) { if (theDipl.GetDiplStatus(GetPlayer(), pTarget->GetPlayer()) == EDI_ENEMY && pTarget->GetUnit(i)->IsNoticableByUnit(this, r)) AddUnitToShootEstimator(pTarget->GetUnit(i)); }
    }
    ...
    *pGun = GetBestShootEstimatedGun();
    ...
    if (GetBestShootEstimatedUnit() == nullptr && CanShootToPlanes())
    {
      CShootEstimatorLighAA estimatorAA;
      estimatorAA.Init(this);

      for (CPlanesIter iter; !iter.IsFinished(); iter.Iterate()) { if (theDipl.GetDiplStatus(GetPlayer(), (*iter)->GetPlayer()) == EDI_ENEMY) estimatorAA.AddUnit(*iter); }
...
void CAIUnit::SetCircularAttack(const bool bCanAttack) { for (int i = 0; i < GetNGuns(); ++i) GetGun(i)->SetCircularAttack(bCanAttack); }
...
BYTE CAIUnit::AnalyzeTargetScan(CAIUnit *pCurTarget, const bool bDamageUpdated, const bool bScanForObstacles, IRefCount *pCheckBuilding)
{
  if (IsTimeToAnalyzeTargetScan() && theScanLimiter.CanScan())
  {
    ...
    {
      if (GetState() && GetState()->IsAttackingState()) targetScanRandom = Random(0, 3000);
      else targetScanRandom = Random(0, 2000);
    }
    else targetScanRandom = Random(0, 1000);

    const SBehaviour &beh = GetBehaviour();
    CAICommand *pCommand = GetCurCmd();

    // if there's fire all over
    if (beh.fire == SBehaviour::EFAtWill)
    {
      // target found
      CAIUnit *pTarget = nullptr;
      CBasicGun *pGun = nullptr;

      LookForTarget(pCurTarget, bDamageUpdated, &pTarget, &pGun);

      if (pTarget != nullptr && pCheckBuilding != nullptr && pTarget->GetStats()->IsInfantry() &&
          static_cast<CSoldier *>(pTarget)->IsInBuilding() && static_cast<CSoldier *>(pTarget)->GetBuilding() == pCheckBuilding)
        pTarget = nullptr;

      if (pTarget != nullptr && (pCurTarget == nullptr || pTarget != pCurTarget))
      {
        if (GetStats()->type == RPG_TYPE_ART_ROCKET) theGroupLogic.InsertUnitCommand(SAIUnitCmd(ACTION_COMMAND_ART_BOMBARDMENT, pTarget->GetCenter()), this);
        else theGroupLogic.InsertUnitCommand(SAIUnitCmd(ACTION_COMMAND_SWARM_ATTACK_UNIT, pTarget), this);

        return 3;
      }
      if (!pTarget && bScanForObstacles)
      {
        // there are no enemies, shoot at obstacles if necessary
...
void CAIUnit::SetAmbush() { updater.Update(ACTION_NOTIFY_SET_AMBUSH, this); }

void CAIUnit::RemoveAmbush() { updater.Update(ACTION_NOTIFY_REMOVE_AMBUSH, this); }

void CAIUnit::GetFogInfo(SFogInfo *pInfo) const
{
  pInfo->pObject = nullptr;
...
void CAIUnit::WarFogChanged()
{
  if (IsAlive())
  {
    ...
    if (bShouldUpdateWarfog)
    {
      ...
      if (fogInfo.bAngleLimited || fogInfo.wVisionAngle < 32768) theWarFog.ChangeUnitState(id, fogInfo);
      else theWarFog.ChangeUnitCoord(id, fogInfo);
    }
  }
}
...
const int CAIUnit::ChooseFatality(const float fDamage)
{
      ...
      Random(0.0f, 0.1f) < SConsts::MASSIVE_DAMAGE_FATALITY_PROBABILITY)
  {
    if (pStats->animdescs.size() > ANIMATION_DEATH_FATALITY && !pStats->animdescs[ANIMATION_DEATH_FATALITY].empty() && !pStats->aabb_as.empty())
    {
      ...
      if (bFree) return nFatality;
    }
  }

  if (pStats->animdescs[ANIMATION_DEATH].empty()) return -1;
  else return -1 * static_cast<int>(Random(pStats->animdescs[ANIMATION_DEATH].size()) + 1);
}

void CAIUnit::SetMoraleSupport() { bHasMoraleSupport = true; }
...
void CAIUnit::SetMorale(float _fMorale)
{
  ...
  if (fMorale != _fMorale)
  {
    if (SConsts::MORALE_MIN_VALUE != fMorale && SConsts::MORALE_MIN_VALUE == _fMorale) RegisterAsBored(ACK_BORED_MINIMUM_MORALE);
    else if (SConsts::MORALE_MIN_VALUE == fMorale && SConsts::MORALE_MIN_VALUE != _fMorale) UnRegisterAsBored(ACK_BORED_MINIMUM_MORALE);

    if (_fMorale == SConsts::MORALE_MIN_VALUE) theSupremeBeing.UnitAskedForResupply(this, ERT_MORALE, true);
    if (_fMorale == 1.0f) theSupremeBeing.UnitAskedForResupply(this, ERT_MORALE, false);
...
void CAIUnit::InitializeShootArea(SShootArea *pArea, CBasicGun *pGun, const float fRangeMin, const float fRangeMax) const
{
  ...
  if (NeedDeinstall() || !CanMove() && !CanRotate())
  {
    const WORD wConstraint =
        pGun ? pGun->GetTurret() ? pGun->GetTurret()->GetHorTurnConstraint() : pGun->GetWeapon()->wDeltaAngle : 65535;
...
void CAIUnit::GetShootAreas(SShootAreas *pShootAreas, int *pnAreas) const
{
  ...
  *pnAreas = 1;
  ...
  // else
  {
    ...
    for (int i = 0; i < GetNGuns(); ++i)
    {
      ...
      )
      {
        ...
        if (fLocalFireRange > fMaxFireRange) fMaxFireRange = fLocalFireRange;

        const float fLocalMinFireRange = pGunI->GetWeapon()->fRangeMin;
        if (fLocalMinFireRange < fMinFireRange) fMinFireRange = fLocalMinFireRange;

        if (!pGun || !pGun->IsOnTurret() && pGunI->IsOnTurret()) pGun = pGunI;
        else if (pGun && !pGun->IsOnTurret() && !pGunI->IsOnTurret()) { if (pGun->GetWeapon()->wDeltaAngle < pGunI->GetWeapon()->wDeltaAngle) pGun = pGunI; }
        else if (pGun && pGun->IsOnTurret() && pGunI->IsOnTurret()) { if (pGun->GetHorTurnConstraint() < pGunI->GetHorTurnConstraint()) pGun = pGunI; }
      }
    }
    if (fMaxFireRange != -1.0f) InitializeShootArea(&(pShootAreas->areas.back()), pGun, fMinFireRange, fMaxFireRange);
  }
  ...
  if (GetStats()->type == RPG_TYPE_ART_AAGUN)
  {
    ...
    InitializeShootArea(&(pShootAreas->areas.back()), nullptr, 0.0f, GetMaxFireRange());
  }
}

bool CAIUnit::IsInTankPit() const { return IsValidObj(pTankPit); }
...
{
  if (IsVisibleByPlayer()) updater.Update(ACTION_NOTIFY_NEW_ST_OBJ, GetTankPit());
  else updater.Update(ACTION_NOTIFY_DELETED_ST_OBJ, GetTankPit());
}

void CAIUnit::SetOffTankPit()
{
  if (IsValidObj(pTankPit)) theStatObjs.DeleteInternalObjectInfo(pTankPit);
...
void CAIUnit::StartCamouflating() { camouflateTime = curTime; }

void CAIUnit::InitAviationPath()
{
  auto pStats = static_cast<const SMechUnitRPGStats *>(GetStats());
...
void CAIUnit::AnalyzeCamouflage()
{
  if (GetStats()->fCamouflage != 0)
  {
    if (!IsIdle())
    {
      ...
      if (IsCamoulflated()) RemoveCamouflage(ECRR_SELF_MOVE);
    }
    else
    {
      const BYTE cParty = theDipl.GetNParty(player);
      ...
      {
        camouflateTime = curTime;
        if (IsCamoulflated()) RemoveCamouflage(ECRR_GOOD_VISIBILITY);
      }
      else if (!IsCamoulflated() && curTime - camouflateTime >= GetTimeToCamouflage()) SetCamoulfage();
    }
  }
}
...
void CAIUnit::EnemyKilled(CAIUnit *pEnemy)
{
      ...
      theDipl.GetDiplStatusForParties(GetParty(), pEnemy->GetParty()) == EDI_ENEMY)
  {
    if (pEnemy->IsVisible(GetParty()))
    {
      ...
      if (::IsInfantry(eType)) SendAcknowledgement(ACK_KILLED_ENEMY_INFANTRY, true);
      else if (IsAviation(eType)) SendAcknowledgement(ACK_KILLED_ENEMY_AVIATION, true);
      else if (IsArmor(eType) || IsSPG(eType) || ::IsTrain(eType)) SendAcknowledgement(ACK_KILLED_ENEMY_TANK, true);
      else SendAcknowledgement(ACK_KILLED_ENEMY, true);
    }
    ...
    // change experience
    if (GetScenarioUnit() != nullptr)
    {
      if (nLevel < pExpLevels->levels.size() - 1)
      {
        fExperience += fPrice;
        if (nLevel == pExpLevels->levels.size() - 2) fExperience = Min(fExperience, static_cast<float>(pExpLevels->levels[nLevel + 1].nExp));
...
void CAIUnit::UpdateArea(const EActionNotify eAction) { updater.Update(eAction, this); }
  ...
  if (pStats && GetPlayer() == theDipl.GetMyNumber() && (bForce || pCommand && pCommand->IsValid() && !pCommand->IsFromAI()))
  {
    ...
    if (pStats->IsInfantry() && GetStats()->type == RPG_TYPE_ENGINEER)
    {
      ...
      if (pCurrentState && pCurrentState->GetName() == EUSN_GUN_CREW_STATE)
      {
        ...
        bSend = true;
        if (pArtillery->GetStats()->type == RPG_TYPE_ART_HEAVY_GUN || pArtillery->GetStats()->type == RPG_TYPE_ART_HOWITZER) nSet = 1;
      }
    }
    ...
    else if (ack == ACK_CANNOT_MOVE_NEED_TO_BE_TOWED_TO_MOVE) bSend = (pStats->IsArtillery() || pStats->IsInfantry());
    else bSend = true;

    if (bSend) theAckManager.AddAcknowledgment(ack, this, nSet);
  }
}

void CAIUnit::SendAcknowledgement(EUnitAckType ack, bool bForce) { SendAcknowledgement(GetCurCmd(), ack, bForce); }
...
bool CAIUnit::DoesExistRejectGunsReason(const EUnitAckType &ackType) const { return GetGuns()->DoesExistRejectReason(ackType); }

const int CAIUnit::GetMinArmor() const { return GetStats()->nMinArmor; }

const int CAIUnit::GetMaxArmor() const { return GetStats()->nMaxArmor; }

const int CAIUnit::GetMinPossibleArmor(const int nSide) const { return GetStats()->GetMinPossibleArmor(nSide); }

const int CAIUnit::GetMaxPossibleArmor(const int nSide) const { return GetStats()->GetMaxPossibleArmor(nSide); }

const int CAIUnit::GetArmor(const int nSide) const { return GetStats()->GetArmor(nSide); }

const int CAIUnit::GetRandomArmor(const int nSide) const { return GetStats()->GetRandomArmor(nSide); }

const float CAIUnit::GetCover() const { return IsInTankPit() ? SConsts::TANKPIT_COVER : 1.0f; }
...
void CAIUnit::SetNewCoordinates(const CVec3 &newCenter, bool bStopUnit) { pPathUnit->SetNewCoordinates(newCenter, bStopUnit); }
...
bool CAIUnit::TurnToDir(const WORD &newDir, const bool bCanBackward, const bool bForward)
{
  ...
  if (GetVisionAngle() != 32768) WarFogChanged();
...
void CAIUnit::FirstSegment() { pPathUnit->FirstSegment(); }
...
void CAIUnit::UnRegisterAsBored(const enum EUnitAckType eBoredType) { theAckManager.UnRegisterAsBored(eBoredType, this); }

void CAIUnit::RegisterAsBored(const enum EUnitAckType eBoredType) { theAckManager.RegisterAsBored(eBoredType, this); }

bool CAIUnit::IsInOneTrain(IBasePathUnit *pUnit) const { return pPathUnit->IsInOneTrain(pUnit); }

bool CAIUnit::IsTrain() const { return GetStats()->IsTrain(); }

const float CAIUnit::GetTimeToForget() const { return SGeneralConsts::TIME_TO_FORGET_UNIT; }

CAIUnitInfoForGeneral *CAIUnit::GetUnitInfoForGeneral() const { return pUnitInfoForGeneral; }

void CAIUnit::SetLastVisibleTime(const NTimer::STime time) { pUnitInfoForGeneral->SetLastVisibleTime(time); }
...
bool CAIUnit::CanRotate() const { return pPathUnit->CanRotate(); }

float CAIUnit::GetPriceMax() const { return GetStats()->fPrice; }

const SAIExpLevel::SLevel &CAIUnit::GetExpLevel() const { return pExpLevels->levels[nLevel]; }
...
void CAIUnit::UnitCommand(CAICommand *pCommand, bool bPlaceInQueue, bool bOnlyThisUnitCommand)
{
  if (!bOnlyThisUnitCommand)
  {
    ...
    if (GetStats()->IsInfantry()) UnlockTiles();
  }

  CCommonUnit::UnitCommand(pCommand, bPlaceInQueue, bOnlyThisUnitCommand);
}

void CAIUnit::Lock(const CBasicGun *pGun) { if (!GetStats()->IsTrain()) CCommonUnit::Lock(pGun); }

void CAIUnit::Unlock(const CBasicGun *pGun) { if (!GetStats()->IsTrain()) CCommonUnit::Unlock(pGun); }

bool CAIUnit::IsLocked(const CBasicGun *pGun) const
{
  if (GetStats()->IsTrain()) return true;
  return CCommonUnit::IsLocked(pGun);
}

void CAIUnit::SetActiveShellType(const enum SWeaponRPGStats::SShell::EDamageType eShellType) { if (GetGuns()->SetActiveShellType(eShellType)) updater.Update(ACTION_NOTIFY_SHELLTYPE_CHANGED, this, eShellType); }

void CAIUnit::AnimationSet(int nAnimation) { pAnimUnit->AnimationSet(nAnimation); }

void CAIUnit::AnimationSegment() { pAnimUnit->Segment(); }

void CAIUnit::Moved() { pAnimUnit->Moved(); }

void CAIUnit::Stopped() { pAnimUnit->Stopped(); }

void CAIUnit::StopCurAnimation() { pAnimUnit->StopCurAnimation(); }

void CAIUnit::TrackDamagedState(const bool bTrackDamaged) { pPathUnit->TrackDamagedState(bTrackDamaged); }

bool CAIUnit::IsColliding() const { return true; }
...
bool CAIUnit::IsRevealed() const { return bRevealed; }

bool CAIUnit::IsInfantry() const { return GetStats()->IsInfantry(); }

const NTimer::STime CAIUnit::GetNextSecondPathSegmTime() const { return pPathUnit->GetNextSecondPathSegmTime(); }
