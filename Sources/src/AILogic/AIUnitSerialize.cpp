...
int CAIUnit::operator&(IStructureSaver &ss)
{
  ...
  // to download old saves
  if (saver.IsReading()) { if (pExpLevels == nullptr) pExpLevels = checked_cast<const SAIExpLevel *>(GetSingleton<IObjectsDB>()->GetExpLevels(RPG_TYPE_INFANTRY)); }
  ...
  if (visible4Party.empty()) visible4Party.resize(3);
...
int CSoldier::operator&(IStructureSaver &ss)
{
  ...
  saver.Add(3, &pStats);
  saver.Add(4, &pGuns);
  if (!saver.IsReading()) { if (IsFree()) pObjInside = nullptr; }
...
int CPlanesFormation::operator&(IStructureSaver &ss)
{
  ...
  if (saver.IsReading() && pPath) { pPath->SetAviationUnit(this, this); }
  return 0;
}
...
int CArtillery::operator&(IStructureSaver &ss)
{
  ...
  int cnt = -1;
  saver.Add(29, &cnt);
  if (saver.IsReading() && cnt == 0) { behUpdateDuration = SConsts::LONG_RANGE_ARTILLERY_UPDATE_DURATION; }
...
int CCommonUnit::operator&(IStructureSaver &ss)
{
  ...
  if (saver.IsReading())
  {
    ...
    // legacy save
    if (dbID == -1) saver.Add(6, &dbID);
  }
  else SaveDBID(&saver, 21, dbID);
