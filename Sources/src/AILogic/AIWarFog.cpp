                   ...
                   const struct SVector &downRight, const struct SVector &upRight)
{
  ...
  if (vis.size() < sizeSum * sizeDiff + 1) vis.resize(sizeSum * sizeDiff + 1);
...
{
  if (IsPointInside(center)) isInside = true;
  else
  {
    isInside = false;
...
{
  if (fogInfo.r <= 2) return;

  // the center and r variables are used in IsInfluencedUnit
  ...
  if (IsInfluencedUnit(center) && fogInfo.wVisionAngle > 0)
  {
    ...
    if (fogInfo.pObject.IsValid()) fogInfo.pObject->RemoveTransparencies();
    ...
    if (fogInfo.pObject.IsValid()) fogInfo.pObject->SetTransparencies();
  }
}
...
void CGlobalWarFog::Init()
{
  ...
  for (int i = 0; i < fogCnts.size(); ++i)
  {
    ...
    for (int k = 0; k < minCoeff2[i].GetSizeY(); ++k) { for (int j = 0; j < minCoeff2[i].GetSizeX(); ++j) minCoeff2[i][k][j] = floatToByte(1.0f); }
  }
...
void CGlobalWarFog::DeleteUnit(const int id)
{
  ...
  if (unitsInfo[id].nHeapPos != -2)
  {
    ...
    if (unitsInfo[id].nHeapPos >= 0) weights.Erase(unitsInfo[id].nHeapPos);
    unitsInfo[id].nHeapPos = -1;

    removedObjects4Units[id].clear();
    addedObjects4Units[id].clear();
  }
  // new, fog not yet calculated
  else
  {
    auto iter = newUnits.begin();
    while (iter != newUnits.end() && (*iter) != id) ++iter;
...
{
  // new, fog has not yet been considered
  if (unitsInfo[id].nHeapPos == -2) unitsInfo[id].fogInfo = newFogInfo;
  else UpdateUnit(id, newFogInfo, SquareOfDistance(unitsInfo[id].fogInfo.center, newFogInfo.center));
}
...
void CGlobalWarFog::ChangeUnitState(const int id, SFogInfo &newFogInfo)
{
  // new, fog has not yet been considered
  if (unitsInfo[id].nHeapPos == -2) unitsInfo[id].fogInfo = newFogInfo;
  else UpdateUnit(id, newFogInfo, GetWeight(unitsInfo[id].fogInfo, newFogInfo));
}

void CGlobalWarFog::ChangeUnitParty(const int id, const int nParty)
{
  // new, fog has not yet been considered
  if (unitsInfo[id].nHeapPos == -2) unitsInfo[id].nParty = nParty;
  else
  {
...
{
  // if the fog for this unit is already calculated
  if (unitsInfo[id].nHeapPos != -2)
  {
    UpdateUnit(id, unitsInfo[id].fogInfo, Max(1.0f, fDist / (SConsts::TILE_SIZE * 5.0f)));

    auto iter(std::find(addedObjects4Units[id].begin(), addedObjects4Units[id].end(), pObject));
    if (iter == addedObjects4Units[id].end()) removedObjects4Units[id].push_back(pObject);
    else addedObjects4Units[id].erase(iter);
  }
}
...
void CGlobalWarFog::RecalculateForAddedObject(const int id, const float fDist, CExistingObject *pObject)
{
  ...
  if (unitsInfo[id].nHeapPos != -2)
  {
    ...
    auto iter(std::find(removedObjects4Units[id].begin(), removedObjects4Units[id].end(), pObject));
    if (iter == removedObjects4Units[id].end()) addedObjects4Units[id].push_back(pObject);
    else removedObjects4Units[id].erase(iter);
  }
}
...
void CGlobalWarFog::ReclaculateFogAfterRemoveObject(CExistingObject *pObj)
{
  ...
  for (CUnitsIter<0, 3> unitsIter(0, ANY_PARTY, pObj->GetCenter(), SConsts::MAX_DIST_TO_RECALC_FOG); !unitsIter.IsFinished(); unitsIter.Iterate())
  {
    ...
    if (IsValidObj(pUnit)) RecalculateForRemovedObject(pUnit->GetID(), fabs(pUnit->GetCenter() - objCenter), pObj);
  }

  theGraveyard.UpdateFog4RemovedObject(pObj);

  for (auto iter = deletedUnits.begin(); iter != deletedUnits.end(); ++iter) iter->removedObjects.push_back(pObj);
}
...
void CGlobalWarFog::ReclaculateFogAfterAddObject(CExistingObject *pObj)
{
  ...
  {
    CAIUnit *pUnit = *unitsIter;
    if (IsValidObj(pUnit)) RecalculateForAddedObject(pUnit->GetID(), fabs(pUnit->GetCenter() - vCenter), pObj);
  }

  theGraveyard.UpdateFog4AddedObject(pObj);

  for (auto iter = deletedUnits.begin(); iter != deletedUnits.end(); ++iter) iter->addedObjects.push_back(pObj);
}

bool operator <(const SVector &a, const SVector &b) { return a.x < b.x || a.x == b.x && a.y < b.y; }

void CGlobalWarFog::RemoveUnitWarfog(SUnitInfo &unitInfo, const CObjectsList &removedObjects, const CObjectsList &addedObjects)
{
  // remove transparency for those objects that were placed between fog calculations for this unit
  for (auto iter = addedObjects.begin(); iter != addedObjects.end(); ++iter) (*iter)->RemoveTransparencies();

  // set transparency for those objects that were removed between fog calculations for this unit
  for (auto iter = removedObjects.begin(); iter != removedObjects.end(); ++iter) { if (IsValidObj(*iter)) (*iter)->SetTransparencies(); }
  ...
  for (auto iter = removedObjects.begin(); iter != removedObjects.end(); ++iter) (*iter)->RemoveTransparencies();

  for (auto iter = addedObjects.begin(); iter != addedObjects.end(); ++iter) (*iter)->SetTransparencies();
}
...
void CGlobalWarFog::ProcessNewUnits(bool bAllUnits)
{
  ...
  while ((bAllUnits || cnt < 20) && !newUnits.empty())
  {
    ...
    if (unitsInfo[id].nParty != theDipl.GetNeutralParty()) AddUnitWarfog(unitsInfo[id]);
...
void CGlobalWarFog::Segment(bool bAllUnits)
{
  ...
  while ((cnt < nCalculatedUnits || bAllUnits) && !weights.IsEmpty())
  {
    ...
    if (unitsInfo[id].nParty != theDipl.GetNeutralParty()) RemoveUnitWarfog(unitsInfo[id], removedObjects4Units[id], addedObjects4Units[id]);
    ...
    if (!weights.IsEmpty())
    {
      if (weights.GetMaxEl().nWeight > 410) nCalculatedUnits = Min(nCalculatedUnits + 1, 4);
      else if (weights.GetMaxEl().nWeight > 220) nCalculatedUnits = Min(nCalculatedUnits + 1, 3);
      else if (weights.GetMaxEl().nWeight > 121) nCalculatedUnits = Min(nCalculatedUnits + 1, 2);
    }
  }
...
{
  if (theCheats.GetTurnOffWarFog()) return true;
  if (!theStaticMap.IsTileInside(tile) || theDipl.GetNeutralParty() == nParty) return false;
  if (!theDipl.IsNetGame() && IsOpenBySriptArea(tile) && theDipl.GetMyParty() == nParty) return true;
...
{
  if (!theDipl.IsNetGame() && IsOpenBySriptArea(tileX, tileY) && theDipl.GetMyParty() == nParty) return SConsts::VIS_POWER;
  if (theCheats.GetTurnOffWarFog()) return SConsts::VIS_POWER;
  if (!theStaticMap.IsTileInside(tileX, tileY) || theDipl.GetNeutralParty() == nParty) return 0;
...
{
  if (theStaticMap.IsTileInside(tileX, tileY) && areasOpenTiles.GetData(tileX, tileY) > 0) return SConsts::VIS_POWER;
  if (!theCheats.IsHistoryPlaying()) return GetTileVis(tileX, tileY, nParty);
...
{
  int nShift;
  if (bFirstTime) nShift = theStaticMap.GetSizeY() / 2;
  else nShift = theStaticMap.GetSizeY() / 2 / (1000 / SConsts::AI_SEGMENT_DURATION) + 1;
  ...
  *pVisBuffer = GetTempBuffer<BYTE>(*pnLen);
  ...
  nMiniMapY = nUpY;
  if (nMiniMapY == theStaticMap.GetSizeY() / 2) nMiniMapY = 0;
...
{
  if (theCheats.GetTurnOffWarFog()) return true;
  if (!IsTileVisible(tile, nParty)) return false;
  if (!bCamouflated) return true;

  if (!theDipl.IsNetGame() && IsOpenBySriptArea(tile) && theDipl.GetMyParty() == nParty) return true;

  return GetMinCoeff2(tile, nParty) <= sqr(fCamouflage * static_cast<float>(GetTileVis(tile, nParty)) / static_cast<float>(SConsts::VIS_POWER));
}
...
void CGlobalWarFog::ToggleOpen4ScriptAreaTiles(const SScriptArea &scriptArea, bool bOpen)
{
  ...
  {
    if (bOpen) areas.insert(scriptArea.szName);
    else areas.erase(scriptArea.szName);
...
bool CGlobalWarFog::IsOpenBySriptArea(const int x, const int y) const { return theStaticMap.IsTileInside(x, y) && areasOpenTiles.GetData(x, y) > 0; }