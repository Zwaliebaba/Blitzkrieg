...
CAntiArtillery::CAntiArtillery(CAIUnit *pOwner) { SetUniqueId(); }
...
{
  if (curTime - lastScan >= SConsts::ANTI_ARTILLERY_SCAN_TIME) Scan(center);
  ...
  for (int i = 0; i < 2; ++i)
  {
    ...
    {
      if (fabs2(center - lastHeardPos[i]) > sqr(SConsts::RELOCATION_RADIUS) || i == 0 && curTime - lastShotTime[i] > SConsts::AUDIBILITY_TIME) nHeardShots[i] = 0;

      CVec2 newCenter;
      const float fCurRadius = GetRadius(Min(static_cast<float>(nHeardShots[i]), static_cast<float>(SConsts::SHOTS_TO_MINIMIZE_LOCATION_RADIUS)), fGunRadius);

      if (nHeardShots[i] > SConsts::SHOTS_TO_MINIMIZE_LOCATION_RADIUS) newCenter = lastRevealCenter[i];
      else
      {
        ...
        else
        {
          ...
          // CRAP { no time to look for a bug
          // NI_ASSERT_T( minX <= maxX, "Wrong box" );
          else if (minX > maxX) newCenter = center;
          // CRAP}
          else
          {
            int cnt = 0;
      ...
      if (nHeardShots[i] <= SConsts::SHOTS_TO_MINIMIZE_LOCATION_RADIUS) ++nHeardShots[i];
...
const CCircle CAntiArtillery::GetRevealCircle(const int nParty) const
{
  return CCircle(lastRevealCenter[nParty],
                 GetRadius(Min(static_cast<float>(nHeardShots[nParty]), static_cast<float>(SConsts::SHOTS_TO_MINIMIZE_LOCATION_RADIUS)), fMaxRadius));
}

const NTimer::STime CAntiArtillery::GetLastHeardTime(const int nParty) const { return lastShotTime[nParty]; }
...
void CAntiArtillery::Segment(bool bOwnerVisible)
{
  ...
  {
    // if player is an enemy, heard a shot not too long ago, and it's time to draw a circle
    ...
    if (bEnemy && bHaveToSendCircle)
    {
      ...
      // must be created so that the general numbering of objects does not depend on the client
      CPtr<CRevealCircle> pCircle = new CRevealCircle(GetRevealCircle(1 - nParty));

      // this is our side and we are not visible to him,
      if (nMyParty == nIterParty && !bOwnerVisible) { if (!theCheats.IsHistoryPlaying()) updater.Update(ACTION_NOTIFY_REVEAL_ARTILLERY, pCircle); }
    }
  }
}