...
CPtr<CArtRocketStatesFactory> CArtRocketStatesFactory::pFactory = nullptr;

IStatesFactory *CArtRocketStatesFactory::Instance()
{
  if (pFactory == nullptr) pFactory = new CArtRocketStatesFactory();
...
IUnitState *CArtRocketStatesFactory::ProduceState(class CQueueUnit *pObj, CAICommand *pCommand)
{
  ...
  switch (cmd.cmdType)
  {
    ...
    case ACTION_COMMAND_MOVE_TO:
    {
      ...
      if (pArtillery->IsInTankPit())// first exit TankPit, then go where you were sent
      {
        theGroupLogic.InsertUnitCommand(pCommand->ToUnitCmd(), pArtillery);
        pResult = CTankPitLeaveState::Instance(pArtillery);
      }
      else pResult = CArtilleryMoveToState::Instance(pArtillery, cmd.vPos);
    }
    ...
    case ACTION_COMMAND_ATTACK_OBJECT:
    {
      ...
      if (pStaticObj->GetObjectType() == ESOT_ARTILLERY_BULLET_STORAGE) { pResult = CArtRocketAttackGroundState::Instance(pArtillery, static_cast<CArtilleryBulletStorage *>(pStaticObj)->GetOwner()->GetCenter()); }
...
IUnitState *CArtRocketAttackGroundState::Instance(CArtillery *pArtillery, const CVec2 &point) { return new CArtRocketAttackGroundState(pArtillery, point); }

CArtRocketAttackGroundState::CArtRocketAttackGroundState(CArtillery *_pArtillery, const CVec2 &_point)
  : eState(EAGS_FIRING), pArtillery(_pArtillery), point(_point), bFired(false), bFinished(false), wDirToRotate(0)
{
  pArtillery->StopUnit();
...
void CArtRocketAttackGroundState::Segment()
{
  ...
  if (bFinished) pArtillery->SetCommandFinished();
  else
  {
    switch (eState)
    {
      ...
      case EAGS_FIRING:
        if (pArtillery->IsInstalled())
        {
          if (!pArtillery->GetFirstArtilleryGun()->IsFiring())
          {
            if (bFired)
            {
              ...
              for (int i = 0; i < nGuns; ++i)
              {
                ...
                if (pTurret == pGun->GetTurret()) pGun->StopFire();
              }
          ...
          else if (pArtillery->GetFirstArtilleryGun()->GetNAmmo() == 0) pArtillery->SendAcknowledgement(ACK_NO_AMMO);
        }

        break;
      default: NI_ASSERT_T(false, NStr::Format( "Wrong CArtRocketAttackGroundState (%d)", static_cast<int>(eState) ));
    }
  }
}
...
{
  if (pCommand == nullptr || !pArtillery->GetFirstArtilleryGun()->IsBursting())
  {
