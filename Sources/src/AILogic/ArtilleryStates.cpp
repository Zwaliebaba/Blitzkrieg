...
CPtr<CArtilleryStatesFactory> CArtilleryStatesFactory::pFactory = nullptr;

IStatesFactory *CArtilleryStatesFactory::Instance()
{
  if (pFactory == nullptr) pFactory = new CArtilleryStatesFactory();
...
IUnitState *CArtilleryStatesFactory::ProduceState(class CQueueUnit *pObj, CAICommand *pCommand)
{
  ...
  auto pArtillery = static_cast<CArtillery *>(pObj);
  ...
  switch (cmd.cmdType)
  {
    ...
    case ACTION_COMMAND_CHANGE_SHELLTYPE:
      pArtillery->SetActiveShellType(static_cast<SWeaponRPGStats::SShell::EDamageType>(static_cast<int>(cmd.fNumber)));
    ...
    case ACTION_COMMAND_MOVE_TO:
      if (pArtillery->IsInTankPit() && !pCommand->IsFromAI())// first exit TankPit, then go where you were sent
      {
    ...
    case ACTION_COMMAND_ATTACK_UNIT:
      ...
      {
        if (!cmd.bFromAI) pArtillery->SetActiveShellType(SWeaponRPGStats::SShell::DAMAGE_HEALTH);
        ...
        if (pTarget->IsAlive())
        {
          ...
          {
            if (pTarget->GetStats()->IsInfantry() && static_cast<CSoldier *>(pTarget)->IsInBuilding()) pResult = CSoldierAttackUnitInBuildingState::Instance(pArtillery, static_cast<CSoldier *>(pTarget), cmd.fNumber == 0, bSwarmAttack);
            else pResult = CMechAttackUnitState::Instance(pArtillery, static_cast_ptr<CAIUnit *>(cmd.pObject), cmd.fNumber == 0, bSwarmAttack);
          }
          else if (pTarget->GetStats()->IsAviation()) pResult = CArtilleryAttackAviationState::Instance(pArtillery, static_cast<CAviation *>(cmd.pObject.GetPtr()));
          else pResult = CArtilleryAttackState::Instance(pArtillery, static_cast<CAIUnit *>(cmd.pObject.GetPtr()), cmd.fNumber == 0, bSwarmAttack);
        }
      }
    ...
    case ACTION_COMMAND_ATTACK_OBJECT:
    {
      ...
      else if (pArtillery->GetStats()->nUninstallRotate == 0 && pArtillery->GetStats()->nUninstallTransport == 0) pResult = CSoldierAttackCommonStatObjState::Instance(pArtillery, pStaticObj, bSwarmAttack);
      else pResult = CArtilleryAttackCommonStatObjState::Instance(pArtillery, pStaticObj);
    }

    break;
    case ACTION_COMMAND_ROTATE_TO:
      if (pArtillery->IsInTankPit() && !pCommand->IsFromAI() && pArtillery->GetStats()->type != RPG_TYPE_ART_AAGUN)// first exit TankPit, then go where you were sent
      {
        theGroupLogic.InsertUnitCommand(pCommand->ToUnitCmd(), pArtillery);
        pResult = CTankPitLeaveState::Instance(pArtillery);
      }
      else pResult = CArtilleryTurnToPointState::Instance(pArtillery, cmd.vPos);
    ...
    case ACTION_COMMAND_ART_BOMBARDMENT:
      if (pArtillery->GetFirstArtilleryGun() != nullptr) pResult = CArtilleryBombardmentState::Instance(pArtillery, cmd.vPos);
      else pArtillery->SendAcknowledgement(pCommand, ACK_NEGATIVE, !pCommand->IsFromAI());

      break;
    case ACTION_COMMAND_RANGE_AREA:
      if (pArtillery->GetFirstArtilleryGun() != nullptr) pResult = CArtilleryRangeAreaState::Instance(pArtillery, cmd.vPos);
      else pArtillery->SendAcknowledgement(pCommand, ACK_NEGATIVE, !pCommand->IsFromAI());
    ...
    case ACTION_COMMAND_SWARM_TO:
      // first exit TankPit, then go where you were sent
      if (pArtillery->IsInTankPit() && !pCommand->IsFromAI())
      {
        theGroupLogic.InsertUnitCommand(pCommand->ToUnitCmd(), pArtillery);
        pResult = CTankPitLeaveState::Instance(pArtillery);
      }
      else if (!pArtillery->CanMove()) pArtillery->SendAcknowledgement(pCommand, ACK_CANNOT_MOVE_NEED_TO_BE_TOWED_TO_MOVE, !pCommand->IsFromAI());
      else pResult = CCommonSwarmState::Instance(pArtillery, cmd.vPos, cmd.fNumber);
...
IUnitState *CArtilleryMoveToState::Instance(CArtillery *pArtillery, const CVec2 &point) { return new CArtilleryMoveToState(pArtillery, point); }

CArtilleryMoveToState::CArtilleryMoveToState(CArtillery *_pArtillery, const CVec2 &_point)
  : eState(EAMTS_WAIT_FOR_PATH), bToFinish(false), pArtillery(_pArtillery), startTime(curTime)
{
  ...
  else
  {
    switch (eState)
    {
      case EAMTS_WAIT_FOR_PATH:
        if (curTime - startTime >= TIME_OF_WAITING)
        {
          ...
          if (pStaticPath)
          {
            ...
            else eState = EAMTS_START_MOVING;
          }
          else
          {
      ...
      case EAMTS_UNINSTALLING:
        ...
        {
          if (bToFinish) pArtillery->SetCommandFinished();
          else eState = EAMTS_START_MOVING;
        }
      ...
      {
        if (!pStaticPath) pArtillery->SetCommandFinished();
        else
        {
      ...
      break;
      case EAMTS_MOVING:
        if (pArtillery->IsOperable()) { if (pArtillery->IsIdle()) pArtillery->SetCommandFinished(); }
        else if (!pArtillery->HasServeCrew()) pArtillery->SetCommandFinished();
...
IUnitState *CArtilleryTurnToPointState::Instance(CArtillery *pArtillery, const CVec2 &targCenter) { return new CArtilleryTurnToPointState(pArtillery, targCenter); }

CArtilleryTurnToPointState::CArtilleryTurnToPointState(CArtillery *_pArtillery, const CVec2 &_targCenter)
  : eState(EATRS_ESTIMATING), pArtillery(_pArtillery), lastCheck(0), targCenter(_targCenter) { pArtillery->StopUnit(); }
  ...
  if (pArtillery->IsOperable())
  {
    switch (eState)
    {
      case EATRS_ESTIMATING:
        ...
        {
          for (int i = 0; i < pArtillery->GetNTurrets(); ++i) pArtillery->GetTurret(i)->TurnHor(GetDirectionByVector(targCenter - pArtillery->GetCenter()) - pArtillery->GetFrontDir());
        ...
        else
        {
          ...
          else
          {
            ...
            if (pArtillery->IsInstalled()) pArtillery->InstallAction(ACTION_NOTIFY_UNINSTALL_ROTATE);
          }
        }
      ...
      case EATPS_TURNING:
        ...
        {
          int i = 0;
          while (i < pArtillery->GetNTurrets() && pArtillery->GetTurret(i)->IsFinished()) ++i;

          if (i >= pArtillery->GetNTurrets()) pArtillery->SetCommandFinished();
        }
        else if (pArtillery->TurnToUnit(targCenter)) pArtillery->SetCommandFinished();
...
{
  if (pArtillery->GetStats()->type == RPG_TYPE_ART_AAGUN) { for (int i = 0; i < pArtillery->GetNTurrets(); ++i) pArtillery->GetTurret(i)->StopTurning(); }

  pArtillery->SetCommandFinished();
  return TSIR_YES_IMMIDIATELY;
}

const CVec2 CArtilleryTurnToPointState::GetPurposePoint() const { return pArtillery->GetCenter(); }

// **********************************************************************
// *CArtilleryBombardmentState*
// **********************************************************************

IUnitState *CArtilleryBombardmentState::Instance(CAIUnit *pUnit, const CVec2 &point) { return new CArtilleryBombardmentState(pUnit, point); }

CArtilleryBombardmentState::CArtilleryBombardmentState(CAIUnit *_pUnit, const CVec2 &_point)
  : eState(EABS_START), pUnit(_pUnit), point(_point), bStop(false) {}
  ...
  if (pUnit->IsOperable())
  {
    switch (eState)
    {
      case EABS_START:
      {
        ...
        if (bCanShootWOMove)
        {
          if (!bNeedTurn) eState = EABS_FIRING;
          else
          {
      ...
      break;
      case EABS_TURNING:
        if (pUnit->TurnToUnit(point)) eState = EABS_FIRING;
      ...
      case EABS_FIRING:
        ...
        {
          if (!pUnit->GetFirstArtilleryGun()->IsFiring()) pUnit->GetFirstArtilleryGun()->StartPointBurst(point, false);
          else if (pUnit->GetFirstArtilleryGun()->GetNAmmo() == 0) pUnit->SendAcknowledgement(ACK_NO_AMMO);
        }
...
{
  if (pCommand == nullptr || !pUnit->GetFirstArtilleryGun()->IsBursting())
  {
...
IUnitState *CArtilleryRangeAreaState::Instance(CAIUnit *pUnit, const CVec2 &point) { return new CArtilleryRangeAreaState(pUnit, point); }

CArtilleryRangeAreaState::CArtilleryRangeAreaState(CAIUnit *_pUnit, const CVec2 &_point)
  : eState(ERAS_RANGING), pUnit(_pUnit), pGun(_pUnit->GetFirstArtilleryGun()), point(_point),
    nShootsLast(SConsts::SHOOTS_TO_RANGE), bFinish(false),
    bFired(true)
{
  pUnit->StopUnit();
  ...
  if (bCanShootWOMove)
  {
    if (!bNeedTurn) eState = ERAS_RANGING;
    else { eState = ERAS_TURNING; }
  }
  else
  {
...
{
  // by units
  pUnit->ResetShootEstimator(nullptr, false);
  ...
  {
    for (CStObjCircleIter<true> iter(point, SConsts::RANGED_AREA_RADIUS); !iter.IsFinished() && pObj == nullptr; iter.Iterate())
    {
      ...
      if (pIteratingObject->GetObjectType() == ESOT_BUILDING && theDipl.GetDiplStatus(pIteratingObject->GetPlayer(), pUnit->GetPlayer()) == EDI_ENEMY &&
          ...
          fabs2(pIteratingObject->GetAttackCenter(point) - point) <= sqr(SConsts::RANGED_AREA_RADIUS) && pGun->CanShootToObjectWOMove(pIteratingObject)) { pObj = pIteratingObject; }
    }
...
void CArtilleryRangeAreaState::Segment()
{
  if (pUnit->IsOperable() && (eState == ERAS_TURNING || pUnit->CanShoot()))
  {
      ...
      else
      {
        switch (eState)
        {
          case ERAS_TURNING:
            ...
            if (pUnit->TurnToUnit(point)) eState = ERAS_RANGING;

            break;
          case ERAS_RANGING:
            pGun->StartPointBurst(point, nShootsLast == SConsts::SHOOTS_TO_RANGE);
            if (bFired) --nShootsLast;
            bFired = false;
            ...
            break;
          case ERAS_WAITING:
            if (curTime - lastCheck >= CHECK_TIME) CheckArea();
          ...
          case ERAS_SHOOT_OBJECT:
            if (IsValidObj(pObj) && theDipl.GetDiplStatus(pObj->GetPlayer(), pUnit->GetPlayer()) == EDI_ENEMY) pGun->StartPointBurst(pObj->GetAttackCenter(pUnit->GetCenter()), false);
            else
            {
              eState = ERAS_WAITING;
              lastCheck = curTime - CHECK_TIME;
    ...
    {
      if (pGun->GetNAmmo() == 0) pUnit->SendAcknowledgement(ACK_NO_AMMO);
...
bool CArtilleryRangeAreaState::IsAttacksUnit() const { return eState == ERAS_SHOOT_UNIT; }

CAIUnit *CArtilleryRangeAreaState::GetTargetUnit() const
{
  if (IsAttacksUnit()) return pEnemy;
  return nullptr;
}
...
IUnitState *CArtilleryInstallTransportState::Instance(CArtillery *pArtillery) { return new CArtilleryInstallTransportState(pArtillery); }

CArtilleryInstallTransportState::CArtilleryInstallTransportState(CArtillery *_pArtillery)
  : eState(AITS_WAITING_FOR_CREW), pArtillery(_pArtillery) {}
...
void CArtilleryInstallTransportState::Segment()
{
  ...
  switch (eState)
  {
    ...
    case AITS_INSTALLING:
      if (pArtillery->IsInstalled()) pArtillery->SetCommandFinished();
      break;
  }
}
...
const CVec2 CArtilleryInstallTransportState::GetPurposePoint() const { return pArtillery->GetCenter(); }

// **********************************************************************
// * CArtilleryUninstallTransportState *
// **********************************************************************

IUnitState *CArtilleryUninstallTransportState::Instance(CArtillery *pArtillery) { return new CArtilleryUninstallTransportState(pArtillery); }

CArtilleryUninstallTransportState::CArtilleryUninstallTransportState(CArtillery *_pArtillery)
  : pArtillery(_pArtillery)
{
  if (pArtillery->IsUninstalled() && pArtillery->GetCurUninstallAction() == ACTION_NOTIFY_UNINSTALL_TRANSPORT) pArtillery->SetCommandFinished();
  else if (!pArtillery->IsInstalled()) eState = AUTS_WAIT_FOR_UNINSTALL;
  else eState = AUTS_INSTALLING;
}
...
void CArtilleryUninstallTransportState::Segment()
{
  switch (eState)
  {
    case AUTS_WAIT_FOR_UNINSTALL:
      // wait until the gun is uninstalled
      // (in case the attackers uninstalled it at the time of hooking it up)
      if (pArtillery->IsUninstalled())
      {
        if (pArtillery->GetCurUninstallAction() == ACTION_NOTIFY_UNINSTALL_TRANSPORT) eState = AUTS_WAIT_FOR_UNINSTALL_TRANSPORT;
        else
        {
    ...
    // break was removed deliberately
    ...
    case AUTS_WAIT_FOR_UNINSTALL_TRANSPORT:
      if (pArtillery->IsUninstalled()) { pArtillery->SetCommandFinished(); }
  }
}

ETryStateInterruptResult CArtilleryUninstallTransportState::TryInterruptState(class CAICommand *pCommand)
{
  if (!pCommand) { pArtillery->SetCommandFinished(); }
  else if (pCommand->ToUnitCmd().cmdType == ACTION_MOVE_BEING_TOWED) { return TSIR_YES_WAIT; }
  return TSIR_NO_COMMAND_INCOMPATIBLE;// this state ends on its own
}

const CVec2 CArtilleryUninstallTransportState::GetPurposePoint() const { return pArtillery->GetCenter(); }

// **********************************************************************
// *CArtilleryBeingTowedState*
// **********************************************************************

IUnitState *CArtilleryBeingTowedState::Instance(class CArtillery *pArtillery, class CAITransportUnit *pTransport) { return new CArtilleryBeingTowedState(pArtillery, pTransport); }
...
void CArtilleryBeingTowedState::Segment()
{
  ...
  // the tug's position has changed
  else if (wLastTagDir != pTransport->GetFrontDir() || vLastTagCenter != pTransport->GetCenter())
  {
    ...
    // if the start of movement and the tractor did not hit the artillery attachment point with the attachment point (the point is closer than necessary)
    ...
    if (newDirVec * tagUnitDir < 0)
    {
      ...
      if (perpVec * newDirVec >= 0) newDirVec = perpVec;
      else newDirVec = CVec2(tagUnitDir.y, -tagUnitDir.x);
    }
  ...
  else pPath->Init(0.0f, pArtillery->GetCenter(), VNULL2);
...
ETryStateInterruptResult CArtilleryBeingTowedState::TryInterruptState(CAICommand *pCommand)
{
  if (pCommand == nullptr)
  {
    if (IsValidObj(pTransport)) pArtillery->SetSelectable(false);
...
const CVec2 CArtilleryBeingTowedState::GetPurposePoint() const { return pArtillery->GetCenter(); }

CAITransportUnit *CArtilleryBeingTowedState::GetTowingTransport() const { return pTransport; }

// **********************************************************************
// *CArtilleryAttackState*
// **********************************************************************

IUnitState *CArtilleryAttackState::Instance(CArtillery *pArtillery, CAIUnit *pEnemy, bool bAim, const bool bSwarmAttack) { return new CArtilleryAttackState(pArtillery, pEnemy, bAim, bSwarmAttack); }

CArtilleryAttackState::CArtilleryAttackState(CArtillery *_pArtillery, CAIUnit *_pEnemy, bool _bAim, bool _bSwarmAttack)
  : CFreeFireManager(_pArtillery), eState(EAS_NONE), pArtillery(_pArtillery), pEnemy(_pEnemy), bAim(_bAim),
...
{
  for (int i = 0; i < pArtillery->GetNGuns(); ++i) pArtillery->GetGun(i)->StopFire();
...
void CArtilleryAttackState::Segment()
{
  if (pArtillery->IsOperable())
  {
    ...
    else if (pArtillery->IsInstalled() || eState == EAS_NONE || eState == EAS_ROTATING)
    {
      ...
      if (bSwarmAttack) pArtillery->AnalyzeTargetScan(pEnemy, damageToEnemyUpdater.IsDamageUpdated(), false);
      ...
      switch (eState)
      {
        case EAS_NONE:
          ...
          else
          {
            ...
            if (pGun->GetTurret()) pGun->GetTurret()->Lock(pGun);
          ...
          break;
        case EAS_ROTATING:
          if (pArtillery->TurnToDir(wDirToRotate, false)) eState = EAS_NONE;
        ...
        case EAS_FIRING:
          if (pArtillery->IsInstalled())
          {
            ...
            else if (pGun->GetNAmmo() == 0) pArtillery->SendAcknowledgement(ACK_NO_AMMO);
          }
          else if (!pArtillery->IsInInstallAction()) pArtillery->ForceInstallAction();
      }
    }
  }
  else if (!pArtillery->HasServeCrew()) FinishState();
}
...
CAIUnit *CArtilleryAttackState::GetTargetUnit() const { return pEnemy; }

// **********************************************************************
// * CArtilleryAttackCommonStatObjState *
// **********************************************************************

IUnitState *CArtilleryAttackCommonStatObjState::Instance(CArtillery *pArtillery, CStaticObject *pObj) { return new CArtilleryAttackCommonStatObjState(pArtillery, pObj); }

CArtilleryAttackCommonStatObjState::CArtilleryAttackCommonStatObjState(CArtillery *_pArtillery, CStaticObject *_pObj)
  : CFreeFireManager(_pArtillery), pArtillery(_pArtillery), pObj(_pObj), eState(EAS_NONE), bAim(true), bFinish(false) { pArtillery->StopUnit(); }
...
ETryStateInterruptResult CArtilleryAttackCommonStatObjState::TryInterruptState(CAICommand *pCommand)
{
  ...
  const SAIUnitCmd &cmd = pCommand->ToUnitCmd();
  if (cmd.cmdType == ACTION_COMMAND_ATTACK_OBJECT && cmd.pObject.GetPtr() == pObj) return TSIR_NO_COMMAND_INCOMPATIBLE;
...
{
  if (IsValidObj(pObj) && pArtillery && pArtillery->IsValid() && pArtillery->IsAlive()) return pObj->GetAttackCenter(pArtillery->GetCenter());
  return CVec2(-1.0f, -1.0f);
}
...
IUnitState *CArtilleryRestState::Instance(CArtillery *pArtillery, const CVec2 &guardPoint, const WORD wDir) { return new CArtilleryRestState(pArtillery, guardPoint, wDir); }

CArtilleryRestState::CArtilleryRestState(CArtillery *_pArtillery, const CVec2 &guardPoint, const WORD wDir)
  : CMechUnitRestState(_pArtillery, guardPoint, wDir, false), pArtillery(_pArtillery) { pArtillery->StopUnit(); }

void CArtilleryRestState::Segment() { if (pArtillery->IsInstalled()) CMechUnitRestState::Segment(); }

// **********************************************************************
// *CArtilleryAttackAviationState*
// **********************************************************************

IUnitState *CArtilleryAttackAviationState::Instance(CArtillery *pArtillery, CAviation *pPlane) { return new CArtilleryAttackAviationState(pArtillery, pPlane); }

CArtilleryAttackAviationState::CArtilleryAttackAviationState(CArtillery *_pArtillery, CAviation *pPlane)
  : CSoldierAttackAviationState(_pArtillery, pPlane), pArtillery(_pArtillery) {}

void CArtilleryAttackAviationState::Segment() { if (pArtillery->IsInstalled()) CSoldierAttackAviationState::Segment(); }