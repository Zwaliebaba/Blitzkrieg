...
CBuildingStorage::CBuildingStorage(const SBuildingRPGStats *pStats, const CVec2 &center, const int dbID, const float fHP, const int nFrameIndex, int player)
  : CBuilding(pStats, center, dbID, fHP, nFrameIndex), nPlayer(player), bConnected(true), timeLastBuildingRepair(0)
{
...
void CBuildingStorage::AddSoldier(CSoldier *pUnit)
{
  ...
  if (SBuildingRPGStats::TYPE_MAIN_RU_STORAGE != pStats->eType) ChangePlayer(pUnit->GetPlayer());
}

void CBuildingStorage::ChangePlayer(const int _nPlayer)
{
  const CVec2 vCenter = GetCenter();
  const DWORD dwParam = static_cast<int>(vCenter.x) << 32 | static_cast<int>(vCenter.y);

  if (EDI_ENEMY == theDipl.GetDiplStatus(_nPlayer, nPlayer))
  {
    if (EDI_FRIEND == theDipl.GetDiplStatus(_nPlayer, theDipl.GetMyNumber())) updater.AddFeedBack(SAIFeedBack(EFB_YOU_CAPTURED_STORAGE, dwParam));
    else if (EDI_FRIEND == theDipl.GetDiplStatus(nPlayer, theDipl.GetMyNumber())) updater.AddFeedBack(SAIFeedBack(EFB_YOU_LOST_STORAGE, dwParam));
  }
...
const BYTE CBuildingStorage::GetPlayer() const
{
  ...
  if (res == theDipl.GetNeutralPlayer()) return nPlayer;
  return res;
}
...
{
  if (SBuildingRPGStats::TYPE_TEMP_RU_STORAGE == pStats->eType && fHP > 0 && fHP - fDamage <= 0) SetConnected(false);
...
void CBuildingStorage::Segment()
{
  ...
  else// TEMP STORAGES can change ownership
  {
    ...
    if (theDipl.GetNParty(GetPlayer()) == theDipl.GetNeutralParty())
    {
      ...
      if (nNewPlayer != GetPlayer()) ChangePlayer(nNewPlayer);
    }
    else
    {
...
bool CBuildingStorage::ShouldSuspendAction(const EActionNotify &eAction) const { return !theDipl.IsEditorMode() && CBuilding::ShouldSuspendAction(eAction); }

// **********************************************************************
// * CBuilding::SHealthySort *
// **********************************************************************

bool CBuilding::SHealthySort::operator()(const CPtr<CSoldier> &a, const CPtr<CSoldier> &b) { return a->GetHitPoints() < b->GetHitPoints(); }

// **********************************************************************
// * CBuilding::SIllSort *
// **********************************************************************

bool CBuilding::SIllSort::operator()(const CPtr<CSoldier> &a, const CPtr<CSoldier> &b) { return a->GetHitPoints() > b->GetHitPoints(); }
...
CBuilding::CBuilding(const SBuildingRPGStats *_pStats, const CVec2 &center, const int dbID, const float fHP, const int nFrameIndex)
  : CGivenPassabilityStObject(center, dbID, fHP, nFrameIndex), bAlarm(false), medical(_pStats->nMedicalSlots),
    fire(_pStats->slots.size()),
    rest(_pStats->nRestSlots), nOveralPlaces(_pStats->nRestSlots + _pStats->nMedicalSlots + _pStats->slots.size()), pLockingUnit(nullptr),
    nextSegmTime(curTime),
    turrets(_pStats->slots.size()), guns(_pStats->slots.size()),
    lastDistibution(0), firePlace2Soldier(_pStats->slots.size()), nLastFreeFireSoldierChoice(0),
    nLastPlayer(theDipl.GetNeutralPlayer()),
    nScriptID(-1), bShouldEscape(false), bEscaped(false), timeOfDeath(0), lastLeave(theDipl.GetNPlayers(), 0),
    pStats(_pStats)
{
  Init();
  ...
  for (int i = 0; i < pStats->slots.size(); ++i)
  {
    ...
    if (pStats->slots[i].gun.pWeapon != nullptr)
    {
  ...
  for (int i = 0; i < 4; ++i)
  {
    ...
    for (int j = 0; j < pStats->slots.size(); ++j)
    {
      ...
      if (takenSlots[j] == 0 && DirsDifference(pStats->slots[j].wDirection, wMiddleDir) < 4000)
      {
        ...
        {
          if (DirsDifference(wSlotDir, wLeftDir) < DirsDifference(wSlotDir, wRightDir)) wLeftDir = wSlotDir;
          else wRightDir = wSlotDir;
        }
...
{
  // if you haven't transferred it to another slot yet
  if (fire.GetMaxEl()->GetSlot() != -1) DelSoldierFromFirePlace(fire.GetMaxEl());
}
...
const BYTE CBuilding::GetFreeFireSlot()
{
  ...
  {
    if (firePlace2Soldier[j] == nullptr && turrets[j] != nullptr)
    {
  ...
  while (i < firePlace2Soldier.size() && firePlace2Soldier[i] != nullptr) ++i;

  if (i >= firePlace2Soldier.size() || firePlace2Soldier[i] != nullptr)
  {
    i = 0;
    while (i < nLastFreeFireSoldierChoice && firePlace2Soldier[i] != nullptr) ++i;
...
const float CBuilding::GetEscapeHitPoints() const { return GetStats()->fMaxHP * SConsts::HP_PERCENT_TO_ESCAPE_FROM_BUILDING; }
...
void CBuilding::AddSoldier(CSoldier *pUnit)
{
  ...
  // if this is the first soldier of the building, then register in segments
  ...
  if (fire.Size() < fire.GetReserved()) PushToFire(pUnit);
  else if (medical.Size() == medical.GetReserved()) PushToRest(pUnit);
  else if (fHP < pUnit->GetStats()->fMaxHP || rest.Size() == rest.GetReserved()) PushToMedical(pUnit);
  else PushToRest(pUnit);
...
void CBuilding::SeatSoldierToFireSlot()
{
  ...
  if (medical.IsEmpty() && rest.IsEmpty()) return;
...
void CBuilding::DelSoldierFromFirePlace(CSoldier *pSoldier)
{
  ...
  // CRAP}
  {
    firePlace2Soldier[slotInfo.nSlot] = nullptr;
...
void CBuilding::DelSoldierFromMedicalPlace(CSoldier *pSoldier)
{
  ...
  medical[slotInfo.nIndex] = nullptr;
...
void CBuilding::DelSoldier(CSoldier *pUnit, const bool bFillEmptyFireplace)
{
  ...
  switch (slotInfo.nType)
  {
    case 0:
      ...
      if (bFillEmptyFireplace) SeatSoldierToFireSlot();

      break;
    case 1:
      DelSoldierFromMedicalPlace(pUnit);
      if (bFillEmptyFireplace) SeatSoldierToMedicalSlot();
...
bool CBuilding::IsIllInRest() { return !rest.IsEmpty() && rest.GetMaxEl()->GetHitPoints() < rest.GetMaxEl()->GetStats()->fMaxHP; }

bool CBuilding::IsIllInFire() { return !fire.IsEmpty() && fire.GetMaxEl()->GetHitPoints() < fire.GetMaxEl()->GetStats()->fMaxHP; }

void CBuilding::DistributeAll()
{
  // exchange those who were cured in medical with patients in fire or rest
  while (!medical.IsEmpty() && medical.GetMaxEl()->GetHitPoints() == medical.GetMaxEl()->GetStats()->fMaxHP && (IsIllInFire() || IsIllInRest()))
  {
    ...
    {
      if (fire.GetMaxEl()->GetHitPoints() < rest.GetMaxEl()->GetHitPoints()) SwapFireMed();
      else SwapRestMed();
    }
    else if (bIllInFire) SwapFireMed();
    else SwapRestMed();
  }

  // kick out freeloaders from medical places to fireplaces
  ...
  // kick out freeloaders from medical places to restplaces
  ...
  // drive those who are possible to get treatment
...
bool CBuilding::TryToPushRestSoldierToObservation(CSoldier *pRestingSoldier)
{
  ...
  for (int j = 0; j < 4; ++j)
  {
    // by observation points on the sides
...
void CBuilding::PushSoldierToFirePlace(CSoldier *pUnit, const int nFirePlace)
{
  ...
  {
    // increase the number of soldiers at observation point on the side
...
{
  if (!pSoldier->IsInFirePlace()) return false;
...
void CBuilding::SetSoldiersToObservationPoints()
{
  for (int i = 0; i < rest.Size(); ++i)
  {
    ...
    // there are soldiers at all observation points
    if (nSide == -1 || fire.Size() == fire.GetReserved()) { if (TryToPushRestSoldierToObservation(pRestingSoldier)) return; }
    else
    {
      rest[i] = nullptr;
      rest.Erase(i);
  ...
  // seat soldiers not at observation points
  for (int i = 0; i < fire.Size(); ++i)
  {
    CSoldier *pSoldier = fire[i];
    if (!IsSoldierInObservationPoint(pSoldier))
    {
      ...
      if (nSide == -1) { if (TryToPushFireSoldierToObservation(pSoldier)) return; }
      else
      {
        DelSoldierFromFirePlace(pSoldier);
  ...
  if (nIndexToTry != -1)
  {
    ...
    const int nSide = ChooseSideToSetSoldier(pSoldier);
    if (nSide == -1) TryToPushFireSoldierToObservation(pSoldier);
    else
    {
      DelSoldierFromFirePlace(pSoldier);
      PushSoldierToObservationPoint(pSoldier, nSide);
...
{
  // exchange those who were cured in medical with patients in rest
  ...
  // kick out those who have recovered
  ...
  // drive the sick out of the rest for treatment
...
{
  // not all fireslots are occupied
  if (fire.Size() < fire.GetReserved())
  {
    for (int i = 0; i < fire.Size(); ++i)
    {
      ...
      if (fire[i]->GetState()->GetName() == EUSN_REST_IN_BUILDING)
      {
        ...
        for (int j = 0; j < 4; ++j) bShouldWatch = bShouldWatch && (sides[j].nSoldiersInObservationPoints == 0);
...
void CBuilding::ExchangeSoldiersToTurrets()
{
  ...
  {
    if (turrets[i] != nullptr && firePlace2Soldier[i] == nullptr)
    {
      int j = 0;
      while (j < firePlace2Soldier.size() && (firePlace2Soldier[j] == nullptr || turrets[j] == nullptr)) ++j;
...
void CBuilding::Segment()
{
  ...
  if (!IsUnitsInside())
  {
    if (SBuildingRPGStats::TYPE_MAIN_RU_STORAGE != pStats->eType && SBuildingRPGStats::TYPE_TEMP_RU_STORAGE != pStats->eType) theStatObjs.UnregisterSegment(this);
  ...
  else if (!CStormableObject::Segment())
  {
    ...
    if (bAlarm)
    {
      while (!fire.Size() == fire.GetReserved() && (!medical.IsEmpty() || !rest.IsEmpty())) SeatSoldierToFireSlot();
    ...
    if (curTime - lastDistibution >= 4000 || curTime - startOfRest < SConsts::TIME_OF_BUILDING_ALARM && curTime - lastDistibution >= 3500)
    {
      ...
      // anxiety
      if (curTime - startOfRest < SConsts::TIME_OF_BUILDING_ALARM) DistributeFiringSoldiers();

      // everyone can be treated
      if (curTime - startOfRest >= SConsts::TIME_OF_BUILDING_ALARM) DistributeAll();
      else
        // only those who are not shooting can be treated
        DistributeNonFires();
    }

    // drive all freeloaders into fireplaces
    ...
    if (pLockingUnit && (!pLockingUnit->IsValid() && !pLockingUnit->IsAlive())) Unlock(pLockingUnit);
  }

  ExchangeSoldiersToTurrets();

  // turrets segments
  for (int i = 0; i < fire.Size(); ++i) { if (IsValidObj(fire[i])) { if (fire[i]->GetSlot() >= 0 && fire[i]->GetSlot() < turrets.size() && turrets[fire[i]->GetSlot()] != nullptr) turrets[fire[i]->GetSlot()]->Segment(); } }
}
...
void CBuilding::KillAllInsiders()
{
  ...
  for (int i = 0; i < medical.Size(); ++i) dead.push_back(medical[i]);

  for (int i = 0; i < rest.Size(); ++i) dead.push_back(rest[i]);

  for (int i = 0; i < fire.Size(); ++i) dead.push_back(fire[i]);

  for (auto iter = dead.begin(); iter != dead.end(); ++iter) (*iter)->Die(false, 0);
}

void CBuilding::Die(const float fDamage)
{
  if (!bEscaped) KillAllInsiders();
...
void CBuilding::SetHitPoints(const float fNewHP)
{
  if (fNewHP != fHP)
  {
    ...
    if (fHP > GetEscapeHitPoints()) bShouldEscape = true;

    if (fHP > 0.0f) timeOfDeath = 0.0f;
  }
}
...
void CBuilding::TakeDamage(const float fDamage, const bool bFromExplosion, const int nPlayerOfShoot, CAIUnit *pShotUnit)
{
  if (fHP > 0)
  {
    ...
    if (bFromExplosion)
    {
      ...
      if (theCheats.GetFirstShoot(theDipl.GetNParty(nPlayerOfShoot)) == 1) fHP = 0.0f;
      ...
      if (GetHitPoints() == 0.0f)
      {
            ...
            SBuildingRPGStats::TYPE_TEMP_RU_STORAGE != pStats->eType)
        {
          ...
          if (!bEscaped)
          {
            for (int i = 0; i < fire.Size(); ++i)
            {
              if (pShotUnit) pShotUnit->EnemyKilled(fire[i]);
            ...
            for (int i = 0; i < medical.Size(); ++i)
            {
              if (pShotUnit) pShotUnit->EnemyKilled(medical[i]);
            ...
            for (int i = 0; i < rest.Size(); ++i)
            {
              if (pShotUnit) pShotUnit->EnemyKilled(rest[i]);
      ...
      else
      {
        if (!bEscaped)
        {
          ...
          {
            // not a tenant
            if (Random(0.0f, 1.0f) < fProbability) dead.push_back(medical[i]);
          }

          for (int i = 0; i < rest.Size(); ++i)
          {
            // not a tenant
            if (Random(0.0f, 1.0f) < fProbability) dead.push_back(rest[i]);
          }

          for (int i = 0; i < fire.Size(); ++i)
          {
            // not a tenant
            if (Random(0.0f, 1.0f) < fProbability) dead.push_back(fire[i]);
          }
          ...
          for (auto iter = dead.begin(); iter != dead.end(); ++iter)
          {
            ...
            if (pShotUnit) pShotUnit->EnemyKilled(pSoldier);
...
void CBuilding::Alarm() { bAlarm = true; }

const int CBuilding::GetNFreePlaces() const
{
  if (GetHitPoints() > 0) return nOveralPlaces - medical.Size() - fire.Size() - rest.Size();

  return 0;
}

bool CBuilding::IsGoodPointForRunIn(const SVector &point, const int nEntrance, const float fMinDist) const { return fabs2(point.ToCVec2() - GetEntrancePoint(nEntrance)) <= sqr(static_cast<float>(SConsts::TILE_SIZE) + fMinDist); }

const int CBuilding::GetNDefenders() const { return medical.Size() + rest.Size() + fire.Size(); }
...
class CSoldier *CBuilding::GetUnit(const int n) const
{
  ...
  if (n < fire.Size()) return fire[n];
  else if (n < fire.Size() + rest.Size()) return rest[n - fire.Size()];
  return medical[n - fire.Size() - rest.Size()];
}

const BYTE CBuilding::GetPlayer() const
{
  if (!fire.IsEmpty()) return fire[0]->GetPlayer();
  if (!rest.IsEmpty()) return rest[0]->GetPlayer();
  if (!medical.IsEmpty()) return medical[0]->GetPlayer();

  return theDipl.GetNeutralPlayer();
}

void CBuilding::Lock(CCommonUnit *pUnit) { pLockingUnit = pUnit; }

bool CBuilding::IsLocked(const int nPlayer) const { return pLockingUnit != nullptr; }

void CBuilding::Unlock(CCommonUnit *pUnit) { pLockingUnit = nullptr; }
...
CBasicGun *CBuilding::GetGunInFireSlot(const int nSlot, const int nGun)
{
  ...
  if (nGun >= guns[nSlot]->GetNTotalGuns()) return nullptr;
...
bool CBuilding::IsSoldierVisible(const int nParty, const CVec2 &center, bool bCamouflated, const float fCamouflage) const
{
  ...
  const int nYShift = (center.y - nDownY) / static_cast<float>(SConsts::TILE_SIZE);
  const int nXShift = (center.x - nDownX) / static_cast<float>(SConsts::TILE_SIZE);
  ...
  for (int y = nYDown; y < nYUp; ++y)
  {
    for (int x = nXDown; x < nXUp; ++x)
    {
      if (passability[y][x] == 1)
      {
        ...
        if (theWarFog.IsUnitVisible(nParty, tile, bCamouflated, fCamouflage)) return true;
      }
    }
  }
...
bool CBuilding::ChooseEntrance(class CCommonUnit *pUnit, const CVec2 &vPoint, int *pnEntrance) const
{
  ...
  *pnEntrance = -1;
  ...
  for (int i = 0; i < GetNEntrancePoints(); ++i)
  {
    ...
    if (pPath != nullptr && (*pnEntrance == -1 || pPath->GetLength() < nMinDistance)/* && fabs2( ( pPath->GetFinishPoint() - vPoint ) ) <= sqr( 2 * SConsts::GOOD_LAND_DIST ) */)
    {
  ...
  if (*pnEntrance == -1 && GetNEntrancePoints() > 0) *pnEntrance = 0;

  return (*pnEntrance != -1);
}

bool CBuilding::IsSelectable() const { return theDipl.GetNParty(GetPlayer()) == theDipl.GetMyParty(); }

bool CBuilding::CanUnitGoThrough(const EAIClass &eClass) const { return (pStats->dwAIClasses & eClass) == 0; }
...
void CBuilding::ExchangeUnitToFireplace(CSoldier *pSoldier, int nFirePlace)
{
  ...
  if (pDeletedSoldier) DelSoldier(pDeletedSoldier, false);

  DelSoldier(pSoldier, false);

  PushSoldierToFirePlace(pSoldier, nFirePlace);

  if (pDeletedSoldier) AddSoldier(pDeletedSoldier);
}

const int CBuilding::GetNFirePlaces() const { return pStats->slots.size(); }

CSoldier *CBuilding::GetSoldierInFireplace(const int nFireplace) const { return firePlace2Soldier[nFireplace]; }
...
{
  if (pSoldier1) pSoldier1->SetSlotIndex(nSoldier2Index);
  if (pSoldier2) pSoldier2->SetSlotIndex(nSoldier1Index);
}

bool CBuilding::IsBetterChangeObservationSoldier(CSoldier *pSoldier, CSoldier *pSoldierInPoint) { return pSoldier->GetSightRadius() > pSoldierInPoint->GetSightRadius(); }

CSoldier *CBuilding::GetSoldierOnSide(const int nSide)
{
  NI_ASSERT_T(nSide < 4, NStr::Format( "Wrong side passed (%d)", nSide ));
  for (int i = 0; i < sides[nSide].nObservationPoints; ++i) { if (firePlace2Soldier[observationPlaces[nSide][i]] != nullptr) return firePlace2Soldier[observationPlaces[nSide][i]]; }
...
const int CBuilding::GetMiddleObservationPoint(const int nSide) const
{
  ...
  if (sides[nSide].nObservationPoints == 0) return -1;
  else if (sides[nSide].nObservationPoints < 3) return observationPlaces[nSide][0];
  else return observationPlaces[nSide][1];
}
...
void CBuilding::GetSidesObservationPoints(const int nSide, int *pnLeftPoint, int *pnRightPoint) const
{
  ...
  if (sides[nSide].nObservationPoints == 0) *pnLeftPoint = *pnRightPoint = -1;
  else if (sides[nSide].nObservationPoints == 1) *pnLeftPoint = *pnRightPoint = observationPlaces[nSide][0];
...
const int CBuilding::ChooseSideToSetSoldier(CSoldier *pSoldier) const
{
  ...
  if (pSoldier->IsInFirePlace())
  {
    ...
    if (firePlace2Observation[nSoldierSlot] != -1) nSoldierSide = firePlace2Observation[nSoldierSlot] & 3;
  }
  ...
  for (int i = 0; i < 4; ++i)
  {
    ...
    if (i == nSoldierSide) --nSoldiersInObservationPoints;
...
{
  // this is storage
  if (SBuildingRPGStats::TYPE_TEMP_RU_STORAGE == pStats->eType) return true;
  // we ran/ran out
  if (theDipl.GetNParty(nLastPlayer) == theDipl.GetMyParty() ||
      theDipl.GetNParty(GetPlayer()) == theDipl.GetMyParty())
    return true;
  // the enemy ran in
  if (GetPlayer() != theDipl.GetNeutralPlayer()) return IsAnyInsiderVisible();
  // the enemy ran out
  return IsVisible(theDipl.GetMyParty());
}

void CBuilding::SetLastLeaveTime(const int nPlayer) { lastLeave[nPlayer] = curTime; }