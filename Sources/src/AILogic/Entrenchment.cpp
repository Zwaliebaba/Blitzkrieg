...
{
  for (auto iter = coveredTiles.begin(); iter != coveredTiles.end(); ++iter)
  {
    const SVector &tile = *iter;
    if (!theWarFog.IsTileVisible(tile, 0) || !theWarFog.IsTileVisible(tile, 1)) return false;
  }
...
{
  // all actions here must be const, because 
  if (!bVisible)
  {
    ...
    if (bVisible) pFullEntrenchment = nullptr;
  }
  else pFullEntrenchment = nullptr;

  // unregister only if visible to all parties - for multiplayer
  if (CanUnregister()) theStatObjs.UnregisterSegment(this);

  // random always call - for mutliplayer
...
void CEntrenchmentPart::TakeDamage(const float fDamage, const bool bFromExplosion, const int nPlayerOfShoot, CAIUnit *pShotUnit) { if (pOwner != nullptr) pOwner->TakeDamage(fDamage, bFromExplosion, nPlayerOfShoot, pShotUnit); }

bool CEntrenchmentPart::IsPointInside(const CVec2 &point) const { return boundRect.IsPointInside(point); }
  ...
  : nBusyFireplaces(0)
{
  ...
  for (int i = 0; i < nLen; ++i)
  {
    ...
    auto pPart = static_cast<CEntrenchmentPart *>(segments[i]);
    ...
    if (pPart->GetType() == SEntrenchmentRPGStats::EST_FIREPLACE || pPart->GetType() == SEntrenchmentRPGStats::EST_LINE)
    {
      ...
      else
      {
        ...
        if (fLength > 0 && fLength > fLengthAhead) fLengthAhead = fLength;

        fLength = fToCenter - pPart->GetSegmStats().vAABBHalfSize.y;
        if (fLength < 0 && -fLength > fLengthBack) fLengthBack = -fLength;
      }

      for (int i = 0; i < pPart->GetSegmStats().fireplaces.size(); ++i) fireplaces.push_back(SFireplaceInfo(pPart->GetCenter() + GetShift(pPart->GetSegmStats().fireplaces[i], vDir), nullptr, pPart->GetFrameIndex()));
    }
  ...
  // so that the center of the guide is in the middle
...
void CEntrenchment::AddSoldier(CSoldier *pUnit)
{
  ...
  if (nBusyFireplaces < fireplaces.size())
  {
    ...
    // find the free fireplace closest to the center of the unit
    int i = 0;
    float fBestDist = 1e10;
    while (i < fireplaces.size())
    {
      if (fireplaces[i].pUnit == nullptr)
      {
...
{
  // there are units in reserve, check for being in a trench, because 
...
{
  auto iter = insiders.begin();
  while (iter != insiders.end() && iter->pUnit.GetPtr() != pUnit) ++iter;

  if (iter != insiders.end())
  {
    int nFireplace = iter->nFireplace;
    // sitting in the fireplace
    if (nFireplace != -1) fireplaces[nFireplace].pUnit = nullptr;

    insiders.erase(iter);

    if (nFireplace != -1 && bFillEmptyFireplace) ProcessEmptyFireplace(nFireplace);
    if (nFireplace != -1 && !bFillEmptyFireplace) --nBusyFireplaces;
...
void CEntrenchment::TakeDamage(const float fDamage, const bool bFromExplosion, const int nPlayerOfShoot, CAIUnit *pShotUnit)
{
  ...
  // all killed
  if (fDamage >= pStats->fMaxHP || theCheats.GetFirstShoot(theDipl.GetNParty(nPlayerOfShoot)) == 1) { for (auto iter = insiders.begin(); iter != insiders.end(); ++iter) dead.push_back(iter->pUnit); }
  else
  {
    const float fProbability = fDamage / pStats->fMaxHP;

    for (auto iter = insiders.begin(); iter != insiders.end(); ++iter) { if (Random(0.0f, 1.0f) < fProbability) dead.push_back(iter->pUnit); }
  }
...
void CEntrenchment::GetCoveredTiles(CTilesSet *pTiles) const { GetTilesCoveredByRect(rect, pTiles); }
...
void CEntrenchment::Iterate()
{
  ...
  {
    ++iter;
    while (iter != insiders.end() && iter->nFireplace == -1) ++iter;
  }
}

const int CEntrenchment::GetNDefenders() const { return insiders.size(); }
...
{
  if (GetNDefenders() == 0 || insiders.empty()) return theDipl.GetNeutralPlayer();
...
void CEntrenchment::Segment()
{
  ...
  if (!IsUnitsInside()) theStatObjs.UnregisterSegment(this);
  else { if (!CStormableObject::Segment()) CRotatingFireplacesObject::Segment(); }
}

void CEntrenchment::Delete() { theStatObjs.DeleteInternalEntrenchmentInfo(this); }
...
bool CEntrenchment::CanRotateSoldier(CSoldier *pSoldier) const { return pSoldier->GetState() && IsRestState(pSoldier->GetState()->GetName()); }
...
const int CEntrenchment::GetNFirePlaces() const { return fireplaces.size(); }

CSoldier *CEntrenchment::GetSoldierInFireplace(const int nFireplace) const { return fireplaces[nFireplace].pUnit; }
...
CEntrenchmentTankPit::CEntrenchmentTankPit(const SMechUnitRPGStats *_pStats,
  ...
  : CGivenPassabilityStObject(center, dbID, 1.0f, nFrameIndex), pStats(_pStats), wDir(dir), vHalfSize(vHalfSize),
    pOwner(_pOwner)
{
  // copy tiles
  for (auto i = _tilesToLock.begin(); _tilesToLock.end() != i; ++i) tilesToLock.push_back(*i);

  boundRect.InitRect(center, GetVectorByDirection(dir), vHalfSize.y, vHalfSize.x);
}

void CEntrenchmentTankPit::GetTilesForVisibility(CTilesSet *pTiles) const { GetCoveredTiles(pTiles); }

void CEntrenchmentTankPit::GetCoveredTiles(CTilesSet *pTiles) const
{
  pTiles->clear();
  for (auto i = tilesToLock.begin(); tilesToLock.end() != i; ++i)
  {
    const SVector v = *i;
    if (theStaticMap.IsTileInside(v)) pTiles->push_back(v);
  }
}
...
void CEntrenchmentTankPit::LockTiles(bool bInitialization) { theStaticMap.UpdateMaxesByTiles(tilesToLock, AI_CLASS_ANY, true); }
...
void CEntrenchmentTankPit::Die(const float fDamage) { Delete(); }
...
void CFullEntrenchment::AddEntrenchmentPart(class CEntrenchmentPart *pEntrenchmentPart) { entrenchParts.push_back(pEntrenchmentPart); }

void CFullEntrenchment::SetVisible() { for (auto iter = entrenchParts.begin(); iter != entrenchParts.end(); ++iter) (*iter)->SetVisible(); }