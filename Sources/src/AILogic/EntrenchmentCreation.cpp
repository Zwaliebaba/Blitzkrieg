...
static constexpr float fNearToNormale = 0.05f;
...
bool CEntrenchmentCreation::SearchTrenches(const CVec2 &vCenter, const SRect &rectToTest)
{
  ...
  for (CStObjCircleIter<false> iter(vCenter, fMaxSize); !iter.IsFinished(); iter.Iterate())
  {
    ...
    if (ESOT_ENTR_PART == pObj->GetObjectType())
    {
      ...
      if (rectToTest.IsIntersected(objRect)) return true;
    }
  }
  return false;
}
...
bool CEntrenchmentCreation::PreCreate(const CVec2 &vFrom, const CVec2 &vTo)
{
  ...
  {
    switcher = ((i + 1) % 3) == 0;
    // decide what to build - fire place or line
    ...
    //
    if (CanDig(pRPG, dbID, pt, wAngle, nFrameIndex)) parts.push_back(AddElement(pRPG, dbID, pt, wAngle, nFrameIndex));
    else
    {
      bSayAck = true;
      break;
...
const int CEntrenchmentCreation::GetMaxIndex() const { return parts.size(); }

const int CEntrenchmentCreation::GetCurIndex() const { return nCurIndex; }
...
void CEntrenchmentCreation::BuildNext()
{
  ...
  if (0 == nCurIndex) theStatObjs.AddEntrencmentPart(pBeginTerminator, true, false);
  ...
  if (pFullEntrenchment)
  {
    ...
    pFullEntrenchment = nullptr;
  }
  if (pEndTerminator)
  {
    theStatObjs.DeleteInternalObjectInfo(pEndTerminator);
    pEndTerminator = nullptr;
  }

  std::vector<IRefCount *> vEntr;

  for (int i = 0; i < nCurIndex; ++i) vEntr.push_back(parts[i]);
  ...
  if (GetCurIndex() < GetMaxIndex()) CalcTilesUnder();
}
...
void CEntrenchmentCreation::CalcTilesUnder()
{
  ...
  if (0 == nCurIndex) pBeginTerminator->GetCoveredTiles(&tilesUnder3);
...
void CEntrenchmentCreation::CreateNewEndTerminator()
{
  ...
  if (nCurIndex + 1 < vPoints.size()) pNewEndTerminator = AddElement(pRPG, dbID, vPoints[nCurIndex + 1], wAngle, nTermInd);
  else pNewEndTerminator = nullptr;
}
...
void CEntrenchmentCreation::GetUnitsPreventing(std::list<CPtr<CAIUnit>> *units)
{
  ...
  if (0 == nCurIndex) pBeginTerminator->GetBoundRect(&r3);
...
bool CEntrenchmentCreation::CanBuildNext() const
{
  ...
  const float fRadius = r1.lengthAhead + r1.lengthBack + r1.width + r2.lengthAhead + r2.lengthBack + r2.width + SConsts::TILE_SIZE * 5;
  // go through the units, unpack everything.
  for (CUnitsIter<0, 2> iter(0, ANY_PARTY, vPoints[nCurIndex], fRadius); !iter.IsFinished(); iter.Iterate())
  {
    CAIUnit *pUnit = *iter;
  ...
  // now check if it is possible to build
  for (auto it = tilesUnder.begin(); it != tilesUnder.end(); ++it) { if (0 != theStaticMap.GetTileLockInfo(*it)) return false; }
  return true;
}

float CEntrenchmentCreation::GetPrice() { return SConsts::ENTRENCHMENT_SEGMENT_RU_PRICE; }

void CEntrenchmentCreation::LockNext()
{
  parts[nCurIndex]->LockTiles();
  pNewEndTerminator->LockTiles();
  if (0 == nCurIndex) pBeginTerminator->LockTiles();
}
...
CEntrenchmentPart *CEntrenchmentCreation::AddElement(const SEntrenchmentRPGStats *pRPG, int dbID, const CVec2 &pt, WORD angle, int nFrameIndex)
{
  ...
  auto ptr = new CEntrenchmentPart(pRPG, pt, angle, nFrameIndex, dbID, pRPG->fMaxHP);
...
WORD CEntrenchmentCreation::GetLineAngle(const CVec2 &vBegin, const CVec2 &vEnd) const
{
  ...
  if (vTmp.y < 0) fAngle = FP_2PI - fAngle;
  return static_cast<WORD>((fAngle / (2.0f * PI)) * 65535);
}


float CEntrenchmentCreation::GetTrenchWidth(int nType)// 0 - section, 1 - turn
{
  IObjectsDB *pGDB = GetSingleton<IObjectsDB>();
  const SGDBObjectDesc *pDesc = pGDB->GetDesc("Entrenchment");
  auto pRPG = static_cast<const SEntrenchmentRPGStats *>(pGDB->GetRPGStats(pDesc));
  int nFrameIndex = 0;
  int nRandom = Random(0, 0xffff);
  if (nType == 0) nFrameIndex = pRPG->GetLineIndex(&nRandom);
  if (nType == 1) nFrameIndex = pRPG->GetArcIndex(&nRandom);
...
void CEntrenchmentCreation::SplitLineToSegrments(std::vector<CVec2> *_vPoints, CVec2 vBegin, CVec2 vEnd, float TRENCHWIDTH)
{
  ...
  if (fabs(vAddSegment.x - currentPoint.x, vAddSegment.y - currentPoint.y) < allLenght) { vPoints.push_back(CVec2(currentPoint.x, currentPoint.y)); }
...
float CFenceCreation::GetPrice() { return SConsts::FENCE_SEGMENT_RU_PRICE; }
...
bool CFenceCreation::PreCreate(const CVec2 &_vFrom, const CVec2 &_vTo)
{
  ...
  auto pStats = static_cast<const SFenceRPGStats *>(pGDB->GetRPGStats(pDesc));
  const int nDBIndex = pGDB->GetIndex(theUnitCreation.GetWireFenceName());

  // determine length of one segment
  int nFrameIndex;
  int nRandom = Random(0, 0xffff);
  if (!isXConst) nFrameIndex = vFrom.x > vTo.x ? pStats->GetCenterIndex(1, &nRandom) : pStats->GetCenterIndex(3, &nRandom);
  else nFrameIndex = vFrom.y > vTo.y ? pStats->GetCenterIndex(0, &nRandom) : pStats->GetCenterIndex(2, &nRandom);
  constexpr int nSegmentLenght = 2;// isXConst? 
  constexpr int nOffset = nSegmentLenght - 1;

  // now we will put fences along the line
  for (auto it = hlpFence.m_points.begin(); it != hlpFence.m_points.end();)
  {
    // check is current points inside static map
    const CVec2 vPoint1(*it);
    bool bCanBuild = true;
    for (int i = 0; i < nSegmentLenght && bCanBuild; ++i)// check that every tile under the segment is inside map
    {
      if (it == hlpFence.m_points.end() || !theStaticMap.IsTileInside(*it)) bCanBuild = false;
      ++it;
    }

    if (!bCanBuild) return !fenceSegements.empty();

    CVec2 vFencePosition(vPoint1.x * SConsts::TILE_SIZE, vPoint1.y * SConsts::TILE_SIZE);
    CVec2 vCenterPosition(vPoint1);
    // Here you need to place depending on the direction
    if (!isXConst)
    {
      ...
      else vCenterPosition -= CVec2(nOffset, -0.5);
    }
    else
    {
      // the second direction needs to change position
      ...
      else vCenterPosition += CVec2(0.5, nOffset);
    }
    ...
    if (/* construction of this piece is necessary */ IsCegmentToBeBuilt(pObj))
    {
...
bool CFenceCreation::IsCegmentToBeBuilt(class CFence *pObj) const
{
  ...
  // check if there is any fence thread on those tiles that pObj is locking

  // find the segment that locks the tiles
  SRect r1;
  ...
  for (CUnitsIter<0, 2> iter(0, ANY_PARTY, pObj->GetCenter(), fRadius); !iter.IsFinished(); iter.Iterate())
  {
    ...
    if (!pUnit->GetStats()->IsInfantry())
    {
      ...
      if (r1.IsIntersected(rect))
      {
        if (pUnit->GetPlayer() == nPlayer &&
            EUSN_REST == pUnit->GetState()->GetName() &&
            pUnit->CanMove()) { (*iter)->UnlockTiles(); }
      }
    }
  }
  CTilesSet tiles;
  ...
  {
    auto pStats = static_cast<const SStaticObjectRPGStats *>(pObj->GetStats());
    if (theStaticMap.IsLocked(*it, pStats->dwAIClasses)) return false;
  }
  return true;
}
...
void CFenceCreation::GetUnitsPreventing(std::list<CPtr<CAIUnit>> *units)
{
  ...
  for (CUnitsIter<0, 2> iter(0, ANY_PARTY, vPoints[nCurIndex], fRadius); !iter.IsFinished(); iter.Iterate())
  {
    ...
    {
      const SRect rect = (*iter)->GetUnitRect();
      if (r1.IsIntersected(rect)) units->push_back(*iter);
    }
  }
}
...
bool CFenceCreation::IsAnyUnitPrevent() const
{
  ...
  for (CUnitsIter<0, 2> iter(0, ANY_PARTY, vPoints[nCurIndex], fRadius); !iter.IsFinished(); iter.Iterate())
  {
    ...
    {
      const SRect rect = (*iter)->GetUnitRect();
      if (r1.IsIntersected(rect)) return true;
    }
  }
  return false;
}

const int CFenceCreation::GetMaxIndex() const { return fenceSegements.size(); }

const int CFenceCreation::GetCurIndex() const { return nCurIndex; }
...
void CFenceCreation::CalcTilesUnder() { fenceSegements[nCurIndex]->GetCoveredTiles(&tilesUnder); }
...
bool CBridgeCreation::SBridgeSpanSort::operator()(const CObj<CBridgeSpan> &s1, const CObj<CBridgeSpan> &s2)
{
  ...
  if (s1->GetBridgeStats()->direction == SBridgeRPGStats::VERTICAL) return v1.y > v2.y;
  return v1.x > v2.x;
}
...
CVec2 CBridgeCreation::SortBridgeSpans(std::vector<CObj<CBridgeSpan>> *spans, class CCommonUnit *pUnit)
{
  ...
  // check which one to go to less - to the first or to the last
  const CBridgeSpan *s1 = *spans->begin();
  const CBridgeSpan *s2 = (*spans)[spans->size() - 1];

  CVec2 vFrom1to2(s2->GetCenter() - s1->GetCenter());
  Normalize(&vFrom1to2);
  // find points close to s1 and s2 that lie outside the bridge
  SRect r1, r2;
  s1->GetBoundRect(&r1);
  s2->GetBoundRect(&r2);
  ...
  {
    // the first span is the closest.
    return v1;
  }
  if (pPath1->GetLength() >= pPath2->GetLength() && fDiff2 < sqr(SConsts::TILE_SIZE * 10))
  {
    // the last span is the closest.
    std::reverse(spans->begin(), spans->end());
    return v2;
  }
  if (fDiff1 < sqr(SConsts::TILE_SIZE * 10))// You can at least get there along the first path
  {
    // the first span is the closest.
    return v1;
  }
  if (fDiff2 < sqr(SConsts::TILE_SIZE * 10))
  {
    std::reverse(spans->begin(), spans->end());
    return v2;
  }
...
bool CBridgeCreation::IsFirstSegmentBuilt() const { return spans[0]->GetHitPoints() >= 0; }

CBridgeCreation::CBridgeCreation(class CFullBridge *pBridge, class CCommonUnit *pUnit)
  : pFullBridge(pBridge)
{
  // just sort all areas by coordinates
  pBridge->EnumSpans(&spans);
  NI_ASSERT_T(spans.size() >= 2, "bridge witout at least 2 spans");
  // calculate the length of the path to 2 extreme points, choose which one to go to.
  vStartPoint = SortBridgeSpans(&spans, pUnit);

  // divided into those already built and those not yet built.
  for (nCurIndex = 0; nCurIndex < spans.size(); ++nCurIndex) { if (spans[nCurIndex]->GetHitPoints() < 0.0f) break; }
...
const CVec2 &CBridgeCreation::GetStartPoint() const { return vStartPoint; }

CLine2 CBridgeCreation::GetCurLine() { return line; }

const int CBridgeCreation::GetMaxIndex() const { return spans.size(); }

const int CBridgeCreation::GetCurIndex() const { return nCurIndex; }
...
const CVec2 CBridgeCreation::GetNextPoint(const int nPlace, const int nMaxPlace) const
{
  ...
  CVec2 vertexes[2];
  // you need to go to the edge of the object.
  int nVertIndex = 0;
  for (int i = 0; i < 4; ++i)
  {
...
{
  CLongObjectCreation::BuildNext();
  if (GetCurIndex() != 0) { pFullBridge->UnlockSpan(spans[nCurIndex]); }
  // transfer the segment to the completed state
  ...
  else { pFullBridge->UnlockAllSpans(); }
}
...
float CBridgeCreation::GetBuildSpeed() { return SConsts::ENGINEER_REPEAR_HP_PER_QUANT; }