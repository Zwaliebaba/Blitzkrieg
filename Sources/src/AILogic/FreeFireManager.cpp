...
void CFreeFireManager::Analyze(CCommonUnit *pUnit, CBasicGun *pActiveGun)
{
  if (curTime - lastCheck >= TIME_TO_CHECK)
  {
    ...
    if (nRandom < curTime) lastCheck = curTime - Random(0, 500);
    ...
    {
      pUnit->ResetShootEstimator(nullptr, false, dwForbidden);
      for (CUnitsIter<1, 3> iter(pUnit->GetParty(), EDI_ENEMY, pUnit->GetCenter(), fSightRadius); !iter.IsFinished(); iter.Iterate()) { if ((*iter)->IsAlive() && (*iter)->IsNoticableByUnit(pUnit, fSightRadius)) pUnit->AddUnitToShootEstimator(*iter); }
      ...
      if (pTarget != nullptr)
      {
        ...
        if (CTurret *pTurret = pGun->GetTurret()) { if (!pTurret->IsLocked(pGun)) pTurret->Lock(pGun); }
...
bool CFreeFireManager::SShotInfo::NeedAim(CAIUnit *pNewTarget, CBasicGun *pGun) const { return pNewTarget != pTarget || shootingPos != pNewTarget->GetCenter() || unitDir != pNewTarget->GetFrontDir() || gunDir != pGun->GetGlobalDir(); }
