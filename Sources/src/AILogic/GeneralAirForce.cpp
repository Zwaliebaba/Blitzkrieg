...
CGeneralAirForce::CGeneralAirForce(const int nParty, IEnemyContainer *pEnemyContainer)
  : nParty(nParty),
    pEnemyContainer(pEnemyContainer), bReservedByFighters(false),
    timeLastCheck(curTime), timeLastFighterCheck(curTime)
{
  for (int i = 0; i < theDipl.GetNPlayers(); ++i) { if (theDipl.GetNParty(i) == nParty) { players.push_back(i); } }
  requests.clear();
...
void CGeneralAirForce::InitCheckPeriod()
{
  ...
  if (Random(0.0f, 1.0f) < SGeneralConsts::AVATION_LONG_PERIOD_PROBABILITY) { checkPeriod *= SGeneralConsts::AVATION_PERIOD_MULTIPLY; }
}
...
void CGeneralAirForce::InitFighterCheckPeriod()
{
  ...
  if (Random(0.0f, 1.0f) < SGeneralConsts::AVATION_LONG_PERIOD_PROBABILITY) { fighterCheckPeriod *= SGeneralConsts::AVATION_PERIOD_MULTIPLY; }
}
...
void CGeneralAirForce::Segment()
{
  for (int i = 0; i < players.size(); ++i)
  {
    if (players[i] != theDipl.GetMyNumber() && !bReservedByFighters)
    {
      ...
      {
        if (theUnitCreation.IsAviaEnabled(nPlayer, SUCAviation::AT_FIGHTER)) PrepeareFighters(nPlayer);
        InitFighterCheckPeriod();
      ...
      if (curTime - timeLastCheck > checkPeriod)
      {
        ...
        if (!bReservedByFighters && IsTimePossible(nPlayer, curTime))// We are waiting for the time when we need to release fighters
        {
          if (theUnitCreation.IsAviaEnabled(nPlayer, SUCAviation::AT_BATTLEPLANE)) LaunchByRequest(nPlayer, SUCAviation::AT_BATTLEPLANE, &requests[FT_AIR_GUNPLANE - _FT_AIR_BEGIN]);

          if (theUnitCreation.IsAviaEnabled(nPlayer, SUCAviation::AT_BOMBER)) { LaunchByRequest(nPlayer, SUCAviation::AT_BOMBER, &requests[FT_AIR_BOMBER - _FT_AIR_BEGIN]); }

          if (theUnitCreation.IsAviaEnabled(nPlayer, SUCAviation::AT_SCOUT)) LaunchScoutFree(nPlayer);
        }
      }
    }
  }
  std::list<int> deleted;
  for (AntiAviation::iterator it = antiAviation.begin(); antiAviation.end() != it; ++it) { if (it->second->IsTimeToForget()) deleted.push_back(it->first); }
...
bool CGeneralAirForce::IsTimePossible(const int nPlayer, const NTimer::STime timeToLaunch) const
{
  ...
  {
    // find out whether it is already possible to send planes and whether they will regenerate by the required date
...
void CGeneralAirForce::LaunchByRequest(const int nPlayer, const int nAvia, Requests *pRequest)
{
  ...
  std::list<CVec2> vPoints;// Stormtrooper call points
  for (Requests::iterator it = pRequest->begin(); it != pRequest->end(); ++it) vPoints.push_back(it->second.vPoint);

  // thin out.
  SSameEnemyPointPredicate pr1;
  auto firstSame = std::unique(vPoints.begin(), vPoints.end(), pr1);
  ...
  {
    for (Requests::iterator it = pRequest->begin(); pRequest->end() != it; ++it) { requestsID.AddToFreeId(it->first); }
    pRequest->clear();
  }
  // CRAP}
}
...
void CGeneralAirForce::LaunchFighters(const int nPlayer)
{
  ...
  // send fighters to a specified point.
...
void CGeneralAirForce::PrepeareFighters(const int nPlayer)
{
  ...
  // look for enemies who may invade our space
  // find the first one we can get to.
  for (CPlanesIter iter; !iter.IsFinished(); iter.Iterate())
  {
        ...
        EDI_ENEMY == theDipl.GetDiplStatus(pPlane->GetPlayer(), nPlayer))
    {
      ...
      {
        // find the first of our units that an enemy plane will fly over
        CAIUnit *pUnit = *iter;
      ...
      if (bFound)
      {
        ...
        // the point, that we have to intercept the enemy's plane before
             ...
             !iter.IsFinished(); iter.Iterate())
        {
          ...
          if (pUnit->IsAlive())
          {
            ...
            if (type == RPG_TYPE_ART_AAGUN || type == RPG_TYPE_SPG_AAGUN) return;
          }
        }
        // there is no AA gun.
        const float fTimeToFly = fAllowedPathLenght / fSpeed;

        // calculate how long it will take our fighters to reach the interception point
  ...
  for (AntiAviation::iterator it = antiAviation.begin(); it != antiAviation.end(); ++it)
  {
    ...
    for (int i = 0; i < nGuns; ++i)
    {
      ...
      if (pStats->nCeiling > fFlyHeight)
      {
        ...
        if (fDistToCenter > fDist1)// the normal from the point does not fall on the segment
          fMinDist = fDist1;
        else
        {
          const float fDist2 = fabs2(vCenter - vFinish);
          if (fDistToCenter > fDist2) fMinDist = fDist2;
          else fMinDist = sqr(fabs(line.DistToPoint(vCenter)));
        }

        if (sqr(pGun->GetFireRangeMax()) > fMinDist) return 1;
      }
    }
  }
  return 0;
...
void CGeneralAirForce::LaunchScoutFree(const int nPlayer)
{
  ...
  for (int x = nStep / 2; x < fSizeX; x += nStep) { for (int y = nStep / 2; y < fSizeY; y += nStep) { if (!theWarFog.IsTileVisible(SVector(x, y), nParty)) points.push_back(CVec2(x, y)); } }
  ...
  const float fCheckRadius = Min(SGeneralConsts::SCOUT_FREE_POINT,
                                 nStep * SConsts::TILE_SIZE);
  ...
  // actually it would be cool to check and return, but this is impossible
  if (!vPointsToFly.empty()) { LaunchPlane(SUCAviation::AT_SCOUT, vPointsToFly, nPlayer); }
  theUnitCreation.LockAppearPoint(nPlayer, false);
}
...
int /* request ID */CGeneralAirForce::RequestForSupport(const CVec2 &vSupportCenter, enum EForceType eType, int nResistanceCellNumber)
{
  if (nResistanceCellNumber != -1)
  {
    ...
    for (Requests::iterator iter = requests[eType - _FT_AIR_BEGIN].begin(); iter != requests[eType - _FT_AIR_BEGIN].end(); ++iter) { if (iter->second.nResistanceCellNumber != -1) delRequest.push_back(iter->first); }
    for (auto iter = delRequest.begin(); iter != delRequest.end(); ++iter) CancelRequest(*iter, eType);
  }
...
bool CGeneralAirForce::EnumEnemy(CAIUnit *pEnemy) { return false; }

void CGeneralAirForce::DeleteAA(CAIUnit *pUnit) { antiAviation.erase(pUnit->GetUniqueId()); }
...
void CGeneralAirForce::SetAAVisible(CAIUnit *pUnit, const bool bVisible)
{
  ...
  {
    // create
    auto pEnemy = new CEnemyRememberer(SGeneralConsts::TIME_SONT_SEE_AA_BEFORE_FORGET);
...
bool CGeneralAirForceLaunchFighters::IsTimeToRun() const { return curTime >= timeToRun; }

void CGeneralAirForceLaunchFighters::Run() { pAirForce->LaunchFighters(nPlayer); }