...
void CMultiplayerInfo::Init()
{
  if (theDipl.IsNetGame())
  {
    ...
    eGameType = static_cast<CMapInfo::GAME_TYPE>(GetGlobalVar("Multiplayer.GameType", CMapInfo::TYPE_SINGLE_PLAYER));
    ...
    if (winConditions.nFlagScoreLimit != 0 && eGameType == CMapInfo::TYPE_FLAGCONTROL)
    {
      ...
      if (!theCheats.IsHistoryPlaying()) updater.AddFeedBack(SAIFeedBack(EFB_UPDATE_TEAM_F_L_AGS, 1));
    }
    if (winConditions.nKillScoreLimit != 0)
    {
      updater.AddFeedBack(SAIFeedBack(EFB_UPDATE_TEAM_F_R_AGS, 0));
      if (!theCheats.IsHistoryPlaying()) updater.AddFeedBack(SAIFeedBack(EFB_UPDATE_TEAM_F_R_AGS, 1));
    }
...
void CMultiplayerInfo::GameFinished(const EFeedBack eGameResult)
{
  if (!bNoWin)
  {
    ...
    if (!theCheats.IsHistoryPlaying()) updater.AddFeedBack(eGameResult);
    else
    {
      if (eGameResult == EFB_WIN) SetGlobalVar("temp.Multiplayer.Win.Partyname", theDipl.GetMyParty());
      else if (eGameResult == EFB_LOOSE) SetGlobalVar("temp.Multiplayer.Win.Partyname", 1 - theDipl.GetMyParty());
      else SetGlobalVar("temp.Multiplayer.Win.Partyname", 2);
...
void CMultiplayerInfo::CheckFlagPoints()
{
  ...
  if (static_cast<int>(flagPoints[nMyParty]) > static_cast<int>(flagPoints[nEnemyParty])) GameFinished(EFB_WIN);
  else if (static_cast<int>(flagPoints[nMyParty]) < static_cast<int>(flagPoints[nEnemyParty])) GameFinished(EFB_LOOSE);
}
...
void CMultiplayerInfo::CheckTroopPoints()
{
  ...
  if (static_cast<int>(troopsPoints[nMyParty]) > static_cast<int>(troopsPoints[nEnemyParty])) GameFinished(EFB_WIN);
  else if (static_cast<int>(troopsPoints[nMyParty]) < static_cast<int>(troopsPoints[nEnemyParty])) GameFinished(EFB_LOOSE);
}
...
void CMultiplayerInfo::CheckSabotageWinConditions()
{
  ...
  if (bFinishedByFlagCapture && bFinishedByTimeLimit) GameFinished(EFB_DRAW);
  else if (bFinishedByFlagCapture) GameFinished(nAttackingParty == theDipl.GetMyParty() ? EFB_WIN : EFB_LOOSE);
  else if (bFinishedByTimeLimit) GameFinished(nAttackingParty == theDipl.GetMyParty() ? EFB_LOOSE : EFB_WIN);
}
...
void CMultiplayerInfo::CheckFlagControlWinConditions()
{
           ...
           (flagPoints[0] >= winConditions.nFlagScoreLimit || flagPoints[1] >= winConditions.nFlagScoreLimit))
  {
    ...
    else if (bWeWin) GameFinished(EFB_WIN);
    else GameFinished(EFB_LOOSE);
  }
  // by fraglimit
  ...
  if (nFlagsAtTheMap != 0)
  {
    if (capturedByPartyFlags[theDipl.GetMyParty()].size() == nFlagsAtTheMap) GameFinished(EFB_WIN);
    else if (capturedByPartyFlags[1 - theDipl.GetMyParty()].size() == nFlagsAtTheMap) GameFinished(EFB_LOOSE);
  }
}

void CMultiplayerInfo::CheckWinConditions()
{
  if (eGameType == CMapInfo::TYPE_SABOTAGE) CheckSabotageWinConditions();
  else CheckFlagControlWinConditions();

  if (eGameType == CMapInfo::TYPE_SABOTAGE)
  {
    if (units.Size(0) == 0 && units.Size(1) == 0) GameFinished(EFB_DRAW);
    if (units.Size(1 - theDipl.GetMyParty()) == 0) GameFinished(EFB_WIN);
    if (units.Size(theDipl.GetMyParty()) == 0) GameFinished(EFB_LOOSE);
  }

  for (int i = 0; i < theDipl.GetNPlayers(); ++i) { if (theDipl.IsPlayerExist(i) && theDipl.GetDiplStatus(i, theDipl.GetMyNumber()) == EDI_ENEMY) return; }
...
void CMultiplayerInfo::Segment()
{
  if (theDipl.IsNetGame() && !bNoWin)
  {
    ...
    const float fPoints = SConsts::PLAYER_POINTS_SPEED * static_cast<float>(SConsts::AI_SEGMENT_DURATION) / 1000.0f;
    ...
    if (eGameType == CMapInfo::TYPE_SABOTAGE) { if (bCapturedByAttackingParty) timeOfFlagCaptured += SConsts::AI_SEGMENT_DURATION; }
  }
}
...
void CMultiplayerInfo::AddFlagPoints(const int nParty, const float fPoints)
{
  if (theDipl.IsNetGame() && eGameType == CMapInfo::TYPE_FLAGCONTROL && nParty != theDipl.GetNeutralParty() && !bNoWin)
  {
    ...
    if (eGameType == CMapInfo::TYPE_FLAGCONTROL) updater.AddFeedBack(SAIFeedBack(EFB_UPDATE_TEAM_F_L_AGS, (static_cast<int>(flagPoints[nParty]) << 2) | nParty));
...
void CMultiplayerInfo::UnitsKilled(const int nKillerPlayer, const float fUnitsPrice, const int nKilledUnitsPlayer)
{
  if (theDipl.IsNetGame() && theDipl.GetDiplStatus(nKillerPlayer, nKilledUnitsPlayer) == EDI_ENEMY)
  {
    ...
    updater.AddFeedBack(SAIFeedBack(EFB_UPDATE_TEAM_F_R_AGS, (static_cast<int>(troopsPoints[nKillerParty]) << 2) | nKillerParty));
  }
}
...
{
  if (eGameType == CMapInfo::TYPE_SABOTAGE) return 0;
  return winConditions.nTimeToCapture;
}

void CMultiplayerInfo::NoWin() { bNoWin = true; }