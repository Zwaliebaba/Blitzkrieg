...
void InitPathToPoint(const CVec3 &vPoint, const bool bSmooth, CAviation *pPlane)
{
  ...
  if (pFormation) { pFormation->SendAlongPath(new CPlanePath(CVec3(pFormation->GetCenter(), pPlane->GetZ()), vPoint)); }
  else
...
CPtr<CPlaneStatesFactory> CPlaneStatesFactory::pFactory = nullptr;

IStatesFactory *CPlaneStatesFactory::Instance()
{
  if (pFactory == nullptr) pFactory = new CPlaneStatesFactory();
...
IUnitState *CPlaneStatesFactory::ProduceState(class CQueueUnit *pObj, CAICommand *pCommand)
{
  ...
  auto pUnit = static_cast<CAviation *>(pObj);
  ...
  switch (cmd.cmdType)
  {
    ...
    case ACTION_COMMAND_PLANE_TAKEOFF_NOW:
    {
      ...
      auto pPatrol = static_cast<CPlanePatrolState *>(pUnit->GetState());
      pPatrol->TakeOff();
    ...
    break;
    case ACTION_MOVE_PLANE_LEAVE: { pResult = CPlaneLeaveState::Instance(pUnit, cmd.fNumber); }
    ...
    case ACTION_MOVE_SHTURMOVIK_PATROL:
      pResult = CPlaneShturmovikPatrolState::Instance(pUnit, cmd.vPos, static_cast<int>(cmd.fNumber));
      break;
...
IUnitState *CPlaneRestState::Instance(CAviation *_pPlane, float fHeight) { return new CPlaneRestState(_pPlane, fHeight == -1.0 ? _pPlane->GetZ() : fHeight); }
...
{
  if (pPlane && pPlane->IsValid() && pPlane->IsAlive()) return pPlane->GetCenter();
...
IUnitState *CPlaneScoutState::Instance(CAviation *pPlane, const CVec2 &point) { return new CPlaneScoutState(pPlane, point); }
...
{
  if (_WAIT_FOR_TAKEOFF != eState && theWeather.IsActive()) Escape(SUCAviation::AT_SCOUT);
  AnalyzeBSU();
  ...
  switch (eState)
  {
    case _WAIT_FOR_TAKEOFF:
      if (pPlane->GetNextCommand()) pPlane->SetCommandFinished();
    ...
    case EPSS_GOING_TO_GUARDPOINT:
      if (pPlane->GetCurPath()->IsFinished()) eState = EPSS_AIM_TO_NEXT_POINT;

      break;
    case EPSS_AIM_TO_NEXT_POINT:
      ToNextPoint();
      if (1 != GetNPoints()) pPlane->SendAcknowledgement(ACK_PLANE_REACH_POINT_START_ATTACK, true);
      eState = EPSS_GOTO_GUARDPOINT;
      break;
    case EPSS_ESCAPE:
      break;
...
CPlaneDeffensiveFire::CPlaneDeffensiveFire(class CAviation *pPlane)
  : pOwner(pPlane), timeLastBSUUpdate(0) { pShootEstimator = new CPlaneDeffensiveFireShootEstimator(pPlane); }
...
void CPlaneDeffensiveFire::AnalyzeBSU()
{
  ...
  // CRAP}
  {
    ...
    auto pStats = static_cast<const SMechUnitRPGStats *>(pOwner->GetStats());
    ...
    for (int i = 0; i < nGuns; ++i)
    {
      ...
      if (pTurret && !pGun->IsFiring())
      {
        ...
        {
          pShootEstimator->Reset(nullptr, true, 0);
          ...
          for (CPlanesIter planes; !planes.IsFinished(); planes.Iterate())
          {
            ...
            if (pOwner != pEnemy &&
                EDI_ENEMY == theDipl.GetDiplStatus(pOwner->GetPlayer(), pEnemy->GetPlayer())) { pShootEstimator->AddUnit(pEnemy); }
          }
          if (pShootEstimator->GetBestUnit()) pGun->StartEnemyBurst(pShootEstimator->GetBestUnit(), true);
        }
        else { pGun->StartEnemyBurst(pTurret->GetTracedUnit(), false); }
      }
    }
  }
}
...
CPlanePatrolState::CPlanePatrolState(CAviation *pPlane, const CVec2 &point)
  : nCurPointIndex(0), pPlane(pPlane) { AddPoint(point /* + pPlane->GetGroupShift() */); }
...
{
  pPlane->SetCommandFinished();
  theGroupLogic.UnitCommand(SAIUnitCmd(ACTION_MOVE_PLANE_LEAVE, static_cast<float>(nAviaType)), pPlane, false);
}
...
CAviation *CPlanePatrolState::FindBetterEnemiyPlane(CAviation *pEnemie, const float fRadius) const
{
  ...
  CPlanesIter planes;
  float Dist = 0;
  auto pStats = static_cast<const SMechUnitRPGStats *>(pPlane->GetStats());
  ...
  if (pBetter != nullptr && pBetter != pEnemie) { return pBetter; }
  return pEnemie;
}
...
IUnitState *CPlaneBombState::Instance(CAviation *pPlane, const CVec2 &point) { return new CPlaneBombState(pPlane, point); }
...
CPlaneBombState::CPlaneBombState(CAviation *_pPlane, const CVec2 &_point)
  : CPlanePatrolState(_pPlane, _point), CPlaneDeffensiveFire(_pPlane),
    eState(_WAIT_FOR_TAKEOFF), bDive(false),
    bDiveInProgress(false), fInitialHeight(_pPlane->GetZ()), fStartAttackDist(0.0f),
    fFormerVerticalSpeed(0.0f), bExitDive(false), timeOfStart(curTime) {}
...
void CPlaneBombState::Segment()
{
  ...
  switch (eState)
  {
    case _WAIT_FOR_TAKEOFF:
      ...
      break;
    case ECBS_ESTIMATE:
      // determine whether our plane is a Dive bomber and what distance to start bombing.
    {
      auto pStats = static_cast<const SMechUnitRPGStats *>(pPlane->GetStats());
      ...
      else// for strategic bombers
      {
        // determine by the speed of the bomber and the speed of the bomb's fall
        CVec3 vSpeed3;
        ...
        // calculate the correction for the length of the queue (so that the middle of the queue hits the target)
        // We assume that all bursts are the same length and all guns have the same number of rounds
      ...
      if (bDive && fabs2(pPlane->GetCenter() - GetPoint()) <= sqr(fStartAttackDist)) eState = ECBS_GAIN_HEIGHT;
      else eState = ECBS_APPROACH;
    }

    break;
    case ECBS_GAIN_HEIGHT:
      if (CPlaneSmoothPath::IsHeightOK(pPlane, fInitialHeight)) eState = ECBS_GAIN_DISTANCE;

      break;
    case ECBS_GAIN_DISTANCE:
      // if the distance to the bombing point is more than 2 aircraft turning radii
      // + bombing start distance - then start approaching the target, otherwise move away from the target.
    {
      auto pStats = static_cast<const SMechUnitRPGStats *>(pPlane->GetStats());
    ...
    {
      CVec2 vPlaneCenter;
      if (pPlane->GetPlanesFormation()) vPlaneCenter = pPlane->GetPlanesFormation()->GetCenter();
      else vPlaneCenter = pPlane->GetCenter();

      if (fabs2(vPlaneCenter - GetPoint()) <= sqr(fStartAttackDist))
      {
        CPlanesFormation *pFormation = pPlane->GetPlanesFormation();
        if (pFormation) eState = ECBS_WAIT_BOMBPOINT_REACH;
        else
        {
          ...
          else eState = ECBS_ATTACK_HORISONTAL;
        }
      }
    }
    ...
    case ECBS_WAIT_BOMBPOINT_REACH:
    {
      ...
      if (DirsDifference(GetDirectionByVector(vDirToBombPoint), pFormation->GetDir()) < 65535 / 4) { eState = ECBS_ATTACK_HORISONTAL; }
    }
    break;
    ...
    case ECBS_ATTACK_HORISONTAL:
    {
      ...
      {
        CBasicGun *pGun = pPlane->GetGun(i);
        if (pGun->GetNAmmo() != 0) { if (pGun->GetShell().trajectory == SWeaponRPGStats::SShell::TRAJECTORY_BOMB) { if (!pGun->IsFiring() && 0 == pGun->GetRestTimeOfRelax()) { pGun->StartPointBurst(pPlane->GetCenter(), false); } } }
      }
    ...
    case ECBS_ATTACK_DIVE:
    {
      ...
      // check that it's time to throw bombs and throw them
      bool bDropped = false;
      ...
      for (int i = 0; i < nGun; ++i)
      {
        ...
        if (pGun->GetShell().trajectory == SWeaponRPGStats::SShell::TRAJECTORY_BOMB)
        {
          if (fFormerVerticalSpeed < vHorVerSpeed.y)// pullout
          {
            // bomb attack.
            if (!pGun->IsFiring() && pGun->GetRestTimeOfRelax() == 0 && pGun->GetNAmmo() != 0)
            {
              ...
              // throw bombs when the non-deviated trajectory has passed the target point
              if (DirsDifference(
        ...
        else if (vHorVerSpeed.y < 0)// when descending - shoot from machine guns
        {
      ...
      if (bDropped) eState = ECBS_AIM_TO_NEXT_POINT;
    }
    ...
    case ECBS_AIM_TO_NEXT_POINT:
    {
      ...
      if (!bFiring) eState = ECBS_AIM_TO_NEXT_POINT_2;
    }
...
IUnitState *CPlaneParaDropState::Instance(CAviation *pPlane, const CVec2 &vPos) { return new CPlaneParaDropState(pPlane, vPos); }

CPlaneParaDropState::CPlaneParaDropState(CAviation *pPlane, const CVec2 &vPos)
  : CPlanePatrolState(pPlane, vPos), CPlaneDeffensiveFire(pPlane),
    eState(_WAIT_FOR_TAKEOFF), nSquadNumber(0), nDroppingSoldier(0),
    vLastDrop(VNULL2) {}

bool CPlaneParaDropState::CanDrop(const CVec2 &point)
{
  if (!theStaticMap.IsTileInside(AICellsTiles::GetTile(point))) return true;
  // checking the possibility of landing
  // 
...
{
  if (_WAIT_FOR_TAKEOFF != eState && PPDS_DROPPING != eState && theWeather.IsActive()) Escape(SUCAviation::AT_PARADROPER);
  ...
  switch (eState)
  {
    ...
    case PPDS_APPROACHNIG:
      if (pPlane->GetCurPath()->IsFinished()) eState = PPDS_PREPARE_TO_DROP;
    ...
    case PPDS_PREPARE_TO_DROP:
    {
      ...
      // const int nParadropScriptID = nPlaneScriptID == -1 ? 

      if (pPlane->GetPlayer() == theDipl.GetNeutralPlayer()) TryInterruptState(nullptr);
      else
      {
    ...
    case PPDS_TO_NEXT_POINT:
      if (nSquadNumber < theUnitCreation.GetNParadropers(pPlane->GetPlayer())) eState = PPDS_ESTIMATE;
      else Escape(SUCAviation::AT_PARADROPER);

      break;
    case PPDS_DROPPING:
      if (fabs2(vLastDrop - pPlane->GetCenter()) > sqr(SConsts::PLANE_PARADROP_INTERVAL))
      {
        // find the soldier who is sitting on the plane
        CPtr<CSoldier> pDropper = nullptr;
        ...
        else
        {
          ...
          // random scattering of parachutists.
          ...
          // calculate whether it is possible to land somewhere.
          bool bSafeLanding = CanDrop(vDropPoint);
          //
          if (bSafeLanding)
          {
            // throw this soldier away

            pDropper->SetFree();
            pDropper->SetNewCoordinates(CVec3(vDropPoint, pPlane->GetZ()));
            pDropper->GetState()->TryInterruptState(nullptr);
...
ETryStateInterruptResult CPlaneParaDropState::TryInterruptState(CAICommand *pCommand)
{
  if (IsValidObj(pSquad))
  {
    ...
    for (int i = 0; i < pSquad->Size(); ++i)
    {
      ...
      if (pSold->IsValid() && pSold->IsAlive() && pSold->IsInSolidPlace()) onboardSoldiers.push_back(pSold);
    }
    for (auto it = onboardSoldiers.begin(); it != onboardSoldiers.end(); ++it) (*it)->Disappear();
  }
  UnRegisterPoints();
  pPlane->SetCommandFinished();
...
const CVec2 CPlaneParaDropState::GetPurposePoint() const { return GetPoint(); }
...
IUnitState *CPlaneLeaveState::Instance(CAviation *_pPlane, const int nAviaType) { return new CPlaneLeaveState(_pPlane, nAviaType); }

CPlaneLeaveState::CPlaneLeaveState(CAviation *_pPlane, const int _nAviaType)
  : CPlaneDeffensiveFire(_pPlane), pPlane(_pPlane), nAviaType(_nAviaType), eState(EPLS_STARTING) {}
...
{
  if (pPlane && pPlane->IsValid() && pPlane->IsAlive()) return pPlane->GetCurPath()->GetFinishPoint();
  return CVec2(-1.0f, -1.0f);
}
...
IUnitState *CPlaneFighterPatrolState::Instance(CAviation *_pPlane, const CVec2 &point) { return new CPlaneFighterPatrolState(_pPlane, point); }

class CAIUnit *CPlaneFighterPatrolState::GetTargetUnit() const { return pEnemie; }

bool CPlaneFighterPatrolState::IsAttacksUnit() const { return IsValidObj(pEnemie); }
...
CPlaneFighterPatrolState::CPlaneFighterPatrolState(CAviation *_pPlane, const CVec2 &vPoint)
  : CPlanePatrolState(_pPlane, vPoint), CPlaneDeffensiveFire(_pPlane),
    eState(_WAIT_FOR_TAKEOFF), fPatrolHeight(_pPlane->GetZ()),
    timeOfStart(curTime), timeOfLastPathUpdate(0), timeLastCheck(curTime)
{
  auto pStats = static_cast<const SMechUnitRPGStats *>(_pPlane->GetStats());
...
void CPlaneFighterPatrolState::Segment()
{
  ...
  if (_WAIT_FOR_TAKEOFF != eState)
  {
    if (ECFS_ESCAPE != eState && (curTime - timeOfStart > SConsts::FIGHTER_PATROL_TIME || theWeather.IsActive())) Escape(SUCAviation::AT_FIGHTER);
    ...
    if (ECFS_ESCAPE != eState && timeLastCheck - curTime > SConsts::TIME_QUANT)
    {
      ...
      if (!bAmmoRemains) Escape(SUCAviation::AT_FIGHTER);
    }
  }
  ...
  switch (eState)
  {
    ...
    case ECFS_FIND_ENEMY_OR_NEXT_POINT:
      if (pEnemie = FindNewEnemyPlane(fPartolRadius)) eState = ECFS_ENGAGE_TARGET;
      else eState = ECFS_AIM_TO_NEXT_POINT;

      break;
    case ECFS_GOING_TO_GUARDPOINT:
      if (pEnemie = FindNewEnemyPlane(fPartolRadius)) eState = ECFS_ENGAGE_TARGET;
      else if (pPlane->GetCurPath()->IsFinished()) eState = ECFS_AIM_TO_NEXT_POINT;
    ...
    case ECFS_ENGAGE_TARGET:
      if (!IsEnemyAlive(pEnemie) ||
          fabs2(pPlane->GetCenter() - GetPoint()) > sqr(fPartolRadius)) { eState = ECFS_FIND_ENEMY_OR_NEXT_POINT; }
      ...
      // enemie not killed yet
      {
        ...
        {
          CBasicGun *pGun = pPlane->GetGun(i);
          // attack with cannons only
          if (!pGun->IsFiring() &&
              pGun->GetRestTimeOfRelax() == 0 &&
              pGun->CanShootWOGunTurn(pEnemie, 1)) { pGun->StartPlaneBurst(pEnemie, false); }
        }
      }
...
void CPlaneFighterPatrolState::TryInitPathToEnemie(bool isEnemieNew)
{
  ...
  {
    // time it takes the fighter to reach the current center of the bomber (1 approximation)
...
void CPlaneFighterPatrolState::TryInitPathToPoint(const CVec3 &v, bool isNewPoint)
{
  ...
  if (isNewPoint || bByTime || pPlane->GetCurPath()->IsFinished())
  {
    ...
    if (fabs2(plCenter3 - v) > (SConsts::TILE_SIZE * SConsts::TILE_SIZE)) InitPathToPoint(v, false, pPlane);
  }
}
...
{
  if (!pTry->IsValid() || !pTry->IsAlive()) return bFire;

  const CVec2 &vSpeed = pTry->GetSpeed();

  // by the time the bomb falls, the unit must be in the bomb scattering circle
  if (fabs2(pTry->GetCenter() + pTry->GetSpeed() * fFlyTime - vCenter) < sqr(fDisp))
  {
    if (pTry->GetStats()->IsInfantry()) ++nInfantry;
    else if (!pTry->GetStats()->IsAviation()) ++nMechUnits;
  }
  if (nInfantry >= SConsts::MIN_INFANTRY_TO_DROP_BOMBS || nMechUnits >= SConsts::MIN_MECH_TO_DROP_BOMBS) bFire = true;
  return bFire;
}
...
{
  if (pEnemy) pEnemy->UpdateTakenDamagePower(-fTakenDamage);
  pEnemy = nullptr;
...
{
  if (IsValidUnit()) return pGun->CanShootWOGunTurn(pEnemy, 1);
  if (IsValidBuilding()) return pGun->CanShootToPointWOMove(GetCenter(), GetZ());
...
void CPlaneShturmovikPatrolState::CEnemyContainer::StartBurst(class CBasicGun *pGun)
{
  ...
  if (IsValidBuilding()) pGun->StartPointBurst(GetCenter(), false);
}

float CPlaneShturmovikPatrolState::CEnemyContainer::GetZ() const
{
  if (IsValidUnit()) return pEnemy->GetZ();
  if (IsValidBuilding()) return 0;
...
{
  if (IsValidUnit()) return pEnemy->GetCenter();
  if (IsValidBuilding()) return pBuilding->GetAttackCenter(pOwner->GetCenter());
  NI_ASSERT_T(false, "asked invalid target about attack center");
  return VNULL2;
}

CAIUnit *CPlaneShturmovikPatrolState::CEnemyContainer::GetEnemy() { return IsValidUnit() ? pEnemy : nullptr; }

CBuilding *CPlaneShturmovikPatrolState::CEnemyContainer::GetBuilding() { return IsValidBuilding() ? pBuilding : nullptr; }
...
bool CPlaneShturmovikPatrolState::CEnemyContainer::IsValid() const { return IsValidUnit() || IsValidBuilding(); }

// **********************************************************************
// * CPlaneShturmovikPatrolState *
// **********************************************************************

IUnitState *CPlaneShturmovikPatrolState::Instance(CAviation *_pPlane, const CVec2 &vPoint, const int /* EGunplaneCalledAs */ _eCalledAs) { return new CPlaneShturmovikPatrolState(_pPlane, vPoint, _eCalledAs); }
...
CPlaneShturmovikPatrolState::CPlaneShturmovikPatrolState(CAviation *_pPlane, const CVec2 &vPoint, const int _eCalledAs)
  : CPlanePatrolState(_pPlane, vPoint),
    ...
    fStartAttackDist(0.0f),
    fFinishAttckDist(0.0f), fTurnRadius(0.0f), eCalledAs(static_cast<EGunplaneCalledAs>(_eCalledAs))
{
  auto pStats = static_cast<const SMechUnitRPGStats *>(pPlane->GetStats());
...
void CPlaneShturmovikPatrolState::Segment()
{
  ...
  if (_WAIT_FOR_TAKEOFF != eState)
  {
        ...
        timeLastCheck - curTime > SConsts::TIME_QUANT)
    {
      ...
      if (!bAmmoRemains) { eState = PSPS_ESCAPE; }
    }

    if (PSPS_ESCAPE != eState &&
        PSPS_GAIN_HEIGHT != eState &&
        (curTime - timeOfStart > SConsts::FIGHTER_PATROL_TIME || theWeather.IsActive())) { eState = PSPS_ESCAPE; }
  }
  ...
  switch (eState)
  {
    ...
    case PSPS_FIND_ENEMY_OR_NEXT_POINT:
      if (FindNewEnemie()) { eState = PSPS_APPROACH_TARGET; }
      else { eState = PSPS_AIM_TO_NEXT_POINT; }

      break;
    case PSPS_GOING_TO_GUARDPOINT:
      if (FindNewEnemie()) { eState = PSPS_APPROACH_TARGET; }
      else if (pPlane->GetCurPath()->IsFinished()) { eState = PSPS_AIM_TO_NEXT_POINT; }
    ...
    case PSPS_APPROACH_TARGET:
      if (!enemie.IsValid()) { eState = PSPS_FIND_ENEMY_OR_NEXT_POINT; }
      else
      {
        const CVec2 vPoint(pPlane->GetCenter() + pPlane->GetDirVector() * SConsts::SHTURMOVIK_APPROACH_RADIUS);
    ...
    case PSPS_APPROACHING_TARGET_TOWARS_IT:
      if (!enemie.IsValid() ||
          fabs2(enemie.GetCenter() - GetPoint()) > sqr(SConsts::PLANE_GUARD_STATE_RADIUS * 2.0f)) { eState = PSPS_FIND_ENEMY_OR_NEXT_POINT; }
      else
      {
        ...
        // approached attack range
        CVec2 vEnemy(enemie.GetCenter());
        const CVec2 vDiff(vEnemy - pPlane->GetCenter());
        const float fDiff = fabs2(vDiff);
        if (fDiff <= sqr(fStartAttackDist) &&// it's time to start the attack
            fDiff > sqr(fFinishAttckDist) &&// it's not time to go out yet
            CPlaneSmoothPath::IsHeightOK(pPlane, fPatrolHeight))
        {
          if (1500 >= DirsDifference(GetDirectionByVector(vDiff), pPlane->GetDir()))
          {
            ...
            // cheating. 
            if (enemie.IsValidUnit()) enemie.GetEnemy()->SendAcknowledgement(ACK_BEING_ATTACKED_BY_AVIATION, true);
          }
        }
        else if (fDiff < sqr(fStartAttackDist)) { eState = PSPS_FIND_ENEMY_OR_NEXT_POINT; }
      }

      break;
    case PSPS_GAIN_HEIGHT:
      if (CPlaneSmoothPath::IsHeightOK(pPlane, fPatrolHeight)) { eState = PSPS_FIND_ENEMY_OR_NEXT_POINT; }
    ...
    case PSPS_ENGAGING_TARGET:
    {
      ...
      // to disallow switching to other target, while current ins'n dead
      if (!enemie.IsValid())
      {
        // find an easier target and attack
        CAIUnit *pNewEnemy = FindEnemyInFiringSector();
        if (pNewEnemy) enemie.SetEnemy(pNewEnemy);
        else pNewEnemy = FindEnemyInPossibleDiveSector();
        if (pNewEnemy) enemie.SetEnemy(pNewEnemy);
      }
    }
    ...
    case PSPS_FIRE_TO_WORLD:
      if (pPlane->GetSpeedHorVer().y < 0) TryBurstAllGunsToPoints();
      else { eState = PSPS_GAIN_HEIGHT; }
...
{
  if (pPlane->GetSpeedHorVer().y >= 0) return;
  ...
  for (int i = 0; i < nGun; ++i)
  {
    ...
    if (pGun->GetShell().trajectory != SWeaponRPGStats::SShell::TRAJECTORY_BOMB &&
        ...
        !pGun->GetTurret() &&
        fRange <= pGun->GetFireRangeMax()) { pGun->StartPointBurst(vShoot, true); }
  }
}
  ...
  for (int i = 0; i < nGun; ++i)
  {
    ...
    if (pGun->GetShell().trajectory == SWeaponRPGStats::SShell::TRAJECTORY_BOMB)
    {
          ...
          pGun->GetNAmmo() != 0)
      {
        ...
        // count the total amount of HP within the scatter radius

        // During the flight, the unit will travel so far at maximum speed
        ...
        // if it is more than critical, then throw it
        if (est.NeedDrop()) { pGun->StartPointBurst(vTrajFinish, false); }
      }
    }
  }
}
  ...
  for (int i = 0; i < nGun; ++i)
  {
    ...
    if (pGun->GetShell().trajectory != SWeaponRPGStats::SShell::TRAJECTORY_BOMB)
    {
      ...
      )
      {
        ...
        // Let's check whether the cannon can fire at a vertical angle.
...
CAIUnit *CPlaneShturmovikPatrolState::FindEnemyInFiringSector()
{
  ...
  // find the bending of the barrel honestly
  const int nGuns = pPlane->GetNGuns();
  float fGnutost = 0;
  float fDisp = 0;
  for (int i = 0; i < nGuns; ++i)
  {
    CBasicGun *pGun = pPlane->GetGun(i);
    if (pGun->GetNAmmo() != 0) { if (pGun->GetShell().trajectory != SWeaponRPGStats::SShell::TRAJECTORY_BOMB) fGnutost = Max(fGnutost, static_cast<float>(pGun->GetWeapon()->wDeltaAngle * 2.0f * PI / 65535)); }
  }
  ...
  {
    CAIUnit *pUnit = *iter;
    if (pUnit->IsValid() && pUnit->IsAlive() && pUnit->IsVisible(pPlane->GetParty())) pShootEstimator->AddUnit(pUnit);
  }
...
CAIUnit *CPlaneShturmovikPatrolState::FindEnemyInPossibleDiveSector()
{
  ...
  // find bendability and maximum range
  const int nGuns = pPlane->GetNGuns();
  float fGnutost = 0;
  float fMaxRange = 0;
  for (int i = 0; i < nGuns; ++i)
  {
    ...
    if (pGun->GetNAmmo() != 0)
    {
      if (pGun->GetShell().trajectory != SWeaponRPGStats::SShell::TRAJECTORY_BOMB)
      {
        fGnutost = Max(fGnutost, static_cast<float>(pGun->GetWeapon()->wDeltaAngle * 2.0f * PI / 65535));
...
IUnitState *CPlaneFlyDeadState::Instance(CAviation *pPlane) { return new CPlaneFlyDeadState(pPlane); }

CPlaneFlyDeadState::CPlaneFlyDeadState(CAviation *_pPlane)
  : eState(EPDS_START_DIVE), pPlane(_pPlane), fHeight(0.0f), bFatality(false),
    bExplodeInstantly(true), timeStart(curTime)
{
  auto pStats = static_cast<const SMechUnitRPGStats *>(pPlane->GetStats());
  bFatality = !pStats->szEffectFatality.empty();
  if (bFatality) bExplodeInstantly = Random(1);
  if (!bExplodeInstantly) timeStart = curTime + Random(0, SConsts::DIVE_BEFORE_EXPLODE_TIME);

  pPlane->SetPlanesFormation(nullptr, VNULL2);
...
void CPlaneFlyDeadState::CDeadZone::AdjustEscapePoint(CVec2 *pPoint)
{
  ...
  if (fXDiff < fYDiff)
  {
    if (fXMaxDiff > fXMinDiff) pPoint->x = fMinX;
    else pPoint->x = fMaxX;
  }
  else
  {
    if (fYMaxDiff > fYMinDiff) pPoint->y = fMinY;
    else pPoint->y = fMaxY;
  }
}

bool CPlaneFlyDeadState::CDeadZone::IsInZone(const CVec2 &vPoint) { return vPoint.x < fMinX || vPoint.x > fMaxX || vPoint.y < fMinY || vPoint.y > fMaxY; }
...
void CPlaneFlyDeadState::Segment()
{
  switch (eState)
  {
    ...
    case EPDS_WAIT_FINISH_PATH:
      if (deadZone.IsInZone(pPlane->GetCenter())) { pPlane->Disappear(); }
      break;
  }
}
