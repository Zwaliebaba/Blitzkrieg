...
// ///////////////////////////////////////////////////////////// 
bool IsCellInExcludingCircle(const CVec2 &vCellCenter, const CCircle &circle) { return fabs2(vCellCenter - circle.center) <= sqr(circle.r + SGeneralConsts::RESISTANCE_CELL_SIZE * SConsts::TILE_SIZE / 3); }
// ///////////////////////////////////////////////////////////// 
...
const float GetCoeffByDeltaTime(const float fDeltaTime, const float fUpperBound, bool bNeedDeinstall, bool bCanMove)
{
  ...
  if (bNeedDeinstall) fCoeff = Min(1.0f, fCoeff * 1.2f);
  if (!bCanMove) fCoeff = Min(1.0f, fCoeff * 1.1f);
...
const float GetWeightByVisiblePos(CAIUnit *pUnit, const NTimer::STime deltaTime)
{
  ...
  if (deltaTime > fTimeToForgetUnit) return 0.0f;
  // coefficient providing the probability that the unit left
...
const float GetWeightByAntiArtPos(CAIUnit *pUnit, const NTimer::STime deltaTime, const float fDistToLastVisibleAntiArt)
{
  ...
  if (deltaTime > fTimeToForget) return 0.0f;
  // coefficient providing the probability that the unit left
  const float fTimeCoeff = GetCoeffByDeltaTime(deltaTime, fTimeToForget, pUnit->NeedDeinstall(), pUnit->CanMove());
  // coefficient responsible for the accuracy of the anti-artillery circle
...
void CResistancesContainer::AddCell(const int nCell, const SSellInfo &cell)
{
  ...
  if (IsCellExcluded(SResistance::GetResistanceCellCenter(nCell))) cellsWeights[nCell].bAllowShoot = false;
}
    ...
    const NTimer::STime lastAntiArtTimeDelta, const CVec2 &vLastVisibleAntiArtCenter, const float fDistToLastVisibleAntiArt)
{
      ...
      (vLastVisiblePos != VNULL2 || fDistToLastVisibleAntiArt != -1.0f))
  {
    ...
    if (cellsWeights.find(nLastCell) != cellsWeights.end())
    {
      ...
      if (cellsWeights[nLastCell].fCellWeight != 0.0f) resistances.insert(SResistance(nLastCell, cellsWeights[nLastCell].fCellWeight));
    }

    CVec2 vNewPos;
    float fNewWeight;
    // count by visible position
    if (lastVisibleTimeDelta <= lastAntiArtTimeDelta || fDistToLastVisibleAntiArt == -1.0f)
    {
      ...
      if (fNewWeight != 0.0f) GetSingleton<IScene>()->GetStatSystem()->UpdateEntry("General: visible enemies", "noticed");
    }
    // We count according to the position of the circle of anti-artillery struggle
    else
    {
      ...
      if (fNewWeight != 0.0f) GetSingleton<IScene>()->GetStatSystem()->UpdateEntry("General: antiartillery circles", "noticed");
    }
    ...
    else AddCell(nNewCell, SSellInfo(pInfo->GetWeight(), false, true));

    if (cellsWeights[nNewCell].fCellWeight != 0.0f) resistances.insert(SResistance(nNewCell, cellsWeights[nNewCell].fCellWeight));
  }
...
// ///////////////////////////////////////////////////////////// 
void CResistancesContainer::SetCellInUse(const int nResistanceCellNumber, bool bInUse) { if (cellsWeights.find(nResistanceCellNumber) != cellsWeights.end()) cellsWeights[nResistanceCellNumber].bInUse = bInUse; }
// ///////////////////////////////////////////////////////////// 
void CResistancesContainer::RemoveExcluded(const CVec2 &vCenter)
{
  ...
  while (it != excluded.end())
  {
    ...
    {
      for (CCellsWeights::iterator iter = cellsWeights.begin(); iter != cellsWeights.end(); ++iter) { if (IsCellInExcludingCircle(SResistance::GetResistanceCellCenter(iter->first), circle)) iter->second.bAllowShoot = true; }
...
// ///////////////////////////////////////////////////////////// 
void CResistancesContainer::AddExcluded(const CVec2 &vCenter, const float fRadius) { excluded.push_back(CCircle(vCenter, fRadius)); }
// ///////////////////////////////////////////////////////////// 
bool CResistancesContainer::IsCellExcluded(const CVec2 &vCellCenter)
{
  for (std::list<CCircle>::const_iterator it = excluded.begin(); it != excluded.end(); ++it)
    if (IsCellInExcludingCircle(vCellCenter, *it)) return true;
  return false;
}
...
{
  auto iter = excluded.begin();
  while (iter != excluded.end() && fabs2(vCenter - iter->center) >= sqr(iter->r)) ++iter;
...
// **********************************************************************
// ///////////////////////////////////////////////////////////// 
void CResistancesContainer::CIter::IterateToNotInUse() { while (iter != pContainter->resistances.end() && pContainter->IsInUse(iter->GetCellNumber())) ++iter; }
// ///////////////////////////////////////////////////////////// 
