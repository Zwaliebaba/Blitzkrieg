...
{
  if (groupUnits.find(pObj->GetUniqueId()) != groupUnits.end()) return groupUnits.find(pObj->GetUniqueId())->second;
...
void CScripts::Init(const SLoadMapInfo &mapInfo)
{
  ...
  for (auto iter = mapInfo.reservePositionsList.begin(); iter != mapInfo.reservePositionsList.end(); ++iter)
  {
...
void CScripts::InitAreas(const SScriptArea scriptAreas[], const int nLen) { for (int i = 0; i < nLen; ++i) areas[scriptAreas[i].szName] = scriptAreas[i]; }
...
bool CScripts::CanUnitLand(const SMapObjectInfo &mapObject, IObjectsDB *pIDB, const CVec2 &vShift)
{
  ...
  auto pStats = dynamic_cast<const SUnitBaseRPGStats *>(pIDB->GetRPGStats(pDesc));
  ...
  if (theStaticMap.CanUnitGo(pStats->nBoundTileRadius, AICellsTiles::GetTile(vCenter), pStats->aiClass))
  {
    ...
    {
      CAIUnit *pUnit = *iter;
      if ((!pStats->IsInfantry() || !pUnit->GetStats()->IsInfantry()) && unitRect.IsIntersected(pUnit->GetUnitRect())) return false;
    }
...
{
  CGDBPtr<SGDBObjectDesc> pDesc = pIDB->GetDesc(mapObject.szName.c_str());
  auto pStats = dynamic_cast<const SSquadRPGStats *>(pIDB->GetRPGStats(pDesc));
  ...
  for (auto iter = centers.begin(); iter != centers.end(); ++iter)
  {
    ...
    if (!CanUnitLand(soldierMapObject, pIDB)) return false;
  }
...
bool CScripts::CanLandWithShift(const SMapObjectInfo &mapObject, IObjectsDB *pIDB, CVec2 *pvShift)
{
  ...
  {
    *pvShift = VNULL2;
    if (CanUnitLand(mapObject, pIDB, *pvShift)) return true;
  ...
  else if (dynamic_cast<const SSquadRPGStats *>(pIDB->GetRPGStats(pDesc)) != nullptr)
  {
    *pvShift = VNULL2;
    if (CanFormationLand(mapObject, pIDB, *pvShift)) return true;
...
{
  CGDBPtr<SGDBObjectDesc> pDesc = pIDB->GetDesc(mapObject.szName.c_str());
  return (dynamic_cast<const SUnitBaseRPGStats *>(pIDB->GetRPGStats(pDesc)) != nullptr);
}
...
void CScripts::LandSuspendedReiforcements()
{
  if (curTime >= lastTimeToCheckSuspendedReinforcs + TIME_TO_CHECK_SUSPENDED_REINF)
  {
    ...
    if (reinforcsIter == suspendedReinforcs.end()) reinforcsIter = suspendedReinforcs.begin();
      ...
      if (reinforcsIter->mapObject.link.nLinkWith == 0)
      {
        ...
        if (CanLandWithShift(reinforcsIter->mapObject, pIDB, &vShift))
        {
          ...
          do
          {
            ...
            auto candreinforcsIter = suspendedReinforcs.begin();
...
void CScripts::Segment()
{
  // retrieve all script calls
  {
    ...
    bShowErrors = true;
    while (const char *pszCommand = pConsole->ReadASCII(CONSOLE_STREAM_SCRIPT)) { if (theCheats.IsPasswordOk() || strncmp(pszCommand, "Password(", 9) == 0) CallScriptFunction(pszCommand); }
  ...
  while (segmIter != activeScripts.end())
  {
    if (curTime - segmIter->second.lastUpdate >= segmIter->second.period)
    {
      ...
      if (bKill && name2script.find(name) != name2script.end()) KillActiveScript(name);
    }
    else ++segmIter;
  }
...
void CScripts::CallScriptFunction(const char *pszCommand)
{
  ...
  if (!szAnswer.empty()) pConsole->WriteASCII(CONSOLE_STREAM_CONSOLE, szAnswer.c_str(), 0xff00ff00);
}
...
void CScripts::DelInvalidBegin(const int targetId)
{
  if (groups.find(targetId) != groups.end())
  {
    while (!groups[targetId].empty())
    {
      ...
      if (pObj == nullptr || !pObj->IsValid() || dynamic_cast<CStaticObject *>(pObj) == nullptr && !pObj->IsAlive()) groups[targetId].pop_back();
      else break;
    }
    ...
    for (std::unordered_map<int, int>::iterator it = groupUnits.begin(); it != groupUnits.end(); ++it)
    {
      ...
      if (!pObj || !pObj->IsValid() || dynamic_cast<CStaticObject *>(pObj) == nullptr && !pObj->IsAlive()) nDeleted = nUniqueId;
      else break;
    }
...
void CScripts::DelInvalidUnits(const int scriptId)
{
  if (groups.find(scriptId) != groups.end())
  {
    ...
    while (iter != groups[scriptId].end())
    {
      ...
      if (pObj == nullptr || !pObj->IsValid() || dynamic_cast<CStaticObject *>(pObj) == nullptr && !pObj->IsAlive()) iter = groups[scriptId].erase(iter);
      else ++iter;
    }
    ...
    for (std::unordered_map<int, int>::iterator it = groupUnits.begin(); it != groupUnits.end(); ++it)
    {
      const int nUniqueId = it->first;
      CLinkObject *pObj = GetObjectByUniqueIdSafe<CLinkObject>(nUniqueId);
      if (!pObj || !pObj->IsValid() || dynamic_cast<CStaticObject *>(pObj) == nullptr && !pObj->IsAlive()) deleted.push_back(nUniqueId);
    }
...
int CScripts::GetCheckObjectsInScriptArea(const SScriptArea &area, const interface ICheckObjects &check)
{
  ...
  for (CStObjCircleIter<false> iter(area.center, fR); !iter.IsFinished(); iter.Iterate())
  {
    ...
    if (pObj->IsValid() && pObj->IsAlive() && check.IsGoodObj(pObj)) ++nResult;
  }

  return nResult;
}

void CScripts::OutScriptError(const char *pszString) { if (bShowErrors) GetSingleton<IConsoleBuffer>()->WriteASCII(CONSOLE_STREAM_CONSOLE, pszString, 0xffff0000, true); }
...
Script::SRegFunction CScripts::pRegList[] =
{
    ...
    {"GetNUnitsInCircle", GetNUnitsInCircle},
    {"LandReinforcement", LandReinforcement},
    {"Win", Win},
    {"Loose", Loose},
    {"Draw", Draw},
    {"GiveCommand", GiveCommand},
    {"Cmd", GiveCommand},
    {"GiveQCommand", GiveQCommand},
    {"QCmd", GiveQCommand},
    {"GetNUnitsInScriptGroup", GetNUnitsInScriptGroup},
    {"ShowActiveScripts", ShowActiveScripts},
    {"GetNUnitsInArea", GetNUnitsInArea},
    {"GetNScriptUnitsInArea", GetNScriptUnitsInArea},
    {"ChangeWarFog", ChangeWarFog},
    ...
    {"EnableAviation", EnableAviation},
    {"DiableAviation", DisableAviation},
    {"DisableAviation", DisableAviation},

    {"GetObjectHPs", GetObjectHPs},
    {"GetNUnitsInParty", GetNUnitsInParty},
    {"GetNUnitsInPartyUF", GetNUnitsInPartyUF},
    {"GetNUnitsInPlayerUF", GetNUnitsInPlayerUF},
    ...
    {"GetPartyOfUnits", GetPartyOfUnits},
    {"ReserveAviationForTimes", ReserveAviationForTimes},
    {"DamageObject", DamageObject},
    {"CallAssert", CallAssert},
    {"GetUnitState", GetUnitState},
    {"GetSquadInfo", GetSquadInfo},
    {"IsFollowing", IsFollowing},
    {"GetFrontDir", GetFrontDir},
    {"IsWarehouseConnected", IsWarehouseConnected},
    {"IsUnitUnderSupply", IsUnitUnderSupply},
    {"GetUnitMorale", GetUnitMorale},
    {"GetActiveShellType", GetActiveShellType},
    {"AskClient", AskClient},
    {"RandomFloat", RandomFloat},
    {"RandomInt", RandomInt},
    {"ChangeSelection", ChangeSelection},
    {"ReturnScriptIDs", ReturnScriptIDs},
    {"GetPlayersMask", GetPlayersMask},
    {"GetObjCoord", GetObjCoord},
    {"GetScriptAreaParams", GetScriptAreaParams},
    {"IsPlayerPresent", IsPlayerPresent},
    {"SwitchWeather", SwitchWeather},
    {"SwitchWeatherAutomatic", SwitchWeatherAutomatic},

    {"GetNUnitsInSide", GetNUnitsInSide},
    {"AddIronMan", AddIronMan},
    {"SetDifficultyLevel", SetDifficultyLevel},
    {"SetCheatDifficultyLevel", SetCheatDifficultyLevel},
    {"DeleteReinforcement", DeleteReinforcement},
    {"ViewZone", ViewZone},
    {"IsStandGround", IsStandGround},
    {"IsEntrenched", IsEntrenched},

    {"GetNAPFencesInScriptArea", GetNAPFencesInScriptArea},
    {"GetNAntitankInScriptArea", GetNAntitankInScriptArea},
    {"GetNFencesInScriptArea", GetNFencesInScriptArea},
    {"GetNTrenchesInScriptArea", GetNTrenchesInScriptArea},
    {"GetNMinesInScriptArea", GetNMinesInScriptArea},
    {"GetAviationState", GetAviationState},
    {"Password", Password},
    {"SetGameSpeed", SetGameSpeed},
    {"GetNUnitsOfType", GetNUnitsOfType},
...
int CScripts::GetNUnitsInCircle(struct lua_State *state)
{
  ...
  for (CUnitsIter<0, 2> iter(theDipl.GetNParty(nPlayer), EDI_FRIEND, center, fR); !iter.IsFinished(); iter.Iterate())
  {
    ...
    if (pUnit->IsValid() && pUnit->IsAlive() &&
        pUnit->GetPlayer() == nPlayer && !pUnit->GetStats()->IsAviation()) { if (fabs2((*iter)->GetCenter() - center) <= fR * fR) ++cnt; }
  }
  ...
  if (pScripts->groups.find(nScriptID) != pScripts->groups.end())
  {
    ...
    if (script.IsNumber(2))
    {
      ...
      for (std::list<CPtr<IUpdatableObj>>::iterator it = pScripts->groups[nScriptID].begin();
           it != pScripts->groups[nScriptID].end(); ++it) { if ((*it)->GetPlayer() == nPlayer) ++nNumber; }
    }
    else
    {
      for (std::list<CPtr<IUpdatableObj>>::iterator it = pScripts->groups[nScriptID].begin();
           it != pScripts->groups[nScriptID].end(); ++it) { if ((*it)->IsAlive()) ++nNumber; }
    }
  }
...
void CScripts::SetNewLinksToReinforcement(CReinfList *pReinf, std::unordered_map<int, int> *pOld2NewLinks)
{
  ...
  for (auto iter = pReinf->begin(); iter != pReinf->end(); ++iter)
  {
  ...
  for (auto iter = pReinf->begin(); iter != pReinf->end(); ++iter)
  {
    ...
    if (pOld2NewLinks->find(nLinkWith) != pOld2NewLinks->end()) iter->mapObject.link.nLinkWith = (*pOld2NewLinks)[nLinkWith];
  }
}
...
void CScripts::LandReinforcementWithoutLandCheck(CReinfList *pReinf, const CVec2 &vShift)
{
  ...
  for (auto iter = pReinf->begin(); iter != pReinf->end(); ++iter)
  {
    ...
    if (!pStats || !pStats->IsTransport())
    {
      ...
      if (pUnit)
      {
        if (auto pUnit1 = dynamic_cast<CCommonUnit *>(pUnit))
        {
          pUnit1->SetScenarioUnit(iter->pScenarioUnit);
          pUnits.push_back(pUnit1);
        }
        if (auto pAIUnit = dynamic_cast<CAIUnit *>(pUnit)) pAIUnit->SetScenarioStats();
      }
    }
    else transports.push_back(*iter);
  }

  // supply all the trucks
...
void CScripts::SendShowReinoforcementPlacementFeedback(std::list<CVec2> *pCenters)
{
  ...
  for (auto iter = pCenters->begin(); iter != pCenters->end(); ++iter) { if (vMostLeftCenter.x > iter->x) vMostLeftCenter = *iter; }

  int nUnits = 0;
  CVec2 vGroupCenter(VNULL2);

  auto iter = pCenters->begin();
...
int CScripts::LandReinforcement(struct lua_State *state)
{
  ...
  for (CReinfList::iterator iter = pScripts->reinforcs[nRein].begin(); iter != pScripts->reinforcs[nRein].end(); ++iter)
  {
    ...
    if (theDipl.IsPlayerExist(nPlayer))
    {
      ...
      //
      if (nPlayer == theDipl.GetMyNumber()) bSendAck = true;

      if (theCheats.IsHistoryPlaying() || nPlayer == theDipl.GetMyNumber()) centers.push_back(CVec2(iter->mapObject.vPos.x, iter->mapObject.vPos.y));
  ...
  if (bSendAck)
  {
    ...
    while (!centers.empty()) pScripts->SendShowReinoforcementPlacementFeedback(&centers);
  }
  ...
  // try¤ to land all the reinforcements¤
...
int CScripts::ReserveAviationForTimes(struct lua_State *pState)
{
  ...
  for (int i = 2; script.IsNumber(i); ++i) times.push_back(script.GetObject(i));
...
int CScripts::Win(struct lua_State *state)
{
  ...
  if (nParty == theDipl.GetMyParty()) updater.AddFeedBack(SAIFeedBack(EFB_WIN));
  else updater.AddFeedBack(SAIFeedBack(EFB_LOOSE));
...
int CScripts::ProcessCommand(struct lua_State *state, const bool bPlaceInQueue)
{
  ...
  // team building
  SAIUnitCmd command;
  command.cmdType = static_cast<EActionCommand>(static_cast<int>(script.GetObject(1)));
  bool bValid = true;
  int nPlayer = 0;
  switch (command.cmdType)
  {
    ...
    case ACTION_COMMAND_ATTACK_UNIT:
...

      {
        ...
        if (!pScripts->groups[targetId].empty()) command.pObject = pScripts->groups[targetId].back();
        else bValid = false;
      }
    ...
    case ACTION_COMMAND_ENTER:
...

      {
        ...
        if (!pScripts->groups[targetId].empty())
        {
          ...
          if (dynamic_cast<CBuilding *>(command.pObject.GetPtr())) command.fNumber = 0;
          else if (dynamic_cast<CEntrenchmentPart *>(command.pObject.GetPtr())) command.fNumber = 2;
          else
    ...
    case ACTION_COMMAND_CALL_SCOUT:
    {
      ...
      if (scriptId != -1)// run through all units and add them to the group
      {
        for (int i = theGroupLogic.BeginGroup(nGroup);
             i != theGroupLogic.EndGroup(); i = theGroupLogic.Next(i)) { pScripts->AddObjToScriptGroup(theGroupLogic.GetGroupUnit(i), scriptId); }
      }
  ...
  if (bValid)
  {
    ...
    else
    {
      ...
      if (scriptId != -1)// run through all units and add them to the group
      {
        for (int i = theGroupLogic.BeginGroup(nGroup); i != theGroupLogic.EndGroup(); i = theGroupLogic.Next(i)) pScripts->AddObjToScriptGroup(theGroupLogic.GetGroupUnit(i), scriptId);
      }
    }
  }
...
int CScripts::GiveCommand(struct lua_State *state) { return ProcessCommand(state, false); }

int CScripts::GiveQCommand(struct lua_State *state) { return ProcessCommand(state, true); }
...
int CScripts::GetNScriptUnitsInArea(struct lua_State *state)
{
  ...
  if (pScripts->groups.find(nScriptGroup) == pScripts->groups.end() || pScripts->groups[nScriptGroup].empty()) script.PushNumber(0);
  else
  {
    const SScriptArea &area = pScripts->areas[szName];
    int nResult = 0;
    if (area.eType == SScriptArea::EAT_CIRCLE)
    {
      // CRAP{ need to combine static objects and units, and get rid of dynamic_cast
      for (std::list<CPtr<IUpdatableObj>>::iterator iter = pScripts->groups[nScriptGroup].begin(); iter != pScripts->groups[nScriptGroup].end(); ++iter)
      {
        ...
        else if (CAIUnit *pUnit = dynamic_cast_ptr<CAIUnit *>(*iter)) { if (pUnit->GetUnitRect().IsIntersectCircle(area.center, area.fR)) ++nResult; }
      }
      // }CRAP
    }
    else
    {
      ...
      for (std::list<CPtr<IUpdatableObj>>::iterator iter = pScripts->groups[nScriptGroup].begin(); iter != pScripts->groups[nScriptGroup].end(); ++iter)
      {
        ...
        else if (CAIUnit *pUnit = dynamic_cast_ptr<CAIUnit *>(*iter)) { if (pUnit->GetUnitRect().IsIntersected(areaRect)) ++nResult; }
      }
      // }CRAP
    }
...
int CScripts::God(struct lua_State *state)
{
  ...
  // nMode = 0 - disable god mode completely
  // nMode = 1 - indestructible
  // nMode = 2 - indestructibility and endless ammo
  // nMode = 3 - infinite ammo
  // nMode = 4 - remove only indestructibility
  // nMode = 5 - only remove infinite ammo

  switch (nMode)
  {
    case 0:
      theCheats.SetImmortals(nPlayer, 0);
      theCheats.SetFirstShoot(nPlayer, 0);
      break;
    case 1:
      theCheats.SetImmortals(nPlayer, 1);
      break;
    case 2:
      theCheats.SetImmortals(nPlayer, 1);
      theCheats.SetFirstShoot(nPlayer, 1);
      break;
    case 3:
      theCheats.SetFirstShoot(nPlayer, 1);
      break;
...
int CScripts::GetNUnitsInParty(struct lua_State *state)
{
  ...
  for (CGlobalIter iter(nParty, EDI_FRIEND); !iter.IsFinished(); iter.Iterate())
  {
    ...
    if (pUnit->IsValid() && pUnit->IsAlive())
    {
      ...
      if (!pState || pState->GetName() != EUSN_GUN_CREW_STATE) ++cnt;
    }
  }
  ...
  switch (pScript->GetTop())
  {
    case 1:
      *pResult = pStr;
      break;
    case 2:
      *pResult = NStr::Format(pStr, static_cast<float>(pScript->GetObject(2)));
      break;
    case 3:
      *pResult = NStr::Format(pStr, static_cast<float>(pScript->GetObject(2)), static_cast<float>(pScript->GetObject(3)));
      break;
    case 4:
      *pResult = NStr::Format(pStr, static_cast<float>(pScript->GetObject(2)), static_cast<float>(pScript->GetObject(3)), static_cast<float>(pScript->GetObject(4)));
      break;
    case 5:
      *pResult = NStr::Format(pStr, static_cast<float>(pScript->GetObject(2)), static_cast<float>(pScript->GetObject(3)), static_cast<float>(pScript->GetObject(4)),
                              static_cast<float>(pScript->GetObject(5)));
      break;
    case 6:
      *pResult = NStr::Format(pStr, static_cast<float>(pScript->GetObject(2)), static_cast<float>(pScript->GetObject(3)), static_cast<float>(pScript->GetObject(4)),
                              static_cast<float>(pScript->GetObject(5)), static_cast<float>(pScript->GetObject(6)));
      break;
    case 7:
      *pResult = NStr::Format(pStr, static_cast<float>(pScript->GetObject(2)), static_cast<float>(pScript->GetObject(3)), static_cast<float>(pScript->GetObject(4)),
                              static_cast<float>(pScript->GetObject(5)), static_cast<float>(pScript->GetObject(6)), static_cast<float>(pScript->GetObject(7)));
      break;
    case 8:
      *pResult = NStr::Format(pStr, static_cast<float>(pScript->GetObject(2)), static_cast<float>(pScript->GetObject(3)), static_cast<float>(pScript->GetObject(4)),
                              static_cast<float>(pScript->GetObject(5)), static_cast<float>(pScript->GetObject(6)), static_cast<float>(pScript->GetObject(7)), static_cast<float>(pScript->GetObject(8)));
      break;
    case 9:
      *pResult = NStr::Format(pStr, static_cast<float>(pScript->GetObject(2)), static_cast<float>(pScript->GetObject(3)), static_cast<float>(pScript->GetObject(4)),
                              static_cast<float>(pScript->GetObject(5)), static_cast<float>(pScript->GetObject(6)), static_cast<float>(pScript->GetObject(7)), static_cast<float>(pScript->GetObject(8)), static_cast<float>(pScript->GetObject(9)));
      break;
    case 10:
      *pResult = NStr::Format(pStr, static_cast<float>(pScript->GetObject(2)), static_cast<float>(pScript->GetObject(3)), static_cast<float>(pScript->GetObject(4)),
                              static_cast<float>(pScript->GetObject(5)), static_cast<float>(pScript->GetObject(6)), static_cast<float>(pScript->GetObject(7)), static_cast<float>(pScript->GetObject(8)), static_cast<float>(pScript->GetObject(9)), static_cast<float>(pScript->GetObject(10)));
      break;
    default:
      break;
  }
...
int CScripts::DisplayTrace(struct lua_State *state)
{
  ...
  const DWORD dwColor = GetGlobalVar(("Scene.Colors." + szSeasonName + ".Text.Chat.Color").c_str(), static_cast<int>(0xffffffff));
...
int CScripts::ChangeFormation(struct lua_State *state)
{
  ...
  for (std::list<CPtr<IUpdatableObj>>::iterator iter = pScripts->groups[nScriptID].begin(); iter != pScripts->groups[nScriptID].end(); ++iter) { if (CFormation *pFormation = dynamic_cast_ptr<CFormation *>(*iter)) { if (nGeometry < pFormation->GetNGeometries()) pFormation->ChangeGeometry(nGeometry); } }
...
int CScripts::GetNAmmo(struct lua_State *state)
{
  ...
  else
  {
    ...
    {
      if (pUnit->GetCommonGunStats(i).bPrimary) nMainAmmo += pUnit->GetNAmmo(i);
      else nSecondaryAmmo += pUnit->GetNAmmo(i);
    }
...
int CScripts::DamageObject(struct lua_State *pState)
{
  ...
  for (std::list<CPtr<IUpdatableObj>>::iterator iter = pScripts->groups[nScriptID].begin(); iter != pScripts->groups[nScriptID].end(); ++iter)
  {
    ...
    {
      if (fDamage == 0.0f) pObj->TakeDamage(pObj->GetHitPoints() * 2, true, theDipl.GetNeutralPlayer(), nullptr);
      else if (fDamage < 0.0f) pObj->SetHitPoints(pObj->GetHitPoints() - fDamage);
      else pObj->TakeDamage(fDamage, true, theDipl.GetNeutralPlayer(), nullptr);
    }
    else if (CAIUnit *pUnit = dynamic_cast<CAIUnit *>(pUpdatableObj))
    {
      if (fDamage == 0.0f) pUnit->TakeDamage(pUnit->GetHitPoints() * 2, nullptr, theDipl.GetNeutralPlayer(), nullptr);
      else if (fDamage < 0.0f) pUnit->IncreaseHitPoints(-fDamage);
      else pUnit->TakeDamage(fDamage, nullptr, theDipl.GetNeutralPlayer(), nullptr);
    }
    else if (CFormation *pFormation = dynamic_cast<CFormation *>(pUpdatableObj))
    {
      std::list<CSoldier *> soldiers;
      for (int i = 0; i < pFormation->Size(); ++i) soldiers.push_back((*pFormation)[i]);
      for (auto iter = soldiers.begin(); iter != soldiers.end(); ++iter)
      {
        CSoldier *pSoldier = *iter;

        if (fDamage == 0.0f) pSoldier->TakeDamage(pSoldier->GetHitPoints() * 2, nullptr, theDipl.GetNeutralPlayer(), nullptr);
        else if (fDamage < 0.0f) pSoldier->IncreaseHitPoints(-fDamage);
        else pSoldier->TakeDamage(fDamage, nullptr, theDipl.GetNeutralPlayer(), nullptr);
      }
    }
  }
...
int CScripts::GetUnitState(struct lua_State *pState)
{
  ...
  if (pScripts->groups.find(nScriptID) == pScripts->groups.end() || pScripts->groups[nScriptID].empty()) script.PushNumber(-1);
  else
  {
    ...
    if (pState == nullptr || !pState->IsValid()) script.PushNumber(0);
    else script.PushNumber(pState->GetName());
  }
...
int CScripts::GetSquadInfo(struct lua_State *pState)
{
  ...
  if (pScripts->groups.find(nScriptID) == pScripts->groups.end() || pScripts->groups[nScriptID].empty()) script.PushNumber(-3);
  else
  {
    IUpdatableObj *pObj = pScripts->groups[nScriptID].front();
    CFormation *pFormation = dynamic_cast<CFormation *>(pObj);
    if (pFormation == nullptr) script.PushNumber(-2);
    else if (pFormation->IsDisabled() || pFormation->Size() == 1 && (*pFormation)[0]->GetMemFormation() != nullptr) script.PushNumber(-1);
    else script.PushNumber(pFormation->GetCurGeometry());
  }
...
int CScripts::IsFollowing(struct lua_State *pState)
{
  ...
  else
  {
    IUpdatableObj *pObj = pScripts->groups[nScriptID].front();
    CCommonUnit *pUnit = dynamic_cast<CCommonUnit *>(pObj);
    if (pUnit == nullptr) script.PushNumber(-1);
    else if (pUnit->IsInFollowState()) script.PushNumber(1);
    else script.PushNumber(0);
  }
...
int CScripts::GetFrontDir(struct lua_State *pState)
{
  ...
  else
  {
    ...
    if (pUnit == nullptr) script.PushNumber(-1);
    else script.PushNumber(pUnit->GetFrontDir());
  }
...
int CScripts::IsWarehouseConnected(struct lua_State *pState)
{
  ...
  script.PushNumber(static_cast<int>(pStorage->IsConnected()));
...
int CScripts::RandomInt(struct lua_State *pState)
{
  ...
  if (n == 1) script.PushNumber(0);
  else script.PushNumber(Random(0, n - 1));
...
int CScripts::ChangeSelection(struct lua_State *pState)
{
  ...
  if (pScripts->groups.find(nScriptID) != pScripts->groups.end()) { for (std::list<CPtr<IUpdatableObj>>::iterator iter = pScripts->groups[nScriptID].begin(); iter != pScripts->groups[nScriptID].end(); ++iter) updater.Update(ACTION_NOTIFY_CHANGE_SELECTION, *iter, nParam); }
...
int CScripts::ReturnScriptIDs(struct lua_State *pState)
{
  ...
  for (int i = 1; i <= nReturns; ++i)
  {
    ...
    auto pObj = reinterpret_cast<IRefCount *>(nPtr);

    NI_ASSERT_T(dynamic_cast<IUpdatableObj*>(pObj) != 0, "Unknown object passed");
    auto pUpdatableObject = dynamic_cast<IUpdatableObj *>(pObj);

    const int nScriptID = pScripts->GetScriptID(pUpdatableObject);
    if (nScriptID != -1) selectedUnits.insert(nScriptID);
  }
...
int CScripts::GetPlayersMask(struct lua_State *pState)
{
  ...
  DWORD wMask = 0;
  for (int i = 0; i < theDipl.GetNPlayers(); ++i) { if (theDipl.IsPlayerExist(i)) wMask |= (1UL << i); }
...
int CScripts::GetObjCoord(struct lua_State *pState)
{
  ...
  {
    IUpdatableObj *pObj = pScripts->groups[nScriptID].front();
    if (CStaticObject *pStaticObj = dynamic_cast<CStaticObject *>(pObj)) vCenter = pStaticObj->GetCenter();
    else if (IBasePathUnit *pPathUnit = dynamic_cast<IBasePathUnit *>(pObj)) vCenter = pPathUnit->GetCenter();
  }
...
int CScripts::IsPlayerPresent(struct lua_State *pState)
{
  ...
  const int nPlayer = script.GetObject(1);
  script.PushNumber(theDipl.IsPlayerExist(nPlayer));
...
int CScripts::DeleteReinforcement(struct lua_State *pState)
{
  ...
  for (std::list<CPtr<IUpdatableObj>>::iterator it = pScripts->groups[nScriptID].begin();
       it != pScripts->groups[nScriptID].end(); ++it) { if (CCommonUnit *pUnit = dynamic_cast_ptr<CCommonUnit *>(*it)) pUnit->Disappear(); }
...
int CScripts::ViewZone(struct lua_State *pState)
{
  ...
  else { CHECK_ERROR(false, "ViewZone: can't perform in multiplayer game", 0); }
...
int CScripts::IsStandGround(struct lua_State *pState)
{
  ...
  int nAnswer = 0;
  if (pScripts->groups.find(nScriptID) != pScripts->groups.end() && !pScripts->groups[nScriptID].empty()) { if (CCommonUnit *pUnit = dynamic_cast_ptr<CCommonUnit *>(pScripts->groups[nScriptID].front())) nAnswer = pUnit->GetBehaviour().moving == SBehaviour::EMHoldPos; }
...
int CScripts::IsEntrenched(struct lua_State *pState)
{
  ...
  int nAnswer = 0;
  if (pScripts->groups.find(nScriptID) != pScripts->groups.end() && !pScripts->groups[nScriptID].empty()) { if (CAIUnit *pUnit = dynamic_cast_ptr<CAIUnit *>(pScripts->groups[nScriptID].front())) nAnswer = pUnit->IsInTankPit(); }
  script.PushNumber(nAnswer);
...
int CScripts::GetNMinesInScriptArea(struct lua_State *pState)
{
  ...
  {
  public:
    bool IsGoodObj(CExistingObject *pObj) const override { return pObj->GetObjectType() == ESOT_MINE; }
  };
...
int CScripts::GetNTrenchesInScriptArea(struct lua_State *pState)
{
  ...
  {
  public:
    bool IsGoodObj(CExistingObject *pObj) const override { return pObj->GetObjectType() == ESOT_ENTR_PART; }
  };
...
int CScripts::GetNFencesInScriptArea(struct lua_State *pState)
{
  ...
  {
  public:
    bool IsGoodObj(CExistingObject *pObj) const override { return pObj->GetObjectType() == ESOT_FENCE; }
  };
...
int CScripts::GetNAntitankInScriptArea(struct lua_State *pState)
{
  ...
  {
  public:
    bool IsGoodObj(CExistingObject *pObj) const override { return theUnitCreation.IsAntiTank(pObj->GetStats()); }
  };
...
int CScripts::GetNAPFencesInScriptArea(struct lua_State *pState)
{
  ...
  {
  public:
    bool IsGoodObj(CExistingObject *pObj) const override { return theUnitCreation.IsAPFence(pObj->GetStats()); }
  };
...
int CScripts::Password(struct lua_State *pState)
{
  ...
  if (script.IsString(1)) theCheats.CheckPassword(std::string(script.GetObject(1)));
