...
CHitInfo::CHitInfo(const class CExplosion *pExpl, IRefCount *_pVictim, const enum SAINotifyHitInfo::EHitType &_eHitType, const CVec3 &_explCoord)
  : pWeapon(pExpl->GetWeapon()), wShell(pExpl->GetShellType()), wDir(pExpl->GetAttackDir()),
    pVictim(_pVictim), explCoord(_explCoord), eHitType(_eHitType) { SetUniqueId(); }
...
{
  if (pUnit != nullptr) Init(pUnit, pWeapon, pWeapon->fDispersion, 1, explCoord, z, attackerPos, nShellType, bRandomize, pUnit->GetPlayer());
  else Init(pUnit, pWeapon, pWeapon->fDispersion, 1, explCoord, z, attackerPos, nShellType, bRandomize, theDipl.GetNeutralPlayer());
}
...
CExplosion::CExplosion(CAIUnit *pUnit, const CBasicGun *pGun, const CVec2 &explCoord, const float z, const CVec2 &attackerPos, const BYTE nShellType, const bool bRandomize)
{
  ...
  if (pUnit != nullptr) Init(pUnit, pGun->GetWeapon(), pGun->GetDispersion(), fDispRatio, explCoord, z, attackerPos, nShellType, bRandomize, pUnit->GetPlayer());
  else Init(pUnit, pGun->GetWeapon(), pGun->GetDispersion(), fDispRatio, explCoord, z, attackerPos, nShellType, bRandomize, theDipl.GetNeutralPlayer());
}
...
const SAINotifyHitInfo::EHitType CExplosion::ProcessExactHit(CAIUnit *pTarget, const SRect &combatRect, const CVec2 &explCoord, const int nRandPiercing, const int nRandArmor) const
{
  ...
  {
    // struck
    if (nRandPiercing >= nRandArmor && !pTarget->IsSavedByCover()) return SAINotifyHitInfo::EHT_HIT;
...
bool CExplosion::ProcessMoraleExplosion() const
{
  ...
  if (rShell.eDamageType == SWeaponRPGStats::SShell::DAMAGE_MORALE)
  {
    ...
    // lower morale for all enemies in radius
...
bool CExplosion::ProcessSmokeScreenExplosion() const
{
  ...
  {
    // large explosion radius - curtain radius,
    // nPiercing - transparency,
    // fDamage - lifetime
...
void CExplosion::AddHitToSend(CHitInfo *pHit)
{
  ...
  if (pHitToSend == nullptr || pHitToSend->eHitType != SAINotifyHitInfo::EHT_HIT) pHitToSend = pHit;
}
...
{
  if (pUnit && pUnit->GetZ() > GetExplZ()) nArmorDir = 2;
  else nArmorDir = 0;
}
...
const SAINotifyHitInfo::EHitType GetHitType(const CVec2 &vPoint)
{
  ...
  switch (eType)
  {
    case ETT_EARTH_TERRAIN:
      return SAINotifyHitInfo::EHT_GROUND;
    case ETT_EARTH_SEA_TERRAIN:
      return SAINotifyHitInfo::EHT_NONE;
    case ETT_RIVER_TERRAIN:
      return SAINotifyHitInfo::EHT_WATER;
  }
...
void CCumulativeExpl::Explode()
{
  ...
  for (CUnitsIter<0, 0> iter(0, ANY_PARTY, explCoord, 0.0f); !iter.IsFinished(); iter.Iterate())
  {
    ...
    {
      if (nShellType == SWeaponRPGStats::SShell::TRAJECTORY_LINE || nShellType == SWeaponRPGStats::SShell::TRAJECTORY_GRENADE) pTarget->Grazed(pUnit);

      // target is alive, target is not the one who shot and the height coincides with the height of the explosion
      if (!bSoldierHit || !pTarget->GetStats()->IsInfantry())
      {
        // so that a function call is not missed due to optimization of the calculation of bool expressions
        ...
        if (nShellType == SWeaponRPGStats::SShell::TRAJECTORY_LINE || nShellType == SWeaponRPGStats::SShell::TRAJECTORY_GRENADE)
        {
          ...
          if (IsValidObj(pWhoFire)) pWhoFire->WantedToReveal(pTarget);
        }
      }
    }
  }
  ...
  if (GetExplZ() == 0.0f)
  {
    // you cannot create 2 iterators on static objects inside ProcessCumulativeExpl
    // An iterator is needed, which means you can’t create an iterator here.
    ...
    for (CStObjCircleIter<false> iter(explCoord, 0); !iter.IsFinished(); iter.Iterate())
    {
      ...
      if (pObj->IsAlive()) hitObjects.push_back(pObj);
    }

    for (auto it = hitObjects.begin(); it != hitObjects.end(); ++it)
    {
      // so that a function call is not missed due to optimization of the calculation of bool expressions
  ...
  if (!bHit)
  {
    if (GetExplZ() <= 0) updater.Update(ACTION_NOTIFY_HIT, new CHitInfo(pWeapon, nShellType, attackDir, CVec3(GetExplCoordinates(), GetExplZ()), GetHitType(explCoord)));
    else updater.Update(ACTION_NOTIFY_HIT, new CHitInfo(pWeapon, nShellType, attackDir, CVec3(GetExplCoordinates(), GetExplZ()), SAINotifyHitInfo::EHT_AIR));
  }
  else if (pHitToSend != nullptr) updater.Update(ACTION_NOTIFY_HIT, pHitToSend);
}
...
CBurstExpl::CBurstExpl(CAIUnit *pUnit, const CBasicGun *pGun, const CVec2 &explCoord, const float z, const CVec2 &attackerPos, const BYTE nShellType, const bool bRandomize, int ArmorDir)
  : CExplosion(pUnit, pGun, explCoord, z, attackerPos, nShellType, bRandomize), nArmorDir(ArmorDir) { if (pWeapon->shells[nShellType].trajectory != SWeaponRPGStats::SShell::TRAJECTORY_LINE || (pUnit && pUnit->GetZ() > GetExplZ())) nArmorDir = 2; }
...
CBurstExpl::CBurstExpl(CAIUnit *pUnit, const SWeaponRPGStats *pWeapon, const CVec2 &explCoord, const float z, const CVec2 &attackerPos, const BYTE nShellType, const bool bRandomize, int ArmorDir)
  : CExplosion(pUnit, pWeapon, explCoord, z, attackerPos, nShellType, bRandomize), nArmorDir(ArmorDir) { if (pWeapon->shells[nShellType].trajectory != SWeaponRPGStats::SShell::TRAJECTORY_LINE || (pUnit && pUnit->GetZ() > GetExplZ())) nArmorDir = 2; }
...
void CBurstExpl::Explode()
{
  ...
  if (nArmorDir != 2) theHitsStore.AddHit(explCoord, CHitsStore::EHT_OPEN_SIGHT);
  else theHitsStore.AddHit(explCoord, CHitsStore::EHT_OVER_SIGHT);
  ...
  : explTime(_explTime), expl(_expl), nGun(_nGun)
{
  ...
  if (pWhoFire == nullptr) vOwnerCenter = expl->GetExplCoordinates();
  else vOwnerCenter = pWhoFire->GetCenter();
...
// ///////////////////////////////////////////////////////////// 
IRefCount *CShell::GetWhoFired() const { return expl->GetWhoFire(); }
// ///////////////////////////////////////////////////////////// 
// **********************************************************************
// *CVisShell*
// **********************************************************************
...
// ///////////////////////////////////////////////////////////// 
const bool CVisShell::IsVisibleByPlayer() const { return theCheats.IsHistoryPlaying() || bVisible; }
// ///////////////////////////////////////////////////////////// 
...
{
  float fRatio;
  if (curTime - timeDiff < GetStartTime()) fRatio = 0;
  else fRatio = static_cast<float>(curTime - timeDiff - GetStartTime()) / static_cast<float>(GetExplTime() - GetStartTime());

  if (pTraj->GetTrajType() == SWeaponRPGStats::SShell::TRAJECTORY_BOMB) return GetFinishVisZ() * fRatio;
...
{
  // detonate invisible projectiles
...
{
  while (!invisShells.empty()) invisShells.pop();
...
void CShellsStore::UpdateCheckSum(uLong *pCheckSum)
{
  ...
  for (auto iter = visShells.begin(); iter != visShells.end(); ++iter)
  {
...
CBombBallisticTraj::CBombBallisticTraj(const CVec3 &_point, const CVec3 &_v, const NTimer::STime &_explTime, const CVec2 &_vRandAcc)
  : point(_point), v(_v), vRandAcc(_vRandAcc),
    wDir(GetDirectionByVector(CVec2(_v.x, _v.y))), startTime(curTime), explTime(_explTime) {}
...
// ///////////////////////////////////////////////////////////// 
float CBombBallisticTraj::GetCoeff(const float &timeDiff) { return (1 - exp(-1.0f * SConsts::TRAJ_BOMB_ALPHA * timeDiff)) / SConsts::TRAJ_BOMB_ALPHA; }
// ///////////////////////////////////////////////////////////// 
float CBombBallisticTraj::GetTimeOfFly(const float fZ, const float fZSpeed) { return (sqrt(sqr(fZSpeed) + 2 * SConsts::TRAJECTORY_BOMB_G * fZ) + fZSpeed) / SConsts::TRAJECTORY_BOMB_G; }
// ///////////////////////////////////////////////////////////// 
// **********************************************************************
// *CFakeBallisticTraj*
// **********************************************************************
...
CFakeBallisticTraj::CFakeBallisticTraj(const CVec3 &_point, const CVec3 &_v, const NTimer::STime &_explTime, const float _A1, const float _A2)
  : startTime(curTime), explTime(_explTime), point(_point),
    v(_v), A1(_A1), A2(_A2), wDir(GetDirectionByVector(CVec2(_v.x, _v.y))) {}
...
CBallisticTraj::CBallisticTraj(const CVec3 &_vStart, const CVec2 &vFinish, float fV, const SWeaponRPGStats::SShell::ETrajectoryType _eType, WORD wMaxAngle, float fMaxRange)
  : vStart3D(_vStart), startTime(curTime), eType(_eType)
{
  ...
  else
  {
    ...
    // add speed if not enough
    if (fV < fCoeff + 0.001f) fV = fCoeff + 0.001f;
