...
// ///////////////////////////////////////////////////////////// 
CCommonStaticPath::CCommonStaticPath(const interface IStaticPathFinder &staticPathFinder, const CVec2 &finishPoint) { SetPath(staticPathFinder, staticPathFinder.GetPathLength(), finishPoint); }
// ///////////////////////////////////////////////////////////// 
CCommonStaticPath::CCommonStaticPath(const interface IStaticPathFinder &staticPathFinder, const int nLen, const CVec2 &finishPoint) { SetPath(staticPathFinder, nLen, finishPoint); }
// ///////////////////////////////////////////////////////////// 
void CCommonStaticPath::SetPath(const interface IStaticPathFinder &staticPathFinder, const int _nLen, const CVec2 &_finishPoint)
{
  ...
  if (path.size() < nLen) path.resize(nLen * 1.5);

  if (nLen > 0) staticPathFinder.GetStopTiles(&(path[0]), nLen);
  // initializing startTile and finishTile
  startTile = staticPathFinder.GetStartTile();
  finishTile = staticPathFinder.GetFinishTile();

  // finishPoint initialization
  if (!theStaticMap.IsPointInside(_finishPoint) || finishTile != AICellsTiles::GetTile(_finishPoint)) finishPoint = AICellsTiles::GetPointByTile(finishTile);
  else finishPoint = _finishPoint;
}
...
// ///////////////////////////////////////////////////////////// 
void CCommonStaticPath::MoveFinishPointBy(const CVec2 &vMove) { if (AICellsTiles::GetTile(finishPoint + vMove) == finishTile) finishPoint += vMove; }
// ///////////////////////////////////////////////////////////// 
// **********************************************************************
// *CStandartPath*
// **********************************************************************
...
void CStandartPath::SaveSmallPath(const int nToSave)
{
  ...
  if (nCurPathPoint > 0) --nCurPathPoint;
...
{
  // try to go to save relative position
  SVector nextTile;
  if (curStPathTile == pStPath->GetFinishTile()) nextTile = AICellsTiles::GetTile(finishPoint);
  else nextTile = AICellsTiles::GetTile(AICellsTiles::GetPointByTile(curStPathTile) + vShift);

  bool bFound = true;
  if (theStaticMap.CanUnitGo(nBoundTileRadius, nextTile, aiClass))
  {
    // path to a point with a length shift no greater than 4 * mDistance( pathPoint[nCurTile], nextTile )
    theStaticMap.MemMode();
    theStaticMap.SetMode(ELM_STATIC);
    const SVector goodTile =
        theStaticMap.CanUnitGo(nBoundTileRadius, pathPoints[nCurTile], aiClass) ? pathPoints[nCurTile] : lastKnownGoodTile;
    theStaticMap.RestoreMode();

    pPathFinder->SetPathParameters
        (
            nBoundTileRadius, aiClass, nullptr,
    ...
    if (pPathFinder->GetPathLength() == -1) bFound = false;
  }
  else bFound = false;
  ...
  // you can't go exactly to the last point tile
  if (curStPathTile == pStPath->GetFinishTile() && nextTile != pPathFinder->GetFinishTile()) finishPoint = AICellsTiles::GetPointByTile(pPathFinder->GetFinishTile());
...
{
  // the short path was too long
  if (bSmallPathTooLong) CalculateSmallPath(nCurStaticPoint == pStPath->GetLength());
  else if (nCurStaticPoint < pStPath->GetLength() || !bShift)
  {
    // move a point along a long path
    int shift;
    if (bShift) shift = Min(nCurStaticPoint + SConsts::BIG_PATH_SHIFT, pStPath->GetLength());
    else shift = nCurStaticPoint;

    // skip all locked tiles
    while (shift < pStPath->GetLength() && !theStaticMap.CanUnitGo(nBoundTileRadius, pStPath->GetTile(shift - 1), aiClass)) ++shift;

    if (shift > 0) curStPathTile = pStPath->GetTile(shift - 1);
...
CStandartPath::CStandartPath(const int _nBoundTileRadius, const BYTE _aiClass, IStaticPathFinder *_pPathFinder, interface IStaticPath *_pStPath, const CVec2 &startPoint, const CVec2 &finishPoint, const SVector &_lastKnownGoodTile)
  : pPathFinder(_pPathFinder), nBoundTileRadius(_nBoundTileRadius), aiClass(_aiClass), lastKnownGoodTile(_lastKnownGoodTile) { InitByStaticPath(_pStPath, startPoint, finishPoint); }

// ///////////////////////////////////////////////////////////// 
void CStandartPath::RecoverState(const CVec2 &point, const SVector &_lastKnownGoodTile)
{
  lastKnownGoodTile = _lastKnownGoodTile;
  pPathFinder->SetPathParameters(nBoundTileRadius, aiClass, nullptr, point, finishPoint, SConsts::INFINITY_PATH_LIMIT, true, lastKnownGoodTile);
  pPathFinder->CalculatePathWOCycles();
  pPathFinder->SmoothPath();
...
const CVec2 CStandartPath::PeekPoint(int nShift)
{
  ...
  {
    if (nShift < nInsertedTiles - nCurInsertedTile) return AICellsTiles::GetPointByTile(insertedTiles[nCurInsertedTile + nShift]);
  ...
  while (inc != nShift && nPoint != nCurPathPoint)
  {
    ...
    if (nPoint == nCurPathPoint && !CalculateNewPath(true)) break;
  }

  CVec2 result;
  // because when checking for the end of the path, pathPoints may change
  if (nPoint != nCurPathPoint) result = AICellsTiles::GetPointByTile(pathPoints[nPoint]);

  if (nPoint == nCurPathPoint || GetNextPos(nPoint) == nCurPathPoint && !CalculateNewPath(true)) return GetFinishPoint();
  return result;
}
...
void CStandartPath::Shift(int nShift)
{
  ...
  while (inc != nShift && nCurTile != nCurPathPoint)
  {
    ...
    if (nCurTile == nCurPathPoint && !CalculateNewPath(true)) break;
  }
}
...
{
  nInsertedTiles = 0;
  if (insertedTiles.size() < tiles.size()) insertedTiles.resize(tiles.size());

  nInsertedTiles = 0;
  for (auto iter = tiles.begin(); iter != tiles.end(); ++iter) insertedTiles[nInsertedTiles++] = *iter;

  nCurInsertedTile = 0;
}

// ///////////////////////////////////////////////////////////// 
const BYTE CStandartPath::GetNextPos(BYTE n) { return (++n == ENPathPoints) ? 0 : n; }
// ///////////////////////////////////////////////////////////// 
bool CStandartPath::CanGoBackward(interface IBasePathUnit *pUnit)
{
  return
      pUnit->CanGoBackward() && pStPath->GetLength() * static_cast<float>(SConsts::TILE_SIZE) <= pUnit->GetAABBHalfSize().y * 3.0f;
}
...
// ///////////////////////////////////////////////////////////// 
bool CStandartDirPath::IsFinished() const { return mDistance(curPoint, finishPoint) < 2; }
// ///////////////////////////////////////////////////////////// 
const CVec2 &CStandartDirPath::GetFinishPoint() const { return finishPoint; }
// ///////////////////////////////////////////////////////////// 
...
const CVec2 CStandartDirPath::PeekPoint(const int nShift)
{
  if (!IsFinished())
  {
    ...
    if ((finishPoint - res) * dir <= 0) return finishPoint;
    return res;
  }
  return finishPoint;
}
...
void CStandartDirPath::Shift(const int nShift)
{
  if (!IsFinished())
  {
    ...
    if ((finishPoint - curPoint) * dir <= 0) curPoint = finishPoint;
  }
}

// ///////////////////////////////////////////////////////////// 
bool CStandartDirPath::CanGoBackward(interface IBasePathUnit *pUnit) { return pUnit->CanGoBackward(); }
// ///////////////////////////////////////////////////////////// 