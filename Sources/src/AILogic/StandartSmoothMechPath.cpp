...
CStandartSmoothMechPath::CStandartSmoothMechPath()
  : pPath(nullptr), pUnit(nullptr), speed(0), bNotified(false), bMinSlowed(false), bMaxSlowed(false), bStopped(false),
    bSmoothTurn(false), nPoints(0), vLastValidatedPoint(VNULL2), bCanGoForward(true), bCanGoBackward(true) {}

void CStandartSmoothMechPath::SetOwner(IBasePathUnit *_pUnit) { pUnit = _pUnit; }

IBasePathUnit *CStandartSmoothMechPath::GetOwner() const { return pUnit; }
...
void CStandartSmoothMechPath::AddSmoothTurn()
{
  ...
  if (pUnit->GetDirVector() * vDir > pUnit->GetSmoothTurnThreshold())
  {
    ...
    {
      if (!theStaticMap.CanUnitGo(pUnit->GetBoundTileRadius(), bres.GetDirection(), pUnit->GetAIClass())) break;
...
bool CStandartSmoothMechPath::CheckTurn(const WORD wNewDir)
{
  ...
  // поворот небольшой
  if (wRightDirsDiff < SConsts::DIR_DIFF_TO_SMOOTH_TURNING || bCanBackward && wBackDirsDiff < SConsts::DIR_DIFF_TO_SMOOTH_TURNING) return true;
  ...
  if (nResult == -1 && bCanRotateForward && bCanRotateBackward) nResult = 1;
  if (nResult == -1 && !bCanBackward && bCanRotateForward) nResult = 1;
  if (nResult == -1 && bCanBackward && wBackDirsDiff < wRightDirsDiff && bCanRotateBackward) nResult = 1;
  ...
  if (nResult == -1 && pUnit->CheckToTurn(wNewDir)) nResult = 1;
...
bool CStandartSmoothMechPath::Init(IBasePathUnit *_pUnit, IPath *_pPath, bool _bSmoothTurn, bool bCheckTurn)
{
  ...
  {
    if (!pPath->CanGoBackward(pUnit)) pUnit->ForceGoByRightDir();
    ...
    if (pPath && bSmoothTurn && nInitSplineResult == SConsts::SPLINE_STEP) AddSmoothTurn();
  ...
  if (DirsDifference(GetDirectionByVector(spline.GetDX()), wOldPathDirection) > 20000) speed = 0.0f;
...
{
  if (fabs2(center - vLastValidatedPoint) >= sqr(static_cast<float>(SAIConsts::TILE_SIZE) / 2.0f))
  /*
      &&
    ...
    if (IsRectOnLockedTiles(unitRect, pUnit->GetAIClass()))
    {
      ...
      while (iter != tiles.end())
      {
        ...
        else
        {
          ...
          Normalize(&vDiff);
          if (vDiff * pUnit->GetDirVector() > 1 / sqrt(2.0f) || vDiff * pUnit->GetDirVector() < -1 / sqrt(2.0f)) iter = tiles.erase(iter);
          else ++iter;
        }
      }
...
bool CStandartSmoothMechPath::IsFinished() const { return bFinished || !pPath; }
...
const CVec2 CStandartSmoothMechPath::GetPointWithoutFormation(NTimer::STime timeDiff)
{
  ...
  while (!bFinished && fabs(spline.GetPoint() - pUnit->GetCenter()) < fRemain)
  {
    ...
    if (fabs(spline.GetPoint() - pUnit->GetCenter()) < fRemain && nIter >= CBSpline::N_OF_ITERATONS) InitSpline();
  }
  ...
  if (!bFinished)
  {
    ...
    if (fSplineSpeed < speed) speed = fSplineSpeed;
  ...
  if (spline.GetDX() != VNULL2)
  {
    ...
    else pUnit->UpdateDirection(spline.GetDX());
  }
  ...
  if (fDist <= sqr(4.0f * pUnit->GetAABBHalfSize().y))
  {
    ...
    if (pUnit->GetDir() == pUnit->GetFrontDir()) rect.InitRect(result, rect.dir, rect.lengthAhead * 1.5f, 0, rect.width);
    else rect.InitRect(result, rect.dir, 0, rect.lengthBack * 1.5f, rect.width);
  ...
  if (!bFinished) ValidateCurPath(pUnit->GetCenter(), result);
...
const CVec3 CStandartSmoothMechPath::GetPoint(NTimer::STime timeDiff)
{
  ...
  else vResult = GetPointWithoutFormation(timeDiff);
...
bool CStandartSmoothMechPath::CanGoBackward() const
{
  if (pPath != nullptr)
  {
    if (bCanGoBackward && !bCanGoForward) return true;
