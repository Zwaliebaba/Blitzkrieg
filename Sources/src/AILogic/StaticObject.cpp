...
bool CStaticObject::CheckStaticObject(const SObjectBaseRPGStats *pStats, const CVec2 &vPos, const int nFrameIndex)
{
  ...
  for (int y = 0; y < passability.GetSizeY(); ++y)
  {
    for (int x = 0; x < passability.GetSizeX(); ++x)
    {
      ...
      if (passability[y][x] == 1 && theStaticMap.IsLocked(tile, AI_CLASS_ANY)) { return false; }
    }
  }
  return true;
}

const BYTE CStaticObject::GetPlayer() const { return theDipl.GetNeutralPlayer(); }
...
const int CExistingObject::GetRandArmorByDir(const int nArmorDir, const WORD wAttackDir)
{
  switch (nArmorDir)
  {
    ...
    case 1:
      return GetStats()->GetRandomArmor(RPG_BOTTOM);
    case 2:
      return GetStats()->GetRandomArmor(RPG_TOP);
...
bool CExistingObject::ProcessCumulativeExpl(CExplosion *pExpl, const int nArmorDir, const bool bFromExpl)
{
  if (pExpl->GetExplZ() <= 0 && IsPointInside(pExpl->GetExplCoordinates()))
  {
    ...
    else pExpl->AddHitToSend(new CHitInfo(pExpl, this, SAINotifyHitInfo::EHT_REFLECT, CVec3(pExpl->GetExplCoordinates(), pExpl->GetExplZ())));
...
void CExistingObject::BurnSegment()
{
  ...
  if (curTime >= burningEnd) theStatObjs.EndBurning(this);
}
...
{
  auto pStats = static_cast<const SObjectBaseRPGStats *>(GetStats());
...
{
  auto pStats = static_cast<const SObjectBaseRPGStats *>(GetStats());
  return Max(0, static_cast<int>(center.x - pStats->GetVisOrigin(nFrameIndex).x + SConsts::TILE_SIZE / 2));
}

const int CGivenPassabilityStObject::GetVisUpX() const
{
  auto pStats = static_cast<const SObjectBaseRPGStats *>(GetStats());
  return Min(theStaticMap.GetSizeX() * SConsts::TILE_SIZE, static_cast<int>(center.x - pStats->GetVisOrigin(nFrameIndex).x + SConsts::TILE_SIZE * pStats->GetPassability(nFrameIndex).GetSizeX() + SConsts::TILE_SIZE / 2));
}

const int CGivenPassabilityStObject::GetVisDownY() const
{
  auto pStats = static_cast<const SObjectBaseRPGStats *>(GetStats());
  return Max(0, static_cast<int>(center.y - pStats->GetVisOrigin(nFrameIndex).y + SConsts::TILE_SIZE / 2));
}

const int CGivenPassabilityStObject::GetVisUpY() const
{
  auto pStats = static_cast<const SObjectBaseRPGStats *>(GetStats());
  return Min(theStaticMap.GetSizeY() * SConsts::TILE_SIZE, static_cast<int>(center.y - pStats->GetVisOrigin(nFrameIndex).y + SConsts::TILE_SIZE * pStats->GetPassability(nFrameIndex).GetSizeY() + SConsts::TILE_SIZE / 2));
}

const int CGivenPassabilityStObject::GetDownX() const
{
  auto pStats = static_cast<const SObjectBaseRPGStats *>(GetStats());
  return Max(0, static_cast<int>(center.x - pStats->GetOrigin(nFrameIndex).x + SConsts::TILE_SIZE / 2));
}

const int CGivenPassabilityStObject::GetUpX() const
{
  auto pStats = static_cast<const SObjectBaseRPGStats *>(GetStats());
  return Min(theStaticMap.GetSizeX() * SConsts::TILE_SIZE, static_cast<int>(center.x - pStats->GetOrigin(nFrameIndex).x + SConsts::TILE_SIZE * pStats->GetPassability(nFrameIndex).GetSizeX() + SConsts::TILE_SIZE / 2));
}

const int CGivenPassabilityStObject::GetDownY() const
{
  auto pStats = static_cast<const SObjectBaseRPGStats *>(GetStats());
  return Max(0, static_cast<int>(center.y - pStats->GetOrigin(nFrameIndex).y + SConsts::TILE_SIZE / 2));
}

const int CGivenPassabilityStObject::GetUpY() const
{
  auto pStats = static_cast<const SObjectBaseRPGStats *>(GetStats());
  return Min(theStaticMap.GetSizeY() * SConsts::TILE_SIZE, static_cast<int>(center.y - pStats->GetOrigin(nFrameIndex).y + SConsts::TILE_SIZE * pStats->GetPassability(nFrameIndex).GetSizeY() + SConsts::TILE_SIZE / 2));
}
...
bool CGivenPassabilityStObject::CanBeMovedTo(const CVec2 &newCenter) const
{
  ...
  const CArray2D<BYTE> &passability = pStats->GetPassability(nFrameIndex);
  for (int y = 0; y < passability.GetSizeY(); ++y) { for (int x = 0; x < passability.GetSizeX(); ++x) { if (!theStaticMap.IsPointInside(CVec2(downX + newCenter.x - GetCenter().x + SConsts::TILE_SIZE * x, downY + newCenter.y - GetCenter().y + SConsts::TILE_SIZE * y))) return false; } }
  ...
  for (int y = 0; y < passability.GetSizeY(); ++y)
  {
    for (int x = 0; x < passability.GetSizeX(); ++x)
    {
      if (passability[y][x] == 1)
      {
        ...
        if ((lockInfo[y][x] & pStats->dwAIClasses) != pStats->dwAIClasses) bPartially = true;
...
void CGivenPassabilityStObject::UnlockTiles(bool bInitialization)
{
  ...
  for (int y = 0; y < passability.GetSizeY(); ++y)
  {
    for (int x = 0; x < passability.GetSizeX(); ++x)
    {
      ...
      {
        const SVector tile(AICellsTiles::GetTile(downX + SConsts::TILE_SIZE * x, downY + SConsts::TILE_SIZE * y));
        if (y < lockInfo.GetSizeY() && x < lockInfo.GetSizeX()) theStaticMap.UnlockTile(tile.x, tile.y, lockInfo[y][x]);
      }
    }
  }
...
void CGivenPassabilityStObject::SetTransparencies()
{
  ...
  if (pStats != nullptr)
  {
    ...
    if (canSetTransparency.IsEmpty()) canSetTransparency.SetSizes(visibility.GetSizeX(), visibility.GetSizeY());
          ...
          if (theStaticMap.IsTileInside(tile))
          {
            ...
            if (canSetTransparency[y][x] == 0xff)
            {
              ...
              canSetTransparency[y][x] =
                  (theStaticMap.GetDissipation(tile) != 0 || theStaticMap.IsOneWayTransp(tile)) ? 0 : 1;
            }
            ...
            if (canSetTransparency[y][x] == 1)
            {
              ...
              if (visibility[y][x] & 8) theStaticMap.SetOneWayTransp(tile.x, tile.y, visibility[y][x] >> 4);

              // If we haven’t set transparency here, then disable setting/removing it
              canSetTransparency[y][x] =
                  (theStaticMap.GetDissipation(tile) != 0 || theStaticMap.IsOneWayTransp(tile)) ? 1 : 0;
            }
          }
        }
      }
    ...
    for (int x = 0; x < visibility.GetSizeX(); ++x)
    {
      if (canSetTransparency[y][x] == 1)
      {
        ...
        if (theStaticMap.IsTileInside(tile))
        {
          ...
          if (visibility[y][x] & 8) theStaticMap.RemoveOneWayTransp(tile.x, tile.y);
        }
      }
    }
  }
...
void CGivenPassabilityStObject::RestoreTransparencies() { if (bTransparencySet) SetTransparencies(); }

bool CGivenPassabilityStObject::IsPointInside(const CVec2 &point) const
{
  if (boundRect.IsPointInside(point))
  {
    if (static_cast<const SObjectBaseRPGStats *>(GetStats())->GetPassability(nFrameIndex).IsEmpty()) return true;
...
const CVec2 CGivenPassabilityStObject::GetAttackCenter(const CVec2 &vPoint) const
{
  ...
  if (tiles.empty()) vBestPoint = boundRect.center;
  else
  {
...
bool CSimpleStaticObject::CanUnitGoThrough(const EAIClass &eClass) const { return (pStats->dwAIClasses & eClass) == 0; }
...
bool CTerraMeshStaticObject::CanUnitGoThrough(const EAIClass &eClass) const { return (pStats->dwAIClasses & eClass) == 0; }
