...
CPtr<CTankStatesFactory> CTankStatesFactory::pFactory = nullptr;

IStatesFactory *CTankStatesFactory::Instance()
{
  if (pFactory == nullptr) pFactory = new CTankStatesFactory();
...
IUnitState *CTankStatesFactory::ProduceState(class CQueueUnit *pObj, CAICommand *pCommand)
{
  ...
  auto pUnit = static_cast<CTank *>(pObj);
  ...
  switch (cmd.cmdType)
  {
    ...
    case ACTION_MOVE_WAIT_FOR_TRUCKREPAIR:
      pResult = CMechUnitRestState::Instance(pUnit, cmd.vPos, cmd.fNumber, true);
    ...
    case ACTION_COMMAND_MOVE_TO:
    {
      ...
      if (pUnit->IsTrackDamaged()) pUnit->SendAcknowledgement(pCommand, ACK_CANNOT_MOVE_TRACK_DAMAGED, !pCommand->IsFromAI());
      else
      {
        if (pUnit->IsInTankPit() && !pCommand->IsFromAI())// first exit TankPit, then go where you were sent
        {
          theGroupLogic.InsertUnitCommand(pCommand->ToUnitCmd(), pUnit);
          pResult = CTankPitLeaveState::Instance(pUnit);
        }
        else pResult = CSoldierMoveToState::Instance(pUnit, cmd.vPos);
      }
    }
    ...
    case ACTION_COMMAND_ROTATE_TO:
      ...
      else if (!pUnit->NeedDeinstall())
      {
        ...
        {
          if (cmd.cmdType == ACTION_COMMAND_ROTATE_TO) pResult = CSoldierTurnToPointState::Instance(pUnit, cmd.vPos);
          else
          {
            CVec2 vDir = cmd.vPos;
            Normalize(&vDir);
    ...
    case ACTION_MOVE_BY_DIR:
      if (!pUnit->NeedDeinstall()) pResult = CSoldierMoveByDirState::Instance(pUnit, cmd.vPos);
    ...
    case ACTION_COMMAND_SWARM_TO:
      ...
      else
      {
        ...
        else pResult = CCommonSwarmState::Instance(pUnit, cmd.vPos, cmd.fNumber);
      }

      break;
    case ACTION_COMMAND_GUARD:
      pResult = CMechUnitRestState::Instance(pUnit, cmd.vPos, cmd.fNumber, false);
    ...
    case ACTION_COMMAND_ART_BOMBARDMENT:
      if (pUnit->GetFirstArtilleryGun() != nullptr)
      {
        if (pUnit->GetFirstArtilleryGun()->CanShootToPointWOMove(cmd.vPos, 0.0f)) pResult = CArtilleryBombardmentState::Instance(pUnit, cmd.vPos);
        else pUnit->SendAcknowledgement(pCommand, pUnit->GetFirstArtilleryGun()->GetRejectReason(), !pCommand->IsFromAI());
      }

      break;
    case ACTION_COMMAND_RANGE_AREA:
      if (pUnit->GetFirstArtilleryGun() != nullptr && pUnit->GetFirstArtilleryGun()->CanShootToPointWOMove(cmd.vPos, 0.0f)) pResult = CArtilleryRangeAreaState::Instance(pUnit, cmd.vPos);
    ...
    case ACTION_COMMAND_FOLLOW:
    {
      ...
      else
      {
        ...
        else pUnit->SetFollowState(pUnitToFollow);
      }
    }
    ...
    case ACTION_COMMAND_CHANGE_SHELLTYPE:
      if (pUnit->GetFirstArtilleryGun() != nullptr) pUnit->SetActiveShellType(static_cast<SWeaponRPGStats::SShell::EDamageType>(static_cast<int>(cmd.fNumber)));
...
IUnitState *CTankStatesFactory::ProduceRestState(class CQueueUnit *pUnit) { return CMechUnitRestState::Instance(checked_cast<CTank *>(pUnit), CVec2(-1, -1), 0); }