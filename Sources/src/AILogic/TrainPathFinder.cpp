...
bool CTrainPathFinder::CalculatePath()
{
  ...
  if (fBestPathLen >= 0.0f)
  {
    if (GetPathLength() == 0)
    {
      if (pStartEdgePoint->GetEdge()->GetLastNode() != pFinishEdgePoint->GetEdge()->GetLastNode()) pStartEdgePoint->Reverse(pFinishEdgePoint->GetEdge());
    ...
    else
    {
      ...
      // the first point is inverted
      if (pStartEdge->GetLastNode() != bestPath.front()) pStartEdgePoint->Reverse(theRailRoadGraph.GetEdge(pStartEdge->GetLastNode(), bestPath.front()));

      CPtr<IEdge> pFinishEdge = pFinishEdgePoint->GetEdge();
      // the last point is inverted
      if (pFinishEdge->GetFirstNode() != bestPath.back()) pFinishEdgePoint->Reverse(theRailRoadGraph.GetEdge(bestPath.back(), pFinishEdge->GetFirstNode()));
    }
...
// //////////////////////////////////////////////////////////// 
const SVector CTrainPathFinder::GetStartTile() const { return AICellsTiles::GetTile(pStartEdgePoint->Get2DPoint()); }
// //////////////////////////////////////////////////////////// 
const SVector CTrainPathFinder::GetFinishTile() const { return AICellsTiles::GetTile(finishPoint); }
// //////////////////////////////////////////////////////////// 
CEdgePoint *CTrainPathFinder::GetStartEdgePoint() const { return pStartEdgePoint; }
// //////////////////////////////////////////////////////////// 
CEdgePoint *CTrainPathFinder::GetFinishEdgePoint() const { return pFinishEdgePoint; }
// //////////////////////////////////////////////////////////// 
void CTrainPathFinder::StartPathIterating() { iter = bestPath.begin(); }
// //////////////////////////////////////////////////////////// 
const int CTrainPathFinder::GetCurPathNode() const
{
  if (iter == bestPath.end()) return -1;
  return *iter;
}

// //////////////////////////////////////////////////////////// 
void CTrainPathFinder::Iterate() { if (iter != bestPath.end()) ++iter; }
// //////////////////////////////////////////////////////////// 
const int CTrainPathFinder::GetPathLength() const
{
  if (fBestPathLen == -1.0f) return -1;
