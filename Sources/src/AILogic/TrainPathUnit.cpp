...
{
  if (pUnit->GetOwner()->GetStats()->type == RPG_TYPE_TRAIN_LOCOMOTIVE) bCanMove = true;
...
{
  if (pCarriage->GetOwner()->GetStats()->type == RPG_TYPE_TRAIN_LOCOMOTIVE) bCanMove = true;

  auto pStats = static_cast<const SMechUnitRPGStats *>(pCarriage->GetStats());
...
void CTrainPathUnit::SecondSegment(const bool bUpdate)
{
  if (CanMovePathfinding())
  {
    ...
    {
      bool bCanGo = true;
      auto iter = carriages.begin();
      while (iter != carriages.end() && (*iter)->GetOwner()->IsUninstalled()) ++iter;
    ...
    for (int i = 0; i < carriages.size(); ++i) oldCenters.push_back(carriages[i]->GetCenter());
    ...
    auto iter = oldCenters.begin();
    for (int i = 0; i < carriages.size(); ++i)
    {
      carriages[i]->SetSpeed(fabs(*iter - carriages[i]->GetCenter()) / static_cast<float>(SConsts::AI_SEGMENT_DURATION) * carriages[i]->GetDirVector());
      ++iter;
    }
  }
}
...
{
  if (!CanMovePathfinding()) return nullptr;
  ...
  if (pTheTrainPathFinder->GetPathLength() == -1.0f) return nullptr;
...
bool CTrainPathUnit::SendAlongPath(IStaticPath *pStaticPath, const CVec2 &vShift, bool bSmoothTurn)
{
  ...
  if (CanMovePathfinding())
  {
    for (auto iter = carriages.begin(); iter != carriages.end(); ++iter) (*iter)->GetOwner()->InstallAction(ACTION_NOTIFY_UNINSTALL_ROTATE);
...
CEdgePoint *CTrainPathUnit::GetCurEdgePoint() { return pCurEdgePoint; }

ISmoothPath *CTrainPathUnit::GetCurPath() const { return pSmoothPath; }

ISmoothPath *CTrainPathUnit::GetSmoothPath() const { return pSmoothPath; }

void CTrainPathUnit::SetCurEdgePoint(CEdgePoint *pEdgePoint) { pCurEdgePoint = pEdgePoint; }
...
const WORD CTrainPathUnit::GetDir() const { return GetDirectionByVector(GetDirVector()); }

const WORD CTrainPathUnit::GetFrontDir() const { return GetDirectionByVector(vDir); }

const CVec2 &CTrainPathUnit::GetDirVector() const { return vDir; }
...
void CTrainPathUnit::SetNewCoordinates(const CVec3 &vNewCenter, bool bStopUnit) { SetCoordWOUpdate(vNewCenter); }
...
bool CTrainPathUnit::IsIdle() const { return GetCurPath() == nullptr || GetCurPath()->IsFinished(); }
...
IStaticPathFinder *CTrainPathUnit::GetPathFinder() const { return pTheTrainPathFinder; }

void CTrainPathUnit::SetDesirableSpeed(const float fDesirableSpeed) {}

void CTrainPathUnit::UnsetDesirableSpeed() {}

float CTrainPathUnit::GetDesirableSpeed() const { return -1.0f; }
...
{
  auto pCarrierStats = static_cast<const SMechUnitRPGStats *>(GetCarriage(n)->GetStats());
  auto pStats = static_cast<const SMechUnitRPGStats *>(GetCarriage(m)->GetStats());

  // distance from the rear wheel of the train in front to the front wheel of ours
...
void CTrainPathUnit::ChangeDirection(const bool bNewFrontDir)
{
  if (bNewFrontDir != bFrontDir)
  {
    ...
    for (int i = 0; i < nCarriagesSize / 2; ++i)
    {
      ...
      if (nCarriagesSize - 1 - i != i) nodesInside[nCarriagesSize - 1 - i].reverse();

      intermNodes[i].reverse();
      if (nCarriagesSize - 1 - i - 1 != i) intermNodes[nCarriagesSize - 1 - i - 1].reverse();
    }

    if (nCarriagesSize & 1) nodesInside[nCarriagesSize >> 1].reverse();
  }

  if (bFrontDir) pCurEdgePoint = carriages[0]->GetFrontWheelPoint();
  else pCurEdgePoint = carriages[0]->GetBackWheelPoint();

  vCenter = pCurEdgePoint->Get2DPoint();

  for (int i = 0; i < carriages.size(); ++i) carriages[i]->SetRightDir(bFrontDir);
}
...
void CTrainPathUnit::SetFrontWheel(const int n)
{
  ...
  float fDist;
  if (bFrontDir) fDist = GetDistFromBackToFrontWheel(n - 1, n);
  else fDist = GetDistFromFrontToBackWheel(n - 1, n);
  ...
  if (!intermNodes[n - 1].empty())
  {
    ...
    else pPoint = pSmoothPath->GetBackWheelPoint(n - 1);
  }
  else pPoint = pSmoothPath->GetBackWheelPoint(n - 1);
  ...
  while (iter != intermNodes[n - 1].rend() && !bFinished)
  {
    ...
    else
    {
      ...
      if (iter1 != intermNodes[n - 1].rend())
      {
        ...
        if (pNewEdge != nullptr) pPoint = pNewEdge->CreateLastEdgePoint();
      }
      else
      {
        ...
        if (v == *iter) v = pNewEdge->GetLastNode();
  ...
  // need to be moved from pNewFrontWheel to front wheel
  if (!bFinished)
  {
    // rib on which to lay
    IEdge *pEdge = pPoint->GetEdge();
    ...
    else pFirstPoint = pSmoothPath->GetFrontWheelPoint(n);
    // point on pEdge where it was postponed
    pNewFrontWheel = pEdge->MakeIndent(vPointToMeasureDist, pPoint, pFirstPoint, fDist);
  }

  // install new front wheel point
  pSmoothPath->SetNewFrontWheel(n, pNewFrontWheel);

  // transfer all untaken vertices to vertices from the first wheel to the last
  for (auto forwardIterator = intermNodes[n - 1].begin(); forwardIterator != iter.base(); ++forwardIterator)
  {
...
void CTrainPathUnit::SetBackWheel(const int n)
{
  ...
  if (!nodesInside[n].empty())
  {
    ...
    else pPoint = pSmoothPath->GetFrontWheelPoint(n);
  }
  else pPoint = pSmoothPath->GetFrontWheelPoint(n);
  ...
  bool bFinished = false;
  auto iter = nodesInside[n].rbegin();
  while (iter != nodesInside[n].rend() && !bFinished)
  {
    ...
    // if postponed between pPoint and pFirstPoint
    if (!pNewBackWheel->IsEqual(pFirstPoint)) bFinished = true;
    else
    {
      auto iter1 = iter;
      std::advance(iter1, 1);
  ...
  // need to be moved from pNewFrontWheel to rear wheel
  if (!bFinished)
  {
    // rib on which to lay
    IEdge *pEdge = pPoint->GetEdge();
    ...
    else pFirstPoint = pSmoothPath->GetBackWheelPoint(n);

    // point on pEdge where it was postponed
    pNewBackWheel = pEdge->MakeIndent(vPointToMeasureDist, pPoint, pFirstPoint, fDist);
  }

  // install new rear wheel point
  pSmoothPath->SetNewBackWheel(n, pNewBackWheel);

  // transfer all untaken vertices to vertices from the first wheel to the last
  if (n < intermNodes.size())
  {
    for (auto forwardIterator = nodesInside[n].begin(); forwardIterator != iter.base(); ++forwardIterator)
    {
...
void CTrainPathUnit::PushNodesToFrontCarriage(std::list<int> &newNodes) { nodesInside[0].splice(nodesInside[0].end(), newNodes); }
...
void CTrainPathUnit::GetTrainNodes(std::list<int> *pNodesOfTrain)
{
  ...
  {
    for (auto iter = nodesInside[n].rbegin(); iter != nodesInside[n].rend(); ++iter) pNodesOfTrain->push_back(*iter);

    if (n < nodesInside.size() - 1) { for (auto iter = intermNodes[n].rbegin(); iter != intermNodes[n].rend(); ++iter) pNodesOfTrain->push_back(*iter); }
  }
}

bool CTrainPathUnit::IsInOneTrain(IBasePathUnit *pUnit) const
{
  if (!pUnit->IsTrain()) return false;
  else
  {
...
void CTrainPathUnit::LocomotiveDead() { bCanMove = false; }

void CTrainPathUnit::CarriageTrackDamaged(const int nOwnerID, const bool bTrackDamagedState)
{
  if (bTrackDamagedState) damagedTrackCarriages.insert(nOwnerID);
  else damagedTrackCarriages.erase(nOwnerID);
}

bool CTrainPathUnit::CanMove() const { return damagedTrackCarriages.size() == 0 && bCanMove; }

bool CTrainPathUnit::CanMovePathfinding() const { return CanMove(); }
...
{
  if (pTrain) return pTrain->GetSmoothPath();
  return nullptr;
}

ISmoothPath *CCarriagePathUnit::GetCurPath() const
{
  if (pTrain) return pTrain->GetCurPath();
  return nullptr;
}
...
{
  if (pTrain) return pTrain->CreateBigStaticPath(vStartPoint, vFinishPoint, pPointChecking);
  return nullptr;
}
...
{
  stayTime = 0;
  if (pTrain) pTrain->SendAlongPath(pStaticPath, vShift, bSmoothTurn);
...
{
  stayTime = 0;
  if (pTrain) pTrain->SendAlongPath(pPath);
...
void CCarriagePathUnit::HookTo(CCarriagePathUnit *pUnit)
{
  ...
  if (pTrain)
  {
    ...
    if (pCarrierBackWheelPoint != nullptr)
    {
      auto pCarrierStats = static_cast<const SMechUnitRPGStats *>(pUnit->GetStats());
      ...
      // finding a position for the front wheel
      ...
      // find space for the rear wheels
      if (pFrontWheelPoint) pBackWheelPoint = theRailRoadGraph.MakeIndent(-GetFrontDirVec(), pFrontWheelPoint, GetDistanceBetweenWheels());

      if (pFrontWheelPoint == nullptr || pBackWheelPoint == nullptr)
      {
...
const CVec3 CCarriagePathUnit::GetBackHookPoint3D() const { return Get3DPointOfUnit(GetCenter(), static_cast<const SMechUnitRPGStats *>(GetStats())->vTowPoint.y); }

const CVec2 CCarriagePathUnit::GetBackHookPoint2D() const { return Get2DPointOfUnit(GetCenter(), static_cast<const SMechUnitRPGStats *>(GetStats())->vTowPoint.y); }

const CVec3 CCarriagePathUnit::GetFrontHookPoint3D() const { return Get3DPointOfUnit(GetCenter(), static_cast<const SMechUnitRPGStats *>(GetStats())->vHookPoint.y); }

const CVec2 CCarriagePathUnit::GetFronHookPoint2D() const { return Get2DPointOfUnit(GetCenter(), static_cast<const SMechUnitRPGStats *>(GetStats())->vHookPoint.y); }

const CVec3 CCarriagePathUnit::GetFrontWheel3D() const { return Get3DPointOfUnit(GetCenter(), static_cast<const SMechUnitRPGStats *>(GetStats())->vFrontWheel.y); }

const CVec2 CCarriagePathUnit::GetFrontWheel2D() const { return Get2DPointOfUnit(GetCenter(), static_cast<const SMechUnitRPGStats *>(GetStats())->vFrontWheel.y); }
...
const float CCarriagePathUnit::GetDistanceToBackWheel() const { return fabs(static_cast<const SMechUnitRPGStats *>(GetStats())->vBackWheel.y); }
...
void CCarriagePathUnit::SetPlacementByWheels(CEdgePoint *_pFrontWheelPoint, CEdgePoint *_pBackWheelPoint)
{
  ...
  if (vNewDir != VNULL2)
  {
    ...
    if (GetRightDir()) UpdateDirection(vNewDir);
    else UpdateDirection(-vNewDir);
  }
}
...
void CCarriagePathUnit::SecondSegment(const bool bUpdate)
{
  if (pTrain)
  {
    ...
    // moved
    if (vOldCenter != GetCenter() || vOldDir != GetFrontDirVec()) stayTime = 0;
    else stayTime += SConsts::AI_SEGMENT_DURATION;

    if (pTrain->IsIdle()) LockTiles();

    CalculateIdle();
  }
}

CEdgePoint *CCarriagePathUnit::GetFrontWheelPoint() const { return pFrontWheelPoint; }

CEdgePoint *CCarriagePathUnit::GetBackWheelPoint() const { return pBackWheelPoint; }

void CCarriagePathUnit::GetPlacement(SAINotifyPlacement *pPlacement, const NTimer::STime timeDiff) const
{
  CVec2 vCenter = vOldCenter + (GetCenter() - vOldCenter) * (1.0f - static_cast<float>(timeDiff) / static_cast<float>(SConsts::AI_SEGMENT_DURATION));
  ...
  if (wOldDir + wTurn == GetDir()) pPlacement->dir = wOldDir + wTurn * (1.0f - static_cast<float>(timeDiff) / static_cast<float>(SConsts::AI_SEGMENT_DURATION));
  else pPlacement->dir = wOldDir - wTurn * (1.0f - static_cast<float>(timeDiff) / static_cast<float>(SConsts::AI_SEGMENT_DURATION));

  pPlacement->fSpeed = fabs(speed);
}

void CCarriagePathUnit::SetRightDir(bool bRightDir) { if (bRightDir != GetRightDir()) CPathUnit::SetRightDir(bRightDir); }

void CCarriagePathUnit::SetNewCoordinates(const CVec3 &newCenter, bool bStopUnit) { CPathUnit::SetNewCoordinates(newCenter, bStopUnit); }
...
void CCarriagePathUnit::SetCoordWOUpdate(const CVec3 &newCenter) { CPathUnit::SetCoordWOUpdate(newCenter); }

const CTrainPathUnit *CCarriagePathUnit::GetTrainOwner() const { return pTrain; }

bool CCarriagePathUnit::IsInOneTrain(IBasePathUnit *pUnit) const
{
  if (pTrain) return pUnit->IsInOneTrain(pTrain);
  return false;
}
...
bool CCarriagePathUnit::CanMove() const { return pTrain && pTrain->CanMove(); }

bool CCarriagePathUnit::CanMovePathfinding() const { return CanMove(); }

void CCarriagePathUnit::UnitDead()
{
  if (pTrain && GetOwner()->GetStats()->type == RPG_TYPE_TRAIN_LOCOMOTIVE) pTrain->LocomotiveDead();
  if (pTrain) pTrain->CarriageTrackDamaged(GetOwner()->GetID(), false);
}

void CCarriagePathUnit::TrackDamagedState(const bool bTrackDamaged) { if (pTrain) pTrain->CarriageTrackDamaged(GetOwner()->GetID(), bTrackDamaged); }