...
IUnitState *CTransportResupplyState::Instance(CAITransportUnit *pTransport, const CVec2 &vServePoint, CAIUnit *_pPreferredUnit) { return new CTransportResupplyState(pTransport, vServePoint, _pPreferredUnit); }
...
{
  if (!bUpdatedActionsBegin) pTransport->SendAcknowledgement(ACK_START_SERVICE_RESUPPLY);
...
IUnitState *CTransportResupplyHumanResourcesState::Instance(CAITransportUnit *pTransport, const CVec2 &vServePoint, CArtillery *_pPreferredUnit) { return new CTransportResupplyHumanResourcesState(pTransport, vServePoint, _pPreferredUnit); }
...
CTransportResupplyHumanResourcesState::CTransportResupplyHumanResourcesState(CAITransportUnit *_pTransport, const CVec2 &_vServePoint, CArtillery *_pPreferredUnit)
  : eState(ETSHR_ESTIMATING), pTransport(_pTransport), vServePoint(_vServePoint + _pTransport->GetGroupShift()), timeLastUpdate(0), pPreferredUnit(_pPreferredUnit), bWaitForPath(false) { if (pPreferredUnit) vServePoint = pPreferredUnit->GetCenter(); }

void CTransportResupplyHumanResourcesState::Segment()
{
  if (pTransport->GetResursUnitsLeft() < SConsts::SOLDIER_RU_PRICE) eState = ETSHR_GOTO_STORAGE;
  ...
  switch (eState)
  {
    ...
    case ETSHR_ESTIMATING:
    {
      ...
      else
      {
        ...
        if (pStaticPath)
        {
          bWaitForPath = fabs2(pStaticPath->GetFinishPoint() - vServePoint) < sqr(static_cast<int>(SConsts::TILE_SIZE));
    ...
    case ETSHR_WAIT_FOR_UNITS:
      if (curTime - timeLastUpdate > pTransport->GetBehUpdateDuration()) eState = ETSHR_START_SERVE_ARTILLERY;
    ...
    case ETSHR_START_SERVE_SQUAD:
    {
      ...
      if (notCompleteSquads.empty()) eState = ETSHR_WAIT_FOR_UNITS;
      else eState = ETSHR_SERVING_SQUAD;
    }
    ...
    case ETSHR_START_SERVE_ARTILLERY:
    {
      ...
      if (!emptyArtillery.empty()) eState = ETSHR_SERVING_ARTILLERY;
      else eState = ETSHR_START_SERVE_SQUAD;
    }
    ...
    case ETSHR_SERVING_SQUAD:
      ...
      {
        timeLastUpdate = curTime;
        if (!notCompleteSquads.empty()) { if (ServeSquad(*notCompleteSquads.begin())) notCompleteSquads.pop_front(); }
        else eState = ETSHR_START_SERVE_SQUAD;
      }
    ...
    case ETSHR_SERVING_ARTILLERY:
      ...
      {
        timeLastUpdate = curTime;
        if (!emptyArtillery.empty()) { if (ServeArtillery(*emptyArtillery.begin())) emptyArtillery.pop_front(); }
        else eState = ETSHR_START_SERVE_ARTILLERY;
      }
...
{
  if (!pSquad || !pSquad->IsValid() || !pSquad->IsAlive()) return true;
  ...
  {
    // determine which soldier to create and create it.
    ...
    // Let's find the first soldier who is missing.
    int nSlot = 0;
    while (nSlot < present.size() && present[nSlot]) ++nSlot;

    // create a soldier and send him to the Squad
    ...
    auto pSoldier = static_cast<CSoldier *>(units[id]);
  ...
  if (rFormation.order.size() == pSquad->Size() + pSquad->VirtualUnitsSize()) return true;
...
bool CTransportResupplyHumanResourcesState::ServeArtillery(CArtillery *pArtillery)
{
  ...
  // create a squad consisting of one soldier and send him to work with artillery.
  const CVec3 vEntrancePoint(pTransport->GetEntrancePoint(), pTransport->GetZ());

  if (pTransport->GetPlayer() == theDipl.GetNeutralPlayer()) pTransport->SetCommandFinished();
  else theUnitCreation.CreateCrew(pArtillery, nullptr, 1, vEntrancePoint, pTransport->GetPlayer(), false);
...
bool CTransportResupplyHumanResourcesState::CheckArtillery(CAIUnit *pU) const
{
  ...
  {
    auto pArtillery = static_cast<CArtillery *>(pU);
    if (pArtillery && !pArtillery->IsBeingCaptured() && !pArtillery->HasServeCrew() && pArtillery->MustHaveCrewToOperate()) { return true; }
  }
  return false;
}
...
{
  if (CheckArtillery(_pPreferredUnit)) pArtillerys->push_back(_pPreferredUnit);

  for (CUnitsIter<0, 2> iter(pTransport->GetParty(), ANY_PARTY, vServePoint, SConsts::RESUPPLY_RADIUS);
       !iter.IsFinished(); iter.Iterate()) { if (CheckArtillery(*iter)) pArtillerys->push_back(static_cast<CArtillery *>(*iter)); }
}
...
void CTransportResupplyHumanResourcesState::FindNotCompleteSquads(std::list<CPtr<CFormation>> *pSquads) const
{
       ...
       !iter.IsFinished(); iter.Iterate())
  {
    ...
    {
      auto pSold = static_cast_ptr<CSoldier *>(pU);
      CFormation *pForm = pSold->GetFormation();
      if (pForm &&
          pForm->IsResupplyable() &&
          pForm->Size() != pForm->GetStats()->members.size()) { pSquads->push_back(pForm); }
    }
  }
...
IUnitState *CTransportRepairState::Instance(CAITransportUnit *pTransport, const CVec2 &vServePoint, CAIUnit *_pPreferredUnit) { return new CTransportRepairState(pTransport, vServePoint, _pPreferredUnit); }
...
{
  if (!bUpdatedActionsBegin) pTransport->SendAcknowledgement(ACK_START_SERVICE_REPAIR);
  bUpdatedActionsBegin = true;
}
...
IUnitState *CTransportLoadRuState::Instance(CAITransportUnit *pTransport, const bool bSubState, CBuildingStorage *_pPreferredStorage) { return new CTransportLoadRuState(pTransport, bSubState, _pPreferredStorage); }
...
CTransportLoadRuState::CTransportLoadRuState(CAITransportUnit *_pTransport, const bool bSubState, CBuildingStorage *_pPreferredStorage)
  : eState(ETLRS_SEARCH_FOR_STORAGE), pStorage(_pPreferredStorage), pTransport(_pTransport), nEntrance(-1),
    bSubState(bSubState) { if (pTransport->GetResursUnitsLeft() == SConsts::TRANSPORT_RU_CAPACITY) pTransport->SetCommandFinished(); }

CBuildingStorage *CTransportLoadRuState::FindNearestSource()
{
  // to find the nearest storage facility
  ...
  public:
    ...
    bool OnlyConnected() const override { return true; }
    // true - finish, what you need has already been found
    bool AddStorage(class CBuildingStorage *pStorage, const float _fPathLength) override
    {
  ...
  switch (eState)
  {
    case ETLRS_SEARCH_FOR_STORAGE:
    {
      ...
      )
      {
        ...
        {
          if (bSubState) pTransport->SendAcknowledgement(ACK_GOING_TO_STORAGE, true);
      ...
      {
        // clear the command queue for the transport
    ...
    case ETLRS_APPROACHING_STORAGE:
      if (!IsValidObj(pStorage)) eState = ETLRS_SEARCH_FOR_STORAGE;
      ...
      else if (pTransport->IsIdle()) eState = ETLRS_START_LOADING_RU;
    ...
    case ETLRS_START_LOADING_RU:
      ...
      else eState = ETLRS_SEARCH_FOR_STORAGE;

      break;
    case ETLRS_LOADING_RU:
      // wait util resources are full
    ...
    case ETLRS_WAIT_FOR_LOADERS:
      ...
      else pTransport->SetCommandFinished();
...
{
  if (bSubState) eState = ETLRS_SUBSTATE_FINISHED;
  else TryInterruptState(nullptr);
}

void CTransportLoadRuState::CreateSquad()
{
  pTransport->Unlock();
  if (IsValidObj(pLoaderSquad)) pLoaderSquad->Disappear();

  if (pTransport->GetPlayer() == theDipl.GetNeutralPlayer())
  {
    pLoaderSquad = nullptr;
    pTransport->SetCommandFinished();
  }
  else
  {
...
{
  // if the loaders are not yet on the transport, then send them to catch up.
...
CTransportServeState::CTransportServeState(class CAITransportUnit *_pTransport, const CVec2 &_vServePoint, CAIUnit *_pPreferredUnit)
  : eState(ETRS_WAIT_FOR_UNLOCK), vServePoint(_vServePoint + _pTransport->GetGroupShift()), timeLastUpdate(curTime),
    bWaitForPath(false), bUpdatedActionsBegin(false),
    pPreferredUnit(_pPreferredUnit), pTransport(_pTransport)
{
  if (IsValidObj(pPreferredUnit)) vServePoint = pPreferredUnit->GetCenter();
  ...
  if (IsValidObj(pPreferredUnit))
  {
    ...
    if (pStats->IsInfantry())
    {
      ...
      {
        auto pGunCrewState = static_cast<CFormationGunCrewState *>(pCurrentState);
...
void CTransportServeState::Segment()
{
  switch (eState)
  {
    ...
    {
      if (pTransport->GetResursUnitsLeft() == 0.0f) eState = ETRS_GOING_TO_STORAGE;
      else
      {
        if (fabs2(pTransport->GetCenter() - vServePoint) < sqr(static_cast<int>(SConsts::TILE_SIZE))) { eState = ETRS_APPROACHING; }
        else
        {
    ...
    case ETRS_CREATE_SQUAD:
      ...
      if (pLoaderSquad) eState = ETRS_FINDING_UNIT_TO_SERVE;
    ...
    case ETRS_FINDING_UNIT_TO_SERVE:
    {
      ...
      else if (isNotEnoughRU && pTransport->GetResursUnitsLeft() != SConsts::TRANSPORT_RU_CAPACITY) { eState = ETRS_GOING_TO_STORAGE; }
      /* else
       */
    }
    ...
    case ETRS_WAIT_FOR_UNIT_TO_SERVE:
      // We stand and wait to see if the services are still needed.
      if (curTime - timeLastUpdate > pTransport->GetBehUpdateDuration()) eState = ETRS_FINDING_UNIT_TO_SERVE;
    ...
    case ETRS_LOADERS_INROUTE:
      ...
      {
        timeLastUpdate = curTime;
        if (!IsValidObj(pLoaderSquad) || pLoaderSquad->IsInTransport()) eState = ETRS_CREATE_SQUAD;
      }
...
ETryStateInterruptResult CTransportServeState::TryInterruptState(class CAICommand *pCommand)
{
  ...
  {
    if (IsValidObj(pLoaderSquad) && !pLoaderSquad->IsInTransport()) { theGroupLogic.UnitCommand(SAIUnitCmd(ACTION_MOVE_CATCH_TRANSPORT, pTransport), pLoaderSquad, false); }
...
void CTransportBuildState::Segment()
{
  ...
  {
    case ETBS_ESTIMATE:
      if (IsEndPointNeeded()) eState = ETBS_WAIT_FOR_ENDPOINT;
      else eState = ETBS_END_POINT_READY;
    ...
    case ETBS_END_POINT_READY:
      ...
      else if (IsEnoughResources()) eState = ETBS_START_APPROACH;
      else eState = ETBS_LOADING_RESOURCES;
    ...
    case ETBS_START_APPROACH:
      ...
      else eState = ETBS_CREATE_SQUAD;

      break;
    case ETBS_LOADING_RESOURCES:
      if (!pLoadRuSubState) pLoadRuSubState = static_cast<CTransportLoadRuState *>(CTransportLoadRuState::Instance(pUnit, true));
      pLoadRuSubState->Segment();
      if (pLoadRuSubState->IsSubStateFinished())
      {
        pLoadRuSubState = nullptr;
        if (IsEnoughResources()) eState = ETBS_START_APPROACH;
        else TryInterruptState(nullptr);
      }
    ...
    case ETBS_APROACHING_BUILDPOINT:
      ...
      {
        if (MustSayNegative()) pUnit->SendAcknowledgement(ACK_NO_ENGINEERS_CANNOT_REACH_BUILDPOINT);
        pUnit->SetCommandFinished();
      }
    ...
    {
      pUnit->Unlock();
      if (IsValidObj(pEngineers)) pEngineers->Disappear();
    ...
    case ETBS_WAIT_FINISH_BUILD:
      if (!IsValidObj(pEngineers) || pEngineers->IsInTransport())
      {
        ...
        if (IsWorkDone()) TryInterruptState(nullptr);
...
ETryStateInterruptResult CTransportBuildState::TryInterruptState(class CAICommand *pCommand)
{
  ...
  {
    if (IsValidObj(pEngineers) && !pEngineers->IsInTransport()) { theGroupLogic.UnitCommand(SAIUnitCmd(ACTION_MOVE_CATCH_TRANSPORT, pUnit), pEngineers, false); }
...
bool CTransportBuildLongObjectState::HaveToSendEngeneersNow()
{
  ...
  if (bRes) { const float fDist = GetDistanceToSegment(vStartPoint, vEndPoint, pUnit->GetCenter()); }
  return bRes;
...
IUnitState *CTransportBuildFenceState::Instance(class CAITransportUnit *pTransport, const class CVec2 &vStartPoint) { return new CTransportBuildFenceState(pTransport, vStartPoint); }
...
IUnitState *CTransportBuildEntrenchmentState::Instance(class CAITransportUnit *pTransport, const class CVec2 &vStartPoint) { return new CTransportBuildEntrenchmentState(pTransport, vStartPoint); }
...
{
  // calculate the time it takes to drive 1 truck length
  auto pStats = static_cast<const SMechUnitRPGStats *>(pTransport->GetStats());
...
{
  // On the way, look for mines that can explode
  if (pUnit->IsIdle())// and when you stop, then find at least some mine
  {
         ...
         !iter.IsFinished(); iter.Iterate())
    {
      ...
      if ((*iter)->GetObjectType() == ESOT_MINE &&
          ...
          fabs2((*iter)->GetCenter() - vClearCenter) <= sqr(static_cast<float>(SConsts::MINE_CLEAR_RADIUS)))
        return true;
    }
  }
  else if (curTime >= timeLastCheck + timeCheckPeriod)
  {
    ...
    for (CMinesIter iter(pUnit->GetCenter(), SConsts::MINE_VIS_RADIUS, nParty, true); !iter.IsFinished(); iter.Iterate()) { if ((*iter)->WillExplodeUnder(pUnit)) return true; }
  }
  return false;
}

bool CTransportClearMineState::IsEnoughResources() const { return true; }

bool CTransportClearMineState::IsWorkDone() const { return bWorkDone; }
...
bool CTransportClearMineState::IsEndPointNeeded() const { return false; }

IUnitState *CTransportClearMineState::Instance(class CAITransportUnit *pTransport, const class CVec2 &vStartPoint) { return new CTransportClearMineState(pTransport, vStartPoint); }
...
{
  auto pStats = static_cast<const SMechUnitRPGStats *>(pUnit->GetStats());

  if (pUnit->IsIdle() && fabs2(pUnit->GetCenter() - vStartPoint) <= sqr(SConsts::TRANSPORT_RESUPPLY_OFFSET * 2)) return true;

  if (!pUnit->IsIdle() && fabs2(pUnit->GetCenter() - vStartPoint) <= sqr(SConsts::TRANSPORT_RESUPPLY_OFFSET)) return true;

  if (bTransportSent && pUnit->IsIdle()) return true;
  return false;
}

bool CTransportPlaceMineState::IsEnoughResources() const { return pUnit->GetResursUnitsLeft() >= SConsts::MINE_RU_PRICE[nNumber]; }

bool CTransportPlaceMineState::IsWorkDone() const { return bWorkDone; }
...
bool CTransportPlaceMineState::IsEndPointNeeded() const { return false; }

IUnitState *CTransportPlaceMineState::Instance(class CAITransportUnit *pTransport, const class CVec2 &vStartPoint, const float fNumber) { return new CTransportPlaceMineState(pTransport, vStartPoint, fNumber); }

// **********************************************************************
// *CTransportPlaceAntitankState*
// **********************************************************************

IUnitState *CTransportPlaceAntitankState::Instance(class CAITransportUnit *pTransport, const class CVec2 &vStartPoint) { return new CTransportPlaceAntitankState(pTransport, vStartPoint); }
...
bool CTransportPlaceAntitankState::HaveToSendEngeneersNow()
{
  ...
  if (bSent && pUnit->IsIdle()) return true;
  return false;
}

bool CTransportPlaceAntitankState::IsEnoughResources() const { return pUnit->GetResursUnitsLeft() >= SConsts::ANTITANK_RU_PRICE; }

bool CTransportPlaceAntitankState::IsWorkDone() const { return bWorkFinished; }
...
bool CTransportPlaceAntitankState::IsEndPointNeeded() const { return false; }

// **********************************************************************
// *CTransportRepairBridgeState*
// **********************************************************************

IUnitState *CTransportRepairBridgeState::Instance(class CAITransportUnit *pTransport, class CFullBridge *pFullBridge) { return new CTransportRepairBridgeState(pTransport, pFullBridge); }
...
bool CTransportRepairBridgeState::HaveToSendEngeneersNow() { return bSentToBuildPoint && pUnit->IsIdle(); }

bool CTransportRepairBridgeState::IsEnoughResources() const { return 1.0f < pUnit->GetResursUnitsLeft(); }

bool CTransportRepairBridgeState::IsWorkDone() const { return pBridgeToRepair->GetHPPercent() == 1.0f; }
...
bool CTransportRepairBridgeState::IsEndPointNeeded() const { return false; }

// **********************************************************************
// *CTransportRepairBridgeState*
// **********************************************************************

IUnitState *CTransportBuildBridgeState::Instance(class CAITransportUnit *pTransport, class CFullBridge *pFullBridge) { return new CTransportBuildBridgeState(pTransport, pFullBridge); }
...
CTransportBuildBridgeState::CTransportBuildBridgeState(class CAITransportUnit *_pTransport, class CFullBridge *_pFullBridge)
  : CTransportBuildState(_pTransport, VNULL2), pFullBridge(_pFullBridge), pCreation(new CBridgeCreation(_pFullBridge, _pTransport)),
    bTransportSent(false) { SetStartPoint(pCreation->GetStartPoint()); }
...
bool CTransportBuildBridgeState::HaveToSendEngeneersNow() { return bTransportSent && pUnit->IsIdle(); }
...
bool CTransportBuildBridgeState::IsWorkDone() const { return pCreation->GetCurIndex() >= pCreation->GetMaxIndex(); }
...
bool CTransportBuildBridgeState::IsEndPointNeeded() const { return false; }

// **********************************************************************
// *CTransportRepairBuildingState*
// **********************************************************************

IUnitState *CTransportRepairBuildingState::Instance(class CAITransportUnit *pTransport, class CBuilding *pStaticObject) { return new CTransportRepairBuildingState(pTransport, pStaticObject); }
...
{
  auto pStats = static_cast<const SMechUnitRPGStats *>(pUnit->GetStats());
  if (bSentToBuildPoint && pUnit->IsIdle() || fabs2(pUnit->GetCenter() - vStartPoint) <= sqr(pStats->vAABBHalfSize.y * 2)) return true;
  return false;
}

bool CTransportRepairBuildingState::IsEnoughResources() const { return pBuilding->GetStats()->fRepairCost <= pUnit->GetResursUnitsLeft(); }

bool CTransportRepairBuildingState::IsWorkDone() const { return pBuilding->GetHitPoints() == pBuilding->GetStats()->fMaxHP; }
...
bool CTransportRepairBuildingState::IsEndPointNeeded() const { return false; }