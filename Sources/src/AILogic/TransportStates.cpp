...
CPtr<CTransportStatesFactory> CTransportStatesFactory::pFactory = nullptr;

IStatesFactory *CTransportStatesFactory::Instance()
{
  if (pFactory == nullptr) pFactory = new CTransportStatesFactory();
...
IUnitState *CTransportStatesFactory::ProduceState(class CQueueUnit *pObj, class CAICommand *pCommand)
{
  ...
  auto pUnit = static_cast<CAITransportUnit *>(pObj);
  ...
  switch (cmd.cmdType)
  {
    ...
    {
      auto pObject = static_cast_ptr<CStaticObject *>(cmd.pObject);
      if (ESOT_BRIDGE_SPAN == pObject->GetObjectType())
      {
        auto pSpan = static_cast_ptr<CBridgeSpan *>(cmd.pObject);
        pResult = CTransportRepairBridgeState::Instance(pUnit, pSpan->GetFullBridge());
      }
      else if (ESOT_BUILDING == pObject->GetObjectType())
      {
        auto pBuilding = static_cast_ptr<CBuilding *>(cmd.pObject);
    ...
    case ACTION_COMMAND_DEPLOY_ARTILLERY:
      if (pUnit->IsTowing()) { if (pUnit->IsTowing() != 0) pResult = CTransportUnhookArtilleryState::Instance(pUnit, cmd.vPos, static_cast<bool>(cmd.fNumber)); }
    ...
    case ACTION_COMMAND_RESUPPLY_HR:
      NI_ASSERT_T(cmd.pObject ? dynamic_cast_ptr<CArtillery*>( cmd.pObject ) != nullptr : true, NStr::Format( "Wrong preferred unit %s",typeid( *pObj ).name()));
    ...
    case ACTION_COMMAND_ATTACK_UNIT:
    {
      if (cmd.pObject && cmd.pObject.IsValid())
      {
        ...
        if (pTarget->IsAlive())
        {
          ...
          else pResult = CMechAttackUnitState::Instance(pUnit, static_cast_ptr<CAIUnit *>(cmd.pObject), cmd.fNumber == 0, bSwarmAttack);
        }
      }
      else pUnit->SendAcknowledgement(pCommand, ACK_INVALID_TARGET, !pCommand->IsFromAI());
    }
...
IUnitState *CTransportStatesFactory::ProduceRestState(class CQueueUnit *pUnit) { return CMechUnitRestState::Instance(checked_cast<CAITransportUnit *>(pUnit), CVec2(-1, -1), 0); }

// **********************************************************************
// *CTransportWaitPassengerState*
// **********************************************************************

IUnitState *CTransportWaitPassengerState::Instance(CMilitaryCar *pTransport, CFormation *pFormation) { return new CTransportWaitPassengerState(pTransport, pFormation); }
...
{
  for (int i = 0; i < pTransport->GetNTurrets(); ++i) pTransport->GetTurret(i)->SetCanReturn();
...
{
  auto iter = formationsToWait.begin();
  while (iter != formationsToWait.end())
  {
    if (!IsValidObj(*iter) || (*iter)->GetState()->GetName() != EUSN_ENTER_TRANSPORT) iter = formationsToWait.erase(iter);
    else ++iter;
  }

  if (formationsToWait.empty()) pTransport->SetCommandFinished();
}
...
{
  auto iter = formationsToWait.begin();
  while (iter != formationsToWait.end() && *iter != pFormation) ++iter;

  if (iter == formationsToWait.end()) formationsToWait.push_back(pFormation);
}

const CVec2 CTransportWaitPassengerState::GetPurposePoint() const { return pTransport->GetCenter(); }

// **********************************************************************
// *CTransportLandState*
// **********************************************************************

IUnitState *CTransportLandState::Instance(CMilitaryCar *pTransport, const CVec2 &vLandPoint) { return new CTransportLandState(pTransport, vLandPoint); }

CTransportLandState::CTransportLandState(CMilitaryCar *_pTransport, const CVec2 &_vLandPoint)
  : state(ELS_STARTING), pTransport(_pTransport), vLandPoint(_vLandPoint)
{
  ...
  if (!pTransport->CanMove()) state = ELS_LANDING;
}

void CTransportLandState::LandPassenger(CSoldier *pLandUnit)
{
  if (pLandUnit->IsInSolidPlace()) pLandUnit->SetCoordWOUpdate(CVec3(pTransport->GetEntrancePoint(), 0));
  else pLandUnit->SetNewCoordinates(CVec3(pTransport->GetEntrancePoint(), 0));

  pLandUnit->SetFree();
  pTransport->DelPassenger(pLandUnit);
  pLandUnit->GetState()->TryInterruptState(nullptr);
...
void CTransportLandState::Segment()
{
  switch (state)
  {
    ...
    break;
    case ELS_MOVING:
      if (fabs2(pTransport->GetCenter() - vLandPoint) <= sqr(2 * pTransport->GetDistanceToLandPoint()) || pTransport->IsIdle()) state = ELS_LANDING;

      break;
    case ELS_LANDING:
    {
      // find all the formations in the transport that do not belong to the cannon that dangles behind
      CPtr<CArtillery> pArt = pTransport->GetTowedArtillery();
      CFormation *pGunCrew = !IsValidObj(pArt) ? nullptr : pArt->GetCrew();
      const int nGunCrew = pGunCrew == nullptr ? 0 : pGunCrew->Size();
      ...
      while (pTransport->GetNPassengers() != nGunCrew)
      {
        ...
        // find the formation to plant
        for (int i = 0; i < nPassangers && pFormation == nullptr; ++i)
        {
          CFormation *pTmp = pTransport->GetPassenger(i)->GetFormation();
          if (pTmp != pGunCrew) pFormation = pTmp;
        }
        ...
        for (int i = 0; i < nFormSize; ++i)
        {
          ...
          if (pSoldier->IsInTransport()) LandPassenger(pSoldier);
        }
        ...
        if (pFormation == pTransport->GetTowedArtilleryCrew()) pTransport->SetTowedArtilleryCrew(nullptr);
      }
...
const CVec2 CTransportLandState::GetPurposePoint() const { return vLandPoint; }

// **********************************************************************
// *CTransportHookArtilleryState*
// **********************************************************************

IUnitState *CTransportHookArtilleryState::Instance(CAITransportUnit *pTransport, CArtillery *pArtillery) { return new CTransportHookArtilleryState(pTransport, pArtillery); }

CTransportHookArtilleryState::CTransportHookArtilleryState(class CAITransportUnit *pTransport, class CArtillery *_pArtillery)
  : eState(TTGS_ESTIMATING), pArtillery(_pArtillery), pTransport(pTransport),
    vArtilleryPoint(_pArtillery->GetCenter()), timeLast(0), wDesiredTransportDir(0), bInterrupted(false) { if (!pArtillery->MustHaveCrewToOperate() || pTransport->IsTowing()) { TryInterruptState(nullptr); } }
...
void CTransportHookArtilleryState::InterruptBecauseOfPath()
{
  ...
  pTransport->SetCommandFinished();
  if (IsValidObj(pArtillery)) { if (pTransport == pArtillery->GetHookingTransport()) pArtillery->SetBeingHooked(nullptr); }
}
  ...
  while (bRepeat)
  {
    ...
    switch (eState)
    {
      case TTGS_ESTIMATING:
      {
        ...
        {
          if (fabs2(pArtillery->GetCenter() - pTransport->GetCenter()) < SConsts::TRANSPORT_MOVE_BACK_DISTANCE) { eState = TTGS_APPROACHING; }
          else
          {
            // drive up ahead.
      ...
      case TTGS_APPROACH_BY_MOVE_BACK:
        // the distance between the gun and the truck is almost equal to the size of the truck and the gun
        if (!pArtillery->HasServeCrew() || pArtillery->GetPlayer() != pTransport->GetPlayer())
        {
          // not possible, cannot take nither other player's artillery nor free artillery
          pArtillery->SetBeingHooked(nullptr);
          pTransport->SetCommandFinished();
        }
        else
        {
          // we approach the required distance to the gun
          ...
          else if (pTransport->IsIdle() && (pArtillery->IsInTankPit() || diff < SConsts::TILE_SIZE * 3)) { eState = TTGS_START_APPROACH_BY_CHEAT_PATH; }
      ...
      case TTGS_WAIT_FOR_LEAVE_TANKPIT:
        if (!pArtillery->IsInTankPit()) eState = TTGS_APPROACHING;
      ...
      case TTGS_START_APPROACH_BY_CHEAT_PATH:
      {
        ...
        else
        {
          ...
          if (pPath) { eState = TTGS_APPROACH_BY_CHEAT_PATH; }
          else
          {
            pTransport->SetRightDir(true);
            InterruptBecauseOfPath();
      ...
      case TTGS_START_UNINSTALL:
        pTransport->StopUnit();
        pArtillery->GetState()->TryInterruptState(nullptr);
      ...
      case TTGS_WAIT_FOR_UNINSTALL:
        ...
        {
          // turn the gun towards the truck
          pTransport->SetRightDir(true);
      ...
      case TTGS_WAIT_FOR_TURN:
      {
        ...
        else
        {
          ...
          if (trTurn && arTurn)
          {
            if (fabs(pTransport->GetHookPoint() - pArtillery->GetHookPoint()) > SConsts::TILE_SIZE) { eState = TTGS_APPROACH_BY_CHEAT_PATH; }
...
ETryStateInterruptResult CTransportHookArtilleryState::TryInterruptState(class CAICommand *pCommand)
{
  ...
  // not after death, it is allowed to interrupt only if you have not started to turn the gun
  ...
  if (!bInterrupted || !CanInterrupt()) return TSIR_NO_COMMAND_INCOMPATIBLE;
  pTransport->SetCommandFinished();
  pTransport->SetRightDir(true);
  bInterrupted = true;
  pArtillery->SetBeingHooked(nullptr);
...
const CVec2 CTransportHookArtilleryState::GetPurposePoint() const { return pTransport->GetCenter(); }

// **********************************************************************
// *CTransportUnhookArtilleryState*
// **********************************************************************

IUnitState *CTransportUnhookArtilleryState::Instance(CAITransportUnit *pTransport, const CVec2 &vDestPoint, const bool _bNow) { return new CTransportUnhookArtilleryState(pTransport, vDestPoint, _bNow); }
...
CTransportUnhookArtilleryState::CTransportUnhookArtilleryState(CAITransportUnit *_pTransport, const CVec2 &_vDestPoint, const bool _bNow)
  : eState(TUAS_START_APPROACH), pTransport(_pTransport),
    vDestPoint(_vDestPoint), nAttempt(0), bInterrupted(false), bNow(_bNow) { NI_ASSERT_T(pTransport->IsTowing(), "wrong towed artillery"); }
...
bool CTransportUnhookArtilleryState::CanPlaceUnit(const class CAIUnit *pUnit) const
{
  ...
  for (auto i = tiles.begin(); i != tiles.end(); ++i) { if (theStaticMap.IsLocked((*i), aiClass)) return false; }
...
void CTransportUnhookArtilleryState::Segment()
{
  ...
  switch (eState)
  {
    ...
    {
      if (bNow) eState = TUAS_ESTIMATING;
      else
      {
        CPtr<IStaticPath> pPath = pTransport->GetCurCmd()->CreateStaticPath(pTransport);
        if (!pPath) pPath = CreateStaticPathToPoint(vDestPoint, pTransport->GetGroupShift(), pTransport, true);
    ...
    case TUAS_APPROACHING:
      ...
      {
        auto pArtStats = static_cast<const SMechUnitRPGStats *>(pArt->GetStats());
        auto pTranspStats = static_cast<const SMechUnitRPGStats *>(pTransport->GetStats());
        ...
        else eState = TUAS_ESTIMATING;
      }
      else if (pTransport->IsIdle()) TryInterruptState(nullptr);
    ...
    case TUAS_ESTIMATING:
    {
      ...
      else { eState = TUAS_ADVANCE_A_LITTLE; }
    }

    break;
    case TUAS_MOVE_ARTILLERY_TO_THIS_POINT:
      if (pTransport->IsIdle()) eState = TUAS_ESTIMATING;
    ...
    case TUAS_START_UNHOOK:
    {
      ...
      // release the gun
      pTransport->SetTowedArtillery(nullptr);
      pArt->GetState()->TryInterruptState(nullptr);

      if (!pTransport->HasTowedArtilleryCrew()) TryInterruptState(nullptr);
      else
      {
        CFormation *pCrew = pTransport->GetTowedArtilleryCrew();
        // drive the gunners out of the transport
        CSoldier *pSold = nullptr;
        ...
        pCrew->StopUnit();
        pCrew->GetState()->TryInterruptState(nullptr);
        ...
        for (int i = 0; i < pCrew->Size(); ++i)
        {
          ...
          pSold->SetFree();
          pSold->GetState()->TryInterruptState(nullptr);
          pSold->RestoreDefaultPath();
          pSold->StopUnit();

          if (pSold->IsInSolidPlace()) pSold->SetCoordWOUpdate(point3D);
          else pSold->SetNewCoordinates(point3D);
        }
        pCrew->SetNewCoordinates(point3D);

        // let the gun now be selectable
        pArt->SetCrew(pCrew);

        // give the command to the gun to install
        theGroupLogic.UnitCommand(SAIUnitCmd(ACTION_COMMAND_INSTALL), pArt, false);

        // everyone, we can leave, they can cope without us
        pTransport->SetCommandFinished();
      }
    }
...
IUnitState *CMoveToPointNotPresize::Instance(class CAIUnit *_pTransport, const CVec2 &_vGeneralCell, const float _fRadius) { return new CMoveToPointNotPresize(_pTransport, _vGeneralCell, _fRadius); }

CMoveToPointNotPresize::CMoveToPointNotPresize(class CAIUnit *_pTransport, const CVec2 &vGeneralCell, const float _fRadius)
  : pTransport(_pTransport), vPurposePoint(vGeneralCell), fRadius(_fRadius) { SendToPurposePoint(); }
...
void CMoveToPointNotPresize::SendToPurposePoint()
{
  ...
  if (pPath) pTransport->SendAlongPath(pPath, VNULL2);
}
...
void CMoveToPointNotPresize::Segment()
{
  ...
  else if (pTransport->IsIdle()) { SendToPurposePoint(); }
}
