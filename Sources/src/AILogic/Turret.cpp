...
void CTurret::SetTurnParameters(SRotating *pRotateInfo, const WORD wAngle, const bool bInstantly)
{
  if (!bInstantly)
  {
    ...
    pRotateInfo->sign = (static_cast<WORD>(wAngle - pRotateInfo->wCurAngle) == wRotateAngle) ? 1 : -1;
...
WORD CTurret::ConstraintAngle(const WORD wDesAngle, const WORD wTurnConstraint) const
{
  ...
  {
    if (DirsDifference(wDesAngle, wTurnConstraint) < DirsDifference(wDesAngle, -wTurnConstraint)) return wTurnConstraint;
...
int GetSignOfTurn(const WORD wStartAngle, const WORD wFinishAngle)
{
  ...
  return (static_cast<WORD>(wFinishAngle - wStartAngle) == wRotateAngle) ? 1 : -1;
}

void CTurret::Segment()
{
  if (!hor.bFinished && curTime >= hor.endTime) hor.bFinished = true, hor.wCurAngle = hor.wFinalAngle;

  if (!ver.bFinished && curTime >= ver.endTime) ver.bFinished = true, ver.wCurAngle = ver.wFinalAngle;
  /* if ( !hor.bFinished && GetHorCurAngle() > GetHorTurnConstraint() || 
          */
  if (pTracedUnit != nullptr)
  {
    ...
    else
    {
      ...
      if (hor.bFinished || GetSignOfTurn(GetHorCurAngle(), wDesHorAngle) != hor.sign) TurnHor(wDesHorAngle);

      if (bVerAiming)
      {
        const WORD wDesVerAngle = ConstraintAngle(GetZAngle(toTracedUnit, pTracedUnit->GetZ() - GetOwnerZ()), GetVerTurnConstraint());
        if (ver.bFinished || GetSignOfTurn(GetVerCurAngle(), wDesVerAngle) != ver.sign) TurnVer(wDesVerAngle + 16384 * 3);
      }
    }
  }
  else if (bCanReturn && IsOwnerOperable())
  {
    ...
    if (bReturnToNULLVerAngle) Turn(wDefaultHorAngle, 16384 * 3);
  }
}

WORD CTurret::GetCurAngle(const SRotating &rotateInfo) const
{
  if (rotateInfo.bFinished) return rotateInfo.wCurAngle;
  if (curTime >= rotateInfo.endTime) return rotateInfo.wFinalAngle;
  return rotateInfo.wCurAngle + rotateInfo.sign * rotateInfo.wRotationSpeed * (curTime - rotateInfo.startTime);
}

void CTurret::TraceAim(CAIUnit *pUnit, CBasicGun *pGun) { if (!IsLocked(pGun)) pTracedUnit = pUnit; }
...
void CTurret::StopHorTurning() { TurnHor(GetHorCurAngle()); }

void CTurret::StopVerTurning() { TurnVer(GetVerCurAngle()); }
...
void CTurret::Lock(const CBasicGun *pGun) { pLockingGun = const_cast<CBasicGun *>(pGun); }

void CTurret::Unlock(const CBasicGun *pGun)
{
  if (pLockingGun != nullptr && (!pLockingGun->IsValid() || !pLockingGun->IsAlive())) pLockingGun = nullptr;
  if (pGun == pLockingGun) pLockingGun = nullptr;
}

bool CTurret::IsLocked(const CBasicGun *pGun) { return IsValidObj(pLockingGun) && pLockingGun != pGun; }

void CTurret::SetCanReturn() { if (bReturnToNULLVerAngle && GetVerCurAngle() != 16384 * 3 || GetHorCurAngle() != 0) bCanReturn = true; }
...
void CUnitTurret::GetHorTurretTurnInfo(SAINotifyTurretTurn *pTurretTurn)
{
  ...
  pTurretTurn->nModelPart = static_cast<DWORD>(nModelPart) | 0xffffff00;
...
CVec2 CUnitTurret::GetOwnerCenter() { return pOwner->GetCenter(); }

WORD CUnitTurret::GetOwnerFrontDir() { return pOwner->GetFrontDir(); }

float CUnitTurret::GetOwnerZ() { return pOwner->GetZ(); }

bool CUnitTurret::IsOwnerOperable() const { return pOwner->IsOperable(); }

bool CUnitTurret::IsAlive() const { return pOwner->IsAlive(); }

const int CUnitTurret::GetOwnerParty() const { return pOwner->GetParty(); }
...
WORD CMountedTurret::GetHorTurnConstraint() const { return wHorTurnConstraint; }

WORD CMountedTurret::GetVerTurnConstraint() const { return wVerTurnConstraint; }

bool CMountedTurret::IsAlive() const { return pBuilding->IsAlive(); }

const int CMountedTurret::GetOwnerParty() const { return theDipl.GetNParty(pBuilding->GetPlayer()); }