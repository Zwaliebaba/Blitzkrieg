...
void CUnitCreation::CLightPlaneCreation::CalcPositions(const int nMax, const CVec2 &box, const CVec2 &direction, std::vector<CVec2> *positions, CVec2 *offset, const bool bRandom)
{
  ...
  {
    if (i % 2 == 0) positions->push_back(vStartOffset /* ^ direction */);
    else
    {
...
void CUnitCreation::CHeavyPlaneCreation::CalcPositions(const int nMax, const CVec2 &box, const CVec2 &direction, std::vector<CVec2> *positions, CVec2 *offset, bool bRandom)
{
  ...
  int iLimit = nSize;
  CVec2 vStartOffset(0, 0);// first point offset ( 1, 5, 11 )

  const float resize = SConsts::PLANES_HEAVY_FORMATION_SIZE;

  *offset = (CVec2(resize * box.y, 0) * static_cast<int>(nSize / 2)) ^ direction;

  while (iLimit > 0)
  {
    // sequentially fill the diagonals
    for (int i = 0; i < iLimit; ++i)
    {
      CVec2 res(CVec2(vStartOffset.x, vStartOffset.y) + CVec2(-resize * i * box.y, 0));
      if (bRandom) res.x -= box.y * Random(0.0f, SConsts::PLANES_START_RANDOM);

      positions->push_back(res/* ^direction */);
      if (positions->size() == nMax) return;
      // non-centers - 2 times
      if (iLimit != nSize)
      {
        CVec2 res(CVec2(vStartOffset.x, -vStartOffset.y) + CVec2(-resize * i * box.y, 0));
...
const char *CUnitCreation::STankPitInfo::GetRandomTankPit(const class CVec2 &vSize, const bool bCanDig, float *pfResize) const
{
  ...
  for (int i = 0; i < pPits->size(); ++i)
  {
    ...
    auto pStats = static_cast<const SMechUnitRPGStats *>(theUnitCreation.GetObjectDB()->GetRPGStats(pDesc));
...
void CUnitCreation::EnableAviationButtons(const bool bInit)
{
  ...
  for (int nAvia = 0; nAvia < SUCAviation::AT_COUNT; ++nAvia)
  {
    if (IsAviaEnabledMapParameters(nPlayer, nAvia) &&
        ...
        !theWeather.IsActive())
    {
      auto eFeed = static_cast<EFeedBack>(feedbacks[nAvia].eEnable);
      updater.AddFeedBack(eFeed);
      bSomePlaneEnabled = true;
    }
    else
    {
      auto eFeed = static_cast<EFeedBack>(feedbacks[nAvia].eDisable);
      updater.AddFeedBack(eFeed);
    }
  }
...
void CUnitCreation::BadWeatherStarted()
{
  ...
  if (bEnabled) DisableMainAviationButton(0);
}
...
void CUnitCreation::EnableAviationScript(const int nPlayer, const int nAvia)
{
  ...
  if (nAvia == -1)
  {
    ...
    for (int i = 0; i < SUCAviation::AT_COUNT; ++i) { EnableAviationScript(nPlayer, i); }
    bForceDisabled[nPlayer] = false;
  ...
  if (IsAviaEnabledMapParameters(nPlayer, nAvia)) inGameUnits[nPlayer].planes[nAvia].bEnabledScript = true;

  // enable main button if was disabled
  if (theDipl.GetMyNumber() == nPlayer) updater.AddFeedBack(static_cast<EFeedBack>(feedbacks[nAvia].eEnable));
}
...
void CUnitCreation::DisableAviationScript(const int nPlayer, const int nAvia)
{
  ...
  if (nAvia == -1)
  {
    ...
    for (int i = 0; i < SUCAviation::AT_COUNT; ++i) { DisableAviationScript(nPlayer, i); }
    return;
  }

  CheckRange(inGameUnits[nPlayer].planes, nAvia);

  // if this plane was allowed, then prohibit it
  ...
  if (IsAviaEnabledMapParameters(nPlayer, nAvia) && IsAviaEnabledScript(nPlayer, nAvia)) planeInfo.bEnabledScript = false;
  ...
  if (theDipl.GetMyNumber() == nPlayer)
  {
    ...
    if (!bMainButtonStillEnable) DisableMainAviationButton(0);
  }
}
...
void CUnitCreation::Segment()
{
  ...
  if (bInit)
  {
    if ((nDipl >= inGameUnits.size()) ||// we are not on the list
        (0 == inGameUnits[nDipl].vAppearPoints.size()))// no airfields
    {
      bForceDisabled[nDipl] = true;
      DisableMainAviationButton(0);// ban calling aviation completely
    }
    else
    {
  ...
  if (!bForceDisabled[nDipl] &&// if airplanes are possible in principle
      bMainButtonDisabled &&// but are currently prohibited
      !theWeather.IsActive())// weather is suitable for planes
  {
    if (curTime >= inGameUnits[nDipl].timeLastCall + inGameUnits[nDipl].timeRelax ||
        inGameUnits[nDipl].timeLastCall == 0)
    {
      // the time has come to clear the planes
...
void CUnitCreation::SLocalInGameUnitCreationInfo::Copy(const SUnitCreation &rSUnitCreation)
{
  ...
  for (auto appearPointIterator = rSUnitCreation.aviation.vAppearPoints.begin();
       appearPointIterator != rSUnitCreation.aviation.vAppearPoints.end(); ++appearPointIterator) { vAppearPoints.push_back(CVec2(appearPointIterator->x, appearPointIterator->y)); }
}

CUnitCreation::SLocalInGameUnitCreationInfo::SLocalInGameUnitCreationInfo(const SUnitCreation &rSUnitCreation) { Copy(rSUnitCreation); }
...
void CUnitCreation::Init(const struct SUnitCreationInfo &rUnitCreationInfo)
{
  ...
  for (auto unitCreationIterator = rUnitCreationInfo.units.begin();
       unitCreationIterator != rUnitCreationInfo.units.end();
       ++unitCreationIterator) { inGameUnits.push_back(SLocalInGameUnitCreationInfo((*unitCreationIterator))); }
...
const NTimer::STime CUnitCreation::GetPlaneRegenerateTime(const int nPlayer) const { return inGameUnits[nPlayer].timeRelax; }

const float CUnitCreation::GetPlaneFlyHeight(const int nPlayer, const int nAvia) { return GetPlaneStats(nPlayer, nAvia)->fMaxHeight; }
...
void CUnitCreation::RegisterAviationCall(const int nPlayer, const int nAviaType)
{
  ...
  if (nPlayer == theDipl.GetMyNumber())
  {
    ...
    if (0 == inGameUnits[nPlayer].planes[nAviaType].nPlanes)// ran out of planes
    {
      auto eFeed = static_cast<EFeedBack>(feedbacks[nAviaType].eDisable);
      updater.AddFeedBack(eFeed);
    }
  }
                              ...
                              IPlaneCreation *pCreation)
{
  ...
  {
    theGroupLogic.RegisterGroup(nullptr, 0, wGroupID);
    return;
  }
  ...
  auto pStats = static_cast<const SMechUnitRPGStats *>(pIDB->GetRPGStats(pDesc));
  const int dbID = pIDB->GetIndex(szPlaneName.c_str());
  constexpr EObjVisType eVisType = SGVOT_MESH;
  ...
  if (bNeedFormation)
  {
    ...
    for (int i = 0; i < positions.size(); ++i) fMaxOffset = Max(fMaxOffset, fabs(positions[i].y));
  ...
  for (int i = 0; i < nAvia; ++i)
  {
    ...
    auto pUnit = static_cast<CAviation *>(units[id]);
    arUnits[i] = pUnit;
...
void CUnitCreation::CreateMine(const enum SMineRPGStats::EType nType, const class CVec2 &vPoint, const int nDipl)
{
  ...
  // mine stats are taken from the structure specified in the map
  const std::string *pszName = nullptr;
  switch (nType)
  {
...
void CUnitCreation::SendFormationToWorld(CFormation *pUnit) const
{
  ...
  for (int j = 0; j < pUnit->Size(); ++j) updater.Update(ACTION_NOTIFY_NEW_FORMATION, (*pUnit)[j]);
...
CFormation *CUnitCreation::CreateCrew(CArtillery *pUnit, IObjectsDB *_pIDB, const int nUnits, const CVec3 vPos, const int _nPlayer, const bool bImmidiateAttach) const
{
  ...
  if (-1 == vPos.x) vCreatePos = CVec3(pUnit->GetCenter(), pUnit->GetZ());

  // if the player is not specified manually, then we use the same one as the unit
  const int nPlayer = (_nPlayer == -1) ? pUnit->GetPlayer() : _nPlayer;

  // for machine guns - your own platoon - machine gunners
  const char *pszName = RPG_TYPE_ART_HEAVY_MG == pUnit->GetStats()->type ? GetPartyDependentInfo(nPlayer).szHeavyMGSquad.c_str() : GetPartyDependentInfo(nPlayer).szGunCrewSquad.c_str();
  ...
  auto pForm = static_cast<CFormation *>
  ...
  if (bImmidiateAttach) pUnit->SetCrew(pForm);
...
{
  CPtr<CAIUnit> pUnit;
  if (pStats->IsInfantry()) { pUnit = pStats->type == RPG_TYPE_SNIPER ? CreateObject<CAIUnit>(AI_SNIPER) : CreateObject<CAIUnit>(AI_INFANTRY); }
  else if (pStats->IsAviation()) pUnit = CreateObject<CAIUnit>(AI_AVIATION);
  else if (pStats->IsTransport()) pUnit = CreateObject<CAIUnit>(AI_TRANSPORT_UNIT);
  else if (pStats->IsArtillery() || pStats->type == RPG_TYPE_TRAIN_SUPER) pUnit = CreateObject<CAIUnit>(AI_ARTILLERY);
  else if (pStats->IsArmor() || pStats->IsSPG() || pStats->IsTrain()) pUnit = CreateObject<CAIUnit>(AI_TANK);
  else
  ...
  if (bSendToWorld) updater.Update(ACTION_NOTIFY_NEW_UNIT, pUnit);
  ...
  // by default all our units are selected
  pUnit->SetSelectable(player == theDipl.GetMyNumber());

  if (pStats->IsArtillery() && theDipl.GetNeutralPlayer() != player)
  {
    auto pArt = static_cast_ptr<CArtillery *>(pUnit);
    if (pArt->MustHaveCrewToOperate() && !IsEditor) CreateCrew(pArt, pIDB);
    else pArt->SetOperable(1.0f);
  }
...
{
  if (!bLockedFlags[nPlayer] && bLocked) vLockedAppearPoints[nPlayer] = GetRandomAppearPoint(nPlayer);
...
CVec2 CUnitCreation::GetRandomAppearPoint(const int _nPlayer, const bool bLeave) const
{
  ...
  if (bLockedFlags[nPlayer]) return vLockedAppearPoints[nPlayer];
  return inGameUnits[nPlayer].vAppearPoints[nCurrentRandom];
}

const char *CUnitCreation::GetRandomAntitankObjectName() const { return commonInfo.antitankObjects[Random(commonInfo.antitankObjects.size())].c_str(); }

const char *CUnitCreation::GetEntrenchmentName() const { return commonInfo.szEntrenchment.c_str(); }

const char *CUnitCreation::GetWireFenceName() const { return commonInfo.szAPFence.c_str(); }

int CUnitCreation::GetNParadropers(const int nPlayer) const { return inGameUnits[nPlayer].nParadropSquadCount; }

int CUnitCreation::GetParadropperDBID(const int nPlayer) const { return pIDB->GetIndex(GetPartyDependentInfo(nPlayer).szParatroopSoldierName.c_str()); }

const char *CUnitCreation::GetRandomTankPit(const class CVec2 &vSize, const bool bCanDig, float *pfResize) const { return tankPitInfo.GetRandomTankPit(vSize, bCanDig, pfResize); }
...
void CUnitCreation::GetCentersOfAllFormationUnits(const SSquadRPGStats *pStats, const CVec2 &vFormCenter, const WORD wFormDir, const int nFormation, const int nUnits, std::list<CVec2> *pCenters) const
{
  ...
  for (int j = 0; j < nSizeOfFormation; ++j)
  {
    ...
    vUnitCenter.x = Clamp(vUnitCenter.x, 0.0f, static_cast<float>(theStaticMap.GetSizeX() * SConsts::TILE_SIZE - 1));
    vUnitCenter.y = Clamp(vUnitCenter.y, 0.0f, static_cast<float>(theStaticMap.GetSizeY() * SConsts::TILE_SIZE - 1));
...
{
  if (nPlayer >= inGameUnits.size()) return -1;
...
const CVec2 CUnitCreation::GetFirstIntercectWithMap(const int nPlayer)
{
  ...
  if (!theStaticMap.IsPointInside(vAppearPoint))
  {
    ...
    float fIntercect = 0;
    // find the intersection point of the course to the center of the map and the edge of the map.
    for (int i = 0; i < 4; ++i)
    {
      const CVec2 vBegin = rect.v[i];
      const CVec2 vEnd = rect.v[(i + 1) % 4];
      if (CI_SKEW_CROSS == ClassifyCross(vBegin, vEnd, vAppearPoint, rect.center, &fIntercect)) { return vBegin + (vEnd - vBegin) * fIntercect; }
    }
  }
  return vAppearPoint;
}
...
CCommonUnit *CUnitCreation::AddNewFormation(const SSquadRPGStats *pStats, const int nFormation, const float fHP, const float x, const float y, const float z, const WORD wDir, const int nDiplomacy, bool bInitialization, bool bSendToWorld, const int nUnits) const
{
  ...
  for (int j = 0; j < nSizeOfFormation; ++j, ++iter)
  {
    ...
    if (bSendToWorld) updater.Update(ACTION_NOTIFY_NEW_FORMATION, static_cast<CSoldier *>(pUnit));
  }
  ...
  for (int i = 0; i < pFormation->Size(); ++i)
  {
    auto vNewCoord = CVec3((*pFormation)[i]->GetUnitPointInFormation(), 0.0f);
    vNewCoord.x = Clamp(vNewCoord.x, 0.0f, static_cast<float>(theStaticMap.GetSizeX() * SConsts::TILE_SIZE - 1));
    vNewCoord.y = Clamp(vNewCoord.y, 0.0f, static_cast<float>(theStaticMap.GetSizeY() * SConsts::TILE_SIZE - 1));
...
{
  for (int i = 0; i < commonInfo.antitankObjects.size(); ++i) { if (pStats->szParentName == commonInfo.antitankObjects[i]) return true; }

  return false;
}

bool CUnitCreation::IsAPFence(const SHPObjectRPGStats *pStats) const { return pStats->szParentName == commonInfo.szAPFence; }