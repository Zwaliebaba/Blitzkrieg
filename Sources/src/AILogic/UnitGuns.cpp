...
void CUnitGuns::AddGun(const interface IGunsFactory &gunsFactory, const SWeaponRPGStats *pWeapon, int *nGuns, const int nAmmo)
{
  ...
  if (commonGunsInfo.size() <= nCommonGun) commonGunsInfo.resize(nCommonGun + 1);
  if (gunsBegins.size() <= nCommonGun + 1) gunsBegins.resize(nCommonGun + 2);
  ...
  if (nCommonGuns < nCommonGun + 1) nCommonGuns = nCommonGun + 1;

  if (guns.size() < *nGuns + pWeapon->shells.size()) guns.resize(*nGuns + pWeapon->shells.size());
  for (int i = 0; i < pWeapon->shells.size(); ++i)
  {
    CBasicGun *pGun;

    if (pWeapon->shells[i].trajectory == SWeaponRPGStats::SShell::TRAJECTORY_BOMB) pGun = gunsFactory.CreateGun(IGunsFactory::PLANE_GUN, i, commonGunsInfo[nCommonGun]);// when create bomb
    else if (pWeapon->shells[i].trajectory == SWeaponRPGStats::SShell::TRAJECTORY_LINE)
    {
      if (pWeapon->shells[i].fArea2 == 0) pGun = gunsFactory.CreateGun(IGunsFactory::MOMENT_CML_GUN, i, commonGunsInfo[nCommonGun]);
      else pGun = gunsFactory.CreateGun(IGunsFactory::MOMENT_BURST_GUN, i, commonGunsInfo[nCommonGun]);
    }
    else
    {
      if (pWeapon->shells[i].fArea2 == 0) pGun = gunsFactory.CreateGun(IGunsFactory::VIS_CML_BALLIST_GUN, i, commonGunsInfo[nCommonGun]);
      else pGun = gunsFactory.CreateGun(IGunsFactory::VIS_BURST_BALLIST_GUN, i, commonGunsInfo[nCommonGun]);
    }

    guns[(*nGuns)++] = pGun;
    if (pGun->GetFireRangeMax() > fMaxFireRange) fMaxFireRange = pGun->GetFireRangeMax();

    if (pGun->GetWeapon()->nCeiling > 0) bCanShootToPlanes = true;

    if (i == 0 && pGun->GetGun().nPriority == 0) nMainGun = (*nGuns) - 1;
  }
}

void CUnitGuns::Segment() { for (int i = 0; i < guns.size(); ++i) guns[i]->Segment(); }

bool CUnitGuns::FindTimeToGo(CAIUnit *pUnit, CAIUnit *pEnemy, std::list<SWeaponPathInfo> *pPathInfo, const SWeaponRPGStats *pStats, SWeaponPathInfo *pInfo) const
{
  const float fFireRangeMax = GetFireRangeMax(pStats, pUnit);
  for (auto iter = pPathInfo->begin(); iter != pPathInfo->end(); ++iter)
  {
...
{
  *pTimeToTurn = 0;
  // you need to take into account the time for turns
  if (pOwner->CanRotate() && (!bIsEnemyInFireRange || pTurret == nullptr))
  {
    ...
    if (dirsDiff > wWillPower) *pTimeToTurn = dirsDiff / pOwner->GetRotateSpeed();
  }
  ...
  if (pTurret != nullptr)
  {
    ...
    WORD finalAngle;
    if (!bIsEnemyInFireRange) finalAngle = GetDirectionByVector((pEnemy->GetTile() - finishTile).ToCVec2());
    else finalAngle = GetDirectionByVector((pEnemy->GetTile() - pOwner->GetTile()).ToCVec2());
...
bool CUnitGuns::FindTimeToStatObjGo(CAIUnit *pUnit, CStaticObject *pObj, std::list<SWeaponPathInfo> *pPathInfo, const SWeaponRPGStats *pStats, SWeaponPathInfo *pInfo) const
{
  const float fFireRangeMax = GetFireRangeMax(pStats, pUnit);
  for (auto iter = pPathInfo->begin(); iter != pPathInfo->end(); ++iter)
  {
...
{
  if (pOwner->GetNGuns() == 0) return nullptr;
  ...
  *pTime = 0;
  ...
  do
  {
    ...
    // explosive shells and possibly finish shooting
    if (shell.eDamageType != SWeaponRPGStats::SShell::DAMAGE_HEALTH || shell.fArea2 <= 0) pGun->SetRejectReason(ACK_NEGATIVE);
    else if (pGun->CanShootToObject(pObj))
    {
      ...
      {
        // if we are in a formation and we cannot go beyond it, but in order to shoot at a unit we have to
        if (pOwner->CanMove() && !pOwner->CanGoToPoint(info.pStaticPath->GetFinishPoint())) continue;

        NTimer::STime time;
        if (pOwner->CanMove()) time = info.time;
        else time = 0;
        ...
        // number of bursts * (aiming time + (number of shots - 1) * time between shots)
    ...
    ++i;
    if (i < GetNTotalGuns()) pWStats = pOwner->GetGun(i)->GetWeapon();
  } while (i < pOwner->GetNGuns());

  if (nGun == -1) return nullptr;
  return pOwner->GetGun(nGun);
}

void CUnitGuns::SetOwner(CAIUnit *pUnit) { for (int i = 0; i < guns.size(); ++i) guns[i]->SetOwner(pUnit); }
...
{
  for (int i = 0; i < guns.size(); ++i) { if (guns[i]->GetRejectReason() == ackType) return true; }
...
{
  if (guns.size() != 0) return guns[nMainGun];
  return nullptr;
}
...
float CUnitGuns::GetMaxFireRange(const CAIUnit *pOwner) const
{
  ...
  return fMaxFireRange * (1 + (SConsts::BAD_WEATHER_FIRE_RANGE_COEFFICIENT - 1) * static_cast<int>(theWeather.IsActive())) * fExpCoeff;
}
...
{
  auto pUnit = static_cast<CAIUnit *>(pCommonUnit);
  ...
  for (int i = 0; i < pStats->platforms.size(); ++i)
  {
    for (int j = pStats->platforms[i].nFirstGun; j < pStats->platforms[i].nFirstGun + pStats->platforms[i].nNumGuns; ++j)
    {
      ...
      if (pStats->guns[j].pWeapon->fRevealRadius > fMaxRevealRadius) fMaxRevealRadius = pStats->guns[j].pWeapon->fRevealRadius;
    }
  }

  if (fMaxRevealRadius > 0) pUnit->CreateAntiArtillery(fMaxRevealRadius);
...
{
  if (nFirstArtGun == -1) return SWeaponRPGStats::SShell::DAMAGE_HEALTH;
  return GetGun(nFirstArtGun)->GetShell().eDamageType;
}

class CBasicGun *CMechUnitGuns::GetFirstArtilleryGun() const
{
  if (nFirstArtGun >= 0) return GetGun(nFirstArtGun);
  return nullptr;
}
...
{
  auto pUnit = static_cast<CSoldier *>(pCommonUnit);
  auto pStats = static_cast<const SInfantryRPGStats *>(pUnit->GetStats());

  int nGuns = 0;
  for (int i = 0; i < pStats->guns.size(); ++i) AddGun(CUnitsGunsFactory(pUnit, i, -1), pStats->guns[i].pWeapon, &nGuns, pStats->guns[i].nAmmo);
}