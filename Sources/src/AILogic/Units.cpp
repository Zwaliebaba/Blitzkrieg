...
const SVector GetLeveledCell(const SVector &bigCell, const int nCellLevel) { return SVector(bigCell.x / (1 << (nCellLevel + 1)), bigCell.y / (1 << (nCellLevel + 1))); }

const bool CUnits::IsUnitInCell(const int nUnitID) const { return posUnitInCell[nUnitID].nUnitPos != 0 || posUnitInCell[nUnitID].nCellID != 0; }
...
{
  // if the unit is the only one in its cell, write the cell to the list
  if (++nUnitsCell[cell.y][cell.x] == 1) nCell[cell.y][cell.x] = cellsIds.GetFreeId();

  // add a unit to the list of those standing on this cell
  const int newId = nCell[cell.y][cell.x] * 2 * 3 + (2 * pUnit->GetParty() + static_cast<BYTE>(pUnit->GetStats()->IsInfantry())) + 1;
  ...
  if (bWithLeveledCelles) AddUnitToLeveledCells(pUnit, cell, nVisIndex);
...
void CUnits::AddUnitToCell(CAIUnit *pUnit, const CVec2 &newPos, bool bWithLeveledCelles)
{
  ...
  if (theStaticMap.IsBigCellInside(bigCell)) AddUnitToConcreteCell(pUnit, bigCell, bWithLeveledCelles);
}
...
void CUnits::DelUnitFromCell(CAIUnit *pUnit, bool bWithLeveledCelles)
{
  ...
  if (IsUnitInCell(nUnitID))
  {
    ...
    if (--nUnitsCell[cell.y][cell.x] == 0) cellsIds.AddToFreeId(nCell[cell.y][cell.x]);

    if (bWithLeveledCelles) DelUnitFromLeveledCells(pUnit, cell, nVisIndex);
...
{
  if (pUnit->GetStats()->IsAviation()) planes.push_back(static_cast<CAviation *>(pUnit));

  const int nUnitID = pUnit->GetID();
  if (nUnitID >= posUnitInCell.size()) posUnitInCell.resize(nUnitID * 1.5);

  // needs to be added to the cell
  if (!pUnit->IsInSolidPlace()) AddUnitToCell(pUnit, true);
  ...
  if (nUnitsOfType[nParty].find(nUnitType) == nUnitsOfType[nParty].end()) nUnitsOfType[nParty][nUnitType] = 1;
  else ++nUnitsOfType[nParty][nUnitType];
}
...
void CUnits::DeleteUnitFromMap(CAIUnit *pUnit)
{
  ...
  const int nUnitID = pUnit->GetID();
  // has not yet been removed from the cells
  if (units.GetEl(nUnitID) != nullptr)
  {
    DelUnitFromCell(pUnit, true);
    units.GetEl(nUnitID) = nullptr;
  }
...
void CUnits::UnitChangedPosition(CAIUnit *pUnit, const CVec2 &newPos)
{
  ...
  {
    if (bInOldCell) DelUnitFromCell(pUnit, false);
    ...
    for (int i = 0; i < N_CELLS_LEVELS; ++i)
    {
      ...
      if (oldLeveledCell != newLeveledCell || !bInOldCell || !bInNewCell)
      {
        ...
        if (bInNewCell) ++numUnits[nVisIndex][i][nUnitParty][!bUnitMech][newLeveledCell.y][newLeveledCell.x];
      }
    }
  }

  if (AICellsTiles::GetGeneralCell(pUnit->GetCenter()) != AICellsTiles::GetGeneralCell(newPos)) theSupremeBeing.UnitChangedPosition(pUnit, newPos);
}

CAIUnit *CUnits::operator[](const int id)
{
  NI_ASSERT_T(units.GetEl( id ) == nullptr || !units.GetEl( id )->IsValid() || units.GetEl( id )->GetID() == id, "Wrong units' id");
