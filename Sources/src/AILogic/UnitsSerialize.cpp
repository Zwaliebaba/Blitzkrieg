...
int CUnits::operator&(IStructureSaver &ss)
{
  ...
  else { for (int nVis = 0; nVis < 2; ++nVis) { for (int i = 0; i < N_CELLS_LEVELS; ++i) { for (int j = 0; j < 3; ++j) { for (int k = 0; k < 2; ++k) saver.Add(cnt++, &(numUnits[nVis][i][j][k])); } } } }

  saver.Add(100, &unitsInCellsSet);
  saver.Add(101, &nUnitsOfType);

  if (nUnitsOfType.empty()) nUnitsOfType.resize(3);

  saver.Add(102, &posUnitInCell);
  // for compatibitility with old saves
  if (posUnitInCell.empty()) posUnitInCell.resize(10000);

  saver.Add(103, &unitsInCells);
  // for compatibitility with old saves
  if (unitsInCells.empty()) unitsInCells.resize(2);
