...
interface IUpdatableObj : IRefCount
{
  virtual bool IsSelectable() const
  {
    NI_ASSERT_T(false, "Wrong call of IsSelectable");
    return false;
  };

  virtual int GetMovingType() const
  {
    NI_ASSERT_T(false, "Wrong call of GetMovingType");
    return 0;
  }
  ...
  virtual const NTimer::STime GetTimeOfDeath() const
  {
    NI_ASSERT_T(false, "Wrong call of GetTimeOfDeath");
    return 0;
  }
  ...
  {
    NI_ASSERT_T(false, "Wrong call of IsAlive");
    return false;
  }
  ...
  {
    NI_ASSERT_T(false, "Wrong call of GetDieAction");
    return ACTION_NOTIFY_NONE;
  }

  virtual const BYTE GetPlayer() const
  {
    NI_ASSERT_T(false, "Wrong call of GetPlayer");
    return 0xff;
  }

  virtual const int GetNAIGroup() const { return -2; }

  virtual float GetTerrainHeight(float x, float y, NTimer::STime timeDiff) const;

  virtual const int GetUnitState() const { return 0; }

  // must be constant so that there are no discrepancies in multiplayerb
  virtual const bool IsVisible(BYTE cParty) const = 0;
  ...
  {
    NI_ASSERT_T(false, "Wrong call of GetDieObject");
    return nullptr;
  }
