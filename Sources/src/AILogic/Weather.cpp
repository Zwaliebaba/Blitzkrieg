...
void CWeather::Init() { SwitchAutomatic(true); }
...
bool CWeather::IsActive() const { return eState != EWS_CLEAR; }
...
void CWeather::Segment()
{
  ...
  {
    case EWS_CLEAR:
      if (bAutoChangeWeather && timeNextCheck < curTime) On();

      break;
    case EWS_BAD:
      if (bAutoChangeWeather && timeNextCheck < curTime) Off();
