...
/* ////////////////////////////////////////////////////////////////////
                           */
// The size of buffer that DiagAssert will use.  
...
// resources.
using HINSTVECTOR = vector<HINSTANCE>;
// The address typedef.
using ADDRVECTOR = vector<ULONG>;
...
BOOL STDCALL DiagAssert(DWORD dwOverrideOpts, LPCTSTR szMsg, LPCSTR szFile, DWORD dwLine)
{
  ...
  for (auto loop = g_HMODVector.begin(); loop != g_HMODVector.end(); ++loop)
  {
  ...
  // Make sure the message gets translated into something.
  LPTSTR szRealLastErr;
  if (nullptr != szFmtMsg) szRealLastErr = szFmtMsg;
  else szRealLastErr = _T("**Last error code does not exist!!!!");

  // Get the module name.
  if (0 == GetModuleFileName(nullptr, szModName, MAX_PATH))
  ...
  if (nullptr != szFmtMsg) LocalFree(szFmtMsg);

  // Am I supposed to show the stack trace too?
  if (DA_SHOWSTACKTRACE == (DA_SHOWSTACKTRACE & dwOpts)) DoStackTrace(pCurrPos, sizeof(szBuff) - ((DWORD) pCurrPos - (DWORD) szBuff), 2);

  // Is this supposed to go to ODS?
  if (DA_SHOWODS == (DA_SHOWODS & dwOpts))
    OutputDebugString(szBuff);

  // By default, treat the return as an IGNORE.  
  // the case the user does not want the MessageBox.
  int iRet = IDIGNORE;
  if (DA_SHOWMSGBOX == (DA_SHOWMSGBOX & dwOpts))
  {
    HWND hWndParent = GetActiveWindow();
    if (nullptr != hWndParent) hWndParent = GetLastActivePopup(hWndParent);
  ...
  if (IDIGNORE == iRet) return FALSE;
  // This will trigger DebugBreak!!
  if (IDRETRY == iRet) return TRUE;

  // The return has to be Abortion....
  ExitProcess(-1);
  return TRUE;
}
...
static DWORD STDCALL GetModBase(HANDLE hProcess, DWORD dwAddr)
{
  ...
  // This is what the MFC stack trace routines forgot to do so them
  ...
  if (0 != VirtualQueryEx(hProcess, (LPCVOID) dwAddr, &stMBI, sizeof(stMBI)))
  {
    ...
    dwNameLen = GetModuleFileName(static_cast<HINSTANCE>(stMBI.AllocationBase), szFile, MAX_PATH);

    HANDLE hFile = nullptr;
    if (0 != dwNameLen) hFile = CreateFile(szFile, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0, nullptr);
#ifdef _DEBUG
    DWORD dwRet =
#endif
        g_cSym.SymLoadModule(hFile, (dwNameLen ? szFile : nullptr), nullptr, (DWORD) stMBI.AllocationBase, 0);
#ifdef _DEBUG
...
static DWORD ConvertAddress(DWORD dwAddr, LPTSTR szOutBuff)
{
  ...
  auto pIHS = (PIMAGEHLP_SYMBOL) &szTemp;
...
static void DoStackTrace(LPTSTR szString, DWORD dwSize, DWORD dwNumSkip)
{
  ...
  if (FALSE == g_bSymIsInit)
  {
    ...
    if (FALSE == g_cSym.SymInitialize(hProcess, nullptr, FALSE))
    {
...
#error ( "Unknown machine!" )
    ...
    {
      if (FALSE == StackWalk(dwMachine, hProcess, hProcess, &stFrame, &stCtx, nullptr, SymFunctionTableAccess, GetModBase, nullptr)) break;
      if (i > dwNumSkip)
      {
    ...
    for (auto loop = vAddrs.begin(); loop != vAddrs.end(); ++loop)
    {
