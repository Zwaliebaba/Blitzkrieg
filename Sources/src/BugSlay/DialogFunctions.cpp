...
inline char *CC(const std::string &str) { return const_cast<char *>(str.c_str()); }

void FillStackList(HWND hwndCallStack, const CCallStackEntryList &aStack)
{
  SendMessage(hwndCallStack, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, LVS_EX_FULLROWSELECT);
  RECT rc;
  ...
  for (auto pos = aStack.begin(); pos != aStack.end(); ++pos)
  {
    char buf[100];
...
void AddCallStackItem(HWND hWnd, const SCallStackEntry *pEntry) { AddListBoxItem(hWnd, pEntry->szFunctionName.c_str(), pEntry); }

const void *GetListBoxItem(HWND hWnd, int nItemIndex)
{
  long nVal = SendMessage(hWnd, LB_GETITEMDATA, nItemIndex, 0);
  if (nVal == LB_ERR) return nullptr;
...
const void *GetListBoxCurrentItem(HWND hWnd)
{
  ...
  if (nItem == LB_ERR) return nullptr;
  return GetListBoxItem(hWnd, nItem);
}

const SCallStackEntry *GetCallStackItem(HWND hWnd, int nItemIndex) { return reinterpret_cast<const SCallStackEntry *>(GetListBoxItem(hWnd, nItemIndex)); }
const SCallStackEntry *GetCurrentCallStackItem(HWND hWnd) { return reinterpret_cast<const SCallStackEntry *>(GetListBoxCurrentItem(hWnd)); }

bool SetWindowText(HWND hwndDlg, const int nElementID, const char *pszString) { return SetWindowText(GetDlgItem(hwndDlg, nElementID), pszString) != FALSE; }
...
void *GetWindowUserData(HWND hwndDlg, const int nElementID) { return reinterpret_cast<void *>(GetWindowLong(GetDlgItem(hwndDlg, nElementID), GWL_USERDATA)); }
void *SetDlgUserData(HWND hwndDlg, void *pUserData) { return reinterpret_cast<void *>(SetWindowLong(hwndDlg, DWL_USER, reinterpret_cast<LONG>(pUserData))); }
void *GetDlgUserData(HWND hwndDlg) { return reinterpret_cast<void *>(GetWindowLong(hwndDlg, DWL_USER)); }

int GetCheckButtonState(HWND hwndDlg, const int nElementID) { return SendMessage(GetDlgItem(hwndDlg, nElementID), BM_GETCHECK, 0, 0); }
                       ...
                       const CCallStackEntryList &entries)
{
  ...
  //
  if (_access(szFileName.c_str(), 0) == 0) return;
  //
  if (FILE *file = fopen(szFileName.c_str(), "at"))
  {
    fprintf(file, "%s\n", pszCondition);
    fprintf(file, "%s\n\n", pszDescription);
    for (auto it = entries.begin(); it != entries.end(); ++it) fprintf(file, "%s(%d): %s\n", it->szFileName.c_str(), it->nLineNumber, it->szFunctionName.c_str());
    fprintf(file, "\n\n");
    //
    fflush(file);
    fclose(file);
