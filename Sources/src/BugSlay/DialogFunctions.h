...
bool SetWindowText(HWND hwndDlg, int nElementID, const char *pszString);
bool SetWindowText(HWND hwndDlg, int nElementID, int nValue);

void *SetWindowUserData(HWND hwndDlg, int nElementID, void *pUserData);
void *GetWindowUserData(HWND hwndDlg, int nElementID);
void *SetDlgUserData(HWND hwndDlg, void *pUserData);
void *GetDlgUserData(HWND hwndDlg);

int GetCheckButtonState(HWND hwndDlg, int nElementID);

inline std::string GetFileName(std::string strFullPath) { return strFullPath.substr(strFullPath.rfind("\\") + 1); }
