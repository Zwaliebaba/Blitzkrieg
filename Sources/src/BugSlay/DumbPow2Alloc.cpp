...
static void InitDumbAlloc()
{
  ...
  for (;;)
  {
    ...
    if (pTest) break;
    pBase = static_cast<char *>(pBase) + 0x8000000;
  }
  for (int k = 0; k < N_WAYS; ++k) pAllocated[k] = static_cast<char *>(pBase) + k * (N_SIZE / N_WAYS);
}
...
static __forceinline void *RealFastDumbAlloc(int _nSize)
{
  ...
  if (!pRes)
  {
    auto pNew = (char **) &pAllocated[nSize - 2];
    char *pNewPlace = *pNew;
    if (!pNewPlace) return malloc(_nSize);
...
{
  if (pData < pBase || pData > (static_cast<char *>(pBase) + N_SIZE)) return false;
  RealEnterCritical();
  int nSize = (static_cast<char *>(pData) - static_cast<char *>(pBase)) / (N_SIZE / N_WAYS);
  *static_cast<void **>(pData) = freePtrs[nSize];
