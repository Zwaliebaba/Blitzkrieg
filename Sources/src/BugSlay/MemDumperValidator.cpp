...
// characters long.
using PFNCOMPARE = int(*)(const void *, const void *);
...
// initializers.  
static LPDVINFO g_pstDVInfo = nullptr;
// The number of items in the g_pstDVInfo array.
static DWORD g_dwDVCount = 0;
// The highest client subblock value used.  
...
// The pointer to the previous dump client function.  
// does not have a dumper for the client block
static _CRT_DUMP_CLIENT g_pfnPrevDumpClient = nullptr;
...
// Finds the registered dumper-validator block for a particular piece of memory.
...
int STDCALL AddClientDV(LPDVINFO lpDVInfo)
{
  ...
  __try
  {
    __try
    {
      ...
      ASSERT(FALSE == IsBadCodePtr((FARPROC)lpDVInfo->pfnDump));
      ASSERT(FALSE == IsBadCodePtr((FARPROC)lpDVInfo->pfnValidate));

      if ((nullptr == lpDVInfo) || (TRUE == IsBadCodePtr((FARPROC) lpDVInfo->pfnDump)) ||
          (TRUE == IsBadCodePtr((FARPROC) lpDVInfo->pfnValidate)))
      {
      ...
      if (FALSE == g_bLibInit) InitializeLibrary();
      ...
      if (0 == g_dwDVCount)
      {
        ...
        {
          g_dwMaxSubtype = CLIENT_BLOCK_SUBTYPE(lpDVInfo->dwValue);
        ...
        g_pstDVInfo = static_cast<LPDVINFO>(HeapAlloc(g_hHeap, HEAP_GENERATE_EXCEPTIONS | HEAP_ZERO_MEMORY, sizeof(DVINFO)));
      ...
      else if (0 != CLIENT_BLOCK_SUBTYPE(lpDVInfo->dwValue))
      {
        ...
        auto lpDV = static_cast<LPDVINFO>(bsearch(lpDVInfo, g_pstDVInfo, g_dwDVCount, sizeof(DVINFO), (PFNCOMPARE) CompareDVInfos));
        ASSERT(NULL == lpDV);

        if (nullptr != lpDV)
        {
          _RPT1(_CRT_WARN, "%08X is already in the MemDumperValidate list!\n", lpDVInfo->dwValue);
          bRet = FALSE;
          __leave;
        }
        ...
        // seen so far.  
        // value off.
        if (g_dwMaxSubtype < CLIENT_BLOCK_SUBTYPE(lpDVInfo->dwValue))
        {
          g_dwMaxSubtype = CLIENT_BLOCK_SUBTYPE(lpDVInfo->dwValue);
          if (FALSE == CheckMaxSubType()) __leave;
        }
      }
      // Bump up the count.
      g_dwDVCount++;
      ...
      if (0 == CLIENT_BLOCK_SUBTYPE(lpDVInfo->dwValue))
      {
        ...
        bTackOnEnd = TRUE;
        lpDVInfo->dwValue = CLIENT_BLOCK_VALUE(g_dwMaxSubtype);
      }

      // Reallocate the array.
      g_pstDVInfo = static_cast<LPDVINFO>(HeapReAlloc(g_hHeap, HEAP_GENERATE_EXCEPTIONS | HEAP_ZERO_MEMORY,
                                                      g_pstDVInfo, g_dwDVCount * sizeof(DVINFO)));

      if (TRUE == bTackOnEnd) g_pstDVInfo[g_dwDVCount - 1] = *lpDVInfo;
      else
      {
        // Bummer, pound through the array and look for the slot
        // to insert it.
        DWORD iCurr = 0;
        DWORD dwToFind = CLIENT_BLOCK_SUBTYPE(lpDVInfo->dwValue);
        while (dwToFind > CLIENT_BLOCK_SUBTYPE(g_pstDVInfo[iCurr].dwValue))
        {
          iCurr++;
        ...
        DWORD dwDest = (DWORD) g_pstDVInfo + ((iCurr + 1) * sizeof(DVINFO));
        DWORD dwSrc = (DWORD) g_pstDVInfo + (iCurr * sizeof(DVINFO));
        DWORD dwCount = ((g_dwDVCount - 1) - iCurr) * sizeof(DVINFO);
  ...
  __finally { LeaveCriticalSection(&g_stCritSec); }
  return bRet;
}
...
void STDCALL ValidateAllBlocks(void *pContext)
{
  __try
  {
    ...
    // blocks.
    _CrtDoForAllClientObjects(DoForAllFunction, pContext);
  }
  __finally { LeaveCriticalSection(&g_stCritSec); }
}
...
static void DumpFunction(void *pData, size_t nSize)
{
  __try
  {
    __try
    {
      ...
      if ((nullptr != lpDV) && (nullptr != lpDV->pfnDump)) lpDV->pfnDump(pData);
      ...
      else if (nullptr != g_pfnPrevDumpClient) (*g_pfnPrevDumpClient)(pData, nSize);
      else
      {
...
#define MAXPRINT 16
        ...
        for (i = 0; i < Min(static_cast<int>(pHead->nDataSize), MAXPRINT); ++i)
        {
          ch = pbData(pHead)[i];
...
static void DoForAllFunction(void *pData, void *pContext)
{
  __try
  {
    __try
    {
      ...
      // Only call the validator if there is one.  
      // one, then there is nothing to do.
      if ((nullptr != lpDV) && (nullptr != lpDV->pfnValidate))
      {
    ...
    {
      ASSERT(FALSE);
      _RPT0(_CRT_WARN, "There was a crash in DoForAllFunction!\n");
    }
  }
  __finally { LeaveCriticalSection(&g_stCritSec); }
}
...
static BOOL InitializeLibrary(void)
{
  ...
  g_pfnPrevDumpClient = _CrtSetDumpClient(DumpFunction);
...
static BOOL ShutdownLibrary(void)
{
  ...
  if (nullptr != g_pfnPrevDumpClient) _CrtSetDumpClient(g_pfnPrevDumpClient);
...
static int CompareDVInfos(LPDVINFO pDV1, LPDVINFO pDV2)
{
  ...
  if (pDV1->dwValue < pDV2->dwValue) return -1;
  if (pDV1->dwValue > pDV2->dwValue) return 1;
  return 0;
}
...
static BOOL CheckMaxSubType()
{
  ASSERT(static_cast<WORD>(g_dwMaxSubtype) < static_cast<WORD>(0xFFFF));

  if (g_dwMaxSubtype >= 0xFFFF)
  {
    _RPT0(_CRT_WARN, "Running max subtype value in MemDumperValidator is to high!");
    return FALSE;
  }
  g_dwMaxSubtype++;
  return TRUE;
...
static LPDVINFO FindRegisteredBlockType(void *pData)
{
  ...
  DVINFO stDVFind;
  stDVFind.dwValue = static_cast<DWORD>(pHead->nBlockUse);

  // Try and find the value.
  auto lpDV = static_cast<LPDVINFO>(bsearch(&stDVFind, g_pstDVInfo, g_dwDVCount, sizeof(DVINFO), (PFNCOMPARE) CompareDVInfos));
  return lpDV;
}
