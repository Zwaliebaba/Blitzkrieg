...
static _CRT_DUMP_CLIENT g_pfnPrevDumpClient = 0;
...
int STDCALL AddClientDV( LPDVINFO lpDVInfo )
{
  ...
  __try
  {
    __try
    {
      ...
      if ( ( 0 == lpDVInfo ) || ( TRUE == IsBadCodePtr((FARPROC)lpDVInfo->pfnDump) )||
      ...
      if ( 0 == g_dwDVCount )
      {
        ...
        g_pstDVInfo = (LPDVINFO)HeapAlloc( g_hHeap, HEAP_GENERATE_EXCEPTIONS | HEAP_ZERO_MEMORY, sizeof(DVINFO) );
      ...
      else if ( 0 != CLIENT_BLOCK_SUBTYPE(lpDVInfo->dwValue) )
      {
        ...
        LPDVINFO lpDV = (LPDVINFO)bsearch( lpDVInfo, g_pstDVInfo, g_dwDVCount, sizeof(DVINFO), (PFNCOMPARE)CompareDVInfos );
        ASSERT( NULL == lpDV );
      ...
      g_pstDVInfo = (LPDVINFO)HeapReAlloc( g_hHeap, HEAP_GENERATE_EXCEPTIONS | HEAP_ZERO_MEMORY, 
...
static BOOL InitializeLibrary ( void )
{
  ...
  g_pfnPrevDumpClient = _CrtSetDumpClient( (_CRT_DUMP_CLIENT)DumpFunction );
...
static LPDVINFO FindRegisteredBlockType( void * pData )
{
  ...
  LPDVINFO lpDV = (LPDVINFO)bsearch( &stDVFind, g_pstDVInfo, g_dwDVCount, sizeof(DVINFO), (PFNCOMPARE)CompareDVInfos );
  return lpDV;
}
