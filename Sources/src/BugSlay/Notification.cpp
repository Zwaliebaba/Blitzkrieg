...
public:
  ...
  const char *GetString() const override { return szErrorString.c_str(); }
  DWORD GetCode() const override { return dwErrorCode; }
};
...
public:
  ...
  const char *GetString() const override
  {
    if (bStringChanged)
    {
      char buff[2048];
      buff[0] = 0;
      for (auto pos = szErrors.begin(); pos != szErrors.end(); ++pos)
      {
  ...
  void Append(const char *pszFormat, ...) override
  {
    char buffer[512];
    va_list va;
...
void AddNotifyErrorShowMBoxFunction(NotificationShowMBox funct) { notifyError.push_back(funct); }
void AddNotifyWarningShowMBoxFunction(NotificationShowMBox funct) { notifyWarning.push_back(funct); }
void AddNotifyReportShowMBoxFunction(NotificationShowMBox funct) { notifyReport.push_back(funct); }
void RemoveNotifyErrorShowMBoxFunction(NotificationShowMBox funct) { notifyError.remove(funct); }
void RemoveNotifyWarningShowMBoxFunction(NotificationShowMBox funct) { notifyWarning.remove(funct); }
void RemoveNotifyReportShowMBoxFunction(NotificationShowMBox funct) { notifyReport.remove(funct); }
// //////////////////////////////////////////////////////////// 
void BreakHere()
{
#ifdef _DEBUG
...
int ShowError(const char *pszString)
{
  ...
  for (auto pos = notifyError.begin(); pos != notifyError.end(); ++pos) nRetVal = Max(nRetVal, (*(*pos))("ERROR!", pszString, MB_ABORTRETRYIGNORE | MB_ICONERROR));
  return nRetVal;
}
...
int ShowWarning(const char *pszString)
{
  ...
  OutputDebugString("\n");
  int nRetVal = -1000000000;
  for (auto pos = notifyWarning.begin(); pos != notifyWarning.end(); ++pos) nRetVal = Max(nRetVal, (*(*pos))("Warning!", pszString, MB_ABORTRETRYIGNORE | MB_ICONEXCLAMATION));
  return nRetVal;
}
...
int ShowReport(const char *pszString)
{
  ...
  OutputDebugString("\n");
  int nRetVal = -1000000000;
  for (auto pos = notifyReport.begin(); pos != notifyReport.end(); ++pos) nRetVal = Max(nRetVal, (*(*pos))("Report", pszString, MB_OK));
  return nRetVal;
}
...
void ThrowExceptionHR(HRESULT dxrval, const char *pszFormat, ...) throw ( ICommonException *)
{
  ...
  //
  auto pException = new CCommonException(buffer, dxrval);
  int nRetCode = ShowError(buffer);
  if (nRetCode == IDRETRY) BreakHere();
  else if (nRetCode == IDABORT) exit(0xDEAD);
...
void ThrowException(const char *pszFormat, ...) throw ( ICommonException *)
{
  ...
  va_end(va);
  //
  auto pException = new CCommonException(buffer);
  int nRetCode = ShowError(buffer);
  if (nRetCode == IDRETRY) BreakHere();
  else if (nRetCode == IDABORT) exit(0xDEAD);
...
void ThrowGuardException(const char *pszFormat, ...) throw ( IGuardException *)
{
  ...
  va_end(va);
  //
  auto pException = new CGuardException(buffer);
  throw pException;
}
