...
enum EActionNotify
{
  // ( EActionNotify & 1 ) == 1 => go to UpdateActions
  // ( EActionNotify & 1 ) == 0 => scattered across various functions
  ...
  ACTION_NOTIFY_DIE = 0x0e1,// died, there will be no update for disappearance

  // Specific soldier actions
  ACTION_NOTIFY_CRAWL = 0x031,
  ACTION_NOTIFY_IDLE_LYING = 0x041,
  ACTION_NOTIFY_AIM_LYING = 0x051,
  ACTION_NOTIFY_SHOOT_LYING = 0x050,// only soldiers can do this
  ACTION_NOTIFY_THROW = 0x060,
  ACTION_NOTIFY_DIE_LYING = 0x061,// died, there will be no update for disappearance

  ACTION_NOTIFY_IDLE_TRENCH = 0x071,
  ACTION_NOTIFY_AIM_TRENCH = 0x081,
  ACTION_NOTIFY_SHOOT_TRENCH = 0x070,// only soldiers can do this
  ACTION_NOTIFY_THROW_TRENCH = 0x080,// only soldiers can do this
  ACTION_NOTIFY_DIE_TRENCH = 0x090,// died, there will be no update for disappearance

  ACTION_NOTIFY_SHOOT_BUILDING = 0x0a0,// only soldiers can do this
  ACTION_NOTIFY_THROW_BUILDING = 0x160,// only soldiers can do this
  ACTION_NOTIFY_DIE_BUILDING = 0x0b0,// died, there will be no update for disappearance

  ACTION_NOTIFY_IDLE_TRANSPORT = 0x0c1,
  ACTION_NOTIFY_DIE_TRANSPORT = 0x0d1,// died, there will be no update for disappearance
  ...
  ACTION_NOTIFY_ENTRANCE_STATE = 0x190,// enter somewhere
  // all kinds of support actions such as setting/removing mines, repairing/recharging equipment, etc.
  ...
  ACTION_NOTIFY_SIDE_CHANGED = 0x431,// player belogness changed. 
  ...
  // needed only for deferred updates
...
enum EMovingType
{
  MOVE_TYPE_MOVE = 0,// in fact it's either Move or Turn
...
struct SAINotifyAction : SSuspendedUpdate
{
...
struct SAINotifyDeadAtAll : SSuspendedUpdate
{
  bool bRot;// true - if later an update comes to disappear
...
struct SAINotifyRPGStats : SSuspendedUpdate
{
  float fHitPoints;// hit points
  float fMorale;
  int nMainAmmo, nSecondaryAmmo;// cartridges for the main cannon and everything else
  NTimer::STime time;
...
struct SAINotifyDiplomacy : SSuspendedUpdate
{
...
// update on entry/exit, units first enter non-shooting cell
struct SAINotifyEntranceState
{
  IRefCount *pInfantry;// who comes in
  IRefCount *pTarget;// where does it come in
  bool bEnter;// true - enters, false - exits

  SAINotifyEntranceState() : pInfantry(nullptr), pTarget(nullptr) {}
...
// placement update
struct SAINotifyPlacement : SSuspendedUpdate
{
  CVec2 center;// (x, y)
  ...
  float fSpeed;
  BYTE cSoil;// soil parameters: smoke from wheels, tracks, etc.
...
struct SNewUnitInfo : SAINotifyPlacement
{
  // placement of a unit
...
struct SAINotifyHitInfo
{
  ...
  EHitType eHitType;// hit type

  IRefCount *pVictim;// if unit was hit
  CVec3 explCoord;

  SAINotifyHitInfo() : pWeapon(nullptr) {}
  ...
  SAINotifyHitInfo(const SWeaponRPGStats *_pWeapon, const WORD _wShell, const WORD &_wDir, const CVec3 _explCoord)
    : pWeapon(_pWeapon), wShell(_wShell), wDir(_wDir), pVictim(nullptr), explCoord(_explCoord) {}
};

// aiming: turret turning update
struct SAINotifyTurretTurn
{
  IRefCount *pObj;// object turret belongs to
  DWORD nModelPart;// turned bodypart
  WORD wAngle;// final angle
  NTimer::STime endTime;// final time

  SAINotifyTurretTurn() : pObj(nullptr) {}
...
struct SAINotifyMechShot : SAINotifyBaseShot
{
...
struct SAINotifyInfantryShot : SAINotifyBaseShot
{
  const SWeaponRPGStats *pWeapon;// weapon
  // if nSlot >= 0, then shooting from an object, nSlot == -1, then shooting into the open
  short int nSlot;// slot number
  //
  SAINotifyInfantryShot() : pWeapon(nullptr), nSlot(-1) {}
...
struct SAINotifyNewProjectile
{
  ...
  SAINotifyNewProjectile() : pObj(nullptr) {}
...
// general rule:
// commands that are sent from the user are numbered from 0 to 999
// commands that are sent within the AI ​​are numbered from 1000 to 32767 and start 4 tabs to the right (to make it easy to distinguish)

// When adding a new custom command, define its pointtogo in GetGoPointByCommand for
// correct issuance of orders to formations in trenches and buildings

// if the most significant bit 0x8000 is set in the command, then this is a self-action

// values ​​must be no more than 65535!
enum EActionCommand
{
  ...
  ACTION_COMMAND_PARADE = 10,// line up in formation
  //
  ...
  ACTION_COMMAND_WAIT_TO_FORM = 1033,// (AI only) - wait until other units are in the right state to organize a formation
  ...
  ACTION_COMMAND_FOLLOW_NOW = 1037,// (AI only) follow the leader
  ...
  ACTION_COMMAND_CATCH_FORMATION = 1038,// (AI only) join formation
  ACTION_MOVE_PLANE_SCOUT_POINT = 1039,// (AI only) recon plane start scouting

  ACTION_COMMAND_RESUPPLY_HR = 43,

  ACTION_COMMAND_SWARM_ATTACK_UNIT = 1040,// (AI only) attack with independent target re-selection (for example, during swarm)
  ACTION_MOVE_SWARM_ATTACK_FORMATION = 1041,// (AI only) attack with independent target re-selection (for example, during swarm)
  // ACTION_COMMAND_FIRE_MORALE_SHELL = 44, //shot with a morale shell
  ...
  ACTION_COMMAND_ROTATE_TO_DIR = 1055,// turn in a direction, vPos specifies the direction vector
...
struct SAIUnitCmd
{
  ...
  // command ACTION_COMMAND_CALL_BOMBERS - number of bombers
  // command ACTION_COMMAND_ENTER: 0 - enter the building, 1 - enter the trench
  // command ACTION_COMMAND_ATTACK_OBJECT: 0 - attack not a trench, 1 - attack a trench
  float fNumber;
  bool bFromAI;// if true, then the command came from the client or from the general
  //
  SAIUnitCmd() : vPos(-1, -1), bFromAI(true) {}

  explicit SAIUnitCmd(const EActionCommand &_cmdType)
    : cmdType(_cmdType), vPos(-1, -1), fromExplosion(false), fNumber(0), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, IRefCount *_pObject)
    : cmdType(_cmdType), vPos(-1, -1), pObject(_pObject), fromExplosion(false), fNumber(0), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, IRefCount *_pObject, const float _fNumber)
    : cmdType(_cmdType), vPos(-1, -1), pObject(_pObject), fromExplosion(false), fNumber(_fNumber), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, const CVec2 &_vPos)
    : cmdType(_cmdType), vPos(_vPos), fromExplosion(false), fNumber(0), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, const float x, const float y)
    : cmdType(_cmdType), vPos(x, y), fromExplosion(false), fNumber(0), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, const CVec2 &_vPos, const float _fNumber)
    : cmdType(_cmdType), vPos(_vPos), fromExplosion(false), fNumber(_fNumber), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, const float x, const float y, const float _fNumber)
    : cmdType(_cmdType), vPos(x, y), fromExplosion(false), fNumber(_fNumber), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, const float _fNumber)
    : cmdType(_cmdType), vPos(-1, -1), fromExplosion(false), fNumber(_fNumber), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, bool _fromExpl)
    : cmdType(_cmdType), vPos(-1, -1), fromExplosion(_fromExpl), fNumber(0), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, const float _fNumber, bool _fromExpl)
    : cmdType(_cmdType), vPos(-1, -1), fromExplosion(_fromExpl), fNumber(_fNumber), bFromAI(true) {}

  SAIUnitCmd(const EActionCommand &_cmdType, const CVec2 &_vPos, const float _fNumber, const bool _fromExplosion)
    : cmdType(_cmdType), vPos(_vPos), fromExplosion(_fromExplosion), fNumber(_fNumber), bFromAI(true) {}
...
// user input actions
// to achieve RESET_USER_ACTION_xx one must bitwise OR correspending user action with 0x00000100
enum EUserAction
{
  ...
  USER_ACTION_ENGINEER_BUILD_FENCE = 23,// build anti-personnel defense (wire fence)
  ...
  USER_ACTION_ENGINEER_BUILD_ANTITANK = 29,// build anti-tank defense
  ...
  USER_ACTION_OFFICER_CALL_GUNPLANES = 35,// call gunplane (stormtrooper) to hunt in the area
  //
  ...
  // formation changes
  // CRAP { because of someone’s crooked hands (let’s not point fingers at Slavik) the numbering of formations is so crooked
  ...
  USER_ACTION_CHANGE_MOVEMENT_ORDER = 60,// go to a point or parallel
  //
  ...
  USER_ACTION_ENABLE_PLANE_PARADROPER = 213,// enable paradropper airplanes
  USER_ACTION_DISABLE_PLANE_PARADROPER = 214,// disable paradropper airplanes
...
inline bool IsDeathAnimation(const int nAnimation) { return (nAnimation == ANIMATION_DEATH || nAnimation == ANIMATION_DEATH_DOWN); }
...
inline bool IsMovingAction(const EActionNotify updateType) { return updateType == ACTION_NOTIFY_MOVE || updateType == ACTION_NOTIFY_CRAWL || updateType == ACTION_COMMAND_ROTATE_TO; }
...
enum EFeedBack
{
  ...
  EFB_AVIA_KILLED,// nParam == SUCAviation::AIRCRAFT_TYPE | 
...
struct SAIFeedBack
{
  ...
  const SAIFeedBack &operator=(const SAIFeedBack &feedBack)
  {
    feedBackType = feedBack.feedBackType;
    nParam = feedBack.nParam;
    return *this;
  }
};
...
// to send to the client a change in the unit's Bored state
struct SAIBoredAcknowledgement
{
  EUnitAckType eAck;
  IRefCount *pObj;// who sounds
  bool bPresent;// new condition
