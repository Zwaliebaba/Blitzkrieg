  ...
  : szInterfaceType(_szInterfaceType), bInterfaceClosed(false)
{
  ...
  nCPUFreq = static_cast<int>(NHPTimer::GetClockRate() / 1000000.0 + 0.5);
...
bool CInterfaceScreenBase::Init()
{
  ...
  // initialize stats system with some unchangeable values
  {
    ...
    pStat->AddEntry("fps");
    //
    pStat->UpdateEntry("CPU freq.", NStr::Format("%dMHz", static_cast<int>(NHPTimer::GetClockRate() / 1000000.0 + 0.5)));
  }
  //
  ChangeResolution();
  //
...
{
  if ((pScene != nullptr) && (pUIScreen != nullptr))
  {
...
{
  if (bSuspend) GetSingleton<IAILogic>()->Suspend();
  else GetSingleton<IAILogic>()->Resume();
}
...
bool CInterfaceScreenBase::OnCursorMove(const CVec2 &vPos)
{
  ...
  {
    vLastCursorPos = vPos;
    if (pUIScreen) bLastCursorScreenMoveRes = pUIScreen->OnMouseMove(vPos, E_MOUSE_FREE);
  }
  // check for tooltip
...
int CInterfaceScreenBase::PlayOverInterface(const char *pszName, const DWORD dwAddFlags, const bool bFadeIn)
{
  ...
  if (nLength > 0) pScene->AddSceneObject(pTransition);
  return nLength;
}
...
{
  RemoveTransition();
  if (szInterfaceType == "Current") return false;
  if (GetGlobalVar("notransition", 0) == 0) { OpenCurtainsForced(); }
  return true;
}

void CInterfaceScreenBase::StartInterface() { RemoveTransition(); }
...
void CInterfaceScreenBase::AddStatistics()
{
  ...
  if (fTotalTime > 1.0)
  {
    fAveFPS = static_cast<float>(nFrameCounter) / fTotalTime;
    fAveTPS = static_cast<float>(nTriCounter) / fTotalTime;
    fTotalTime = 0;
    nFrameCounter = 0;
    nTriCounter = 0;
  }
...
bool CInterfaceScreenBase::ProcessTextMessage(const STextMessage &msg)
{
  if (pUIScreen)
  {
    ...
    // Screen could generate a message, for example about terminating TEXT_MODE, it must be processed immediately
    SGameMessage uiMessage;
    while (pUIScreen->GetMessage(&uiMessage)) ProcessMessage(uiMessage);
    //
    return true;
  }
  return false;
...
void CInterfaceScreenBase::AddMessage(const SGameMessage &msg)
{
  ...
  switch (msg.nEventID)
  {
    ...
    case TUTORIAL_BUTTON_ID: { ShowTutorial(); }
    break;
    default:
...
{
  if ((msg.nParam & 0x40000000) == 0) return pCursor->GetPos();
...
bool CInterfaceScreenBase::ProcessUIMessage(const SGameMessage &msg)
{
  if (pUIScreen)
  {
    switch (msg.nEventID)
    {
      ...
      // a special case, handling mouse clicks, should be handled differently by the UI
      case CMD_MOUSE0_DBLCLK:
        if (pUIScreen->OnLButtonDblClk(GetPosFromMsg(pCursor, msg)) == false) ProcessAndAdd(msg);
        break;
      case CMD_BEGIN_ACTION1:
        if (pUIScreen->OnLButtonDown(GetPosFromMsg(pCursor, msg), E_LBUTTONDOWN) == false) ProcessAndAdd(msg);
        break;
      case CMD_END_ACTION1:
        if (pUIScreen->OnLButtonUp(GetPosFromMsg(pCursor, msg), E_MOUSE_FREE) == false) ProcessAndAdd(msg);
        break;
      // right mouse
      case CMD_BEGIN_ACTION2:
        if (pUIScreen->OnRButtonDown(GetPosFromMsg(pCursor, msg), E_RBUTTONDOWN) == false) ProcessAndAdd(msg);
        break;
      case CMD_END_ACTION2:
        if (pUIScreen->OnRButtonUp(GetPosFromMsg(pCursor, msg), E_MOUSE_FREE) == false) ProcessAndAdd(msg);
        break;
      // special case - you need to process messages that were generated inside the UI screen
      case -1:
        break;
      // all other cases
      default:
    ...
    SGameMessage uiMessage;
    while (pUIScreen->GetMessage(&uiMessage)) ProcessAndAdd(uiMessage);
...
bool CInterfaceScreenBase::ChangeResolution()
{
  ...
  {
    const auto eFullScreen = static_cast<EGFXFullscreen>(GetGlobalVar(("GFX.Mode." + szInterfaceType + ".FullScreen").c_str(), 0));
    const int nDesiredFreq = GetGlobalVar(("GFX.Mode." + szInterfaceType + ".Frequency").c_str(), 0);
    //
    if (pGFX->SetMode(nDesiredSizeX, nDesiredSizeY, nDesiredBPP, nDesiredStencil, eFullScreen, nDesiredFreq) == false) { NI_ASSERT_T(false, NStr::Format("Can't set mode (%d:%d:%d:%d) for interface screen type \"%s\"", nDesiredSizeX, nDesiredSizeY, nDesiredBPP, nDesiredStencil, szInterfaceType.c_str())); }
    ...
    SetGlobalVar("GFX.Mode.Current.FullScreen", static_cast<int>(eFullScreen));
...
void CInterfaceScreenBase::OnGetFocus(bool bFocus)
{
  if (bFocus)
  {
    ...
    if ((pInput != nullptr) && !szBindSection.empty()) pInput->SetBindSection(szBindSection.c_str());
    //
    if (ChangeResolution()) GetSingleton<IScene>()->Reposition();
    // open "curtains", if it were closed
    if (GetGlobalVar("CurtainsClosed", 0) != 0) { if (OpenCurtains()) RemoveGlobalVar("CurtainsClosed"); }
    else ShowTutorialIfNotShown();
  }
}
...
void CInterfaceScreenBase::StoreScreen()
{
  if (pScene)
  {
    ...
    if (pStoredScreen != nullptr) pScene->RemoveUIScreen(pStoredScreen);
  }
}
...
{
  if (pUIScreen == nullptr) return true;
  ...
  if (pTutorial != nullptr)
  {
    ...
    if (pText && pText[0] != 0)
    {
      ...
      GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_IM_TUTORIAL, nullptr);
      return true;
    }
  }
  return false;
...
void CInterfaceScreenBase::CloseInterface(const bool bCurtains)
{
  if (!bInterfaceClosed)
  {
    if (bCurtains) FinishInterface(MAIN_COMMAND_POP, nullptr);
    else GetSingleton<IMainLoop>()->Command(MAIN_COMMAND_POP, nullptr);
