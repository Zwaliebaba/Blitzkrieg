                       ...
                       float fNewHP, interface IVisObjBuilder *pVOB, IObjectsDB *pGDB)
{
  ...
  switch (pDesc->eGameType)
  {
    case SGVOGT_OBJECT:
    {
      ...
      pVisObj = pVOB->BuildObject((pDesc->szPath + pszName).c_str(), nullptr, pDesc->eVisType);
      if (pVisObj)// try to create normal object (take season into account)
        pShadow = pVOB->BuildObject((pDesc->szPath + pszName + "s").c_str(), nullptr, SGVOT_SPRITE);
      else// try to create summer (default) object, if correct one failed
      {
        auto pszName = "\\1";
        pVisObj = pVOB->BuildObject((pDesc->szPath + pszName).c_str(), nullptr, pDesc->eVisType);
        if (pVisObj) pShadow = pVOB->BuildObject((pDesc->szPath + pszName + "s").c_str(), nullptr, SGVOT_SPRITE);
      }
    }
    break;
    case SGVOGT_EFFECT:
      pVisObj = pVOB->BuildObject(pDesc->szPath.c_str(), nullptr, pDesc->eVisType);
      break;
    ...
    case SGVOGT_TANK_PIT:
      ...
      break;
    case SGVOGT_MINE:
      pVisObj = pVOB->BuildObject((pDesc->szPath + "\\" + static_cast_gdb<const SMineRPGStats *>(pRPG)->szFlagModel).c_str(), nullptr, pDesc->eVisType);
      break;
    case SGVOGT_BRIDGE:
    {
      const SBridgeRPGStats::SSegmentRPGStats &segment = static_cast_gdb<const SBridgeRPGStats *>(pRPG)->GetSegmentStats(nFrameIndex);
      if (segment.szModel.empty() || segment.nFrameIndex == -1) return false;
      pVisObj = pVOB->BuildObject((pDesc->szPath + "\\" + segment.szModel).c_str(), nullptr, SGVOT_SPRITE);
      if (pVisObj == nullptr)// bridge have no this part... skip it
        return false;
      if (fNewHP < 0)// special case - "blueprint" of the bridge
        pVisObj->SetOpacity(0x80);
      pShadow = pVOB->BuildObject((pDesc->szPath + "\\" + segment.szModel + "s").c_str(), nullptr, SGVOT_SPRITE);
      nFrameIndex = segment.nFrameIndex;
    }
    break;
    case SGVOGT_FENCE:
      pVisObj = pVOB->BuildObject((pDesc->szPath + "\\1").c_str(), nullptr, pDesc->eVisType);
      if (pVisObj) pShadow = pVOB->BuildObject((pDesc->szPath + "\\1s").c_str(), nullptr, SGVOT_SPRITE);
      break;
    default:
      pVisObj = pVOB->BuildObject((pDesc->szPath + "\\1").c_str(), nullptr, pDesc->eVisType);
  }
  ...
  {
    // main sprite
    auto pAnim = static_cast<ISpriteAnimation *>(static_cast_ptr<IObjVisObj *>(pVisObj)->GetAnimation());
...
int CMOObject::operator&(IStructureSaver &ss)
{
  ...
  if (!szFlagSide.empty()) saver.Add(4, &nMarkerID);
  if (saver.IsReading()) FillActions();
  return 0;
}

bool CMOObject::AddAction(int nAction)
{
  if (nAction > 63) return false;
  const int nIndex = nAction >> 5;
  actions[nIndex] |= 1UL << (nAction - nIndex * 32);
  return true;
}

void CMOObject::FillActions() { actions[0] = actions[1] = 0; }
...
void CMOObject::SetPlacement(const CVec3 &vPos, const WORD &wDir)
{
  ...
  if (!szFlagSide.empty())
  {
    ...
    if (pUIScreen) { if (auto pUIMiniMap = checked_cast<IUIMiniMap *>(pUIScreen->GetChildByID(20000))) { pUIMiniMap->AddMarker(szFlagSide + "Flag", CVec2(vPos.x, vPos.y), true, nMarkerID, GetSingleton<IGameTimer>()->GetAbsTime(), 0, false); } }
  }
}
...
{
  if (const CUserActions *pUserActions = pRPG->GetUserActions(eActions == ACTIONS_BY)) *pActions = *pUserActions;
  else if (eActions == ACTIONS_WITH)
  {
    ...
    {
      if (fHP < 0) pActions->SetAction(USER_ACTION_ENGINEER_BUILD_BRIDGE);
      else if (fHP < 1) pActions->SetAction(USER_ACTION_ENGINEER_REPAIR_BUILDING);
    }
  }
}
...
void CMOObject::UpdateModelWithHP(const float fNewHP, IVisObjBuilder *pVOB)
{
  ...
  // CRAP { redo in the general scheme of destruction
  ...
  if (nNewState != nOldState)
  {
    ...
    {
      pVOB->ChangeObject(pVisObj, NStr::Format("%s\\%d", pDesc->szPath.c_str(), nNewState + 1), nullptr, SGVOT_SPRITE);
      if (pShadow) pVOB->ChangeObject(pShadow, NStr::Format("%s\\%ds", pDesc->szPath.c_str(), nNewState + 1), nullptr, SGVOT_SPRITE);
    }
  }
  // CRAP}
}
  ...
  switch (action.typeID)
  {
    case ACTION_NOTIFY_SILENT_DEATH:
      if ((pDesc->eGameType == SGVOGT_OBJECT) || (pDesc->eGameType == SGVOGT_FENCE))
      {
        if (pRPG && pVisObj)
        {
          auto pRPGStats = static_cast_gdb<const SStaticObjectRPGStats *>(pRPG);
    ...
    case ACTION_NOTIFY_SIDE_CHANGED:
           ...
           !pIt->IsEnd(); pIt->Next())
      {
        ...
        if (pPlayer->GetDiplomacySide() == action.nParam)
        {
          ...
          if (auto pUIMiniMap = checked_cast<IUIMiniMap *>(pScene->GetMissionScreen()->GetChildByID(20000)))
          {
            WORD wCircleColor;
          ...
          if (action.nParam == 2) SetDiplomacy(EDI_NEUTRAL);
          else if (action.nParam == pUserPlayer->GetDiplomacySide()) SetDiplomacy(EDI_FRIEND);
          else SetDiplomacy(EDI_ENEMY);
          ...
          if (const SGDBObjectDesc *pNewDesc = GetSingleton<IObjectsDB>()->GetDesc(szFlagName.c_str()))
          {
            ...
            if (pVOB->ChangeObject(pVisObj, szFlagFileName.c_str(), nullptr, SGVOT_SPRITE))
            {
              pDesc = pNewDesc;
              ...
              if (pShadow)
              {
                pVOB->ChangeObject(pShadow, (szFlagFileName + "s").c_str(), nullptr, SGVOT_SPRITE);
                pShadow->Update(currTime, true);
              }
            }
          }
