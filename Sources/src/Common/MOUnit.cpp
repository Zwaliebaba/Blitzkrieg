...
{
  if (dwFlashFireColor == 0xffffffff) dwFlashFireColor = GetGlobalVar((std::string("Scene.Colors.") + GetSeasonName(nSeason) + ".FlashFire.Color").c_str(), static_cast<int>(0xffffffff));
  if (dwFlashExpColor == 0xffffffff) dwFlashExpColor = GetGlobalVar((std::string("Scene.Colors.") + GetSeasonName(nSeason) + ".FlashExplode.Color").c_str(), static_cast<int>(0xffffffff));
  // add icon with level for scenario units
  if (nScenarioIndex >= 0) UpdateLevel(GetSingleton<IScenarioTracker>()->GetUserPlayer()->GetUnit(nScenarioIndex)->GetValue(STUT_LEVEL));
  //
  return true;
}
...
void CMOUnit::GetActionsLocal(EActionsType eActions, CUserActions *pActions) const
{
  ...
  {
    if (pRPG != nullptr) *pActions = static_cast_gdb<const SUnitBaseRPGStats *>(pRPG)->availUserActions;
    else pActions->Clear();
  }
  else if (eActions == ACTIONS_WITH)
  {
    if ((pRPG != nullptr) && IsAlive())
    {
      *pActions = static_cast_gdb<const SUnitBaseRPGStats *>(pRPG)->availUserExposures;
      // damaged unit can be repaired
      if (fHP < 1) pActions->SetAction(USER_ACTION_ENGINEER_REPAIR);
      // units with spend ammos can be resupplied
      if ((ammos[0] < static_cast_gdb<const SUnitBaseRPGStats *>(pRPG)->nAmmos[0]) || (ammos[1] < static_cast_gdb<const SUnitBaseRPGStats *>(pRPG)->nAmmos[1])) pActions->SetAction(USER_ACTION_SUPPORT_RESUPPLY);
    }
...
{
  if (GetContainer() && GetContainer()->GetDesc()->eGameType != SGVOGT_ENTRENCHMENT) return false;
  return true;
}

const bool CMOUnit::CanAddIcon(int nType) const
{
  if (!IsFriend() && ((nType == ICON_LOW_AMMO) || (nType == ICON_NO_AMMO) || (nType == ICON_LOW_MORAL) || (nType == ICON_UNIT_NO_SUPPLY) || (nType == ICON_ALT_SHELL) || (nType == ICON_AMBUSH))) return false;
  return true;
}

void CMOUnit::SetIcon(int nType, IVisObjBuilder *pVOB)
{
  if (!CanAddIcon(nType)) return;
  //
  ...
  if (auto pIcon = static_cast<ISceneIcon *>(pVOB->BuildSceneObject(pszIconName, SCENE_OBJECT_TYPE_ICON)))
  {
    ...
    static_cast_ptr<IObjVisObj *>(pVisObj)->AddIcon(pIcon, nTypeID, GetIconAddValue(), VNULL3, nTypeID,
                                                    ...
                                                    CanShowIcons());
    if (pObserver) pObserver->AddIcon(nType, pIcon);
  }
}
...
void CMOUnit::RemoveIcon(int nType)
{
  ...
  //
  if (pObserver) pObserver->RemoveIcon(nType);
}

void CMOUnit::SetPlacement(const CVec3 &vPos, const WORD &wDir) { pVisObj->SetPlacement(vPos, wDir); }
...
void CMOUnit::GetStatus(struct SMissionStatusObject *pStatus) const
{
  ...
  //
  auto pRPGStats = static_cast_gdb<const SUnitBaseRPGStats *>(pRPG);
  // primary attributes
  ...
  if (IText *pName = GetLocalNameLocal())
  {
    ...
    const std::string szFrozenInfo = NStr::Format(",(%d,%d)", static_cast<int>(bFrozen), static_cast<int>(bFrozenByState));
...
#else
  if (IText *pName = GetLocalNameLocal()) memcpy(pStatus->pszName, pName->GetString(), (pName->GetLength() + 1) * 2);
#endif // #if !defined(_FINALRELEASE) && !defined(_BETARELEASE)
  else
  {
...
bool CMOUnit::CommonUpdateHP(const float fNewHP, bool bUpdateHPBar)
{
  ...
  {
    // change HP bar
    if (auto pBar = static_cast<ISceneIconBar *>(static_cast_ptr<IObjVisObj *>(pVisObj)->GetIcon(ICON_HP_BAR)))
    {
      pBar->SetLength(fNewHP);
      pBar->SetColor(MakeHPBarColor(fNewHP));
    }
    //
    if (pObserver) pObserver->UpdateHP(fNewHP);
  }
  //
  fHP = fNewHP;
  //
...
static constexpr float VALUE_MORALE_LOW = 0.5f;
static constexpr float VALUE_AMMO_LOW = 0.3f;
static constexpr float VALUE_AMMO_NO = 0.0f;
inline bool IsValueDecreased(const float fOld, const float fNew, const float fBound) { return (fOld > fBound) && (fNew <= fBound); }
inline bool IsValueIncreased(const float fOld, const float fNew, const float fBound) { return (fOld <= fBound) && (fNew > fBound); }

void CMOUnit::CommonUpdateRPGStats(const float fNewHP, const SAINotifyRPGStats &stats, IVisObjBuilder *pVOB, bool bUpdateHPBar)
{
  if (CommonUpdateHP(fNewHP, bUpdateHPBar) == false) return;
  // modify morale icon
  if (IsValueDecreased(fMorale, stats.fMorale, VALUE_MORALE_LOW)) SetIcon(ICON_LOW_MORAL, pVOB);
  else if (IsValueIncreased(fMorale, stats.fMorale, VALUE_MORALE_LOW)) RemoveIcon(ICON_LOW_MORAL);
  //
  fMorale = stats.fMorale;
  // modify ammos
  auto pRPGStats = static_cast_gdb<const SUnitBaseRPGStats *>(pRPG);
  float fNewAmmoValue = 1;
  if (pRPGStats->nAmmos[AMMO_TYPE_PRIMARY]) fNewAmmoValue = static_cast<float>(stats.nMainAmmo) / static_cast<float>(pRPGStats->nAmmos[AMMO_TYPE_PRIMARY]);
  if (pRPGStats->nAmmos[AMMO_TYPE_SECONDARY]) fNewAmmoValue = Min(fNewAmmoValue, static_cast<float>(stats.nSecondaryAmmo) / static_cast<float>(pRPGStats->nAmmos[AMMO_TYPE_SECONDARY]));
  ...
  if (fAmmoValue != fNewAmmoValue)
  {
    if (fNewAmmoValue < fAmmoValue)
    {
      ...
      else if (IsValueDecreased(fAmmoValue, fNewAmmoValue, VALUE_AMMO_LOW)) SetIcon(ICON_LOW_AMMO, pVOB);
    }
    else
    {
...
bool CMOUnit::AIUpdateDiplomacy(const SAINotifyDiplomacy &_diplomacy)
{
  ...
  if (auto pBar = static_cast<ISceneIconBar *>(static_cast_ptr<IObjVisObj *>(pVisObj)->GetIcon(ICON_HP_BAR))) pBar->SetBorderColor(GetGlobalVar(NStr::Format("Scene.PlayerColors.Player%d", GetPlayerIndex()), static_cast<int>(0xff000000)));
  if (!IsFriend())
  {
    RemoveIcon(ICON_NO_AMMO);
    RemoveIcon(ICON_LOW_AMMO);
  ...
  else
  {
    // restore state icons
    // ammo
    if (fAmmoValue <= VALUE_AMMO_NO) SetIcon(ICON_NO_AMMO, GetSingleton<IVisObjBuilder>());
    else if (fAmmoValue <= VALUE_AMMO_LOW) SetIcon(ICON_LOW_AMMO, GetSingleton<IVisObjBuilder>());
    // morale
    if (fMorale <= VALUE_MORALE_LOW) SetIcon(ICON_LOW_MORAL, GetSingleton<IVisObjBuilder>());
  }
  return IsFriend();
}
...
void CMOUnit::AssignSelectionGroup(const int nGroupID)
{
  if (nSelectionGroupID != nGroupID)
  {
    ...
    // set new one
    if ((nGroupID >= 0) && (nGroupID <= 9)) SetIcon(ICON_GROUP | (nGroupID << 16), GetSingleton<IVisObjBuilder>());
...
{
  auto pStats = static_cast_gdb<const SUnitBaseRPGStats *>(pRPG);
  const float r = 1.0f * rand() / RAND_MAX;
  std::string szSound;
  // if there is no sound of a negative ask, then try to play Generic
  if (!pStats->ChooseAcknowledgement(r, eAck, &szSound, nSet) &&
      pAckManager->IsNegative(eAck) &&
      !pStats->ChooseAcknowledgement(0.0f, eAck, nullptr, nSet)) { pStats->ChooseAcknowledgement(r, ACK_NEGATIVE, &szSound, nSet); }
...
{
  if (ack.bPresent) pAckManager->RegisterAsBored(ack.eAck, this);
  else pAckManager->UnRegisterAsBored(ack.eAck, this);
}
...
void CMOUnit::SetObserver(IUnitStateObserver *_pObserver)
{
  ...
  if (pObserver)
  {
    auto pVO = static_cast_ptr<IObjVisObj *>(pVisObj);
    for (int i = 1; i < ICON_NUM_ICONS; ++i) { if (ISceneIcon *pIcon = pVO->GetIcon(i)) pObserver->AddIcon(i, pIcon); }
                             ...
                             IVisObjBuilder *pVOB, IScene *pScene, interface IClientAckManager *pAckManager)
{
  switch (action.typeID)
  {
    ...
    case ACTION_NOTIFY_STORAGE_CONNECTED:
      if (action.nParam == 0) SetIcon(ICON_UNIT_NO_SUPPLY, pVOB);
      else RemoveIcon(ICON_UNIT_NO_SUPPLY);
      break;
