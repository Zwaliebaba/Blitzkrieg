...
CMOUnitMechanical::~CMOUnitMechanical()
{
  ...
  //
  for (auto it = passangers.begin(); it != passangers.end(); ++it) it->pUnit->SetContainer(nullptr);
}
                               ...
                               float fNewHP, interface IVisObjBuilder *pVOB, IObjectsDB *pGDB)
{
  ...
  if (pExtPassangers) pExtPassangers->SetVisible(IsVisibleLocal());
  //
  OnCreate();
  //
...
void CMOUnitMechanical::PrepareToRemove() { for (auto it = effects.begin(); it != effects.end(); ++it) { if (it->pEffect != nullptr) it->pEffect->Stop(); } }
...
void CMOUnitMechanical::UpdateModelWithHP(const float fNewHP, IVisObjBuilder *pVOB)
{
  ...
  {
    pVOB->ChangeObject(pVisObj, nullptr, NStr::Format("%s\\%d%s", pDesc->szPath.c_str(), nNewState + 1, GetSeasonApp(GetSeason())), pDesc->eVisType);
    if (nNewState == 1) GetVisObj()->RemoveEffector(-1, -1);
  }
}
...
void CMOUnitMechanical::GetStatus(struct SMissionStatusObject *pStatus) const
{
  ...
  //
  if (GetRPGStats()->IsTransport()) pStatus->params[1] = PackParams(GetAmmo(0), 1000);
  // 
...
void CMOUnitMechanical::GetActions(CUserActions *pActions, EActionsType eActions) const
{
  ...
  if (IsValid())
  {
    ...
    {
      if (GetNumFreeSlots() == GetNumTotalSlots()) actions.RemoveAction(USER_ACTION_LEAVE);
      // check for installed/uninstalled artillery (for artillery)
      if (bInstalled) actions.RemoveAction(USER_ACTION_INSTALL);
      else
      {
      ...
      else actions.RemoveAction(USER_ACTION_DEPLOY_ARTILLERY);
    }
    else if (eActions == ACTIONS_WITH)
    {
      if (!CanSelect()) actions.RemoveAction(USER_ACTION_HOOK_ARTILLERY);
      // check for hooked/unhooked artillery (for artillery itself)
      if (bArtilleryHooked) actions.RemoveAction(USER_ACTION_HOOK_ARTILLERY);
      // check for neutral unit
      if (IsNeutral()) actions.RemoveAction(USER_ACTION_BOARD);
    }
  }
  else { DEBUG_BREAK; }
  *pActions |= actions;
...
bool CMOUnitMechanical::Load(IMOUnit *pMO, bool bEnter)
{
  if (bEnter)
  {
    if (GetGlobalVar("MultiplayerGame", 0) == 1) { if (auto pMOInf = dynamic_cast<CMOUnitInfantry *>(pMO)) pMOInf->SetHPSimpleBar(); }
  ...
  {
    if (GetGlobalVar("MultiplayerGame", 0) == 1) { if (auto pMOInf = dynamic_cast<CMOUnitInfantry *>(pMO)) pMOInf->SetHPSimpleBar(false); }
...
void CMOUnitMechanical::RemoveExhaustedEffects(const NTimer::STime &time)
{
  ...
  {
    if (it->pEffect->IsFinished(time)) it = effects.erase(it);
    else ++it;
  }
}
...
void CMOUnitMechanical::UpdateAttachedEffects(const NTimer::STime &currTime, IScene *pScene)
{
  ...
  {
    if (it->nPointIndex >= nNumNodes) continue;
    ...
    if (fabs2(vNewPos - vPos) >= 25)
    {
      ...
      const CVec3 vPosStep = (vNewPos - vPos) / (static_cast<float>(diff) / INTERIM_STEP);
      //
      while (diff > INTERIM_STEP)
      {
        ...
        vPos += vPosStep;
        //
        if ((pScene == nullptr) || (pScene->MoveObject(it->pEffect, vPos) == false)) it->pEffect->SetPlacement(vPos, 0);
        it->pEffect->Update(it->timeLastUpdate);
      }
    }
    // final update
    if ((pScene == nullptr) || (pScene->MoveObject(it->pEffect, vNewPos) == false)) it->pEffect->SetPlacement(vNewPos, 0);
...
{
  // change model
  for (auto it = modelchanges.begin(); it != modelchanges.end();)
  {
    if (it->time <= currTime)
    {
  ...
  // change animation
  for (auto it = animchanges.begin(); it != animchanges.end();)
  {
    if (it->time <= currTime)
    {
...
int CMOUnitMechanical::ChangeModel(const std::string &szModelName, const NTimer::STime &currTime, const NTimer::STime &timeChange)
{
  if (timeChange <= currTime)
  {
    ...
    GetSingleton<IVisObjBuilder>()->ChangeObject(pVisObj, (pDesc->szPath + "\\" + szModelName).c_str(), nullptr, pDesc->eVisType);
    GetAnim()->AddEffector(pEffector, ANIM_EFFECTOR_LEVELING, -2);
    return 0;
  }
...
void CMOUnitMechanical::AIUpdatePlacement(const SAINotifyPlacement &placement, const NTimer::STime &currTime, IScene *pScene)
{
  ...
  if (0 != wMoveSoundID) pScene->SetSoundPos(wMoveSoundID, pVisObj->GetPosition());
  if (0 != wNonCycleSoundID) pScene->SetSoundPos(wNonCycleSoundID, pVisObj->GetPosition());
  // update attached effects
  UpdateAttachedEffects(currTime, pScene);
  // update leveling
  IMeshAnimation *pAnim = GetAnim();
  if (auto pEffector = static_cast<IMatrixEffectorLeveling *>(pAnim->GetEffector(ANIM_EFFECTOR_LEVELING, -2)))
  {
    pVisObj->Update(currTime);
  ...
  if (pStats->bLeavesTracks)
  {
    ...
    {
      auto corner = CVec3(pStats->vAABBVisCenter.x, pStats->vAABBVisCenter.y, 0);
      auto shift = CVec3(pStats->vAABBHalfSize.x, 0, 0);
      AI2Vis(&corner);
      AI2Vis(&shift);
      ...
      if (!bSkipTrack)
      {
        if (dirChanged) bLastTracedDir = !bLastTracedDir;
        else vLastPos = vPos;
...
void CMOUnitMechanical::LeaveTrace(SMechTrace *pTrace, const SAINotifyPlacement &placement, const NTimer::STime &currTime, bool secondTrack, const SMechUnitRPGStats *pStats, const CVec3 &vPos, bool isForward, const CVec3 &dir, IScene *pScene)
{
  ...
  DWORD col = static_cast<DWORD>(pTrace->alpha);
  pTrace->dwColor = true || col << 16 || col << 8 || col;
  ...
  pTrace->nNumTracks = static_cast<int>(fabs(pTrace->vCorners[0] - pTrace->vCorners[3])) / 10 + 1;
  if (IsVisible() && !bInEditor)
  {
    if (placement.cSoil & STerrTypeDesc::ESP_TRACE && !bSkipTrack) { pScene->AddMechTrace(*pTrace); }
    if (pStats->szEffectWheelDust.length() != 0 && (placement.cSoil & STerrTypeDesc::ESP_DUST) && !pScene->IsRaining())
    {
      ...
      if (isForward) vDustPos -= dir * (pStats->vAABBVisHalfSize.y * 1.5f * (1 - pStats->fTrackStart - pStats->fTrackEnd));
      else vDustPos += dir * (pStats->vAABBVisHalfSize.y * 1.5f * (1 - pStats->fTrackStart - pStats->fTrackEnd));
      float fSideOffset = pStats->vAABBVisHalfSize.y * pStats->fTrackOffset;
      if (secondTrack) vDustPos.Set(vDustPos.x + dir.y * fSideOffset, vDustPos.y - dir.x * fSideOffset, vDustPos.z);
      else vDustPos.Set(vDustPos.x - dir.y * fSideOffset, vDustPos.y + dir.x * fSideOffset, vDustPos.z);
...
void CMOUnitMechanical::SetPlacement(const CVec3 &vPos, const WORD &wDir)
{
  ...
  if (pStats->bLeavesTracks)
  {
    ...
    auto corner = CVec3(pStats->vAABBVisCenter.x, pStats->vAABBVisCenter.y - pStats->vAABBHalfSize.y * (1 - 2 * pStats->fTrackEnd), 0);
    auto shift = CVec3(pStats->vAABBHalfSize.x, 0, 0);
    AI2Vis(&corner);
    AI2Vis(&shift);
    mPlacement.RotateVector(&corner, corner);
                                                       ...
                                                       const NTimer::STime &currTime, IVisObjBuilder *pVOB)
{
  ...
  if (gun.nShootPoint >= 0)
  {
    ...
    if (gun.nShootPoint < GetAnim()->GetNumNodes())
    {
      ...
      if (fabs(vDiffPos) > 5.0f * fWorldCellSize) vDiffPos = VNULL3;
    }
    else
    {
...
int CMOUnitMechanical::AIUpdateActions(const SAINotifyAction &action, const NTimer::STime &currTime, IVisObjBuilder *pVOB, IScene *pScene, interface IClientAckManager *pAckManager)
{
  ...
  switch (action.typeID)
  {
    ...
    case ACTION_NOTIFY_ENTRENCHMENT_STARTED:
      ...
      else { RemoveIcon(ICON_ENTRENCHED); }
      nRetVal = CMOUnit::AIUpdateActions(action, currTime, pVOB, pScene, pAckManager);
      break;
    default:
      nRetVal = CMOUnit::AIUpdateActions(action, currTime, pVOB, pScene, pAckManager);
...
void CMOUnitMechanical::ActionMove(const SAINotifyAction &action, const NTimer::STime &currTime, IVisObjBuilder *pVOB, IScene *pScene)
{
  ...
  if (IsVisibleLocal() && !pRPG->exhaustPoints.empty())
  {
    ...
    for (int i = 0; i < pRPG->exhaustPoints.size(); ++i)
    {
      ...
      {
        if (auto pEffect = static_cast<IEffectVisObj *>(pVOB->BuildObject(szEffectName.c_str(), nullptr, SGVOT_EFFECT)))
        {
          ...
          pScene->AddObject(pEffect, SGVOGT_EFFECT, nullptr);
...
void CMOUnitMechanical::ActionDie(const SAINotifyAction &action, const NTimer::STime &currTime, IVisObjBuilder *pVOB, IScene *pScene)
{
  ...
  // set death texture
  pVOB->ChangeObject(pVisObj, nullptr, (pDesc->szPath + "\\2" + GetSeasonApp2(GetSeason())).c_str(), pDesc->eVisType);
  ...
  pObj->RemoveIcon(-1);
  // upon death, remove jogging from the object
  if (!pRPG->platforms.empty() && (pRPG->platforms[0].nModelPart >= 0)) GetAnim()->RemoveEffector(ANIM_EFFECTOR_JOGGING, pRPG->platforms[0].nModelPart);
  ...
  else GetAnim()->CutProceduralAnimation(timeEffect);
  // remove motion sound
  if (0 != wMoveSoundID)
  {
  ...
  if ((static_cast<int>((action.nParam >> 16) & 0x0fff) == ANIMATION_DEATH) || ((action.nParam & 0xffff) == 0xffff))
  {
  ...
  else if (static_cast<int>((action.nParam >> 16) & 0x0fff) == ANIMATION_DEATH_FATALITY)
  {
  ...
  // add 'shit-after-death' under the tank
                                       ...
                                       const int nRPGDuration, const char *pszModel2)
{
  ...
  // change mode to animated
  if (pszModel2) ChangeModel(pszModel2, timeAction, timeAction);
  // run animation (currently in animated model mode)
  IMeshAnimation *pAnim = GetAnim();
  int nAddTime = 0;
  if (const SUnitBaseRPGStats::SAnimDesc *pAnimDesc = GetAnimationByType(nAnimation))
  {
    ...
    pAnim->SetAnimSpeedCoeff(static_cast<float>(pAnimDesc->nLength) / static_cast<float>(nRPGDuration));
                                         ...
                                         const int nRPGDuration, IVisObjBuilder *pVOB, const char *pszModel3, bool bInstantly)
{
  ...
  if (!bInstantly)
  {
    // set animated model and run animation
  ...
  return pszModel3 == nullptr ? 0 : ChangeModel(pszModel3, timeAction, timeAction + nAddTime);
}
...
{
  if (!bCanSelect && bSelect) return;
  if (bSelectSuper) pSelector->Select(this, bSelect, false);
  else pVisObj->Select(bSelect ? SGVOSS_SELECTED : SGVOSS_UNSELECTED);
}
                                     ...
                                     IVisObjBuilder *pVOB, IScene *pScene)
{
  ...
  if ((GetAnimationByType(ANIMATION_SHOOT) == nullptr) && gun.bRecoil && (gun.nModelPart != -1))// add procedural recoil
  {
  ...
  // add effect
  if ((gun.nShootPoint != -1) && (gun.pWeapon != nullptr) && !shell.szEffectGunFire.empty())
  {
    pVisObj->Update(currTime);
    IMeshVisObj *pObj = GetVisObj();
    const SHMatrix *matrices = pObj->GetMatrices();
    ...
    if (IsVisibleLocal() || GetRPGStats()->IsAviation())
    {
      ...
      if ((pRPG->pPrimaryGun == &(pRPG->guns[shot.cGun])) && !pRPG->szEffectShootDust.empty())
      {
        if (IEffectVisObj *pEffect = PlayEffect(pRPG->szEffectShootDust, pVisObj->GetPosition(), timeEffect, false, pScene, pVOB, 0, SFX_MIX_IF_TIME_EQUALS, SAM_ADD_N_FORGET, ESCT_COMBAT))
        {
          ...
          else pEffect->SetPosition(pVisObj->GetPosition());
        }
      }
    }
  }
