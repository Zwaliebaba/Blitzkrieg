...
{
  NI_ASSERT_T((eDiplomacy == EDI_NEUTRAL) || (eDiplomacy == EDI_FRIEND) || (eDiplomacy == EDI_ENEMY), NStr::Format("Wrong diplomacy %d", static_cast<int>(eDiplomacy)));
  diplomacy = static_cast<BYTE>(eDiplomacy);
}
...
IText *GetLocalName(const SGDBObjectDesc *pDesc)
{
  ...
  if (pLocalName == nullptr) pLocalName = GetSingleton<ITextManager>()->GetDialog("textes\\strings\\widget-object-name");
  return pLocalName;
}