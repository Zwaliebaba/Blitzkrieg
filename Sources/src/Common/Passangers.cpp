                  ...
                  const CVec3 &vAdd, const CVec3 &vStep, DWORD dwAlignment)
{
  ...
  {
    for (CPassangersList::const_iterator it = passangers.begin(); it != passangers.end(); ++it) nPassangerID = Max(nPassangerID, it->nHPIconID);
    ++nPassangerID;
...
void RemovePassanger(CPassangersList &passangers, IMOUnit *pUnit, IObjVisObj *pVisObj)
{
  ...
  for (auto it = passangers.begin(); it != passangers.end(); ++it)
  {
    if (it->pUnit == pUnit)
    {
      ...
      if (ISceneIcon *pIcon = pVO->GetIcon(ICON_HP_BAR))
      {
        pVO->AddIcon(nullptr, 0, VNULL3, VNULL3, 0, 0);
        if (!pUnit->IsSelected()) pVO->Select(SGVOSS_UNSELECTED);
        // update unit's HP bar visibility
        if (pUnit->IsVisible() && ((pUnit->fHP < 1.0f) || pUnit->IsSelected())) pIcon->Enable(true);
        else pIcon->Enable(false);
      }
      // remove unit from container
      pUnit->SetContainer(nullptr);
      passangers.erase(it);
      return;
    }
  }
...
{
  for (auto it = passangers.begin(); it != passangers.end(); ++it) { if (it->pUnit->IsVisible()) return true; }
  return false;
}

void EnablePassangersIcons(const CPassangersList &passangers, IObjVisObj *pVisObj, bool bEnable) { for (auto it = passangers.begin(); it != passangers.end(); ++it) { if (ISceneIcon *pIcon = pVisObj->GetIcon(it->nHPIconID)) pIcon->Enable(bEnable); } }
