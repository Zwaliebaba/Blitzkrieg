...
void CUIUnitObserver::UpdateHP(const float fValue) { fHP = fValue; }

void CUIUnitObserver::RemoveUnit() { if (pSquad) pSquad->RemovePassanger(this); }
...
bool CUISquadElement::Update(const NTimer::STime &currTime) { return false; }
...
void CUISquadElement::Reposition(const CTRect<float> &rcParent)
{
  switch (nPositionFlag & 0xf)
  {
    ...
    case UIPLACE_RIGHT:
      ...
      break;
    case UIPLACE_HMID:
      rcWindow.x1 = static_cast<int>(rcParent.x1 + vPos.x + rcParent.Width() / 2 - vSize.x / 2);
      rcWindow.x2 = static_cast<int>(rcParent.x1 + vPos.x + rcParent.Width() / 2 + vSize.x / 2);
      break;
  }
  ...
  switch (nPositionFlag & 0xf0)
  {
    ...
    case UIPLACE_BOTTOM:
      ...
      break;
    case UIPLACE_VMID:
      rcWindow.y1 = static_cast<int>(rcParent.y1 + vPos.y + rcParent.Height() / 2 - vSize.y / 2);
      rcWindow.y2 = static_cast<int>(rcParent.y1 + vPos.y + rcParent.Height() / 2 + vSize.y / 2);
      break;
  }
}
...
{
  if (pSquadIcon == nullptr) { pSquadIcon = GetSingleton<ITextureManager>()->GetTexture("UI\\container"); }
  ...
  for (CPassangersList::const_iterator it = passangers.begin(); it != passangers.end(); ++it)
  {
    ...
    if (!(*(passangers.begin()))->GetMOUnit()->CanSelect()) { rect.color = 0xff808080; }
  ...
  for (CPassangersList::const_iterator it = passangers.begin(); it != passangers.end(); ++it)
  {
    ...
    pVisitor->VisitUIRects(nullptr, 3, &rect, 1);
    fX += rcWindow.Height();
  }
  // draw icons
  fX = rcWindow.x1;
...
void CUISquadElement::Draw(interface IGFX *pGFX) { NI_ASSERT_SLOW_T(false, "Can't call Draw() directly - use visit pattern"); }
...
{
  if (pvPos) vPos = *pvPos;
  if (pvSize) vSize = *pvSize;
}

void CUISquadElement::SetWindowID(int _nID) { nID = _nID; }

void CUISquadElement::SetBoundRect(const CTRect<float> &rc) {}

bool CUISquadElement::IsInside(const CVec2 &vPos) { return rcWindow.IsInside(vPos); }

bool CUISquadElement::OnChar(int nAsciiCode, int nVirtualKey, bool bPressed, DWORD keyState) { return false; }

void CUISquadElement::SetParent(interface IUIContainer *_pParent) { pParent = _pParent; }
...
void CUISquadElement::EnableWindow(bool bEnable) { bWindowActive = bEnable; }

bool CUISquadElement::IsWindowEnabled() { return bWindowActive; }

void CUISquadElement::SetState(int nState, bool bNotify) {}

int CUISquadElement::GetState() { return 0; }

bool CUISquadElement::IsVisible() { return nCmdShow; }

int CUISquadElement::GetVisibleState() { return nCmdShow; }

void CUISquadElement::ShowWindow(int _nCmdShow) { nCmdShow = _nCmdShow; }

int CUISquadElement::GetWindowID() { return nID; }

int CUISquadElement::GetWindowPlacement(CVec2 *pPos, CVec2 *pSize, CTRect<float> *pScreenRect)
{
  if (pPos) *pPos = vPos;
  if (pSize) *pSize = vSize;
  if (pScreenRect) *pScreenRect = rcWindow;
  return nPositionFlag;
}

int CUISquadElement::GetPositionFlag() { return nPositionFlag; }

bool CUISquadElement::ProcessMessage(const SUIMessage &msg) { return false; }

IUIElement *CUISquadElement::PickElement(const CVec2 &vPos, int nRecursion) { return IsInside(vPos) ? this : nullptr; }

IManipulator *CUISquadElement::GetManipulator() { return nullptr; }

int CUISquadElement::operator&(IDataTree &ss) { return 0; }
...
bool CUISquadElement::OnLButtonDblClk(const CVec2 &vPos) { return IsInside(vPos); }

bool CUISquadElement::OnMouseMove(const CVec2 &vPos, EMouseState mouseState) { return IsInside(vPos); }

bool CUISquadElement::OnMouseWheel(const CVec2 &vPos, EMouseState mouseState, float fDelta) { return IsInside(vPos); }
...
bool CUISquadElement::OnLButtonUp(const CVec2 &vPos, EMouseState mouseState) { return true; }

bool CUISquadElement::OnRButtonDown(const CVec2 &vPos, EMouseState mouseState) { return true; }

bool CUISquadElement::OnRButtonUp(const CVec2 &vPos, EMouseState mouseState) { return true; }

void CUISquadElement::AddPassanger(IUnitStateObserver *pObserver) { passangers.push_back(dynamic_cast<CUIUnitObserver *>(pObserver)); }
...
void CUISquadElement::RemovePassanger(IUnitStateObserver *pObserver)
{
  ...
  if (passangers.empty()) GetSingleton<IInput>()->AddMessage(SGameMessage(MC_UPDATE_WHO_IN_CONTAINER));
}

int CUISquadElement::GetPassangerCount() { return passangers.size(); }