...
int CComplexObjects::operator&(IStructureSaver &ss)
{
  ...
  using CObjectPair = std::pair<CPtr<SMapObject>, CPtr<IRefCount>>;
  std::list<CObjectPair> objects;
  // 
  if (!saver.IsReading()) { for (auto it = visai.begin(); it != visai.end(); ++it) objects.push_back(CObjectPair(it->first, it->second)); }
  //
  saver.Add(1, &objects);
  //
  if (saver.IsReading())
  {
    visai.clear();
    aivis.clear();
    for (auto it = objects.begin(); it != objects.end(); ++it) AddSegment(it->second, it->first);
  }
  return 0;
}
...
void AddSpan(IRefCount *pAIObj, SBridgeSpanObject *pSpan, std::list<SSpanStorage> &spans)
{
  ...
  storage.pSpan = pSpan;
  if (pSpan->pSlab) storage.visobjects.push_back(pSpan->pSlab->pVisObj);
  if (pSpan->pBackGirder) storage.visobjects.push_back(pSpan->pBackGirder->pVisObj);
  if (pSpan->pFrontGirder) storage.visobjects.push_back(pSpan->pFrontGirder->pVisObj);
}
...
int CWorldBase::operator&(IStructureSaver &ss)
{
  ...
  // such a hemorrhoidal way to ensure that all indicators are correctly restored through the automatic system
  // Vis & AI objects must be stored separately, because 
  {
    std::list<SMapObjectStorage> mos;
    std::list<SSpanStorage> spans;
    using CObjectsPair = std::pair<CPtr<IRefCount>, CPtr<IRefCount>>;
    ...
    if (saver.IsReading())
    {
      ...
      for (auto it = mos.begin(); it != mos.end(); ++it)
      {
        ...
        if (storage.pAIObj) aiobjects[storage.pAIObj] = storage.pMO;
        if (storage.pVisObj) visobjects[storage.pVisObj] = storage.pMO;
      }
      //
      for (auto it = spans.begin(); it != spans.end(); ++it)
      {
        SSpanStorage &storage = *it;
        if (storage.pAIObj) aispans[storage.pAIObj] = storage.pSpan;
        for (auto vis = storage.visobjects.begin(); vis != storage.visobjects.end(); ++vis) { if (*vis) visspans[*vis] = storage.pSpan; }
      }
      //
    ...
    {
      // map objects
      for (auto it = aiobjects.begin(); it != aiobjects.end(); ++it) AddMapObj(it->first, it->second, mos);
      for (auto it = visobjects.begin(); it != visobjects.end(); ++it) { if (FindByAI(it->second->pAIObj) == nullptr) AddMapObj(nullptr, it->second, mos); }
      // spans
      for (auto it = aispans.begin(); it != aispans.end(); ++it) AddSpan(it->first, it->second, spans);
      for (auto it = visspans.begin(); it != visspans.end(); ++it) { if (FindSpanByAI(it->second->pAIObj) == nullptr) AddSpan(nullptr, it->second, spans); }
      //
  ...
  if (saver.IsReading())
  {
    dwFlashFireColor = GetGlobalVar((std::string("Scene.Colors.") + GetSeasonName() + ".FlashFire.Color").c_str(), static_cast<int>(0xffffffff));
    dwFlashExpColor = GetGlobalVar((std::string("Scene.Colors.") + GetSeasonName() + ".FlashExplode.Color").c_str(), static_cast<int>(0xffffffff));
  }
...
CWorldBase::CWorldBase()
{
  ...
  //
  // first you need to update new objects, because they can die in the same frame
  // but new ones cannot be created in place of dead ones in the same frame - this is monitored in Updater
  ...
  // AIUpdateRemoveObjects come after AIUpdateDeadUnits, since units can disappear after dying in the same segment
...
SMapObject *CWorldBase::CreateMapObject(IRefCount *pAIObj, int nDBID, int nFrameIndex, const float fNewHP)
{
  ...
  //
  if (pMO->Create(pAIObj, pDesc, nSeason, nFrameIndex, fNewHP, pVOB, pGDB) == true) return static_cast<SMapObject *>(pMO);
  pMO->AddRef();
  pMO->Release();
  return nullptr;
}
...
void CWorldBase::AddToScene(SMapObject *pMO, bool bOutbound, EObjGameType eGameType)
{
  ...
  pMO->Visit(&visitor);
  for (auto it = visitor.objects.begin(); it != visitor.objects.end(); ++it)
  {
    if (bOutbound) pScene->AddOutboundObject(it->pVisObj, it->eGameType);
    else if (it->bOutbound) pScene->AddOutboundObject2(it->pVisObj, it->eGameType);
    else if (it->eGameType == SGVOGT_SHADOW)
    {
      it->pVisObj->SetColor(0x00000000);
      it->pVisObj->SetOpacity(0x80);
      pScene->AddObject(it->pVisObj, SGVOGT_SHADOW, nullptr);
    }
    else if (it->eGameType == SGVOGT_FLAG) pScene->AddObject(it->pVisObj, SGVOGT_UNIT, pMO->pDesc);
    else if (eGameType == SGVOGT_UNKNOWN) pScene->AddObject(it->pVisObj, it->eGameType, pMO->pDesc);
    else pScene->AddObject(it->pVisObj, eGameType, pMO->pDesc);
  }
...
{
  // remove shadow
  if (pMO->pShadow) pScene->RemoveObject(pMO->pShadow);
  // remove main object
  pScene->RemoveObject(pMO->pVisObj);
...
{
  if (bDelayed) delayedRemoveAIObjes.push_back(pMO);
  else
  {
...
{
  if (SMapObject *pContainer = GetContainer(pMO)) static_cast<IMOContainer *>(pContainer)->Load(static_cast<IMOUnit *>(pMO), false);
...
void CWorldBase::GetAllObjectsByMatch(std::list<SMapObject *> &mapobjects, const SGDBObjectDesc *pDesc, const bool bSelectableOnly) { for (auto it = visobjects.begin(); it != visobjects.end(); ++it) { if ((it->second->pDesc == pDesc) && (!bSelectableOnly || (bSelectableOnly && it->second->CanSelect()))) mapobjects.push_back(it->second); } }
...
SBridgeSpanObject *CWorldBase::CreateSpanObject(int nDBID, int nFrameIndex, float fNewHP)
{
  ...
  auto pRPG = static_cast<const SBridgeRPGStats *>(pGDB->GetRPGStats(pDesc));
  // create new empty span object
  SBridgeSpanObject *pSpan = CreateObject<SBridgeSpanObject>(MISSION_MO_BRIDGE_SPAN);
  pSpan->Create(nullptr, pDesc, nSeason, nFrameIndex, fNewHP, pVOB, pGDB);
  //
  return pSpan;
}
...
SBridgeSpanObject *CWorldBase::AddSpanToWorld(IRefCount *pAIObj, int nDBID, int nFrameIndex, float fNewHP)
{
  ...
  if (pSpan->pBackGirder) pSpan->pBackGirder->pAIObj = pAIObj;
  if (pSpan->pFrontGirder) pSpan->pFrontGirder->pAIObj = pAIObj;
  pSpan->pAIObj = pAIObj;
  // register in AI map
  aispans[pSpan->GetAIObj()] = pSpan;
  //
  NewObjectAdded(pSpan->pSlab);
  if (pSpan->pBackGirder) NewObjectAdded(pSpan->pBackGirder);
  if (pSpan->pFrontGirder) NewObjectAdded(pSpan->pFrontGirder);
  //
  return pSpan;
}
...
void CWorldBase::AddToScene(SBridgeSpanObject *pSpan)
{
  ...
  if (pSpan->pBackGirder) visspans[pSpan->pBackGirder->pVisObj] = pSpan;
  if (pSpan->pFrontGirder) visspans[pSpan->pFrontGirder->pVisObj] = pSpan;
  // add main object
  AddToScene(pSpan->pSlab, false, SGVOGT_BRIDGE);
  if (pSpan->pBackGirder) AddToScene(pSpan->pBackGirder, false, SGVOGT_OBJECT);
  if (pSpan->pFrontGirder) AddToScene(pSpan->pFrontGirder, false, SGVOGT_OBJECT);
}

void CWorldBase::RemoveFromWorld(SBridgeSpanObject *pSpan)
{
  aispans.erase(pSpan->GetAIObj());
  pSpan->pAIObj = nullptr;
  if (pSpan->pSlab) pSpan->pSlab->pAIObj = nullptr;
  if (pSpan->pBackGirder) pSpan->pBackGirder->pAIObj = nullptr;
  if (pSpan->pFrontGirder) pSpan->pFrontGirder->pAIObj = nullptr;
}
...
void CWorldBase::RemoveFromScene(SBridgeSpanObject *pSpan)
{
  ...
  if (pSpan->pBackGirder) RemoveFromScene(pSpan->pBackGirder);
  if (pSpan->pFrontGirder) RemoveFromScene(pSpan->pFrontGirder);
  // unregister main object
  visspans.erase(pSpan->pSlab->pVisObj);
  if (pSpan->pBackGirder) visspans.erase(pSpan->pBackGirder->pVisObj);
  if (pSpan->pFrontGirder) visspans.erase(pSpan->pFrontGirder->pVisObj);
}
...
void CWorldBase::Update(const NTimer::STime &currTime)
{
  ...
  {
    if ((*it)->Update(currTime) == true) it = updatable.erase(it);
    else ++it;
  }
  //
  ...
  {
    for (auto it = fnAIUpdates.begin(); it != fnAIUpdates.end(); ++it) (this->*(*it))(currTime);
    pAILogic->EndUpdates();
    UpdatePick(GetSingleton<ICursor>()->GetPos(), currTime, true);
  }
  ...
  while (!delayedRemoveAIObjes.empty())
  {
    ...
    {
      if (pMO->pVisObj) pMO->pVisObj->Update(currTime);
  ...
  if (bForceRotation)
  {
    ...
    for (auto it = visobjects.begin(); it != visobjects.end(); ++it) it->second->pVisObj->SetDirection(nRotateDirection);
  }
  // CRAP}
...
void CWorldBase::GetVisibilityRectBounds(const CTRect<float> &rcScreen, CVec2 *pvLT, CVec2 *pvRT, CVec2 *pvLB, CVec2 *pvRB)
{
  ...
  const float fCellCoeff = 1.0f / fWorldCellSize;
  {
    ...
    pvLT->Set(static_cast<int>(vPos.x * fCellCoeff + 0.5f) - 3, static_cast<int>(vPos.y * fCellCoeff + 0.5f));
  }
  {
    CTPoint<float> point = rcScreen.GetRightBottom();
    GetPos3(&vPos, point.x, point.y);
    pvRB->Set(static_cast<int>(vPos.x * fCellCoeff + 0.5f) + 3, static_cast<int>(vPos.y * fCellCoeff + 0.5f));
  }
  // correct result
  pvLB->x = 0.5f * (pvRB->x - pvRB->y + pvLT->x + pvLT->y);
  pvLB->y = 0.5f * (pvLT->x + pvLT->y - pvRB->x + pvRB->y);
  if ((MINT(pvLB->x) != pvLB->x) || (MINT(pvLB->y) != pvLB->y)) pvLT->x--;
  // recalc
  pvLB->x = 0.5f * (pvRB->x - pvRB->y + pvLT->x + pvLT->y);
  pvLB->y = 0.5f * (pvLT->x + pvLT->y - pvRB->x + pvRB->y);
...
void CWorldBase::AIUpdateWarFogLocal()
{
  ...
  // war fog
  struct SAIVisInfo *pObjects = nullptr;
  int nNumObjects = 0;
...
bool CWorldBase::ToggleAIInfo()
{
  ...
  if (!bEnableAIInfo) pScene->GetTerrain()->SetAIMarker(nullptr, 0);
  else if (bEnableAIInfo) vLastAnchor.Set(-1000000, -1000000, -1000000);
...
{
  if (!bEnableAIInfo) return;
  //
  SAIPassabilityInfo *pObjects = nullptr;
  int nNumObjects = 0;
...
void CWorldBase::AIUpdateNewObjects(const NTimer::STime &currTime)
{
  ...
  for (int i = 0; i < nNumObjects; ++i)
  {
    ...
    if (info.nFrameIndex == -2)// special case - "shit-after-death"
    {
      if (const SMechUnitRPGStats *pRPG = NGDB::GetRPGStats<SMechUnitRPGStats>(pGDB, pDesc)) { if (!pRPG->deathCraters.empty()) SetCraterEffect(pRPG->deathCraters[rand() % pRPG->deathCraters.size()], nSeason, vPos, 100, pScene, pVOB); }
    }
    else// ordinary static object
    {
      ...
      // CRAP{ set scale
      if (pObjects[i].fResize != 1.0f) static_cast_ptr<IObjVisObj *>(pMO->pVisObj)->SetScale(pObjects[i].fResize, pObjects[i].fResize, pObjects[i].fResize);
      // CRAP}
      // add to scene
      AddToScene(pMO);
      ...
      {
        auto pStats = static_cast_gdb<const SObjectBaseRPGStats *>(pMO->pRPG);
        const CVec3 vPos = pMO->pVisObj->GetPosition();
        SObjectSounds info;
        if (!pStats->szAmbientSound.empty()) info.wSoundID = pScene->AddSoundToMap(pStats->szAmbientSound.c_str(), vPos);
        if (!pStats->szCycledSound.empty()) info.wLoopedSoundID = pScene->AddSoundToMap(pStats->szCycledSound.c_str(), vPos);
        if (info.wSoundID || info.wLoopedSoundID) objectsSounds[pMO] = info;
      }
    }
  }
}
...
void CWorldBase::AIUpdateRemoveObjects(const NTimer::STime &currTime)
{
  ...
  for (int i = 0; i < nNumObjects; ++i)
  {
    if (SMapObject *pMO = FindByAI(ppObjects[i]))
    {
      ...
      {
        auto pMOUnit = static_cast<IMOUnit *>(pMO);
        pMOUnit->PrepareToRemove();
        pMOUnit->SetSquad(nullptr);
  ...
  for (int i = 0; i < nNumObjects; ++i)
  {
    if (SMapObject *pMO = FindByAI(ppObjects[i]))
    {
      if (pMO->pDesc->eGameType == SGVOGT_ENTRENCHMENT) entrenchments.RemoveSegment(pMO);
      RemoveMapObj(pMO);
    }
    else if (SBridgeSpanObject *pSpan = FindSpanByAI(ppObjects[i])) { RemoveSpanObj(pSpan); }
  }
}
...
void CWorldBase::AIUpdateNewUnits(const NTimer::STime &currTime)
{
  ...
  for (int i = 0; i < nNumObjects; ++i)
  {
    ...
    // if ( info.eDipl != EDI_NEUTRAL )
    {
      ...
      if (dynamic_cast<IMOSelectable *>(pMO) != nullptr) static_cast<IMOSelectable *>(pMO)->AIUpdateDiplomacy(SAINotifyDiplomacy(info.eDipl, pMO->GetAIObj(), info.nPlayer));
    }
    // and, at rest, add object to scene
    if (pMO->GetDesc()->eGameType == SGVOGT_UNIT)
    {
      auto pRPG = static_cast<const SUnitBaseRPGStats *>(pMO->pRPG.GetPtr());
...
{
  SAINotifyNewProjectile *pObjects = nullptr;
  int nNumObjects = 0;
  ...
  if (nNumObjects > 0)
  {
    for (int i = 0; i < nNumObjects; ++i)
    {
      ...
      if (auto pMO = static_cast<SMapObject *>(static_cast<IMOUnit *>(pUnit)->AIUpdateFireWithProjectile(pObjects[i], currTime, pVOB)))
      {
        AddToWorld(pMO);
...
{
  SAINotifyPlacement *pObjects = nullptr;
  int nNumObjects = 0;
  // unit
  pAILogic->UpdatePlacements(&pObjects, &nNumObjects);
  for (int i = 0; i < nNumObjects; ++i) { if (IMapObj *pMO = FindByAI(pObjects[i].pObj)) pMO->AIUpdatePlacement(pObjects[i], currTime, pScene); }
  // static objects
...
{
  SAINotifyRPGStats *pObjects = nullptr;
  int nNumObjects = 0;
  ...
  for (int i = 0; i < nNumObjects; ++i)
  {
    if (SMapObject *pMO = FindByAI(pObjects[i].pObj))
    {
      if (pMO->AIUpdateRPGStats(pObjects[i], pVOB, pScene) == false)
      {
        ...
        RemoveFromSelectionGroup(pMO);
        // delete sounds
        auto soundIter = objectsSounds.find(pMO);
    ...
    else if (SBridgeSpanObject *pSpan = FindSpanByAI(pObjects[i].pObj)) pSpan->AIUpdateRPGStats(pObjects[i], pVOB, pScene);
  }
}

void CWorldBase::AIUpdateDiplomacy(const NTimer::STime &currTime)
{
  SAINotifyDiplomacy *pObjects = nullptr;
  int nNumObjects = 0;
...
{
  SAINotifyAction *pObjects = nullptr;
  int nNumObjects = 0;
  pAILogic->UpdateActions(&pObjects, &nNumObjects);
  for (int i = 0; i < nNumObjects; ++i)
  {
    SMapObject *pMO = FindByAI(pObjects[i].pObj);
    if (pMO == nullptr) continue;
    // process selection messages
    switch (pObjects[i].typeID)
    {
      case ACTION_NOTIFY_CHANGE_SELECTION:
        if (pObjects[i].nParam == 1) Select(pMO);
        else if (pObjects[i].nParam == 0) ResetSelectionOverridable(pMO->pVisObj);
        break;
      case ACTION_NOTIFY_SELECT_CHECKED:
      {
        auto pMOSelectable = dynamic_cast<IMOSelectable *>(FindByAI(reinterpret_cast<IRefCount *>(pObjects[i].nParam)));
        if (pMOSelectable && pMOSelectable->IsSelected()) Select(pMO);
      }
      break;
      case ACTION_SET_SELECTION_GROUP:
      {
        auto pMOToCheck = dynamic_cast<IMOSquad *>(FindByAI(pAILogic->GetObjByUniqueID(pObjects[i].nParam)));
        int nSelGroup = -1;
        ...
        {
          auto pMOTemp = dynamic_cast<IMOSelectable *>(FindByAI(pAILogic->GetObjByUniqueID(pObjects[i].nParam)));
          if (pMOTemp)
          {
        ...
        auto pMOUnitToGroup = dynamic_cast<IMOSquad *>(pMO);
        if (pMOUnitToGroup)
        {
          const int nNumPassangers = pMOUnitToGroup->GetPassangers(nullptr);
          if (nNumPassangers > 0)
          {
            std::vector<IMOUnit *> vMOUnit(nNumPassangers);
            vMOUnit.resize(nNumPassangers);
            ...
            for (auto it = vMOUnit.begin(); it != vMOUnit.end(); ++it)
            {
              ...
              if (isSelected) Select(*it);
            }
          }
        }
        else
        {
          auto pMOTemp = dynamic_cast<IMOUnit *>(pMO);
          if (pMOTemp)
          {
            AddUnitToSelectionGroup(pMOTemp, nSelGroup);
            if (isSelected) Select(pMOTemp);
          }
        }
      }
      break;
      case ACTION_NOTIFY_SELECTABLE_CHANGED:
        if (pObjects[i].nParam == 0) ResetSelection(pMO);
        break;
    }
    if (pMO->AIUpdateActions(pObjects[i], currTime, pVOB, pScene, pAckManager) != 0) AddUpdatableUnit(static_cast<IMOUnit *>(pMO));
  }
}

const SUnitBaseRPGStats::SAnimDesc *GetAnimationByType(int nType, const SUnitBaseRPGStats *pStats)
{
  if ((nType >= pStats->animdescs.size()) || pStats->animdescs[nType].empty()) return nullptr;
...
void CWorldBase::AIUpdateShots(const NTimer::STime &currTime)
{
  ...
  {
    SAINotifyMechShot *pObjects = nullptr;
    int nNumObjects = 0;
    pAILogic->UpdateShots(&pObjects, &nNumObjects);
    for (int i = 0; i < nNumObjects; ++i) { if (SMapObject *pMO = FindByAI(pObjects[i].pObj)) static_cast<IMOContainer *>(pMO)->AIUpdateShot(pObjects[i], currTime, pVOB, pScene); }
  }
  // infantry unit shots
  {
    SAINotifyInfantryShot *pObjects = nullptr;
    int nNumObjects = 0;
    pAILogic->UpdateShots(&pObjects, &nNumObjects);
    for (int i = 0; i < nNumObjects; ++i) { if (SMapObject *pMO = FindByAI(pObjects[i].pObj)) static_cast<IMOContainer *>(pMO)->AIUpdateShot(pObjects[i], currTime, pVOB, pScene); }
  }
...
{
  SAINotifyTurretTurn *pObjects = nullptr;
  int nNumObjects = 0;
  ...
  for (int i = 0; i < nNumObjects; ++i)
  {
    if (pObjects[i].nModelPart != -1)
    {
      if (SMapObject *pMO = FindByAI(pObjects[i].pObj))
      {
        if (auto pAnim = static_cast<IMeshAnimation *>(static_cast_ptr<IObjVisObj *>(pMO->pVisObj)->GetAnimation()))
        {
          const float fAngle = static_cast<float>(pObjects[i].wAngle) / 65536.0f * FP_2PI;
          const NTimer::STime timeStart = currTime, timeEnd = pObjects[i].endTime;
          // for all model parts, packed in the 'nModelPart'
          for (int j = 0; j != 4; ++j)
          {
            const int nModelPart = (static_cast<DWORD>(pObjects[i].nModelPart) >> (j * 8)) & 0x000000ff;
            if (nModelPart != 0xff) pAnim->AddProceduralNode(nModelPart, currTime, timeStart, timeEnd, fAngle);
          }
        }
      }
    }
...
{
  SAINotifyDeadAtAll *pObjects = nullptr;
  int nNumObjects = 0;
  pAILogic->GetDeadUnits(&pObjects, &nNumObjects);
  for (int i = 0; i < nNumObjects; ++i)
  {
    auto pUnit = static_cast<IMOUnit *>(FindByAI(pObjects[i].pObj));
    ...
    if (pUnit->pDesc->eVisType == SGVOT_MESH) static_cast_ptr<IMeshVisObj *>(pUnit->pVisObj)->RemoveEffector(-1, -1);

    pUnit->RemoveSounds(pScene);

    if (SMapObject *pContainer = GetContainer(pUnit))
    {
      if (pUnit->GetContainer()) static_cast<IMOContainer *>(pContainer)->Load(pUnit, false);
      RemoveFromContainer(pUnit);
      RemoveFromScene(pUnit);
    }
    else
    {
      auto pos = inContainer.find(pUnit->pAIObj);
    ...
    if (pObjects[i].bRot == false) RemoveAIObj(pUnit, true);
  }
}
...
void CWorldBase::AIUpdateDeadProjectiles(const NTimer::STime &currTime)
{
  ...
  for (int i = 0; i < nNumObjects; ++i) { if (SMapObject *pMO = FindByAI(ppObjects[i])) RemoveMapObj(pMO); }
}

void CWorldBase::AIUpdateHits(const NTimer::STime &currTime)
{
  SAINotifyHitInfo *pObjects = nullptr;
  int nNumObjects = 0;
  ...
  if (nNumObjects > 0)
  {
    for (int i = 0; i != nNumObjects; ++i)
    {
      if (pObjects[i].pWeapon == nullptr) continue;
      ...
      if (pObjects[i].pVictim)// to object
      {
        ...
        else { NStr::DebugTrace("Victim 0x%x (%s) for hit is not a valid map object\n", pObjects[i].pVictim, typeid(pObjects[i].pVictim).name()); }
      }
      else// to ground
      {
        ...
        const std::string *pEffectName = nullptr;
        ...
        // effects
        if (pEffectName) PlayEffect(*pEffectName, vPos, currTime, false, pScene, pVOB, 0, SFX_MIX_IF_TIME_EQUALS, SAM_ADD_N_FORGET, ESCT_COMBAT);
        // crater picture
        if (bNeedCrater && shell.HasCraters()) SetCraterEffect(shell.GetRandomCrater(), nSeason, vPos, 110, pScene, pVOB);
        // flash
        if (bNeedCrater && shell.flashExplosion.HasFlash()) SetFlashEffect(shell.flashExplosion, currTime, vPos, dwFlashExpColor, pScene, pVOB);
      }
      // add earthquake
      if ((vPos != VNULL3) && (shell.fDetonationPower > 0)) pCamera->AddEarthquake(vPos, shell.fDetonationPower);
    }
  }
}
...
{
  {
    SAIAcknowledgment *pAcks = nullptr;
    int nNumObjs = 0;
  ...
  {
    SAIBoredAcknowledgement *pBoredAcks = nullptr;
    int nNumObjs = 0;
  ...
  for (int i = 0; i < nNumObjects; ++i)
  {
    switch (pObjects[i].feedBackType)
    {
      ...
      case EFB_SCOUT_DISABLED: // disable scouts
        ...
        break;
      case EFB_FIGHTERS_ENABLED: // enable figures
        pInput->AddMessage(SGameMessage(USER_ACTION_ENABLE_PLANE_FIGHTER, 0));
        break;
      case EFB_FIGHTERS_DISABLED: // disable figures
      ...
      case EFB_AAGUN_FIRED:
        if (bAADetectedFlag) pInput->AddMessage(SGameMessage(MC_VISUALIZE_FEEDBACK_AA_NEWDETECTED, pObjects[i].nParam));
        else
        {
      ...
      case EFB_BAD_WEATER:
      {
        ...
        if (bStart) pInput->AddMessage(SGameMessage(MC_VISUALIZE_FEEDBACK_BAD_WEATHER));
      }
      break;
...
void CWorldBase::AIUpdateEntrances(const NTimer::STime &currTime)
{
  ...
  for (int i = 0; i < nNumObjects; ++i)
  {
    ...
    {
      // add to container
      if (pMO) static_cast<IMOContainer *>(pMO)->Load(static_cast<IMOUnit *>(pInf), true);
    ...
    else
    {
      ...
      if (pMO) static_cast<IMOContainer *>(pMO)->Load(static_cast<IMOUnit *>(pInf), false);
      else
      {
        auto pUnit = checked_cast<IMOUnit *>(pInf);
        ...
        {
          if ((pUnit->fHP < 1.0f) || bSelected) pIcon->Enable(true);
...
{
  SSegment2Trench *pObjects = nullptr;
  int nNumObjects = 0;
...
{
  if (auto pSquad = static_cast<IMOSquad *>(FindByAI(pAISquad)))
  {
    pSquad->Load(pUnit, true);
    if (pSquad->IsSelected()) Select(pUnit);
...
{
  SSoldier2Formation *pObjects = nullptr;
  int nNumObjects = 0;
  pAILogic->GetFormations(&pObjects, &nNumObjects);
  for (int i = 0; i != nNumObjects; ++i) { if (auto pUnit = static_cast<IMOUnit *>(FindByAI(pObjects[i].pSoldier))) AddToSquad(pObjects[i].pFormation, pUnit); }
}
...
void CWorldBase::AIUpdateBridges(const NTimer::STime &currTime)
{
  ...
  // update bridges
  {
    while (true)
    {
      ...
      if (nNumObjects == 0) break;
      for (int i = 0; i != nNumObjects; ++i) { SBridgeSpanObject *pSpan = FindSpanByAI(ppObjects[i]); }
    }
  }
}
...
void CWorldBase::AIUpdateCombatSituationInfo(const NTimer::STime &currTime) { if (pAILogic->IsCombatSituation()) pScene->CombatNotify(); }
...
void CWorldBase::ShowIcons(int nID, bool bShow)
{
  ...
  {
    // showicons[nID] = bShow;
    for (auto it = visobjects.begin(); it != visobjects.end(); ++it)
    {
      if (it->second && it->second->pDesc && it->second->pDesc->IsObj())
      {
        ...
        if (pIcon) pIcon->Enable(bShow);
      }
    }
  }
  else { for (auto it = visobjects.begin(); it != visobjects.end(); ++it) { if (it->second && it->second->pDesc && it->second->pDesc->IsObj()) { for (int i = 0; i < ICON_NUM_ICONS; ++i) { if (ISceneIcon *pIcon = static_cast<IObjVisObj *>(it->second->pVisObj.GetPtr())->GetIcon(i)) pIcon->Enable(bShow); } } } }
}
...
const char *CWorldBase::GetSeasonName() const { return ::GetSeasonName(nSeason); }

void CWorldBase::ReportReinforcementArrived()
{
  const DWORD dwColor = GetGlobalVar((std::string("Scene.Colors.") + GetSeasonName() + ".Text.Information.Color").c_str(), static_cast<int>(0xffffffff));
  if (IText *pText = GetSingleton<ITextManager>()->GetString("reinforcement_arrived"))
  {
    ...
    if (pScene) pScene->AddSound("sounds\\reports\\information", VNULL3, SFX_INTERFACE, SAM_ADD_N_FORGET);
  }
}
...
void CWorldBase::UpdateAllUnits()
{
  ...
  for (auto it = visobjects.begin(); it != visobjects.end(); ++it) { if (auto pUnit = dynamic_cast_ptr<IMOUnit *>(it->second)) pUnit->ChangeWithBlood(pVOB); }
}