...
struct SSpriteAnimationFormat : ISharedResource
{
private:
...
public:
  struct SSpriteAnimation
  {
    ...
    const SSpriteRect &GetRect(const int nDirection, const int nTime) const
    {
      ...
      int nFrame;
      if (bCycled) nFrame = (nTime / nFrameTime) % dir.frames.size();
      else
      {
        if (nTime < nFrameTime * dir.frames.size()) nFrame = nTime / nFrameTime;
        else nFrame = dir.frames.size() - 1;
      }
  ...
  using CAnimations = std::vector<SSpriteAnimation>;
