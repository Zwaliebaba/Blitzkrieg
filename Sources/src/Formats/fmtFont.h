...
struct SFontFormat
{
  ...
  SFontFormat &operator=(const SFontFormat &format)
  {
    ...
    chars.clear();
    for (CCharacterMap::const_iterator it = format.chars.begin(); it != format.chars.end(); ++it) memcpy(&(chars[it->first]), &(it->second), sizeof(it->second));
    for (CKernMap::const_iterator it = format.kerns.begin(); it != format.kerns.end(); ++it) kerns[it->first] = it->second;
    //
    return *this;
  }
  ...
  {
    CKernMap::const_iterator pos = kerns.find((static_cast<DWORD>(wLastChar) << 16) | static_cast<DWORD>(wCurrChar));
