...
static constexpr float fAnimTimeStep = 62.5f;// 16 fps
...
struct SMeshFormat
{
  struct SVertexComponent
  {
    ...
    int tex;// texture coordinate index
    //
    int operator&(interface IStructureSaver &ss);
  };
  ...
  // face indices
  std::vector<WORD> indices;// indices from the 'components' array, which forms faces. 
...
struct SSkeletonFormat
{
  struct SNodeFormat
  {
    ...
    using CChildrenList = std::vector<int>;
  ...
  //
  using CNodesList = std::vector<SNodeFormat>;
