...
namespace NSaveLoad
{
  ...
  struct SFileHeader
  {
    ...
    //
    SFileHeader()
      : nVersion(VERSION), bRandomMission(false), nPictureSizeX(0), nPictureSizeY(0) {}
  };
  ...
  struct SRandomHeader
  {
    ...
    std::string szChapterUnitsTableFileName;// file with chapter units
    int nLevel;// complexity
    std::string szGraphName;// Count's name
    int nAngle;// turn
    //
    SRandomHeader()
