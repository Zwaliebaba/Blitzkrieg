...
inline DWORD GetPow2(int nSize) { return GetMSB(GetNextPow2(nSize)); }

inline int GetChunkSize(int nChunk) { return 1UL << nChunk; }
inline int GetOptimalChunk(int nSize) { return GetMSB(GetNextPow2(nSize)); }
inline int GetOptimalChunkSize(int nSize) { return GetChunkSize(GetOptimalChunk(nSize)); }
// r2 and r2 will contain in the second field not the size, but the index of the chunk
...
{
  // clear all ranges
  for (auto pos = ranges.begin(); pos != ranges.end(); ++pos) pos->clear();
...
{
  if (ranges[nChunk].empty()) return false;
...
bool CPow2Allocator::AllocateChunk(int nOptimalChunk, int nChunk, const SRangeLimits &range, SRangeLimits *pResult)
{
  ...
  if (r1.second >= nOptimalChunk)
  {
    ...
    // select the part we need from r1
    return AllocateChunk(nOptimalChunk, r1.second, r1, pResult);
  }
  if (r2.second >= nOptimalChunk)
  {
    // give range r1
    FreeLocal(r1.first, r1.second);
    // select the part we need from r2
...
void CPow2Allocator::FreeLocal(int nStart, int nChunk)
{
  ...
  else
  {
    ...
    for (auto it = subranges.begin(); it != subranges.end(); ++it)
    {
      ...
      if (nStart + nChunkSize == it->first)// can we collapse our block with 'it' as next ?
      {
        subranges.erase(it);
        FreeLocal(nStart, nChunk + 1);
        return;
    ...
    if (nChunk - 1 >= 0)
    {
      ...
      for (auto it = subranges2.begin(); it != subranges2.end(); ++it)
      {
        if (it->first == nStart1) its[0] = it;
        else if (it->first == nStart2) its[1] = it;
      }
      //
    ...
    if (nChunk < ranges.size() - 1)
    {
      ...
      for (auto it = subranges.begin(); it != subranges.end(); ++it)
      {
        if (it->first == nStart1)
        {
          ...
          std::list<SRangeLimits> &subranges2 = ranges[nChunk + 1];
          for (auto range = subranges2.begin(); range != subranges2.end(); ++range)
          {
...
EAllocVals CPow2Allocator::Allocate(int nAmount, SRangeLimits *pRange)
{
  ...
  if (nAmount > GetTotalSize()) return EAV_NOSIZE;
  //
  ...
  for (; nTopChunk != ranges.size(); ++nTopChunk)
  {
    ...
    {
      SRangeLimits range;
      if (GetRangeFromChunk(nTopChunk, &range) == false) continue;
...
EAllocVals CPow2Allocator::HasBlock(int nAmount) const
{
  ...
  for (; pos != ranges.end(); ++pos) { if (!pos->empty()) return EAV_SUCCESS; }
  return EAV_NOFREE;
}
...
{
  int nNumBlocks = 0;
  for (auto it = ranges.begin(); it != ranges.end(); ++it) nNumBlocks += it->size();
  return nNumBlocks;
}
...
int CPow2Allocator::GetFree() const
{
  int nAllocated = 0;
  int i = 0;
  for (auto it = ranges.begin(); it != ranges.end(); ++it, ++i) nAllocated += it->size() * GetChunkSize(i);
  return nAllocated;
}
