...
public:
  ...
  struct SShadeValues
  {
    ...
    std::vector<CShadesList> ssses;// sampler states
  };
...
private:
  ...
  {
    if (nStage >= shades.ssses.size()) shades.ssses.resize(nStage + 1);
    shades.ssses[nStage].push_back(SShade(dwState, dwValue));
  }
