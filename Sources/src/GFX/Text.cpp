...
{
  if (bRedLine) { if (pFont && (fRedLineSize == 0)) fRedLineSize = 2.0f * pFont->GetFormat().metrics.nAveCharWidth; }
  else fRedLineSize = 0;
}
...
int CGFXText::GetLineSpace() const { return pFont != nullptr ? pFont->GetFormat().GetLineSpace() : -1; }

int CGFXText::GetWidth(int nNumCharacters) const
{
  if (pText == nullptr) return 1;
  const wchar_t *pszStringBegin = pText->GetString();
  if (pszStringBegin == nullptr) return 1;
...
// text pre-formatting
// split text on the lines, each of which consists of words and additional info like spaceless width etc.
...
inline const bool IsSpecial(const wchar_t *pszCurr, const wchar_t *pszEnd) { return (pszCurr >= pszEnd) || IsEOL(*pszCurr) || IsSPC(*pszCurr) || IsEOF(*pszCurr); }
...
void CGFXText::PreFormat() const
{
  ...
  pft.lines.reserve(MINT(static_cast<float>(format.metrics.nAveCharWidth * nStrLen) / fWidth * 1.5f) + 1);
  //
  ...
  while (true)
  {
    ...
    if (IsSpecial(pszLine, pszStringEnd))
    {
      ...
      if (pszLine > pszWordBegin)
      {
        ...
        if (pLine->nNumWords == 1) pLine->fPreSpace = format.GetChar(*pszWordBegin).fA;
        // add num spaces in this line
        if (pLine->IsFirstLine() || (pLine->nNumWords > 1)) pLine->nNumSpaces += nNumWordSpaces;
        //
        nNumWordSpaces = 0;
      }
      // process more special cases
      if (IsEOF(c))// end-of-file. 
      {
        ...
        if (!pLine->IsFirstLine() && !pLine->words.empty()) pLine->words.front().nNumPreSpaces = 0;
        break;
      }
      else if (IsEOL(c))// end-of-line. 
      {
        // finish this line
        pLine->properties |= SPreFormattedText::SLine::PROP_LAST_LINE;
        pLine->fWidth = fLineWordWidth - pLine->fPreSpace;
    ...
    else// simply symbol
    {
      ...
      {
        // special case: line width exceeded, but no words added! 
        if (pLine->words.empty())
        {
          ...
          if (pszLine - pszWordBegin > 0)
          {
            ...
            // add num spaces in this line
            if (pLine->IsFirstLine()) pLine->nNumSpaces += nNumWordSpaces;
            nNumWordSpaces = 0;
  ...
  while (true)
  {
    ...
    if (IsSpecial(pszLine, pszStringEnd))
    {
      ...
      if (pszLine > pszWordBegin)
      {
        ...
        if (pLine->nNumWords == 1) pLine->fPreSpace = format.GetChar(*pszWordBegin).fA;
        // add num spaces in this line
        if (pLine->nNumWords > 1) pLine->nNumSpaces += nNumWordSpaces;
        //
        nNumWordSpaces = 0;
      }
      // process more special cases
      if (IsEOF(c))// end-of-file. 
      {
        // finish this line and finish text processing
        pLine->fWidth = fLineWordWidth - pLine->fPreSpace;
        if (!pLine->words.empty()) pLine->words.front().nNumPreSpaces = 0;
        break;
      }
      else if (IsSPC(c))// white-space
        ++nNumWordSpaces;
      else { NI_ASSERT_SLOW_T(IsEOL(c), "EOL characters doesn't supported in the single-line mode!!!"); }
      //
      pszWordBegin = pszLine + 1;
      wLastChar = 0;
    }
...
// service function to calculate number of triangles, was generated during this line geometry data filling
void EstimateNumVerticesAndIndices(const SPreFormattedText::SLine &line, int &nNumVertices, int &nNumIndices)
{
  for (auto pos = line.words.begin(); pos != line.words.end(); ++pos)
  {
                                       ...
                                       std::vector<SGFXLVertex> &vertices, std::vector<WORD> &indices) const
{
  ...
  if (pszStringBegin == pszStringEnd) return sx;// can't render zero-length string
...
constexpr DWORD FNT_CLIP_TOP = 1;
constexpr DWORD FNT_CLIP_BOTTOM = 2;
                                ...
                                std::vector<SGFXLVertex> &vertices, std::vector<WORD> &indices) const
{
  ...
  {
    PreFormatLine();
    if (pft.lines.empty()) return true;
    //
    const float sx = rect.left;
    if (dwFlags & FNT_FORMAT_LEFT) FillGeometryDataLeft(pft.lines.front(), sx, sy, rect, dwColor, dwSpecular, FNT_CLIP_TOP | FNT_CLIP_BOTTOM, vertices, indices);
    else if (dwFlags & FNT_FORMAT_RIGHT) FillGeometryDataRight(pft.lines.front(), sx, sy, rect, dwColor, dwSpecular, FNT_CLIP_TOP | FNT_CLIP_BOTTOM, vertices, indices);
    else
    {
  ...
  // at the beginning, find first and last lines, which fits in our rect
  const SFontFormat &format = pFont->GetFormat();
  int nY = MINT(sy);
  int nLineSpace = format.GetLineSpace();
  int nFirstLine = 0, nLastLine = lines.size();
  bool bClipFirstLine = false, bClipLastLine = false;
  // check for text completely outside
  if ((nY >= rect.bottom) || (nY + static_cast<int>(lines.size()) * nLineSpace <= rect.top)) return true;
  // first line
  for (int i = nFirstLine; i < nLastLine; ++i)
  {
    ...
    {
      nFirstLine = i;
      if ((nY < rect.top) && (nY + nLineSpace > rect.top)) bClipFirstLine = true;
      break;
    }
    nY += nLineSpace;
  }
  ...
  for (int i = nFirstLine; i < nLastLine; ++i)
  {
    ...
    {
      nLastLine = i + 1;
      if ((nY < rect.bottom) && (nY + nLineSpace > rect.bottom)) bClipLastLine = true;
      break;
    }
    nY += nLineSpace;
  }
  ...
  if (dwFlags & FNT_FORMAT_LEFT)// left
  {
    ...
    {
      // setup clip flags
      if ((i == nFirstLine) && bClipFirstLine) dwClipFlags = FNT_CLIP_TOP;
      else if ((i == nLastLine - 1) && bClipLastLine) dwClipFlags = FNT_CLIP_BOTTOM;
      else dwClipFlags = 0;
      // draw line
  ...
  else if (dwFlags & FNT_FORMAT_JUSTIFY)// justify
  {
    for (int i = nFirstLine; i < nLastLine; ++i)
    {
      ...
      // draw line
      if (lines[i].IsLastLine()) FillGeometryDataLeft(lines[i], sx, sy, rect, dwColor, dwSpecular, dwClipFlags, vertices, indices);
      else FillGeometryDataJustify(lines[i], sx, sy, rect, dwColor, dwSpecular, dwClipFlags, vertices, indices);
      sy += format.GetLineSpace();
    }
  }
...
// fill geometric data for a left-aligned row
bool CGFXText::FillGeometryDataLeft(const SPreFormattedText::SLine &line, float sx, const float sy, const RECT &rect,
                                    ...
                                    std::vector<SGFXLVertex> &vertices, std::vector<WORD> &indices) const
{
  if (line.words.empty()) return true;
  const SFontFormat &format = pFont->GetFormat();
  ...
  {
    for (auto it = line.words.begin(); it != line.words.end(); ++it)
    {
...
// fill geometric data for a right aligned row
                                     ...
                                     std::vector<SGFXLVertex> &vertices, std::vector<WORD> &indices) const
{
  ...
  {
    // do first shift to achieve 'right' formatting
...
// fill in geometric data for a center-aligned row
                                      ...
                                      std::vector<SGFXLVertex> &vertices, std::vector<WORD> &indices) const
{
  ...
  if (line.nNumWords == 1)// do not need spacing
  {
    sx += static_cast<float>(floor((fWidth - line.fWidth) * 0.5)) + line.words.back().nNumPreSpaces * format.metrics.fSpaceWidth;
    (this->*pfnFillGeometryData)(line.words.back().pszBegin, line.words.back().pszEnd, sx, sy, rcClipRect, dwColor, dwSpecular, vertices, indices);
  }
  else// add spaces before each word
  {
    // do first shift to achieve 'center' formatting
    sx += static_cast<float>(floor(((fWidth - line.fWidth) - (line.nNumSpaces * format.metrics.fSpaceWidth)) * 0.5));
    for (auto it = line.words.begin(); it != line.words.end(); ++it)
    {
      sx += it->nNumPreSpaces * format.metrics.fSpaceWidth;
      sx = (this->*pfnFillGeometryData)(it->pszBegin, it->pszEnd, sx, sy, rcClipRect, dwColor, dwSpecular, vertices, indices);
...
// fill geometric data for a line with width alignment
                                       ...
                                       std::vector<SGFXLVertex> &vertices, std::vector<WORD> &indices) const
{
  ...
  {
    // calculate average space width to achieve a 'justify' formatting
    float fTotalSpace = 0, fLastTotalSpace = 0;
    const float fSpaceSize = (fWidth - line.fWidth) / static_cast<float>(line.nNumSpaces);
    for (auto it = line.words.begin(); it != line.words.end(); ++it)
    {
      // calculate integer space step
      fTotalSpace += it->nNumPreSpaces * fSpaceSize;
      const float fSpace = static_cast<float>(int(fTotalSpace)) - fLastTotalSpace;
      sx += fSpace;
