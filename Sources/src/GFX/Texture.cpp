                  ...
                  const int nSizeX, const int nSizeY, const int nBPP, const int nCurrMipLevel, const int nLineModifier)
{
  ...
  else
  {
    ...
    auto pDstData = static_cast<DWORD *>(lockinfo.pData);
...
bool CTexture::Load(const bool bPreLoad)
{
  ...
  //
  constexpr int nStartMipLevel = 0;
  //
  if (pStream != nullptr)// load DDS image
  {
    ...
    if (dwSignature != SDDSHeader::SIGNATURE) return false;
    // read header
    SDDSHeader hdr;
    ...
    {
      int nToSeek = 0;
      for (int i = 0; i < nStartMipLevel; ++i) nToSeek += (hdr.dwWidth >> i) * (hdr.dwHeight >> i) * nBPP / 8;
