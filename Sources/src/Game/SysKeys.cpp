...
namespace NSysKeys
{
  ...
  LRESULT CALLBACK LowLevelKeyboardProc(INT nCode, WPARAM wParam, LPARAM lParam)
  {
    ...
    auto pkbhs = (KBDLLHOOKSTRUCT *) lParam;
    ...
    switch (nCode)
    {
      case HC_ACTION:
        ...
        if ((pkbhs->vkCode == VK_LWIN) || (pkbhs->vkCode == VK_RWIN) || (pkbhs->vkCode == VK_APPS)) return 1;
        break;
    }
  ...
  void EnableSystemKeys(bool bEnable, HINSTANCE hInstance)
  {
    ...
    {
      if (bEnable && (hHook != nullptr)) UnhookWindowsHookEx(hHook);
      else hHook = SetWindowsHookEx(WH_KEYBOARD_LL, LowLevelKeyboardProc, hInstance, 0);
    }
    bCurrEnable = bEnable;
  }
