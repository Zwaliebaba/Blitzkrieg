...
static char *mytok(char *instr, char delim)
{
  ...
  if (instr) thestr = instr;
  result = thestr;
  while (*thestr && *thestr != delim) { thestr++; }
  if (thestr == result) result = NULL;
...
static void ServerSetAddressFromString(GServer server, char *address)
{
  char *cpos;
  cpos = strchr(address, ':');
  if (cpos == NULL) return;//no : character
  *cpos = 0;
...
GServer ServerNewData(char **fieldlist, int fieldcount, char *serverdata, GQueryType qtype, HashTable keylist)
{
  ...
  while (curfield < fieldcount)
  {
    ...
    curfield++;
    if (curfield < fieldcount) v = mytok(NULL, '\\');
  }
  return server;
}
...
static int CheckValidKey(char *key)
{
  ...
  int i;
  for (i = 0; i < sizeof(InvalidKeys) / sizeof(InvalidKeys[0]); i++) { if (strcmp(key, InvalidKeys[i]) == 0) return 0; }
  return 1;
}
...
int ServerGetPing(GServer server) { return server->ping; }

/* ServerGetAddress
-------------------
Returns the string, dotted IP address for the specified server */
char *ServerGetAddress(GServer server) { return inet_ntoa(*(struct in_addr *) &server->ip); }

/* ServerGetInetAddress
-------------------
Returns the IP address for the specified server */
unsigned int ServerGetInetAddress(GServer server) { return server->ip; }


/* ServerGetPort
----------------
Returns the "query" port for the specified server. */
int ServerGetQueryPort(GServer server) { return server->port; }
...
static GKeyValuePair *ServerRuleLookup(GServer server, char *key)
{
  ...
  if (keyindex == NULL) return NULL;//otherwise, the keyindex->keyindex is valid, so use it to lookup
  kvp.key = *keyindex;
...
gbool ServerGetBoolValue(GServer server, char *key, gbool bdefault)
{
  ...
  kv = ServerRuleLookup(server, key);
  if (!kv) return bdefault;

  // check the first char for known "false" values
  if ('0' == *kv->value || 'F' == *kv->value || 'f' == *kv->value || 'N' == *kv->value || 'n' == *kv->value) return 0;
...
{
  GKeyValuePair *kv = elem;
  GEnumData *ped = clientData;
...
int GStringHash(const void *elem, int numbuckets) { return StringHash(*(char **) elem, numbuckets); }

static int KeyValHashKeyP(const void *elem, int numbuckets) { return StringHash(((GKeyValuePair *) elem)->key, numbuckets); }
 ...
 */
int GCaseInsensitiveCompare(const void *entry1, const void *entry2) { return strcasecmp(*(char **) entry1, *(char **) entry2); }

/* keyval
 * Compares two gkeyvaluepair 
 */
static int KeyValCompareKeyP(const void *entry1, const void *entry2) { return ((GKeyValuePair *) entry1)->key - ((GKeyValuePair *) entry2)->key; }
