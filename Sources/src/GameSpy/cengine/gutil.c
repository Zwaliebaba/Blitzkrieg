...
crypt_encrypt(crypt_key *L, goa_uint32 *dest, int nodes)
{
  ...
  while (dest < dest_max)
  {
    ...
    *dest++ = h(x, y);
    ...
    --index;
    if (index < 0) index = 0;//crt - fix problem
    x = L->x_stack[index];
    y = L->y_stack[index];
    z = L->z_stack[index];
               ...
               unsigned int bytes_in_key, crypt_key *L)
{
  ...
  for (i = 0; i < CRYPT_TABLE_SIZE; i++) F[i] = 0;

  for (j = 0; j < 4; j++)
  {
    for (i = 0; i < CRYPT_TABLE_SIZE; i++) F[i] = F[i] * CRYPT_TABLE_SIZE + i;
    index = j;
  ...
  for (i = 0; i < CRYPT_TABLE_SIZE; i++) F[i] ^= i;
  crypt_seek(L, 0, 0);
}
