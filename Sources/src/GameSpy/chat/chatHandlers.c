...
static ciServerMessageFilter *ciFindGetKeyFilter(CHAT chat, const char *channel)
{
  ...
  for (filter = connection->filterList; filter != NULL; filter = filter->pnext)
  {
    if (filter->type == TYPE_GETKEY)
    {
      ...
      if (strcasecmp(data->channel, channel) == 0) return filter;
    }
  }
...
static void ciRemoveFilter(CHAT chat, ciServerMessageFilter *filter)
{
  ...
  for (pcurr = connection->filterList; pcurr != NULL; pcurr = pcurr->pnext)
  {
    if (pcurr == filter)
    {
      if (connection->filterList == pcurr) connection->filterList = pcurr->pnext;

      if (connection->lastFilter == pcurr) connection->lastFilter = pprev;
...
static void ciFinishFilter(CHAT chat, ciServerMessageFilter *filter, void *params)
{
  ...
  {
    LISTData *data = filter->data;

    if (filter->callback2) ciAddCallback_(chat, CALLBACK_ENUM_CHANNELS_ALL, filter->callback2, params, filter->param, filter->ID, NULL, sizeof(ciCallbackEnumChannelsAllParams));
  ...
  else if (filter->type == TYPE_JOIN) { if (filter->callback != NULL) ciAddCallback_(chat, CALLBACK_ENTER_CHANNEL, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackEnterChannelParams)); }
  else if (filter->type == TYPE_TOPIC)
  {
    const char *channel = ((ciCallbackGetChannelTopicParams *) params)->channel;

    if (filter->callback) ciAddCallback_(chat, CALLBACK_GET_CHANNEL_TOPIC, filter->callback, params, filter->param, filter->ID, channel, sizeof(ciCallbackGetChannelTopicParams));
  }
  else if (filter->type == TYPE_NAMES)
  {
    NAMESData *data = filter->data;

    if (filter->callback) ciAddCallback_(chat, CALLBACK_ENUM_USERS, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackEnumUsersParams));
  ...
  else if (filter->type == TYPE_WHOIS)
  {
    ...
    if (filter->callback) ciAddCallback_(chat, CALLBACK_GET_USER_INFO, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackGetUserInfoParams));
  ...
  else if (filter->type == TYPE_WHO) { if (filter->callback) ciAddCallback_(chat, CALLBACK_GET_BASIC_USER_INFO, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackGetBasicUserInfoParams)); }
  else if (filter->type == TYPE_CWHO) { if (filter->callback) ciAddCallback_(chat, CALLBACK_GET_CHANNEL_BASIC_USER_INFO, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackGetBasicUserInfoParams)); }
  else if (filter->type == TYPE_CMODE) { if (filter->callback) ciAddCallback_(chat, CALLBACK_GET_CHANNEL_MODE, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackGetChannelModeParams)); }
  else if (filter->type == TYPE_UMODE) { if (filter->callback) ciAddCallback_(chat, CALLBACK_GET_USER_MODE, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackGetUserModeParams)); }
  ...
  else if (filter->type == TYPE_GETBAN)
  {
    ...
    if (filter->callback) ciAddCallback_(chat, CALLBACK_ENUM_CHANNEL_BANS, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackEnumChannelBansParams));
  ...
  else if (filter->type == TYPE_NICK) { if (filter->callback) ciAddCallback_(chat, CALLBACK_CHANGE_NICK, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackChangeNickParams)); }
  else if (filter->type == TYPE_GETKEY)
  {
    GETKEYData *data = filter->data;

    if (filter->callback) ciAddCallback_(chat, CALLBACK_GET_GLOBAL_KEYS, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackGetGlobalKeysParams));
  ...
  {
    GETCKEYData *data = filter->data;

    if (filter->callback) ciAddCallback_(chat, CALLBACK_GET_CHANNEL_KEYS, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackGetChannelKeysParams));
  ...
  {
    GETCHANKEYData *data = filter->data;
  ...
  else if (filter->type == TYPE_UNQUIET)
  {
    ...
    if (filter->callback) ciAddCallback_(chat, CALLBACK_NEW_USER_LIST, filter->callback, params, filter->param, filter->ID, NULL, sizeof(ciCallbackNewUserListParams));
...
static void ciFilterTimedout(CHAT chat, ciServerMessageFilter *filter)
{
  ...
  else if (filter->type == TYPE_BAN) { ciFinishFilter(chat, filter, NULL); }
  else if (filter->type == TYPE_GETBAN)
  {
  ...
  else if (filter->type == TYPE_UNQUIET) { ciRemoveFilter(chat, filter); }
  else { assert(0); }
}
...
int ciGetNextID(CHAT chat)
{
  ...
  // Increment the ID.
  ////////////////////
  if (connection->nextID == INT_MAX) connection->nextID = 1;
...
CHATBool ciCheckFiltersForID(CHAT chat, int ID)
{
  ...
  for (filter = connection->filterList; filter != NULL; filter = filter->pnext)
  {
    ...
    ///////////////////////////
    if (filter->ID == ID) return CHATTrue;
  }
...
void ciCleanupFilters(CHAT chat)
{
  ...
  while (connection->filterList) ciFilterTimedout(chat, connection->filterList);// PANTS|03.13.01 - changed from ciRemoveFilter
}
...
static int ciAddFilter(CHAT chat, int type, const char *name, const char *name2, void *callback, void *callback2, void *param, void *data)
{
  ...
  if (filter == NULL) return 0;//ERRCON
  ...
  filter->param = param;
  filter->data = data;
  if (name) filter->name = goastrdup(name);
  else filter->name = NULL;
  if (name2) filter->name2 = goastrdup(name2);
  else filter->name2 = NULL;
  filter->ID = ciGetNextID(chat);

  // Add the filter to the end of the list.
  /////////////////////////////////////////
  if (connection->filterList == NULL) connection->filterList = filter;
  else connection->lastFilter->pnext = filter;
...
{
  LISTData *data = gsimalloc(sizeof(LISTData));
  if (data == NULL) return 0;//ERRCON
...
int ciAddTOPICFilter(CHAT chat, const char *channel, chatGetChannelTopicCallback callback, void *param) { return ciAddFilter(chat, TYPE_TOPIC, channel, NULL, callback, NULL, param, NULL); }

int ciAddNAMESFilter(CHAT chat, const char *channel, chatEnumUsersCallback callback, void *param)
{
  NAMESData *data = gsimalloc(sizeof(NAMESData));
  if (data == NULL) return 0;//ERRCON
...
{
  int rcode;
  WHOISData *data = gsimalloc(sizeof(WHOISData));
  if (data == NULL) return 0;//ERRCON
...
int ciAddWHOFilter(CHAT chat, const char *user, chatGetBasicUserInfoCallback callback, void *param) { return ciAddFilter(chat, TYPE_WHO, user, NULL, callback, NULL, param, NULL); }

int ciAddCWHOFilter(CHAT chat, const char *channel, chatGetChannelBasicUserInfoCallback callback, void *param) { return ciAddFilter(chat, TYPE_CWHO, channel, NULL, callback, NULL, param, NULL); }

int ciAddCMODEFilter(CHAT chat, const char *channel, chatGetChannelModeCallback callback, void *param) { return ciAddFilter(chat, TYPE_CMODE, channel, NULL, callback, NULL, param, NULL); }

int ciAddUMODEFilter(CHAT chat, const char *user, const char *channel, chatGetUserModeCallback callback, void *param) { return ciAddFilter(chat, TYPE_UMODE, user, channel, callback, NULL, param, NULL); }

int ciAddBANFilter(CHAT chat, const char *user, const char *channel)
{
  BANData *data = gsimalloc(sizeof(BANData));
  if (data == NULL) return 0;//ERRCON
...
{
  GETBANData *data = gsimalloc(sizeof(GETBANData));
  if (data == NULL) return 0;//ERRCON
...
int ciAddNICKFilter(CHAT chat, const char *oldNick, const char *newNick, chatChangeNickCallback callback, void *param) { return ciAddFilter(chat, TYPE_NICK, oldNick, newNick, callback, NULL, param, NULL); }
...
{
  int i;
  GETKEYData *data = gsimalloc(sizeof(GETKEYData));
  if (!data) return 0;
...
int ciAddGETCKEYFilter(CHAT chat, const char *cookie, int num, const char **keys, CHATBool channel, CHATBool getBroadcastKeys, chatGetChannelKeysCallback callback, void *param)
{
  int src;
  int dest;
  GETCKEYData *data = gsimalloc(sizeof(GETCKEYData));
  if (!data) return 0;
...
int ciAddGETCHANKEYFilter(CHAT chat, const char *cookie, int num, const char **keys, CHATBool getBroadcastKeys, chatGetChannelKeysCallback callback, void *param)
{
  int src;
  int dest;
  GETCHANKEYData *data = gsimalloc(sizeof(GETCHANKEYData));
  if (!data) return 0;
...
int ciAddUNQUIETFilter(CHAT chat, const char *channel)
{
  ...
  if (!callbacks || !callbacks->newUserList) return 0;
...
ciModeChange *ciParseMode(char *mode, char **params, int numParams)
{
  ...
  /////////////////////////////////
  if (*mode == '+') enable = CHATTrue;
  else if (*mode == '-') enable = CHATFalse;
  else return NULL;//ERRCON
  mode++;
...
void ciApplyChangesToMode(CHATChannelMode *mode, ciModeChange *changes)
{
  ...
  for (change = changes; change->mode != MODE_END; change++)
  {
    switch (change->mode)
    {
      ...
      case MODE_LIMIT:
        if (change->enable && change->param) mode->Limit = atoi(change->param);
        else mode->Limit = 0;
        break;
...
void ciNickError(CHAT chat, int type)
{
  ...
  else
  {
    ...
    // Call the callback.
    /////////////////////
    if (connection->connectCallback != NULL) connection->connectCallback(chat, CHATFalse, connection->connectParam);
  }
}
...
void ciPrivmsgHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (message->numParams != 2) return;//ERRCON
  ...
  if (strcasecmp(target, connection->nick) == 0)
  {
    if (connection->globalCallbacks.privateMessage != NULL)
    {
      ...
      params.user = from;
      params.message = msg;
      if (action) params.type = CHAT_ACTION;
      else params.type = CHAT_MESSAGE;
...
void ciNoticeHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (strcasecmp(target, connection->nick) == 0)
  {
    ...
    {
      ciCallbackPrivateMessageParams params;
      if (message->nick != NULL) params.user = message->nick;
...
void ciNickHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (message->numParams != 1) return;//ERRCON
...
void ciJoinHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (strcmp(nick, connection->nick) == 0)
  {
    ...
    /////////////////////////////////////////
    if (!ciIsEnteringChannel(chat, channel)) return;
  ...
  ///////////////////////////////
  if (ciInChannel(chat, channel)) ciUserEnteredChannel(chat, nick, channel, mode, user, address);
...
void ciKickHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if ((message->numParams != 2) && (message->numParams != 3)) return;//ERRCON

  channel = message->params[0];
  kicker = message->nick;
  kickee = message->params[1];
  if (message->numParams == 3) reason = message->params[2];
  else reason = "";
...
void ciModeHandler(CHAT chat, ciServerMessage *message)
{
  ...
  ////////////////////////////////////
  if (!ciInChannel(chat, channel)) return;

  // Parse the changes.
  /////////////////////
  changes = ciParseMode(mode, message->params + 2, message->numParams - 2);
  if (changes == NULL) return;//ERRCON
  ...
  for (change = changes; change->mode != MODE_END; change++)
  {
    switch (change->mode)
    {
      ...
      case MODE_KEY:
        if (change->enable) ciSetChannelPassword(chat, channel, change->param);
        else ciSetChannelPassword(chat, channel, NULL);
        break;
      ...
      case MODE_OP:
        if (change->param) ciUserChangedMode(chat, change->param, channel, CHAT_OP, change->enable);
        break;

      case MODE_VOICE:
        if (change->param) ciUserChangedMode(chat, change->param, channel, CHAT_VOICE, change->enable);
        break;
...
void ciErrorHandler(CHAT chat, ciServerMessage *message)
{
  ...
  else { ciHandleDisconnect(chat, message->params[0]); }
}
...
void ciNameReplyHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (message->numParams != 4) return;//ERRCON
  ...
  // Get the data.
  ////////////////
  if (filter->type != TYPE_JOIN) data = filter->data;
...
void ciEndOfNamesHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (message->numParams != 3) return;//ERRCON
  ...
  /////////////////////////////////////////////
  if (strcmp(channel, "*") == 0) channel = NULL;
...
void ciErrNickInUseHandler(CHAT chat, ciServerMessage *message)
{
  ...
  else
  {
    ...
    if (connection->connecting) ciNickError(chat, CHAT_IN_USE);
  }
}
...
void ciRplWhoReplyHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (message->numParams != 8) return;//ERRCON
  ...
  if (filter->type == TYPE_UMODE)
  {
    ...
    //////////////////
    mode = 0;
    if (strchr(message->params[6], '@')) mode |= CHAT_OP;
    if (strchr(message->params[6], '+')) mode |= CHAT_VOICE;
...
void ciRplGetKeyHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (filter)
  {
    ...
    ///////////////////////////////////////////
    if (!data->channel) { FINISH_FILTER; }
    else { ciAddCallback(chat, CALLBACK_GET_GLOBAL_KEYS, filter->callback, &params, filter->param, filter->ID, NULL); }
...
void ciRplGetCKeyHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (message->numParams != 5) return;//ERRCON
  ...
  if (filter)
  {
    ...
    ///////////////////////////////////////////
    if (!data->channel) { FINISH_FILTER; }
    else { ciAddCallback(chat, CALLBACK_GET_CHANNEL_KEYS, filter->callback, &params, filter->param, filter->ID, NULL); }
...
void ciRplWhoisUserHandler(CHAT chat, ciServerMessage *message)
{
  ...
  if (message->numParams != 6) return;//ERRCON
...
void ciRplWelcomeHandler(CHAT chat, ciServerMessage *message)
{
  ...
  // Call the callback.
  /////////////////////
  if (connection->connectCallback != NULL) connection->connectCallback(chat, CHATTrue, connection->connectParam);
}
...
void ciErrErroneusNicknameHandler(CHAT chat, ciServerMessage *message)
{
  ...
  // Are we connecting?
  /////////////////////
  if (connection->connecting) ciNickError(chat, CHAT_INVALID);
}