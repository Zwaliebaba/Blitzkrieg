...
static CHATBool ciCheckForID(CHAT chat, int ID) { return (ciCheckFiltersForID(chat, ID) || ciCheckCallbacksForID(chat, ID)); }
...
static void ciThink(CHAT chat, int ID)
{
  ...
  if (connection->chatSocket.connectState == ciConnected)
  {
    ...
    ///////////////////////////
    if (connection->chatSocket.connectState == ciDisconnected) { ciHandleDisconnect(chat, "Disconnected"); }
    else
    {
...
void ciSendNickAndUser(CHAT chat)
{
  ...
  if (strchr("0123456789", connection->nick[0])) invalidNick = CHATTrue;
  for (str = connection->nick; str[0]; str++) { if (!strchr(VALID_NICK_CHARS, str[0])) invalidNick = CHATTrue; }
  ...
  // Hacky, but it works.
  ////////////////////
  if (!invalidNick) sendingNick = connection->nick;
  else sendingNick = "1";
                     ...
                     CHATBool blocking)
{
  ...
  if (connection == NULL) return NULL;//ERRCON

  // Initialize the connection.
  /////////////////////////////
  memset(connection, 0, sizeof(ciConnection));
  strcpy(connection->nick, nick);
  if (user) strcpy(connection->user, user);
  ...
  else { ciSendNickAndUser(connection); }
...
void chatDisconnect(CHAT chat)
{
  ...
  ////////////////////////////////////////////////////////
  if (!connection->disconnected && connection->globalCallbacks.disconnected) connection->globalCallbacks.disconnected(chat, "", connection->globalCallbacks.param);
...
void chatThink(CHAT chat) { if (chat) ciThink(chat, 0); }
                    ...
                    CHATBool blocking)
{
  ...
  // No nick.
  ///////////
  if (!newNick || !newNick[0] || (strlen(newNick) >= MAX_NICK)) success = CHATFalse;

  // Check for same nick.
  ///////////////////////
  if (success && (strcasecmp(newNick, connection->nick) == 0)) success = CHATFalse;
                 ...
                 const char *oldNick)
{
  ...
  if (isdigit(*oldNick) || (*oldNick == '-')) *newNick++ = '_';

  while (c = *oldNick++)
  {
    if (!strchr(legalIRCChars, c)) c = '_';
                      ...
                      CHATBool quiet)
{
  ...
  /////////////////////////////////
  if (connection->quiet == quiet) return;

  // Send the message.
  ////////////////////
  if (quiet) ciSocketSendf(&connection->chatSocket, "MODE %s +q", connection->nick);
  else ciSocketSendf(&connection->chatSocket, "MODE %s -q", connection->nick);
                            ...
                            int type)
{
  ...
  if (type == CHAT_MESSAGE) ciSocketSendf(&connection->chatSocket, "PRIVMSG %s :%s", channel, message);
  else if (type == CHAT_ACTION) ciSocketSendf(&connection->chatSocket, "PRIVMSG %s :\001ACTION %s\001", channel, message);
  else if (type == CHAT_NOTICE) ciSocketSendf(&connection->chatSocket, "NOTICE %s :%s", channel, message);
  else if (type == CHAT_UTM) ciSocketSendf(&connection->chatSocket, "UTM %s :%s", channel, message);
  else if (type == CHAT_ATM) ciSocketSendf(&connection->chatSocket, "ATM %s :%s", channel, message);
  else return;
                        ...
                        CHATChannelMode *mode)
{
  ...
  if (mode->InviteOnly) buffer[0] = '+';
  else buffer[0] = '-';
  if (mode->Private) buffer[2] = '+';
  else buffer[2] = '-';
  if (mode->Secret) buffer[4] = '+';
  else buffer[4] = '-';
  if (mode->Moderated) buffer[6] = '+';
  else buffer[6] = '-';
  if (mode->NoExternalMessages) buffer[8] = '+';
  else buffer[8] = '-';
  if (mode->OnlyOpsChangeTopic) buffer[10] = '+';
  else buffer[10] = '-';
  if (mode->Limit > 0) buffer[12] = '+';
  else buffer[12] = '-';

  // Add limit if needed.
  ///////////////////////
  if (mode->Limit > 0) sprintf(&buffer[strlen(buffer)], " %d", mode->Limit);
                            ...
                            const char *password)
{
  ...
  if (enable) ciSocketSendf(&connection->chatSocket, "MODE %s +k %s", channel, password);
  else ciSocketSendf(&connection->chatSocket, "MODE %s -k %s", channel, password);
}
                            ...
                            CHATBool blocking)
{
  ...
  ///////////////////////////////////
  if (!ciInChannel(chat, channel)) return;//ERRCON
                           ...
                           const char *channel)
{
  ...
  if (!channel || !channel[0]) return -1;

  if (!ciInChannel(chat, channel)) return -1;
...
void chatEnumJoinedChannels(CHAT chat,
                            ...
                            void *param) { ciEnumJoinedChannels(chat, callback, param); }
                         ...
                         int type)
{
  ...
  if (type == CHAT_MESSAGE) ciSocketSendf(&connection->chatSocket, "PRIVMSG %s :%s", user, message);
  else if (type == CHAT_ACTION) ciSocketSendf(&connection->chatSocket, "PRIVMSG %s :\001ACTION %s\001", user, message);
  else if (type == CHAT_NOTICE) ciSocketSendf(&connection->chatSocket, "NOTICE %s :%s", user, message);
  else if (type == CHAT_UTM) ciSocketSendf(&connection->chatSocket, "UTM %s :%s", user, message);
  else if (type == CHAT_ATM) ciSocketSendf(&connection->chatSocket, "ATM %s :%s", user, message);
}
                                    ...
                                    const char **address)
{
  ...
  if (!connection) return CHATFalse;
  if (!connection->connected) return CHATFalse;
                         ...
                         const char **keys)
{
  ...
  for (i = 0; i < num; i++)
  {
    ...
    if ((len + keyLen + 1) >= sizeof(buffer)) return;
                       ...
                       CHATBool blocking)
{
  ...
  if (!target || !target[0]) target = connection->nick;
                        ...
                        const char **values)
{
  ...
  if (!user || !user[0]) sprintf(buffer, "SETCHANKEY %s :", channel);
  else sprintf(buffer, "SETCKEY %s %s :", channel, user);
  for (i = 0; i < num; i++)
  {
    value = values[i];
    if (!value) value = "";
                                    ...
                                    const char **keys)
{
  ...
  // Start off the buffer.
  ////////////////////////
  if (!nick || !nick[0]) sprintf(buffer, "GETCHANKEY %s %s 0 :", channel, cookie);
  else sprintf(buffer, "GETCKEY %s %s %s 0 :", channel, nick, cookie);
  len = strlen(buffer);
  ...
  for (i = 0; i < num; i++)
  {
    ...
    /////////////////
    keyLen = strlen(keys[i]);
    if ((len + keyLen + 1) >= sizeof(buffer)) continue;
                        ...
                        CHATBool blocking)
{
  ...
  if (!user || !user[0]) ID = ciAddGETCHANKEYFilter(chat, cookie, num, keys, getBroadcastKeys, callback, param);
  else ID = ciAddGETCKEYFilter(chat, cookie, num, keys, strcmp(user, "*") == 0, getBroadcastKeys, callback, param);
