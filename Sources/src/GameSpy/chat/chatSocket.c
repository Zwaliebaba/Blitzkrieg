...
const char *ciGetTime(void)
{
  ...
  timer = time(NULL);
  now = localtime(&timer);
  if (now->tm_year > 99) now->tm_year -= 100;
...
static CHATBool ciBufferInit(ciBuffer *buffer)
{
  ...
  if (buffer->buffer == NULL) return CHATFalse;
...
static CHATBool ciBufferPreAppend(ciBuffer *buffer, int len)
{
  ...
  if (total <= buffer->size) return CHATTrue;
  ...
  if (tempPtr == NULL) return CHATFalse;
...
CHATBool ciSocketInit(ciSocket *sock)
{
  ...
  if (!ciBufferInit(&sock->inputQueue)) return CHATFalse;
                         ...
                         int port)
{
  ...
  if (sock->sock == INVALID_SOCKET) return CHATFalse;
...
static void ciSocketSelect(SOCKET sock, CHATBool *readFlag, CHATBool *writeFlag, CHATBool *exceptFlag)
{
  ...
  if (rcode == SOCKET_ERROR) return;

  // Check results.
  /////////////////
  if (readFlag != NULL)
  {
    if (rcode > 0 && FD_ISSET(sock, readfds)) *readFlag = CHATTrue;
    else *readFlag = CHATFalse;
  }
  if (writeFlag != NULL)
  {
    if (rcode > 0 && FD_ISSET(sock, writefds)) *writeFlag = CHATTrue;
    else *writeFlag = CHATFalse;
  }
  if (exceptFlag != NULL)
  {
    if (rcode > 0 && FD_ISSET(sock, exceptfds)) *exceptFlag = CHATTrue;
    else *exceptFlag = CHATFalse;
  }
}
...
static void ciSocketThinkSend(ciSocket *sock)
{
  ...
  while (sock->outputQueue.length > 0)
  {
    ...
    if (rcode == 0) return;
    if (rcode == SOCKET_ERROR) return;//ERRCON
...
static void ciSocketThinkRecv(ciSocket *sock)
{
  ...
  while (CHATTrue)
  {
    ...
    /////////////////////////////////
    if (!ciBufferPreAppend(&sock->inputQueue, RECV_LEN)) return;//ERRCON
...
*/
...
#if 0
    {
      ...
      for (i = 0; i < len; i += 1023) OutputDebugString(&buffer[i]);
      OutputDebugString("<<<---XXXRECV\n");
    }
#endif
    ...
    ////////////////////////////////////////
    if (sock->secure) gs_crypt((unsigned char *) pos, len, &sock->inKey);
...
void ciSocketThink(ciSocket *sock)
{
  ...
  // Disconnected?
  ////////////////
  if (sock->connectState == ciDisconnected) return;
                      ...
                      const char *buffer)
{
  ...
  // Disconnected?
  ////////////////
  if (sock->connectState == ciDisconnected) return CHATTrue;
  ...
  //////////////////////////////////////
  if (!ciBufferPreAppend(&sock->outputQueue, len + 2)) return CHATFalse;
  ...
  ////////////////////////////////////////
  if (sock->secure) gs_crypt((unsigned char *) pos, len + 2, &sock->outKey);
                       ...
                       ...)
{
  ...
  if (num != -1) buffer[num] = '\0';
  else buffer[sizeof(buffer) - 1] = '\0';//ERRCON
...
static CHATBool ciAddParam(const char *param, ciServerMessage *message)
{
  ...
  if (tempPtr == NULL) return CHATFalse;//ERRCON
...
CHATBool ciParseParam(const char *pText, ciServerMessage *message)
{
  ...
  if (p == NULL) return CHATFalse;//ERRCON
  strcpy(p, pText);
...
static CHATBool ciParseMessage(ciSocket *sock, const char *sText)
{
  ...
  if (message->message == NULL) return CHATFalse;//ERRCON
...
ciServerMessage *ciSocketRecv(ciSocket *sock)
{
  ...
  if (sock->connectState == ciDisconnected) return NULL;

  // Check for an empty buffer.
  /////////////////////////////
  if (sock->inputQueue.length == 0) return NULL;
