 ...
 */
static void FreeElement(DArray array, int n) { if (array->elemfreefn != NULL) array->elemfreefn(ArrayNth(array, n)); }
 ...
 */
static void SetElement(DArray array, const void *elem, int pos) { memcpy(ArrayNth(array, pos), elem, array->elemsize); }
                ...
                ArrayElementFreeFn elemFreeFn)
{
  ...
  assert(array);
  assert(elemSize);
  if (numElemsToAllocate == 0) numElemsToAllocate = DEF_GROWBY;
  array->count = 0;
...
void ArrayFree(DArray array)
{
  ...
  assert(array);
  for (i = 0; i < array->count; i++) { FreeElement(array, i); }
  gsifree(array->list);
  gsifree(array);
}


int ArrayLength(const DArray array) { return array->count; }
 ...
 */
void ArrayAppend(DArray array, const void *newElem) { ArrayInsertAt(array, newElem, array->count); }
...
void ArraySort(DArray array, ArrayCompareFn comparator) { qsort(array->list, array->count, array->elemsize, comparator); }
                ...
                int fromIndex, int isSorted)
{
  void *res;
  int found = 1;
  if (!array || array->count == 0) return NOT_FOUND;
  ...
  if (res != NULL && found) return (((char *) res - (char *) array->list) / array->elemsize);
  return NOT_FOUND;
}
...
void ArrayMap(DArray array, ArrayMapFn fn, void *clientData)
{
  ...
  for (i = 0; i < array->count; i++) fn(ArrayNth(array, i), clientData);
...
void ArrayMapBackwards(DArray array, ArrayMapFn fn, void *clientData)
{
  ...
  for (i = (array->count - 1); i >= 0; i--) fn(ArrayNth(array, i), clientData);
...
void *ArrayMap2(DArray array, ArrayMapFn2 fn, void *clientData)
{
  ...
  for (i = 0; i < array->count; i++)
  {
    ...
    if (!fn(pcurr, clientData)) return pcurr;
  }
...
void ArrayClear(DArray array)
{
  ...
  //////////////////////////////
  for (i = (ArrayLength(array) - 1); i >= 0; i--) ArrayDeleteAt(array, i);
}
...
{
  int i;
  for (i = 0; i < count; i++) { if (comparator(key, (char *) base + size * i) == 0) return (char *) base + size * i; }
  return NULL;
}
