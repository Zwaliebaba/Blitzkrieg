...
HashTable TableNew(int elemSize, int nBuckets,
                   ...
                   TableElementFreeFn freeFn) { return TableNew2(elemSize, nBuckets, 4, hashFn, compFn, freeFn); }
...
void TableFree(HashTable table)
{
  ...
  for (i = 0; i < table->nbuckets; i++) ArrayFree(table->buckets[i]);
...
int TableCount(HashTable table)
{
  ...
  for (i = 0; i < table->nbuckets; i++) count += ArrayLength(table->buckets[i]);
...
void TableEnter(HashTable table, const void *newElem)
{
  ...
  if (itempos == NOT_FOUND) ArrayAppend(table->buckets[hash], newElem);
  else ArrayReplaceAt(table->buckets[hash], newElem, itempos);
}
...
void *TableLookup(HashTable table, const void *elemKey)
{
  ...
  itempos = ArraySearch(table->buckets[hash], elemKey, table->compfn, 0,
                        0);
  if (itempos == NOT_FOUND) return NULL;
...
void TableMap(HashTable table, TableMapFn fn, void *clientData)
{
  ...
  for (i = 0; i < table->nbuckets; i++) ArrayMap(table->buckets[i], fn, clientData);
...
void TableMapSafe(HashTable table, TableMapFn fn, void *clientData)
{
  ...
  for (i = 0; i < table->nbuckets; i++) ArrayMapBackwards(table->buckets[i], fn, clientData);
...
void TableClear(HashTable table)
{
  ...
  for (i = 0; i < table->nbuckets; i++) ArrayClear(table->buckets[i]);
}