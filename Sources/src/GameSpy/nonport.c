...
int isspace(int c) { return ((c >= 0x9 && c <= 0xD) || (c == 0x20)); }
...
void *fixed_realloc(void *ptr, int newsize)
{
  ...
  else if (ptr) return realloc(ptr, newsize);
  else//ptr is null
    ...
    )
{
  ...
  if (((long) (yr -= 1900) < _BASE_YEAR) || ((long) yr > _MAX_YEAR)) return (time_t) (-1);

  tmpdays = dy + _days[mo - 1];
  if (!(yr & 3) && (mo > 2)) tmpdays++;
...
unsigned long current_time()//returns current time in msec
{
  ...
  ticks = GetTicks();
  if (lastticks > ticks) msec += ((sizeof(unsigned int) - lastticks) + ticks) / 300000;
  else msec += (unsigned int) (ticks - lastticks) / 300000;
  lastticks = ticks;
...
#endif
#ifdef KGTRN_PLANET_WEB
  if ((argp = net_fcntl(sock, F_GETFL, 0)) < 0) return -1;
  if (isblocking) argp &= ~O_NONBLOCK;
  else argp |= O_NONBLOCK;
  rcode = net_fcntl(sock, F_SETFL, argp);
#endif
#ifdef KGTRN_NEXGEN
  if ((argp = fcntlsocket(sock, F_GETFL, 0)) < 0) return -1;
  if (isblocking) argp &= ~O_NONBLOCK;
  else argp |= O_NONBLOCK;
...
unsigned long fixed_inet_addr(const char *cp)
{
  ...
  {
    if ((*pos < '0' || *pos > '9') && *pos != '.') return INADDR_NONE;
    if (*pos == '.') pc++;//increment the period counter
  }
  if (pc != 3) return INADDR_NONE;//not enough periods

  sscanf(cp, "%d.%d.%d.%d", idata, idata + 1, idata + 2, idata + 3);
  data[0] = (unsigned char) idata[0];
  data[1] = (unsigned char) idata[1];
  data[2] = (unsigned char) idata[2];
  data[3] = (unsigned char) idata[3];
...
struct hostent *pwgethostbyname(const char *name)
{
  ...
  if (err != 0 || ai == NULL) { return NULL; }
  hent.h_name = ai->ai_canonname;
  hent.h_aliases = &aliases;
  hent.h_addrtype = 2;
  hent.h_length = 4;

  for (i = 0, curai = ai; i < MAX_HOST_ADDRS - 1, curai != NULL; i++, curai = curai->ai_next) paddrs[i] = &((struct sockaddr_in *) curai->ai_addr)->sin_addr;
  paddrs[i] = NULL;
...
const char *GOAGetUniqueID(void)
{
  ...
  size = 0;
  for (size = 0; size < sizeof(buff); size++) buff[size] = 0;
  size = sizeof(buff);
  FirmwareGetSerialNumber(buff, &size);
  for (size = 0; size < sizeof(buff); size++) { sprintf(keyval + (size * 2), "%02x", buff[size]); }
  return keyval;
}
...
static void GenerateID(char *keyval)
{
  ...
  LARGE_INTEGER l1;
  UINT seed;
  if (QueryPerformanceCounter(&l1)) seed = (l1.LowPart ^ l1.HighPart);
  else seed = 0;
...
#endif
  for (i = 0; i < 19; i++)
    if (i == 4 || i == 9 || i == 14) keyval[i] = '-';
    else keyval[i] = crypttab[Util_RandInt(0, 62)];
  keyval[19] = 0;
}
...
const char *GOAGetUniqueID(void)
{
  ...
  if (RegOpenKeyEx(HKEY_CURRENT_USER, REG_KEY, 0, KEY_ALL_ACCESS, &thekey) != ERROR_SUCCESS) docreate = 1;
  else docreate = 0;
  ...
  if (ret != 0 || strlen(keyval) != 19)//need to generate a new key
  {
    GenerateID(keyval);
#ifdef _WIN32
    if (docreate) { ret = RegCreateKeyEx(HKEY_CURRENT_USER, REG_KEY, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &thekey, &disp); }
    ...
    else keyval[0] = 0;//don't generate one each time!!
#endif
  }
#ifdef _WIN32
  RegCloseKey(thekey);
