...
#define PEER_STOP_REPORTING      1  // Stop reporting.  
...
(
    PEER peer,// The peer object.
    unsigned int IP,// The IP of the host, in network-byte order. 
...
(
    PEER peer,// The peer object.
    RoomType roomType,// The room the info has gotten in.
...
/* ************
 */
// This creates and initializes a peer object.
...
// Call peerRetryWithNick() to try another nick.  
...
// Returns PEERFalse if an error, or PEERTtrue if success.
// //////////////////////////////////////////////////////
...
(
    ...
    int engineMaxUpdates,// The maximum number of competitive updates
...
(
    PEER peer,// The peer object.
    RoomType roomType// Only TitleRoom is currently suggested.
    );
...
(
    PEER peer,// The peer object.
    const char *reason// The away reason.  
    );

// When using peerStartReportingWithSocket or peerCreateStagingRoomWithSocket,
// any qureries received on the sockets need to be passed to the SDK.  
...
(
    ...
    const char password[PEER_PASSWORD_LEN],// The password of the room being joined.  
    peerJoinRoomCallback callback,// Called when finished.
    void *param,// Passed to the callback.
    PEERBool blocking// If PEERTrue, don't return until finished.
    );
...
// or "", this will create a passworded room.  
...
// sending heartbeats and query replies.  
...
(
    ...
    const char *reason// The reason the player is leaving (can be NULL).  
    );
...
// So all games listed are still considered valid.  
...
(
    ...
    int index,// The index of the player (0 to (N - 1)).  
...
// Gets a player's flags in a room.  
...
// All the other people in the staging room will have them
...
// server reporting, and calling the program-supplied callbacks.  
