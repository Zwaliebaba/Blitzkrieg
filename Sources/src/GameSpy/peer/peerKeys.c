    ...
    )
{
  ...
  if (!connection->globalWatchCache) return PEERFalse;
  for (roomType = 0; roomType < NumRooms; roomType++)
  {
    ...
    if (!connection->globalWatchKeys[roomType] || !connection->roomWatchKeys[roomType] ||
        !connection->roomWatchCache[roomType]) { return PEERFalse; }
  }
    ...
    )
{
  ...
  if (connection->globalWatchCache) TableFree(connection->globalWatchCache);

  for (roomType = 0; roomType < NumRooms; roomType++)
  {
    if (connection->globalWatchKeys[roomType]) TableFree(connection->globalWatchKeys[roomType]);
    if (connection->roomWatchKeys[roomType]) TableFree(connection->roomWatchKeys[roomType]);
    if (connection->roomWatchCache[roomType]) TableFree(connection->roomWatchCache[roomType]);
  }
}
    ...
    )
{
  ...
  if (cacheKey->value) return cacheKey->value;
  return "";
}
    ...
    )
{
  piPlayer *player;
  RoomType roomType;
  piCacheKey *cacheKey = elem;
  PEER peer = clientData;
  piConnection *connection = peer;
  piWatchKey watchKeyTemp;

  watchKeyTemp.key = cacheKey->key;

  player = piGetPlayer(peer, cacheKey->nick);
  if (player) { for (roomType = 0; roomType < NumRooms; roomType++) { if (player->inRoom[roomType]) { if (TableLookup(connection->globalWatchKeys[roomType], &watchKeyTemp)) return; } } }
    ...
    )
{
  piPlayer *player;
  piCacheKey *cacheKey = elem;
  piCleanseRoomCacheMapData *data = clientData;
  piConnection *connection = data->peer;
  piWatchKey watchKeyTemp;

  watchKeyTemp.key = cacheKey->key;

  player = piGetPlayer(data->peer, cacheKey->nick);
  if (player && player->inRoom[data->roomType]) { if (TableLookup(connection->roomWatchKeys[data->roomType], &watchKeyTemp)) return; }
    ...
    )
{
  ...
  for (roomType = 0; roomType < NumRooms; roomType++)
  {
    ...
    else { TableClear(connection->roomWatchCache[roomType]); }
  }
}
    ...
    )
{
  ...
  if (success && user)
  {
    ...
    for (i = 0; i < num; i++) piGlobalKeyChanged(peer, user, keys[i], values[i]);
  }
}
    ...
    )
{
  ...
  if (!user && success)
  {
    ...
    for (i = 0; i < num; i++) piRoomKeyChanged(peer, roomType, szEndName, keys[i], NULL);
  ...
  if (success /*&& user*/)
  {
    ...
    for (i = 0; i < num; i++) piRoomKeyChanged(peer, roomType, user, keys[i], values[i]);
  }
}
    ...
    )
{
  ...
  piWatchKey *key = elem;
  piRemoveExistingKeysData *data = clientData;

  for (i = 0; i < data->num; i++)
    if (strcasecmp(key->key, data->keys[i]) == 0) return;
    ...
    )
{
  ...
  // Update them all.
  ///////////////////
  if (ENTERING_ROOM || IN_ROOM) chatGetGlobalKeys(connection->chat, ROOM, num, keys, piGetGlobalKeysCallback, peer, CHATFalse);
}
    ...
    )
{
  ...
  // Update them all.
  ///////////////////
  if (ENTERING_ROOM || IN_ROOM) chatGetChannelKeys(connection->chat, ROOM, "*", num, keys, piGetRoomKeysCallback, peer, CHATFalse);
}
    ...
    )
{
  ...
  /////////////////////////////////////
  if (!nick || !nick[0]) return PEERTrue;
  ...
  if (strcasecmp(key, "username") == 0)
  {
    ...
    if (inRoom)
    {
      if (player && player->gotIPAndProfileID) piAddPlayerInfoCallback(peer, roomType, nick, player->IP, player->profileID);
      else piAddPlayerInfoCallback(peer, roomType, nick, 0, 0);
    }
  }

  // Is this flags?
  /////////////////
  if (inRoom && strcasecmp(key, "b_flags") == 0) piSetPlayerRoomFlags(peer, nick, roomType, value);
  ...
  if (!TableLookup(watchKeys, &watchKeyTemp))
  {
    ...
    /////////////////////////
    if (inRoom && (strncmp(key, "b_", 2) == 0)) return PEERTrue;
    return PEERFalse;
  }
    ...
    )
{
  ...
  if (piKeyChanged(peer, nick, key, value, connection->roomWatchKeys[roomType], connection->roomWatchCache[roomType], PEERTrue, roomType)) piAddRoomKeyChangedCallback(peer, roomType, nick, key, value);
}
...
{
  piCacheKey *key = elem;
  piPlayerChangedNickMapData *data = clientData;
...
{
  piWatchKey *key = elem;
  piSetupKeysMapData *data = clientData;
    ...
    )
{
  ...
  if (!IN_ROOM && !ENTERING_ROOM) return;

  // Get the IP if we're pinging this room.
  /////////////////////////////////////////
  if (!nick) getIP = connection->pingRoom[roomType];
  ...
  if (!nick)
  {
    ...
    if (num)
    {
      ...
      if (getIP) data.keys[data.next++] = "username";
      if (getFlags) data.keys[data.next++] = "b_flags";
      assert(data.next == num);
