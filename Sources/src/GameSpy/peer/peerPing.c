    ...
    )
{
  ...
  ///////////////////////////
  if (player->pingHistoryNum > 0) memmove(player->pingHistory + 1, player->pingHistory, min(player->pingHistoryNum, PI_PING_HISTORY_LEN - 1) * sizeof(int));
  player->pingHistory[0] = ping;
  if (player->pingHistoryNum < PI_PING_HISTORY_LEN) player->pingHistoryNum++;

  // Recompute the ping average.
  //////////////////////////////
  total = 0;
  for (i = 0; i < player->pingHistoryNum; i++) total += player->pingHistory[i];
    ...
    )
{
  ...
  if (!connection->xpings) return PEERFalse;

  // Init pinger.
  ///////////////
  if (!pingerInit(NULL, PI_PINGER_PORT, piPinged, peer, NULL, NULL)) return PEERFalse;
    ...
    )
{
  ...
  ///////////////////////////////////////////////
  if (connection->stayInTitleRoom) return;
  ...
  ////////////////////////
  if (connection->xpings) TableFree(connection->xpings);
...
{
  piPlayer *player = elem;
  piPingerReplyData *data = clientdata;
  ...
  ///////////////////////////////
  if (!player->waitingForPing) return;

  // Check the IP.
  ////////////////
  if (!player->gotIPAndProfileID || (player->IP != data->IP)) return;
...
{
  piPlayer *player = elem;
  piPickPingPlayersData *data = clientData;
  PEER peer = data->peer;
  piPlayer *other;
  unsigned long now;
  unsigned long delay;
  ...
  // Not pinging?
  ///////////////
  if (!player->inPingRoom) return;

  // Don't have IP?
  /////////////////
  if (!player->gotIPAndProfileID) return;
  ...
  if (!player->mustPing)
  {
    ...
    //////////////////////////////
    if ((player->pingsLostConsecutive >= 4) && ((now - player->lastPingSend) < 120000)) return;

    // Pinged recently?
    ///////////////////
    if (player->inRoom[StagingRoom]) delay = 2000;
    else if (player->pingsReturned < 3) delay = 5000;
    else delay = 30000;
    if ((now - player->lastPingSend) < delay) return;
    if ((now - player->lastPingRecv) < (delay + 1500)) return;
  }
  ...
  ////////////////////////////
  if (i == data->max) return;

  // Bump down the other player and all below.
  ////////////////////////////////////////////
  for (j = (data->max - 1); j > i; j--) data->players[j] = data->players[j - 1];
    ...
    )
{
  ...
  for (roomType = 0; roomType != NumRooms; roomType++)
  {
    if (player->inRoom[roomType] && connection->xpingRoom[roomType])
    {
      ...
      if (IN_ROOM || ENTERING_ROOM) piSendChannelUTM(peer, ROOM, PI_UTM_XPING, message, PEERFalse);
    }
  }
...
{
  piPlayer *player = elem;
  piPickXpingPlayerData *data = clientData;
  PEER peer = data->peer;
  unsigned long now;
  ...
  ///////////////////////////////////
  if (!player->numPings) return;

  // Don't xping with the same info.
  //////////////////////////////////
  if (player->xpingSent) return;

  // Don't xping too often.
  /////////////////////////
  now = current_time();
  if ((now - player->lastXping) < PI_XPING_PLAYER_INTERVAL) return;

  // First player?
  ////////////////
  if (!data->player) { data->player = player; }
  else if ((now - player->lastXping) > (now - data->player->lastXping)) { data->player = player; }
}
    ...
    )
{
  ...
  // Check for no players.
  ////////////////////////
  if (!connection->players || !TableCount(connection->players)) return NULL;
    ...
    )
{
  ...
  if (connection->lastPingTimeMod) numPings = (pingTimeMod - connection->lastPingTimeMod);
  else numPings = 1;
  if (numPings) connection->lastPingTimeMod = pingTimeMod;

  // Send pings.
  //////////////
  players = piPickPingPlayers(peer, &numPings);
  if (players) { for (i = 0; (i < numPings) && players[i]; i++) piPingPlayer(peer, players[i]); }
  ...
  if ((now - connection->lastXpingSend) > PI_XPING_INTERVAL)
  {
    ...
    if (player) piXpingPlayer(peer, player);
  }
    ...
    )
{
  ...
  // Check if we're not doing pings.
  //////////////////////////////////
  if (!connection->doPings) return PEERTrue;
  ...
  for (i = 0; i < PI_PING_HISTORY_LEN; i++) player->pingHistory[i] = 0;
    ...
    )
{
  ...
  // Is this a ping room?
  ///////////////////////
  if (connection->pingRoom[roomType]) player->inPingRoom = PEERTrue;

  // Is this an xping room?
  /////////////////////////
  if (connection->xpingRoom[roomType]) player->inXpingRoom = PEERTrue;

  // Immediately ping players when they join a staging room.
  //////////////////////////////////////////////////////////
  if (roomType == StagingRoom) player->mustPing = PEERTrue;
}
...
{
  piXping *xping = elem;
  piPingPlayerLeftRoomData *data = clientdata;
  assert(xping);
  assert(data);

  // Check if this player is part of this xping.
  //////////////////////////////////////////////
  if ((strcmp(xping->nicks[0], data->nick) == 0) || (strcmp(xping->nicks[1], data->nick) == 0)) piRemoveXping(data->peer, xping);
}
    ...
    )
{
  ...
  if (player->inPingRoom)
  {
    ...
    //////////////////////////////
    for (i = 0; i < NumRooms; i++) { if (player->inRoom[i] && connection->pingRoom[i]) inPingRoom = PEERTrue; }
  ...
  if (player->inXpingRoom)
  {
    ...
    // Is he still in a ping room?
    //////////////////////////////
    for (i = 0; i < NumRooms; i++) { if (player->inRoom[i] && connection->xpingRoom[i]) inXpingRoom = PEERTrue; }
    ...
    )
{
  ...
  if (!player1) return;
  if (!player1->inXpingRoom) return;
  player2 = piGetPlayer(peer, nick2);
  if (!player2) return;
  if (!player2->inXpingRoom) return;
    ...
    )
{
  ...
  // Check if we're not doing pings.
  //////////////////////////////////
  if (!connection->doPings) return PEERFalse;

  // Get the xping.
  /////////////////
  xping = piFindXping(peer, nick1, nick2);
  assert(xping);
  if (!xping) return PEERFalse;
