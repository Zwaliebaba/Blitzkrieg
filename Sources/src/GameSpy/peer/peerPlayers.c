    ...
    )
{
  ...
  if (connection->stayInTitleRoom) return PEERTrue;
  ...
  if (!connection->players) return PEERFalse;

  // No players in any room.
  //////////////////////////
  for (i = 0; i < NumRooms; i++) connection->numPlayers[i] = 0;
    ...
    )
{
  ...
  /////////////////////////
  if (connection->players) TableFree(connection->players);
    ...
    )
{
  ...
  if (initialize)
  {
    ...
    if (!piPingInitPlayer(peer, player)) return NULL;
  }
    ...
    )
{
  ...
  //////////////////////////////////////////
  if (!ENTERING_ROOM && !IN_ROOM) return NULL;
  ...
  player->flags[roomType] = 0;
  if (mode & CHAT_OP) player->flags[roomType] |= PEER_FLAG_OP;
  if (mode & CHAT_VOICE) player->flags[roomType] |= PEER_FLAG_VOICE;
...
{
  piPlayer *player = elem;
  piLeftRoomData *data = clientdata;
  assert(player);
  assert(player->nick);
  assert(data);
    ...
    )
{
  ...
  // Check for no table.
  //////////////////////
  if (!connection->players) return NULL;
...
{
  piPlayer *player = elem;
  piEnumRoomPlayersData *data = clientdata;
    ...
    )
{
  ...
  // Is this the same IP?
  ///////////////////////
  if (player->gotIPAndProfileID && (player->IP == IP)) return 0;
    ...
    )
{
  ...
  return TableMap2(connection->players, piFindPlayersByIPMap, &IP);
}
    ...
    )
{
  ...
  if (player)
  {
    ...
    if (!player->gotIPAndProfileID)
    {
      ...
      ////////////////////////////////////////
      if (player->inRoom[TitleRoom]) piPingPlayerJoinedRoom(peer, player, TitleRoom);
      if (player->inRoom[GroupRoom]) piPingPlayerJoinedRoom(peer, player, GroupRoom);
      if (player->inRoom[StagingRoom]) piPingPlayerJoinedRoom(peer, player, StagingRoom);
    }
  }
}
    ...
    )
{
  ...
  ///////////////////
  player = piGetPlayer(peer, nick);
  if (!player || !player->inRoom[roomType]) return;
  ...
  /////////////////////////////////////////////////////////
  if ((roomType == StagingRoom) && ((oldFlags & PEER_FLAG_READY) != (flags & PEER_FLAG_READY))) piAddReadyChangedCallback(peer, player->nick, player->flags[roomType] & PEER_FLAG_READY);
    ...
    )
{
  ...
  if (strchr(flags, 's')) nFlags |= PEER_FLAG_STAGING;
  if (strchr(flags, 'r')) nFlags |= PEER_FLAG_READY;
  if (strchr(flags, 'g')) nFlags |= PEER_FLAG_PLAYING;
  if (strchr(flags, 'a')) nFlags |= PEER_FLAG_AWAY;
    ...
    )
{
  ...
  // Add the new flags.
  /////////////////////
  if (mode & CHAT_OP) nFlags |= PEER_FLAG_OP;
  if (mode & CHAT_VOICE) nFlags |= PEER_FLAG_VOICE;
    ...
    )
{
  ...
  if (!connection->inRoom[roomType]) return PEERFalse;
