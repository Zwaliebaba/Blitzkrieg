...
static uint16 piGetNextID(void)
{
  ...
  // Increment the ID.
  ////////////////////
  if (piNextID == USHRT_MAX) piNextID = 1;
  else piNextID++;
...
static void piSocketSelect(PINGERBool *readFlag, PINGERBool *writeFlag, PINGERBool *exceptFlag)
{
  ...
  if (readFlag != NULL)
  {
    if (rcode > 0 && FD_ISSET(piSocket, readfds)) *readFlag = PINGERTrue;
    else *readFlag = PINGERFalse;
  }
  if (writeFlag != NULL)
  {
    if (rcode > 0 && FD_ISSET(piSocket, writefds)) *writeFlag = PINGERTrue;
    else *writeFlag = PINGERFalse;
  }
  if (exceptFlag != NULL)
  {
    if (rcode > 0 && FD_ISSET(piSocket, exceptfds)) *exceptFlag = PINGERTrue;
    else *exceptFlag = PINGERFalse;
  }
}
...
static PINGERBool piBytesToPing(unsigned char *buffer, piUDPPing *udpPing, char *data)
{
  ...
  if (udpPing->magic != PI_MAGIC) return PINGERFalse;

  // Check the version.
  /////////////////////
  assert(udpPing->version == PI_VERSION);
  if (udpPing->version != PI_VERSION) return PINGERFalse;

  // Check the trip.
  //////////////////
  assert((udpPing->trip >= 1) && (udpPing->trip <= 3));
  if ((udpPing->trip < 1) || (udpPing->trip > 3)) return PINGERFalse;
...
static PINGERBool piSendPing(SOCKADDR_IN *to, uint16 trip, uint16 ID_A, uint16 ID_B, const char *data)
{
  ...
  if (data != NULL) memcpy(buffer + sizeof(piUDPPing), data, PI_DATA_MAX_LEN);
  else memset(buffer + sizeof(piUDPPing), 0, PI_DATA_MAX_LEN);

  // Send the outgoing ping.
  //////////////////////////
  rcode = sendto(piSocket, buffer, PINGER_UDP_PING_SIZE, 0, (SOCKADDR *) to, sizeof(SOCKADDR_IN));
                      ...
                      void *setDataParam)
{
  ...
  /////////////////////////////////////
  if (piInitialized) return PINGERFalse;

  // An empty localAddress is them same as a NULL.
  ////////////////////////////////////////////////
  if ((localAddress != NULL) && (localAddress[0] == '\0')) localAddress = NULL;
  ...
  if (piActivePingList == NULL) return PINGERFalse;
...
static void piProcessTrip1(piUDPPing *udpPing, const char *data, SOCKADDR_IN *from, unsigned long recvTime)
{
  ...
  if (udpPing->ID_A == 0) return;
  assert(udpPing->ID_B == 0);
  if (udpPing->ID_B != 0) return;
...
static void piProcessPing(piUDPPing *udpPing, const char *data, SOCKADDR_IN *from, unsigned long recvTime)
{
  ...
  /////////////////////////////
  if (udpPing->trip == 1) piProcessTrip1(udpPing, data, from, recvTime);
  else if (udpPing->trip == 2) piProcessTrip2(udpPing, data, from, recvTime);
  else if (udpPing->trip == 3) piProcessTrip3(udpPing, data, from, recvTime);
}
...
static void piProcessIncoming(void)
{
  ...
  do
  {
    ...
    if (readFlag)
    {
      ...
      rcode = recvfrom(piSocket, buffer, PINGER_UDP_PING_SIZE, 0, (SOCKADDR *) &from, &len);
                ...
                unsigned long timeout)
{
  ...
  if (piSendPing(&to, 1, ID, 0, NULL))
  {
    ...
    activePing.timestamp = current_time();
    if (timeout == 0) activePing.timeout = 0;
    else activePing.timeout = (activePing.timestamp + timeout);
