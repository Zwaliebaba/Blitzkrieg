            ...
            void *userdata)
{
...
#endif
  if (INVALID_SOCKET == querysock || INVALID_SOCKET == hbsock) { return E_GOA_WSOCKERROR; }
  ...
  if (*pnBaseport == 0)//we bound it dynamically
  {
    ...
    if (lasterror) return E_GOA_BINDERROR;
...
void qr_check_queries(qr_t qrec)
{
  ...
  if (qrec->qport == 0) return;//no sockets to work with!
...
void qr_check_send_heartbeat(qr_t qrec)
{
  ...
  if (INVALID_SOCKET == qrec->hbsock) return;//no sockets to work with!

  //check if we need to send a heartbet
  if (tc - qrec->lastheartbeat > HB_TIME || qrec->lastheartbeat == 0 || tc < qrec->lastheartbeat) send_heartbeat(qrec, 0);
  else if (qrec->no_query > 0 && tc - qrec->lastheartbeat > FIRST_HB_TIME)
  {
    ...
    qrec->no_query++;
    if (qrec->no_query > MAX_FIRST_COUNT) qrec->no_query = 0;//stop trying to get first query
  }
}
...
void qr_shutdown(qr_t qrec)
{
  ...
  if (INVALID_SOCKET != qrec->hbsock && qrec->hbsock != qrec->querysock) { closesocket(qrec->hbsock); }
...
static int do_connect(SOCKET sock, char *addr, int port, struct sockaddr_in *hbaddr)
{
  ...
  if (connect(sock, (struct sockaddr *) &saddr, sizeof(saddr)) == SOCKET_ERROR) { return E_GOA_CONNERROR; }
#else
...
static int get_sockaddrin(const char *host, int port, struct sockaddr_in *saddr, struct hostent **savehent)
{
  ...
  if (host == NULL) saddr->sin_addr.s_addr = INADDR_ANY;
  else saddr->sin_addr.s_addr = inet_addr(host);
...
static void packet_send(qr_t qrec, struct sockaddr *addr, char *buffer)
{
  ...
  if (strlen(buffer) == 0) return;//dont need to send an empty one!
...
static void buffer_send(qr_t qrec, struct sockaddr *sender, char *buffer, char *newdata)
{
  ...
  if (strlen(buffer) + strlen(newdata) < MAX_DATA_SIZE - 50) { strcat(buffer, newdata); }
  else
  {
    if (strlen(newdata) > MAX_DATA_SIZE - 50)//incoming data is too big already!
    {
      ...
      while (pos - newdata < MAX_DATA_SIZE - 50)
      {
        if ('\\' == *pos)
        {
          if (bcount % 2 == 0) lastkey = pos;
          bcount++;
        }
        pos++;
      }
      if (lastkey == newdata) return;//endless loop - single key was too big!
      *lastkey = '\0';
...
static void send_echo(qr_t qrec, struct sockaddr *sender, char *outbuf, char *echostr)
{
  ...
  if (strlen(echostr) > MAX_DATA_SIZE - 50) return;
...
static void send_final(qr_t qrec, struct sockaddr *sender, char *outbuf, char *validation)
{
  ...
  if (validation[0])
  {
    ...
    gs_encrypt(qrec->secret_key, strlen(qrec->secret_key), encrypted_val, keylen);
...
void qr_parse_query(qr_t qrec, char *query, int len, struct sockaddr *sender)
{
  ...
  {
    if (qrec->cdkeyprocess != NULL) qrec->cdkeyprocess(query, len, sender);
    return;
  }

  qrec->queryid++;
  qrec->packetnumber = 0;
  if (qrec->no_query > 0) qrec->no_query = 0;
...
static void send_heartbeat(qr_t qrec, int statechanged)
{
  ...
  if (statechanged) { sprintf(buf + strlen(buf), "\\statechanged\\%d", statechanged); }
  ...
  if (ret < 0)/* try to reconnect (tcp only) */
  {
    ...
    if (do_connect(qrec->hbsock, qr_hostname, MASTER_PORT) == 0) send(qrec->hbsock, buf, strlen(buf), 0);/* try again */
  }
#else
