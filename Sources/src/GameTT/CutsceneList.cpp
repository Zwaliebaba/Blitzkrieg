...
void CCutsceneList::PostCreate(IMainLoop *pML, CInterfaceCutsceneList *pILM) { pML->PushInterface(pILM); }
...
void CInterfaceCutsceneList::StartInterface()
{
  ...
  for (std::list<std::string>::const_iterator it = cutscenes.begin(); it != cutscenes.end(); ++it, ++nSceneIndex)
  {
    ...
    if (CPtr<IText> pText = pTextMan->GetDialog(szVideoName.c_str())) pContainer->SetWindowText(0, pText->GetString());
    else
    {
      const int nPos = szVideoName.rfind('\\');
      if (nPos != std::string::npos) szVideoName = szVideoName.substr(nPos + 1);
  ...
  if (nSelItem >= 0) pList->SetSelectionItem(nSelItem);
