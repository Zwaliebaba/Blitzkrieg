...
#ifdef _STREAMIO_DLL
ISaveLoadSystem * GetSLS_Hook();
ISingleton * GetSingletonGlobal_Hook();
...
public:
  CGlobalsLoader()
  {
    ...
    if (GETSLS_HOOK pfnGetSLS_Hook = pStreamIO->GetProcAddress("GetSLS_Hook", static_cast<GETSLS_HOOK>(nullptr))) g_pGlobalSaveLoadSystem = (*pfnGetSLS_Hook)();
    // hook GetSingletonGlobal() function
    if (GETSINGLETONGLOBAL_HOOK pfnGetSingletonGlobal_Hook = pStreamIO->GetProcAddress("GetSingletonGlobal_Hook", static_cast<GETSINGLETONGLOBAL_HOOK>(nullptr))) g_pGlobalSingleton = (*pfnGetSingletonGlobal_Hook)();
    // hook GetTempRawBuffer() function
    g_pfnGlobalGetTempRawBuffer = pStreamIO->GetProcAddress("GetTempRawBuffer_Hook", static_cast<GETTEMPRAWBUFFER_HOOK>(nullptr));
