...
bool CInterfaceMultiplayerScreen::StepLocal(bool bAppActive)
{
  ...
  {
    SToUICommand cmd;
    if (pCommandManager->PeekCommandToUI(&cmd) || pCommandManager->PeekChatMessageToUI()) { GetSingleton<IMainLoop>()->Command(MAIN_COMMAND_CMD, NStr::Format("%d", WCC_MULTIPLAYER_TO_UI_UPDATE)); }
  }
