...
void CInterfaceMessageBox::Create(const std::string &szCaptionKey, const std::string &szMessageKey, const bool _bDoubleButton, const std::string &_szGlobalVarOnOk)
{
  ...
  if (!szGlobalVarOnOk.empty()) RemoveGlobalVar(szGlobalVarOnOk.c_str());
  bDoubleButton = _bDoubleButton;// ok == ok if true. 

  const bool bInMission = GetGlobalVar("AreWeInMission", 0);

  if (bInMission) pUIScreen->Load("ui\\Popup\\MessageBoxMission");
  else pUIScreen->Load("ui\\Popup\\MessageBox");
  ...
  if (pCaption && pCaptionText) pCaption->SetWindowText(0, pCaptionText->GetString());

  IText *pMessageText = pTM->GetDialog(szMessageKey.c_str());
  IUIElement *pMessage = pUIScreen->GetChildByID(E_MESSAGE);
  if (pMessage && pMessageText) pMessage->SetWindowText(0, pMessageText->GetString());
...
bool CInterfaceMessageBox::ProcessMessage(const SGameMessage &msg)
{
  ...
  {
    case IMC_OK:
      if (bDoubleButton && !szGlobalVarOnOk.empty()) SetGlobalVar(szGlobalVarOnOk.c_str(), 1);
