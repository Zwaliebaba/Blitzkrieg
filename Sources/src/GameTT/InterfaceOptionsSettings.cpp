...
{
  if (!GetGlobalVar("AreWeInMission", 0)) OpenCurtainsForced();
  return true;
}
...
void CInterfaceOptionsSettings::SuspendAILogic(bool bSuspend)
{
  ...
  else { CInterfaceInterMission::SuspendAILogic(bSuspend); }
}
...
bool CInterfaceOptionsSettings::Init()
{
  ...
  if (GetGlobalVar("AreWeInMission", 0) && GetGlobalVar("MultiplayerGame", 0) == 1) SuspendAILogic(false);
  return true;
}

void CInterfaceOptionsSettings::Done() { CInterfaceInterMission::Done(); }

void CInterfaceOptionsSettings::Create()
{
  pUIScreen = CreateObject<IUIScreen>(UI_SCREEN);
  const bool bInMission = GetGlobalVar("AreWeInMission", 0);
  if (bInMission) pUIScreen->Load("ui\\MissionOptionsSettings");
  else pUIScreen->Load("ui\\OptionsSettings");
  ...
  const int nOptionFlag = GetGlobalVar("MultiplayerGame", 0) == 1 ? OPTION_FLAG_IN_MP_MISSION : (bInMission ? OPTION_FLAG_IN_MISSION : OPTION_FLAG_MAIN_OPTIONS);
  ...
  for (std::hash_map<std::string, OptionDescs>::iterator it = sections.begin(); it != sections.end(); ++it)
  {
    ...
    auto pList = checked_cast<IUIListControl *>(pUIScreen->GetChildByID(_E_LIST_BEGIN + nMaxDivision));
    auto pCaption = checked_cast<IUIStatic *>(pList->GetChildByID(10));

    optionsLists.push_back(new COptionsListWrapper(pList, it->second, 100));
    auto pButton = checked_cast<IUIButton *>(pUIScreen->GetChildByID(_E_BUTTON_CHANGE_DIVISION_BEGIN + nMaxDivision));

    pButton->SetWindowText(-1, pT->GetString());
    pCaption->SetWindowText(-1, pT->GetString());

    pT = pTM->GetString(szKeyTooltip.c_str());
    if (pT) pButton->SetHelpContext(0, pT->GetString());
...
void CInterfaceOptionsSettings::Close()
{
  ...
  else FinishInterface(MISSION_COMMAND_MAIN_MENU, NStr::Format("%d", CInterfaceMainMenu::E_OPTIONS));
}
...
bool CInterfaceOptionsSettings::ProcessMessage(const SGameMessage &msg)
{
  ...
  // changing division
  if (msg.nEventID >= _E_BUTTON_CHANGE_DIVISION_BEGIN && msg.nEventID < nMaxDivision + _E_BUTTON_CHANGE_DIVISION_BEGIN) { OnChangeDivision(msg.nEventID - _E_BUTTON_CHANGE_DIVISION_BEGIN); }
  //

  // process message to options.
  if (nActive >= 0 &&
      optionsLists.size() > nActive &&
      optionsLists[nActive]->ProcessMessage(msg)) { return true; }
  ...
  switch (msg.nEventID)
  {
    case IMC_OK:
    {
      for (int i = 0; i < optionsLists.size(); ++i) optionsLists[i]->Apply();
      // save options
    ...
    case IMC_CANCEL:
      for (int i = 0; i < optionsLists.size(); ++i) optionsLists[i]->CancelChanges();
      Close();

      return true;
    case E_BUTTON_DEFAULT:
      for (int i = 0; i < optionsLists.size(); ++i) optionsLists[i]->ToDefault();
