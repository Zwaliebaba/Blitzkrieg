...
void ResetCampaignStatus()
{
  ...
  if (IDataStorage *pDataStorage = GetSingleton<IDataStorage>()) { pGV->DumpVars(NStr::Format("%s%s", pDataStorage->GetName(), "logs\\vars.txt")); }
...
void CICMainMenu::Configure(const char *pszConfig)
{
  ...
  if (szStrings.size() > 1)
  {
    ...
    if (!szNextICConfig.empty() && szNextICConfig[szNextICConfig.size() - 1] == ';') szNextICConfig.resize(szNextICConfig.size() - 1);
  }
  else nNextIC = -1;
}
...
{
  if (GetGlobalVar("demoversion", 0)) { nState = CInterfaceMainMenu::E_DEMOVERSION_MAIN_MENU; }
  pIMM->Create(nState);
  pML->PushInterface(pIMM);
  if (nNextIC != -1) pML->Command(nNextIC, szNextICConfig.c_str());
}
...
void GetFileVersionString(const std::string &szFileName, std::string *pVersion)
{
  ...
  if (GetFileVersion(szFileName, &version) != false) *pVersion = NStr::Format("%d.%d", (version.dwProductVersionMS >> 16) & 0xffff, version.dwProductVersionMS & 0xffff);
...
void CInterfaceMainMenu::Create(int nState)
{
  ...
  else if (!szVersion.empty())
  {
    if (IUIElement *pElement = pUIScreen->GetChildByID(666))
    {
      ...
      if (CPtr<IText> pText = GetSingleton<ITextManager>()->GetDialog("textes\\strings\\version")) wszVersion = pText->GetString();
      else wszVersion = L"Version:";
      //
