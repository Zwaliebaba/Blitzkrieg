...
bool CMessageAtomReactionSetWindowText::Execute()
{
  ...
  if (pText) { GetSingleton<IMessageLinkContainer>()->SetWindowText(nWindowID, pText->GetString()); }
  return true;
}
...
bool CMessageAtomReactionMessageToMainLoop::Execute()
{
  ...
  NStr::DebugTrace("\t\t MessageToMainLoop \tmsg =\t\"%s\"\t(%d), \tparam =\t\"%s\"\n",
                   ...
                   szParam.c_str());
#endif // #if !defined(_FINALRELEASE) && !defined(_BETARELEASE)
  GetSingleton<IMainLoop>()->Command(nCommandID, szParam.empty() ? nullptr : szParam.c_str());
  return true;
}
...
bool CMessageReaction::Execute()
{
  ...
  if (nCustomCheckType) nCustomCheckReturn = GetSingleton<IMessageLinkContainer>()->CustomCheck(nCustomCheckType, customCheckParams);

  CMessageSequences::iterator chosen = atomReactions.find(nCustomCheckReturn);
  if (chosen == atomReactions.end()) chosen = atomReactions.find(ECCR_DEFAULT);
...
{
  if (pToExecute->empty()) return false;
  // execute other sections
  for (auto reaction = pToExecute->begin(); reaction != pToExecute->end(); ++reaction) { if (!(*reaction)->Execute()) return false; }
  return true;
}
...
CMessageReaction::CMessageReaction(const SMessageReactionForLoad &loaded, IMessageLinkContainer *pHelpers)
{
  ...
  for (CAtomReactionSequencesForLoad::const_iterator it = loaded.atomReactions.begin(); it != loaded.atomReactions.end(); ++it)
  {
    ...
    for (int nAtomReaction = 0; nAtomReaction < loadedAtomSequence.size(); ++nAtomReaction)
    {
      ...
      IMessageReaction *pReaction = nullptr;
      switch (nType)
      {
      ...
      if (pReaction) atomReactions[nCustomCheckReturn].push_back(pReaction);
    }
  }
}
...
{
  auto it = messageReactions.find(CIncomingMessage(nMessageID, nParam));
...
void CMessageLink::Load(const std::string &szFileName, IMessageLinkContainer *pHelpers)
{
  ...
  {
    auto pReaction = new CMessageReaction(forLoad[i], pHelpers);
...
void CMessageLinkContainer::SetInterface(class CInterfaceScreenBase *_pInterface) { pInterface = _pInterface; }
...
#endif // #if !defined(_FINALRELEASE) && !defined(_BETARELEASE)
  for (CMessageLinks::iterator it = messageLinks.begin(); !pReaction && it != messageLinks.end(); ++it) { pReaction = it->second->Configure(msg.nEventID, msg.nParam); }
  if (pReaction)
  {
...
int CMessageLinkContainer::CustomCheck(const int nCustomCheckKey, const CCustomCheckParams &checkParams)
{
  switch (nCustomCheckKey)
  {
    ...
    {
      int nResult = 0;
      for (int i = 0; i < checkParams.size(); ++i) { nResult |= (GetGlobalVar(checkParams[i].c_str(), 0) << i); }
      return nResult;
    }
    break;
    ...
    {
      int nResult = 0;
      for (int i = 0; i < checkParams.size(); ++i) { if (GetGlobalVar(checkParams[i].c_str(), 0) != 0) return i + 1; }
      return 0;
    }
    break;
  }
