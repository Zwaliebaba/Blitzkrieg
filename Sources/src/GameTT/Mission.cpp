...
// ////////////////////////////////////////////////
static constexpr int ADD_SELECTED_CONST = 100;
...
void CInterfaceAboutMission::StartInterface()
{
  ...
  if (pStats->IsTemplate() && (GetGlobalVar(szVarName.c_str(), 0) != 1))
  {
    ...
    if (IUserProfile *pUserProfile = GetSingleton<IUserProfile>())
    {
      ...
      for (int nGraphIndex = 1; nGraphIndex < randomMapTemplate.graphs.size(); ++nGraphIndex)
      {
        ...
        if (nMinimumUsedGraph > nUsedTemplateGraph) { nMinimumUsedGraph = nUsedTemplateGraph; }
      }
      ...
      {
        int nUsedTemplateGraph = pUserProfile->GetUsedTemplateGraphs(pStats->szTemplateMap, randomMapTemplate.graphs[nGraphIndex]);
        if (nMinimumUsedGraph == nUsedTemplateGraph) { graphIndices.push_back(nGraphIndex, randomMapTemplate.graphs.GetWeight(nGraphIndex)); }
      }
      if (!graphIndices.empty())
      {
        ...
        else { nFoundGraphIndex = graphIndices.GetRandom(); }
      }
      ...
      for (int nAngleIndex = 1; nAngleIndex < 4; ++nAngleIndex)
      {
        ...
        if (nMinimumUsedAngle > nUsedAngle) { nMinimumUsedAngle = nUsedAngle; }
      }
      std::vector<int> angleIndices;
      for (int nAngleIndex = 0; nAngleIndex < 4; ++nAngleIndex) { if (nMinimumUsedAngle == pUserProfile->GetUsedAngles(nAngleIndex)) { angleIndices.push_back(nAngleIndex); } }
      if (!angleIndices.empty())
      {
        ...
        else { nFoundAngle = angleIndices[rand() % angleIndices.size()]; }
      }
    }
  ...
  else if (pStats->IsTemplate()) { NStr::DebugTrace("CInterfaceAboutMission::StartInterface(), Template Mission exists: %s\n", pStats->szParentName.c_str()); }
  ...
  constexpr int DELTAX = 5;
  constexpr int DELTAY = 3;
  CVec2 vMapControlSize;
  {
    pMap->GetWindowPlacement(nullptr, &vMapControlSize, nullptr);
  ...
  if (p2 != nullptr) pHeader->SetWindowText(0, p2->GetString());
  ...
  pObjectivesScreen->ShowWindow(UI_SW_SHOW);// objectives will auto load here

  // load the Objectives flags for display on the minimap
  ...
  for (int i = 0; i < pStats->objectives.size(); i++)
  {
    ...
    // calculate the coordinates of the objective in the static minimap control
    ...
    CVec2 vFlagSize;
    pFlag->GetWindowPlacement(nullptr, &vFlagSize, nullptr);
    vPos.x -= vFlagSize.x / 2;
    vPos.x += DELTAX;
    vPos.y -= vFlagSize.y;
    vPos.y += DELTAY;
    pFlag->SetWindowPlacement(&vPos, nullptr);
    ...
    pSelectedFlag->SetWindowPlacement(&vPos, nullptr);
  ...
  // initialize free slots in the mission with default units
...
bool CInterfaceAboutMission::ProcessMessage(const SGameMessage &msg)
{
  ...
  switch (msg.nEventID)
  {
    ...
    case MC_UNITS:
      FinishInterface(MISSION_COMMAND_ADD_UNIT_TO_MISSION, nullptr);
      return true;
...
{
  auto pObjectivesScreen = checked_cast<IUIContainer *>(pUIScreen->GetChildByID(4000));
  auto pBar = checked_cast<IUIShortcutBar *>(pObjectivesScreen->GetChildByID(10));
  ...
  // hide the previous active objective flag
  ShowActiveObjective(false);

  // show the new active objective flag
...
void CInterfaceAboutMission::ShowActiveObjective(bool bShow)
{
  ...
  if (m_nActiveObjective != -1)
  {
    ...
    if (bShow) pElement->ShowWindow(UI_SW_HIDE);
    else pElement->ShowWindow(UI_SW_SHOW);
    ...
    if (bShow) pElement->ShowWindow(UI_SW_SHOW);
