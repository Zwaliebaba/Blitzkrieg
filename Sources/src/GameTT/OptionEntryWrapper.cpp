  ...
  : pDialog(_pDialog), pOption(_pOption), pOptionName(_pOptionName)
{
  ...
  if (pText) pOptionName->SetWindowText(0, pText->GetString());
  pText = pTM->GetString(szKeyTooltip.c_str());
  if (pText) pOptionName->SetHelpContext(0, pText->GetString());
...
void CUIOption::PositionChanged() { if (EOT_SLIDER == pOption->GetType() && pOption->IsInstant()) { SaveOption(); } }

void CUIOption::OnSelected()
{
  if (EOT_GAMESPY_TEXTENTRY == pOption->GetType())
  {
    auto pStatic = checked_cast<IUIEditBox *>(pSubDialog->GetChildByID(E_EDITBOX_GAMESPY));
    pStatic->SetFocus(true);
  }
  else if (EOT_TEXTENTRY == pOption->GetType())
  {
    auto pStatic = checked_cast<IUIEditBox *>(pSubDialog->GetChildByID(E_EDITBOX));
    pStatic->SetFocus(true);
  }
  else if (EOT_NUMERICENTRY == pOption->GetType())
  {
    auto pStatic = checked_cast<IUIEditBox *>(pSubDialog->GetChildByID(E_EDITBOX_NUMERIC));
    pStatic->SetFocus(true);
  }
}
...
void CUIOption::ChangeSelection(const int nCurSelection)
{
  ...
  ITextManager *pTM = GetSingleton<ITextManager>();
  auto pStatic = checked_cast<IUIStatic *>(pSubDialog->GetChildByID(E_SELECTIONS_ENTRY));

  IText *pText = pTM->GetString(szKeyName.c_str());
  if (pText) pStatic->SetWindowText(0, pText->GetString());
  else
  {
  ...
  pText = pTM->GetString(szKeyTooltip.c_str());
  if (pText) pStatic->SetHelpContext(0, pText->GetString());
  else
  {
#ifndef _FINALRELEASE
    GetSingleton<IConsoleBuffer>()->WriteASCII(CONSOLE_STREAM_CHAT, NStr::Format("cannot find localized string %s", szKeyTooltip.c_str()));
#endif // _FINALRELEASE
    IText *pHelpContext = pOptionName->GetHelpContext(VNULL2, nullptr);
    if (pHelpContext) pStatic->SetHelpContext(0, pHelpContext->GetString());
  }
}
...
void CUIOption::OnClicked(const bool bLeft)
{
  ...
  {
    if (bLeft) ++nCurSelection;
    else { nCurSelection = nCurSelection == 0 ? szSelections.size() - 1 : nCurSelection - 1; }
    nCurSelection %= szSelections.size();
    ChangeSelection(nCurSelection);
    if (pOption->IsInstant()) SaveOption();
  }
  else if (EOT_TEXTENTRY == pOption->GetType())
  {
    auto pStatic = checked_cast<IUIEditBox *>(pSubDialog->GetChildByID(E_EDITBOX));
...
void CUIOption::SetSliderOption(const int _nMin, const int _nMax, const int _nInitial)
{
  ...
  auto pSlider = checked_cast<IUISlider *>(pSubDialog->GetChildByID(E_SLIDER));
...
void CUIOption::ResetSlider()
{
  ...
  IText *pHelpContext = pOptionName->GetHelpContext(VNULL2, nullptr);
  if (pHelpContext) pSlider->SetHelpContext(0, pHelpContext->GetString());
}
...
int CUIOption::GetSelectionOption() const { return nCurSelection; }
...
COptionsListWrapper::COptionsListWrapper(const DWORD _dwFlags, IUIListControl *_pList, const int _nIDToStartFrom, IOptionSystem *pSystem, const bool _bDisableChange)
  : pList(_pList), nIDToStartFrom(_nIDToStartFrom), dwFlags(_dwFlags), pSetOptionSystem(pSystem), bDisableChange(_bDisableChange)
{
...
COptionsListWrapper::COptionsListWrapper(IUIListControl *_pList, OptionDescs &_optionDescs, const int _nIDToStartFrom, IOptionSystem *pSystem, const bool _bDisableChange)
  : optionsDescs(_optionDescs), pList(_pList), nIDToStartFrom(_nIDToStartFrom), pSetOptionSystem(pSystem), bDisableChange(_bDisableChange) { InitList(false); }
  ...
  for (OptionDescs::const_iterator it = optionsDescs.begin(); it != optionsDescs.end(); ++it)
  {
    ...
    switch (pDesc->nEditorType)
    {
      case EOET_GAMESPY_TEXT_ENTRY:
      {
        ...
        variant_t val;
        if (!bDefault && pSystem->Get(pDesc->szName, &val)) pOption = new COptionTextEntryGameSpyCharacters(pDesc->szName.c_str(), pDesc->bInstantApply, static_cast<wchar_t *>((_bstr_t) val));
        else pOption = new COptionTextEntryGameSpyCharacters(pDesc->szName.c_str(), pDesc->bInstantApply, static_cast<wchar_t *>((_bstr_t) pDesc->defaultValue));
      }
      ...
      case EOET_TEXT_ENTRY:
      {
        NI_ASSERT_T(pDesc->nDataType == VT_BSTR, "EOET_TEXT_ENTRY is allowed only for VT_BSTR type");
        variant_t val;
        if (!bDefault && pSystem->Get(pDesc->szName, &val)) pOption = new COptionTextEntry(pDesc->szName.c_str(), pDesc->bInstantApply, static_cast<wchar_t *>((_bstr_t) val));
        else pOption = new COptionTextEntry(pDesc->szName.c_str(), pDesc->bInstantApply, static_cast<wchar_t *>((_bstr_t) pDesc->defaultValue));
      }
      ...
      case EOET_NUMERIC_ENTRY:
      {
        ...
        variant_t val;
        if (!bDefault && pSystem->Get(pDesc->szName, &val)) pOption = new COptionNumericEntry(pDesc->szName.c_str(), pDesc->bInstantApply, static_cast<long>(val));
        else pOption = new COptionNumericEntry(pDesc->szName.c_str(), pDesc->bInstantApply, static_cast<long>(pDesc->defaultValue));
      }
      ...
      case EOET_SLIDER:
      {
        ...
        variant_t val;
        if (!bDefault && pSystem->Get(pDesc->szName, &val)) pOption = new COptionSlider(pDesc->szName.c_str(), pDesc->bInstantApply, 0, 100, static_cast<long>(val));
        else pOption = new COptionSlider(pDesc->szName.c_str(), pDesc->bInstantApply, 0, 100, static_cast<long>(pDesc->defaultValue));
      }
      ...
      case EOET_CLICK_SWITCHES:
      {
        ...
        variant_t val;
        if (!bDefault && pSystem->Get(pDesc->szName, &val)) szCurrent = static_cast<_bstr_t>(val);
        else szCurrent = static_cast<_bstr_t>(pDesc->defaultValue);
    ...
    if (pOption)
    {
      if (pSetOptionSystem) pOption->SetOptionSystem(pSetOptionSystem);
...
{
  for (int i = 0; i < options.size(); ++i)
    if (options[i]->IsOptionValid()) options[i]->Apply();
  pList->InitialUpdate();
}
...
{
  for (int i = 0; i < options.size(); ++i)
    if (options[i]->IsOptionValid()) options[i]->CancelChanges();
  pList->InitialUpdate();
}

void COptionsListWrapper::ToDefault()
{
  while (pList->GetNumberOfItems()) pList->RemoveItem(0);
  InitList(true);
  Apply();
}
  ...
  switch (msg.nEventID)
  {
    case UI_NOTIFY_SELECTION_CHANGED:
      if (msg.nParam == 7777)
      {
        ...
        if (-1 != nOption && options[nOption]->IsOptionValid()) options[nOption]->OnSelected();
        return true;
      }
    ...
    case 7777:
      ...
      {
        const int nOption = pList->GetSelectionItem();
        if (-1 != nOption && options[nOption]->IsOptionValid()) options[nOption]->OnClicked(true);
        return true;
      }
    ...
    case 7779:
      ...
      {
        const int nOption = pList->GetSelectionItem();
        if (-1 != nOption && options[nOption]->IsOptionValid()) options[nOption]->OnClicked(false);
        return true;
      }
    ...
    case 7778:
      if (msg.nParam == 7778)
      {
        ...
        if (-1 != nOption && options[nOption]->IsOptionValid()) { options[nOption]->PositionChanged(); }
        return true;
      }
