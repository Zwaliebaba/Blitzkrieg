...
void CPlayersInterface::OnGetFocus(bool bFocus)
{
  ...
  if (bFocus) commandMsgs.Init(pInput, commands);
  ...
  if (pPopups)
  {
    ...
    if (pPopups->IsNeedFinish())
    {
      ...
      if (pEl) pEl->ShowWindow(pPopups->IsNeedBlack() ? UI_SW_SHOW : UI_SW_HIDE);
    }
  }
}
...
void CPlayersInterface::Create(const bool _bAfterMission)
{
  ...
  if (szChapterName == "tutorial")
  {
    ...
    else
    {
      ...
      pML->Command(MISSION_COMMAND_TUTORIAL_LIST, nullptr);
    }
    return;
  }
  ...
  if (bAfterMission) pPopups = new CAfterMissionPopups;
  ...
  for (int nMedalSlot = 0; nMedalSlot < NUM_MEDAL_SLOTS; ++nMedalSlot)
  {
    ...
    if (!szMedalName.empty())
    {
      ...
      CVec2 vStaticSize;
      CVec2 vStaticPos;
      pMedalStatic->GetWindowPlacement(&vStaticPos, &vStaticSize, nullptr);
  ...
  auto pRankValue = checked_cast<IUIStatic *>(pUIScreen->GetChildByID(E_RANK_VALUE));
  ...
  if (pTextCurrentRank) pRankValue->SetWindowText(0, pTextCurrentRank->GetString());
  // next rank;
  auto pRankList = checked_cast<IUIListControl *>(pUIScreen->GetChildByID(E_RANK_LIST));
  ...
  auto pElementName = checked_cast<IUIStatic *>(pRow->GetElement(0));
  ...
  if (pTextNextRank) pElementName->SetWindowText(0, pTextNextRank->GetString());
  pRankList->InitialUpdate();
  //

  // set caption:
  auto pCaption = checked_cast<IUIStatic *>(pUIScreen->GetChildByID(20000));
  ...
  // set stats
  auto pListControl = checked_cast<IUIListControl *>(pUIScreen->GetChildByID(1000));
  for (int nStat = 0; nStat < _EPST_COUNT; ++nStat)
  {
    ...
    if (pText) pElementName->SetWindowText(0, pText->GetString());
    const std::string szToolTip = rSkill.szSkillName + ".tooltip";
    pText = pTextM->GetString(szToolTip.c_str());
    if (pText) pElementName->SetHelpContext(0, pText->GetString());
  }
  ...
  if (pDifficulty && pDifficutlyName) pDifficulty->SetWindowText(0, pDifficutlyName->GetString());
  ...
  if (pDifficultyVal && pDifficultyValName) pDifficultyVal->SetWindowText(0, pDifficultyValName->GetString());
  ...
  if (GetGlobalVar("FinishingCampaign", 0) != 0)
  {
    ...
    if (nPos == std::string::npos) nPos = szCampaignStatsName.rfind("/");
  ...
  if (bAfterMission)
  {
    ...
    if (nCampaign >= 0)
    {
      // we play the campaign, launch the corresponding music
...
{
  // set value
  auto pElementDialog = checked_cast<IUIDialog *>(pRow->GetElement(1));

  IUINumberIndicator *pCurrent = nullptr, *pFormer = nullptr;
  ...
  if (pCurrent) pCurrent->SetValue(fCurrentVal);
  if (pFormer) pFormer->SetValue(fFormerVal);

  auto pText = checked_cast<IUIStatic *>(pElementDialog->GetChildByID(E_TEXT_SKILL));
...
{
  if (msg.nEventID == TUTORIAL_WINDOW_ID || msg.nEventID == TUTORIAL_BUTTON_ID) { bDisableGetFocus = true; }
  ...
  switch (msg.nEventID)
  {
    case IMC_CANCEL:
      ...
      if (GetGlobalVar("FinishingCampaign", 0) != 0)
      {
        ...
        const SCampaignStats *pStats = szCampaignStatsName.empty() ? nullptr : NGDB::GetGameStats<SCampaignStats>(szCampaignStatsName.c_str(), IObjectsDB::CAMPAIGN);
        if (pStats)
        {
        ...
        else FinishInterface(MISSION_COMMAND_MAIN_MENU, nullptr);
      }
      else if (bAfterMission) { OnGetFocus(true); }
