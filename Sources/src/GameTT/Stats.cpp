...
void CICStats::Configure(const char *pszConfig)
{
  ...
  if (pszConfig != nullptr && strlen(pszConfig) > 0) nCurrentMissionStats = NStr::ToInt(pszConfig);
  else nCurrentMissionStats = 0;
}

void CICStats::PreCreate(IMainLoop *pML)
{
  const std::string szChapterName = GetGlobalVar("Chapter.Current.Name", "");
  if (GetGlobalVar("MultiplayerGame", 0) || szChapterName == "custom_mission") pML->ResetStack();
}
...
int CInterfaceStats::CCommonStats::GetNStats(const bool bMultiplayer) const { return bMultiplayer ? 1 : 4; }
...
std::wstring CInterfaceStats::CCommonStats::GetStatValue(const int nIndex) const
{
  switch (nIndex)
  {
    ...
    case 2: // difficulty //failed object
    {
...
CInterfaceStats::SStatConfugure::SStatConfugure(const int _eStatType,
  ...
  : eStatType(_eStatType), eAccumulateType(_eAccumulateType),
    nIndexToCountBest(_nIndexToCountBest), bLeaderIsGreatest(bGreatest)
{
  eType[0] = _eType1;
  eType[1] = _eType2;
}

bool CInterfaceStats::SStatConfugure::IsToDisplay(const bool bMultiplayerGame) const { return EST_BOTH == eStatType || (bMultiplayerGame ? EST_MULTIPLAYER == eStatType : EST_SINGLEPLAYER == eStatType); }


// SPlayerStatInfo


std::wstring CInterfaceStats::SPlayerStatInfo::GetValueToSort(const SStatConfugure &config) const { return NStr::ToUnicode(NStr::Format("%d", static_cast<int>(fVal[config.nIndexToCountBest]))); }

std::wstring CInterfaceStats::SPlayerStatInfo::GetValue(const SStatConfugure &config) const
{
  if (E_BEST_NO_PLAYER_DISPLAY == config.eAccumulateType) { return L"--"; }
  if (config.eType[1] != -1)
  {
    // 2 values ​​at 1 a string
    return NStr::ToUnicode(NStr::Format("%d(%d)", static_cast<int>(fVal[0]), static_cast<int>(fVal[1])));
  }
  return NStr::ToUnicode(NStr::Format("%d", static_cast<int>(fVal[0])));
}

void CInterfaceStats::SPlayerStatInfo::Init(const CPlayerFullInfo &playerInfo, const SStatConfugure &config)
{
  for (int i = 0; i < 2; ++i)
  {
    if (config.eType[i] != -1)
    {
      ...
      if (config.eAccumulateType == E_SUMMARY_DEVIDED_BY_1000) fVal[i] /= 1000;
    }
  }
}
...
std::wstring CInterfaceStats::SPartyInfo::GetValForSort(const SStatConfugure &config) const
{
  ...
  return NStr::ToUnicode(NStr::Format("%d", static_cast<int>(fVal[config.nIndexToCountBest])));
}
...
void CInterfaceStats::SPartyInfo::Init(const CPlayersFullInformation &info, const SStatConfugure &config)
{
  fVal[0] = fVal[1] = 0;
      ...
      if (config.eType[i] != -1)
      {
        ...
        switch (config.eAccumulateType)
        {
          ...
          case E_SUMMARY_DEVIDED_BY_1000:
            fVal[i] += static_cast<int>(fCurrentVal / 1000.0f);
            break;
          default:
...
void CInterfaceStats::FillCommonStatsList(const bool bMultiplayer, const CCommonStats &commonStats)
{
  auto pList = checked_cast<IUIListControl *>(pUIScreen->GetChildByID(bMultiplayer ? E_COMMON_STATS_LIST_MP : E_COMMON_STATS_LIST_SP));
...
void CInterfaceStats::InitSorter() { pSorter = new CSorter; }
...
void CInterfaceStats::RepositionList()
{
  ...
  commonStats.Init(playerInfos[0].empty() ? playerInfos[1][0] : playerInfos[0][0]);// it doesn't matter what player to use in MP, in SP we must use local. 
  ...
  for (int nParty = 0; nParty < 2; ++nParty)
  {
    if (playerInfos[nParty].empty()) continue;// don't fill empty party's info
    ...
    // PARTY
    {
      ...
      for (int nStatFeild = 0; nStatFeild < playerStatsConfigure.size(); ++nStatFeild)
      {
        if (playerStatsConfigure[nStatFeild].IsToDisplay(bMultiplayer))
        {
          ...
          auto pElement = checked_cast<IUIContainer *>(pRow->GetElement(nIndex++));
    ...
    // PLAYERS
    const bool bDisplayStar = playerInfos[1].size() + playerInfos[0].size() > 1;// only if more than 1 play exists
    {
    ...
    pList->Sort(1, -1);
    while (pList->GetNumberOfItems() > 5) { pList->RemoveItem(5); }
    pList->InitialUpdate();
  }
}
...
void CInterfaceStats::Create(const int /* EStatsComplexity */ nStatsType)
{
  ...
  pUIScreen->Reposition(pGFX->GetScreenRect());// first reposition to initialize ListControl size
  ...
  switch (m_nStatsType)
  {
    ...
    case STATS_COMPLEXITY_MISSION:
    {
      ...
      {
        // display the Save Replay button
  ...
  for (CPtr<IPlayerScenarioInfoIterator> pIt = pST->CreatePlayerScenarioInfoIterator(); !pIt->IsEnd(); pIt->Next(), ++nListItemsCounter)
  {
    ...
    if (pPlayer->GetDiplomacySide() == 2) continue;// We do not display statistics for neutral players

    IScenarioStatistics *pStats = nullptr;
    switch (m_nStatsType)
    {
      case STATS_COMPLEXITY_TOTAL:
    ...
    if (pStats == nullptr) continue;

    CollectPlayerStats(pPlayer, pStats);
  }

  RepositionList();
  // for MP and SP back button has different tooltips

  pUIScreen->Reposition(pGFX->GetScreenRect());
  if (GetGlobalVar("MultiplayerGame", 0))
  {
    StoreScreen();
    // let's start inter-mission music
...
void CInterfaceStats::OnGetFocus(bool bFocus)
{
  ...
  if (!bPopupsShowed) { bPopupsShowed = true; }
}

bool CInterfaceStats::ProcessMessage(const SGameMessage &msg)
{
  if (msg.nEventID == TUTORIAL_WINDOW_ID) { bTutorialWindow = true; }
  ...
  if (pPartyList[0] && pPartyList[1])
  {
    ...
    if (pPartyList[0]->GetWindowID() == msg.nEventID) pPartyList[1]->Sort(nColumn);
    else if (pPartyList[1]->GetWindowID() == msg.nEventID) pPartyList[0]->Sort(nColumn);
  }
  ...
  switch (msg.nEventID)
  {
    case IMC_CANCEL:
    {
      ...
      if (GetGlobalVar("History.Playing", 0) > 0)
      {
        // let's go to the replays viewing screen
        ...
        pML->Command(MISSION_COMMAND_REPLAY_LIST, nullptr);
        return true;
      }
    }
      {
        if (GetGlobalVar("MultiplayerGame", 0))
        {
          ...
          if (GetSingleton<IMPToUICommandManager>()->GetConnectionType() == EMCT_INTERNET) FinishInterface(MISSION_COMMAND_ADDRESS_BOOK, nullptr);
        ...
        if (szChapterName == "custom_mission")
        {
          ...
          pML->Command(MISSION_COMMAND_CUSTOM_MISSION, nullptr);
          return true;
        }
      }
      CloseInterface(true);
    ...
    case IMC_SAVE_REPLAY:
      bCheckReplay = true;
      GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_SAVE_REPLAY, nullptr);
      return true;
  }
...
void CInterfaceStats::OnDemoversionExit()
{
  ...
  {
    FinishInterface(MISSION_COMMAND_MAIN_MENU, nullptr);
    return;
  }
  ...
  for (; nCurrentMission < nDemomissions; ++nCurrentMission)
  {
    ...
    if (szMissionName == szCurrentMission) break;
  }
