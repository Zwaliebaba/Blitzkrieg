...
void IUIState::Show()
{
  GetSingleton<IScene>()->SetToolTip(nullptr, CVec2(0, 0), CTRect<float>(0, 0, 0, 0));
...
bool CUIMainMenuState::ProcessMessage(const SGameMessage &msg)
{
  switch (msg.nEventID)
  {
      ...
      // if it is, then display message, else switch to multiplayer directly
    {
      ...
      if (bFileExists) GetMainInterface()->SetActiveState(CInterfaceMainMenu::E_MULTIPLAYER);
      else
      {
    ...
    case IMC_SETTINGS:
      ...
      return true;
    case IMC_EXIT_GAME:
      GetSingleton<IMainLoop>()->Command(MAIN_COMMAND_EXIT_GAME, nullptr);
      return true;
  }
...
void CUINewGameState::Show()
{
  ...
  // check if player's name differs from default
  ...
  const std::wstring szNameFromOptions = static_cast<wchar_t *>((bstr_t) varPlayerName);
      ...
      szNameFromOptions == szDefault || szNameFromOptions.empty())
  {
    ...
    GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_PLAYER_PROFILE, nullptr);
  }
}
...
bool CUINewGameState::ProcessMessage(const SGameMessage &msg)
{
  switch (msg.nEventID)
  {
    case IMC_CANCEL:
      ...
      return true;
    case IMC_OK:
      if (GetGlobalVar("MOD.Active", 0) != 1) { GetMainInterface()->SetActiveState(CInterfaceMainMenu::E_SELECT_CAMPAIGN); }
      else
      {
      ...
      return true;
    case IMC_TUTORIAL:
      GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_TUTORIAL_LIST, nullptr);
      return true;
...
void CUISelectCampaignState::Show()
{
  ...
  // CRAP{ our campaigns at the interface level are crooked...
  constexpr int indices[] = {10004, 10003, 10002};
  ...
  for (int i = 0; i < 3; ++i)
  {
    ...
    if (pStorage->IsStreamExist((szCampaignName + ".xml").c_str()) == false) { if (IUIElement *pElement = pUIScreen->GetChildByID(indices[i])) pElement->EnableWindow(false); }
    else { if (IUIElement *pElement = pUIScreen->GetChildByID(indices[i])) pElement->EnableWindow(true); }
  }
}
...
bool CUIOptionsState::ProcessMessage(const SGameMessage &msg)
{
  switch (msg.nEventID)
  {
    case IMC_CANCEL:
      ...
      return true;
    case IMC_OPTIONS:
      GetMainInterface()->FinishInterface(MISSION_COMMAND_OPTIONSSETTINGS, nullptr);
      return true;
    case IMC_MODS:
      GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_MODS_LIST, nullptr);
      return true;
    case IMC_VIDEO:
      GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_CUTSCENE_LIST, nullptr);
      return true;
    case IMC_CREDITS:
...
bool CUIMultiplayerState::ProcessMessage(const SGameMessage &msg)
{
  switch (msg.nEventID)
  {
    ...
    case IMC_INTERNET:
      ...
      GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_ADDRESS_BOOK, nullptr);
...
bool CUICustomGameState::ProcessMessage(const SGameMessage &msg)
{
  switch (msg.nEventID)
  {
    ...
    case IMC_CUSTOM_MISSIONS:
      GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_CUSTOM_MISSION, nullptr);
      return true;
  }
  //
  return false;
...
bool CUILoadGameState::ProcessMessage(const SGameMessage &msg)
{
  switch (msg.nEventID)
  {
    ...
    case IMC_LOAD_LOAD_GAME:
      GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_IM_LOAD_MISSION, nullptr);
      return true;
    case IMC_LOAD_LOAD_REPLAY:
      GetSingleton<IMainLoop>()->Command(MISSION_COMMAND_REPLAY_LIST, nullptr);
      return true;
  }
  //
  return false;
...
void CUICreditsState::Show()
{
  ...
  if (!szCreditsStreamSound.empty()) GetSingleton<ISFX>()->PlayStream(szCreditsStreamSound.c_str(), true, nTimeToFade);
...
bool CUICreditsState::ProcessMessage(const SGameMessage &msg)
{
  switch (msg.nEventID)
  {
    ...
    case MC_CANCEL_CREDITS:
      if (bLeaveToMainMenu) GetMainInterface()->SetActiveState(CInterfaceMainMenu::E_MAIN_MENU);
      else GetMainInterface()->SetActiveState(CInterfaceMainMenu::E_OPTIONS);
      return true;
    default:
      break;
  }
