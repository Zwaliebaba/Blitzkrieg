  ...
  : nSizeX(_nSizeX), nSizeY(_nSizeY)
{
  ...
  for (int i = 0; i < data.size(); ++i) data[i].color = _data[i];
}

void CImage::Set(SColor color) { std::fill(data.begin(), data.end(), color); }

bool CImage::SetAlpha(const IImage *pAlpha)
{
  if ((nSizeX != pAlpha->GetSizeX()) || (nSizeY != pAlpha->GetSizeY())) return false;
  const SColor *pAlphaColors = pAlpha->GetLFB();
  for (int i = 0; i < nSizeX * nSizeY; ++i) data[i].color = (data[i].color & 0x00ffffff) | (pAlphaColors[i].color & 0xff000000);
  return true;
}

void CImage::SetAlpha(BYTE alpha)
{
  DWORD dwAlpha = static_cast<DWORD>(alpha) << 24;
  for (auto pos = data.begin(); pos != data.end(); ++pos) pos->color = (pos->color & 0x00ffffff) | dwAlpha;
}

void CImage::SetColor(DWORD color)
{
  color &= 0x00ffffff;
  for (auto pos = data.begin(); pos != data.end(); ++pos) pos->color = (pos->color & 0xff000000) | color;
}

bool CImage::SetColor(const IImage *pColor)
{
  if ((nSizeX != pColor->GetSizeX()) || (nSizeY != pColor->GetSizeY())) return false;
  const SColor *pColors = pColor->GetLFB();
  for (int i = 0; i < nSizeX * nSizeY; ++i) data[i].color = (data[i].color & 0xff000000) | (pColors[i].color & 0x00ffffff);
  return true;
}
...
{
  for (int i = 0; i < nSizeX * nSizeY; ++i) data[i].a = 255 - data[i].a;
}

void CImage::SharpenAlpha(BYTE ref)
{
  for (int i = 0; i < nSizeX * nSizeY; ++i) data[i].a = data[i].a >= ref ? 255 : 0;
}
...
{
  RECT rcRect;
  if (pSrcRect == nullptr) SetRect(&rcRect, 0, 0, pSrc->GetSizeX(), pSrc->GetSizeY());
  else rcRect = *pSrcRect;
  //
...
bool CImage::CopyFromAB(IImage *pSrc, const RECT *pSrcRect, int nPosX, int nPosY)
{
  ...
  for (int j = 0; j < Height(rcRect); ++j)
  {
    ...
    {
      pDstColors[i].r = (static_cast<DWORD>(pSrcColors[i].r) * static_cast<DWORD>(pSrcColors[i].a) + static_cast<DWORD>(pDstColors[i].r) * static_cast<DWORD>(255 - pSrcColors[i].a)) / 255;
      pDstColors[i].g = (static_cast<DWORD>(pSrcColors[i].g) * static_cast<DWORD>(pSrcColors[i].a) + static_cast<DWORD>(pDstColors[i].g) * static_cast<DWORD>(255 - pSrcColors[i].a)) / 255;
      pDstColors[i].b = (static_cast<DWORD>(pSrcColors[i].b) * static_cast<DWORD>(pSrcColors[i].a) + static_cast<DWORD>(pDstColors[i].b) * static_cast<DWORD>(255 - pSrcColors[i].a)) / 255;
...
bool CImage::ModulateAlphaFrom(IImage *pSrc, const RECT *pSrcRect, int nPosX, int nPosY)
{
  ...
  for (int j = 0; j < Height(rcRect); ++j)
  {
    ...
    for (int i = 0; i < Width(rcRect); ++i)
    {
      pDstColors[i].r = static_cast<DWORD>(pDstColors[i].r) * static_cast<DWORD>(pSrcColors[i].a) / 255;
      pDstColors[i].g = static_cast<DWORD>(pDstColors[i].g) * static_cast<DWORD>(pSrcColors[i].a) / 255;
      pDstColors[i].b = static_cast<DWORD>(pDstColors[i].b) * static_cast<DWORD>(pSrcColors[i].a) / 255;
      pDstColors[i].a = static_cast<DWORD>(pDstColors[i].a) * static_cast<DWORD>(pSrcColors[i].a) / 255;
    }
  }
  //
  return true;
...
bool CImage::ModulateColorFrom(IImage *pSrc, const RECT *pSrcRect, int nPosX, int nPosY)
{
  ...
  for (int j = 0; j < Height(rcRect); ++j)
  {
    ...
    for (int i = 0; i < Width(rcRect); ++i)
    {
      pDstColors[i].r = static_cast<DWORD>(pDstColors[i].r) * static_cast<DWORD>(pSrcColors[i].r) / 255;
      pDstColors[i].g = static_cast<DWORD>(pDstColors[i].g) * static_cast<DWORD>(pSrcColors[i].g) / 255;
      pDstColors[i].b = static_cast<DWORD>(pDstColors[i].b) * static_cast<DWORD>(pSrcColors[i].b) / 255;
      pDstColors[i].a = static_cast<DWORD>(pDstColors[i].a) * static_cast<DWORD>(pSrcColors[i].a) / 255;
    }
  }
  //
...
bool SPixelConvertInfo::InitMaskInfo(DWORD dwABitMask, DWORD dwRBitMask, DWORD dwGBitMask, DWORD dwBBitMask)
{
  ...
  if (dwMask)
  {
    for (; (dwMask & 0x1) == 0; dwMask >>= 1) dwBitShift++;
    for (; (dwMask & 0x1) == 1; dwMask >>= 1) dwBitCount++;
  }
  dwAMask = dwABitMask;
  dwABits = dwBitCount;
  dwAShift = dwBitShift;
...
{
  // Convert Alpha component
  DWORD a = static_cast<DWORD>(double((dwColor >> 24) & 0xFF) / double(1 << (8 - dwABits))) << dwAShift;
  // Convert Red component
  DWORD r = static_cast<DWORD>(double((dwColor >> 16) & 0xFF) / double(1 << (8 - dwRBits))) << dwRShift;
  // Convert Green component
  DWORD g = static_cast<DWORD>(double((dwColor >> 8) & 0xFF) / double(1 << (8 - dwGBits))) << dwGShift;
  // Convert Blue component
  DWORD b = static_cast<DWORD>(double((dwColor) & 0xFF) / double(1 << (8 - dwBBits))) << dwBShift;
  // Return converted color
  return (r | g | b | a);
}
