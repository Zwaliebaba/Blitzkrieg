...
{
  WORD wFirstEntryIndex;// Index of the first color map entry. 
  WORD wColorMapLength;// Total number of color map entries included
  BYTE cColorMapEntrySize;// Establishes the number of bits per entry. 
...
struct SImageSpecification
{
  ...
  BYTE cPixelDepth;// number of bits per pixel. 
...
public:
  ...
  {
    palette.reserve(256);
    for (int i = 0; i < 256; ++i) palette.push_back(SColor(255, i, i, i));
  }
  ...
  CTGAPaletteConvertor(const STGAFileHeader &hdr, IDataStream *pStream)
  {
    switch (hdr.colormap.cColorMapEntrySize)
    {
      ...
      case 24:
      {
        ...
        for (std::vector<SColor24>::const_iterator it = palette24.begin(); it != palette24.end(); ++it) palette.push_back(SColor(0xff, it->r, it->g, it->b));
      }
      break;
      default:
...
public:
  ...
  const SColor operator()(const SColor24 &input) const { return 0xff000000 | (static_cast<DWORD>(input.r) << 16) | (static_cast<DWORD>(input.g) << 8) | static_cast<DWORD>(input.b); }
...
class CTGARawLoader
{
  static bool LoadLocal(SColor *pColors, const int nNumElements, const TConvertor &convertor, IDataStream *pStream, ...)
  {
    ...
    for (int i = 0; i < nNumElements; ++i) *pColors++ = convertor(buffer[i]);
...
public:
  static bool Load(const STGAFileHeader &hdr, IImage *pImage, IDataStream *pStream)
  {
    ...
    if (!convertor.IsReady()) return false;
...
// ** Run-length packet: single color value
...
bool LoadColorMappedTGA(const STGAFileHeader &hdr, CImage *pImage, IDataStream *pStream) { return CTGARawLoader<BYTE, CTGAPaletteConvertor<BYTE>>::Load(hdr, pImage, pStream); }
...
bool LoadRLEColorMappedTGA(const STGAFileHeader &hdr, CImage *pImage, IDataStream *pStream) { return CTGARLELoader<BYTE, CTGAPaletteConvertor<BYTE>>::Load(hdr, pImage, pStream); }

bool LoadRLEBlackAndWhiteTGA(const STGAFileHeader &hdr, CImage *pImage, IDataStream *pStream) { return CTGARLELoader<BYTE, CTGAGrayConvertor>::Load(hdr, pImage, pStream); }
...
CImage *NImage::LoadImageTGA(IDataStream *pStream)
{
  ...
  pStream->Read(&hdr, sizeof(hdr));
  // skip image ID
  if (hdr.cIDLength != 0) pStream->Seek(hdr.cIDLength, STREAM_SEEK_CUR);
  //
  auto pImage = new CImage(hdr.imagespec.wImageWidth, hdr.imagespec.wImageHeight);
  bool bLoaded = false;
  switch (hdr.cImageType)
  {
    case TGAIT_TRUE_COLOR:
    ...
    default:
    {
      ...
      const char *pszImageName = stats.pszName == nullptr ? "" : stats.pszName;
  ...
  if (bLoaded)
  {
    if (hdr.imagespec.descriptor.cTopToBottomOrder == 0) pImage->FlipY();
    return pImage;
  }
  else
  {
...
bool NImage::SaveImageAsTGA(IDataStream *pStream, const IImage *pImage)
{
  ...
  // write color data
  const int nNumElements = static_cast<int>(hdr.imagespec.wImageWidth) * static_cast<int>(hdr.imagespec.wImageHeight);
  const int nWriteSizeBytes = nNumElements * 4;// 4 = hdr.imagespec.cPixelDepth / 8
  NI_ASSERT_T(nWriteSizeBytes > 0, NStr::Format("image size %d : %d are too big (>2GB) - can't save it", static_cast<int>(hdr.imagespec.wImageWidth), static_cast<int>(hdr.imagespec.wImageHeight)));
  check = pStream->Write(pImage->GetLFB(), nWriteSizeBytes);
  if (check != nWriteSizeBytes) return false;
