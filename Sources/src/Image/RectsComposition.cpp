...
struct SRectOptimizeStructure
{
  ...
  SRectOptimizeStructure(GRect rect, int num)
  {
    nNum = num;
    rRect = rect;
  }
...
public:
  ...
  {
    if (r.rRect.bottom() > dy) dy = r.rRect.bottom();
    if (r.rRect.right() > dx) dx = r.rRect.right();
  }
};
...
public:
  ...
  {
    if (r1.rRect.height() == r2.rRect.height()) return r1.rRect.width() > r2.rRect.width();
...
public:
  ...
  void operator()(const SRectOptimizeStructure &r) const
  {
    ...
    if (!rRectTemp.isEmpty()) intersect = true;
  }
};

// move rect down until it no longer intersects with anyone
//	
//
//
void SliceRectToDown(const std::vector<SRectOptimizeStructure> &rects, SRectOptimizeStructure &rect)
{
  ...
  for (auto it = rects.begin(); it != rects.end(); ++it) yCorners.insert(it->rRect.bottom() + 1);

  for (auto it = yCorners.begin(); it != yCorners.end(); ++it)
  {
...
void OptimizeRectsByWidth(std::vector<SRectOptimizeStructure> &rects, int textureWidth)
{
  ...
  for (std::vector<SRectOptimizeStructure>::const_iterator it = rects.begin(); it != rects.end(); ++it)
  {
    ...
    if (curDirection == toRight)
    {
      ...
      else nCurrentX = newRect.right() + 1;
    }
    else
    {
...
{
  // let's calculate the widest Rect
  CFindFillRectHelper hlp;
  hlp = std::for_each(rects.begin(), rects.end(), hlp);
  ...
  for (; TEXTUREWIDTH <= MAX_TEXTURE_SIZE;)
  {
    ...
    if (rcTempRect.width() == rcTempRect.height()) rcSquareRect = rcTempRect;
    // closer square
    const float fRespect = Max(1.0f, static_cast<float>(GetNextPow2(rcTempRect.height())) / static_cast<float>(GetNextPow2(rcTempRect.width())));
    ...
    for (std::vector<SRectOptimizeStructure>::const_iterator it = vTmp.begin(); it != vTmp.end(); ++it) size += it->rRect.width() * it->rRect.height();

    if (procent < (size / static_cast<float>(maxRectSize + 0.001)) && rcTempRect.height() < MAX_TEXTURE_SIZE)
    {
      procent = size / static_cast<float>(maxRectSize + 0.001);
  ...
  if (nSqrWidth * nSqrHeight == nOptWidth * nOptHeight) OptimizeRectsByWidth(rects, nSqrWidth);
  else if (nCloseSqrWidth * nCloseSqrHeight == nOptWidth * nOptHeight) OptimizeRectsByWidth(rects, nCloseSqrWidth);
  else OptimizeRectsByWidth(rects, nOptWidth);
}

constexpr int nBoundValue = 1000000;
...
IImage *CImageProcessor::ComposeImages(IImage **pImages, RECT *pRects, RECT *pRectsMain, int nNumImages, int nSizeX, int nSizeY) const
{
  ...
  for (int i = 0; i != nNumImages; ++i)
  {
    ...
    // CRAP{ for images 1 pixel wide or long, expand by 1
    if ((pRects[i].left == pRects[i].right) && (pRects[i].left != 0)) pRects[i].right += 1;
    if ((pRects[i].top == pRects[i].bottom) && (pRects[i].top != 0)) pRects[i].bottom += 1;
    // CRAP}
  }
  memcpy(&rectsTemp[0], pRects, sizeof(RECT) * nNumImages);

  for (int i = 0; i != nNumImages; ++i) vTemp.push_back(SRectOptimizeStructure(pRects[i], i));

  vTemp2 = vTemp;
  std::sort(vTemp.begin(), vTemp.end(), CRectSortFunc());
  OptimizeRects(vTemp);
  for (int i = 0; i != nNumImages; ++i) pRects[vTemp[i].nNum] = vTemp[i].rRect;

  GRect texRect = GetFillRect(vTemp);
  auto pImage = new CImage(texRect.width(), texRect.height());
  pImage->Set(0);
  for (int i = 0; i < nNumImages; ++i)
  {
  ...
  // now we need to move the big rects
  for (int i = 0; i != nNumImages; ++i)
  {
