    ...
    if (pDBLCLK)
    {
      ...
      if (bPressed)
      {
        if ((pLastPressedKey == this) && (time <= dwLastTimePressed + TIME_DIFF_DBL_CLK) &&
          ...
          pDBLCLK->ChangeState(0x80, time, nParam, nullptr);
        dwLastTimePressed = time;
        vLastPos = vPos;
      }
      else pDBLCLK->ChangeState(0, time, nParam, nullptr);
    }
  }
  return bPressed;
}
...
static const SNameID kbdKeyMaps[] =
{
    ...
    {"LEFT", DIK_LEFT},// Left Arrow on arrow keypad
    {"RIGHT", DIK_RIGHT},// Right Arrow on arrow keypad
...
{
  if (kbdKeyMaps[nID].dwID == nID) return kbdKeyMaps[nID].pszName;
  //
  for (const SNameID *pDesc = kbdKeyMaps; pDesc->pszName != nullptr; ++pDesc) { if (pDesc->dwID == nID) return pDesc->pszName; }
  return "UNKNOWN_KEY";
}
...
struct SDeviceEnumDesc : SDeviceDesc
{
  bool bPoll;
...
{
  auto pDesc = reinterpret_cast<SDevicesEnumDesc *>(pvRef);
  ...
  if (((eDeviceType != DEVICE_TYPE_KEYBOARD) && (eDeviceType != DEVICE_TYPE_MOUSE)) ||
      (pDesc->devtypes[eDeviceType] >= 1)) { return DIENUM_CONTINUE; }
  // CRAP}
  //
...
BOOL CALLBACK DIEnumDeviceObjectsCallback(LPCDIDEVICEOBJECTINSTANCE lpddoi, LPVOID pvRef)
{
  ...
  auto pDesc = reinterpret_cast<SDeviceEnumDesc *>(pvRef);
  // determine type
  ...
  {
    eControlType = CONTROL_TYPE_KEY;
    if (pDesc->eType == DEVICE_TYPE_KEYBOARD) szControlName = GetKeyName(instance.dwOfs);
    else szControlName = NStr::Format("KEY%d", pDesc->cntrltypes[eControlType]);
    pGUID = &GUID_Key;
  }
  ...
  //
  if (eControlType == CONTROL_TYPE_UNKNOWN) return DIENUM_CONTINUE;
  //
  // composite control's name with the device's one
  if (pDesc->eType != DEVICE_TYPE_KEYBOARD) szControlName = pDesc->szName + "_" + szControlName;
  //
...
CInputAPI::CInputAPI()
{
  ...
  pLastControlKey = nullptr;
}

CInputAPI::~CInputAPI() { Done(); }

bool CInputAPI::Init(HWND _hWindow)
{
  // create input
  {
    IDirectInput8 *pTemp = nullptr;
    HRESULT dxrval = DirectInput8Create(GetModuleHandle(nullptr), DIRECTINPUT_VERSION, IID_IDirectInput8, (void **) &pTemp, nullptr);
  ...
  for (auto device = devdesc.devices.begin(); device != devdesc.devices.end(); ++device) AddDevice(&(*device), ++nDeviceCounter);
  //
  hWindow = _hWindow;
  bInitialized = true;
  // double click time
  ...
  {
    int nDelay = 0;// first input repeat delay. 
    ...
    TIME_DIFF_REPEAT_PERIOD = 1000.0f / (2.5f + (static_cast<float>(dwSpeed) / 31.0f) * (30.0f - 2.5f));
  }
  else
  {
...
void CInputAPI::AddDevice(SDeviceEnumDesc *pDesc, const int nID)
{
  ...
  HRESULT dxrval = pInput->CreateDevice(pDesc->guid, &pTempDevice, nullptr);
  ...
  // calc data set size
  DWORD dwDataSize = 0;
  for (auto object = pDesc->objects.begin(); object != pDesc->objects.end(); ++object) dwDataSize = Max(dwDataSize, object->dwOfs);
  dwDataSize += 4;
  if (dwDataSize & 3) dwDataSize = (dwDataSize & (~3UL)) + 4;
  ...
  for (int i = 0; i != pDesc->objects.size(); ++i)
  {
    ...
    if ((object.dwType & DIDFT_AXIS) == 0) continue;
    ...
    // prop.dwData = ( control.eType == CONTROL_TYPE_RAXIS ) ? 
...
{
  for (auto it = devices.begin(); it != devices.end(); ++it) { if (it->nID == nID) return &(*it); }
  return nullptr;
}
...
bool CInputAPI::AddDoubleClick(const std::string &szControlName)
{
  ...
  if ((pos == controlNames.end()) || (pos->second->GetType() != CONTROL_TYPE_KEY)) return false;
  //
...
{
  CControlNamesMap::iterator pos = controlNames.find(szControlName);
  if (pos == controlNames.end()) return false;
...
{
  if (bCoopLevelSet) return true;
  for (auto it = devices.begin(); it != devices.end(); ++it)
  {
    if (it->bEmulated) continue;
    // set coop level
    HRESULT dxrval;
    // In order for the keyboard to enter text normally, you need a non-exclusive mode
    if (it->eType == DEVICE_TYPE_KEYBOARD) dxrval = it->pDevice->SetCooperativeLevel(hWindow, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND);
    else dxrval = it->pDevice->SetCooperativeLevel(hWindow, DISCL_EXCLUSIVE | DISCL_FOREGROUND);
    //
...
// grab all the devices / give control over all the devices
bool CInputAPI::SetFocus(bool bFocus)
{
  if (!bInitialized) return false;
  if (bFocusCaptured == bFocus) return true;
  if (bFocus)
  {
    ...
    if (!SetCoopLevel()) return false;
    // try to grab all the devices under you
    for (auto it = devices.begin(); it != devices.end(); ++it)
    {
      if (it->bEmulated) continue;
      //
      HRESULT dxrval = it->pDevice->Acquire();
      if (dxrval == DIERR_OTHERAPPHASPRIO) return false;
      ...
      {
        for (auto control = it->controls.begin(); control != it->controls.end(); ++control)
        {
  ...
  {
    // give control over all devices
    for (CDevicesList::const_iterator it = devices.begin(); it != devices.end(); ++it) it->pDevice->Unacquire();
    // force reset all controls and commands
...
{
  for (auto it = devices.begin(); it != devices.end(); ++it) it->pDevice->Unacquire();
  //
  for (auto it = devices.begin(); it != devices.end(); ++it)
  {
...
{
  for (auto it = devices.begin(); it != devices.end(); ++it) { if (it->eType == eDeviceType) return it->bEmulated; }
  return false;
}
...
template<class TYPE>
bool GetMessage(std::list<TYPE> &messages, TYPE *pMsg)
{
...
bool CInputAPI::GetMessage(SGameMessage *pMsg) { return ::GetMessage(messages, pMsg); }
bool CInputAPI::GetTextMessage(STextMessage *pMsg) { return ::GetMessage(chars, pMsg); }
...
{
  bool operator()(const DIDEVICEOBJECTDATA &dido1, const DIDEVICEOBJECTDATA &dido2) const { return (dido1.dwSequence < dido2.dwSequence); }
};
...
bool CInputAPI::PumpMessagesLocal(bool bFocus)
{
  ...
  SetFocus(bFocus);
  if (!bFocusCaptured) return false;
  //
  for (auto it = activecontrols.begin(); it != activecontrols.end(); ++it) it->bActive = false;
  //
  ...
  for (auto it = devices.begin(); it != devices.end(); ++it)
  {
    ...
    if (it->bPoll)
    {
      ...
      if (FAILED(dxrval)) it->pDevice->Acquire();
    }
    ...
    if (dxrval != DI_OK)
    {
      ...
      while (dxrval == DIERR_INPUTLOST) dxrval = it->pDevice->Acquire();
      //
      it->bNeedResync = true;
      // dxrval may be DIERR_OTHERAPPHASPRIO or other errors.  
      // may occur when the app is minimized or in the process of
      // switching, so just try again later
      continue;
    }
    if (dxrval == DIERR_OTHERAPPHASPRIO) continue;
    NI_ASSERTHR_SLOW_TF(dxrval, "Can't get bufferred data from the device", return false);
    // process text input
    if (it->eType == DEVICE_TYPE_KEYBOARD)
    {
      if (eTextMode != INPUT_TEXT_MODE_NOTEXT) Convert2Text(pDIDOs, dwElements);
      if (eTextMode == INPUT_TEXT_MODE_TEXTONLY) dwElements = 0;
    }
    // due to controls from different devices can have equal IDs, we need to add an device ID to this controls
    for (int i = 0; i != dwElements; ++i) pDIDOs[i].dwOfs |= (it->nID << 16);
    //
  ...
  //
  for (std::vector<DIDEVICEOBJECTDATA>::const_iterator it = events.begin(); it != events.end(); ++it) EventCame(&(*it), 0);
  for (std::deque<DIDEVICEOBJECTDATA>::const_iterator it = emulatedMessages.begin(); it != emulatedMessages.end(); ++it) EventCame(&(*it), it->dwSequence);
  emulatedMessages.clear();
  // check for activate/deactivate controls
  for (auto it = activecontrols.begin(); it != activecontrols.end();)
  {
    if (!it->bActive)
    {
      ...
      if (!it->pControl->IsActive()) it = activecontrols.erase(it);
      else
      {
...
{
  BYTE keys[256];
  if (GetKeyboardState(keys) == 0) memset(keys, 0, sizeof(keys));
...
void CInputAPI::EventCame(const DIDEVICEOBJECTDATA *pEvent, const int nParam)
{
  ...
  // for POV dwData is expressed in hundredths of a degree. 
  if (pControl->GetType() == CONTROL_TYPE_POV_X)
  {
    const float fAngle = static_cast<float>(pEvent->dwData) / 36000.0f * FP_2PI;
    ...
    // add as POV_X
    if (pControl->ChangeState(fAxisX, pEvent->dwTimeStamp, nParam, pLastControlKey)) AddActiveControl(pControl);
    // add as POV_Y
    if (CControl *pControlY = controlIDs[pControl->GetID() | 0x8000]) { if (pControlY->ChangeState(fAxisY, pEvent->dwTimeStamp, nParam, pLastControlKey)) AddActiveControl(pControlY); }
  }
  else if (pControl->ChangeState(pEvent->dwData, pEvent->dwTimeStamp, nParam, pLastControlKey))
  {
    AddActiveControl(pControl);
    if (pControl->GetType() == CONTROL_TYPE_KEY) pLastControlKey = pControl;
  }
}

void CInputAPI::AddActiveControl(CControl *pControl)
{
  for (auto it = activecontrols.begin(); it != activecontrols.end(); ++it)
  {
...
{
  if ((eTextMode == INPUT_TEXT_MODE_NOTEXT)) return;
...
{
  for (CControlIDsMap::iterator it = controlIDs.begin(); it != controlIDs.end(); ++it) { if (it->second) pVisitor->VisitControl(it->second); }
}
