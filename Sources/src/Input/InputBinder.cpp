...
{
  if (szName == "event down") return INPUT_BIND_ACTIVATION_TYPE_EVENT_DOWN;
  if (szName == "event up") return INPUT_BIND_ACTIVATION_TYPE_EVENT_UP;
  if (szName == "slider plus") return INPUT_BIND_ACTIVATION_TYPE_SLIDER_PLUS;
  if (szName == "slider minus") return INPUT_BIND_ACTIVATION_TYPE_SLIDER_MINUS;
  //
...
{
  if (bActivated) { for (auto it = combos.begin(); it != combos.end(); ++it) (*it)->NotifyControlStateChanged(bActivated, time, nParam); }
  else { for (auto it = combos.rbegin(); it != combos.rend(); ++it) (*it)->NotifyControlStateChanged(bActivated, time, nParam); }
}

struct SCombosCmp
{
  const bool operator()(const CObj<CCombo> &c1, const CObj<CCombo> &c2) const { return c1->GetNumControls() > c2->GetNumControls(); }
};
...
{
  for (auto it = combos.begin(); it != combos.end(); ++it) (*it)->ChangeMappingSection(szName);
}
...
CCombo::CCombo(const CControlsPtrList &_controls)
  : pMapping(nullptr), nSuppressCounter(0), bFormed(false), bLastNotifyFormed(false), fPower(0), nBestParam(0)
{
  controls = _controls;
  eControlType = CONTROL_TYPE_KEY;
  for (CControlsPtrList::const_iterator it = controls.begin(); it != controls.end(); ++it) eControlType = Max(eControlType, (*it)->GetType());
}
...
{
  for (auto it = suppressives.begin(); it != suppressives.end(); ++it) (*it)->Suppress(nAdd, time);
}
...
void CCombo::Suppress(const int nAdd, DWORD time)
{
  ...
  if ((nSuppressCounter == 0) && bFormed) NotifyBinds(false, time);
  // change suppression counter
  nSuppressCounter += nAdd;
  // notify about this combo reactivation (unsuppressed)
  if (nSuppressCounter == 0) NotifyBinds(bFormed, time);
}

void CCombo::NotifyBinds(const bool bFormedLocal, const DWORD time, const bool bForced)
{
  if ((bLastNotifyFormed != bFormedLocal) || bForced) { for (auto it = pMapping->binds.begin(); it != pMapping->binds.end(); ++it) (*it)->NotifyComboStateChanged(bFormedLocal, fPower, eControlType, time, nBestParam); }
...
void CCombo::NotifyControlStateChanged(const bool bActivated, const DWORD time, const int nParam)
{
  ...
  if (!IsBinded()) return;
  //
  if (nParam & 0x40000000) nBestParam = nParam;
  ...
  for (CControlsPtrList::const_iterator it = controls.begin(); it != controls.end(); ++it)
  {
    ...
    if (fabsf(fPower) == fabsf(fValue)) fPower = Min(fPower, fValue);
    else if (fabsf(fPower) < fabsf(fValue)) fPower = fValue;
  }
  if (bFormedLocal != bFormed)
  {
    ...
    // notify all binds
    if (nSuppressCounter == 0) NotifyBinds(bFormedLocal, time);
  }
  else if (bFormedLocal && (nSuppressCounter == 0))// re-activation with a different value is only possible for 'rotational axis'
    NotifyBinds(bFormedLocal, time, true);
  bFormed = bFormedLocal;
  //
  if (!bActivated) nBestParam = 0;
}
...
void CCombo::AddBind(CBind *pBind)
{
  ...
  else { NI_ASSERT_T(false, "Bind already exist in the mapping!"); }
}

// is c1 subset of c2
inline bool IsSubset(const std::vector<const CControl *> &c1, const std::vector<const CControl *> &c2)
{
  for (auto it = c1.begin(); it != c1.end(); ++it) { if (std::find(c2.begin(), c2.end(), *it) == c2.end()) return false; }
  return true;
}

inline const ESetCompare Compare(const std::vector<const CControl *> &c1, const std::vector<const CControl *> &c2)
{
  if (c1.size() == c2.size()) return IsSubset(c1, c2) ? SET_COMPARE_EQUAL : SET_COMPARE_UNRELATED;
  if (c1.size() < c2.size()) return IsSubset(c1, c2) ? SET_COMPARE_SUBSET : SET_COMPARE_UNRELATED;
...
void CBindEventForm::NotifyComboStateChanged(const bool bFormed, const float fPower, const EControlType eControlType,
                                             const DWORD time, const int nParam) { if (bFormed) pCommand->ActivateEvent(time, nParam); }
...
void CBindSliderPlus::NotifyComboStateChanged(const bool bFormed, const float fPower, const EControlType eControlType,
                                              const DWORD time, const int nParam) { pCommand->ActivateSlider(bFormed, fPower, eControlType, time, nParam); }
...
{
  if ((pInput->GetTextModeLocal() == INPUT_TEXT_MODE_SYSKEYS) && !bSystem) return;

  const char *pszType = nullptr;
  switch (eControlType)
  {
    case CONTROL_TYPE_KEY:
      if (bActivate) accumulator.ActivateKey(time, fPower);
      else accumulator.DeactivateKey(time);
      pszType = "key";
      break;
    case CONTROL_TYPE_AXIS:
      if (bActivate) accumulator.ActivateAxis(time, fPower);
      else accumulator.DeactivateAxis(time);
      pszType = "axis";
      break;
    case CONTROL_TYPE_RAXIS:
    case CONTROL_TYPE_POV_X:
    case CONTROL_TYPE_POV_Y:
      if (bActivate) accumulator.ActivateRAxis(time, fPower);
      else accumulator.DeactivateRAxis(time);
...
CInputBinder::CInputBinder()
  : szCurrentMapping("default") { bMappingChanged = false; }

template<class TYPE>
void Add(std::list<TYPE> &lst, TYPE element) { if (std::find(lst.begin(), lst.end(), element) == lst.end()) lst.push_back(element); }
...
void CInputBinder::TransformBind(SBindsConfig::SBindSection::SCommandBind *pRes, const IInputBind *pBind)
{
  ...
  for (int i = 0; i < nNumControls; ++i) pRes->controls.push_back(pBind->GetControl(i));
}
...
void CInputBinder::TransformBind(IInputBind *pRes, const SBindsConfig::SBindSection::SCommandBind *pBind)
{
  ...
  for (auto it = pBind->controls.begin(); it != pBind->controls.end(); ++it) pRes->AddControl(it->c_str());
}
...
void CInputBinder::AddBind(const IInputBind *pInputBind)
{
  ...
  {
    SBindsConfig::SBindSection *pSection = nullptr;
    for (auto it = config.sections.begin(); it != config.sections.end(); ++it)
    {
    ...
    for (auto cmd = pSection->commands.begin(); cmd != pSection->commands.end(); ++cmd)
    {
...
{
  if (bind.szName.empty() || bind.szBindType.empty() || bind.controls.empty()) return false;
  //
  ...
  for (auto it = bind.controls.begin(); it != bind.controls.end(); ++it)
  {
  ...
  if (pCombo == nullptr) pCombo = new CCombo(controls);
  pCombo->ChangeMappingSection(szCurrentMapping);
  // add this combo to all controls
  for (auto it = controls.begin(); it != controls.end(); ++it) const_cast<CControl *>((*it))->AddCombo(pCombo);
  // add all subset combos to suppress by this one
  for (auto it = subsets.begin(); it != subsets.end(); ++it) pCombo->AddSuppressive(*it);
  // add this combo to all supersets to suppress
  for (auto it = supersets.begin(); it != supersets.end(); ++it) (*it)->AddSuppressive(pCombo);
  // create new bind...
  CBind *pLocalBind = nullptr;
...
void CInputBinder::RemoveBind(const IInputBind *pBind)
{
  ...
  for (int i = 0; i < pBind->GetNumControls(); ++i)
  {
    ...
    if (pControl != nullptr) controls.push_back(pControl);
    else
    {
  ...
  if (pos == commands.end()) return;
  //
  SCommand *pCommand = &(pos->second);
  CFindBindVisitor visitor(pCommand, eType, controls);
  VisitControls(&visitor);
  if ((visitor.GetBind() == nullptr) || (visitor.GetCombo() == nullptr)) return;
  //
...
void CInputBinder::UnRegisterCommand(const char *pszName)
{
  ...
  if (pos != commands.end()) pos->second.Register(false);
}

IInputSlider *CInputBinder::CreateSlider(const char *pszName, const float fPower) { return new CInputSlider(this, GetCommand(pszName), fPower); }
...
void CInputBinder::Repair(IDataTree *pSS, const bool bToDefault)
{
  ...
  const SBindsConfig::SBindSection *pDefaultSection = nullptr;
  ...
  for (std::vector<SBindsConfig::SBindSection>::const_iterator itRepairSect = repairs.sections.begin(); itRepairSect != repairs.sections.end(); ++itRepairSect)
  {
    ...
    // add this section entirely, if not found in the config
    if (!bFoundSection)
    {
      // set bind section
      SetBindSection(itRepairSect->szName.c_str());
      // add this section bind
      for (auto bind = itRepairSect->commands.begin(); bind != itRepairSect->commands.end(); ++bind) AddBindLocal(*bind);
      // add default section binds
      if (pDefaultSection) { for (auto bind = pDefaultSection->commands.begin(); bind != pDefaultSection->commands.end(); ++bind) AddBindLocal(*bind); }
      else if (itRepairSect->szName == "default") pDefaultSection = &(*itRepairSect);
      //
...
{
  if (pSS == nullptr) return false;
  //
  CTreeAccessor saver = pSS;
  if (saver.IsReading()) config.Clear();
  saver.Add("Binds", &config);
  if (saver.IsReading())
  {
    // setup from config
    // double clicks
    for (std::vector<std::string>::const_iterator it = config.dblclks.begin(); it != config.dblclks.end(); ++it) AddDoubleClick(*it);
    // powers
    for (std::vector<SBindsConfig::SControlPower>::const_iterator it = config.powers.begin(); it != config.powers.end(); ++it) SetPower(it->szControlName, it->fPower);
    // binds
    const SBindsConfig::SBindSection *pDefaultSection = nullptr;
    for (std::vector<SBindsConfig::SBindSection>::const_iterator section = config.sections.begin(); section != config.sections.end(); ++section)
    {
      ...
      // add this section bind
      for (auto bind = section->commands.begin(); bind != section->commands.end(); ++bind) AddBindLocal(*bind);
      // add default section binds
      if (pDefaultSection) { for (auto bind = pDefaultSection->commands.begin(); bind != pDefaultSection->commands.end(); ++bind) AddBindLocal(*bind); }
    ...
    // system commands
    for (std::vector<std::string>::const_iterator it = config.syscmds.begin(); it != config.syscmds.end(); ++it) SetSystemCommand(*it);
  }
