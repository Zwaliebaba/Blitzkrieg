...
TObject *luaA_index(lua_State *L, int index) { return Index(L, index); }
...
LUA_API int lua_stackspace(lua_State *L) { return (L->stack_last - L->top); }
...
LUA_API int lua_gettop(lua_State *L) { return (L->top - L->Cbase); }


LUA_API void lua_settop(lua_State *L, int index)
{
  if (index >= 0) luaD_adjusttop(L, L->Cbase, index);
  else L->top = L->top + index + 1;/* index is negative */
}
...
LUA_API void lua_pushcclosure(lua_State *L, lua_CFunction fn, int n) { luaV_Cclosure(L, fn, n); }


LUA_API void lua_pushusertag(lua_State *L, void *u, int tag)
{
  /* ORDER LUA_T */
  if (!(tag == LUA_ANYTAG || tag == LUA_TUSERDATA || validtag(tag))) luaO_verror(L, "invalid tag for a userdata (%d)", tag);
...
{
  int ref;
  if (ttype(L->top-1) == LUA_TNIL) ref = LUA_REFNIL;
  else
  {
    if (L->refFree != NONEXT)
    {
...
LUA_API void lua_rawcall(lua_State *L, int nargs, int nresults) { luaD_call(L, L->top - (nargs + 1), nresults); }
...
LUA_API int lua_getgcthreshold(lua_State *L) { return GCscale(L->GCthreshold); }

LUA_API int lua_getgccount(lua_State *L) { return GCscale(L->nblocks); }

LUA_API void lua_setgcthreshold(lua_State *L, int newthreshold)
{
  if (newthreshold > GCscale(ULONG_MAX)) L->GCthreshold = ULONG_MAX;
  else L->GCthreshold = GCunscale(newthreshold);
  luaC_checkGC(L);
}
...
LUA_API int lua_getn(lua_State *L, int index)
{
  ...
  if (ttype(value) == LUA_TNUMBER) return (int) nvalue(value);
  Number max = 0;
  int i = h->size;
  Node *n = h->node;
  while (i--)
