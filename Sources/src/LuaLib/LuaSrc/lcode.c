...
void luaK_error(LexState *ls, const char *msg) { luaX_error(ls, msg, ls->t.token); }
...
static void luaK_fixjump(FuncState *fs, int pc, int dest)
{
  ...
  else
  {
    ...
    if (abs(offset) > MAXARG_S) luaK_error(fs->ls, "control structure too long");
    SETARG_S(*jmp, offset);
  }
}
...
void luaK_deltastack(FuncState *fs, int delta)
{
  ...
  {
    if (fs->stacklevel > MAXSTACK) luaK_error(fs->ls, "function or expression too complex");
    fs->f->maxstacksize = fs->stacklevel;
  }
}


void luaK_kstr(LexState *ls, int c) { luaK_code1(ls->fs, OP_PUSHSTRING, c); }
...
static int number_constant(FuncState *fs, Number r)
{
  ...
  while (--c >= lim) if (f->knum[c] == r) return c;
...
{
  if (f <= (Number) MAXARG_S && (Number) (int) f == f) luaK_code1(fs, OP_PUSHINT, (int) f);/* f has a short integer value */
...
int luaK_lastisopen(FuncState *fs)
{
  ...
  if (GET_OPCODE(i) == OP_CALL && GETARG_B(i) == MULT_RET) return 1;
  return 0;
}
...
static void discharge1(FuncState *fs, expdesc *var)
{
  ...
  if (var->u.l.t == NO_JUMP && var->u.l.f == NO_JUMP) luaK_setcallreturns(fs, 1);/* call must return 1 value */
}
...
void luaK_patchlist(FuncState *fs, int list, int target)
{
  ...
  else luaK_patchlistaux(fs, list, target, OP_END, 0);
}
...
static int need_value(FuncState *fs, int list, OpCode hasvalue)
{
  ...
  for (; list != NO_JUMP; list = luaK_getjump(fs, list)) if (GET_OPCODE(fs->f->code[list]) != hasvalue) return 1;
  return 0;/* not found */
}
...
static void luaK_testgo(FuncState *fs, expdesc *v, int invert, OpCode jump)
{
  ...
  if (!ISJUMP(GET_OPCODE(*previous))) prevpos = luaK_code1(fs, jump, NO_JUMP);
  else
  {
...
void luaK_goiftrue(FuncState *fs, expdesc *v, int keepvalue) { luaK_testgo(fs, v, 1, keepvalue ? OP_JMPONF : OP_JMPF); }


static void luaK_goiffalse(FuncState *fs, expdesc *v, int keepvalue) { luaK_testgo(fs, v, 0, keepvalue ? OP_JMPONT : OP_JMPT); }
...
void luaK_tostack(LexState *ls, expdesc *v, int onlyone)
{
  ...
  if (!discharge(fs, v))
  {
    ...
    if (!ISJUMP(previous) && v->u.l.f == NO_JUMP && v->u.l.t == NO_JUMP)
    {
      ...
      if (onlyone) luaK_setcallreturns(fs, 1);/* call must return 1 value */
    }
    else
    {
          ...
          || need_value(fs, v->u.l.t, OP_JMPONT))
      {
        ...
        if (ISJUMP(previous)) luaK_concat(fs, &v->u.l.t, fs->pc - 1);/* put `previous' in t. list */
        else
        {
...
static void codelineinfo(FuncState *fs)
{
  ...
  if (ls->lastline > fs->lastline)
  {
    ...
    if (ls->lastline > fs->lastline + 1) f->lineinfo[f->nlineinfo++] = -(ls->lastline - (fs->lastline + 1));
...
int luaK_code0(FuncState *fs, OpCode o) { return luaK_code2(fs, o, 0, 0); }


int luaK_code1(FuncState *fs, OpCode o, int arg1) { return luaK_code2(fs, o, arg1, 0); }
...
int luaK_code2(FuncState *fs, OpCode o, int arg1, int arg2)
{
  ...
  switch (o)
  {
    ...
    case OP_PUSHNIL:
    {
      ...
      switch (GET_OPCODE(i))
      {
        case OP_PUSHNIL: SETARG_U(i, GETARG_U(i)+arg1);
          optm = 1;
          break;
        default:
          break;
    ...
    case OP_POP:
    {
      ...
      switch (GET_OPCODE(i))
      {
        case OP_SETTABLE: SETARG_B(i, GETARG_B(i)+arg1);
          optm = 1;
          break;
        default:
          break;
    ...
    case OP_ADD:
    {
      switch (GET_OPCODE(i))
      {
        case OP_PUSHINT: SET_OPCODE(i, OP_ADDI);
          optm = 1;
          break;/* `a+k' */
        default:
          break;
