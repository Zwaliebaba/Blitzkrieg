...
static int isLmark(StkId o) { return (o && ttype(o) == LUA_TMARK && !infovalue(o)->func->isC); }
...
{
  int refi;
  if (lineinfo == NULL || pc == -1) return -1;/* no line info or function is not active */
  refi = prefi ? *prefi : 0;
  if (lineinfo[refi] < 0) refline += -lineinfo[refi++];
  ...
  for (;;)
  {
    ...
    int nextref = refi + 1;
    if (lineinfo[nextref] < 0) nextline += -lineinfo[nextref++];
    LUA_ASSERT(lineinfo[nextref] >= 0, "invalid line info");
    if (lineinfo[nextref] > pc) break;
    refline = nextline;
    refi = nextref;
  }
  if (prefi) *prefi = refi;
...
static int currentpc(StkId f)
{
  ...
  if (ci->pc) return (*ci->pc - ci->func->f.l->code) - 1;
  return -1;
...
{
  if (!isLmark(f)) return -1;/* only active lua functions have current-line information */
  CallInfo *ci = infovalue(f);
...
static Proto *getluaproto(StkId f) { return (isLmark(f) ? infovalue(f)->func->f.l : NULL); }
...
static void funcinfo(lua_State *L, lua_Debug *ar, StkId func)
{
  ...
  if (ar->linedefined == 0) ar->what = "main";
}
  ...
  if (ttype(o) == LUA_TFUNCTION)
  {
    ...
    for (e = 0; e < TM_N; e++)
    {
      ...
      for (t = 0; t <= L->last_tag; t++)
        if (clvalue(o) == luaT_gettm(L, t, e)) return luaT_eventname[e];
    }
  }
  return NULL;
}
...
static void getname(lua_State *L, StkId f, lua_Debug *ar)
{
  ...
  if ((ar->name = travglobals(L, &o)) != NULL) ar->namewhat = "global";
    /* not found: try tag methods */
  else if ((ar->name = travtagmethods(L, &o)) != NULL) ar->namewhat = "tag-method";
  ...
  for (; *what; what++)
  {
    switch (*what)
    {
      ...
      case 'n':
      {
        ...
        if (ar->namewhat == NULL) getname(L, func, ar);
        break;
      }
      case 'f':
      {
  ...
  while (pc < lastpc)
  {
    ...
    switch (GET_OPCODE(i))
    {
      ...
      case OP_JMPONF:
      {
        ...
        else top--;/* do not jump; pop value */
        break;
      }
      default:
      {
...
static const char *getobjname(lua_State *L, StkId obj, const char **name)
{
  ...
  if (!isLmark(func)) return NULL;/* not an active Lua function */
...
void luaG_typeerror(lua_State *L, StkId o, const char *op)
{
  ...
  if (kind)
    luaO_verror(L, "attempt to %.30s %.20s `%.40s' (a %.10s value)",
                op, kind, name, t);
  else luaO_verror(L, "attempt to %.30s a %.10s value", op, t);
}
...
void luaG_ordererror(lua_State *L, StkId top)
{
  ...
  if (t1[2] == t2[2]) luaO_verror(L, "attempt to compare two %.10s values", t1);
  else luaO_verror(L, "attempt to compare %.10s with %.10s", t1, t2);
}