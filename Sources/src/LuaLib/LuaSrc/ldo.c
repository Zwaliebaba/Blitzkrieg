...
static void restore_stack_limit(lua_State *L) { if (L->top - L->stack < L->stacksize - 1) L->stack_last = L->stack + (L->stacksize - 1); }
...
void luaD_call(lua_State *L, StkId func, int nResults)
{
  ...
  if (ttype(func) != LUA_TFUNCTION)
  {
    ...
    if (tm == NULL) luaG_typeerror(L, func, "call");
  ...
  callhook = L->callhook;
  if (callhook) luaD_callHook(L, func, callhook, "call");
  ...
  else
  {
    ...
    for (; nResults > 0 && firstResult < L->top; nResults--) *func++ = *firstResult++;
    L->top = func;
...
void luaD_breakrun(lua_State *L, int errcode)
{
  ...
  if (errcode != LUA_ERRMEM) message(L, "unable to recover; exiting\n");
  exit(EXIT_FAILURE);
}
