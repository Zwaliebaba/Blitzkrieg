...
void luaX_error(LexState *ls, const char *s, int token)
{
  ...
  if (buff[0] == '\0') luaX_syntaxerror(ls, s, ls->L->Mbuffer);
  else luaX_syntaxerror(ls, s, buff);
}
...
void luaX_token2str(int token, char *s)
{
  ...
  else strcpy(s, token2string[token - FIRST_RESERVED]);
}
...
static void read_number(LexState *LS, int comma, SemInfo *seminfo)
{
  ...
  save(L, '\0', l);
  if (!luaO_str2d(L->Mbuffer, &seminfo->r)) luaX_error(LS, "malformed number", TK_NUMBER);
}
  ...
  for (;;)
  {
    switch (LS->current)
    {
      ...
      default:
        ...
        {
          int c = LS->current;
          if (iscntrl(c)) luaX_invalidchar(LS, c);
          next(LS);
          return c;
        }
      tname:
