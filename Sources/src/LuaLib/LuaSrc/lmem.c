...
static void *debug_realloc(void *block, size_t size)
{
  ...
  if (memdebug_total + size > memdebug_memlimit) return NULL;/* to test memory allocation errors */
  ...
  *(unsigned long *) newblock = size;
  for (i = 0; i < MARKSIZE; i++) *(newblock + HEADER + size + i) = (char) (MARK + i);
...
void *luaM_realloc(lua_State *L, void *block, lint32 size)
{
  ...
  if (size >= MAX_SIZET) lua_error(L, "memory allocation error: block too big");
  block = realloc(block, size);

  if (block == NULL)
  {
    if (L) luaD_breakrun(L, LUA_ERRMEM);/* break run without error message */
