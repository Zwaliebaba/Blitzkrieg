...
/* ** most `malloc' libraries allocate memory in blocks of 8 bytes.  */
...
typedef struct TString
{
  ...
  int marked;
  char str[TSPACK];/* variable length string!!  */
} TString;
...
{
  Number *knum;/* Number used numbers by the function */
...
typedef struct Hash
{
  ...
  int htag;
  int size;
  Node *firstfree;/* this position is free;  */
  struct Hash *next;
