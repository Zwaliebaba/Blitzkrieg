...
static void next(LexState *ls)
{
  ...
  else ls->t.token = luaX_lex(ls, &ls->t.seminfo);/* read next token */
}
...
static void check_condition(LexState *ls, int c, const char *msg) { if (!c) luaK_error(ls, msg); }
...
static void code_string(LexState *ls, TString *s) { luaK_kstr(ls, string_constant(ls->fs, s)); }
...
static int checkname(LexState *ls) { return string_constant(ls->fs, str_checkname(ls)); }
...
{
  FuncState *fs = ls->fs;
  while (nvars--) fs->f->locvars[fs->actloc[fs->nactloc++]].startpc = fs->pc;
}
...
{
  FuncState *fs = ls->fs;
  while (nvars--) fs->f->locvars[fs->actloc[--fs->nactloc]].endpc = fs->pc;
}


static void new_localvarstr(LexState *ls, const char *name, int n) { new_localvar(ls, luaS_newfixed(ls->L, name), n); }
...
static int indexupvalue(LexState *ls, expdesc *v)
{
  FuncState *fs = ls->fs;
  int i;
  for (i = 0; i < fs->nupvalues; i++) { if (fs->upvalues[i].k == v->k && fs->upvalues[i].u.index == v->u.index) return i; }
  /* new one */
...
static void pushupvalue(LexState *ls, TString *n)
{
  ...
  {
    /* global? */
    if (fs->prev == NULL) luaX_syntaxerror(ls, "cannot access upvalue in main", n->str);
...
static void pushclosure(LexState *ls, FuncState *func)
{
  ...
  Proto *f = fs->f;
  int i;
  for (i = 0; i < func->nupvalues; i++) luaK_tostack(ls, &func->upvalues[i], 1);
...
static int recfields(LexState *ls)
{
  ...
  {
    next(ls);
    if (ls->t.token == ';' || ls->t.token == '}') break;
    recfield(ls);
    n++;
    if (n % RFIELDS_PER_FLUSH == 0) luaK_code1(fs, OP_SETMAP, RFIELDS_PER_FLUSH);
  }
...
static int listfields(LexState *ls)
{
  ...
  while (ls->t.token == ',')
  {
    ...
    if (n % LFIELDS_PER_FLUSH == 0) luaK_code2(fs, OP_SETLIST, n / LFIELDS_PER_FLUSH - 1, LFIELDS_PER_FLUSH);
  }
...
static int assignment(LexState *ls, expdesc *v, int nvars)
{
  ...
  if (v->k != VINDEXED) luaK_storevar(ls, v);
  else
  {
...
static void fornum(LexState *ls, TString *varname)
{
  ...
  check(ls, ',');
  exp1(ls);/* limit */
  if (optional(ls, ',')) exp1(ls);/* optional step */
  else luaK_code1(fs, OP_PUSHINT, 1);/* default step */
...
static void ifstat(LexState *ls, int line)
{
  ...
  else luaK_concat(fs, &escapelist, v.u.l.f);
...
static void localstat(LexState *ls)
{
  ...
  if (optional(ls, '=')) nexps = explist1(ls);
  else nexps = 0;
  adjust_mult_assign(ls, nvars, nexps);
...
static void retstat(LexState *ls)
{
  ...
  FuncState *fs = ls->fs;
  next(ls);/* skip RETURN */
  if (!block_follow(ls->t.token)) explist1(ls);/* optional return values */
...
static void breakstat(LexState *ls)
{
  ...
  Breaklabel *bl = fs->bl;
  if (!bl) luaK_error(ls, "no loop to break");
  next(ls);/* skip BREAK */
