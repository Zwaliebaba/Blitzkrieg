...
static unsigned long hash_s(const char *s, size_t l)
{
  ...
  for (; l >= step; l -= step) h = h ^ ((h << 5) + (h >> 2) + (unsigned char) *(s++));
  return h;
}
...
TString *luaS_newlstr(lua_State *L, const char *str, size_t l)
{
  ...
  TString *ts;
  for (ts = L->strt.hash[h1]; ts; ts = ts->nexthash) { if (ts->len == l && (memcmp(str, ts->str, l) == 0)) return ts; }
  /* not found */
...
{
  union L_UTString *uts = luaM_malloc(L,
...
TString *luaS_createudata(lua_State *L, void *udata, int tag)
{
  ...
  TString *ts;
  for (ts = L->udt.hash[h1]; ts; ts = ts->nexthash) { if (udata == ts->u.d.value && (tag == ts->u.d.tag || tag == LUA_ANYTAG)) return ts; }
  /* not found */
  ts = luaS_newudata(L, 0, udata);
  if (tag != LUA_ANYTAG) ts->u.d.tag = tag;
  return ts;
}


TString *luaS_new(lua_State *L, const char *str) { return luaS_newlstr(L, str, strlen(str)); }
