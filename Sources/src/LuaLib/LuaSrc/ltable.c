                                  ...
                                  const TObject *key)
{
  ...
  if (!n) lua_error(L, "table index is nil");
  else
    do
    {
      if (luaO_equalObj(key, &n->key)) return &n->val;
      n = n->next;
    } while (n);
...
const TObject *luaH_getnum(const Hash *t, Number key)
{
  ...
  do
  {
    if (ttype(&n->key) == LUA_TNUMBER && nvalue(&n->key) == key) return &n->val;
    n = n->next;
  } while (n);
  return &luaO_nilobject;/* key not found */
}
...
const TObject *luaH_getstr(const Hash *t, TString *key)
{
  ...
  do
  {
    if (ttype(&n->key) == LUA_TSTRING && tsvalue(&n->key) == key) return &n->val;
    n = n->next;
  } while (n);
  return &luaO_nilobject;/* key not found */
}
...
{
  int i;
  if (ttype(key) == LUA_TNIL) i = 0;/* first iteration */
  else
  {
    const TObject *v = luaH_get(L, t, key);
    if (v == &luaO_nilobject) lua_error(L, "invalid key for `next'");
    i = (int) (((const char *) v -
  ...
  {
    Node *n = node(t, i);
    if (ttype(val(n)) != LUA_TNIL) return n;
  }
...
void luaH_remove(Hash *t, TObject *key)
{
  ...
  {
    if (n >= MAX_INT - t->size) return;/* give up; (to avoid overflow) */
    n += t->size;
  }
...
{
  int i;
  if (size > MAX_INT) lua_error(L, "table overflow");
...
static int numuse(const Hash *t)
{
  ...
  int realuse = 0;
  int i;
  for (i = 0; i < size; i++) { if (ttype(&v[i].val) != LUA_TNIL) realuse++; }
  return realuse;
}
...
static void rehash(lua_State *L, Hash *t)
{
  ...
  else setnodevector(L, t, oldsize);
  for (i = 0; i < oldsize; i++)
  {
    Node *old = nold + i;
    if (ttype(&old->val) != LUA_TNIL) *luaH_set(L, t, &old->key) = old->val;
  }
...
TObject *luaH_set(lua_State *L, Hash *t, const TObject *key)
{
  Node *mp = luaH_mainposition(t, key);
  Node *n = mp;
  if (!mp) lua_error(L, "table index is nil");
  do
  {
    /* check whether `key' is somewhere in the chain */
    if (luaO_equalObj(key, &n->key)) return &n->val;/* that's all */
    n = n->next;
  } while (n);
  /* `key' not found; must insert it */
  if (ttype(&mp->key) != LUA_TNIL)
  {
  ...
  {
    /* correct `firstfree' */
    if (ttype(&t->firstfree->key) == LUA_TNIL) return &mp->val;/* OK; table still has a free place */
    if (t->firstfree == t->node) break;/* cannot decrement from here */
    (t->firstfree)--;
  }
...
const TObject *luaH_getglobal(lua_State *L, const char *name) { return luaH_getstr(L->gt, luaS_new(L, name)); }