...
static void unexpectedEOZ(lua_State *L, ZIO *Z) { luaO_verror(L, "unexpected end of file in `%.99s'", ZNAME(Z)); }
...
static void LoadConstants(lua_State *L, Proto *tf, ZIO *Z, int swap)
{
  ...
  for (i = 0; i < n; i++) tf->kstr[i] = LoadString(L, Z, swap);
  ...
  for (i = 0; i < n; i++) tf->kproto[i] = LoadFunction(L, Z, swap);
}
...
static Proto *LoadChunk(lua_State *L, ZIO *Z) { return LoadFunction(L, Z, LoadHeader(L, Z)); }
...
Proto *luaU_undump(lua_State *L, ZIO *Z)
{
  Proto *tf = NULL;
  int c = zgetc(Z);
  if (c == ID_CHUNK) tf = LoadChunk(L, Z);
  c = zgetc(Z);
  if (c != EOZ) luaO_verror(L, "`%.99s' apparently contains more than one chunk", ZNAME(Z));
  return tf;
}
