...
{
  if (ttype(obj) != LUA_TSTRING) return 1;
  if (!luaO_str2d(svalue(obj), &nvalue(obj))) return 2;
...
{
  /* LUA_NUMBER */
  if (ttype(obj) != LUA_TNUMBER) return 1;
...
static Closure *luaV_closure(lua_State *L, int nelems)
{
  ...
  L->top -= nelems;
  while (nelems--) c->upvalue[nelems] = *(L->top + nelems);
  clvalue(L->top) = c;
...
const TObject *luaV_gettable(lua_State *L, StkId t)
{
  ...
  if (ttype(t) == LUA_TTABLE &&/* `t' is a table? */
      ((tg = hvalue(t)->htag) == LUA_TTABLE ||/* with default tag? */
       luaT_gettm(L, tg, TM_GETTABLE) == NULL))
  {
    /* or no TM? */
    /* do a primitive get */
    const TObject *h = luaH_get(L, hvalue(t), L->top - 1);
    /* result is no nil or there is no `index' tag method? */
    if (ttype(h) != LUA_TNIL || ((tm = luaT_gettm(L, tg, TM_INDEX)) == NULL)) return h;/* return result */
...
{
  if (ttype(l) == LUA_TNUMBER && ttype(r) == LUA_TNUMBER) return (nvalue(l) < nvalue(r));
  if (ttype(l) == LUA_TSTRING && ttype(r) == LUA_TSTRING) return (luaV_strcomp(tsvalue(l), tsvalue(r)) < 0);
  /* call TM */
  luaD_checkstack(L, 2);
  *top++ = *l;
  *top++ = *r;
  if (!call_binTM(L, top, TM_LT)) luaG_ordererror(L, top - 2);
  L->top--;
...
void luaV_strconc(lua_State *L, int total, StkId top)
{
  do
  {
    ...
    if (tostring(L, top-2) || tostring(L, top-1)) { if (!call_binTM(L, top, TM_CONCAT)) luaG_binerror(L, top - 2, LUA_TSTRING, "concat"); }
...
static void luaV_pack(lua_State *L, StkId firstelem)
{
  ...
  for (i = 0; firstelem + i < L->top; i++) *luaH_setint(L, htab, i + 1) = *(firstelem + i);
...
static void adjust_varargs(lua_State *L, StkId base, int nfixargs)
{
  ...
  if (nvararg < 0) luaD_adjusttop(L, base, nfixargs);
...
StkId luaV_execute(lua_State *L, const Closure *cl, StkId base)
{
  ...
  else luaD_adjusttop(L, base, tf->numparams);
  top = L->top;
  /* main loop of interpreter */
  for (;;)
  {
    const Instruction i = *pc++;
    if (linehook) traceexec(L, base, top, linehook);
    switch (GET_OPCODE(i))
    {
      case OP_END:
      {
      ...
      case OP_PUSHNIL:
      {
        ...
        do { ttype(top++) = LUA_TNIL; } while (--n > 0);
        break;
      }
      case OP_POP:
      {
      ...
      case OP_SETLIST:
      {
        ...
        for (; n; n--) *luaH_setint(L, arr, n + aux) = *(--top);
        break;
      }
      case OP_SETMAP:
      {
      ...
      case OP_ADD:
      {
        if (tonumber(top-2) || tonumber(top-1)) call_arith(L, top, TM_ADD);
        else
      ...
      case OP_SUB:
      {
        if (tonumber(top-2) || tonumber(top-1)) call_arith(L, top, TM_SUB);
        else
          nvalue(top-2) -= nvalue(top-1);
        top--;
        break;
      ...
      case OP_MULT:
      {
        if (tonumber(top-2) || tonumber(top-1)) call_arith(L, top, TM_MUL);
        else
          nvalue(top-2) *= nvalue(top-1);
        top--;
        break;
      ...
      case OP_DIV:
      {
        if (tonumber(top-2) || tonumber(top-1)) call_arith(L, top, TM_DIV);
        else
          nvalue(top-2) /= nvalue(top-1);
        top--;
        break;
      ...
      case OP_POW:
      {
        if (!call_binTM(L, top, TM_POW)) lua_error(L, "undefined operation");
        top--;
        break;
      }
      case OP_CONCAT:
      ...
      case OP_FORPREP:
      {
        if (tonumber(top-1)) lua_error(L, "`for' step must be a number");
        if (tonumber(top-2)) lua_error(L, "`for' limit must be a number");
        if (tonumber(top-3)) lua_error(L, "`for' initial value must be a number");
      ...
      case OP_FORLOOP:
      {
        ...
        if (ttype(top-3) != LUA_TNUMBER) lua_error(L, "`for' index must be a number");
        nvalue(top-3) += nvalue(top-1);/* increment index */
        if (nvalue(top-1) > 0 ? nvalue(top-3) > nvalue(top-2) : nvalue(top-3) < nvalue(top-2)) top -= 3;/* end loop: remove control variables */
        else
      ...
      {
        Node *node;
        if (ttype(top-1) != LUA_TTABLE) lua_error(L, "`for' table must be a table");
