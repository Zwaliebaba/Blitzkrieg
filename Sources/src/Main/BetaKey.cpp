...
namespace NMain
{
  ...
  static constexpr int s_nKey2Length = 20;
  ...
  bool CheckBetaKey()
  {
    ...
    //
    {
      ...
      if (!szFileName.empty() && (szFileName[szFileName.size() - 1] != '\\')) szFileName += "\\beta.key";
      else szFileName += "beta.key";
    ...
    //
    if (szStrings.size() != 3) return false;
    //
    ...
    if (szStrings.size() != 3) return false;
    if (nCurrYear > NStr::ToInt(szStrings[2])) return false;
    else if ((NStr::ToInt(szStrings[2]) == nCurrYear))
    {
      if (nCurrMonth > NStr::ToInt(szStrings[1])) return false;
      else if (NStr::ToInt(szStrings[1]) == nCurrMonth) { if (nCurrDay > NStr::ToInt(szStrings[0])) return false; }
    }
    // check 'checksum' code
    if (uCode != MakeCheckSum(szUserName, szExpiryDate)) return false;
