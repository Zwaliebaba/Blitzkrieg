...
{
  if (bStored) GetSingleton<IMainLoop>()->RestoreScenarioTracker();
  bStored = false;
  RemoveGlobalVar("History.Playing");
...
bool CCommandsHistory::Load(const char *pszFileName)
{
  ...
  // it can really be 0, but this is rare
  // if ( checkSumRes == 0.0f )
  {
...
#endif // #if defined(_FINALRELEASE) || 
  {
...
void CCommandsHistory::Save(const char *pszFileName)
{
  ...
  if ((szFileName == "") || szFileName.empty()) szFileName = GetGlobalVar("SaveHistoryFileName", "");
  ...
  if ((szFileName != "") && !szFileName.empty())
  {
    ...
    if (pStartScenarioTracker == nullptr) pStartScenarioTracker = GetSingleton<IScenarioTracker>()->Duplicate();
...
void CCommandsHistory::AddCommand(const int nSegment, IAILogicCommand *pCmd) { if (pCmd->NeedToBeStored()) savingHistory[nSegment].push_back(pCmd); }

void CCommandsHistory::ExecuteSegmentCommands(const int nSegment, ITransceiver *pTranceiver) { if (GetGlobalVar("HistoryClient", 0) == 0) { if (loadedHistory.find(nSegment) != loadedHistory.end()) { for (std::list<CPtr<IAILogicCommand>>::iterator iter = loadedHistory[nSegment].begin(); iter != loadedHistory[nSegment].end(); ++iter) pTranceiver->AddCommandToSend(*iter); } } }
...
{
  const std::string szLoadedHistoryFileName = GetGlobalVar("LoadHistoryFileName");
  if (szLoadedHistoryFileName.size() != 0) { if (checkSum != startMapCheckSum) InvalidHistory("Invalid history: maps are different"); }
