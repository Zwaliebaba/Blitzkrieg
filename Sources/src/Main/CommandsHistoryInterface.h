...
interface ICommandsHistory : IRefCount
{
  ...
  virtual void PrepareToStartMission() = 0;

  // loading and initialization for multiplayer if -lh option is given
  virtual bool LoadCommandLineHistory() = 0;
  ...
  virtual void Save(const char *pszFileName = "") = 0;

  virtual void Clear() = 0;

  virtual void AddCommand(int nSegment, interface IAILogicCommand *pCmd) = 0;
  virtual void ExecuteSegmentCommands(int nSegment, interface ITransceiver *pTranceiver) = 0;
  virtual void CheckStartMapCheckSum(int nCheckSum) = 0;
  ...
  virtual const char * GetModVersion() const = 0;
};
