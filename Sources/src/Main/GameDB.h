...
{
  SGVOGT_UNKNOWN = 0,
  SGVOGT_UNIT = 1,// dynamic unit with AI behavior. 
  SGVOGT_BUILDING = 2,// static building. 
  SGVOGT_FORTIFICATION = 3,// DOT, DZOT, etc... ground fortifications
  SGVOGT_ENTRENCHMENT = 4,// entrenchment segment. 
  SGVOGT_TANK_PIT = 5,// tank pit
  SGVOGT_BRIDGE = 6,// bridge segment. 
...
// general descriptor
struct SGDBObjectDesc : IGDBObject
{
...
namespace NGDB
{
  ...
  template<class TYPE>
  const TYPE *GetRPGStats(IObjectsDB *pGDB, int nIndex)
  {
  ...
  template<class TYPE>
  const TYPE *GetRPGStats(int nIndex) { return GetRPGStats<TYPE>(GetSingleton<IObjectsDB>(), nIndex); }

  template<class TYPE>
  const TYPE *GetRPGStats(IObjectsDB *pGDB, const SGDBObjectDesc *pDesc)
  {
    const IGDBObject *pStats = pGDB->GetRPGStats(pDesc);
    NI_ASSERT_SLOW_TF(dynamic_cast<const TYPE*>(pStats) != 0, NStr::Format("RPG stats for \"%s\" is not a right class", pDesc->szKey.c_str()), return 0);
    return static_cast<const TYPE *>(pStats);
  ...
  template<class TYPE>
  const TYPE *GetRPGStats(const SGDBObjectDesc *pDesc) { return GetRPGStats<TYPE>(GetSingleton<IObjectsDB>(), pDesc); }

  template<class TYPE>
  const TYPE *GetRPGStats(IObjectsDB *pGDB, const char *pszName)
  {
  ...
  template<class TYPE>
  const TYPE *GetRPGStats(const char *pszName) { return GetRPGStats<TYPE>(GetSingleton<IObjectsDB>(), pszName); }

  template<class TYPE>
  const TYPE *GetAddStats(IObjectsDB *pGDB, const char *pszName, IObjectsDB::EAddStatsType type)
  {
  ...
  template<class TYPE>
  const TYPE *GetAddStats(const char *pszName, IObjectsDB::EAddStatsType type) { return GetAddStats<TYPE>(GetSingleton<IObjectsDB>(), pszName, type); }

  template<class TYPE>
  const TYPE *GetGameStats(IObjectsDB *pGDB, const char *pszName, IObjectsDB::EGameStatsType type)
  {
  ...
  template<class TYPE>
  const TYPE *GetGameStats(const char *pszName, IObjectsDB::EGameStatsType type) { return GetGameStats<TYPE>(GetSingleton<IObjectsDB>(), pszName, type); }
