...
int SCommonGameStats::operator&(IDataTree &ss)
{
  ...
  if (saver.IsReading()) NStr::ToLower(szMapImage);
  return 0;
}
...
void SChapterStats::SMission::RetrieveShortcuts(IObjectsDB *pGDB) { pMission = static_cast<const SMissionStats *>(pGDB->GetGameStats(szMission.c_str(), IObjectsDB::MISSION)); }

void SChapterStats::RetrieveShortcuts(IObjectsDB *pGDB)
{
  for (auto it = missions.begin(); it != missions.end(); ++it) it->RetrieveShortcuts(pGDB);
}
...
void SChapterStats::AddMission(const SMission &mission) { missions.push_back(mission); }
...
int SChapterStats::SMission::operator&(IDataTree &ss)
{
  ...
  if (saver.IsReading()) NStr::ToLower(szMission);
  return 0;
}
...
int SChapterStats::SMission::operator&(IStructureSaver &ss)
{
  ...
  if (saver.IsReading()) pMission = static_cast<const SMissionStats *>(GetSingleton<IObjectsDB>()->GetGameStats(szMission.c_str(), IObjectsDB::MISSION));
  return 0;
}
...
void SCampaignStats::SChapter::RetrieveShortcuts(IObjectsDB *pGDB) { pChapter = static_cast<const SChapterStats *>(pGDB->GetGameStats(szChapter.c_str(), IObjectsDB::CHAPTER)); }

void SCampaignStats::RetrieveShortcuts(IObjectsDB *pGDB)
{
  for (auto it = chapters.begin(); it != chapters.end(); ++it) it->RetrieveShortcuts(pGDB);
}
...
int SCampaignStats::SChapter::operator&(IDataTree &ss)
{
  ...
  saver.Add("Secret", &bSecret);
  if (saver.IsReading()) NStr::ToLower(szChapter);
  return 0;
}
...
int SCampaignStats::operator&(IDataTree &ss)
{
  ...
  if (saver.IsReading())
  {
    ...
    NStr::ToLower(szInterfaceMusic);
    for (auto it = templateMissions.begin(); it != templateMissions.end(); ++it) NStr::ToLower(*it);
  }
  return 0;
}
...
int SMedalStats::operator&(IDataTree &ss)
{
  ...
  if (saver.IsReading()) NStr::ToLower(szTexture);
  return 0;
}