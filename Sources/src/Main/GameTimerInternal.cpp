...
{
  fGuarantieFPS = fFPS;
  nGuarantieTimeStep = (fGuarantieFPS > 0) && (fGuarantieFPS < 1000.0f) ? static_cast<int>(1000.0f / fGuarantieFPS) : 0;
}
...
void CSingleTimer::Update(const NTimer::STime &time)
{
  ...
  if (!bPaused)
  {
    float fdt = static_cast<float>(dT * fTimeScale) + fTimeError;
    dT = static_cast<NTimer::STime>(fdt);
    fTimeError = fdt - static_cast<float>(dT);
    currTime += nGuarantieTimeStep > 0 ? static_cast<int>(nGuarantieTimeStep * fTimeScale) : dT;
  }
}
...
void CGameTimer::DoPause(bool bPause, int nType, ISingleTimer *pTimer, CPausesMap &pauses)
{
  ...
  else
  {
    ...
    if (pos != pauses.end()) pauses.erase(pos);
    if (pauses.empty()) pTimer->Pause(bPause);
  }
  // re-calc pause reason
  nPauseReason = -1;
  for (CPausesMap::const_iterator it = pauses.begin(); it != pauses.end(); ++it) nPauseReason = Max(nPauseReason, it->first);
}

void CGameTimer::PauseGame(bool bPause, int nType) { DoPause(bPause, nType, pGameTimer, gamepauses); }
void CGameTimer::PauseSync(bool bPause, int nType) { DoPause(bPause, nType, pSyncTimer, syncpauses); }

bool CGameTimer::HasPause(const int nReason) const { return gamepauses.find(nReason) != gamepauses.end(); }
