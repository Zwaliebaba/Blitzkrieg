...
namespace NMain
{
  ...
  struct SDllModule
  {
    ...
    //
    SDllModule() : pDLLHandle(nullptr), pDesc(nullptr) {}

    ~SDllModule()
    {
      if (pDLLHandle)
      {
        if (pDesc) NStr::DebugTrace("Unloading module \"%s\" of version 0x%x\n", pDesc->pszName, pDesc->nVersion);
        delete pDLLHandle;
      }
    }
  };
  ...
  int STDCALL LoadAllModules(const char *pszPath)
  {
    ...
    for (NFile::CFileIterator it((szPath + "*.dll").c_str()); !it.IsEnd(); ++it)
    {
      ...
      //
      GETMODULEDESCRIPTOR pfnGetModuleDescriptor = pDLL->GetProcAddress("GetModuleDescriptor", static_cast<GETMODULEDESCRIPTOR>(nullptr));
      if (pfnGetModuleDescriptor != nullptr)
      {
  ...
  {
    if (nIndex >= modules.size()) return nullptr;
    auto pos = modules.begin();
  ...
  {
    for (auto it = modules.begin(); it != modules.end(); ++it) { if (it->pDesc == pModule) return it->pDLLHandle->GetModuleName(); }
    return "";
  }
...
public:
  CModuleLoadAutoMagic()
  {
    ...
    // add modules factory
    for (auto it = NMain::modules.begin(); it != NMain::modules.end(); ++it)
    {
      if (it->pDesc->pFactory) GetSLS()->AddFactory(it->pDesc->pFactory);
      if (it->pDesc->pChecker) it->pDesc->pChecker->SetModuleFunctionalityLimits();
    }
  }
