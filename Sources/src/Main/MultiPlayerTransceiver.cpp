...
void CMultiPlayerTransceiver::LoadGameSettings()
{
  ...
  for (int i = 0; i != nNumPlayers; ++i)
  {
    ...
    player.nSide = GetGlobalVar(szValueName.c_str(), static_cast<int>(-1));

    szValueName = NStr::Format("Multiplayer.Player%d.LogicID", i);
    player.nLogicID = GetGlobalVar(szValueName.c_str(), static_cast<int>(-1));
    ...
    if (player.nSide != -1 && szSideName != "Unknown") SetGlobalVar(NStr::Format("Multiplayer.Side%d.Name", player.nSide), szSideName.c_str());
    ...
    else wMask = wMask | (1UL << player.nLogicID);
...
void CMultiPlayerTransceiver::InitVariables()
{
  ...
  receivedCmds.clear();
  segmFinished.clear();
  segmFinished.resize(MAX_LATENCY, 0);
  lastFinishedSegment.clear();
  lastFinishedSegment.resize(MAX_LATENCY, 0);
  timeOfLastSegmFinished.clear();
  timeOfLastSegmFinished.resize(16, -1);
  noSegmDataLags.clear();
  noSegmDataLags.resize(16, 0);
  loadingPlayers.clear();
  loadingPlayers.resize(16, 0);
  nLoadingPlayers = 0;
  lastTimeToCheckNoSegmDataLag = 0;

  nLatency = 14;// GetGlobalVar( "Multiplayer.InternetGame", 0 ) == 1 ? 

  nSegmentsPackSize = nLatency / 2;
  for (int i = MAX_LATENCY - nLatency; i < MAX_LATENCY; ++i) { if ((MAX_LATENCY - i) % nSegmentsPackSize == 0) segmFinished[i] = wMask; }

  for (int i = 0; i < nNumPlayersInMap; ++i) lastFinishedSegment[i] = -nSegmentsPackSize;
...
void CMultiPlayerTransceiver::SendChatMessages()
{
  ...
  {
    // copy to a temporary variable so that it is not overwritten by the next call pBuffer->Read
...
void CMultiPlayerTransceiver::SegmentFinished()
{
  if (!bTotalOutOfSync)
  {
    ...
    while (const char *pszString = pBuffer->ReadASCII(CONSOLE_STREAM_MULTIPLAYER_CHECK))
    {
      ...
      auto pCheckSumCommand = new CControlSumCheckCommand(nMyNumber, checkSum);

      AddCommandToSend(pCheckSumCommand);
    }

    // add segment data
    pktOutgoing << static_cast<BYTE>(NGM_ID_SEGMENT);
    // send about the end of my segment commands
...
{
  // CRAP{ it's not that simple. 
  // execute extra segments
  nLatency = nSegments;
  // CRAP}
}
...
{
  // MAX_LATENCY is added because 
  return (nSegment - nLatency + MAX_LATENCY) % MAX_LATENCY;
}

const int CMultiPlayerTransceiver::GetCommonPastSegmentNum() const { return (nCommonSegment - nLatency); }
...
void CMultiPlayerTransceiver::RemovePlayer(const int nLogicID)
{
  ...
  while (iter != players.end() && iter->nLogicID != nLogicID) ++iter;
  ...
  if (timeOut.GetTimeOutPlayer() == nLogicID) timeOut.UnsetTimeOut(false);
}
...
void CMultiPlayerTransceiver::SetPlayerAlive(const int nPlayer)
{
  ...
  if (noSegmDataLags[nPlayer] == 1)
  {
    ...
    if (timeOfLastSegmFinished[nPlayer] == -1) SetPlayerLoading(nPlayer, false);
    else SetPlayerLag(nPlayer, false);
  ...
  while (pCommand = pMultiplayer->GetCommand())
  {
    switch (pCommand->eCommandType)
    {
      ...
      case IMultiplayer::GPC_PLAYER_REMOVED:
        if (bGameStarted && (wMask & (1UL << pCommand->nPlayer)) && pCommand->nPlayer != nMyNumber) RemovePlayer(pCommand->nPlayer);
      ...
      case IMultiplayer::GPC_PLAYER_LAG:
        ...
        {
          auto iter = players.begin();
          while (iter != players.end() && iter->nLogicID != pCommand->nPlayer) ++iter;

          if (iter != players.end()) SetPlayerLag(iter->nLogicID, pCommand->nParam == 1);
        }
      ...
      case IMultiplayer::GPC_PAUSE:
        if (bGameStarted) GetSingleton<IInput>()->AddMessage(SGameMessage(CMD_GAME_PAUSE));
      ...
      case IMultiplayer::GPC_GAME_SPEED:
        if (bGameStarted)
        {
          ...
          // we send a command through the interface so that after the start of multiplayer it is possible
          // immediately give the command to change speed
          for (int i = 0; i < abs(nChange); ++i)
          {
            if (nChange < 0) pMainLoop->Command(MAIN_COMMAND_CMD, NStr::Format("%d", CMD_GAME_SPEED_DEC));
            else pMainLoop->Command(MAIN_COMMAND_CMD, NStr::Format("%d", CMD_GAME_SPEED_INC));
          }
        }
      ...
      case IMultiplayer::GPC_TIMEOUT:
        if (bGameStarted)
        {
          if (pCommand->nParam) timeOut.SetTimeOut(pCommand->nPlayer);
          else timeOut.UnsetTimeOut(false);
        }
...
{
  if (loadingPlayers[nPlayer] == 1 && bSet || loadingPlayers[nPlayer] == 0 && !bSet) return;

  auto iter = players.begin();
  while (iter != players.end() && iter->nLogicID != nPlayer) ++iter;
...
void CMultiPlayerTransceiver::SetPlayerLag(const int nPlayer, const bool bSet)
{
  ...
  if (iter != players.end())
  {
    if (bSet) iter->lastLagUpdateTime = GetSingleton<IGameTimer>()->GetAbsTime();
    else iter->lastLagUpdateTime = 0;
...
void CMultiPlayerTransceiver::DoSegments()
{
  ...
  //
  if (!bHistoryPlaying && pMultiplayer->GetState() != IMultiplayer::EMS_PLAYING) bGameStarted = false;
  ...
  if (bGameStarted && bOldGameStarted && (bHistoryPlaying || pMultiplayer->GetState() == IMultiplayer::EMS_PLAYING))
  {
    ...
    if (bHistoryPlaying && !bSpeedSet)
    {
      ...
      for (int i = 0; i < abs(nGameSpeed); ++i) GetSingleton<IMainLoop>()->Command(MAIN_COMMAND_CMD, szCmd.c_str());
    ...
    while (pGameSegment->BeginSegments(pGameTimer->GetGameTime()))
    {
      ...
      if (nCommonPastSegmentNum % nSegmentsPackSize == 0)
      {
        ...
        // history or bitwise >= is played
        if (bHistoryPlaying || (~segmFinished[nPastSegment] & wMask) == 0)
        {
          // segments have arrived from all players, you can open fog and units
          if (nCommonSegment >= nLatency && nCommonSegment <= 2 * nLatency) GetSingleton<IAILogic>()->NetGameStarted();

          // remove forced segment pause
          pGameTimer->PauseGame(false, PAUSE_TYPE_MP_NO_SEGMENT_DATA);
          // execute loaded history commands
          if (nCommonSegment >= nSegmentsPackSize)
          {
            // loop so that single player history can be played in multiplayer
            for (int i = nCommonSegment - nSegmentsPackSize + 1; i <= nCommonSegment; ++i) pCmdsHistory->ExecuteSegmentCommands(i, this);
          }
          // finish segment => send data
          if (!bHistoryPlaying) SegmentFinished();
          // do all commands for 'past segment'
          _control87(_EM_INVALID | _EM_ZERODIVIDE | _EM_OVERFLOW | _EM_UNDERFLOW | _EM_INEXACT | _EM_DENORMAL | _PC_24, 0xfffff);
          //
          for (int i = 0; i < nNumPlayersInMap; ++i)
          {
            for (auto it = cmds[nPastSegment][i].begin(); it != cmds[nPastSegment][i].end(); ++it)
            {
              (*it)->Execute(pAILogic);

              // the command came on the segment GetCommonPastSegmentNum(), add to the list of commands
          ...
          if (!bHistoryPlaying && !bTotalOutOfSync) CControlSumCheckCommand::Check(nMyNumber);
    ...
    if (segmFinished[GetPastSegmentNum(nLatency)] == wMask) pGameTimer->PauseGame(false, PAUSE_TYPE_MP_NO_SEGMENT_DATA);
  }
}
...
void CMultiPlayerTransceiver::CommandClientDropPlayer(const WORD *pszPlayerNick)
{
  ...
  {
    if (!bTotalOutOfSync) pMultiplayer->DropPlayer(iter->nLogicID);

    RemovePlayer(iter->nLogicID);
  }
}

int CMultiPlayerTransceiver::GetNumberOfPlayers() const { return pMultiplayer->GetNumberOfPlayers(); }

void CMultiPlayerTransceiver::CommandTimeOut(const bool bSet)
{
  if (bSet) timeOut.SetTimeOut(nMyNumber);
  else timeOut.UnsetTimeOut(true);
}

bool CMultiPlayerTransceiver::JoinToServer(const char *pszIPAddress, const int nPort, bool bPasswordRequired, const char *pszPassword) { return pMultiplayer->InitJoinToServer(pszIPAddress, nPort, bPasswordRequired, pszPassword); }

void CMultiPlayerTransceiver::CreateServer() { pMultiplayer->InitServersList(); }

void CMultiPlayerTransceiver::InitByCreateServersList() { if (pMultiplayer->GetState() == IMultiplayer::EMS_NONE) pMultiplayer->InitServersList(); }
...
NTimer::STime CMultiPlayerTransceiver::GetMultiplayerTime()
{
  if (!GetSingleton<IAILogic>()->IsNoWin())
  {
    ...
    fLastSentMultiplayerTime = static_cast<float>(pTimer->GetGameTime()) / NTimer::GetCoeffFromSpeed(static_cast<float>(pTimer->GetSpeed()));
  }

  return fLastSentMultiplayerTime;
}

void CMultiPlayerTransceiver::SetTotalOutOfSync() { bTotalOutOfSync = true; }
...
{
  if (pMultiplayer) return pMultiplayer->GetInGameNetDriver();
  return nullptr;
}
...
void CTimeOut::InitGameStart(const int n)
{
  nMyNumber = n;
  lastTimeOutTime = 0;
  if (GetGlobalVar("History.Playing", 0) == 0) SetGlobalVar("temp.LocalPlayer.TimeOutEnable", 1);
...
{
  NTimer::STime curTime = GetSingleton<IGameTimer>()->GetAbsTime();
  // either the command did not come from our client, or time has already passed since the previous timeOut
...
void CTimeOut::UnsetTimeOut(const bool bByClientCommand)
{
  if (IsActive())
  {
    ...
    if (bByClientCommand) pMultiplayer->CommandTimeOut(false);
...
void CTimeOut::Segment()
{
  if (IsActive())
  {
    ...
    if (curTime > lastTimeOutTime) UnsetTimeOut(false);
    else GetSingleton<IInput>()->AddMessage(SGameMessage(CMD_GAME_TIMEOUT_UPDATE, (lastTimeOutTime - curTime) / 1000));
  }
  else if (GetGlobalVar("temp.LocalPlayer.TimeOutEnable", 0) == 0)
  {
    const NTimer::STime curTime = GetSingleton<IGameTimer>()->GetAbsTime();

    if (lastTimeOutTime + timeBWTimeOuts < curTime && GetGlobalVar("History.Playing", 0) == 0) SetGlobalVar("temp.LocalPlayer.TimeOutEnable", 1);
  }
}