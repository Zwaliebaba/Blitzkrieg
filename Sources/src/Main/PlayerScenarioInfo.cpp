...
void CScenarioUnit::SetValue(const int nType, const int nValue)
{
  ...
  if (nType == STUT_LEVEL) SetExpToNextLevel();
}
...
// personal name
interface IText *CScenarioUnit::GetName() const { return pName; }
...
void CScenarioUnit::ChangeRPGStats(const std::string &szStatsName)
{
  ...
  //
  // it is necessary to recalculate exp into a new one in accordance with the new levels after upgrade
  ...
  auto pOldExpLevels = static_cast<const SAIExpLevel *>(pGDB->GetExpLevels(pOldRPG->type));
  const SUnitBaseRPGStats *pNewRPG = NGDB::GetRPGStats<SUnitBaseRPGStats>(pGDB, szStatsName.c_str());
  auto pNewExpLevels = static_cast<const SAIExpLevel *>(pGDB->GetExpLevels(pNewRPG->type));
  //
  ...
  const int nNextLevel = Clamp(nCurrLevel + 1, 0, static_cast<int>(pOldExpLevels->levels.size() - 1));
  //
  if (nNextLevel > nCurrLevel)
  {
    ...
    const float fExpPercents = static_cast<float>(nCurrExp - nCurrLevelExp) / static_cast<float>(nNextLevelExp - nCurrLevelExp);
...
const std::string &CScenarioUnit::GetRPGStats() const { return szRPGStats; }
...
{
  for (int i = 0; i < values.size(); ++i) valueDiffs[i] = currValues[i] - values[i];
  values = currValues;
}
...
void CScenarioUnit::SetExpToNextLevel()
{
  ...
  auto pExpLevel = static_cast<const SAIExpLevel *>(pGDB->GetExpLevels(pRPG->type));
  const int nNextLevel = Clamp(currValues[STUT_LEVEL] + 1, 0, static_cast<int>(pExpLevel->levels.size() - 1));
...
void CScenarioUnit::Reincarnate(const bool bLowerLevel)
{
  ...
  if (bLowerLevel)
  {
    ...
    const SUnitBaseRPGStats *pRPG = NGDB::GetRPGStats<SUnitBaseRPGStats>(pGDB, szRPGStats.c_str());
    auto pExpLevel = static_cast<const SAIExpLevel *>(pGDB->GetExpLevels(pRPG->type));
    const int nLevel = Clamp(currValues[STUT_LEVEL] - 1, 0, static_cast<int>(pExpLevel->levels.size() - 1));
...
int CScenarioUnit::operator&(IStructureSaver &ss)
{
  ...
  if (saver.IsReading())
  {
    if (values.size() != STUT_NUM_ELEMENTS) values.resize(STUT_NUM_ELEMENTS);
    if (currValues.size() != STUT_NUM_ELEMENTS) currValues.resize(STUT_NUM_ELEMENTS);
    if (valueDiffs.size() != STUT_NUM_ELEMENTS) valueDiffs.resize(STUT_NUM_ELEMENTS);
  }
  return 0;
}
...
void CPlayerScenarioInfo::Init()
{
  ...
  for (auto it = skills.begin(); it != skills.end(); ++it) it->NormalizeValues(true);
}

void CPlayerScenarioInfo::SetName(const std::wstring &_wszName)
{
  wszName = _wszName;
  if (pNameObject == nullptr) pNameObject = CreateObject<IText>(TEXT_STRING);
...
const std::string &CPlayerScenarioInfo::GetSide() const { return szSide; }

const std::string &CPlayerScenarioInfo::GetGeneralSide() const { return szGeneralSide; }

interface IText *CPlayerScenarioInfo::GetSideName() const { return pSideName; }

// diplomacy side [0..2] (0 <=> 1 opponents, 2 - neutral)
void CPlayerScenarioInfo::SetDiplomacySide(const int _nDiplomacySide) { nDiplomacySide = _nDiplomacySide; }

const int CPlayerScenarioInfo::GetDiplomacySide() const { return nDiplomacySide; }

// player color
void CPlayerScenarioInfo::SetColor(const DWORD _dwColor) { dwColor = _dwColor; }

DWORD CPlayerScenarioInfo::GetColor() const { return dwColor; }
...
// player's rank
const struct SPlayerRank &CPlayerScenarioInfo::GetRankInfo() const { return rank; }

void CPlayerScenarioInfo::ClearLevelGain() { bGainLevel = false; }

bool CPlayerScenarioInfo::IsGainLevel() const { return bGainLevel; }
...
bool CPlayerScenarioInfo::SetExperience(const double _fExperience)
{
  ...
  if (IsGainLevel()) rank.fFormerValue = 0.0f;
  else rank.fFormerValue = rank.fValue;
...
// units
//
int CPlayerScenarioInfo::GetNumUnits() const { return units.size(); }

IScenarioUnit *CPlayerScenarioInfo::GetUnit(const int nIndex) const
{
  CheckRange(units, nIndex);
  return nIndex >= 0 && nIndex < units.size() ? units[nIndex] : nullptr;
}

int CPlayerScenarioInfo::GetNumNewUnits() const { return newUnits.size(); }
...
{
  for (auto it = medalSlots.begin(); it != medalSlots.end(); ++it) { if (*it == szName) return true; }
  return false;
}

// new medals
int CPlayerScenarioInfo::GetNumNewMedals() const { return newMedals.size(); }
...
const std::string &CPlayerScenarioInfo::GetUpgrade() const { return szUpgrade; }

// depot (unlimited) upgrades
int CPlayerScenarioInfo::GetNumDepotUpgrades() const { return depotUpgrades.size(); }

int CPlayerScenarioInfo::GetNumNewDepotUpgrades() const { return depotNewUpgrades.size(); }

const std::string &CPlayerScenarioInfo::GetNewDepotUpgrade(const int nIndex) const { return depotNewUpgrades[nIndex]; }
...
ICampaignStatistics *CPlayerScenarioInfo::GetCampaignStats() const { return pCampaignStats; }

// get current chapter stats
IChapterStatistics *CPlayerScenarioInfo::GetChapterStats() const { return pChapterStats; }

// get current mission stats
IMissionStatistics *CPlayerScenarioInfo::GetMissionStats() const { return pMissionStats; }
...
void CPlayerScenarioInfo::StartCampaign(CCampaignStatistics *pStats)
{
  ...
  pChapterStats = nullptr;
  pMissionStats = nullptr;
}
...
void CPlayerScenarioInfo::StartMission(CMissionStatistics *pStats)
{
  ...
  for (auto it = units.begin(); it != units.end(); ++it) (*it)->BeginMission();
  //
  newMedals.clear();
  newUnits.clear();
  ClearLevelGain();
...
void CPlayerScenarioInfo::FinishMission(const EMissionFinishStatus eStatus)
{
  ...
  if (eStatus == MISSION_FINISH_WIN) { for (auto it = units.begin(); it != units.end(); ++it) (*it)->AcceptMission(); }
  else { for (auto it = units.begin(); it != units.end(); ++it) (*it)->ClearMission(); }
}
...
void CPlayerScenarioInfo::SetUpgrade(const std::string &szUpgradeRPGStats) { szUpgrade = szUpgradeRPGStats; }
...
void CPlayerScenarioInfo::ClearNewDepotUpgrade() { depotNewUpgrades.clear(); }

void CPlayerScenarioInfo::RemoveDepotUpgrade(const std::string &szRPGStats)
{
  auto pos = std::find(depotUpgrades.begin(), depotUpgrades.end(), szRPGStats);
  if (pos != depotUpgrades.end()) depotUpgrades.erase(pos);
}
...
void CPlayerScenarioInfo::AddMedal(const std::string &szMedal, const int nSlot)
{
  ...
  if (std::find(newMedals.begin(), newMedals.end(), nSlot) == newMedals.end()) newMedals.push_back(nSlot);
}
