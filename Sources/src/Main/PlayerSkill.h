...
struct SPlayerRank
{
  std::string szCurrentRank;// key to local rank name (short name) ( leutenant, general )
  std::string szFullTextKey;// key to full text (long text describing the title received)
  std::string szRankPicture;// rank picture resource (packed as if it is a medal)
  float fValue;// 0 - just received current rank, 1 - near next rank.
  float fFormerValue;// former rank value
  int nRankNumber;//
  //
  SPlayerRank()
    : fValue(0), fFormerValue(0), nRankNumber(0) {}
...
struct SPlayerSkill
{
  ...
  SPlayerSkill(const char *pszSkillName, const float _fValue, const float _fFormerValue)
    : szSkillName(pszSkillName), fValue(_fValue), fFormerValue(_fFormerValue) { NormalizeValues(false); }

  void NormalizeValues(bool bInitial);
