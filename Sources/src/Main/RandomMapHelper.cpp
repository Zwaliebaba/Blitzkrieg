...
void STDCALL RestoreRandomMap(const std::string &szMissionName, const NSaveLoad::SRandomHeader &rndhdr, IRandomGenSeed *pSeed)
{
  if (const SMissionStats *pMission = NGDB::GetGameStats<SMissionStats>(szMissionName.c_str(), IObjectsDB::MISSION))
  {
    ...
    if (stats.mtime != rndhdr.dwRandomDateTime)
    {
      ...
      const bool bRes = CMapInfo::CreateRandomMap(const_cast<SMissionStats *>(pMission), rndhdr.szChapterUnitsTableFileName, rndhdr.nLevel, nFoundGraphIndex, nFoundAngle, true, true, nullptr, pProgress);
      pProgress->Stop();
  ...
  else { NI_ASSERT_T(false, NStr::Format("Can't find mission stats (\"%s\") for random mission to re-create map", szMissionName.c_str())); }
}