...
const std::string &CMissionStatistics::GetName() const { return szName; }
...
EMissionFinishStatus CMissionStatistics::GetFinishStatus() const { return eStatus; }
...
int CMissionStatistics::operator&(IStructureSaver &ss)
{
  ...
  if (saver.IsReading())
  {
    if (values.size() != STMT_NUM_ELEMENTS) values.resize(STMT_NUM_ELEMENTS);
    if (values[STMT_OBJECTIVES_RECIEVED] < values[STMT_OBJECTIVES_COMPLETED]) values[STMT_OBJECTIVES_RECIEVED] = values[STMT_OBJECTIVES_COMPLETED];
  }
  return 0;
}
...
// get mission/chapter/campaign name
const std::string &CChapterStatistics::GetName() const { return szName; }

// retrieve statistics value by type
int CChapterStatistics::GetValue(const int nType) const
{
  int nValue = 0;
  for (auto it = missions.begin(); it != missions.end(); ++it) { if ((*it)->GetFinishStatus() == MISSION_FINISH_WIN) nValue += (*it)->GetValue(nType); }
  //
  return nValue;
}

// get num missions, ever started in this chapter
int CChapterStatistics::GetNumMissions() const { return missions.size(); }
...
{
  int nAmount = 0;
  for (auto it = missions.begin(); it != missions.end(); ++it) { if ((*it)->GetFinishStatus() == MISSION_FINISH_WIN) nAmount += (*it)->GetNumKIA(); }
  return nAmount;
}
...
const std::string &CChapterStatistics::GetKIAName(const int nIndex) const
{
  ...
  for (auto it = missions.begin(); it != missions.end(); ++it)
  {
    if ((*it)->GetFinishStatus() == MISSION_FINISH_WIN)
    {
      ...
      if ((nAmount <= nIndex) && (nAmount + nNumKIA > nIndex)) return (*it)->GetKIAName(nIndex - nAmount);
...
const std::string &CChapterStatistics::GetKIANewName(const int nIndex) const
{
  ...
  for (auto it = missions.begin(); it != missions.end(); ++it)
  {
    ...
    {
      const int nNumKIA = (*it)->GetNumKIA();
      if ((nAmount <= nIndex) && (nAmount + nNumKIA > nIndex)) return (*it)->GetKIANewName(nIndex - nAmount);
      nAmount += (*it)->GetNumKIA();
    }
  }
...
const std::string &CChapterStatistics::GetKIAStats(const int nIndex) const
{
  ...
  for (auto it = missions.begin(); it != missions.end(); ++it)
  {
    ...
    {
      const int nNumKIA = (*it)->GetNumKIA();
      if ((nAmount <= nIndex) && (nAmount + nNumKIA > nIndex)) return (*it)->GetKIAStats(nIndex - nAmount);
      nAmount += (*it)->GetNumKIA();
    }
  }
...
// get mission/chapter/campaign name
const std::string &CCampaignStatistics::GetName() const { return szName; }

ECampaignType CCampaignStatistics::GetType() const { return eType; }

// retrieve statistics value by type
int CCampaignStatistics::GetValue(const int nType) const
{
  int nValue = 0;
  for (auto it = chapters.begin(); it != chapters.end(); ++it) nValue += (*it)->GetValue(nType);
  return nValue;
}

// get num chapters, ever started in this campaign
int CCampaignStatistics::GetNumChapters() const { return chapters.size(); }
...
{
  int nAmount = 0;
  for (auto it = chapters.begin(); it != chapters.end(); ++it) nAmount += (*it)->GetNumKIA();
  return nAmount;
}
