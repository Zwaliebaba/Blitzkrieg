...
void CPlayerScenarioInfoIterator::Next() { ++itCurrPlayer; }

// check, if we've reached end?
bool CPlayerScenarioInfoIterator::IsEnd() const { return itCurrPlayer == players.end(); }

// get current iterator's player scenario info
IPlayerScenarioInfo *CPlayerScenarioInfoIterator::Get() const { return itCurrPlayer->second; }

// get current iterator's player ID
int CPlayerScenarioInfoIterator::GetID() const { return itCurrPlayer->first; }
...
namespace NScenarioScript2
{
  ...
  using SMedal = std::pair<std::string, int>;
  ...
  int GetStatisticsValue(struct lua_State *state)
  {
    ...
    IScenarioStatistics *pStatistics = nullptr;
    ...
    const double fValue = pStatistics != nullptr ? pStatistics->GetValue(nStatType) : 0;
    // push to LUA stack
  ...
  {
    int nNumMissions = 0;
    for (int i = 0; i < pChapter->GetNumMissions(); ++i) nNumMissions += static_cast<int>(pChapter->GetMission(i)->GetFinishStatus() == MISSION_FINISH_WIN);
    return nNumMissions;
  }
  ...
  int GetNumMissions(struct lua_State *state)
  {
    ...
    if (bInChapter)
    {
      ...
      fValue = pChapter != nullptr ? GetNumMissionsFromChapter(pChapter, bWon) : 0;
    }
    else
    {
      ICampaignStatistics *pCampaign = pPlayer->GetCampaignStats();
      if (pCampaign != nullptr) { for (int i = 0; i < pCampaign->GetNumChapters(); ++i) fValue += GetNumMissionsFromChapter(pCampaign->GetChapter(i), bWon); }
    }
    //
    script.PushNumber(fValue);
    return 1;
...
int CScenarioTracker2::SOpponentDesc::SRPGClassDesc::operator&(IDataTree &ss)
{
  ...
  if (saver.IsReading())
  {
    ...
    if (szClassName == "artillery") eRPGClass = RPG_CLASS_ARTILLERY;
    else if (szClassName == "tank") eRPGClass = RPG_CLASS_TANK;
    else eRPGClass = RPG_CLASS_UNKNOWN;
    //
    for (auto it = names.begin(); it != names.end(); ++it) NStr::ToLower(*it);
  }
  return 0;
}
...
int CScenarioTracker2::SOpponentDesc::operator&(IDataTree &ss)
{
  ...
  if (saver.IsReading()) NStr::ToLower(szSide);
  return 0;
}
...
void CScenarioTracker2::ProcessScriptChanges(const bool bPostMission)
{
  ...
  {
    if (CheckRPGStats(*it) == false) continue;
  ...
  if (bPostMission) pUserPlayer->GetMissionStats()->SetValue(STMT_NEW_UNITS, szNewSlots.size());
  szNewSlots.clear();
  // add new upgrades
  ...
  {
    if (!CheckRPGStats(it->szOldValue) || !CheckRPGStats(it->szNewValue)) continue;
...
{
  Zero(guidMission);
  randomBonuses.resize(3);// by the number of difficulties of random missions
  return true;
}
...
{
  for (auto it = players.begin(); it != players.end(); ++it) { if (it->first == nPlayerID) return it->second; }
  return nullptr;
}
...
IPlayerScenarioInfo *CScenarioTracker2::GetUserPlayer() const { return pUserPlayer; }

int CScenarioTracker2::GetUserPlayerID() const { return nUserPlayerID; }

// iterate through all players
IPlayerScenarioInfoIterator *CScenarioTracker2::CreatePlayerScenarioInfoIterator() const { return new CPlayerScenarioInfoIterator(players); }
...
std::string CScenarioTracker2::GetBestPersonalName(const std::string &szRPGStats, const std::string &szSide) const
{
  ...
  for (std::vector<SOpponentDesc>::const_iterator opponent = opponents.begin(); opponent != opponents.end(); ++opponent)
  {
    if (opponent->szSide == szSide)
    {
      ...
      for (auto it = opponent->classes.begin(); it != opponent->classes.end(); ++it)
      {
...
{
  if (szProgName == "Easy") return 0;
  if (szProgName == "Normal") return 1;
  if (szProgName == "Hard") return 2;
  if (szProgName == "Ironman") return 3;

  return 0;
}

const std::string &CScenarioTracker2::GetMinimumDifficulty() const { return szMinimumDifficulty; }

void CScenarioTracker2::InitMinimumDifficulty()
{
  variant_t var;
  GetSingleton<IOptionSystem>()->Get("GamePlay.Difficulty", &var);
  szMinimumDifficulty = bstr_t(var);
}

void CScenarioTracker2::UpdateMinimumDifficulty()
{
  // CRAP{ FOR SAVES COMPATIBILITY
  if (szMinimumDifficulty.empty()) InitMinimumDifficulty();
  // CRAP}

  variant_t var;
  GetSingleton<IOptionSystem>()->Get("GamePlay.Difficulty", &var);
  const std::string szNewMinimumDifficulty = bstr_t(var);
  const int nNewDifficulty = Difficulty2Int(szNewMinimumDifficulty);

  const int nDifficulty = Difficulty2Int(szMinimumDifficulty);
  if (nNewDifficulty < nDifficulty) szMinimumDifficulty = szNewMinimumDifficulty;
}
...
void CScenarioTracker2::StartCampaign(const std::string &_szCampaignName, const ECampaignType eType)
{
  ...
  if (pStats)
  {
    ...
    for (std::vector<std::string>::const_iterator it = templateMissions.begin(); it != templateMissions.end(); ++it) { NI_ASSERT_T(NGDB::GetGameStats<SMissionStats>(it->c_str(), IObjectsDB::MISSION) != 0, NStr::Format("Chaeck - can't load mission stats \"%s\"", it->c_str())); }
  }
  //
  szCurrChapter.clear();
  szCurrMission.clear();
  ...
  if (eType != CAMPAIGN_TYPE_MULTIPLAYER)
  {
    ...
    if (IPlayerScenarioInfo *pPlayer = AddPlayer(0))
    {
      ...
      if (pStats) pPlayer->SetSide(pStats->szSideName);
      variant_t varPlayerName;
      GetSingleton<IOptionSystem>()->Get("GamePlay.PlayerName", &varPlayerName);
      pPlayer->SetName(static_cast<wchar_t *>(bstr_t(varPlayerName)));
    }
    // enemy player (1)
    if (IPlayerScenarioInfo *pPlayer = AddPlayer(1))
    {
      ...
      if (pText) pPlayer->SetName(pText->GetString());
...
bool CScenarioTracker2::StartChapter(const std::string &_szChapterName)
{
  ...
  // if we are in the same chapter, just leave this function
  if (szCurrChapter == szChapter) return false;
...
void CScenarioTracker2::StartMission(const std::string &szMissionName)
{
  ...
  if (CoCreateGuid(&guidMission) != S_OK) Zero(guidMission);
  ...
  {
    DWORD dwColor;
    if (player->second.GetPtr() == pUserPlayer.GetPtr()) dwColor = GetGlobalVar("Scene.PlayerColors.Player", static_cast<int>(0xff000000));
    else if (nOurSide == 2 || player->second->GetDiplomacySide() == 2) dwColor = GetGlobalVar("Scene.PlayerColors.Neutral", static_cast<int>(0xff808080));
...
void CScenarioTracker2::FinishMission(const EMissionFinishStatus eStatus)
{
  // calc mission time
  {
    ...
    const float fTimeCoeff = GetGlobalVar("MultiplayerGame", 0) ? NTimer::GetCoeffFromSpeed(static_cast<float>(pTimer->GetSpeed())) : 1.0f;

    const NTimer::STime timeStart = GetGlobalVar("Mission.Current.StartTime", -1);
    const NTimer::STime timeFinish = pTimer->GetGameTime();
    if ((timeStart != -1) && (timeStart <= timeFinish)) nMissionTime = static_cast<int>(static_cast<double>(timeFinish - timeStart) / (60.0 * 1000.0) / fTimeCoeff);
  ...
  if (pUserPlayer)
  {
    if (IMissionStatistics *pStats = pUserPlayer->GetMissionStats())
    {
      ...
      // add hidden objectives to RECIEVED only if they are given.
  ...
  switch (eStatus)
  {
      ...
      // add info about successfully finished mission to global vars
    {
      ...
      // reincarnate units
      {
        ...
        for (int i = 0; i < pUserPlayer->GetNumUnits(); ++i)
        {
          auto pUnit = checked_cast<CScenarioUnit *>(pUserPlayer->GetUnit(i));
          if (!pUnit->IsKilled()) continue;
        ...
        if (!kiaUnits.empty())
        {
          ...
          for (auto it = kiaUnits.begin(); it != kiaUnits.end(); ++it)
          {
      ...
      for (auto player = players.begin(); player != players.end(); ++player) player->second->FinishMission(eStatus);
      // recalc player skills
      RecalcPlayerSkills(pUserPlayer);
      // calc player's experience
      if (pUserPlayer) pUserPlayer->SetExperience(pUserPlayer->GetCampaignStats()->GetValue(STMT_PLAYER_EXPERIENCE));
...
int CScenarioTracker2::GetNumRandomTemplates() const { return templateMissions.size(); }
...
{
  auto pNewTracker = new CScenarioTracker2();
...
int CScenarioTracker2::operator&(IDataTree &ss) { return 0; }
...
int CScenarioTracker2::operator&(IStructureSaver &ss)
{
  ...
  if (saver.IsReading())
  {
    ...
    if (randomBonuses.size() < 3) { randomBonuses.resize(3); }
  }
  return 0;
}
...
{
  if (randomBonuses.size() < 3) { randomBonuses.resize(3); }
  if ((nDifficulty >= 0) && (nDifficulty < 3)) { randomBonuses[nDifficulty].clear(); }
}
