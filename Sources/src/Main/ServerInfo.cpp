...
{
  if (pAddress == nullptr && info.pAddress == nullptr) return true;
  return
...
void SServerInfo::Unpack(const INetDriver::SGameInfo &gameInfo)
{
  ...
  if (gameInfo.szGameType == "Flag control") eGameType = CMapInfo::TYPE_FLAGCONTROL;
  else if (gameInfo.szGameType == "Sabotage") eGameType = CMapInfo::TYPE_SABOTAGE;
  else
  {
