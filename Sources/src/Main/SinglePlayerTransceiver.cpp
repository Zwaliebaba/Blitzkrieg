...
int CSinglePlayerTransceiver::operator&(IStructureSaver &ss)
{
  ...
  // so as not to interfere with multiplayer history
...
void CSinglePlayerTransceiver::LoadAllGameParameters() { bHistoryPlaying = GetGlobalVar("History.Playing", 0) != 0; }
...
void CSinglePlayerTransceiver::CommandShowAreas(int nGroupID, int nAreaType, bool bShow)
{
  if (!bHistoryPlaying)
  {
    pAILogic->ShowAreas(nGroupID, static_cast<EActionNotify>(nAreaType), bShow);
    pCmdsHistory->AddCommand(nCommonSegment, new CShowAreasCommand(nGroupID, nAreaType, bShow));
  }
}

void CSinglePlayerTransceiver::AddCommandToSend(IAILogicCommand *pCommand) { pCommand->Execute(pAILogic); }

void CSinglePlayerTransceiver::CommandClientTogglePause() { GetSingleton<IInput>()->AddMessage(SGameMessage(CMD_GAME_PAUSE)); }

void CSinglePlayerTransceiver::CommandClientSpeed(const int nChange) { GetSingleton<IInput>()->AddMessage(SGameMessage(nChange == 1 ? CMD_GAME_SPEED_INC : CMD_GAME_SPEED_DEC)); }