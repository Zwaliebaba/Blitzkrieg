...
bool ExtractKeyAndValue(const std::wstring &szInput, std::string *pKey, std::wstring *pValue)
{
  ...
  if (szString.size() < 3) return false;// eject too short (read wrong) strings
  //
  const int nPos = szString.find(L'#');
  if (nPos == std::wstring::npos) return false;// string w/o separator (#) we'll treat as wrong... ejecting
  // key
  ...
  *pValue = szString.substr(nPos + 1, std::wstring::npos);
  ...
  // trim value right
  {
    ...
    if (nPos == std::wstring::npos) { if (pValue->find_first_of(L' ') == 0) pValue->clear(); }
    else pValue->erase(nPos + 1, std::wstring::npos);
  }
  //
...
bool CTextManager::AddTextFile(const char *pszFileName)
{
  ...
  //
  constexpr wchar_t wSeparator = L'\n';
  ...
  do
  {
    ...
    {
      // fill in the new value in share
...
void CTextManager::Clear(const EClearMode eMode, const int nUsage, const int nAmount)
{
  if (eMode == CLEAR_ALL) shareDialog.Clear();
  else shareDialog.ClearUnreferencedResources();
}

IText *CTextManager::GetString(const char *pszKey)
{
  if (shareString.HasData(pszKey)) return shareString.Get(pszKey);
  // CRAP{ to be compatible with old strings-based system
  if (IText *pText = GetDialog(NStr::Format("textes\\strings\\%s", pszKey))) return pText;
  if (IText *pText = GetDialog(NStr::Format("textes\\tooltips\\%s", pszKey))) return pText;
  if (IText *pText = GetDialog(NStr::Format("textes\\acks\\%s", pszKey))) return pText;
  if (IText *pText = GetDialog(pszKey)) return pText;
  return nullptr;
  // CRAP}
}
