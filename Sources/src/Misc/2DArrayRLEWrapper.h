...
class CArray2DRLEWrapper
{
  ...
  //
  CArray2DRLEWrapper();
  void SaveCode(T *pRes, int *pNResCnt, int nCode);
...
template<class T>
void CArray2DRLEWrapper<T>::SaveCode(T *pRes, int *pNResCnt, const int nCode)
{
  switch (sizeof(T))
  {
    case 1:
...
const int CArray2DRLEWrapper<T>::ReadCode(T *pRes, int *pNResCnt)
{
  ...
  {
    case 1:
      nCode = (static_cast<int>(pRes[*pNResCnt]) << 24) | (static_cast<int>(pRes[(*pNResCnt) + 1]) << 16) |
              (static_cast<int>(pRes[(*pNResCnt) + 2]) << 8) | static_cast<int>(pRes[(*pNResCnt) + 3]);
      *pNResCnt += 4;

      break;
    case 2:
      nCode = (static_cast<int>(pRes[*pNResCnt]) << 16) | static_cast<int>(pRes[(*pNResCnt) + 1]);
      *pNResCnt += 2;

      break;
    default:
      nCode = static_cast<int>(pRes[(*pNResCnt)++]);
  }
...
template<class T>
int CArray2DRLEWrapper<T>::operator&(IStructureSaver &ss)
{
  CSaverAccessor saver = &ss;
    ...
    if (a.GetSizeX() * a.GetSizeY() != 0)
    {
      ...
      while (cnt < a.GetSizeX() * a.GetSizeY())
      {
        ...
        while (i < a.GetSizeX() * a.GetSizeY())
        {
          ...
          {
            ++nZeros;
            // last read data - sequence of zeros
            if (nZeros > NZEROS)
            {
              // type is undefined, put “sequence of zeros” in type
              if (nType == -1) nType = 0;
                // type "unique sequence", the sequence has ended,
                // move the pointer back and exit the loop
              else if (nType == 1)
              {
                i -= NZEROS;
                break;
          ...
          {
            nZeros = 0;
            // type is undefined, set type to "unique sequence"
            if (nType == -1) nType = 1;
              // "sequence of zeros" has ended, exit the loop
            else if (nType == 0) break;
          }
        ...
        if (nType == 0) SaveCode(pRLE, &nRLECnt, nLength);
        else
        {
  ...
  else
  {
    ...
    if (packingData == 0) a.SetZero();
    else a.Set(packingData);
