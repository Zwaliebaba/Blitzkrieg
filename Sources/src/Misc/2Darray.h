...
class CArray2D
{
  ...
  struct CBoundCheck
  {
    ...
    {
      NI_ASSERT_SLOW_T(i>=0 && i<nSize, NStr::Format("X size (%d) miss in 2D array (%d)", nSize, i));
      return data[i];
    }
  };
#endif // _DEBUG
  ...
  void Copy(const CArray2D &a)
  {
    ...
    nSizeY = a.nSizeY;
    Create();
    for (int i = 0; i < nSizeX * nSizeY; ++i) data[i] = a.data[i];
  }
  ...
  void Create()
  {
    data = new T[nSizeX * nSizeY];
    pData = new PT[nSizeY];
    for (int i = 0; i < nSizeY; ++i) pData[i] = data + i * nSizeX;
  }

public:
  CArray2D() : data(nullptr), pData(nullptr), nSizeX(0), nSizeY(0) {}
  CArray2D(int xsize, int ysize) : nSizeX(xsize), nSizeY(ysize) { Create(); }
  CArray2D(const CArray2D &a) { Copy(a); }
  ~CArray2D() { Destroy(); }
  //
  const CArray2D &operator=(const CArray2D &a)
  {
    Destroy();
    Copy(a);
    return *this;
  ...
  {
    if (nSizeX == xsize && nSizeY == ysize) return;
    Destroy();
    nSizeX = xsize;
    nSizeY = ysize;
    Create();
  ...
  void Set(const T &a)
  {
    for (int i = 0; i < nSizeX * nSizeY; ++i) data[i] = a;
  }

  //
#ifdef _DEBUG
  CBoundCheck operator[](int i) const
  {
    NI_ASSERT_SLOW_T(i>=0 && i<nSizeY, NStr::Format("Y size (%d) miss in 2D array (%d)", nSizeY, i));
    return CBoundCheck(pData[i], nSizeX);
  }
#else
  T *operator[](int i) const
  {
