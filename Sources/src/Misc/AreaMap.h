...
{
public:
  using CDataList = std::list<TStorageType>;
  using CBaseArea = CArray2D<CDataList>;
...
public:
  ...
  void MoveTo(TYPE *pObj, const TPosition &vNewPos)
  {
    ...
    const int nOldIndexX = static_cast<int>(vOldPos.x / tCellSize);
    const int nOldIndexY = static_cast<int>(vOldPos.y / tCellSize);
    const int nNewIndexX = static_cast<int>(vNewPos.x / tCellSize);
    const int nNewIndexY = static_cast<int>(vNewPos.y / tCellSize);
  ...
  void Add(TYPE *pObj)
  {
    ...
    const int nX = static_cast<int>(vPos.x / tCellSize);
    const int nY = static_cast<int>(vPos.y / tCellSize);
    AddTo(nX, nY, pObj);
  }
  ...
  //
  const bool IsInArea(const TPosition &vPos) const { return (vPos.x >= 0) && (vPos.y >= 0) && (vPos.x < GetSizeX() * tCellSize) && (vPos.y < GetSizeY() * tCellSize); }
};
...
private:
  ...
  void AddTo(int nX, int nY, const TYPE &obj)
  {
    ...
    if (pos == data.end()) data.push_back(obj);
  }
