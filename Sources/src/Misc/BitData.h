...
public:
  ...
  int operator&(interface IDataTree &ss)
  {
    ...
    saver.Add("Size", &xSize);
    saver.Add("BitArray", &array);
    return 0;
  }
  ...
  ~CArray1Bit() {}
  //
  const CArray1Bit &operator=(const CArray1Bit &a)
  {
    xSize = a.xSize;
    array = a.array;
    return *this;
...
public:
  ...
  ~CArray2D4Bit() {}
  //
  const CArray2D4Bit &operator=(const CArray2D4Bit &a)
  {
    xSize = a.xSize;
    array = a.array;
    return *this;
  ...
  //
  void SetSizes(const int xsize, const int ysize)
  {
    xSize = xsize;
    array.SetSizes((xsize >> 1) + 1, ysize);
  }
  ...
  const BYTE GetData(const int x, const int y) const { return (array(x >> 1, y) >> ((x & 1) << 2)) & 0xf; }

  void RemoveData(const int x, const int y) { array(x >> 1, y) &= 0xf0 >> ((x & 1) << 2); }
...
public:
  ...
  ~CArray2D1Bit() {}
  //
  const CArray2D1Bit &operator=(const CArray2D1Bit &a)
  {
    xSize = a.xSize;
    array = a.array;
    return *this;
  ...
  {
    xSize = xsize;
    array.SetSizes((xsize >> 3) + 1, ysize);
  }
  ...
  void Set1() { memset(array.GetBuffer(), static_cast<unsigned int>(-1), array.GetSizeX() * array.GetSizeY()); };
  ...
  void SetData(const int x, const int y) { array(x >> 3, y) |= 1 << (x & 7); }

  const BYTE GetData(const int x, const int y) const { return (array(x >> 3, y) >> (x & 7)) & 1; }

  void RemoveData(const int x, const int y) { array(x >> 3, y) &= ~(1 << (x & 7)); }
