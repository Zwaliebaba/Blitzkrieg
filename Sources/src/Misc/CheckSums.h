...
namespace NCheckSums
{
  ...
  struct SCheckSumBufferStorage
  {
    ...
    void CopyToBuf(const T &data)
    {
      ...
      if (nRequiredSize >= buf.size()) buf.resize(nRequiredSize * 1.5);
    ...
    const uLong GetCRC() const { return nCnt > 0 ? crc32(0, &(buf[0]), nCnt) : 0; }
    //
    int GetSize() const { return nCnt; }
  };

  template<class T>
  void CopyToBuf(SCheckSumBufferStorage *pStorage, const T &data) { pStorage->CopyToBuf(data); }

  inline void CopyToBufRaw(SCheckSumBufferStorage *pStorage, const void *pData, const int nDataSize) { pStorage->CopyToBufRaw(pData, nDataSize); }

  inline const uLong GetCRC(SCheckSumBufferStorage *pStorage) { return pStorage->GetCRC(); }
