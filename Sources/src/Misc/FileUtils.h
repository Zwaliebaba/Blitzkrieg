...
namespace NFile
{
  ...
  public:
    ...
    CFileIterator(const char *pszMask)
    {
      FindFirstFile(pszMask);
    }
  ...
  void EnumerateFiles(const char *pszStartDir, const char *pszMask, TEnumFunc callback, bool bRecurse)
  {
    ...
    for (CFileIterator it((szDir + pszMask).c_str()); !it.IsEnd(); ++it) { if (!it.IsDirectory()) callback(it); }
    ...
    for (CFileIterator it((szDir + "*.*").c_str()); !it.IsEnd(); ++it)
    {
      ...
      {
        // skip 'dots'
        if (it.IsDots()) continue;
        // dive into recurse
        if (bRecurse) EnumerateFiles((it.GetFilePath() + "\\").c_str(), pszMask, callback, bRecurse);
        //
        callback(it);
      }
    }
  ...
  public:
    ...
    //
    void operator()(const CFileIterator &it) { if (!it.IsDirectory()) { pFileVector->push_back(it.GetFilePath()); } }
  };

  // 'unctor for EnumerateFiles, enumerates all files in a directory and takes their relative name'
  ...
  public:
    CGetAllFilesRelative(const char *pszDir, std::vector<std::string> *pFiles) : pFileVector(pFiles), szInitDir(pszDir) {}
    //
  ...
  // 'unctor for EnumerateFiles, enumerates all subdirectories in a directory and takes their relative name'
  ...
  public:
    CGetAllDirectoriesRelative(const char *pszInitDir, std::vector<std::string> *pFiles) : pFileVector(pFiles), szInitDir(pszInitDir) {}
    //
    void operator()(const CFileIterator &it)
    {
      if (!it.IsDirectory())
      {
        ...
        szFileName = szFileName.substr(szInitDir.size());// cut off the starting directory
        int nRes = szFileName.rfind('\\');
        if (nRes == std::string::npos) return;
        szFileName = szFileName.substr(0, nRes);// cut off the file, leaving only the directory
        if (szFileName.size() == 0) return;

        for (int i = 0; i < pFileVector->size(); i++) { if ((*pFileVector)[i] == szFileName) return; }
...
// general template
template<class TYPE>
NFile::CFile &operator<<(NFile::CFile &file, const TYPE &data)
{
...
template<class TYPE>
NFile::CFile &operator>>(NFile::CFile &file, TYPE &data)
{
