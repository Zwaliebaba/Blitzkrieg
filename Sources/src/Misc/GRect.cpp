...
void GRect::center(const GPoint &new_center) { origin += new_center - center(); }
...
bool GRect::clip_line(GSPos &X1, GSPos &Y1, GSPos &X2, GSPos &Y2) const
{
  ...
  while (!Outside && !Inside)
  {
    ...
    {
      Y1 += static_cast<GSPos>((long) (Y2 - Y1) * (XUL - X1) / (X2 - X1));
      X1 = XUL;
    }
    else if (ocu1.ocs.code1)/* clip above */
    {
      X1 += static_cast<GSPos>((long) (X2 - X1) * (YUL - Y1) / (Y2 - Y1));
      Y1 = YUL;
    }
    else if (ocu1.ocs.code2)/* clip right */
    {
      Y1 += static_cast<GSPos>((long) (Y2 - Y1) * (XLR - X1) / (X2 - X1));
      X1 = XLR;
    }
    else if (ocu1.ocs.code3)/* clip below */
    {
      X1 += static_cast<GSPos>((long) (X2 - X1) * (YLR - Y1) / (Y2 - Y1));
      Y1 = YLR;
    }
...
bool GRect::clip_hline(GSPos &x0, GSPos y, GSPos &x1) const
{
  ...
  if (y < top() || y > bottom() || x0 > bx || x1 < ax) return false;
  if (x0 <= ax) x0 = ax;
  if (x1 >= bx) x1 = bx;
  return true;
}
...
bool GRect::clip_vline(GSPos x0, GSPos &y0, GSPos &y1) const
{
  ...
  if (x0 < left() || x0 > right() || y0 > by || y1 < ay) return false;
  if (y0 <= ay) y0 = ay;
  if (y1 >= by) y1 = by;
  return true;
}
...
GRect::GRect(const RECT &r)
  : origin(static_cast<GSPos>(r.left), static_cast<GSPos>(r.top)),
    size(static_cast<GSPos>(r.right - r.left + 1), static_cast<GSPos>(r.bottom - r.top + 1)) {}
