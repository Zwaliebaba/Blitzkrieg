...
{
  if ((pszName == nullptr) || (pProp == nullptr)) return nullptr;
...
const SPropertyDesc * STDCALL CManipulator::GetPropertyDesc(const char *pszName)
{
  ...
  else return FillTempProps(pszName, pProperty);
}
...
SBaseProperty *CManipulator::GetProperty(const std::string &szFullName, std::string *pRestName, int *pnIndex)
{
  ...
  else if (pProp->nodeType == SBaseProperty::SINGLE) *pnIndex = -1;
  //
  return pProp;
}
...
IManipulator *CManipulator::GetPropertyAsManipulator(SBaseProperty *pProp, int nIndex)
{
  ...
  {
    auto pMan = reinterpret_cast<IManipulator *>(varMan.byref);
    NI_ASSERT_TF(pMan != nullptr && pMan != 0, "returned value for property is not a manipulator", return 0);
    varMan.vt = VT_EMPTY;
    return pMan;
  }
}
...
bool CManipulator::GetValue(const char *pszValueName, variant_t *pValue)
{
  ...
  if (pProp->nodeType == SBaseProperty::LEAF) return pProp->Get(this, pValue);
  else
  {
    CPtr<IManipulator> pMan = GetPropertyAsManipulator(pProp, nIndex);
    return pMan != nullptr ? pMan->GetValue(szRest.c_str(), pValue) : 0;
  }
}
...
bool CManipulator::SetValue(const char *pszValueName, const variant_t &value)
{
  ...
  if (pProp->nodeType == SBaseProperty::LEAF) return pProp->Set(this, value);
  else
  {
    CPtr<IManipulator> pMan = GetPropertyAsManipulator(pProp, nIndex);
    return pMan != nullptr ? pMan->SetValue(szRest.c_str(), value) : 0;
  }
}
...
IManipulatorIterator *CManipulator::Iterate() { return new CManipulatorIterator(this); }

const SPropertyDesc *CManipulatorIterator::GetFirst()
{
  if (!pPropLocal) return nullptr;
  if ((pPropLocal->nodeType == SBaseProperty::SINGLE) && (nPropIndexLocal != -1)) return nullptr;
  if ((pPropLocal->nodeType == SBaseProperty::VECTOR) && (nPropIndexLocal < 0)) return nullptr;
  //
  ...
  //
  if (pMan == nullptr) return nullptr;
  pItLocal = pMan->Iterate();
  if (pItLocal->IsEnd()) return GetNext();
...
{
  if ((pProperties == nullptr) || (itProps == pProperties->end())) return false;
  ++itProps;
  if (IsEnd())
  {
    pPropLocal = nullptr;
...
{
  if (pPropLocal == nullptr) return nullptr;
  //
...
const SPropertyDesc *CManipulatorIterator::GetNextProperty()
{
  ...
  if (GetNextLocal() == false) return nullptr;
  if (pPropLocal->nodeType == SBaseProperty::LEAF)
  {
    szPropName = itProps->szName;
    propDesc.pszName = szPropName.c_str();
...
struct SVariantCmpEq1
{
  ...
  VARIANT variant;
  //
  explicit SVariantCmpEq1(const VARIANT &var) { variant = var; }
...
void CMultiManipulator::AddManipulator(IManipulator *pMan)
{
  ...
  for (CPtr<IManipulatorIterator> pIt = pMan->Iterate(); !pIt->IsEnd(); pIt->Next(), ++nOrder)
  {
    ...
    if (pDesc)
    {
      ...
      {
        if (prop.ePropType != pDesc->ePropType) --prop.nCounter;
        // values ​​intersection
        if (!prop.values.empty())
        {
          for (std::vector<variant_t>::iterator it = prop.values.begin(); it != prop.values.end();)
          {
            if (std::find_if(pDesc->values.begin(), pDesc->values.end(), SVariantCmpEq1(*it)) == pDesc->values.end()) it = prop.values.erase(it);
            else ++it;
          }
        }
      }
...
{
  bool operator()(const SMultiManipulatorProperty *pProp1, const SMultiManipulatorProperty *pProp2) const { return pProp1->nOrderIndex < pProp2->nOrderIndex; }
};

void CMultiManipulator::BuildProps()
{
  if (bPropsAlreadyBuilt) return;
  //
  int nMans = manipulators.size();
  for (CPropsMap::iterator it = propsMap.begin(); it != propsMap.end(); ++it) { if (it->second.nCounter == nMans) propsList.push_back(&(it->second)); }
  //
...
bool CMultiManipulator::GetValue(const char *pszValueName, variant_t *pValue)
{
  ...
  {
    if ((*it)->GetValue(pszValueName, &tmpvar) == false) continue;
    ++nPropsCounter;
    //
    if (var.vt == VT_EMPTY) var = tmpvar;
    else
    {
