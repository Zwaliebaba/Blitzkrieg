...
namespace NStr
{
  ...
  const char GetCloseBracket(char cOpenBracket);
  // Is the character an opening parenthesis?
  bool IsOpenBracket(char cSymbol);
  // add a new pair of parentheses
  void AddBrackets(char cOpenBracket, char cCloseBracket);
  // remove a couple of parentheses
  void RemoveBrackets(char cOpenBracket, char cCloseBracket);
  // split a string into an array of strings using a given delimiter
  void SplitString(const std::string &szString, std::vector<std::string> &szVector, char cSeparator);
  void SplitString(const std::wstring &szString, std::vector<std::wstring> &szVector, wchar_t cSeparator);
  // divide a string into an array of strings using a given separator, taking into account parentheses of the same nesting
  void SplitStringWithBrackets(const std::string &szString, std::vector<std::string> &szVector, char cSeparator);
  // divide a string into an array of strings using a given separator, taking into account parentheses of any nesting
  void SplitStringWithMultipleBrackets(const std::string &szString, std::vector<std::string> &szVector, char cSeparator);
  // find the closing parenthesis without taking into account the inner parentheses
  int FindCloseBracket(const std::string &szString, int nPos, char cOpenBracket);
  // find the closing parenthesis taking into account the inner parentheses
  int FindMultipleCloseBracket(const std::string &szString, int nPos, char cOpenBracket);
  ...
  int GetStrLen(const std::basic_string<T1, T2, T3> &szString) { return szString.length(); }
  ...
  void TrimRight(std::string &szString, char cTrim);
  ...
  inline void TrimBoth(std::string &szString, const char *pszTrim)
  {
    TrimLeft(szString, pszTrim);
    TrimRight(szString, pszTrim);
  }

  // cut off all 'cTrim' from both ends
  inline void TrimBoth(std::string &szString, const char cTrim)
  {
    TrimLeft(szString, cTrim);
    TrimRight(szString, cTrim);
  }
  ...
  void ToDotString(std::string *pDst, int nVal, char cSeparator = '.');
  ...
  public:
    CStringIterator(const char *pszInput, TSeparator &_tSeparator, int _nPos = 0)
      : tSeparator(_tSeparator), szInput(pszInput), nLastPos(_nPos), nPos(_nPos) { Next(); }

    CStringIterator(const std::string &_szInput, TSeparator &_tSeparator, int _nPos = 0)
      : tSeparator(_tSeparator), szInput(_szInput), nLastPos(_nPos), nPos(_nPos) { Next(); }
