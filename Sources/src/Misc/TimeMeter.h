...
{
  static void Report(const char *pszName, const int nMilliseconds) { NStr::DebugTrace("*** Time meter for \"%s\" = %d\n", pszName, nMilliseconds); }
};
...
public:
  CTimeMeter() { Reset(); }
  void Reset() { NHPTimer::GetTime(&time); }
  void Sample(const char *pszName) { TReporter::Report(pszName, NHPTimer::GetTimePassed(&time) * 1000.0); }
};
