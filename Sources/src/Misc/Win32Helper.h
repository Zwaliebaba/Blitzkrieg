...
namespace NWin32Helper
{
  ...
  public:
    CEvent(bool bInitState = false, bool bManualReset = true) { h = CreateEvent(nullptr, bManualReset, bInitState, nullptr); }
  ...
  public:
    CCriticalSectionLock(CCriticalSection &_lock) : lock(_lock)
    {
      bInsideCriticalSection = true;
      lock.Enter();
    }
    ...
    void Leave()
    {
      if (bInsideCriticalSection) lock.Leave();
      bInsideCriticalSection = false;
    }
  };
