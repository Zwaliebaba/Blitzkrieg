...
CGSQueryReportingDriver::~CGSQueryReportingDriver()
{
  if (bInitialized)
  {
    gameInfo.eGameMode = ESGM_EXITING;
...
bool CGSQueryReportingDriver::Init(const APPLICATION_ID _nApplicationID, int _nGamePort, bool _bClientOnly)
{
  ...
  const int nResult =
      qr_init(
          &gsHandler, nullptr, &nGamePort, GetGlobalVar("GameSpyGameName"), szSecretKey.c_str(),
          qr_basic_callback,
          qr_info_callback,
          qr_rules_callback,
          qr_players_callback,
...
{
  if (bInitialized) return ACTIVE;
  return INACTIVE;
}

// //////////////////////////////////////////////
INetDriver::EReject CGSQueryReportingDriver::GetRejectReason() const { return NONE; }
// //////////////////////////////////////////////
...
// //////////////////////////////////////////////
void CGSQueryReportingDriver::StopGameInfoSend() { CCriticalSectionLock criticalSectionLock(criticalSection); }
// //////////////////////////////////////////////
void CGSQueryReportingDriver::StartNewPlayerAccept()
{
  ...
  if (GetState() == ACTIVE) qr_send_statechanged(gsHandler);
}
...
{
  CCriticalSectionLock criticalSectionLock(criticalSection);
  if (GetState() == ACTIVE) qr_process_queries(gsHandler);
}
...
{
  CCriticalSectionLock criticalSectionLock(criticalSection);
  if (GetSingleton<IGlobalVars>()) sprintf(pszOutBuf, "\\gamename\\%s\\gamever\\%d", GetGlobalVar("GameSpyGameName"), GetGlobalVar("NetGameVersion", 1));
}
...
void CGSQueryReportingDriver::QRInfoCallBack(char *pszOutBuf, int nMaxLen)
{
  ...
  for (int i = 0; i < szGSMapName.size(); ++i) { if (szGSMapName[i] == '\\') szGSMapName[i] = '/'; }

  //
  std::string szGameMode = GetMode(gameInfo.eGameMode);

  std::string szGameType = gameInfo.szGameType;
  if (gameInfo.szModName != "" || gameInfo.szModVersion != "") szGameType = gameInfo.szModName + "/" + gameInfo.szModVersion + "/" + szGameType;
  ...
  if (gameInfo.bPasswordRequired) szFormatString += NStr::Format("\\password\\1");
...
// //////////////////////////////////////////////
void CGSQueryReportingDriver::QRPlayersCallBack(char *pszOutBuf, int nMaxLen) { CCriticalSectionLock criticalSectionLock(criticalSection); }
// //////////////////////////////////////////////
// callbacks
void CGSQueryReportingDriver::qr_basic_callback(char *pszOutBuf, int nMaxLen, void *pUserData) { reinterpret_cast<CGSQueryReportingDriver *>(pUserData)->QRBasicCallBack(pszOutBuf, nMaxLen); }
// //////////////////////////////////////////////
void CGSQueryReportingDriver::qr_info_callback(char *pszOutBuf, int nMaxLen, void *pUserData) { reinterpret_cast<CGSQueryReportingDriver *>(pUserData)->QRInfoCallBack(pszOutBuf, nMaxLen); }
// //////////////////////////////////////////////
void CGSQueryReportingDriver::qr_rules_callback(char *pszOutBuf, int nMaxLen, void *pUserData) { reinterpret_cast<CGSQueryReportingDriver *>(pUserData)->QRRulesCallBack(pszOutBuf, nMaxLen); }
// //////////////////////////////////////////////
void CGSQueryReportingDriver::qr_players_callback(char *pszOutBuf, int nMaxLen, void *pUserData) { reinterpret_cast<CGSQueryReportingDriver *>(pUserData)->QRPlayersCallBack(pszOutBuf, nMaxLen); }
// //////////////////////////////////////////////