...
namespace NNet
{
  constexpr int CS_PACKET_ID_RANGE = 0x10000;
  constexpr float FP_RTT_KEEP = 0.93f;// RTT recalc rate
  constexpr float TIME_TO_UPDATE_PING = 4;
  ...
  CAckTracker::CAckTracker()
  {
    ...
    fCurrentTime = 0;
    // nUpdateSize = 220;//512;
    nPktSent = static_cast<PACKET_ID>(dwTick);
    nPktLastReceived = 0;
  ...
  {
    if (nFlyPackets + 1 <= fWindow) { return true; }
    return false;
    /* if ( fUpdateTimeElapsed > fUpdateTimeDelay )
       */
  }

  bool CAckTracker::NeedSend() const { return fTimeSinceLastSend > 1; }
  ...
  void CAckTracker::Step(std::vector<PACKET_ID> *pRolled, std::vector<PACKET_ID> *pErased, double fDeltaTime)
  {
    ...
    fRTTDiscard = Min(fRTTDiscard, static_cast<float>(nMaxRTT));
  ...
  void CAckTracker::ReceiveAck(std::vector<PACKET_ID> *pAcked, PACKET_ID nPkt)
  {
    ...
    for (i = sentUpdates.begin(); i != sentUpdates.end();)
    {
      ...
      {
        --nFlyPackets;
        if (i->bOnTheWindowEdge) fWindow += 0.02f;// /fWindow;
    ...
    // this should be very rare case so performance is not critical
  ...
  void CAckTracker::ReceivePktAcks(std::vector<PACKET_ID> *pAcked, CBitStream &bits)
  {
    ...
    if (nShift > CS_PACKET_ID_RANGE / 2) nShift -= CS_PACKET_ID_RANGE;
    if (nShift <= 0)
    {
      if (nShift < -32) dwAckedBits = 0;
      else dwAckedBits >>= -nShift;
    }
    else
    {
      dwAckedBits <<= 1;
      dwAckedBits |= 1;
      nShift--;
      if (nShift < 32) dwAckedBits <<= nShift;
      else dwAckedBits = 0;
    }
    ...
    {
      nLast--;
      if (dwNewAckBits & (1 << (i - 1))) ReceiveAck(pAcked, nLast);
    }
  }
  ...
  bool CAckTracker::CheckRecvPacketNumber(UPDATE_ID nPkt)
  {
    ...
    for (auto k = receivedPkts.begin(); k != receivedPkts.end(); ++k)
    {
      if (*k == nPkt)
      {
  ...
  bool CAckTracker::ReadAcks(std::vector<PACKET_ID> *pAcked, CBitStream &bits)
  {
    ...
    if (nPktLastReceived == 0) nPktLastReceived = nCurUpdate;
    int nDif = nCurUpdate - nPktLastReceived;
    if (nDif > CS_PACKET_ID_RANGE / 2) nCurUpdate -= CS_PACKET_ID_RANGE;
    ...
    if (!CheckRecvPacketNumber(nCurUpdate)) return false;
  ...
  void CAckTracker::SendPktAcks(CBitStream *pBits)
  {
    int i;
    DWORD dwBits = 0;
    PACKET_ID nLast = static_cast<PACKET_ID>(nPktLastReceived);
    ...
    for (i = 1; i <= 32; i++)
    {
      ...
      if (k != receivedPkts.end()) dwBits |= 1 << (i - 1);
    }
    pBits->Write(&nLast, sizeof(nLast));
    pBits->Write(&dwBits, 4);
    // removing too old ack Pkts
    for (i = receivedPkts.size() - 1; i >= 0; i--) { if (receivedPkts[i] + 32 < nPktLastReceived) receivedPkts.erase(receivedPkts.begin() + i); }
  }
