...
namespace NNet
{
  ...
  bool CStreamTracker::IsBefore(CHANNEL_DATA_OFFSET border, CHANNEL_DATA_OFFSET test) { return test - border > 0x80000000; }
  // //////////
  void CStreamTracker::WriteMsg(PACKET_ID nPkt, CBitStream *pBits, int nSizeLimit)
  {
    ...
    {
      // find first block
      auto i = channelOutList.begin(), ibest = i;
  ...
  void CStreamTracker::ReadMsg(CBitStream &bits)
  {
    ...
    for (;;)
    {
      ...
      /* int nCount = 0;
           */
      for (auto i = channelInList.begin(); i != channelInList.end();)
      {
        if (i->nOffset == nChannelInputOffset)
        {
          if (channelInBuf.GetBufSize() < i->nLength) break;// avoid buffer overflow
    ...
    for (auto i = channelInList.begin(); i != channelInList.end();)
    {
      if (IsBefore(nChannelInputOffset, i->nOffset)) i = channelInList.erase(i);
      else ++i;
    }
  }
