...
namespace NNet
{
  ...
  public:
    ...
    int Peek(void *pBuf, int nBufSize)
    {
      ...
      if (nReadPtr + nRes > N_SIZE)
      {
        ...
        memcpy(static_cast<char *>(pBuf) + nRight, szBuffer, nRes - nRight);
      }
      else { memcpy(pBuf, szBuffer + nReadPtr, nRes); }
      return nRes;
    }
    ...
    int Write(const void *pBuf, int nLength)
    {
      ...
      if (nWritePtr + nRes > N_SIZE)
      {
        ...
        memcpy(szBuffer, static_cast<const char *>(pBuf) + nRight, nRes - nRight);
  ...
  constexpr int N_STREAM_BUFFER = 32768;
  constexpr int N_MAX_PACKET_SIZE = 1024;// low level packet size
  ...
  private:
    ...
    using CHANNEL_DATA_OFFSET = unsigned int;
#pragma pack(push,1)
...
#pragma pack(pop)
    ...
    // current offset of sent and received data
