...
bool CDataStream::ReadString(std::string &res, int nMaxSize)
{
  int nSize = 0;
  Read(&nSize, 1);
  if (nSize & 1) Read(((char *) &nSize) + 1, 3);
  nSize >>= 1;
...
void CFixedMemStream::AllocForDirectWriteAccess(unsigned int nSize) { ASSERT(0); }
...
void CMemoryStream::FixupBufferSize(int nNewSize)
{
  ...
  auto pNewBuf = new unsigned char[nNewSize];
...
void CMemoryStream::AllocForDirectReadAccess(unsigned int nSize) { FixupBufferSize(GetPosition() + nSize); }

// must store the current buffer contents in memory
void CMemoryStream::AllocForDirectWriteAccess(unsigned int nSize) { FixupBufferSize(GetPosition() + nSize); }
...
CMemoryStream::~CMemoryStream() { delete[] pBuffer; }
...
CMemoryStream::CMemoryStream(const CMemoryStream &src) { CopyMemoryStream(src); }
...
void CBufferedStream::FlushBuffer()
{
  if (IsWasted())
  {
    ...
    if (DoWrite(nBufferStart, pBuffer, nSize) != nSize) SetFailed();// throw
    FixupSize();
  }
}
...
void CBufferedStream::SetNewBufferSize(unsigned int nSize)
{
  ...
  if (nNewSize < pReservedEnd - pBuffer) return;
  auto pNewBuf = new unsigned char [nNewSize];
  if (pBuffer)
  {
    memcpy(pNewBuf, pBuffer, pReservedEnd - pBuffer);
...
void CBufferedStream::LoadBufferForced(int nPos)
{
  ...
  if (DoRead(nPos, pBuffer, nRead) != nRead) SetFailed();// throw
...
void CBufferedStream::AllocForDirectWriteAccess(unsigned int nSize) { SetNewBufferSize(nSize); }

void CBufferedStream::NotifyFinishDirectAccess() { ShiftBuffer(); }
