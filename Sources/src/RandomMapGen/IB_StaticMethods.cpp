...
void CSpritesPackBuilder::GetActualRect(SSpritesPack::CSpritesList::const_iterator spritesListIterator, CTRect<int> *pActualRect)
{
  ...
  for (auto squareIterator = spritesListIterator->squares.begin(); squareIterator != spritesListIterator->squares.end(); ++squareIterator)
  {
    ...
    if (actualSquareRect.minx < bounds.minx) { bounds.minx = actualSquareRect.minx; }
    if (actualSquareRect.miny < bounds.miny) { bounds.miny = actualSquareRect.miny; }
    if (actualSquareRect.maxx > bounds.maxx) { bounds.maxx = actualSquareRect.maxx; }
    if (actualSquareRect.maxy > bounds.maxy) { bounds.maxy = actualSquareRect.maxy; }
  }
  if (pActualRect) { (*pActualRect) = bounds; }
}
...
{
  // determine the area occupied by the squares
  ...
  for (auto spritesListIterator = pSpritesPack->sprites.begin(); spritesListIterator != pSpritesPack->sprites.end(); ++spritesListIterator)
  {
    for (auto squareIterator = spritesListIterator->squares.begin(); squareIterator != spritesListIterator->squares.end(); ++squareIterator)
    {
      if (pCollectedSquareSideSize->max < static_cast<int>(squareIterator->fSize)) { pCollectedSquareSideSize->max = static_cast<int>(squareIterator->fSize); }
      if (pCollectedSquareSideSize->min > static_cast<int>(squareIterator->fSize)) { pCollectedSquareSideSize->min = static_cast<int>(squareIterator->fSize); }
      ++(*pnSquaresCount);
  ...
  // determine the size of the packed image based on the required area
  pMinimalImageSize->x = 1;
  pMinimalImageSize->y = 1;
  ...
  while ((pMinimalImageSize->x * pMinimalImageSize->y) < nPackedSquareSize)
  {
    ...
    if ((pMinimalImageSize->x * pMinimalImageSize->y) < nPackedSquareSize) { pMinimalImageSize->y *= 2; }
  }
...
bool CSpritesPackBuilder::CollectSquares(int nCurrentDepth, const CTPoint<int> &rLeftTop, int nSquareSideSize, SCollectSquaresParameter *pParameter)
{
  ...
  for (int nSquareYIndex = 0; nSquareYIndex < 2; ++nSquareYIndex)
  {
    for (int nSquareXIndex = 0; nSquareXIndex < 2; ++nSquareXIndex)
    {
      ...
      {
        // greatest recursion depth
        // if the square falls into the picture
  ...
  {
    // remove squares
    for (int index = 0; index < nSquaresCount; ++index) { pParameter->pSprite->squares.pop_back(); }
    return true;
  }
...
bool CSpritesPackBuilder::CollectSquares(SSpritesPack *pSpritesPack, const CPackParameters &rPackParameters, int nMaxSquareSideSize, int nMaxDepth, int nMinDepth)
{
  ...
  for (auto packParameterIterator = rPackParameters.begin(); packParameterIterator != rPackParameters.end(); ++packParameterIterator)
  {
    // add a packed image - empty for now
    ...
    // center the squares relative to the origin of coordinates
    if ((pSpritesPack->sprites.back().center.x % nMaxSquareSideSize) != 0) { collectSquaresParameter.shift.x = nMaxSquareSideSize - (pSpritesPack->sprites.back().center.x % nMaxSquareSideSize); }
    else { collectSquaresParameter.shift.x = 0; }
    if ((pSpritesPack->sprites.back().center.y % nMaxSquareSideSize) != 0) { collectSquaresParameter.shift.y = nMaxSquareSideSize - (pSpritesPack->sprites.back().center.y % nMaxSquareSideSize); }
    else { collectSquaresParameter.shift.y = 0; }

    // the number of squares should be sufficient to contain the entire picture
    ...
    if ((shiftedImageSize.x % nMaxSquareSideSize) != 0) { shiftedImageSize.x += nMaxSquareSideSize - (shiftedImageSize.x % nMaxSquareSideSize); }
    if ((shiftedImageSize.y % nMaxSquareSideSize) != 0) { shiftedImageSize.y += nMaxSquareSideSize - (shiftedImageSize.y % nMaxSquareSideSize); }

    // number of squares of maximum size
    mainSquaresCount.x = (shiftedImageSize.x / nMaxSquareSideSize);
    mainSquaresCount.y = (shiftedImageSize.y / nMaxSquareSideSize);

    // collecting squares of various sizes
    for (int nMainYIndex = 0; nMainYIndex < mainSquaresCount.y; ++nMainYIndex)
    {
      for (int nMainXIndex = 0; nMainXIndex < mainSquaresCount.x; ++nMainXIndex)
      {
        ...
        {
          // it is necessary to insert a square of the maximum size
...
IImage *CSpritesPackBuilder::Pack(SSpritesPack *pSpritesPack, const CPackParameters &rPackParameters, int nMaxSquareSideSize, int nDepth)
{
  ...
  // area of ​​the image with maximum depth and one less depth:
  {
    ...
    if (!CollectSquares(&spritesPackMaxDepth, rPackParameters, nMaxSquareSideSize, nDepth, 1)) { return nullptr; }
    ...
    if (nDepth > 1)
    {
      ...
      if (!CollectSquares(&spritesPackSubDepth, rPackParameters, nMaxSquareSideSize, nDepth - 1, 1)) { return nullptr; }
  ...
  if (nSquaresCount == 0) { return nullptr; }

  // place the squares in a new picture (for now we’re just setting the texture coordinates)
  ...
  int nGranularity = collectedSquareSideSize.max /
                     collectedSquareSideSize.min;// how many minimal squares to locate when writing a square
  int nFilledSquaresCount = 0;// how many squares are there in total?
  float fFilledSquareSideSize = collectedSquareSideSize.max;// current size of the square being written
  CTPoint<int> filledLockedSquare(0, 0);// current coordinate of the square being written in the array of locked squares
  while (nFilledSquaresCount < nSquaresCount)
  {
    NI_ASSERT_T(( nGranularity > 0 ),
                NStr::Format( "Invalid prediction: (%d) in [%d]", nGranularity, collectedSquareSideSize.max / collectedSquareSideSize.min ));
        ...
        if (squareIterator->fSize == fFilledSquareSideSize)
        {
          while (true)
          {
            while ((filledLockedSquare.x < lockedSquares.GetSizeX()) &&
                   (lockedSquares[filledLockedSquare.y][filledLockedSquare.x] == RMGC_LOCKED)) { filledLockedSquare.x += nGranularity; }
            if ((filledLockedSquare.x < lockedSquares.GetSizeX())) { break; }
          ...
          int nXPosition = filledLockedSquare.x * collectedSquareSideSize.min;
          int nYPosition = filledLockedSquare.y * collectedSquareSideSize.min;
          ...
          for (int nYIndex = 0; nYIndex < nGranularity; ++nYIndex) { for (int nXIndex = 0; nXIndex < nGranularity; ++nXIndex) { lockedSquares[filledLockedSquare.y + nYIndex][filledLockedSquare.x + nXIndex] = RMGC_LOCKED; } }
          ++nFilledSquaresCount;
        }
      }
    }
  ...
  // create a packed picture and place images in it
  IImage *pPackedImage = pImageProcessor->CreateImage(packedImageSize.x, packedImageSize.y);
  if (pPackedImage == nullptr) { return nullptr; }
  pPackedImage->Set(0);

  auto packParameterIterator = rPackParameters.begin();
  for (auto spritesListIterator = pSpritesPack->sprites.begin(); spritesListIterator != pSpritesPack->sprites.end(); ++spritesListIterator)
  {
    if (packParameterIterator != rPackParameters.end())
    {
      ...
      for (auto squareIterator = spritesListIterator->squares.begin(); squareIterator != spritesListIterator->squares.end(); ++squareIterator)
      {
        ...
        // determine for each square its depth
        if (pLockArrayImage)
        {
          ...
          if ((nLockArrayMinXIndex >= 0) && (nLockArrayMinXIndex < pLockArrayImage->GetSizeX()))
          {
            for (int nYIndex = (pLockArrayImage->GetSizeY() - 1); nYIndex >= 0; --nYIndex)
            {
              ...
              {
                nLockArrayLeftIndex = (-1) * (nYIndex + imageActualSquareRect.miny);// upside down
          ...
          if ((nLockArrayMaxXIndex >= 0) && (nLockArrayMaxXIndex < pLockArrayImage->GetSizeX()))
          {
            for (int nYIndex = (pLockArrayImage->GetSizeY() - 1); nYIndex >= 0; --nYIndex)
            {
              ...
              {
                nLockArrayRightIndex = (-1) * (nYIndex + imageActualSquareRect.miny);// upside down
          ...
          if ((nMinXIndex >= 0) && (nMinXIndex < packParameterIterator->pImage->GetSizeX()))
          {
            for (int nYIndex = (packParameterIterator->pImage->GetSizeY() - 1); nYIndex >= 0; --nYIndex)
            {
              ...
              {
                nLeftIndex = spritesListIterator->center.y - nYIndex;// upside down
                bLeftFilled = true;
                break;
              }
            }
          ...
          if ((nMaxXIndex >= 0) && (nMaxXIndex < packParameterIterator->pImage->GetSizeX()))
          {
            for (int nYIndex = (packParameterIterator->pImage->GetSizeY() - 1); nYIndex >= 0; --nYIndex)
            {
              ...
              {
                nRightIndex = spritesListIterator->center.y - nYIndex;// upside down
                bRightFilled = true;
                break;
              }
            }
          ...
          if (bLeftFilled && !bRightFilled) { nRightIndex = nLeftIndex; }

          if (bRightFilled && !bLeftFilled) { nLeftIndex = nRightIndex; }

          if (bLockArrayLeftFilled &&
              (((nLeftIndex - nLockArrayLeftIndex) > (nCellSizeY / 2)) ||
               true /* ( nLeftIndex < nLockArrayLeftIndex ) */)) { nLeftIndex = nLockArrayLeftIndex; }

          if (bLockArrayRightFilled &&
              (((nRightIndex - nLockArrayRightIndex) > (nCellSizeY / 2)) ||
               true /* ( nRightIndex < nLockArrayRightIndex ) */)) { nRightIndex = nLockArrayRightIndex; }
      ...
      // create a sprite border for precise mouse selection
      ...
      // if there is the required alpha!
      if (!spritesListIterator->squares.empty())
      {
        ...
        for (int nYindex = 0; nYindex < imageSize.y; ++nYindex)
        {
          for (int nXindex = 0; nXindex < imageSize.x; ++nXindex)
          {
            ...
            {
              if (bounds.minx > nXindex) { bounds.minx = nXindex; }
              if (bounds.maxx < nXindex) { bounds.maxx = nXindex; }
              if (bounds.miny > nYindex) { bounds.miny = nYindex; }
              if (bounds.maxy < nYindex) { bounds.maxy = nYindex; }
            }
          }
        }
        ...
        // we get the number of elements with horizontal edges
        ...
        if ((nHorizontalElements > 0) || (nVerticalElements > 0))
        {
          ...
          if (nHorizontalElements == 0) { spritesListIterator->edge.bHorizontal = false; }
          else if (nVerticalElements == 0) { spritesListIterator->edge.bHorizontal = true; }
              ...
              for (int nXindex = bounds.minx; nXindex <= bounds.maxx; ++nXindex)
              {
                if ((imageAccessor[nYindex][nXindex].a < packParameterIterator->dwMinAlpha) == isOdd)
                {
                  if ((nXindex != bounds.maxx) &&
                      ((imageAccessor[nYindex][nXindex + 1].a < packParameterIterator->dwMinAlpha) == isOdd))
                  {
                    spritesListIterator->edge.edges[nYindex - bounds.miny][nElements] = static_cast<short>((nXindex - bounds.minx) - (isOdd ? 1 : 0));
                    ++nElements;
                    isOdd = !isOdd;
                  }
                }
              }
              if (isOdd) { spritesListIterator->edge.edges[nYindex - bounds.miny][nElements] = static_cast<short>(bounds.maxx - bounds.minx); }
            }
          }
          else
          {
              ...
              for (int nYindex = bounds.miny; nYindex <= bounds.maxy; ++nYindex)
              {
                if ((imageAccessor[nYindex][nXindex].a < packParameterIterator->dwMinAlpha) == isOdd)
                {
                  if ((nYindex != bounds.maxy) &&
                      ((imageAccessor[nYindex + 1][nXindex].a < packParameterIterator->dwMinAlpha) == isOdd))
                  {
                    spritesListIterator->edge.edges[nXindex - bounds.minx][nElements] = static_cast<short>(nYindex - bounds.miny - (isOdd ? 1 : 0));
                    ++nElements;
                    isOdd = !isOdd;
                  }
                }
              }
              if (isOdd) { spritesListIterator->edge.edges[nXindex - bounds.minx][nElements] = static_cast<short>(bounds.maxy - bounds.miny); }
            }
          }
        }
      }
...
{
  NI_ASSERT_TF(( pSpritesPack != nullptr ) && ( pPackedImage != nullptr ),
  ...
  if (nSpriteIndex >= pSpritesPack->sprites.size()) { return nullptr; }

  // picture
  SSpritesPack::CSpritesList::const_iterator spritesListIterator = pSpritesPack->sprites.begin();
  for (int nSpritePackIndex = 0; nSpritePackIndex < nSpriteIndex; ++nSpritePackIndex) { ++spritesListIterator; }
...
IImage *CSpritesPackBuilder::UnpackAndMarkEdge(SSpritesPack *pSpritesPack, IImage *pPackedImage, int nSpriteIndex, CTRect<int> *pActualRect)
{
  ...
  if (pUnpackedImage == nullptr) { return nullptr; }
  IImageProcessor *pImageProcessor = GetImageProcessor();
  NI_ASSERT_TF(pImageProcessor != 0,
               NStr::Format( "Can't get IImageProcessor: %x\n", pImageProcessor ),
               return 0);
  ...
  if (pActualRect) { (*pActualRect) = actualRect; }
...
bool CRMImageBuilder::ApplyFilter(IImage *pImage, const CArray2D<int> &rFilter, DWORD dwMinAlpha)
{
  ...
  if (!pImageProcessor) { return false; }

  CPtr<IImage> pDestImage = pImageProcessor->CreateImage(pImage->GetSizeX(), pImage->GetSizeY());
  if (!pDestImage) { return false; }

  int nDivider = 0;
  for (int nXIndex = 0; nXIndex < rFilter.GetSizeX(); ++nXIndex) { for (int nYIndex = 0; nYIndex < rFilter.GetSizeY(); ++nYIndex) { nDivider += rFilter[nYIndex][nXIndex]; } }
  ...
  // ********
  // ********
  for (int nXIndex = indices.minx; nXIndex < indices.maxx; ++nXIndex) { for (int nYIndex = indices.miny; nYIndex < indices.maxy; ++nYIndex) { filterFunctional(nXIndex, nYIndex); } }
  ...
  {
    for (int nXIndex = 0; nXIndex < indices.minx; ++nXIndex) { filterInBoundsFunctional(nXIndex, nYIndex); }
    for (int nXIndex = indices.maxx; nXIndex < pImage->GetSizeX(); ++nXIndex) { filterInBoundsFunctional(nXIndex, nYIndex); }
  }
  ...
  for (int nXIndex = indices.minx; nXIndex < indices.maxx; ++nXIndex)
  {
    for (int nYIndex = 0; nYIndex < indices.miny; ++nYIndex) { filterInBoundsFunctional(nXIndex, nYIndex); }
    for (int nYIndex = indices.maxy; nYIndex < pImage->GetSizeY(); ++nYIndex) { filterInBoundsFunctional(nXIndex, nYIndex); }
  }

  return pImage->CopyFrom(pDestImage, nullptr, 0, 0);
}
...
bool CRMImageBuilder::Emboss(IImage *pImage, const CTPoint<int> &rShiftPoint, const CArray2D<int> &rFilter, DWORD dwMinAlpha)
{
  ...
  if (!pHeightsImage) { return false; }

  CImageAccessor heightsImageAccessor = pHeightsImage;
  CImageAccessor imageAccessor = pImage;

  for (int nXIndex = 0; nXIndex < pImage->GetSizeX(); ++nXIndex) { for (int nYIndex = 0; nYIndex < pImage->GetSizeY(); ++nYIndex) { heightsImageAccessor[nYIndex][nXIndex] = (imageAccessor[nYIndex][nXIndex].a >= dwMinAlpha) ? WHITE_COLOR : BLACK_COLOR; } }

  if (!ApplyFilter(pHeightsImage, rFilter, 1)) { return false; }
  ...
  for (int nXIndex = 0; nXIndex < pImage->GetSizeX(); ++nXIndex)
  {
    for (int nYIndex = 0; nYIndex < pImage->GetSizeY(); ++nYIndex)
    {
      ...
      if ((nXIndex < -(rShiftPoint.x)) ||
          ...
          (nYIndex >= (pImage->GetSizeY() - rShiftPoint.y))) { shiftedHeigthColor = heightsImageAccessor[nYIndex][nXIndex]; }
      else { shiftedHeigthColor = heightsImageAccessor[nYIndex + rShiftPoint.y][nXIndex + rShiftPoint.x]; }
...
{
  NI_ASSERT_T(( pImage != nullptr ) && ( pNoise != nullptr ),
...
{
  NI_ASSERT_T(( pImage != nullptr ) && ( pImageToAdd != nullptr ),
              NStr::Format( "Wrong parameter: %x, %x\n", pImage, pImageToAdd ));
  ...
  for (int nYIndex = 0; nYIndex < pImage->GetSizeY(); ++nYIndex)
  {
    for (int nXIndex = 0; nXIndex < pImage->GetSizeX(); ++nXIndex)
    {
      ...
      if (rColor.a >= dwMinAlpha) { imageAccessor[nYIndex][nXIndex] = rColor; }
    }
  }
...
bool CRMImageBuilder::FastAddImageByColor(IImage *pImage, IImage *pImageToAdd, SColor color)
{
  ...
  for (int nYIndex = 0; nYIndex < pImage->GetSizeY(); ++nYIndex)
  {
    ...
    {
      SColor &rColor = imageToAddAccessor[nYIndex][nXIndex];
      if (rColor.a != color) { imageAccessor[nYIndex][nXIndex] = rColor; }
    }
  }
...
IImage *CRMImageBuilder::GetEdge(IImage *pImage, SColor edgeColor, SColor nonEdgeColor, DWORD dwMinAlpha)
{
  ...
  CPtr<IImageProcessor> pImageProcessor = GetImageProcessor();
  if (!pImageProcessor) { return nullptr; }
  ...
  if (pEdgedImage)
  {
    ...
    // ********
    // ********
    for (int nXIndex = 1; nXIndex < (pImage->GetSizeX() - 1); ++nXIndex) { for (int nYIndex = 1; nYIndex < (pImage->GetSizeY() - 1); ++nYIndex) { getImageEdgeFunctional(nXIndex, nYIndex); } }
    // left and right edges
    // 00****00
    // 00****00
    // 00****00
      ...
      for (int nXIndex = 0; nXIndex < pImage->GetSizeX(); ++nXIndex)
      {
        if (imageAccessor[nYIndex][nXIndex].a >= dwMinAlpha)
        {
          ...
          if ((shiftPoint.x >= 0) &&
              ...
              (shiftPoint.y < pImage->GetSizeY())) { if (imageAccessor[shiftPoint.y][shiftPoint.x] < dwMinAlpha) { shadowImageAccessor[shiftPoint.y][shiftPoint.x] = shadowColor; } }
        }
      }
    }
  }
...
IImage *CRMImageBuilder::GetAlphaEmboss(IImage *pImage, const CTPoint<int> &rShiftPoint, int nFilterSize, DWORD dwMinAlpha)
{
  ...
  CPtr<IImage> pHeightsImage = pImageProcessor->CreateImage(pImage->GetSizeX(), pImage->GetSizeY());
  if (!pHeightsImage) { return nullptr; }

  // shifts relative to the starting point
  CTPoint<int> frontShift((rShiftPoint.x / 2), (rShiftPoint.y / 2));
  CTPoint<int> backShift(frontShift * (-1));
  if ((rShiftPoint.x & 0x01) > 0) { frontShift.x += (rShiftPoint.x > 0) ? 1 : (-1); }
  if ((rShiftPoint.y & 0x01) > 0) { frontShift.y += (rShiftPoint.y > 0) ? 1 : (-1); }
  ...
  if (!ApplyFilter(pHeightsImage, embossFilter, dwMinAlpha)) { return nullptr; }
    ...
    for (int nXIndex = 0; nXIndex < pImage->GetSizeX(); ++nXIndex)
    {
      for (int nYIndex = 0; nYIndex < pImage->GetSizeY(); ++nYIndex)
      {
        ...
        if (((nXIndex + frontShift.x) >= 0) &&
            ...
            ((nYIndex + frontShift.y) < pImage->GetSizeY())) { frontShiftHeigthColor = heightsImageAccessor[nYIndex + frontShift.y][nXIndex + frontShift.x]; }
        else { frontShiftHeigthColor = heightsImageAccessor[nYIndex][nXIndex]; }

        SColor backShiftHeigthColor;
        if (((nXIndex + backShift.x) >= 0) &&
            ((nYIndex + backShift.y) >= 0) &&
            ((nXIndex + backShift.x) < pImage->GetSizeX()) &&
            ((nYIndex + backShift.y) < pImage->GetSizeY())) { backShiftHeigthColor = heightsImageAccessor[nYIndex + backShift.y][nXIndex + backShift.x]; }
        else { backShiftHeigthColor = heightsImageAccessor[nYIndex][nXIndex]; }
...
IImage *CRMImageBuilder::FastComposeImagesByAlpha(const std::vector<CPtr<IImage>> &rImages, DWORD dwMinAlpha)
{
  ...
  if (pComposeImage)
  {
    ...
    for (int nImageIndex = 0; nImageIndex < rImages.size(); ++nImageIndex) { imageAccessors.push_back(CImageAccessor(rImages[nImageIndex])); }
