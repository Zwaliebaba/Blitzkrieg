...
void CMapInfo::GetTerrainCheckSum(uLong *pResourcesCheckSum, uLong *pMapCheckSum)
{
  ...
  for (int i = 0; i < terrain.altitudes.GetSizeY(); ++i) { for (int j = 0; j < terrain.altitudes.GetSizeX(); ++j) CopyToBuf(&bufMap, terrain.altitudes[i][j].fHeight); }
  ...
  for (int i = 0; i < tilesetDesc.terrtypes.size(); ++i)
  {
    ...
    for (int j = 0; j < tilesetDesc.terrtypes[i].tiles.size(); ++j) CopyToBuf(&bufRes, tilesetDesc.terrtypes[i].tiles[j].nIndex);
  ...
  for (int y = 0; y < terrain.tiles.GetSizeY(); ++y) { for (int x = 0; x < terrain.tiles.GetSizeX(); ++x) CopyToBuf(&bufMap, terrain.tiles[y][x].tile); }
...
void CMapInfo::ProcessObjectCheckSum(const char *pszObjectName, SCheckSumBufferStorage *pBufRes, SCheckSumBufferStorage *pBufMap)
{
  ...
  if (nRequiredSize >= pBufMap->buf.size()) pBufMap->buf.resize(nRequiredSize * 1.5);
...
void CMapInfo::GetEntrenchmentsCheckSum(uLong *pResourcesCheckSum, uLong *pMapCheckSum)
{
  ...
  for (int i = 0; i < entrenchments.size(); ++i) { for (int j = 0; j < entrenchments[i].sections.size(); ++j) { for (int k = 0; k < entrenchments[i].sections[j].size(); ++k) CopyToBuf(&bufMap, entrenchments[i].sections[j][k]); } }
...
void CMapInfo::GetBridgesCheckSum(uLong *pResourcesCheckSum, uLong *pMapCheckSum)
{
  ...
  for (int i = 0; i < bridges.size(); ++i) { for (int j = 0; j < bridges[i].size(); ++j) CopyToBuf(&bufMap, bridges[i][j]); }
...
void CMapInfo::GetDiplomaciesCheckSum(uLong *pResourcesCheckSum, uLong *pMapCheckSum)
{
  ...
  for (int i = 0; i < diplomacies.size(); ++i) CopyToBuf(&bufMap, diplomacies[i]);
...
void CMapInfo::GetUnitCreationCheckSum(uLong *pResourcesCheckSum, uLong *pMapCheckSum)
{
  ...
  for (int i = 0; i < unitCreation.units.size(); ++i)
  {
    ...
    for (auto iter = aviation.vAppearPoints.begin(); iter != aviation.vAppearPoints.end(); ++iter) CopyToBuf(&bufMap, *iter);
    ...
    if (!unitCreation.units[i].szPartyName.empty())
    {
      ...
      if (bufMap.buf.size() <= bufMap.nCnt + nSize) bufMap.buf.resize(bufMap.nCnt + nSize);
...
void CMapInfo::GetStartCommandsListCheckSum(uLong *pResourcesCheckSum, uLong *pMapCheckSum)
{
  ...
  for (auto iter = startCommandsList.begin(); iter != startCommandsList.end(); ++iter)
  {
    ...
    for (int i = 0; i < iter->unitLinkIDs.size(); ++i) CopyToBuf(&bufMap, iter->unitLinkIDs[i]);
  }
...
void CMapInfo::GetReservePositionsListCheckSum(uLong *pResourcesCheckSum, uLong *pMapCheckSum)
{
  ...
  for (auto iter = reservePositionsList.begin(); iter != reservePositionsList.end(); ++iter)
  {
...
void CMapInfo::GetAIConstsCheckSum(uLong *pResourcesCheckSum, uLong *pMapCheckSum)
{
  ...
  for (auto iter = entryNames.begin(); iter != entryNames.end(); ++iter) CopyToBuf(&bufRes, constsTbl.GetDouble("AI", iter->c_str(), -1.0));
