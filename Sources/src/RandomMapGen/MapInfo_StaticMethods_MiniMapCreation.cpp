...
struct SSetColorFunctional
{
  ...
  void operator()(int nXIndex, int nYIndex) { if (IsValidPointForNormalRect(imageRect, nXIndex, nYIndex)) { imageAccessor[nYIndex][nXIndex] = color; } }
};
...
struct SSetColorWithoutCheckPointFunctional
{
  ...
  void operator()(int nXIndex, int nYIndex) { imageAccessor[nYIndex][nXIndex] = color; }
};
...
bool CMapInfo::CreateMiniMapImage(const SLoadMapInfo &rLoadMapInfo, const CRMImageCreateParameterList &rImageCreateParameterList, IProgressHook *pProgressHook)
{
  ...
  if ((!pImageProcessor) || (!pDataStorage) || (!pODB)) { return false; }
  ...
  for (int nLayerIndex = 0; nLayerIndex < SRMMiniMapCreateParameter::MML_COUNT; ++nLayerIndex)
  {
    ...
    if (layers.back() == nullptr) { return false; }
    layers.back()->Set(0);
  }
  ...
  // terrain
  // Direct Y-axis access
  {
    // get ImageAccessor for TileSet
    ...
    for (int nYIndex = 0; nYIndex < rLoadMapInfo.terrain.tiles.GetSizeY(); ++nYIndex)
    {
      ...
      {
        SColor color;
        if (terrainColorsHashMap.find(rLoadMapInfo.terrain.tiles[nYIndex][nXIndex].tile) != terrainColorsHashMap.end()) { color = terrainColorsHashMap[rLoadMapInfo.terrain.tiles[nYIndex][nXIndex].tile]; }
        else
        {
          // get the color
        ...
        for (int nInnerYIndex = (nYIndex * 2); nInnerYIndex < ((nYIndex + 1) * 2); ++nInnerYIndex) { for (int nInnerXIndex = (nXIndex * 2); nInnerXIndex < ((nXIndex + 1) * 2); ++nInnerXIndex) { layerImageAccessor[nInnerYIndex][nInnerXIndex] = color; } }
      }
    }
  }
  ...
  // Inverted Y-axis access
  {
    ...
    for (int nRiverIndex = 0; nRiverIndex < rLoadMapInfo.terrain.rivers.size(); ++nRiverIndex)
    {
      ...
      {
        if (miniMapBorderColor.a >= createParameter.dwMinAlpha) { layerImageAccessor[imageSize.y - storeBorderTiles.tiles[nTileIndex].y - 1][storeBorderTiles.tiles[nTileIndex].x] = miniMapBorderColor; }
        else if (miniMapCenterColor.a < createParameter.dwMinAlpha) { layerImageAccessor[imageSize.y - storeBorderTiles.tiles[nTileIndex].y - 1][storeBorderTiles.tiles[nTileIndex].x] = createParameter.layers[SRMMiniMapCreateParameter::MML_RIVERS].color; }
      }

      if (miniMapCenterColor.a >= createParameter.dwMinAlpha) { for (int nTileIndex = 0; nTileIndex < storeCenterTiles.tiles.size(); ++nTileIndex) { layerImageAccessor[imageSize.y - storeCenterTiles.tiles[nTileIndex].y - 1][storeCenterTiles.tiles[nTileIndex].x] = miniMapCenterColor; } }
    }
  }
  ...
  // Inverted Y-axis access
  {
    ...
    for (int nRoad3DIndex = 0; nRoad3DIndex < rLoadMapInfo.terrain.roads3.size(); ++nRoad3DIndex)
    {
      ...
      {
        if (miniMapBorderColor.a >= createParameter.dwMinAlpha) { for (int nTileIndex = 0; nTileIndex < storeBorderTiles.tiles.size(); ++nTileIndex) { railRoadLayerImageAccessor[imageSize.y - storeBorderTiles.tiles[nTileIndex].y - 1][storeBorderTiles.tiles[nTileIndex].x] = miniMapBorderColor; } }
        else if (miniMapCenterColor.a < createParameter.dwMinAlpha) { for (int nTileIndex = 0; nTileIndex < storeBorderTiles.tiles.size(); ++nTileIndex) { railRoadLayerImageAccessor[imageSize.y - storeBorderTiles.tiles[nTileIndex].y - 1][storeBorderTiles.tiles[nTileIndex].x] = createParameter.layers[SRMMiniMapCreateParameter::MML_RAILROADS].color; } }
        if (miniMapCenterColor.a >= createParameter.dwMinAlpha) { for (int nTileIndex = 0; nTileIndex < storeCenterTiles.tiles.size(); ++nTileIndex) { railRoadLayerImageAccessor[imageSize.y - storeCenterTiles.tiles[nTileIndex].y - 1][storeCenterTiles.tiles[nTileIndex].x] = miniMapCenterColor; } }
      }
      else
      {
        if (miniMapBorderColor.a >= createParameter.dwMinAlpha) { for (int nTileIndex = 0; nTileIndex < storeBorderTiles.tiles.size(); ++nTileIndex) { roadLayerImageAccessor[imageSize.y - storeBorderTiles.tiles[nTileIndex].y - 1][storeBorderTiles.tiles[nTileIndex].x] = miniMapBorderColor; } }
        else if (miniMapCenterColor.a < createParameter.dwMinAlpha) { for (int nTileIndex = 0; nTileIndex < storeBorderTiles.tiles.size(); ++nTileIndex) { roadLayerImageAccessor[imageSize.y - storeBorderTiles.tiles[nTileIndex].y - 1][storeBorderTiles.tiles[nTileIndex].x] = createParameter.layers[SRMMiniMapCreateParameter::MML_ROADS3D].color; } }
        if (miniMapCenterColor.a >= createParameter.dwMinAlpha) { for (int nTileIndex = 0; nTileIndex < storeCenterTiles.tiles.size(); ++nTileIndex) { roadLayerImageAccessor[imageSize.y - storeCenterTiles.tiles[nTileIndex].y - 1][storeCenterTiles.tiles[nTileIndex].x] = miniMapCenterColor; } }
      }
    }
  }
  ...
  std::hash_map<int, SMapObjectInfo> nLinkID2BridgesHashMap;
  {
    ...
    for (int nObjectIndex = 0; nObjectIndex < rLoadMapInfo.objects.size(); ++nObjectIndex)
    {
      ...
      if (pGDBObjectDesc)
      {
        if (pGDBObjectDesc->eGameType == SGVOGT_BUILDING)
        {
          ...
          if (ValidateIndices(imageRect, &indices) >= 0) { for (int nXIndex = indices.minx; nXIndex < indices.maxx; ++nXIndex) { for (int nYIndex = indices.miny; nYIndex < indices.maxy; ++nYIndex) { if (createParameter.bAllBuildingPassability || (rPassability[nYIndex - start.y][nXIndex - start.x] != RMGC_UNLOCKED)) { buildingSetColorFunctional(nXIndex, imageSize.y - nYIndex - 1); } } } }
        }
        else if (pGDBObjectDesc->eGameType == SGVOGT_BRIDGE) { if (rLoadMapInfo.objects[nObjectIndex].link.nLinkID != RMGC_INVALID_LINK_ID_VALUE) { nLinkID2BridgesHashMap[rLoadMapInfo.objects[nObjectIndex].link.nLinkID] = rLoadMapInfo.objects[nObjectIndex]; } }
        else
        {
    ...
    for (int nBridgeIndex = 0; nBridgeIndex < rLoadMapInfo.bridges.size(); ++nBridgeIndex)
    {
      ...
      for (int nSpanIndex = 0; nSpanIndex < rLoadMapInfo.bridges[nBridgeIndex].size(); ++nSpanIndex)
      {
        if (nLinkID2BridgesHashMap.find(rLoadMapInfo.bridges[nBridgeIndex][nSpanIndex]) != nLinkID2BridgesHashMap.end())
        {
          ...
          if (pBridgeRPGStats)
          {
            ...
            if (nWidth < static_cast<int>(pBridgeRPGStats->GetSpanStats(spanInfo.nFrameIndex).fWidth)) { nWidth = static_cast<int>(pBridgeRPGStats->GetSpanStats(spanInfo.nFrameIndex).fWidth); }
      ...
      if (createParameter.dwBridgeWidth > 0) { nWidth = createParameter.dwBridgeWidth; }
      if ((bridgeRect.minx >= 0) && (bridgeRect.maxx >= 0))
      {
        ...
        if (ValidateIndices(imageRect, &actualBridgeRect) >= 0) { for (int nYIndex = actualBridgeRect.miny; nYIndex < actualBridgeRect.maxy; ++nYIndex) { for (int nXIndex = actualBridgeRect.minx; nXIndex < actualBridgeRect.maxx; ++nXIndex) { bridgesSetColorFunctional(nXIndex, imageSize.y - nYIndex - 1); } } }
      }
    }
  }
  ...
  for (int nLayerIndex = 0; nLayerIndex < SRMMiniMapCreateParameter::MML_COUNT; ++nLayerIndex)
  {
    if (createParameter.layers[nLayerIndex].borderColor.a >= createParameter.dwMinAlpha)
    {
      ...
      imagesToCompress.push_back(nullptr);
      ...
      if (imagesToCompress.back() == nullptr) { return false; }
      imagesToCompress.push_back(nullptr);
      ...
      if (layers[nLayerIndex] == nullptr) { return false; }
    }
  }
  ...
  std::vector<CPtr<IImage>> shadows;
  {
    for (int nLayerIndex = 0; nLayerIndex < SRMMiniMapCreateParameter::MML_COUNT; ++nLayerIndex)
    {
          ...
          (createParameter.layers[nLayerIndex].shadowPoint.y != 0))
      {
        ...
        if (shadows.back() == nullptr) { return false; }
      }
    }
  }
  ...
  // extrusion collecting
  std::vector<CPtr<IImage>> aphaEmbosses;
  {
    for (int nLayerIndex = 0; nLayerIndex < SRMMiniMapCreateParameter::MML_COUNT; ++nLayerIndex)
    {
      aphaEmbosses.push_back(nullptr);
          ...
          (createParameter.layers[nLayerIndex].embossPoint.y != 0))
      {
        aphaEmbosses.back() = CRMImageBuilder::GetAlphaEmboss(layers[nLayerIndex],
                                                              ...
                                                              createParameter.dwMinAlpha);
        if (aphaEmbosses.back() == nullptr) { return false; }
      }
    }
  }
  ...
  // shadow overlay from 3d terrain
  if (createParameter.bTerrainShades)
  {
    ...
    if (pTerrainShades)
    {
      CPtr<IImage> pCompressedTerrainShades = nullptr;
          ...
          (pTerrainShades->GetSizeY() != layers[SRMMiniMapCreateParameter::MML_TERRAIN]->GetSizeY()))
      {
        ...
        if (pCompressedTerrainShades == nullptr) { return false; }
      }
      else { pCompressedTerrainShades = pTerrainShades; }
      if (!CRMImageBuilder::Noise(layers[SRMMiniMapCreateParameter::MML_TERRAIN], pCompressedTerrainShades, createParameter.dwMinAlpha)) { return false; }
    }
  }

  // We generate data depending on the size of the image
  for (auto imageCreateParameterIterator = rImageCreateParameterList.begin(); imageCreateParameterIterator != rImageCreateParameterList.end(); ++imageCreateParameterIterator)
  {
    // Collecting Noises
        ...
        if (!createParameter.layers[nLayerIndex].noiseImage.empty())
        {
          ...
          if (createParameter.layers[nLayerIndex].bScaleNoise)
          {
            if ((layers[nLayerIndex]->GetSizeX() != imageCreateParameterIterator->size.x) || (layers[nLayerIndex]->GetSizeY() != imageCreateParameterIterator->size.y))
            {
              ...
              if (pCompressedNoiseImage == nullptr) { return false; }
            }
            else { pCompressedNoiseImage = pNoiseImage; }
          }
          else { pCompressedNoiseImage = pNoiseImage; }

          if (!CRMImageBuilder::Noise(noises.back(), pCompressedNoiseImage, createParameter.dwMinAlpha)) { return false; }
        }
      }
    }

    // putting together an overall picture
    // for each layer:
    // add a layer
    // put Emboss (if any)
    // put a shadow (if any)
    CPtr<IImage> pMiniMapImage = nullptr;
    for (int nLayerIndex = (SRMMiniMapCreateParameter::MML_COUNT - 1); nLayerIndex >= 0; --nLayerIndex)
    {
      if (nLayerIndex == (SRMMiniMapCreateParameter::MML_COUNT - 1)) { pMiniMapImage = noises[nLayerIndex]; }
      else { if (!CRMImageBuilder::FastAddImageByAlpha(pMiniMapImage, noises[nLayerIndex], createParameter.dwMinAlpha)) { return false; } }
      if (aphaEmbosses[nLayerIndex] != nullptr) { if (!CRMImageBuilder::Noise(pMiniMapImage, aphaEmbosses[nLayerIndex], createParameter.dwMinAlpha)) { return false; } }
      if (shadows[nLayerIndex] != nullptr) { if (!CRMImageBuilder::Noise(pMiniMapImage, shadows[nLayerIndex], createParameter.dwMinAlpha)) { return false; } }
    }

    // click the image to the desired size
    CPtr<IImage> pCompressedMiniMapImage = nullptr;
    if ((pMiniMapImage->GetSizeX() != imageCreateParameterIterator->size.x) || (pMiniMapImage->GetSizeY() != imageCreateParameterIterator->size.y))
    {
      ...
      if (pCompressedMiniMapImage == nullptr) { return false; }
    }
    else { pCompressedMiniMapImage = pMiniMapImage; }

    // if necessary, adjust the colors:
    CPtr<IImage> pColorCorrectedCompressedMiniMapImage = nullptr;
    if (imageCreateParameterIterator->bColorCorrection)
    {
      ...
      if (pColorCorrectedCompressedMiniMapImage == nullptr) { return false; }
    }
    else { pColorCorrectedCompressedMiniMapImage = pCompressedMiniMapImage; }

    if (imageCreateParameterIterator->bDDS) { if (!SaveImageToDDSImageResource(pColorCorrectedCompressedMiniMapImage, imageCreateParameterIterator->szImageFileName)) { return false; } }
    else { if (!SaveImageToTGAImageResource(pColorCorrectedCompressedMiniMapImage, imageCreateParameterIterator->szImageFileName)) { return false; } }
  }
