...
bool CMapInfo::AddSounds(const SLoadMapInfo &rLoadMapInfo, TMapSoundInfoList *pSoundsList, DWORD dwSoundTypeBits)
{
  ...
  if ((!pODB) || (!pSoundsList)) { return false; }

  int nRiverPointsPeriod = 8;// period between sounds on the river
  int nDelimiter = 16;// size in VIS tiles of the forest array (one VIS tile can contain up to 4 trees)
  int nMinWoodsCount = 4;// nDelimiter * nDelimiter * 4;
  int nCirclePeriod = 1;// period (in nDelimiter dimension) of setting cyclic sounds for forests
  int nAmbientPeriod = 2;// period (in nDelimiter dimension) of staging cyclic ambient sounds for forests
  CVec3 vCircleShiftPoint = VNULL3;// shift relative to the setting center
  CVec3 vAmbientShiftPoint = VNULL3;// shift relative to the setting center
  bool bExactCirclePoint = true;// to the center of mass of sounds
  bool bExactAmbientPoint = true;// to the center of mass of sounds
  float bCircleRatio = 1.0f;// probability of setting
  float bAmbientRatio = 0.5f;// probability of setting
  ...
  {
    int nRiverIndex = 0;
    for (auto riverIterator = rLoadMapInfo.terrain.rivers.begin(); riverIterator != rLoadMapInfo.terrain.rivers.end(); ++riverIterator)
    {
      ...
      if (!rLoadMapInfo.szForestCircleSounds.empty())
      {
        ...
        for (int nXIndex = 0; nXIndex < forests.GetSizeX(); nXIndex += nCirclePeriod)
        {
          for (int nYIndex = 0; nYIndex < forests.GetSizeY(); nYIndex += nCirclePeriod)
          {
            ...
            for (int nForestXIndex = 0; nForestXIndex < nCirclePeriod; ++nForestXIndex) { for (int nForestYIndex = 0; nForestYIndex < nCirclePeriod; ++nForestYIndex) { nWoodsCount += forests[nYIndex + nForestYIndex][nXIndex + nForestXIndex]; } }
      ...
      if (!rLoadMapInfo.szForestAmbientSounds.empty())
      {
        ...
        for (int nXIndex = 0; nXIndex < forests.GetSizeX(); nXIndex += nAmbientPeriod)
        {
          for (int nYIndex = 0; nYIndex < forests.GetSizeY(); nYIndex += nAmbientPeriod)
          {
            ...
            for (int nForestXIndex = 0; nForestXIndex < nAmbientPeriod; ++nForestXIndex) { for (int nForestYIndex = 0; nForestYIndex < nAmbientPeriod; ++nForestYIndex) { nWoodsCount += forests[nYIndex + nForestYIndex][nXIndex + nForestXIndex]; } }
