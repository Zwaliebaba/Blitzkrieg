...
namespace NPerlinNoise
{
  ...
  void Init()
  {
    ...
    for (i = 0; i < B; ++i)
    {
      do
      {
        ...
        for (j = 0; j < 3; ++j) v[j] = Random(-1.0f, 1.0f);
        s = fabs2(v);
  ...
  inline void Setup(float val, int &b0, int &b1, float &r0, float &r1)
  {
    ...
    b0 = static_cast<int>(t) & (B - 1);
    b1 = (b0 + 1) & (B - 1);
    r0 = t - static_cast<int>(t);
    r1 = r0 - 1.0f;
  }

  inline float At(const CVec3 &q, float rx, float ry, float rz) { return rx * q.x + ry * q.y + rz * q.z; }

  inline float Scurve(float t) { return t * t * (3.0f - 2.0f * t); }
