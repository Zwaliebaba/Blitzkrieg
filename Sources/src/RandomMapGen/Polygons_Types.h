...
template<class TYPE>
const TYPE GetPointType(const CVec2 &vec, TYPE *pTargetType) { NI_ASSERT_T(false, "Can convert only to CVec2 and CVec3!"); }

template<>
inline const CVec2 GetPointType<CVec2>(const CVec2 &vec, CVec2 *pVec2) { return vec; }

template<>
inline const CVec3 GetPointType<CVec3>(const CVec2 &vec, CVec3 *pVec3) { return CVec3(vec.x, vec.y, 0.0f); }

template<class TYPE>
const TYPE GetPointType(const CVec3 &vec, TYPE *pTargetType) { NI_ASSERT_T(false, "Can convert only to CVec2 and CVec3!"); }

template<>
inline const CVec2 GetPointType<CVec2>(const CVec3 &vec, CVec2 *pVec2) { return CVec2(vec.x, vec.y); }

template<>
inline const CVec3 GetPointType<CVec3>(const CVec3 &vec, CVec3 *pVec3) { return vec; }

// ////////////////////////////////////////////////
// determinant that the difference of elements is less than a given value¤
template<class TYPE>
struct SInRangeFunctional
{
  ...
  bool operator()(const TYPE &rElement1, const TYPE &rElement2) const { return fabs2(rElement1 - rElement2) <= fRange; }
};
...
// ////////////////////////////////////////////////
inline EClassifyEdge GetNegativeClassifyEdge(EClassifyEdge classifyEdge) { return NEGATIVE_CLASSIFY_EDGE[static_cast<int>(classifyEdge)]; }

// ////////////////////////////////////////////////
inline EClassifyPolygon GetNegativeClassifyPolygon(EClassifyPolygon classifyPolygon) { return NEGATIVE_CLASSIFY_POLYGON[static_cast<int>(classifyPolygon)]; }

// ////////////////////////////////////////////////
inline EClassifyRotation GetNegativeClassifyRotation(EClassifyRotation classifyRotation) { return NEGATIVE_CLASSIFY_ROTATION[static_cast<int>(classifyRotation)]; }

// ////////////////////////////////////////////////
inline EClassifyEdge GetClassifyNormal() { return CE_LEFT; }

// ////////////////////////////////////////////////
inline EClassifyEdge GetClassifyEdgeInnerSpace(EClassifyRotation classifyRotation)
{
  if (classifyRotation == CR_CLOCKWISE) { return CE_RIGHT; }
  if (classifyRotation == CR_COUNTERCLOCKWISE) { return CE_LEFT; }
  return CE_UNKNOWN;
}
...
EClassifyEdge ClassifyEdge(const PointType &rvBegin, const PointType &rvEnd, const PointType &v)
{
  ...
  if (s1020 > 0.0f) { return CE_LEFT; }
  if (s1020 < 0.0f) { return CE_RIGHT; }
  if (((v10.x * v20.x) < 0.0f) || ((v10.y * v20.y) < 0.0f)) { return CE_BEHIND; }
  if (fabs2(v10.x, v10.y) < fabs2(v20.x, v20.y)) { return CE_BEYONG; }
  if ((v.x == rvBegin.x) && (v.y == rvBegin.y)) { return CE_BEGIN; }
  if ((v.x == rvEnd.x) && (v.y == rvEnd.y)) { return CE_END; }
  return CE_BETWEEN;
}
...
EClassifyIntersection ClassifyIntersect(const PointType &rvBegin0, const PointType &rvEnd0, const PointType &rvBegin1, const PointType &rvEnd1, float *pfIntercectionPoint)
{
  ...
  if (fDenominator == 0.0f)
  {
    ...
    if ((classifyEdge == CE_LEFT) || (classifyEdge == CE_RIGHT)) { return CI_PARALLEL; }
    return CI_COLLINEAR;
  }
...
EClassifyIntersection ClassifyCross(const PointType &rvBegin0, const PointType &rvEnd0, const PointType &rvBegin1, const PointType &rvEnd1, float *pfIntercectionPoint)
{
  ...
  if (pfIntercectionPoint) { (*pfIntercectionPoint) = fInnerIntersectionPoint; }
  if (innerClassifyIntersection != CI_SKEW) { return innerClassifyIntersection; }
  if ((fInnerIntersectionPoint < 0.0f) || (fInnerIntersectionPoint > 1.0f)) { return CI_SKEW_NO_CROSS; }
  ...
  if ((fOuterIntersectionPoint < 0.0f) || (fOuterIntersectionPoint > 1.0f)) { return CI_SKEW_NO_CROSS; }
...
EClassifyPolygon ClassifyConvexPolygon(const Type &rPolygon, const PointType &v)
{
  ...
  {
    if ((currentPointIterator0->x == v.x) && (currentPointIterator0->y == v.y)) { return CP_VERTEX; }
    return CP_OUTSIDE;
  }
  ...
  while ((classifyEdge != CE_LEFT) && (classifyEdge != CE_RIGHT))
  {
    ...
    if (classifyEdge == CE_BETWEEN) { return CP_BOUNDARY; }
    if ((classifyEdge == CE_BEGIN) || (classifyEdge == CE_END)) { return CP_VERTEX; }

    ++currentPointIterator0;
    if (currentPointIterator0 == rPolygon.end()) { return CP_OUTSIDE; }
    ++currentPointIterator1;
  ...
  {
    classifyEdge = ClassifyEdge((*currentPointIterator0), (*currentPointIterator1), v);
    if (classifyEdge == negativeClassifyEdge) { return CP_OUTSIDE; }
    ++currentPointIterator0;
    ++currentPointIterator1;
    if (currentPointIterator1 == rPolygon.end()) { currentPointIterator1 = rPolygon.begin(); }
  }
  ...
  while (currentPointIterator0 != rPolygon.end())
  {
    ...
    if ((currentPointIterator1->y <= v.y) && (v.y < currentPointIterator0->y) &&
        (((v.x - currentPointIterator1->x) * (currentPointIterator0->y - currentPointIterator1->y)) <
         ((v.y - currentPointIterator1->y) * (currentPointIterator0->x - currentPointIterator1->x)))) { classifyPolygon = GetNegativeClassifyPolygon(classifyPolygon); }
...
{
  // zero points
  if (rPolygon.empty()) { return 0.0f; }

  typename Type::const_iterator currentPointIterator0 = rPolygon.begin();
  typename Type::const_iterator currentPointIterator1 = rPolygon.begin();
  ++currentPointIterator1;
  // one point
  if (currentPointIterator1 == rPolygon.end()) { return 0.0f; }
...
float GetSignedPolygonSquare(const Type &rPolygon)
{
  ...
  ++currentPointIterator2;
  // two points
  if (currentPointIterator2 == rPolygon.end()) { return 0.0f; }
  ...
  while (currentPointIterator0 != rPolygon.end())
  {
    ...
    ++currentPointIterator2;
    if (currentPointIterator1 == rPolygon.end()) { currentPointIterator1 = rPolygon.begin(); }
    if (currentPointIterator2 == rPolygon.end()) { currentPointIterator2 = rPolygon.begin(); }
  }
  return (fSquare / 2.0f);
}
...
EClassifyRotation ClassifyRotation(const PointType &v0, const PointType &v1, const PointType &v2)
{
  ...
  if (classifyEdge == CE_RIGHT) { return CR_CLOCKWISE; }
  if (classifyEdge == CE_LEFT) { return CR_COUNTERCLOCKWISE; }
  return CR_LINE;
}
...
EClassifyRotation ClassifyRotation(const Type &rPolygon)
{
  ...
  if (fPolygonSquare < 0) { return CR_CLOCKWISE; }
  if (fPolygonSquare > 0) { return CR_COUNTERCLOCKWISE; }
  return CR_LINE;
}
...
float GetSignedAngle(const PointType &rvBegin, const PointType &rvEnd, const PointType &v)
{
  ...
  if (fvBeginVAngle == (-1.0f) || (fvEndVAngle == (-1.0f))) { return FP_PI; }
  const float fEngle = fvEndVAngle - fvBeginVAngle;
  if ((fEngle == FP_PI) || (fEngle == (-FP_PI))) { return FP_PI; }
  if (fEngle < (-FP_PI)) { return (fEngle + FP_2PI); }
  if (fEngle > FP_PI) { return FP_2PI - fEngle; }
  return fEngle;
...
// ////////////////////////////////////////////////
template<class Type>
Type GetPointOnEdge(const Type &rvBegin, const Type &rvEnd, float fPoint) { return (rvBegin + fPoint * (rvEnd - rvBegin)); }
...
template<class PointType>
void RotatePoint(PointType *pPoint, float a)
{
  PointType point = PointType((pPoint->x * cos(a)) - (pPoint->y * sin(a)), (pPoint->x * sin(a)) + (pPoint->y * cos(a)));
  (*pPoint) = point;
}
...
{
  if ((v.x == 0.0f) && (v.y == 0.0f)) { return -1.0f; }
  if (v.x == 0.0f) { return ((v.y > 0.0f) ? (FP_PI2) : (FP_PI + FP_PI2)); }
  float a = atan(v.y / v.x);
  if (v.x > 0.0f) { return ((v.y >= 0.0f) ? (a) : (FP_2PI + a)); }
  return (FP_PI + a);
}

// ////////////////////////////////////////////////
template<class PointType>
float GetPolarLength(const PointType &v) { return fabs(v.x, v.y); }
...
{
  NI_ASSERT_T(( pvBegin != nullptr ) && ( pvEnd != nullptr ),
...
{
  if (pLeftPolygon) { pLeftPolygon->clear(); }
  if (pRightPolygon) { pRightPolygon->clear(); }

  if (rSourcePolygon.empty()) { return true; }
  ...
  if (sourcePointIterator1 == rSourcePolygon.end())
  {
    ...
    if (classifyEdge != CE_LEFT) { if (pRightPolygon) { pRightPolygon->push_back((*sourcePointIterator0)); } }
    if (classifyEdge != CE_RIGHT) { if (pLeftPolygon) { pLeftPolygon->push_back((*sourcePointIterator0)); } }
    return true;
  }
  ...
  while (sourcePointIterator0 != rSourcePolygon.end())
  {
    ...
    if (classifyIntersect == CI_SKEW) { vIntersectionPoint = GetPointOnEdge(rvBegin, rvEnd, fIntersectPoint); }
    ...
    if (beginClassifyEdge == CE_RIGHT)
    {
      if (endClassifyEdge != CE_RIGHT)
      {
        NI_ASSERT_TF(classifyIntersect == CI_SKEW,
                     ...
                     return false);
        if (pRightPolygon) { pRightPolygon->push_back(vIntersectionPoint); }
        if (pLeftPolygon)
        {
          if (endClassifyEdge == CE_LEFT) { pLeftPolygon->push_back(vIntersectionPoint); }
          pLeftPolygon->push_back(*sourcePointIterator1);
        }
      }
      else { if (pRightPolygon) { pRightPolygon->push_back(*sourcePointIterator1); } }
    }
    else if (beginClassifyEdge == CE_LEFT)
    {
      if (endClassifyEdge != CE_LEFT)
      {
        ...
        if (pRightPolygon)
        {
          if (endClassifyEdge == CE_RIGHT) { pRightPolygon->push_back(vIntersectionPoint); }
          pRightPolygon->push_back(*sourcePointIterator1);
        }
        if (pLeftPolygon) { pLeftPolygon->push_back(vIntersectionPoint); }
      }
      else { if (pLeftPolygon) { pLeftPolygon->push_back(*sourcePointIterator1); } }
    }
    else
    {
      if (endClassifyEdge == CE_RIGHT) { if (pRightPolygon) { pRightPolygon->push_back(*sourcePointIterator1); } }
      else if (endClassifyEdge == CE_LEFT) { if (pLeftPolygon) { pLeftPolygon->push_back(*sourcePointIterator1); } }
      else
      {
        if (pRightPolygon) { pRightPolygon->push_back(*sourcePointIterator1); }
        if (pLeftPolygon) { pLeftPolygon->push_back(*sourcePointIterator1); }
      }
    }

    ++sourcePointIterator0;
    ++sourcePointIterator1;
    if (sourcePointIterator1 == rSourcePolygon.end()) { sourcePointIterator1 = rSourcePolygon.begin(); }
  }
  return true;
}
...
bool CutByPolygonCore(const Type &rPolygon, const Type &rPolygonCore, Type *pCutPolygon)
{
  ...
  while (currentPointIterator0 != rPolygonCore.end())
  {
    ...
    if ((nCount & 0x01) > 0)
    {
      if (classifyRotation == CR_CLOCKWISE) { SplitByEdge(polygon1, vBegin, vEnd, static_cast<Type *>(nullptr), &polygon0); }
      else { SplitByEdge(polygon1, vBegin, vEnd, &polygon0, static_cast<Type *>(nullptr)); }
      polygon1.clear();
    }
    else
    {
      if (classifyRotation == CR_CLOCKWISE) { SplitByEdge(polygon0, vBegin, vEnd, static_cast<Type *>(nullptr), &polygon1); }
      else { SplitByEdge(polygon0, vBegin, vEnd, &polygon1, static_cast<Type *>(nullptr)); }
      polygon0.clear();
    }
    ++nCount;
    ...
    ++currentPointIterator0;
    ++currentPointIterator1;
    if (currentPointIterator1 == rPolygonCore.end()) { currentPointIterator1 = rPolygonCore.begin(); }
  }

  pCutPolygon->clear();
  if ((nCount & 0x01) > 0) { pCutPolygon->insert(pCutPolygon->end(), polygon1.begin(), polygon1.end()); }
  else { pCutPolygon->insert(pCutPolygon->end(), polygon0.begin(), polygon0.end()); }
...
bool GetVoronoyPolygon(const Type &rBoundingPolygon, const Type &rPoints, const PointType &rPoint, Type *pVoronoyPolygon)
{
  ...
  if (rBoundingPolygon.empty()) { return true; }
  ...
  for (typename Type::const_iterator pointIterator = rBoundingPolygon.begin(); pointIterator != rBoundingPolygon.end(); ++pointIterator)
  {
    if ((pointIterator->x != rPoint.x) || (pointIterator->y != rPoint.y))
    {
      ...
      if ((nCount & 0x01) > 0)
      {
        SplitByEdge(polygon1, vBegin, vEnd, &polygon0, static_cast<Type *>(nullptr));
        polygon1.clear();
      }
      else
      {
        SplitByEdge(polygon0, vBegin, vEnd, &polygon1, static_cast<Type *>(nullptr));
        polygon0.clear();
      }
      ++nCount;
    }
  ...
  if ((nCount & 0x01) > 0) { pVoronoyPolygon->insert(pVoronoyPolygon->end(), polygon1.begin(), polygon1.end()); }
  else { pVoronoyPolygon->insert(pVoronoyPolygon->end(), polygon0.begin(), polygon0.end()); }
...
// ////////////////////////////////////////////////
// ”taking pairs of points from a vector located at a distance fRange between each other
...
// ////////////////////////////////////////////////
// The minimum rectangle includes all points of the polygon
template<class Type>
void GetPolygonBoundingBox(const Type &rPolygon, CTRect<float> *pBoundingBox)
{
  ...
  if (!rPolygon.empty())
  {
    ...
    {
      if (pointIterator->x < pBoundingBox->minx) { pBoundingBox->minx = pointIterator->x; }
      if (pointIterator->x > pBoundingBox->maxx) { pBoundingBox->maxx = pointIterator->x; }
      if (pointIterator->y < pBoundingBox->miny) { pBoundingBox->miny = pointIterator->y; }
      if (pointIterator->y > pBoundingBox->maxy) { pBoundingBox->maxy = pointIterator->y; }
    }
  }
}
...
// ////////////////////////////////////////////////
template<class Type>
CTRect<float> GetPolygonBoundingBox(const Type &rPolygon)
{
...
PointType GetRandomBetweenPoint(const PointType &rvBegin, const PointType &rvEnd, float fMinSideDistanceRatio, const CTPoint<float> &rShiftRatio, EClassifyEdge classifyEdge)
{
  ...
  if (classifyEdge == CE_LEFT) { vPoint += vNormal * fShift; }
  else if (classifyEdge == CE_RIGHT) { vPoint -= vNormal * fShift; }
  ...
  while (nCount < nDepth)
  {
    ...
    if ((nCount & 0x01) > 0)
    {
      ...
      while (sourcePointIterator0 != sequence1.end())
      {
        ...
        {
          if (bPolygon) { sourcePointIterator1 = sequence1.begin(); }
          else
          {
            sequence0.push_back((*sourcePointIterator0));
            break;
    ...
    else
    {
      ...
      while (sourcePointIterator0 != sequence0.end())
      {
        ...
        {
          if (bPolygon) { sourcePointIterator1 = sequence0.begin(); }
          else
          {
            sequence1.push_back((*sourcePointIterator0));
            break;
    ...
    ++nCount;
    if (bNewPointsNotInserted) { break; }
  }

  pRandomizedSequence->clear();
  if ((nCount & 0x01) > 0) { pRandomizedSequence->insert(pRandomizedSequence->end(), sequence1.begin(), sequence1.end()); }
  else { pRandomizedSequence->insert(pRandomizedSequence->end(), sequence0.begin(), sequence0.end()); }
...
bool EnlargePolygonCore(const Type &rBoundingPolygon, const Type &rPolygon, float fDistance, Type *pEnlargedPolygon)
{
  ...
  else if (classifyRotation == CR_COUNTERCLOCKWISE) { fDistance *= (-1); }
  ...
  pEnlargedPolygon->clear();
  if ((nCount & 0x01) > 0) { pEnlargedPolygon->insert(pEnlargedPolygon->end(), polygon1.begin(), polygon1.end()); }
  else { pEnlargedPolygon->insert(pEnlargedPolygon->end(), polygon0.begin(), polygon0.end()); }
...
float PolygonDistance(const Type &rPolygon, const PointType &v)
{
  ...
  // one point
  ++currentPointIterator1;
  if (currentPointIterator1 == rPolygon.end()) { return fabs(v - (*currentPointIterator0)); }

  EClassifyPolygon classifyPolygon = ClassifyPolygon(rPolygon, v);

  // point on the polygon boundary
  if ((classifyPolygon == CP_BOUNDARY) || (classifyPolygon == CP_VERTEX)) { return 0.0f; }

  float fDistance = fabs(v - (*currentPointIterator0));
  // we determine the minimum distance for the side and for the vertex of the side
  while (currentPointIterator0 != rPolygon.end())
  {
    // ( *currentPointIterator0 );
    // ( *currentPointIterator1 );

    // distance to point ( *currentPointIterator0 )
    const float fVertexDistance = fabs(v - (*currentPointIterator0));
    if (fVertexDistance < fDistance) { fDistance = fVertexDistance; }
    ...
    if ((dotPoduct_v0_10 > FP_EPSILON) && (dotPoduct_v1_01 > FP_EPSILON))
    {
      ...
      if (fEdgeDistance < fDistance) { fDistance = fEdgeDistance; }
    }
...
bool GetBoundingPolygon(const SVectorStripeObject &rVectorStripeObject, Type *pBoundingPolygon)
{
  ...
  for (auto pointIterator = rVectorStripeObject.points.begin(); pointIterator != rVectorStripeObject.points.end(); ++pointIterator) { if (pointIterator->bKeyPoint) { pBoundingPolygon->push_back(pointIterator->vPos + pointIterator->vNorm * pointIterator->fWidth); } }
  for (auto pointIterator = rVectorStripeObject.points.rbegin(); pointIterator != rVectorStripeObject.points.rend(); ++pointIterator) { if (pointIterator->bKeyPoint) { pBoundingPolygon->push_back(pointIterator->vPos - pointIterator->vNorm * pointIterator->fWidth); } }
  return true;
}
