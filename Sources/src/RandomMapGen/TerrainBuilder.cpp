...
inline BYTE GetTileType(BYTE color) { return color >> 4; }

// transition options and masks for them in the form: mask <=> value
// check is done as: (neighbors & mask) == value
// basic
...
static constexpr int nNumMaskOrders = 12;
...
int CTerrainBuilder::ComparePriority(BYTE r1, BYTE r2) const { return Sign(tileset.terrtypes[r1].nPriority - tileset.terrtypes[r2].nPriority); }

// placement and numbering of adjacent tiles
// 2 4 7
// 1 X 6
// 0 3 5
//
// create neighbors mask with only one type of color
...
public:
  ...
  bool operator()(const int type1, const int type2) const { return tileset.terrtypes[type1].nPriority < tileset.terrtypes[type2].nPriority; }
};

// create neighbors mask with all types of color
                                       ...
                                       SComplexCrosses *pCrosses) const
{
  ...
  if (localcrosses.empty()) { return 0; }
  ...
  if (bHasNoiseAnd)// crosses with one noise over all crosses in this tile
  {
    ...
    tile.tile = 0;
    tile.cross = 0;
    if (HasNoise(tiles[nY][nX].tile)) tiles[nY][nX].noise = 2;
    ++nNumCrosses;
  }
  else if (bHasNoiseOr)// layered crosses with noise
  {
    pCrosses->layers.resize(Max(static_cast<int>(pCrosses->layers.size()), nNumCrosses));
...
void CTerrainBuilder::ConvertImageSegment(CImageAccessor &image, const CTRect<int> &rect) const
{
  for (int i = rect.y1; i < rect.y2; ++i)
    for (int j = rect.x1; j < rect.x2; ++j) image[i][j].r = GetTileType(image[i][j].r);
}
...
{
  // go through the tiles to remove too narrow gaps. 
  bool bChanged = false;
  int nPassCounter = 0;// pass counter for limiting (so as not to enter an infinite loop)
  do
  {
    bChanged = false;
...
{
  for (int i = rect.y1; i != rect.y2; ++i) { for (int j = rect.x1; j != rect.x2; ++j) tiles[i][j].noise = HasNoise(tiles[i][j].tile); }
}
                                               ...
                                               const CTRect<int> &rcSuper, SComplexCrosses *pCrosses) const
{
  ...
  int nNumCrosses = 0;
  for (int i = rect.y1; i < rect.y2; ++i)
    for (int j = rect.x1; j < rect.x2; ++j) nNumCrosses += GetNeighboursMask(tiles, j, i, rect, rcSuper, pCrosses);
  //
  return nNumCrosses;
}
...
void CopyCrossesList(STerrainPatchInfo::CCrossesList *pDst, const CTerrainBuilder::CCrossesList &src)
{
  ...
  for (auto it = src.begin(); it != src.end(); ++it) pDst->push_back(*it);
}
...
void CTerrainBuilder::CopyCrosses(STerrainPatchInfo *pPatch, const SComplexCrosses &crosses) const
{
  ...
  int nLayerIndex = 0;
  for (auto it = crosses.layers.begin(); it != crosses.layers.end(); ++it, ++nLayerIndex) CopyCrossesList(&pPatch->layercrosses[nLayerIndex], *it);
}
...
int CTerrainBuilder::GetTerrainType(BYTE tile) const
{
  ...
  if (pos != terratypes.end()) return pos->second;
  else
  {
...
int CTerrainBuilder::GetCrossType(BYTE tile) const
{
  ...
  if (pos != crosstypes.end()) return pos->second;
  else
  {
...
int CTerrainBuilder::GetCrossetType(BYTE tile) const
{
  ...
  if (pos != crossettypes.end()) return pos->second;
  else
  {
    for (int i = 0; i < crosset.crosses.size(); ++i)
    {
