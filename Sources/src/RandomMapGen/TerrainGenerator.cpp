...
void CHField::Init(int width, int height, float _fGridStep, float val)
{
  ...
  if (width == dwDimX && height == dwDimY) return;
  dwDimX = width;
  dwDimY = height;
...
{
  if (dwDimX != val.dwDimX || dwDimY != val.dwDimY) Init(val.dwDimX, val.dwDimY, val.fGridStep, 0);
...
float CHField::Multifractal(CVec3 pt, float lacunarity, float octaves, float offset)
{
  ...
  float remainder = octaves - static_cast<int>(octaves);
  if (remainder != 0.0) value += remainder * NPerlinNoise::Noise3(pt) * fBmExponents[i];
  return value;
}
...
float CHField::HybridMultifractal(CVec3 pt, float lacunarity, float octaves, float offset)
{
  ...
  float remainder = octaves - static_cast<int>(octaves);
  if (remainder != 0.0f) ret += remainder * NPerlinNoise::Noise3(pt) * fBmExponents[i];
...
{
  fBmExponents.resize(static_cast<int>(octaves) + 1);
  float frequency = 1.0;
...
void CHField::Generate(const SfBmValues &vals)
{
  ...
  if (dh > FP_EPSILON) Scale(vals.fRange / dh);
...
{
  for (int i = 0; i < hf.size(); ++i) hf[i] *= scale;
  bChanged = true;
}
...
float GaussRand()
{
  ...
  else x = grv2 * sqrt(-2 * log(grs) / grs);
  grphase = !grphase;
...
float CHField::AveHeight() const
{
  ...
  for (int i = 0; i < hf.size(); ++i) ret += hf[i];
...
{
  for (int i = 0; i < hf.size(); ++i) hf[i] += shift;
}

float CHField::MinHeight() const
{
  if (hf.empty()) return 0;
  float ret = hf[0];

  for (int i = 1; i < hf.size(); ++i) ret = Min(ret, hf[i]);
...
float CHField::MaxHeight() const
{
  ...
  for (int i = 1; i < hf.size(); ++i) ret = Max(ret, hf[i]);
...
{
  float vol = 0;
  for (int i = 0; i < hf.size(); ++i) vol += H(i);
...
{
  if (hf.empty()) return;
  float range = AltitudeRange(nullptr, nullptr);
  float min = MinHeight();

  Translate(-min);
  for (int i = 0; i < hf.size(); ++i) H(i) = pow(H(i), exp);
  Scale(range / AltitudeRange(nullptr, nullptr));
  Translate(min);
}
...
{
  for (int i = 0; i < hf.size(); ++i) hf[i] = -hf[i];
  return *this;
}
...
CVec3 CHField::GetNormal(float x, float y) const
{
  ...
  int nx = static_cast<int>(x * invStep);
  int ny = static_cast<int>(y * invStep);
