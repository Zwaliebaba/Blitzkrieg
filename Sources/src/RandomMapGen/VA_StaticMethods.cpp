...
const CVec3 CVertexAltitudeInfo::V3_CAMERA_NEGATIVE(-1, -1, -(CAMERA_ALPHA));
const CVec3 CVertexAltitudeInfo::V3_CAMERA_POSITIVE(1, 1, -(CAMERA_ALPHA));
...
{
  if (!IsValidIndices(rAltitude, nXPos, nYPos)) { return VNULL3; }
  ...
  if (Normalize(&vNorm)) { return vNorm; }
  return VNULL3;
}


const CVec3 CVertexAltitudeInfo::GetNormale(const STerrainInfo::TVertexAltitudeArray2D &rAltitude, const CTPoint<int> &rPoint) { return GetNormale(rAltitude, rPoint.x, rPoint.y); }
    ...
    for (int nXIndex = updateRect.minx; nXIndex < updateRect.maxx; ++nXIndex)
    {
      for (int nYIndex = updateRect.miny; nYIndex < updateRect.maxy; ++nYIndex)
      {
        ...
        if (vNorm != VNULL3)
        {
          ...
          (*pAltitude)[nYIndex][nXIndex].shade = static_cast<BYTE>(fShade * 255.0f);
        }
      }
    }
  }
...
bool CVertexAltitudeInfo::GetHeight(const STerrainInfo::TVertexAltitudeArray2D &rAltitude, const CVec2 &rvPos, float *pfHeight) { return GetHeight(rAltitude, rvPos.x, rvPos.y, pfHeight); }
...
bool CVertexAltitudeInfo::GetHeight(const STerrainInfo::TVertexAltitudeArray2D &rAltitude, float fXPos, float fYPos, float *pfHeight)
{
  ...
  *pfHeight = 0.0f;
  ...
  if (fXPos < 0.0f) { fXPos = 0.0f; }
  if (fYPos < 0.0f) { fYPos = 0.0f; }
  if (fXPos >= ((rAltitude.GetSizeX() - 1) * fWorldCellSize)) { fXPos = ((rAltitude.GetSizeX() - 1) * fWorldCellSize) - 0.1f; }
  if (fYPos >= ((rAltitude.GetSizeY() - 1) * fWorldCellSize)) { fYPos = ((rAltitude.GetSizeY() - 1) * fWorldCellSize) - 0.1f; }
...
bool CVertexAltitudeInfo::IsValidHeight(const STerrainInfo::TVertexAltitudeArray2D &rAltitude, const CTPoint<int> &rPoint) { return IsValidHeight(rAltitude, rPoint.x, rPoint.y); }
...
bool CVertexAltitudeInfo::IsValidHeight(const STerrainInfo::TVertexAltitudeArray2D &rAltitude, const CTRect<int> &rRect)
{
  ...
  for (int nXIndex = rect.minx; nXIndex < rect.maxx; ++nXIndex) { for (int nYIndex = rect.miny; nYIndex < rect.maxy; ++nYIndex) { if (!IsValidHeight(rAltitude, nXIndex, nYIndex)) { return false; } } }
  return true;
}
...
bool CVertexAltitudeInfo::ValidateHeights(STerrainInfo::TVertexAltitudeArray2D *pAltitude, const CTPoint<int> &rPoint, int nSize, CTRect<int> *pAffectedRect) { return ValidateHeights(pAltitude, rPoint.x, rPoint.y, nSize, pAffectedRect); }
...
{
  NI_ASSERT_T(( pfMinHeight != nullptr ) && ( pfMaxHeight != nullptr ),
  ...
  for (int nYIndex = 0; nYIndex < rAltitude.GetSizeY(); ++nYIndex)
  {
    for (int nXIndex = 0; nXIndex < rAltitude.GetSizeX(); ++nXIndex)
    {
      ...
      if (fHeight > (*pfMaxHeight)) { (*pfMaxHeight) = fHeight; }
      else if (fHeight < (*pfMinHeight)) { (*pfMinHeight) = fHeight; }
    }
  }
  return true;
}
...
{
  NI_ASSERT_T(( pfMinShade != nullptr ) && ( pfMaxShade != nullptr ),
  ...
  for (int nYIndex = 0; nYIndex < rAltitude.GetSizeY(); ++nYIndex)
  {
    for (int nXIndex = 0; nXIndex < rAltitude.GetSizeX(); ++nXIndex)
    {
      ...
      if (fShade > (*pfMaxShade)) { (*pfMaxShade) = fShade; }
      else if (fShade < (*pfMinShade)) { (*pfMinShade) = fShade; }
    }
  }
  return true;
}
...
IImage *CVertexAltitudeInfo::GetHeightsImage(const STerrainInfo::TVertexAltitudeArray2D &rAltitude, float fRatio, bool bTerrainSize)
{
  ...
  if (pHeightsImage)
  {
    ...
    if ((fMaxHeight - fMinHeight) <= FP_EPSILON) { pHeightsImage->Set(CRMImageBuilder::GRAY_LIGHTER_COLOR); }
    else
    {
...
IImage *CVertexAltitudeInfo::GetShadesImage(const STerrainInfo::TVertexAltitudeArray2D &rAltitude, float fRatio, bool bTerrainSize)
{
  ...
  if (pShadesImage)
  {
    ...
    if ((fMaxShade - fMinShade) <= FP_EPSILON) { pShadesImage->Set(CRMImageBuilder::GRAY_LIGHTER_COLOR); }
    else
    {
      for (int nYIndex = 0; nYIndex < imageSize.y; ++nYIndex)
      {
