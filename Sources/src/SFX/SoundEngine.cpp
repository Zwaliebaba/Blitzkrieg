...
class CPlayVisitor : public ISFXVisitor
{
  ...
  int RegisterSound(CBaseSound *pSound, const int nChannel)
  {
    ...
    else { NStr::DebugTrace("Sound error %d\n", FSOUND_GetError()); }
...
CSoundEngine::CSoundEngine()
  : timeLastUpdate(-1), pStreamingSound(nullptr), nStreamingChannel(-1), timeStreamFinished(-1),
    bInited(false), bEnableSFX(true), bEnableStreaming(true), bPaused(false),
    bStreamingPaused(false), cSFXMasterVolume(255), cStreamMasterVolume(255),
    fStreamCurrentVolume(1.0f), bStreamPlaying(false) {}
...
bool CSoundEngine::SearchDevices()
{
  ...
  for (int i = 0; i < nNumDrivers; ++i)
  {
    ...
    dr.szDriverName = static_cast<const char *>(FSOUND_GetDriverName(i));
    unsigned int nCaps;
...
bool CSoundEngine::IsInitialized() { return bInited; }

IRefCount *CSoundEngine::QI(int nInterfaceTypeID)
{
  if (nInterfaceTypeID == 0) return reinterpret_cast<IRefCount *>(FSOUND_GetOutputHandle());
  return nullptr;
}
...
void CSoundEngine::SetDistanceFactor(float fFactor) { FSOUND_3D_Listener_SetDistanceFactor(fFactor); }
...
void CSoundEngine::Update(interface ICamera *pCamera)
{
  ...
  //
  if ((timeStreamFinished != -1) && (timeStreamFinished < timeLastUpdate) && (timeLastUpdate - timeStreamFinished > 15000)) PlayNextMelody();
  //
  ...
  if (nNumChannels > 0) ClearChannels();
}
...
{
  auto pSFX = reinterpret_cast<CSoundEngine *>(param);
...
{
  if (!bEnableStreaming || nextMelody.szName.empty()) return false;
...
{
  if (nTimeToFade > 0 && bStreamPlaying) streamFadeOff.Fade(nTimeToFade);
  else
  {
    CloseStreaming();
...
void CSoundEngine::SetStreamVolume(const float fVolume)
{
  ...
  if (nStreamingChannel != -1) { FSOUND_SetVolume(nStreamingChannel, fStreamCurrentVolume * cStreamMasterVolume); }
}
...
float CSoundEngine::GetStreamVolume() const { return fStreamCurrentVolume; }

void CSoundEngine::SetStreamMasterVolume(float fVolume)
{
  Clamp(fVolume, 0.0f, 1.0f);
  cStreamMasterVolume = static_cast<BYTE>(fVolume * 255.0f);
  if (bStreamPlaying && nStreamingChannel != -1) { FSOUND_SetVolume(nStreamingChannel, fStreamCurrentVolume * cStreamMasterVolume); }
}
...
void CSoundEngine::PlayStream(const char *pszFileName, bool bLooped, const unsigned int nTimeToFadePrevious)
{
  ...
  if (bStreamPlaying && curMelody.IsValid() && curMelody.szName == pszFileName) return;

  if (nTimeToFadePrevious && bStreamPlaying)
  {
    nextMelody.szName = pszFileName;
    nextMelody.bLooped = bLooped;
    if (!streamFadeOff.IsFading()) StopStream(nTimeToFadePrevious);
  }
  else
  {
    ...
    if (!pStreamingSound) pStreamingSound = FSOUND_Stream_OpenFile(szFileName1.c_str(), FSOUND_2D | (bLooped ? FSOUND_LOOP_NORMAL : FSOUND_LOOP_OFF), 0);
    ...
    if (pStreamingSound)
    {
      ...
      if (bStreamingPaused) FSOUND_SetPaused(nStreamingChannel, bStreamingPaused);
      //
...
bool CSoundEngine::IsPaused() { return bPaused; }

bool CSoundEngine::PauseStreaming(bool bPause)
{
  if (bStreamingPaused != bPause)
  {
    if (nStreamingChannel != -1) FSOUND_SetPaused(nStreamingChannel, bPause);
...
bool CSoundEngine::Pause(bool bPause)
{
  if (bPaused != bPause)
  {
    for (CChannelSoundMap::iterator it = soundsMap.begin(); it != soundsMap.end(); ++it) { if (nStreamingChannel != it->first) { FSOUND_SetPaused(it->first, bPause); } }
    bPaused = bPause;
  }
  return bPause;
}
...
void CSoundEngine::ClearChannels()
{
  ...
  {
    if (!it->second->IsValid()) { FSOUND_StopSound(it->first); }
...
{
  if (pSound == nullptr || !bEnableSFX) return -1;
  //
  thePlayVisitor.Init(this);
  if (static_cast<CBaseSound *>(pSound)->GetSample() == nullptr) return -1;
  ...
  if (0 != nStartPos) FSOUND_SetCurrentPosition(nChannel, nStartPos);
...
{
  auto pos = channelsMap.find(pSound);
  if (pos != channelsMap.end())
  {
    const int nChannel = pos->second;
    const int nPan = Clamp(static_cast<int>(128 + pSound->GetPan() * 127), 0, 255);
    FSOUND_SetPan(nChannel, nPan);
    const int nVolume = Clamp(static_cast<int>(pSound->GetVolume() >= 0 ? pSound->GetVolume() * GetSFXMasterVolume() : GetSFXMasterVolume()), 0, 255);
    FSOUND_SetVolume(nChannel, nVolume);
  }
}
...
{
  auto pos = channelsMap.find(pSound);
  if (pos != channelsMap.end()) { StopChannel(pos->second); }
}
...
void CSoundEngine::ReEnableSounds()
{
  ...
  if (!bEnableSFX)
  {
    for (CChannelSoundMap::iterator it = soundsMap.begin(); it != soundsMap.end(); ++it) { if (FSOUND_IsPlaying(it->first)) FSOUND_StopSound(it->first); }
    soundsMap.clear();
    channelsMap.clear();
  }
  // turn stream off
  if (!bEnableStreaming) CloseStreaming();
}

void CSoundEngine::NotifyMelodyFinished()
{
  NWin32Helper::CCriticalSectionLock lock(critSection);
  if (nextMelody.IsValid())// 眢骓?桡疣螯耠邃簋?
  {
    PlayNextMelody();
  }
...
bool CSoundEngine::IsStreamPlaying() const { return bStreamPlaying; }
