...
{
  // run function
  auto pFade = reinterpret_cast<CStreamFadeOff *>(lpParameter);
  pFade->Start();

  while (pFade->HaveToRun())
  {
    // run
    if (pFade->Segment(100)) Sleep(100);
    else break;
  }
...
int CStreamFadeOff::operator&(IStructureSaver &ss)
{
  ...
  if (saver.IsReading())
  {
    ...
    {
      // run thread again
      Fade(static_cast<int>(fVolume / fVolumeSpeed));
    }
  }
  else
  {
    if (hThread && WAIT_OBJECT_0 != WaitForSingleObject(hFinishReport, 0))// the thread itself did not terminate
    {
      bool bRun = true;
      saver.Add(4, &bRun);
    }
...
bool CStreamFadeOff::Segment(const int nTimeDelta)
{
  ...
  if (fVolume != 0.0f && fDVolume > 0.01)
  {
    ...
    if (fVolume >= 0) pSFX->SetStreamVolume(fVolume);
    timeAccumulator = 0;
  }
...
void CStreamFadeOff::Clear()
{
  if (hThread)
  {
    if (WAIT_OBJECT_0 != WaitForSingleObject(hFinishReport, 0))// the thread itself did not terminate
    {
...
bool CStreamFadeOff::IsFading() const { return hThread && WAIT_OBJECT_0 != WaitForSingleObject(hFinishReport, 0); }

bool CStreamFadeOff::HaveToRun() { return WAIT_OBJECT_0 != WaitForSingleObject(hStopCommand, 0); }
