...
void CCamera::Update()
{
  ...
  fComponentX = static_cast<int>(fComponentX);
  fComponentY = static_cast<int>(fComponentY / 2.0f) * 2.0f;
  ...
  {
    float fVal = 0;
    for (auto it = earthquakes.begin(); it != earthquakes.end();)
    {
      // exp(-x*0.3*15)*cos(fps*(Pi/2)*x)*2; 
      const float fTime = it->fTime * 0.001f;
      fVal += exp(-fTime * it->fAttenuation) * cos(fEQPeriod * FP_2PI * fTime) * it->fAmplitude;
      it->fTime += fTimeDiff;
      if (it->fTime >= it->fDuration) it = earthquakes.erase(it);
      else ++it;
    }
    vPos.z += fVal;
    vAnchor1.z += fVal;
...
void CCamera::AddEarthquake(const CVec3 &vPos, const float fPower)
{
  ...
  {
    float fCoeff = fPower;
    if (fDist > FP_MIN_QUAKE_RADIUS) fCoeff *= (1.0f - (fDist - FP_MIN_QUAKE_RADIUS) / (FP_MAX_QUAKE_RADIUS - FP_MIN_QUAKE_RADIUS));
