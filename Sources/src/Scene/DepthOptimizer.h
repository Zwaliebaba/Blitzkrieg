...
class CDepthOptimizer
{
  ...
  bool CanAddLocal(const int nX1, const int nY1, const int nX2, const int nY2) const
  {
    ...
    //
    return fTotalDepth / static_cast<float>((nX2 - nX1) * (nY2 - nY1)) <= fAllowedDepth;
  }
...
public:
  ...
  void SetScreenSize(const int nSizeX, const int nSizeY)
  {
    nScreenSizeX = nSizeX;
    nScreenSizeY = nSizeY;
    fCoeffX = static_cast<float>(depthbuffer.GetSizeX()) / static_cast<float>(nScreenSizeX);
    fCoeffY = static_cast<float>(depthbuffer.GetSizeY()) / static_cast<float>(nScreenSizeY);
  }

  //
  bool CheckAndAdd(const float fX1, const float fY1, const float fX2, const float fY2)
  {
    const int nX1 = static_cast<int>(ClampF(fX1 * fCoeffX, 0.0f, float(depthbuffer.GetSizeX())));
    const int nY1 = static_cast<int>(ClampF(fY1 * fCoeffY, 0.0f, float(depthbuffer.GetSizeY())));
    const int nX2 = static_cast<int>(ClampF(fX2 * fCoeffX + 1.0f, 0.0f, float(depthbuffer.GetSizeX())));
    const int nY2 = static_cast<int>(ClampF(fY2 * fCoeffY + 1.0f, 0.0f, float(depthbuffer.GetSizeY())));
  ...
  //
  void Clear() { depthbuffer.SetZero(); }
};
