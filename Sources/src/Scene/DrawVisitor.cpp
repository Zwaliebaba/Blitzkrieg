...
// particles
void CDrawVisitor::VisitParticles(IParticleSource *pObj) { AddSingleParticleEffect(pObj, &particles); }

// unknown scene object
void CDrawVisitor::VisitSceneObject(ISceneObject *pObj) { unknowns.push_back(pObj); }

void CDrawVisitor::AddSingleSprite(const SBasicSpriteInfo *pObj, CSpriteVisList *pSprites, WORD wPriority)
{
  auto pInfo = const_cast<SBasicSpriteInfo *>(pObj);
  pInfo->dwCheckFlags = (pInfo->dwCheckFlags & 0x0000ffff) | (static_cast<DWORD>(wPriority) << 16);
...
void CDrawVisitor::AddSingleParticleEffect(IParticleSource *pPS, CParticlesVisMap *pParticles)
{
  ...
  if (nNumParticles != 0)
  {
    ...
    for (int i = 0; i < nNumParticles; ++i)
    {
      ...
      if (rcRect.IsIntersect(rcScreen)) { if (depthoptimizer.CheckAndAdd(rcRect.x1, rcRect.y1, rcRect.x2, rcRect.y2)) particles.push_back(info); }
    }
  }
}

void CDrawVisitor::AddSingleMeshUnchecked(IMeshVisObj *pMesh, CMeshVisList *pMeshes) { pMeshes->push_back(pMesh); }
...
void CDrawVisitor::AddSingleMesh(IMeshVisObj *pMesh, CMeshVisList *pMeshes)
{
  ...
  if (dwClipFlags != GFXCP_OUT) pMeshes->push_back(pMesh);
}
...
void CDrawVisitor::VisitBoldLine(CVec3 *corners, float fWidth, DWORD color) { boldLines.push_back(SBoldLineObject(corners, color)); }

void CDrawVisitor::VisitMechTrace(const SMechTrace &trace) { traces.push_back(trace); }

void CDrawVisitor::VisitGunTrace(const SGunTrace &trace) { gunTraces.push_back(trace); }
...
void CDrawVisitor::VisitUIRects(IGFXTexture *pTexture, const int nShadingEffect, SGFXRect2 *pRects, const int nNumRects)
{
  ...
  //
  {
    ...
    if (pTexture)
    {
      fTexDiffX = -0.5f / static_cast<float>(pTexture->GetSizeX(0));
      fTexDiffY = -0.5f / static_cast<float>(pTexture->GetSizeY(0));
    }
  ...
  //
  if (uiObjects.back().rects.empty()) uiObjects.pop_back();
}
...
{
  if ((pText == nullptr) || (pText->GetText()->GetLength() == 0) || !rcScreen.IsIntersect(rcRect)) return;
...
{
  bool operator()(const IMeshVisObj *pMesh1, const IMeshVisObj *pMesh2) const { return pMesh1->GetTexture() < pMesh2->GetTexture(); }
};
