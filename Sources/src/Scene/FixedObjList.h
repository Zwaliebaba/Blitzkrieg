...
public:
  ...
  {
    for (int i = 0; i < TNumObjects; ++i) objects[i] = lst.objects[i];
    return *this;
  }
  ...
  void Clear()
  {
    for (int i = 0; i < TNumObjects; ++i) objects[i] = 0;
  }
  ...
  TObj *operator[](const int nIndex) const { return Get(nIndex); }
  //
  void Add(TObj *pObj)
  {
    for (int i = TNumObjects - 2; i >= 0; --i) objects[i + 1] = objects[i];
    objects[0] = pObj;
  }
  ...
  int operator&(IStructureSaver &ss)
  {
    ...
    if (saver.IsReading())
    {
      ...
      cNumObjects = Min(static_cast<char>(TNumObjects), cNumObjects);
    }
    else
    {
      ...
      for (int i = 0; i < TNumObjects; ++i)
      {
        if (objects[i] == 0)
        {
          cNumObjects = static_cast<char>(i);
          break;
        }
      }
      if (cNumObjects > 0) saver.Add(1, &cNumObjects);
    }
    //
    for (char i = 0; i < cNumObjects; ++i) saver.Add(2 + i, &(objects[i]));
    //
    return 0;
  }
};
...
{
public:
  using CFixedList = CFixedObjList<nNumObjects, TObj, THolder>;
  using CBase = CArray2D<CFixedList>;
  ...
  public:
      ...
      : nNextLineAdd(array.GetSizeX() - nBlockSize)
    {
      ...
      nElement = 0;
      while (!IsEnd() && pLineCurr->IsEmpty()) NextStepLine();
    }
...
public:
  ...
  //
  void SetSizes(const int nSizeX, const int nSizeY) { array.SetSizes(nSizeX, nSizeY); }
  ...
  bool Add(TObj *pObj)
  {
    ...
    if ((nCellX < 0) || (nCellY < 0) || (nCellX >= array.GetSizeX()) || (nCellY >= array.GetSizeY())) return false;
    array[nCellY][nCellX].Add(pObj);
    return true;
  }

  //
  iterator Iterate(const int nPatchX, const int nPatchY) { return iterator(nPatchX * nBlockSize, nPatchY * nBlockSize, array); }
  //
  int operator&(IStructureSaver &ss)
  {
    CSaverAccessor saver = &ss;
