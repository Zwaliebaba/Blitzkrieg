...
IGFXTexture *CKeyBasedParticleSource::GetTexture() const { return pTexture; }

const CVec3 CKeyBasedParticleSource::GetPos() const { return vPosition; }

void CKeyBasedParticleSource::SetPos(const CVec3 &vPos) { vPosition = vPos; }

const CVec3 CKeyBasedParticleSource::GetDirection() const { return vDirection; }
...
const NTimer::STime CKeyBasedParticleSource::GetStartTime() const { return nStartTime; }

const NTimer::STime CKeyBasedParticleSource::GetEffectLifeTime() const { return MINT(static_cast<float>(pData->nLifeTime) + pData->trackLife.GetValue(1.0f)); }

bool CKeyBasedParticleSource::IsFinished() const { return particles.empty() && (nLastUpdateTime > nStartTime + pData->nLifeTime || bStopped); }

const int CKeyBasedParticleSource::GetNumParticles() const { return particles.size(); }
...
void CKeyBasedParticleSource::GetInfo(SParticleSourceInfo &info)
{
  ...
  for (int i = 0; i < GetEffectLifeTime(); i += 64)
  {
    ...
    for (std::list<SExtendedParticle>::const_iterator it = particles.begin(); it != particles.end(); ++it) { area += it->fSize * it->fSize; }
...
void CKeyBasedParticleSource::Update(const NTimer::STime &time)
{
  ...
  {
    constexpr int nStep = 16;// that is, we work at intervals of 16 milliseconds.
    for (int i = 0; i < dt; i += nStep)
    {
      ...
      while (nNumForGenerating > 0 && !bStopped && !bSuspended)
      {
        ...
        part.color.a = static_cast<BYTE>(part.fOpacity * pData->trackOpacity.GetValue(0));
  ...
  if (!particles.empty())
  {
    ...
    {
      if (it->deathTime <= time) { it = particles.erase(it); }
      else
      {
        // update particle
        const float fTime = (time - it->birthTime) / static_cast<float>(it->deathTime - it->birthTime);
        if (fTime > it->contextSpeed.fTime && fTime < 1.0f)
        {
          const float fLifetime = it->deathTime - it->birthTime;
          it->color.a = static_cast<BYTE>(it->fOpacity * pData->trackOpacity.GetValue(fTime));
...
void CKeyBasedParticleSource::Init(SParticleSourceData *_pData)
{
  ...
  for (int i = 0; i < pData->nTextureDY; ++i) { for (int j = 0; j < pData->nTextureDX; ++j) rcRects.push_back(CTRect<float>(dx * j, dy * i, dx * (j + 1), dy * (i + 1))); }
...
int CKeyBasedParticleSource::operator&(IStructureSaver &ss)
{
  ...
  if (saver.IsReading()) { if (fScale == 0) fScale = 1.0f; }
  return 0;
}

void CKeyBasedParticleSource::SetScale(float _fScale) { fScale *= _fScale; }
...
void CKeyBasedParticleSource::Stop() { bStopped = true; }

int CKeyBasedParticleSource::GetOptimalUpdateTime() const
{
  STrackContext context;
  pData->trackDensity.CreateStartContext(&context);
  int nTime = static_cast<int>(Clamp(1.0f / pData->trackDensity.Integrate(&context, 1.0f) * pData->fDensityCoeff, 4.0f, 32.0f)) >> 2;
  int result = 4;
...
void CKeyBasedParticleSource::SetSuspendedState(bool bState) { bSuspended = bState; }
...
void CParticleGenerator::ResetGenerator(int nNextNumParticles)
{
  if (nNextNumParticles > 0)
  {
    ...
    nCurrParticle = 0;
    fStep = FP_2PI / static_cast<float>(nNextNumParticles);
  }
}
