...
BYTE CMaterialEffector::GetAlpha() const { return (0xFF - static_cast<BYTE>((0xFF - bAlpha) * fCoeff)); }

DWORD CMaterialEffector::GetSpecular() const
{
  DWORD result = 0xFF000000;
  result |= static_cast<DWORD>((dwSpecular & 0x00FF0000) * fCoeff) & 0x00FF0000;
  result |= static_cast<DWORD>((dwSpecular & 0x0000FF00) * fCoeff) & 0x0000FF00;
  result |= static_cast<DWORD>((dwSpecular & 0x000000FF) * fCoeff);
  return result;
}
...
{
  if (time >= nStartTime + nDuration) return false;
  if (time > nStartTime + nDuration * 0.5f) fCoeff = 2.0f - (time - nStartTime) / (nDuration * 0.5f);
  else fCoeff = (time - nStartTime) / (nDuration * 0.5f);
  return true;
}
