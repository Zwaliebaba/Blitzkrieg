...
constexpr float fHeightCoeff = 0.86602540378444f;// cos( ToRadian(30.0f) );
                   ...
                   const float fNoiseSizeX, const float fNoiseSizeY, struct STerrainPatch *pPatch)
{
  ...
  for (auto it = patch.basecrosses.begin(); it != patch.basecrosses.end(); ++it)
  {
  ...
  for (auto it = patch.noisecrosses.begin(); it != patch.noisecrosses.end(); ++it)
  {
    const int i = it->y, j = it->x;
    const int nMapY = patch.nStartY + i;
    const int nMapY1 = nMapY + 1;
  ...
  for (int nLayer = 0; nLayer != patch.layercrosses.size(); ++nLayer)
  {
    ...
    for (auto it = patch.layercrosses[nLayer].begin(); it != patch.layercrosses[nLayer].end(); ++it)
    {
      const int i = it->y, j = it->x;
      const int nMapY = patch.nStartY + i;
      const int nMapY1 = nMapY + 1;
                  ...
                  const STerrainInfo &info, struct STerrainPatch *pPatch)
{
  ...
  for (int k = 0; k < 2; ++k)// y
  {
    for (int l = 0; l < 2; ++l)// x
    {
      if (pPatch->bSubPatches[(1 - k) * 2 + l] == false) continue;
...
inline const float GetHeight(int x, int y, const STerrainInfo &info)
{
  ...
  {
    if ((x & 1) && (y & 1)) return (heights[nMapY + 1][nMapX].fHeight + heights[nMapY][nMapX + 1].fHeight) / 2.0f * fHeightCoeff;
    if (x & 1) return (heights[nMapY][nMapX + 1].fHeight + heights[nMapY + 1][nMapX + 1].fHeight) / 2.0f * fHeightCoeff;
    if (y & 1) return (heights[nMapY + 1][nMapX].fHeight + heights[nMapY + 1][nMapX + 1].fHeight) / 2.0f * fHeightCoeff;
                    ...
                    struct SAIPassabilityInfo *marker, int nMarkerSize, IGFXVertices *pVertices, IGFXIndices *pIndices)
{
  const CArray2D<SVertexAltitude> &heights = info.altitudes;
  {
    CVerticesLock<SMarkerVertex> vertices(pVertices);
    SMarkerVertex *pVerts = vertices.GetBuffer();
    constexpr DWORD dwSpecular = 0xff000000;
    for (int k = 0; k < nMarkerSize; ++k)
    {
      // humans, track, half-track, wheel <= colors: from dark green (128) to light green (255)
      // 3, 2, 1, 0
      DWORD dwColor = marker[k].pass == 0 ? 0 : (255UL - (GetMSB(marker[k].pass) + 1) * 32 << 8) | 0x80000000;

      const int i = info.AIY2Terra(static_cast<int>(marker[k].y));
      const int j = marker[k].x;
      //
                ...
                std::vector<SGFXLineVertex> &vertices, std::vector<WORD> &indices)
{
  const CArray2D<SMainTileInfo> &tiles = info.tiles;
  const CArray2D<SVertexAltitude> &heights = info.altitudes;
  constexpr DWORD dwColor = 0x80ffffff;
  //
