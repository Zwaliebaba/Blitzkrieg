...
bool IsPointInConvexPolygon(const CVec3 *points, const int *indices, const int nNumIndices, const CVec2 &vPoint)
{
  ...
  for (int i = 0; i < nNumIndices; ++i)
  {
    ...
    if ((vEnd.y <= vPoint.y) && (vPoint.y < vBeg.y) && ((vBeg.y - vEnd.y) * (vPoint.x - vEnd.x) < (vPoint.y - vEnd.y) * (vBeg.x - vEnd.x))) bInside = !bInside;
    else if ((vBeg.y <= vPoint.y) && (vPoint.y < vEnd.y) && ((vBeg.y - vEnd.y) * (vPoint.x - vEnd.x) > (vPoint.y - vEnd.y) * (vBeg.x - vEnd.x))) bInside = !bInside;
  }
                      ...
                      const CVec3 &pt0, const CVec3 &pt1, const CVec3 &pt2, const CVec3 &pt3)
{
  ...
  // projection of an arbitrary polygon
  CVec2 vMinMax2;
...
// 2 rectangles do not intersect if their projections at least on one of the perpendiculars to their faces do not intersect.
// Projecting perpendicular to 'screen rect' is simply choosing 'x' or 'y' coordinates
// when projecting onto an arbitrary line given by the vector (x, y), we simply multiply each component by (x, y)
// in total there are 6 direct ones that need to be projected onto: {1, 0}, {0, 1}, {x1, y1}, {x2, y2}, {x3, y3}, {x4, y4}
bool IsIntersected(const CTRect<float> &rect, const CVec3 *points, const int *indices)
{
  ...
  // {1, 0} (horizontal)
  {
    ...
    // projection of an arbitrary rectangle
    CVec2 vMinMax2(Min(Min(pt0.x, pt1.x), Min(pt2.x, pt3.x)),
                   Max(Max(pt0.x, pt1.x), Max(pt2.x, pt3.x)));
    //
    if ((vMinMax1.x >= vMinMax2.y) || (vMinMax2.x >= vMinMax1.y)) return false;
  }
  ...
  // {x4, y4} (4¤ rectangle of any rectangle)
  {
    // axis
...
bool CMeshVisObj::IsHit(const SHMatrix &matTransform, const CVec2 &point, CVec2 *pShift)
{
  ...
  if (fabs2(vScreenSphereCenter.x - point.x, vScreenSphereCenter.y - point.y) <= fabs2(sphere.fRadius))
  {
    ...
    if (!InPointInAABB(matrix, pMesh->GetAABB(), point)) return false;
...
bool CMeshVisObj::Update(const NTimer::STime &time, bool bForced)
{
  if (dwLastUpdateTime != time || bForced)
  {
    ...
    if (bHasScale) MultiplyScale(&matPlacement, matPlacement, vScale.x, vScale.y, vScale.z);
  ...
  if (!effectors.empty())
  {
    ...
    for (auto it = effectors.begin(); it != effectors.end();)
    {
      if (it->pEffector->Update(time) == false) it = effectors.erase(it);
      else
      {
        if (it->nPart == -1) mstack.Push(it->GetMatrix());
        ++it;
      }
    }
  ...
  else matPlacement1 = matPlacement;
  if (pMaterialEffector != nullptr)
  {
...
bool CMeshVisObj::DrawBB(IGFX *pGFX)
{
  // form AABB corners
  {
    ...
    auto vertices = reinterpret_cast<SGFXLineVertex *>(pGFX->GetTempVertices(8, SGFXLineVertex::format, GFXPT_LINELIST));
    //
  ...
  //
  {
    auto indices = reinterpret_cast<WORD *>(pGFX->GetTempIndices(24, GFXIF_INDEX16, GFXPT_LINELIST));
    indices[0] = 0;
    indices[1] = 1;
    indices[2] = 1;
    indices[3] = 3;
...
inline DWORD CheckLessZero(float fVal) { return (bit_cast<DWORD>(fVal) & 0x80000000); }

inline DWORD CheckForViewVolume(const CVec3 &vCenter, float fRadius, const SPlane *pViewVolumePlanes)
{
  DWORD dwClipFlags = 0;
  // There are two checks for each plane: if the distance to it is < -radius (i.e. far below the plane), then it is invisible
  // otherwise, set the corresponding bit based on the fact that the distance to the plane < radius (i.e. -radius <= dist < radius).
  // left
  float fDist = pViewVolumePlanes[GFXCP_LEFT].GetDistanceToPoint(vCenter);
  if (fDist < -fRadius) return GFXCP_OUT;
  ...
  // we don't need to check for near and far planes here
  /* // near
   */
  //
  return dwClipFlags;
...
void CMeshVisObj::AddEffector(int nID, ISceneMatrixEffector *pEffector, int nPart)
{
  ...
  // add new effector
  if (pEffector) effectors.push_back(SMatrixEffector(nID, pEffector, nPart));
}
...
void CMeshVisObj::AddMaterialEffector(ISceneMaterialEffector *pEffector) { if (pMaterialEffector == nullptr) { pMaterialEffector = pEffector; } }

void CMeshVisObj::RemoveMaterialEffector()
{
  pMaterialEffector = nullptr;
