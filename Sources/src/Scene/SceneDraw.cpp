...
public:
  CRawBuffer() : pData(nullptr), nSize(0), nCapacity(0) {}
  //
  ...
  void reserve(const int _nCapacity)
  {
    if (nCapacity < _nCapacity)
    {
      ...
      if (nSize > 0) memcpy(pNewData, pData, nSize * sizeof(TYPE));
...
constexpr float fCircleWidth = 10.0f;
constexpr int nNumSegments = 64;
constexpr float fAngleStep = FP_2PI / nNumSegments;
...
{
  constexpr float fCircleWidth = 10.0f;
  if (area.fMaxR < 3.0f * fCircleWidth) return;
  ...
  if (fMinR > 0.0f) DrawCircle(pGFX, area.vCenter3D, area.fMinR, dwColor);

  if (area.wStartAngle != area.wFinishAngle && bFill)
  {
    float fStartAngle = static_cast<float>(area.wStartAngle) / 65535.0f * FP_2PI + PI * 0.5;
    float fEndAngle = static_cast<float>(area.wFinishAngle) / 65535.0f * FP_2PI + PI * 0.5;
    if (fEndAngle < fStartAngle) fEndAngle += FP_2PI;
...
void DrawMeshes(CMeshVisList &meshes, IGFX *pGFX, bool bEnableBBs, const SGFXLightDirectional &sunlight)
{
  ...
  for (auto it = meshes.begin(); it != meshes.end(); ++it) (*it)->Draw(pGFX);
  pGFX->EnableLighting(false);
  // draw bounding boxes
  if (bEnableBBs)
  {
    pGFX->SetTexture(0, nullptr);
    for (auto it = meshes.begin(); it != meshes.end(); ++it) (*it)->DrawBB(pGFX);
  }
}
...
void DrawMeshesChecked(CMeshVisList &meshes, IGFX *pGFX, bool bEnableBBs, const SGFXLightDirectional &sunlight, const SPlane *pvViewVolumePlanes)
{
  ...
  for (auto it = meshes.begin(); it != meshes.end(); ++it)
  {
    ...
    if (dwClipFlags != GFXCP_OUT) (*it)->Draw(pGFX);
  }
  pGFX->EnableLighting(false);
  // draw bounding boxes
  if (bEnableBBs)
  {
    ...
    {
      const DWORD dwClipFlags = (*it)->CheckForViewVolume(pvViewVolumePlanes);
      if (dwClipFlags != GFXCP_OUT) (*it)->DrawBB(pGFX);
    }
  }
}
...
void CScene::Draw(ICamera *pCamera)
{
  ...
  if (bEnableDepthComplexity) pGFX->SetShadingEffect(300);
  // draw terrain
  ...
  if (pDrawVisitor == nullptr) pDrawVisitor = new CDrawVisitor(GetGlobalVar("particlesdepth", 20.0f));
  pDrawVisitor->Clear();
  ...
  if (bEnableObjects)
  {
    ...
    // new draw areas
    {
      ...
      for (auto it = areas.begin(); it != areas.end(); ++it)
      {
        ...
        for (auto innerIter = it->areas.begin(); innerIter != it->areas.end(); ++innerIter) { if (innerIter->eType == SShootArea::ESAT_BALLISTIC) bIsBallistic = true; }

        for (auto innerIter = it->areas.begin(); innerIter != it->areas.end(); ++innerIter) DrawArea(pGFX, *innerIter, innerIter->eType != SShootArea::ESAT_LINE || !bIsBallistic);
      }
      //
    ...
    if (pTerrain != nullptr && pCamera != nullptr && bEnableShowBorder)
    {
      pGFX->SetTexture(0, nullptr);
    ...
    if (bEnableShadows)
    {
      ...
      // to render shadows to cut off intersections, set the z-buffer mode to '!=', i.e. 
      ...
      if (!pDrawVisitor->meshes.empty())
      {
        ...
        pGFX->SetTexture(0, nullptr);
        // main meshes
        for (auto it = pDrawVisitor->meshes.begin(); it != pDrawVisitor->meshes.end(); ++it)
        {
          CVisObjDescMap::const_iterator pos = objdescs.find(*it);
          if ((pos->second.gametype != SGVOGT_ENTRENCHMENT) && (pos->second.gametype != SGVOGT_TANK_PIT)) (*it)->DrawShadow(pGFX, nullptr, sunlight.vDir);
        }
        pGFX->EnableLighting(false);
      }
  ...
  if (bEnableObjects)
  {
    ...
    if (!pDrawVisitor->textes.empty())
    {
      ...
      for (auto it = pDrawVisitor->textes.begin(); it != pDrawVisitor->textes.end(); ++it)
      {
  ...
  if (bEnableShadows && !pDrawVisitor->aviation.empty())
  {
    ...
    // draw shadows from 3D mesh objects
    {
      ...
      pGFX->SetTexture(0, nullptr);
      // avaiation
      for (auto it = pDrawVisitor->aviation.begin(); it != pDrawVisitor->aviation.end(); ++it) (*it)->DrawShadow(pGFX, &matShadow, sunlight.vDir);
      //
      pGFX->EnableLighting(false);
    }
    pGFX->SetShadingEffect(113);
  ...
  if (!pDrawVisitor->boldLines.empty())
  {
    ...
    int nLine = 0;
    for (auto it = pDrawVisitor->boldLines.begin(); it != pDrawVisitor->boldLines.end(); ++it, ++nLine)
    {
  ...
  pGFX->SetWorldTransforms(0, &MONE, 1);
  for (auto it = tempmeshes.begin(); it != tempmeshes.end();)
  {
    pGFX->SetTexture(0, it->pTexture);
    pGFX->SetShadingEffect(it->nShadingEffect);
    pGFX->Draw(it->pVertices, it->pIndices);
    if (it->bTemporal) it = tempmeshes.erase(it);
    else ++it;
  }
  // draw additional meshes 2
  for (auto it = meshpairs2.begin(); it != meshpairs2.end();)
  {
    pGFX->SetTexture(0, it->pTexture);
    pGFX->SetShadingEffect(it->nShadingEffect);
    ...
    pGFX->DrawTemp();
    if (it->bTemporary) it = meshpairs2.erase(it);
    else ++it;
  }
  // pGFX->SetWorldTransforms( 0, &MONE, 1 );
  // pGFX->SetShadingEffect( 3 );
  ...
  // draw fog of war
  if (bEnableWarFog && pTerrain) pTerrain->DrawWarFog();
  // units - aviation
  if (bEnableUnits) DrawMeshesChecked(pDrawVisitor->aviation, pGFX, bEnableBBs, sunlight, &(vViewVolumePlanes[0]));
  //
  // CODE{ 2D scene part
  //
  pGFX->SetupDirectTransform();
  ...
  if (pFrameSelection->IsActive()) pFrameSelection->Draw(pGFX);
  // squad icons
  if (!squads.empty())
  {
    CVec2 vPos(5, 5);
    ...
    for (CDrawVisitor::CUIObjectsList::const_iterator it = pDrawVisitor->uiObjects.begin(); it != pDrawVisitor->uiObjects.end(); ++it)
    {
      switch (it->eType)
      {
        case SUIObject::TYPE_RECTS:
          pGFX->SetTexture(0, it->pTexture);
          if (it->nShadingEffect != -1) pGFX->SetShadingEffect(it->nShadingEffect);
          pGFX->DrawRects(&(it->rects[0]), it->rects.size());
          break;
        case SUIObject::TYPE_TEXT:
          if (it->dwColor != 0) it->pText->SetColor(it->dwColor);
          pGFX->SetShadingEffect(3);
  ...
  if (!alwaysObjects.empty())
  {
    ...
    for (auto it = alwaysObjects.begin(); it != alwaysObjects.end();)
    {
      if ((*it)->Update(currTime) == false) it = alwaysObjects.erase(it);
      else
      {
        (*it)->Draw(pGFX);
        ++it;
...
public:
  ...
  {
    if ((pSp1->dwCheckFlags & 0xffff0000) == (pSp2->dwCheckFlags & 0xffff0000)) return pSp1->pTexture == pSp2->pTexture ? pSp1->relpos.z > pSp2->relpos.z : pSp1->pTexture < pSp2->pTexture;
...
template<class TYPE>
void Reserve2Draw(TYPE &sprites, const int nNumElements)
{
                 ...
                 const CTRect<float> &rcScreen, IGFX *pGFX)
{
  ...
  if (!sprites2Draw.sprites.empty()) DrawSingleSpritesPack(sprites2Draw.sprites, calculator, rcScreen, pGFX);
  if (!complexsprites2Draw.sprites.empty()) DrawComplexSpritesPack(complexsprites2Draw.sprites, calculator, rcScreen, pGFX);
}

#ifdef _USE_HWTL
typedef SGFXLVertex SSpriteVertex;
#else
using SSpriteVertex = SGFXTLVertex;
#endif // _USE_HWTL
...
public:
  ...
  //
  const float GetZ1(const SComplexSpriteInfo *pInfo, const SSpritesPack::SSprite::SSquare &square) const { return pInfo->relpos.z - fZBias * (square.vLeftTop.y + square.fDepthLeft + square.fSize) + fZBias2 * square.fDepthLeft; }
  const float GetZ2(const SComplexSpriteInfo *pInfo, const SSpritesPack::SSprite::SSquare &square) const { return pInfo->relpos.z - fZBias * (square.vLeftTop.y + square.fDepthLeft) + fZBias2 * square.fDepthLeft; }
  const float GetZ3(const SComplexSpriteInfo *pInfo, const SSpritesPack::SSprite::SSquare &square) const { return pInfo->relpos.z - fZBias * (square.vLeftTop.y + square.fDepthRight + square.fSize) + fZBias2 * square.fDepthRight; }
  const float GetZ4(const SComplexSpriteInfo *pInfo, const SSpritesPack::SSprite::SSquare &square) const { return pInfo->relpos.z - fZBias * (square.vLeftTop.y + square.fDepthRight) + fZBias2 * square.fDepthRight; }
};
              ...
              const float fTexDiffX, const float fTexDiffY, const float fScrDiff)
{
  ...
  if (pInfo->dwCheckFlags & 1)
  {
    ...
    if ((dwPoint1 & dwPoint2 & dwPoint3 & dwPoint4) != 0) return 0;
  }
  //
                           ...
                           const CTRect<float> &rcScreen, IGFX *pGFX)
{
  ...
  if (sprites[0]->pTexture != nullptr)
  {
    fTexDiffX = -0.5f / static_cast<float>(sprites[0]->pTexture->GetSizeX(0));
    fTexDiffY = -0.5f / static_cast<float>(sprites[0]->pTexture->GetSizeY(0));
    fScrDiff = -0.5f;
  }
  //
  WORD wCurrVertex = 0;
                            ...
                            const CTRect<float> &rcScreen, IGFX *pGFX)
{
  ...
  int nNumSquares = 0;
  for (auto it = sprites.begin(); it != sprites.end(); ++it) nNumSquares += (*it)->pSprite->squares.size();
  drawvertices.resize(0);
  ...
  for (auto sprite = sprites.begin(); sprite != sprites.end(); ++sprite)
  {
    ...
    for (auto it = pInfo->pSprite->squares.begin(); it != pInfo->pSprite->squares.end(); ++it)
    {
...
{
  auto pVertices = static_cast<SSpriteVertex *>(pGFX->GetTempVertices(nNumParticles * 4, SSpriteVertex::format, GFXPT_TRIANGLELIST));
  auto pIndices = static_cast<WORD *>(pGFX->GetTempIndices(nNumParticles * 6, GFXIF_INDEX16, GFXPT_TRIANGLELIST));
  WORD wCurrVertex = 0;
  bool bSpecularEnable = false;
  float fX, fY;
  for (auto it = particles.begin(); it != particles.end(); ++it, wCurrVertex += 4)
  {
...
void CScene::DrawMechTraces(const std::list<SMechTrace> &traces)
{
  ...
  if (pTrackTexture == nullptr) pTrackTexture = GetSingleton<ITextureManager>()->GetTexture("units\\technics\\tanktrack");
  ...
  WORD wCurrVertex = 0;
  for (auto it = traces.begin(); it != traces.end(); ++it, wCurrVertex += 4)
  {
...
void CScene::DrawMarkers()
{
  if (bDrawArrow)
  {
    ...
    vNDir.y = vArrowDir.x;
    vNDir.z = vArrowDir.z;
    constexpr float fArrowLen = 20.0f;
    constexpr float fArrowHeadLen = 5.0f;
  ...
  if (!posMarkers.empty())
  {
    ...
    for (auto it = posMarkers.begin(); it != posMarkers.end(); ++it)
    {
      ...
      constexpr float fInnerRadius = 10;
      constexpr float fOuterRadius = 15;
      constexpr float fCosCoeff = 0.707f;// sqrt(2)/2
  ...
  for (auto it = clickMarkers.begin(); it != clickMarkers.end(); ++it)
  {
    ...
    else
    {
      ...
      const DWORD dwColor = (static_cast<DWORD>(0xff * (1.0 - fStage)) << 24) | dwMarkerColor;
      CTempBufferLock<SGFXLineVertex> vertices = pGFX->GetTempVertices(18, SGFXLineVertex::format, GFXPT_TRIANGLESTRIP);
      vertices[0].Setup(vPos.x, vPos.y + fOuterRadius, vPos.z, dwColor);
      vertices[1].Setup(vPos.x, vPos.y + fInnerRadius, vPos.z, dwColor);
      vertices[2].Setup(vPos.x + fOuterRadius * fCosCoeff, vPos.y + fOuterRadius * fCosCoeff, vPos.z, dwColor);
...
{
  if (pSandTexture == nullptr) pSandTexture = GetSingleton<ITextureManager>()->GetTexture("effects\\particles\\sanddust");
  int nNumCone = 0;
  for (std::vector<SSandParticle>::const_iterator it = sandParticles.begin(); it != sandParticles.end(); ++it) { if (it->bConeDraw) nNumCone++; }
...
{
  if (time > pTrace->deathTime) return false;
  DWORD col = static_cast<DWORD>(pTrace->alpha + (1.0 - (pTrace->deathTime - time) / static_cast<float>(pTrace->deathTime - pTrace->birthTime)) * (255.0 - pTrace->alpha));
...
bool UpdateGunTrace(SGunTrace *pTrace, const NTimer::STime &time, float fTraceLen)
{
  ...
  {
    const float fTime = (time - pTrace->birthTime) / static_cast<float>(pTrace->deathTime - pTrace->birthTime);
...
void CScene::UpdateWeather()
{
  ...
  if (!bWeatherInitialized)
  {
    ...
    if (strcmp(szSeason.c_str(), "Summer") == 0) eCurrSetting = ST_RAIN;
    else if (strcmp(szSeason.c_str(), "Winter") == 0) eCurrSetting = ST_SNOW;
    else if (strcmp(szSeason.c_str(), "Africa") == 0) eCurrSetting = ST_SAND;
    else eCurrSetting = ST_SNOW;
    if (eCurrSetting == ST_SNOW) snowFlakes.resize(nMinSnowDensity);
  ...
  if (nLastWeatherUpdate != 0)
  {
    ...
    {
      rainDrops.resize(Clamp(static_cast<int>(rainDrops.size() + static_cast<float>(time - nLastWeatherUpdate) / fChangeSpeed * nRainDensity), 0, nRainDensity));
    ...
    {
      if (it->fLength * vRainDir.z < -it->vPos.z || it->vPos.x < viewableTerrainRect.x1 || it->vPos.x > viewableTerrainRect.x2 || it->vPos.y < viewableTerrainRect.y1 || it->vPos.y > viewableTerrainRect.y2) RandomizeRainDrop(*it);
      else it->fLength += time - nLastWeatherUpdate;
    }
    // snow
    if (eCurrSetting == ST_SNOW && snowFlakes.size() != nMaxSnowDensity && (eWeatherCondition == SC_STARTING || eWeatherCondition == SC_ON)) { snowFlakes.resize(Clamp(static_cast<int>(snowFlakes.size() + static_cast<float>(time - nLastWeatherUpdate) / fChangeSpeed * (nMaxSnowDensity - nMinSnowDensity)), nMinSnowDensity, nMaxSnowDensity)); }
    else if (eCurrSetting == ST_SNOW && snowFlakes.size() != nMinSnowDensity && (eWeatherCondition == SC_FINISHING || eWeatherCondition == SC_NONE)) { snowFlakes.resize(Clamp(static_cast<int>(snowFlakes.size() - static_cast<float>(time - nLastWeatherUpdate) / fChangeSpeed * (nMaxSnowDensity - nMinSnowDensity)), nMinSnowDensity, nMaxSnowDensity)); }
    CVec3 vSpeed = VNULL3;
    for (auto it = snowFlakes.begin(); it != snowFlakes.end(); ++it)
    {
      if (it->vPos.z < 0 || it->vPos.x < viewableTerrainRect.x1 || it->vPos.x > viewableTerrainRect.x2 || it->vPos.y < viewableTerrainRect.y1 || it->vPos.y > viewableTerrainRect.y2) RandomizeSnowFlake(*it);
      vSpeed.z = -fabs(sin(it->fPhase)) * fSnowFallingSpeed;
      vSpeed.x = cos(it->fPhase) * fSnowAmplitude;
      it->fPhase += static_cast<float>(time - nLastWeatherUpdate) * fSnowFrequency;
      it->vPos += static_cast<float>(time - nLastWeatherUpdate) * vSpeed;
    }
    // sand
    if (eCurrSetting == ST_SAND && sandParticles.size() != nSandDensity && (eWeatherCondition == SC_STARTING || eWeatherCondition == SC_ON)) { sandParticles.resize(Clamp(static_cast<int>(sandParticles.size() + static_cast<float>(time - nLastWeatherUpdate) / fChangeSpeed * nSandDensity), 0, nSandDensity)); }
    else if (eCurrSetting == ST_SAND && sandParticles.size() != 0 && (eWeatherCondition == SC_FINISHING || eWeatherCondition == SC_NONE)) { sandParticles.resize(Clamp(static_cast<int>(sandParticles.size() - static_cast<float>(time - nLastWeatherUpdate) / fChangeSpeed * nSandDensity), 0, nSandDensity)); }
    CVec2 vConeDir;
    float fRangeCoeff;
    if (vSandCone.x < viewableTerrainRect.x1 || vSandCone.x > viewableTerrainRect.x2 || vSandCone.y < viewableTerrainRect.y1 || vSandCone.y > viewableTerrainRect.y2 || time - nLastConeGenerated > 20000)
    {
      ...
      const float fAngle = NWin32Random::Random(0.0f, 2.0f * static_cast<float>(PI));
      vConeSpeed.x = cos(fAngle);
      vConeSpeed.y = sin(fAngle);
      nLastConeGenerated = time;
      for (auto it = sandParticles.begin(); it != sandParticles.end(); ++it) it->bCone = false;
    }
    for (auto it = sandParticles.begin(); it != sandParticles.end(); ++it)
    {
      if (it->vPos.z > fSandHeight || it->vPos.z < 0 || it->vPos.x < viewableTerrainRect.x1 || it->vPos.x > viewableTerrainRect.x2 || it->vPos.y < viewableTerrainRect.y1 || it->vPos.y > viewableTerrainRect.y2) RandomizeSand(*it);
      if (it->bCone)
      {
        ...
        it->vPos += (static_cast<float>(time - nLastWeatherUpdate) * fSandSpeed) * vSpeed;
      }
      else
      {
        vConeDir.x = vSandCone.x - it->vPos.x;
        ...
        if (it->bConeDraw) vSpeed.z -= 0.02f;
        it->vPos += (static_cast<float>(time - nLastWeatherUpdate) * fSandSpeed) * vSpeed;
        it->vPhase.x += static_cast<float>(time - nLastWeatherUpdate) * fSandFrequency;
        it->vPhase.y += static_cast<float>(time - nLastWeatherUpdate) * fSandFrequency;
        it->vPhase.z += static_cast<float>(time - nLastWeatherUpdate) * fSandFrequency;
      }
    }
    vSandCone += (static_cast<float>(time - nLastWeatherUpdate) * fSandConeSpeed) * vConeSpeed;
    if (wAmbientID != 0 && pSoundScene->GetMode() == ESSM_INGAME)
    {
      switch (eCurrSetting)
      {
        case ST_RAIN:
          ...
          {
            if (nNextRandomSound != 0) AddSound("Amb_Rain", CVec3((viewableTerrainRect.x1 + viewableTerrainRect.x2) * 0.5f, (viewableTerrainRect.y1 + viewableTerrainRect.y2) * 0.5f, 0.0f), SFX_MIX_IF_TIME_EQUALS, SAM_ADD_N_FORGET);
...
void CScene::FormVisibilityLists(ICamera *pCamera, ISceneVisitor *pVisitor)
{
  ...
  if (bWeatherOn && bEnableEffects)
  {
    ...
    {
      viewableTerrainRect.x1 = Min(viewableTerrainRect.x1, static_cast<float>(it->first));
      viewableTerrainRect.y1 = Min(viewableTerrainRect.y1, static_cast<float>(it->second));
      viewableTerrainRect.x2 = Max(viewableTerrainRect.x2, static_cast<float>(it->first));
      viewableTerrainRect.y2 = Max(viewableTerrainRect.y2, static_cast<float>(it->second));
    }
  ...
  for (CPatchesList::const_iterator pos = patches.begin(); pos != patches.end(); ++pos)
  {
    ...
    // sprite objects (static - don't need update!)
    {
      ...
      for (auto it = data.begin(); it != data.end(); ++it) (*it)->Visit(pVisitor, SGVOGT_OBJECT);
    }
    // terrain objects
    {
      CObjVisObjArea::CDataList &data = terraObjectsArea[pos->second][pos->first];
      for (auto it = data.begin(); it != data.end(); ++it) (*it)->Visit(pVisitor, SGVOGT_TERRAOBJ);
    }
    // craters and shell-holes
    for (auto it(areaCraters.Iterate(pos->first, pos->second)); !it.IsEnd(); it.Next())
    {
    ...
    // shadow objects
    {
      ...
      for (auto it = data.begin(); it != data.end(); ++it) (*it)->Visit(pVisitor, SGVOGT_SHADOW);
    }
    // mech trace objects
    // CRAP{ unfortunately there is no time to think it through...
    {
      CMechTraceArea::CDataList &data = mechTracesArea[pos->second][pos->first];
      for (auto it = data.begin(); it != data.end();)
      {
        if (UpdateMechTrace(&(*it), time) == false) it = data.erase(it);
        else
        {
    ...
    // CRAP{ unfortunately there is no time to think it through...
    {
      ...
      for (auto it = data.begin(); it != data.end();)
      {
        if (UpdateGunTrace(&(*it), time, fTraceLen) == false) it = data.erase(it);
        else
        {
  ...
  // outbound sprites
  for (auto it = outboundSprites.begin(); it != outboundSprites.end(); ++it)
  {
    if (IsVisible(*it))
    {
      (*it)->Update(time);
  ...
  for (auto it = outboundObjects.begin(); it != outboundObjects.end(); ++it)
  {
    (*it)->Update(time);
    (*it)->Visit(pVisitor, -1);
  }
  // outbound objects 2
  for (auto it = outboundObjects2.begin(); it != outboundObjects2.end(); ++it)
  {
    if (IsVisible(*it))
    {
      (*it)->Update(time);
  ...
  // outbound effects
  for (auto it = outboundEffects.begin(); it != outboundEffects.end();)
  {
    // update. 
    if ((*it)->Update(time) == false)
    {
  ...
  // line objects
  for (auto it = boldLines.begin(); it != boldLines.end(); ++it) (*it)->Visit(pVisitor);
  // UI screens
  if (bShowUI) { for (auto it = uiScreens.begin(); it != uiScreens.end(); ++it) (*it)->Visit(pVisitor); }
}
