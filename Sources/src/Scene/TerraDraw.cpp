...
{
  {
    ...
    {
      pGFX->SetTexture(1, bEnableNoise ? pNoise : nullptr);
...
{
  if (ExtractVisiblePatches(pCamera)) MovePatches();
  //
...
bool CTerrain::DrawVectorObjects()
{
  ...
  for (int i = 0; i != roads.size(); ++i) roads[i].DrawBorder(pGFX);
  // draw road layers (center)
  pGFX->SetShadingEffect(8);
  for (int i = 0; i != roads.size(); ++i) roads[i].DrawCenter(pGFX);
  // draw water base (bottom)
  pGFX->SetShadingEffect(8);
  for (int i = 0; i != rivers.size(); ++i) rivers[i].DrawBase(pGFX);
  // draw water layers
  pGFX->SetShadingEffect(303);
  for (int i = 0; i != rivers.size(); ++i) rivers[i].DrawWater(pGFX);
...
bool CTerrain::DrawMarkers()
{
  ...
  if (bGridOn)
  {
    ...
    pGFX->SetWorldTransforms(0, &MONE, 1);
    for (auto it = patches.begin(); it != patches.end(); ++it) DrawGrid(*it);
  }
  pGFX->SetDepthBufferMode(GFXDB_USE_Z);
  return true;
}
...
bool CTerrain::DrawWarFog()
{
  ...
  {
    if (it->warfogverts.empty() || it->warfoginds.empty()) continue;
...
{
  if (vismarker.info.empty() && aimarker.info.empty()) return;
  //
...
bool IsPatchVisible(const STerrainPatchInfo &patch, const STerrainInfo &info, const SPlane *pViewVolumePlanes, bool *flags)
{
  ...
  const float fMinY = static_cast<float>(info.tiles.GetBoundY() - patch.nStartY - 16) * fWorldCellSize;
...
bool CTerrain::ExtractVisiblePatches(ICamera *pCamera)
{
  ...
  if ((vCamera.x == vOldAnchor.x) && (vCamera.y == vOldAnchor.y) && (vCamera.z == vOldAnchor.z)) return false;
  vOldAnchor = vCamera;
  ...
  //
  //
  // define a rough rectangle (in the terrain coordinate system, in whole tiles) into which the screen fits
  // the determination is made based on the distance from the corners of the screen to the coordinate axes of the terrain system
  ...
  // Now let’s check all the patches from the resulting rectangle (rcL0Rect)
  ...
  for (int j = rcL0Rect.miny; j < rcL0Rect.maxy; ++j)
  {
    for (int i = rcL0Rect.minx; i < rcL0Rect.maxx; ++i)
    {
      ...
      if (IsPatchVisible(terrainInfo.patches[j][i], terrainInfo, viewVolumePlanes, flags))
      {
        ...
        // adding new peppers to the old list
        bool bExist = false;
        for (auto it = patches.begin(); it != patches.end(); ++it)
        {
          if ((it->nX == i) && (it->nY == j))
  ...
  // we remove from the old list those who were not included in the new one
  for (auto it = patches.begin(); it != patches.end();)
  {
    if (std::find(accepted.begin(), accepted.end(), std::pair<int, int>(it->nX, it->nY)) == accepted.end()) it = patches.erase(it);
    else ++it;
  }
  ...
  {
    const DWORD dwPatch = (static_cast<DWORD>(it->nX) << 16) | static_cast<DWORD>(terrainInfo.patches.GetBoundY() - it->nY);
    roadPatches.push_back(dwPatch);
  }
  std::sort(roadPatches.begin(), roadPatches.end());
  for (int i = 0; i != roads.size(); ++i) roads[i].SelectPatches(roadPatches);
  for (int i = 0; i != rivers.size(); ++i) rivers[i].SelectPatches(roadPatches);
  //
  return true;
}
...
bool CTerrain::DrawAISurface(const STerrainPatch &patch)
{
  ...
  if (pAILogic == nullptr) return false;
  //
...
{
  using CIndexPair = std::pair<int, int>;
  ...
  for (CPatchesList::const_iterator it = patches.begin(); it != patches.end(); ++it)
  {
    if (it->nX == 0 || it->nY == 0 || it->nX == terrainInfo.patches.GetSizeX() - 1 || it->nY == terrainInfo.patches.GetSizeY() - 1)
    {
      ...
      if (maxX < it->nX) maxX = it->nX;
      if (maxY < it->nY) maxY = it->nY;
      if (minX > it->nX) minX = it->nX;
      if (minY > it->nY) minY = it->nY;
    }
  }
  for (std::list<CIndexPair>::const_iterator it = patchIndexes.begin(); it != patchIndexes.end(); ++it)
  {
    ...
    {
      if (it->second != 0 && it->second == minY) { patchIndexes.push_front(CIndexPair(it->first, it->second - 1)); }
      if (it->second != terrainInfo.patches.GetSizeY() - 1 && it->second == maxY) { patchIndexes.push_front(CIndexPair(it->first, it->second + 1)); }
    }
    if (it->second == minY || it->second == maxY)
    {
      if (it->first != 0 && it->first == minX) { patchIndexes.push_front(CIndexPair(it->first - 1, it->second)); }
      if (it->first != terrainInfo.patches.GetSizeX() - 1 && it->first == maxX) { patchIndexes.push_front(CIndexPair(it->first + 1, it->second)); }
    }
  }
  constexpr float fBorderOverflow = 1.0f;
  for (std::list<CIndexPair>::const_iterator it = patchIndexes.begin(); it != patchIndexes.end(); ++it)
  {
    int nX = it->first;
    int nY = it->second;
    if (nX == 0)
    {
      auto vStartPos = CVec3(fBorderOverflow, (terrainInfo.patches.GetSizeY() - nY) * fPatchSizeY, 0);
      auto vFarShift = CVec3(-fBorderWidth, 0, 0);
      auto vStepShift = CVec3(0, -fCellSize, 0);
    ...
    if (nY == 0)
    {
      auto vStartPos = CVec3(fPatchSizeX * nX, terrainInfo.patches.GetSizeY() * fPatchSizeY - fBorderOverflow, 0);
      auto vFarShift = CVec3(0, fBorderWidth, 0);
      auto vStepShift = CVec3(fCellSize, 0, 0);
    ...
    if (nX == terrainInfo.patches.GetSizeX() - 1)
    {
      auto vStartPos = CVec3(terrainInfo.patches.GetSizeX() * fPatchSizeX - fBorderOverflow, (terrainInfo.patches.GetSizeY() - nY) * fPatchSizeY, 0);
      auto vFarShift = CVec3(fBorderWidth, 0, 0);
      auto vStepShift = CVec3(0, -fCellSize, 0);
    ...
    if (nY == terrainInfo.patches.GetSizeY() - 1)
    {
      auto vStartPos = CVec3(nX * fPatchSizeX, fBorderOverflow, 0);
      auto vFarShift = CVec3(0, -fBorderWidth, 0);
      auto vStepShift = CVec3(fCellSize, 0, 0);
...
void CTerrain::DrawPatchBorder(CVec3 &vStartPos, const CVec3 &vFarOffset, const CVec3 &vStepOffset, bool bRightOrder, bool bXIncrement, int nXStart, int nYStart, DWORD dwColor, bool bUseFog)
{
  ...
  {
    if (bXIncrement) vStartPos.z = terrainInfo.altitudes[nXStart + i][nYStart].fHeight;
    else vStartPos.z = terrainInfo.altitudes[nXStart][nYStart + i].fHeight;
    ...
    if (bXIncrement) dwVertColor = bUseFog ? GetVisibilityColor(((nXStart + i) << 16) | nYStart, visibilities) : dwColor;
    else dwVertColor = bUseFog ? GetVisibilityColor((nXStart << 16) | (nYStart + i), visibilities) : dwColor;
  }
  pGFX->DrawTemp();
}
