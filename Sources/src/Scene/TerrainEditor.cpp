                              ...
                              bool bTerrainCoords, bool isExact)
{
  ...
  // x
  {
    ...
    if (isExact) { *pnX = static_cast<int>(fDist / fCellSize); }
    else { *pnX = static_cast<int>(fDist / fCellSize + 0.5f); }
    if ((fDist < 0) || (fDist > fTerraSizeX)) { result = false; }
  }
  // y
  {
    ...
    if (bTerrainCoords)
    {
      if (isExact) { *pnY = static_cast<int>((fTerraSizeY - fDist) / fCellSize); }
      else { *pnY = static_cast<int>((fTerraSizeY - fDist) / fCellSize + 0.5f); }
    }
    else
    {
      if (isExact) { *pnY = static_cast<int>(fDist / fCellSize); }
      else { *pnY = static_cast<int>(fDist / fCellSize + 0.5f); }
    }
    if ((fDist < 0) || (fDist > fTerraSizeY)) { result = false; }
  }
  //
  return result;
}
...
public:
  ...
  bool operator()(const SCrossTileInfo &_cross) const { return (cross.x == _cross.x) && (cross.y == _cross.y) && (cross.tile == _cross.tile); }
};
...
void CTerrain::SetShade(int x, int y, BYTE shade) { terrainInfo.altitudes[y][x].shade = shade; }
BYTE CTerrain::GetShade(int x, int y) { return terrainInfo.altitudes[y][x].shade; }
    ...
    for (int i = rcPatches.top; i <= rcPatches.bottom; ++i)
    {
      for (int j = rcPatches.left; j <= rcPatches.right; ++j)
      {
        ...
        builder.MapSegmentGenerateCrosses(terrainInfo.tiles, rcRect, rcSuper, &newCrosses);
        {
          ...
          /* // remove old crosses and insert new ones
           */
          // kill this patch and rebuild the landscape
...
{
  if ((terrainInfo.tiles.GetSizeX() == 0) || (terrainInfo.tiles.GetSizeY() == 0)) return nullptr;
...
{
  CTerrainRoad *pRoad = nullptr;
  SVectorStripeObject *pRoadDesc = nullptr;
  for (int i = 0; i != terrainInfo.roads3.size(); ++i)
  {
    // find terrain road with such ID
    if (roads[i].GetID() == nRoadID) pRoad = &(roads[i]);
    // find road descriptor with such ID
    if (terrainInfo.roads3[i].nID == nRoadID) pRoadDesc = &(terrainInfo.roads3[i]);
    // re-init road
    if (pRoad && pRoadDesc)
    {
...
void CTerrain::SmoothCurveWidth(SVectorStripeObjectPoint *points, const int nNumPoints)
{
  // count key points
  int nNumKeyPoints = 0;
  for (SVectorStripeObjectPoint *it = points; it != points + nNumPoints; ++it) nNumKeyPoints += it->bKeyPoint;
  // collect key point indices in the vector
  std::vector<int> keypoints;
  keypoints.reserve(nNumKeyPoints);
  for (int i = 0; i != nNumPoints; ++i) { if (points[i].bKeyPoint) keypoints.push_back(i); }
  // build spline over width in the key points
  std::vector<float> widthes(nNumPoints);
  for (int i = 0; i != nNumPoints; ++i) widthes[i] = points[i].fWidth;
  // form indices for spline
  std::vector<int> indices;
  indices.reserve(nNumKeyPoints + 4);
  indices.push_back(0);
  indices.push_back(0);
  for (int i = 0; i < nNumKeyPoints; ++i) indices.push_back(i);
  ...
  for (int i = 0; i != (indices.size() - 3); ++i)
  {
    ...
    {
      const float t = static_cast<float>(j - idx1) / static_cast<float>(idx2 - idx1);
      widthes[j] = spline(t);
    }
  }
  // write widths to the result array
  for (int i = 0; i != nNumPoints; ++i) points[i].fWidth = widthes[i];
}