...
inline const float GetRandomValue(const float fValue, const float fTime, const CTrack &rnd) { return fValue * (1.0f + GetRandomFromTrack(fTime, rnd)); }

inline const float Integrate(const float fTime1, const float fVal1, const float fTime2, const float fVal2) { return 0.5f * (fVal2 + fVal1) * (fTime2 - fTime1); }
...
{
  auto pos = FindKey(fTime);
  if ((pos != keys.end()) && (pos->fTime == fTime)) keys.erase(pos);
}
...
const float CTrack::GetValue(const float fTime) const
{
  ...
  //
  auto posEnd = keys.end();
  --posEnd;
  CKeysList::const_iterator posLow, posHigh = keys.begin();
  while ((posHigh != posEnd) && (fTime >= posHigh->fTime)) posLow = posHigh++;
...
const float CTrack::GetValue(const float fTime, const CTrack &rnd) const { return GetRandomValue(GetValue(fTime), fTime, rnd); }
...
const float CTrack::Integrate(STrackContext *pContext, const float _fTimeEnd, const CTrack &rnd) const
{
  ...
  if (pContext->nUpperKeyIndex == keys.size()) return 0;
  ...
  auto pos = std::upper_bound(keys.begin(), keys.end(), SKey(fTimeEnd, 0));
  ...
  if (nUpperKeyIndex > pContext->nUpperKeyIndex + 1)
  {
    ...
    for (auto it = keys.begin() + pContext->nUpperKeyIndex + 1; it != pos; ++it)
    {
...
const float CTrack::Integrate(STrackContext *pContext, const float _fTimeEnd) const
{
  ...
  NI_ASSERT_SLOW_TF(pContext->fTime <= fTimeEnd, "Can't integrate track - start time are greater then finish one", return 0);
  // check for upper bound
  if (pContext->nUpperKeyIndex == keys.size() || pContext->nUpperKeyIndex < 0) return 0;
  // check, are we in the same time range
  if (fTimeEnd <= keys[pContext->nUpperKeyIndex].fTime)
  {
    const float fCoeff = (fTimeEnd - keys[pContext->nUpperKeyIndex - 1].fTime) /
...
void CTrack::Normalize(float _fScale)
{
  ...
  if (_fScale != fScale)
  {
    ...
    for (auto it = keys.begin(); it != keys.end(); ++it) it->fTime *= fMultiplier;
    if (keys[0].fTime > 0) { AddKey(0, keys[0].fValue); }
    if (keys[keys.size() - 1].fTime < _fScale) { AddKey(_fScale, keys[keys.size() - 1].fValue); }
    fScale = _fScale;
  }
}
...
float CTrack::GetTimeByIndex(int index) const { return keys[index].fTime; }

int CTrack::GetNumKeys() const { return keys.size(); }