...
class CTrack
{
  ...
  //
  using CKeysList = std::vector<SKey>;
  CKeysList keys;// keys
  //
  float fScale;// time scale
  //
...
public:
  CTrack();
  //
  CTrack &operator=(const CTrack &trc)
  {
    keys = trc.keys;
    fScale = trc.fScale;
    return *this;
  ...
  // keys management
  void AddKey(float fTime, float fValue);
  void RemoveKey(float fTime);
  void Normalize(float _fScale);
  // key bounds
  ...
  const float GetValue(float fTime) const;
  const float GetValue(float fTime, const CTrack &rnd) const;
  void CreateStartContext(STrackContext *pContext) const;
  // integrate from track context to the 'fTimeEnd'
  const float Integrate(STrackContext *pContext, float fTimeEnd) const;
  void CreateStartContext(STrackContext *pContext, const CTrack &rnd) const;
  // integrate from track context to the 'fTimeEnd'
  const float Integrate(STrackContext *pContext, float fTimeEnd, const CTrack &rnd) const;
  //
  int operator&(IDataTree &ss);
