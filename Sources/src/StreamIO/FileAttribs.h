...
inline time_t Win32ToDOSDateTime(const SWin32Time &time)
{
  ...
  tm tmTime;
  Zero(tmTime);
  tmTime.tm_year = static_cast<int>(time.year) + 80;
  tmTime.tm_mon = static_cast<int>(time.month) - 1;
  tmTime.tm_mday = static_cast<int>(time.day);
  tmTime.tm_hour = static_cast<int>(time.hours);
  tmTime.tm_min = static_cast<int>(time.minutes);
  tmTime.tm_sec = static_cast<int>(time.seconds) * 2;
