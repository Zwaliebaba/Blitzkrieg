...
inline std::string GetFullPath(const std::string &szPath)
{
  ...
  if (!szResult.empty() && szResult[szResult.size() - 1] != '\\') szResult += '\\';
  return szResult;
}

inline SWin32Time MakeWin32Time(const FILETIME &filetime) { return filetime.dwLowDateTime != 0 ? FILETIMEToWin32DateTime(filetime) : 0; }
// ************************************************************************************************************************ //
// **
...
CFileSystemEnumerator::~CFileSystemEnumerator() { Close(); }

void CFileSystemEnumerator::Close()
{
  if (IsFindValid()) FindClose(hFind);
  hFind = INVALID_HANDLE_VALUE;
}

void CFileSystemEnumerator::FillStats()
{
  if (!IsFindValid()) Zero(stats);
  else
  {
...
bool CFileSystemEnumerator::FindFirstFile()
{
  ...
  if (pos == std::string::npos) szPath.clear();
  else
  {
...
{
  if (!IsFindValid()) return false;
  if (::FindNextFile(hFind, &findinfo) == 0)
  {
    Close();
    return false;
  }
  if (IsDots()) return FindNextFile();
  return true;
}
...
bool CFileSystemEnumerator::Next()
{
  if (!IsFindValid())
  {
    if (!FindFirstFile()) return false;
    if (IsDots()) { if (!FindNextFile()) Close(); }
  }
  else { if (!FindNextFile()) Close(); }
  FillStats();
  return IsFindValid();
}
  ...
  : nStreamBegin(0)
{
  ...
  hFile = ::CreateFile(pszFileName, dwDesiredAccess, dwShareMode, nullptr, dwCreationDesposition, dwFlagsAndAttributes, nullptr);
  ...
  if (hFile != INVALID_HANDLE_VALUE)
  {
    ...
    stats.nSize = GetFileSize(hFile, nullptr);
...
CFileStream::~CFileStream() { if (hFile != INVALID_HANDLE_VALUE) CloseHandle(hFile); }

inline int GetCurrPos(HANDLE hFile) { return SetFilePointer(hFile, 0, nullptr, FILE_CURRENT); }
...
int CFileStream::GetPos() const { return GetCurrPos(hFile) - nStreamBegin; }
...
int CFileStream::Seek(int offset, STREAM_SEEK from)
{
  ...
  if (nSeek == FILE_BEGIN) SetFilePointer(hFile, nStreamBegin + offset, nullptr, nSeek);
  else SetFilePointer(hFile, offset, nullptr, nSeek);
  return GetPos();
}
...
int CFileStream::Read(void *pBuffer, int nLength)
{
  if (!CanRead()) return 0;
  DWORD dwRead = 0;
  return ReadFile(hFile, pBuffer, nLength, &dwRead, nullptr) != FALSE ? dwRead : 0;
}
...
int CFileStream::Write(const void *pBuffer, int nLength)
{
  ...
  DWORD dwWritten = 0;
  return WriteFile(hFile, pBuffer, nLength, &dwWritten, nullptr) != FALSE ? dwWritten : 0;
}

int CFileStream::GetSize() const
{
  const int nLength = IsOpen() ? GetFileSize(hFile, nullptr) : 0;
  return nLength - nStreamBegin;
}

bool CFileStream::SetSize(int nSize) { return false; }
...
int CFileStream::CopyTo(IDataStream *pDstStream, int nLength)
{
  ...
  while (nLength >= nBufferSize)
  {
    ...
    nLength -= nReaded;
    if (nReaded != nBufferSize) return nTotalWrote;
  }
  // copy remains...
  if (nLength)
  {
...
void CFileStream::Flush() { FlushFileBuffers(hFile); }
  ...
  : szBase(pszName), dwStorageAccessMode(dwAccessMode)
{
  ...
  if (bCreate) CreatePathRecursive(szBase);
  // create absolute path from the relative one
...
bool CFileSystem::DestroyElement(const char *pszName) { return DeleteFile(pszName) != 0; }

// rename element
bool CFileSystem::RenameElement(const char *pszOldName, const char *pszNewName) { return MoveFile(pszOldName, pszNewName) != 0; }

// enumeration of elements
IStorageEnumerator *CFileSystem::CreateEnumerator() { return new CFileSystemEnumerator(szBase); }
