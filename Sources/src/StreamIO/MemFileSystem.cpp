...
int CMemFileStream::Read(void *pBuffer, int nLength)
{
  ...
  nLength = Min(nLength, static_cast<int>(data.size() - nCurrPos));
  if (nLength > 0) memcpy(pBuffer, &(data[nCurrPos]), nLength);
  nCurrPos += nLength;
  return nLength;
}
...
int CMemFileStream::Write(const void *pBuffer, int nLength)
{
  ...
  if (nLength > 0) memcpy(&(data[nCurrPos]), pBuffer, nLength);
  nCurrPos += nLength;
  return nLength;
}
...
int CMemFileStream::GetPos() const { return nCurrPos - nBeginPos; }
...
// get stream size
int CMemFileStream::GetSize() const { return data.size() - nBeginPos; }

// change stream size
bool CMemFileStream::SetSize(int nSize)
{
  data.resize(nBeginPos + nSize);
  if (nCurrPos > nBeginPos + nSize) nCurrPos = nBeginPos + nSize;
  return true;
}
...
int CMemFileStream::operator&(IStructureSaver &ss)
{
  ...
  saver.Add(5, &szName);
  saver.Add(6, &stats);
  if (saver.IsReading()) stats.pszName = szName.c_str();
  return 0;
}
...
{
  CStreamsMap::iterator pos = streams.find(pszName);
  if (pos == streams.end()) return false;
...
IStorageEnumerator *CMemFileSystem::CreateEnumerator() { return nullptr; }
...
const bool CMemFileSystem::IsStreamExist(const char *pszName) { return streams.find(pszName) != streams.end(); }