...
{
  for (auto it = filesystems.begin(); it != filesystems.end(); ++it) { if (IDataStream *pStream = it->second->OpenStream(pszName, dwAccessMode)) return pStream; }
  return nullptr;
}
...
{
  for (auto it = filesystems.begin(); it != filesystems.end(); ++it) { if (it->second->GetStreamStats(pszName, pStats)) return true; }
  return false;
}
...
{
  for (CFileSystemsList::const_iterator it = filesystems.begin(); it != filesystems.end(); ++it) { if (it->second->IsStreamExist(pszName)) return true; }
  return false;
}

const char *CModFileSystem::GetName() const { return filesystems.empty() ? ".\\" : filesystems.back().second->GetName(); }

// enumeration of elements
IStorageEnumerator *CModFileSystem::CreateEnumerator()
{
  if (filesystems.size() == 0) return filesystems.back().second->CreateEnumerator();
  auto pModEnum = new CModFileSystemEnumerator();
