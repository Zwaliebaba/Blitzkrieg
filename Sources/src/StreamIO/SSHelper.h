...
template<class T>
char operator&(T &c, IStructureSaver &ss) { return 0; }
...
class CSaverAccessor
{
  ...
  template<class T>
  void __cdecl CallObjectSerialize(const SSChunkID idChunk, T *pData, SGenericNumber<1> *pp) { pSS->DataChunk(idChunk, pData, sizeof(T)); }
  ...
  void __cdecl AddInternal(const SSChunkID idChunk, T1 *p, std::basic_string<T2> *pData)
  {
    ...
    Add(1, &nSize);
    if (IsReading()) pData->resize(nSize);
  ...
  {
    if (!pSS->StartChunk(idChunk)) return;
    if (sizeof(TestDataPath(&(*pData)[0])) == 1 && sizeof((*pData)[0] & (*pSS)) == 1) DoDataVector(*pData);
    else DoVector(*pData);
    pSS->FinishChunk();
  }
  ...
  {
    if (!pSS->StartChunk(idChunk)) return;
    if (sizeof(TestDataPath(&(*pData)[0][0])) == 1 && sizeof((*pData)[0][0] & (*pSS)) == 1) Do2DArrayData(*pData);
    else Do2DArray(*pData);
    pSS->FinishChunk();
  }
  ...
  {
    if (!pSS->StartChunk(idChunk)) return;
    if (IsReading()) *pData = CastToUserObject(pSS->LoadObject(), static_cast<T1 *>(nullptr));
    else pSS->StoreObject(CastToRefCount(pData->GetPtr()));
    pSS->FinishChunk();
  }
  ...
  void Do2DArray(CArray2D<T> &a)
  {
    ...
    Add(1, &nSizeX);
    Add(2, &nSizeY);
    if (IsReading()) a.SetSizes(nSizeX, nSizeY);
  ...
  void Do2DArrayData(CArray2D<T> &a)
  {
    ...
    Add(2, &nSizeY);
    if (IsReading()) a.SetSizes(nSizeX, nSizeY);
    if (nSizeX * nSizeY > 0) AddRawData(3, &a[0][0], sizeof(T) * nSizeX * nSizeY);
  }
...
public:
  ...
  const CSaverAccessor &operator=(IStructureSaver *_pSS)
  {
    pSS = _pSS;
    return *this;
  }

  const CSaverAccessor &operator=(const CSaverAccessor &accessor)
  {
    pSS = accessor.pSS;
    return *this;
  }
