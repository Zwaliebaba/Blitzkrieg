...
CZipFileSystemEnumerator::CZipFileSystemEnumerator(const CZipFilesList &_zipfiles, IDataStorage *_pStorage)
  : pStorage(_pStorage), zipfiles(_zipfiles), posZipFile(_zipfiles.begin()), nFileInZip(0) { Zero(stats); }
...
{
  if (posZipFile == zipfiles.end()) return false;
  ++nFileInZip;
  if (nFileInZip < posZipFile->GetNumFiles()) return true;
  while (true)
  {
    ++posZipFile;
    if (posZipFile == zipfiles.end()) return false;
...
{
  if (NextEntry() == false) return false;
  while (posZipFile->IsDirectory(nFileInZip)) { if (NextEntry() == false) return false; }
  //
  ...
  if (zipfile.IsOk())
  {
    ...
    for (int i = 0; i < zipfile.GetNumFiles(); ++i)
    {
      if (!zipfile.IsDirectory(i))
      {
        ...
        if ((info.pZipFile == nullptr) || (zipfile.GetModDateTime(i) > info.pZipFile->GetModDateTime(info.nIndex)))
        {
          info.nIndex = i;
...
IStorageEnumerator *CZipFileSystem::CreateEnumerator() { return new CZipFileSystemEnumerator(zipfiles, this); }
