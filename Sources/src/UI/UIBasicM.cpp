...
CUIWindowManipulator::CUIWindowManipulator()
  : CManipulator(&thePropertiesRegister, "UIWindow"), pWindow(nullptr)
{
  BEGIN_PROPERTIES_MAP(CUIWindow);
    ...
    AddNewProperty(this, "States", static_cast<CUIWindowProperty::SETFUNCTION>(nullptr), &CUIWindowManipulator::GetTexture, SPropertyDesc::VAL_BROWSEFILE, SBaseProperty::VECTOR);
...
void CUIWindowManipulator::SetPosX(const variant_t &value) { pWindow->vPos.x = value.intVal; }

void CUIWindowManipulator::SetPosY(const variant_t &value) { pWindow->vPos.y = value.intVal; }
...
void CUIWindowManipulator::SetSizeX(const variant_t &value)
{
  ...
  for (int i = 0; i < pWindow->states.size(); i++)
  {
    for (int k = 0; k < 4; k++)
    {
      ...
      for (int a = 0; a < subState.subRects.size(); a++) { subState.subRects[a].rc.right = pWindow->vSize.x; }
    }
  }
}
...
void CUIWindowManipulator::SetSizeY(const variant_t &value)
{
  ...
  for (int i = 0; i < pWindow->states.size(); i++)
  {
    ...
    {
      CUIWindowSubState &subState = pWindow->states[i].subStates[k];
      for (int a = 0; a < subState.subRects.size(); a++) { subState.subRects[a].rc.bottom = pWindow->vSize.y; }
    }
  }
}

void CUIWindowManipulator::SetWindowID(const variant_t &value) { pWindow->nID = value.intVal; }

void CUIWindowManipulator::SetVisibleStatus(const variant_t &value) { pWindow->nCmdShow = value.intVal; }

void CUIWindowManipulator::SetHighSound(const variant_t &value)
{
  bstr_t bstrVal = value.bstrVal;
  std::string szName = bstrVal;
  if (szName.size() == 0) return;
