...
void CUIComplexScroll::GetBorderRect(CTRect<float> *pBorderRect) const
{
  ...
  if (pScrollBar->IsVisible()) pBorderRect->right -= pScrollBar->GetWidth();
}

void CUIComplexScroll::Draw(IGFX *pGFX) { NI_ASSERT_T(false, "wrong call"); }

void CUIComplexScroll::Visit(interface ISceneVisitor *pVisitor)
{
  if (!GetCmdShow()) return;

  // this function is actually duplicated for some controls, so when you change it you need to change them, for example CUIComplexScroll
  VisitBackground(pVisitor);

  CWindowList &children = GetChildList();
  // draw children
  for (auto ri = children.rbegin(); ri != children.rend(); ++ri) (*ri)->Visit(pVisitor);
...
void CUIComplexScroll::UpdatePosition()
{
  ...
  pItemContainer->GetWindowPlacement(&vPos, nullptr, nullptr);
  vPos.y = m_nY + nTopSpace;
  pItemContainer->SetWindowPlacement(&vPos, nullptr);
...
void CUIComplexScroll::AddItem(IUIElement *pElement, const bool bResizeToFitText)
{
  ...
  if (bResizeToFitText)
  {
    ...
    pElement->SetWindowPlacement(nullptr, &vSize);
    ...
    pElement->SetWindowPlacement(nullptr, &vNewSize);
  }
  ...
  CVec2 vElementSize;
  pElement->GetWindowPlacement(nullptr, &vElementSize, nullptr);

  // resize container to fit new element
  CVec2 vContainerSize;
  pItemContainer->GetWindowPlacement(nullptr, &vContainerSize, nullptr);
