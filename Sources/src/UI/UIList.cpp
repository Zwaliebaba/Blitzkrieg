...
CUIList::CUIList() : pScrollBar(nullptr), nLeftSpace(10), nTopSpace(5), nHeaderTopSpace(0), nItemHeight(30),
                     nHSubSpace(2), nVSubSpace(2), bLeftScrollBar(false), bScrollBarAlwaysVisible(true),
                     nHeaderSize(0), nScrollBarWidth(30), nSelection(-1), nSortedHeaderIndex(-1), bSortAscending(false)
{
  SetMouseWheelMultiplyer(25.0f / 4.8f);// selection method
}
...
void CUIList::InitItemHeight()
{
  ...
  for (int i = 0; i < columnProperties.size(); i++)
  {
    ...
    if (vSize.y > nItemHeight) nItemHeight = vSize.y;
...
{
  auto pRow = new SUIListRow;
  listItems.push_back(pRow);
  pRow->subItems.resize(columnProperties.size());
  pRow->SetUserData(nData);
  for (int i = 0; i < columnProperties.size(); i++) { pRow->subItems[i] = CreateComponent(columnProperties[i].szFileName.c_str()); }

  /* doing this is bad, because every time you add an item, the entire list will be resorted
     */
}
...
void CUIList::RemoveItem(int nIndex)
{
  ...
  if (nIndex <= nSelection) { nSelection--; }

  // remove item from list control
  auto it = listItems.begin() + nIndex;
  for (int i = 0; i < (*it)->subItems.size(); i++) { RemoveChild((*it)->subItems[i]); }
  listItems.erase(it);
...
{
  for (int i = 0; i < listItems.size(); i++) { if (listItems[i]->nUserData == nID) return i; }
...
void CUIList::InitialUpdate()
{
  ...
  {
    bSortAscending = !bSortAscending;// after calling Sort() the variable¤ is inverted¤
...
{
  // *we recalculate coordinates for all internal controls
  int nY = -pScrollBar->GetPosition();
  CTRect<float> rect;
  GetWindowPlacement(nullptr, nullptr, &rect);
  CTRect<float> boundRc = rect;
  if (nHeaderSize > 0) boundRc.y1 += nHeaderSize + nHeaderTopSpace;
  else boundRc.y1 += nTopSpace;
  boundRc.y2 -= nTopSpace;
  CVec2 vElementPos;
  CVec2 vElementSize;

  for (auto item = listItems.begin(); item != listItems.end(); ++item)
  {
    if (nY + nItemHeight - nVSubSpace < 0 || nY + nVSubSpace > rect.Height() - 2 * nTopSpace)
    {
      // item not visible
      for (auto it = (*item)->subItems.begin(); it != (*item)->subItems.end(); ++it) { (*it)->ShowWindow(UI_SW_HIDE); }
    }
    else
    {
      // item visible
      int left = nLeftSpace;
      if (bLeftScrollBar && pScrollBar && pScrollBar->IsVisible()) left += nScrollBarWidth;
      int nActiveRaw = 0;
      for (auto it = (*item)->subItems.begin(); it != (*item)->subItems.end(); ++it)
      {
  ...
  CTRect<float> rc;
  GetParent()->GetWindowPlacement(nullptr, nullptr, &rc);
  Reposition(rc);
}
...
void CUIList::UpdateScrollBarStatus()
{
  ...
  if (nMaxSBValue < 0) nMaxSBValue = 0;
  pScrollBar->SetMaxValue(nMaxSBValue);

  if (!bScrollBarAlwaysVisible)
  {
    if (nMaxSBValue > 0) pScrollBar->ShowWindow(UI_SW_SHOW);
    else pScrollBar->ShowWindow(UI_SW_HIDE);
  }
}
...
bool CUIList::OnChar(int nAsciiCode, int nVirtualKey, bool bPressed, DWORD keyState)
{
  ...
  switch (nVirtualKey)
  {
    case VK_UP:
      // case VK_LEFT:
      if (pFocused.GetPtr() == pScrollBar.GetPtr()) return pScrollBar->OnChar(nAsciiCode, nVirtualKey, bPressed, keyState);
      {
        if (nSelection > 0)
        {
          RemoveFocusFromItem(nSelection);
...
bool CUIList::ProcessMessage(const SUIMessage &msg)
{
  ...
  switch (msg.nMessageCode)
  {
    case UI_NOTIFY_POSITION_CHANGED:
      if (msg.nFirst != 1 && msg.nFirst != 3) return false;

      UpdateItemsCoordinates();
      return true;

    case MESSAGE_KEY_UP:
      if (pFocused.GetPtr() == pScrollBar.GetPtr()) return pScrollBar->ProcessMessage(msg);
      {
        if (nSelection > 0)
        {
          RemoveFocusFromItem(nSelection);
    ...
    case UI_NOTIFY_STATE_CHANGED_MESSAGE:
      ...
      {
        // clicked on one of the headings, let's try to sort this column
        ...
        msg.nFirst = GetWindowID();
        msg.nSecond = (nColumn << 8) | static_cast<int>(bSortAscending);
        GetParent()->ProcessMessage(msg);
        return true;
      }
  }
    ...
    if (nY + nItemHeight + nTopSpace + nHeaderSize + nHeaderTopSpace > 0 && nY < rect.Height() - nTopSpace - nHeaderSize - nHeaderTopSpace)
    {
      ...
      if (!selSubRects.empty())
      {
        ...
        for (int i = 0; i < nSize; i++)
        {
          ...
          int nAdd = 0;
          if (nHeaderSize > 0) nAdd = nHeaderSize + nHeaderTopSpace;
          else nAdd = nTopSpace;
...
bool CUIList::OnLButtonDown(const CVec2 &vPos, EMouseState mouseState)
{
  ...
  // now fSelIndex contains the mouse offset relative to the beginning of items
  // if fSelIndex < 0, then it was clicked outside of visible items, for example in the header area
  if (fSelIndex < 0) return true;

  fSelIndex = (fSelIndex + pScrollBar->GetPosition() - nTopSpace) / nItemHeight;
  if (fSelIndex < 0 || fSelIndex >= listItems.size()) return true;

  // now fSelIndex is the index of the item under the mouse
  if (nSelection == static_cast<int>(fSelIndex)) return true;// the previous visible item has the same index, nothing has changed
...
{
  if (pScrollBar->IsInside(vPos)) return CMultipleWindow::OnLButtonDblClk(vPos);

  bool bRet = CMultipleWindow::OnLButtonDblClk(vPos);
  if (!bRet) return bRet;// mouse outside the window

  // double click inside the list is processed here
  CTRect<float> rect = GetScreenRect();
  float fSelIndex = vPos.y - rect.y1 - nHeaderSize - nHeaderTopSpace;
  // now fSelIndex contains the mouse offset relative to the beginning of items
  // if fSelIndex < 0, then it was clicked outside of visible items, for example in the header area
  ...
  // now fSelIndex is the index of the item under the mouse
  // send a message about double click
...
{
  // sort the items
  if (nSortedHeaderIndex == -1) return false;// we don't know which column to sort by
...
bool CUIList::Sort(int nColumn, const int nSortType)
{
  // -1 - descending
  // 1 - ascending
  // 0 - doesn't matter (inner determination)

  NI_ASSERT_T(nColumn < columnProperties.size(), "Invalid column index in CUIList::Sort()");
  if (!headers.subItems[nColumn].pSorter) return false;

  // Before sorting, save the currently selected element
  SUIListRow *pSelectedRow = nullptr;
  if (nSelection < listItems.size() && nSelection >= 0) pSelectedRow = listItems[nSelection].GetPtr();

  if (nSortedHeaderIndex != nColumn)
  {
    // reset the selection from nSortedHeaderIndex
    if (nSortedHeaderIndex != -1) headers.subItems[nSortedHeaderIndex].pElement->SetState(0, false);
...
{
  if (nIndex < 0 || nIndex >= listItems.size()) return;

  auto it = listItems.begin() + nIndex;
  for (int i = 0; i < (*it)->subItems.size(); i++) { (*it)->subItems[i]->SetFocus(false); }
}

void CUIList::MoveSelectionItemUp()
{
  if (nSelection < 0 || nSelection >= listItems.size()) return;

  auto it = listItems.begin() + nSelection;
  for (int i = 0; i < (*it)->subItems.size(); i++) { MoveWindowUp((*it)->subItems[i]); }
}
