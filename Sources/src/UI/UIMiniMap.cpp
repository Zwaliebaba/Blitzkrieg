...
void CUIMiniMap::CreateMiniMapTextures()
{
  ...
  if (IsInitialized())
  {
    ...
    if (pWarFog)
    {
      // initialize the pWarFog texture
      {
        ...
        for (int nYIndex = 0; nYIndex < pWarFog->GetSizeY(0); ++nYIndex) { MemSetDWord(reinterpret_cast<DWORD *>(textureLock[nYIndex]), dwValue, nNumElements / 2); }
        isWarFogNeedUpdate = true;
      }
    }
    if (pInstantObjects)
    {
      // initialize the pInstantObjects texture
      {
        CTextureLock<SGFXColor4444> textureLock(pInstantObjects, 0);
        int nBytesCount = pInstantObjects->GetSizeX(0) * sizeof(WORD);
        for (int nYIndex = 0; nYIndex < pInstantObjects->GetSizeY(0); ++nYIndex) { memset(textureLock[nYIndex], 0x00, nBytesCount); }
        isInstantObjectsNeedUpdate = true;
      }
    }

    // Updating the pWarFog texture in video memory
    ...
    // Update the pInstantObjects texture in video memory
...
int CUIMiniMap::operator&(IStructureSaver &ss)
{
  ...
  // not needed, set in CreateMiniMapTextures()
  ...
  if (saver.IsReading()) { CreateMiniMapTextures(); }
  return 0;
}
...
void CUIMiniMap::GetZeroPoint(float *pfXZeroPoint, float *pfYZeroPoint, bool isTopLeft)
{
  ...
  if (terrainSize.x > terrainSize.y)
  {
    *pfXZeroPoint = (nSize - ((nSize * terrainSize.y) / (terrainSize.x * 1.0f))) / 4.0f;
    if (isTopLeft) { *pfYZeroPoint = nSize / 4.0f - (nSize - ((nSize * terrainSize.y) / (terrainSize.x * 1.0f))) / 8.0f; }
    else { *pfYZeroPoint = (nSize - ((nSize * terrainSize.y) / (terrainSize.x * 1.0f))) / 8.0f + nSize / 4.0f; }
  }
  else if (terrainSize.x < terrainSize.y)
  {
    *pfXZeroPoint = (nSize - ((nSize * terrainSize.x) / (terrainSize.y * 1.0f))) / 4.0f;
    if (isTopLeft) { *pfYZeroPoint = (nSize - ((nSize * terrainSize.x) / (terrainSize.y * 1.0f))) / 8.0f + nSize / 4.0f; }
    else { *pfYZeroPoint = nSize / 4.0f - (nSize - ((nSize * terrainSize.x) / (terrainSize.y * 1.0))) / 8.0f; }
  }
  else
  {
    *pfXZeroPoint = 0.0f;
...
void CUIMiniMap::PointToTextureMiniMap(float fXPos, float fYPos, float *pfXMiniMapPos, float *pfYMiniMapPos, bool isLeftTop)
{
  ...
  *pfXMiniMapPos = fXLocalPos + fYLocalPos + zeroPoint.x;
  if (isLeftTop) { *pfYMiniMapPos = (fXLocalPos - fYLocalPos) / 2.0f + zeroPoint.y; }
  else { *pfYMiniMapPos = (fYLocalPos - fXLocalPos) / 2.0f + zeroPoint.y; }
}
...
void CUIMiniMap::GetVerticalClippedScreenEdge(const CTPoint<float> &v0, const CTPoint<float> &v1, std::vector<CTPoint<float>> *pvPoints)
{
  ...
  // consider vertex 0 and edge 0 - 1
  if (InMiniMap(v0.x, v0.y))
  {
    if (InMiniMap(v1.x, v1.y))
    {
      // add edge 0 - 1 without changes
      // both points inside
    ...
    else
    {
      ...
      if (fXbyY0_0_1 <= fXMax)
      {
        // add edge 0 - 1 with cutting along edge (0,0)-(fXMax, 0)
        // point 0 inside, point 1 outside on the side of point (0,0)
        pvPoints->push_back(CTPoint<float>(v0.x, v0.y));
      ...
      else
      {
        ...
        // add edge 0 - 1 with cutting along edge (fXMax,0)-(fXMax,fYMax)
        // point 0 inside, point 1 outside on the point side (fXMax,fYMax)
        pvPoints->push_back(CTPoint<float>(v0.x, v0.y));
  ...
  else
  {
    ...
    if (InMiniMap(v1.x, v1.y))
    {
      if (fYbyX0_0_1 <= fYMax)
      {
        // add edge 0 - 1 with cutting along edge (0,0)-(0, fYMax)
        // point 0 outside, point 1 inside on the side of point (0,0)
      ...
      else
      {
        ...
        // add edge 0 - 1 with cutting along edge (0,fYMax)-(fXMax, fYMax)
        // point 0 outside, point 1 inside from point side (fXMax, fYMax)
    ...
    else
    {
      if ((fYbyX0_0_1 >= 0) && (fYbyX0_0_1 <= fYMax))
      {
        ...
        {
          float fXbyY0_0_1 = GetXByY(0.0f, v0.x, v0.y, v1.x, v1.y);
          // add edge 0 - 1 with cutting along edges (0,0)-(0, fYMax) and (0,0)-(fXMax, 0)
          // point 0 is outside, point 1 is outside on the side of point (0, 0)
          pvPoints->push_back(CTPoint<float>(0, fYbyX0_0_1));
          pvPoints->push_back(CTPoint<float>(fXbyY0_0_1, 0));
        }
        else
      ...
      else
      {
        ...
        if ((fYbyXmax_0_1 >= 0) && (fYbyXmax_0_1 <= fYMax))
        {
          ...
          {
            float fXbyYmax_0_1 = GetXByY(fYMax, v0.x, v0.y, v1.x, v1.y);
            // add edge 0 - 1 with cutting along edges (0,fYmax)-(fXMax, fYMax) and (fXmax,0)-(fXMax, fYMax)
            // point 0 outside, point 1 outside from point side (fXMax, fYMax)
            pvPoints->push_back(CTPoint<float>(fXbyYmax_0_1, fYMax));
            pvPoints->push_back(CTPoint<float>(fXMax, fYbyXmax_0_1));
          }
          else
...
void CUIMiniMap::GetHorizontalClippedScreenEdge(const CTPoint<float> &v1, const CTPoint<float> &v2, std::vector<CTPoint<float>> *pvPoints)
{
  ...
  if (InMiniMap(v1.x, v1.y))
  {
    ...
    else
    {
      ...
      {
        // add edge 1 - 2 with cutting along the edge (fXMax,0)-(fXMax, fYmax)
        // point 1 inside, point 2 outside from point side (fXMax,0)
        pvPoints->push_back(CTPoint<float>(v1.x, v1.y));
      ...
      else
      {
        ...
        // add edge 1 - 2 with cutting along the edge (0,fYMax)-(fXMax,fYMax)
        // point 1 inside, point 2 outside on the side of point (0,fYMax)
        pvPoints->push_back(CTPoint<float>(v1.x, v1.y));
  ...
  else
  {
    ...
    if (InMiniMap(v2.x, v2.y))
    {
      if (fXbyY0_1_2 >= 0)
      {
        // add edge 1 - 2 with cutting along edge (0,0)-(0, fYMax)
        // point 1 outside, point 2 inside from point side (fXMax,0)
      ...
      else
      {
        ...
        // add edge 1 - 2 with cutting along the edge (0,0-(0, fYMax)
        // point 1 outside, point 2 inside from point side (0, fYMax)
    ...
    else
    {
      if ((fXbyY0_1_2 >= 0) && (fXbyY0_1_2 <= fXMax))
      {
        ...
        {
          float fYbyXmax_1_2 = GetYByX(fXMax, v1.x, v1.y, v2.x, v2.y);
          // add edge 1 - 2 with cutting along edges (0,0)-(fXMax, 0) and (fXMax,0)-(fXMax, fYMax)
          // point 1 outside, point 2 outside from point side (fxMax, 0)
...
void CUIMiniMap::GetClippedScreenFrame(std::vector<CTPoint<float>> *pvPoints, IGFX *_pGFX)
{
  ...
  // get the world coordinates of the screen frame
...
void CUIMiniMap::SetTerrainSize(int nXTerrainSize, int nYTerrainSize, int _nPlayersCount)
{
  ...
  {
    const DWORD dwColor = static_cast<DWORD>(GetGlobalVar(NStr::Format("Scene.PlayerColors.Player%d", nPartyIndex), 0x00000000));
...
bool CUIMiniMap::AddWarFogData(const BYTE *pVizBuffer, int nLength)
{
  if (IsInitialized() && nLength != 0)
  {
    ...
    // add data from AI to the pWarFog texture in RAM
    if (nFiledVISTiles < (terrainSize.x * terrainSize.y))
    {
      ...
      // updating the pWarFog texture in RAM
    ...
    if (nFiledVISTiles >= (terrainSize.x * terrainSize.y))
    {
      // Sets the vis tile counter to zero
      isWarFogNeedUpdate = true;
      nFiledVISTiles = 0;
      return true;
    }
...
void CUIMiniMap::AddUnitsData(const SMiniMapUnitInfo *pUnitsBuffer, int nUnitsCount)
{
  if (IsInitialized())
  {
    if (nUnitsCount) { NI_ASSERT_SLOW_T(pUnitsBuffer != 0, NStr::Format( "Wrong parameter: (%x)", pUnitsBuffer )); }
...
void CUIMiniMap::AddFireRangeAreas(const SShootAreas *pShootAreasBuffer, int nShootAreasCount)
{
  if (IsInitialized())
  {
    if (nShootAreasCount > 0) { NI_ASSERT_SLOW_T(pShootAreasBuffer != 0, NStr::Format( "Wrong parameter: (%x)", pShootAreasBuffer )); }
    // updating firing lines
    shootAreas.clear();
    for (int index = 0; index < nShootAreasCount; ++index) shootAreas.push_back(pShootAreasBuffer[index]);
  }
}
...
void CUIMiniMap::AddCircle(const CVec2 &vCenter, const float fRadius, int nStyle, WORD wColor, const NTimer::STime &rStart, const NTimer::STime &rDuration, bool bRelative, LPARAM lParam)
{
  if (IsInitialized())
  {
    if (bRelative) { circles.push_back(SMiniMapCircle(CVec2(vCenter.x * terrainSize.x * fWorldCellSize, vCenter.y * terrainSize.y * fWorldCellSize), fRadius, rStart, rDuration, nStyle, wColor, lParam)); }
    else { circles.push_back(SMiniMapCircle(vCenter, fRadius, rStart, rDuration, nStyle, wColor, lParam)); }
  }
}
...
int STDCALL CUIMiniMap::AddMarker(const std::string &rszName, const CVec2 &vPos, bool _bActive, int _nID, const NTimer::STime &rStart, const NTimer::STime &rDuration, bool bRelative)
{
  ...
  else { markers.push_back(SMiniMapMarker(markersTypes[rszName], rszName, vPos, _bActive, _nID, rStart, rDuration)); }
  return _nID;
}
...
void STDCALL CUIMiniMap::RemoveMarker(int _nID)
{
  ...
  {
    if (markerIterator->nID == _nID) { markerIterator = markers.erase(markerIterator); }
    else { ++markerIterator; }
  }
}
...
void STDCALL CUIMiniMap::RemoveMarker(const std::string &rszName)
{
  for (auto markerIterator = markers.begin(); markerIterator != markers.end();)
  {
    if (markerIterator->szName == rszName) { markerIterator = markers.erase(markerIterator); }
    else { ++markerIterator; }
  }
}
...
void CUIMiniMap::DrawFireRanges(CTextureLock<SGFXColor4444> *pTextureLock)
{
  ...
  {
    for (auto iter = shootAreas[index].areas.begin(); iter != shootAreas[index].areas.end(); ++iter)
    {
      ...
      if (area.eType != SShootArea::ESAT_LINE)
      {
        ...
        if (area.wStartAngle != area.wFinishAngle)
        {
          // first corner
          {
            const float fAngle = fmod(static_cast<float>(area.wStartAngle) / 65535.0f * FP_2PI + FP_PI2, FP_2PI);
          ...
          // second corner
          {
            const float fAngle = fmod(static_cast<float>(area.wFinishAngle) / 65535.0f * FP_2PI + FP_PI2, FP_2PI);
            const float fCos = cos(fAngle);
            const float fSin = sin(fAngle);
...
bool CUIMiniMap::Update(const NTimer::STime &currTime)
{
  ...
  IGFX *_pGFX = GetSingleton<IGFX>();
  if (!_pGFX) { return false; }
  IGameTimer *_pGameTimer = GetSingleton<IGameTimer>();
  if (!_pGameTimer) { return false; }
  ...
  if (pInstantObjects)
  {
        ...
        if (units[nUnitIndex].player < nPlayersCount)
        {
          ...
          for (int index = (nXPos - nUnitCrossSize); index <= (nXPos + nUnitCrossSize); ++index)
          {
            if ((index >= 0) &&
                ...
                (nYPos >= 0) &&
                (nYPos < pInstantObjects->GetSizeY(0))) { textureLock[nYPos][index] = pPartyColors[units[nUnitIndex].player]; }
          }
          for (int index = (nYPos - nUnitCrossSize); index <= (nYPos + nUnitCrossSize); ++index)
          {
            if ((index >= 0) &&
                ...
                (nXPos >= 0) &&
                (nXPos < pInstantObjects->GetSizeX(0))) { textureLock[index][nXPos] = pPartyColors[units[nUnitIndex].player]; }
          }
        }
      }
    }

    // add firing lines if they exist
    ...
    // Circles
    {
      for (auto it = circles.begin(); it != circles.end();)
      {
        ...
        if ((it->timeStart + it->timeDuration) < currentAbsTime) { it = circles.erase(it); }
        else
        {
          float fRadius = 0.0f;
            ...
            if (fRadius > 0.0f)
            {
              ...
              if (wParts > 1 && wParts < 17)
              {
                ...
                if (wPeriod > 0)
                {
                  ...
                  if (it->nStyle < MMC_STYLE_RPOLYGON_DIVERGENT) { fAngle = fDelimiter * FP_2PI * fabs((currentAbsTime - it->timeStart) / (it->timeDuration * 1.0f)); }
                  else { fAngle = (-1.0f) * fDelimiter * FP_2PI * fabs((currentAbsTime - it->timeStart) / (it->timeDuration * 1.0f)); }
                }
                if (wParts == 4)
                {
  ...
  // Updating the pWarFogTexture texture in video memory
  if (pWarFog && pWarFogTexture && isWarFogNeedUpdate)
  {
    pWarFog->AddDirtyRect(&(static_cast<RECT>(CTRect<int>(0, 0, pWarFog->GetSizeX(0), pWarFog->GetSizeY(0)))));
    _pGFX->UpdateTexture(pWarFog, pWarFogTexture, false);
...
void CUIMiniMap::Visit(interface ISceneVisitor *pVisitor) { if (IsInitialized()) pVisitor->VisitUICustom(dynamic_cast<IUIElement *>(this)); }
...
void CUIMiniMap::Draw(IGFX *_pGFX)
{
  if (IsInitialized())
  {
    ...
    // set the coordinates of the vertices, the sequence of traversing the vertices in triangles
    ...
    // draw texture pBackgroundTexture and pWarFogTexture
    ...
    {
      // We obtain arrays of vertices and sequences of traversal of vertices in triangles
      ...
      // Filling the array of sequences of traversing vertices in triangles
      indices[0] = 0;
      indices[1] = 1;
      indices[2] = 2;
      indices[3] = 0;
    ...
    for (auto markerIterator = markers.begin(); markerIterator != markers.end();)
    {
      ...
      {
        if ((markerIterator->timeDuration > 0) && ((markerIterator->timeStart + markerIterator->timeDuration) < currentAbsTime)) { markerIterator = markers.erase(markerIterator); }
        else
        {
          if (markerIterator->bActive) { ++nActiveMarkersCount; }
          ++markerIterator;
        }
      }
      else { ++markerIterator; }
    ...
    /**/
    _pGFX->SetTexture(0, nullptr);

    // draw a frame
    // fill in information along the edges of the screen frame
    vPoints.clear();
    GetClippedScreenFrame(&vPoints, _pGFX);

    if (vPoints.size() > 0)
    {
      // Getting arrays of vertices
      CTempBufferLock<SGFXLineVertex> vertices = _pGFX->GetTempVertices(vPoints.size() * 2, SGFXLineVertex::format, GFXPT_LINELIST);

      // fill the vertices of the four sides of the screen frame
      for (int index = 0; index < vPoints.size(); ++index)
      {
        // get the screen coordinates of the top of the screen frame
        PointToTextureMiniMap(vPoints[index].x, vPoints[index].y, &(miniMapPoint.x), &(miniMapPoint.y));
...
{
  if (IsInitialized()) { if (mouseState & E_LBUTTONDOWN) { return OnLButtonDown(vPos, mouseState); } }
  return true;
}
...
bool CUIMiniMap::OnRButtonUp(const CVec2 &vPos, EMouseState mouseState)
{
  if (IsInitialized())
  {
    ...
    {
      // these are the coordinates obtained from the message parameter
      // (to send it you need to do it in the opposite direction)
      // CVec2( DWORD(msg.nParam) & 0x00007fff, (DWORD(msg.nParam) & 0x7fff0000) >> 15 );
      // to determine that the parameter contains coordinates, the most significant bit must be turned on
      // (msg.nParam |= 0x80000000) msg.nEventID = CMD_END_ACTION2;
      // sent as GetSingleton<IInput>()->AddMessage( SGameMessage(ID, PARAM) );
      // coordinates must be in screen coordinates on the plane (i.e. (0, 0) => in the upper left corner,
      // Y => down, X => right)
      DWORD dwParam = 0x80000000 +
                      (static_cast<DWORD>(mapMousePos.x * fWorldCellSize) & 0x00007fff) +
                      (((static_cast<DWORD>(mapMousePos.y * fWorldCellSize) & 0x00007fff) << 15) & 0x7fff0000);
