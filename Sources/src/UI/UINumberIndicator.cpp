...
void CUINumberIndicator::Visit(interface ISceneVisitor *pVisitor)
{
  ...
  if (valueColors.size() >= 2)
  {
    do
    {
      ...
      int i = 1;
      for (; i < valueColors.size(); i++) { if (m_fVal >= valueColors[i - 1].fVal && m_fVal <= valueColors[i].fVal) break; }

      NI_ASSERT_T(i != valueColors.size(), "Wrong color values");
      dwColor = 0xff000000;
      float fMul = (m_fVal - valueColors[i - 1].fVal) / (valueColors[i].fVal - valueColors[i - 1].fVal);
      int a = valueColors[i - 1].dwColor;
      int b = valueColors[i].dwColor;
      DWORD dwMask = 0x00ff0000;// red
      int nTemp = fMul * (static_cast<float>((b & dwMask) >> 16) - ((a & dwMask) >> 16)) + ((a & dwMask) >> 16);
      dwColor |= nTemp << 16;
      dwMask = 0x0000ff00;// green
      nTemp = fMul * (static_cast<float>((b & dwMask) >> 8) - ((a & dwMask) >> 8)) + ((a & dwMask) >> 8);
      dwColor |= nTemp << 8;
      dwMask = 0x000000ff;// blue
      nTemp = fMul * (static_cast<float>(b & dwMask) - (a & dwMask)) + (a & dwMask);
      dwColor |= nTemp;
    } while (false);
  }
  //
...
void CUINumberIndicator::SetValue(float fVal)
{
  ...
  if (fVal < valueColors.front().fVal) m_fVal = valueColors.front().fVal;
  else if (fVal > valueColors.back().fVal) m_fVal = valueColors.back().fVal;
  else m_fVal = fVal;
}