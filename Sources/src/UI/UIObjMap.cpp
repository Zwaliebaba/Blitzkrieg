...
void CUIObjMap::Init()
{
  ...
  vertices.resize(4);
  {
    ...
    const DWORD dwColor = GetGlobalVar("Scene.Colors.ObjMap.InterMission.Color", static_cast<int>(0xffffffff));
...
void CUIObjMap::Draw(IGFX *pGFX)
{
  ...
  if (pMapTexture)
  {
    ...
    if (!vertices.empty() && !indices.empty()) DrawTemp(pGFX, vertices, indices);
  }
}