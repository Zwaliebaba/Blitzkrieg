...
static constexpr int GLOBAL_CONSOLE_ID = 0xAC07A918;

static constexpr int ACKS_VERTICAL_POSITION = 30;// Message appearance position, counting from the top of the screen
static constexpr int TEXT_VERTICAL_SIZE = 20;// Vertical font size
static constexpr int TEXT_TWO = 20;// Size >> horizontal
static constexpr int TEXT_LEFT_SPACE = 0;// Indentation from the left edge of the screen to the text in the acknowledgement
static constexpr int CHAT_MESSAGE_LEFT = 0;// Indent from the left edge of the screen to the text in the chat message
static constexpr int CHAT_MESSAGE_TOP = 0;// Indent from the top edge of the screen to the text in a chat message
// constants loaded from a file
static int TEXT_ANIMATION_TIME = 5000;// Time to display text message, then it disappears
...
int CUIScreen::operator&(interface IStructureSaver &ss)
{
  ...
  if (saver.IsReading()) { pMouseWheelSlider = GetSingleton<IInput>()->CreateSlider("mouse_wheel"); }
  return 0;
}
...
int CUIScreen::Load(const char *pszResourceName, bool bRelative)
{
  ...
  else pStream = OpenFileStream(pszResourceName, STREAM_ACCESS_READ);
  if (pStream)
  {
    // mouse wheel slider
    ...
    return pElement == nullptr ? 0 : pElement->GetWindowID();
  }
  else
  {
...
{
  // Yurik moved the check to Input
  return CMultipleWindow::OnLButtonDblClk(vPos);
}

bool CUIScreen::OnMouseMove(const CVec2 &vPos, EMouseState mouseState) { return CMultipleWindow::OnMouseMove(vPos, static_cast<EMouseState>(m_mouseState)); }
...
bool CUIScreen::OnLButtonDown(const CVec2 &vPos, EMouseState mouseState)
{
  ...
  // here you need to save the state
  m_mouseState |= E_LBUTTONDOWN;
  return CMultipleWindow::OnLButtonDown(vPos, static_cast<EMouseState>(m_mouseState));
}
...
bool CUIScreen::OnLButtonUp(const CVec2 &vPos, EMouseState mouseState)
{
  // here you need to save the state
  m_mouseState &= ~E_LBUTTONDOWN;
  return CMultipleWindow::OnLButtonUp(vPos, static_cast<EMouseState>(m_mouseState));
}
...
bool CUIScreen::OnRButtonDown(const CVec2 &vPos, EMouseState mouseState)
{
  ...
  return CMultipleWindow::OnRButtonDown(vPos, static_cast<EMouseState>(m_mouseState));
}
...
bool CUIScreen::OnRButtonUp(const CVec2 &vPos, EMouseState mouseState)
{
  // here you need to save the state
  m_mouseState &= ~E_RBUTTONDOWN;
  return CMultipleWindow::OnRButtonUp(vPos, static_cast<EMouseState>(m_mouseState));
}
...
bool CUIScreen::OnChar(int nAsciiCode, int nVirtualKey, bool bPressed, DWORD keyState)
{
  ...
  {
    if (bPressed) m_keyboardState |= E_SHIFT_KEY_DOWN;
    else if (m_keyboardState & E_SHIFT_KEY_DOWN) m_keyboardState ^= E_SHIFT_KEY_DOWN;
  }
  else if (nVirtualKey == VK_MENU)
  {
    if (bPressed) m_keyboardState |= E_ALT_KEY_DOWN;
    else if (m_keyboardState & E_ALT_KEY_DOWN) m_keyboardState ^= E_ALT_KEY_DOWN;
  }
  else if (nVirtualKey == VK_CONTROL)
  {
    if (bPressed) m_keyboardState |= E_CTRL_KEY_DOWN;
    else if (m_keyboardState & E_CTRL_KEY_DOWN) m_keyboardState ^= E_CTRL_KEY_DOWN;
  }

  // if ~ is pressed without flags, then I hide the console
  // 192 is a virtual code ~
  ...
  if (bChatMode && bPressed) UpdateChatString(nAsciiCode, nVirtualKey, bPressed);
...
bool CUIScreen::ProcessMessage(const SUIMessage &msg)
{
  ...
  if (/* !GetModalFlag() && */ msg.nMessageCode == MC_SHOW_CONSOLE)
  {
    ...
    if (pConsole)
    {
      ...
      else pConsole->ShowWindow(UI_SW_HIDE);
    }
  ...
  {
    if (IsNotifyMessage(msg)) return false;
    uiMessageList.push_back(msg);
  }
  return true;
}
...
void CUIScreen::Visit(interface ISceneVisitor *pVisitor)
{
  ...
  // 
  auto pConsole = checked_cast<IUIConsole *>(GetChildByID(GLOBAL_CONSOLE_ID));
  if (!pConsole || (!pConsole->IsVisible() && !pConsole->IsAnimationStage())) pVisitor->VisitUICustom(dynamic_cast<IUIElement *>(this));
}
...
void CUIScreen::Draw(interface IGFX *pGFX)
{
  ...
  if (!pConsole || (!pConsole->IsVisible() && !pConsole->IsAnimationStage()))
  {
    ...
    // if there is no console, or there is a console, but it is invisible
    const CTRect<float> &rc = GetScreenRect();
    int nCurrentY = rc.y1 + ACKS_VERTICAL_POSITION;
    for (auto it = listOfAcks.begin(); it != listOfAcks.end(); ++it)
    {
    ...
    if (bChatMode)
    {
      ...
      if (pToWho) szResult = pToWho->GetString();
...
bool CUIScreen::Update(const NTimer::STime &currTime)
{
  ...
  DWORD color = GetGlobalVar("Scene.Colors.Summer.Text.Chat.Color", static_cast<int>(0xffd8bd3e));
  ...
  //
  for (auto it = listOfAcks.begin(); it != listOfAcks.end();)
  {
    if (it->time + TEXT_ANIMATION_TIME < currTime) it = listOfAcks.erase(it);
    else ++it;
  }
  ...
  if (pMouseWheelSlider && pScene->GetUIScreen() == static_cast<CUIScreenBridge *>(this))
  {
    ...
    if (fVal) { CMultipleWindow::OnMouseWheel(GetSingleton<ICursor>()->GetPos(), static_cast<EMouseState>(m_mouseState), fVal); }
  }
  ...
  switch (nVirtualKey)
  {
    case VK_RETURN:
    {
      if (nNumChatDublicates < 2 || szChatMessage != szLastChatMessage)
      {
        ...
        else ++nNumChatDublicates;
        if (bMessagesToEveryone)
        {
          const DWORD dwChatColor = GetGlobalVar("Scene.Colors.Summer.Text.Chat.Color", static_cast<int>(0xffd8bd3e));
        ...
        {
          const DWORD dwChatColor = GetGlobalVar("Scene.Colors.Summer.Text.ChatAllies.Color", static_cast<int>(0xff00ff00));
          // GetSingleton<IConsoleBuffer>()->Write( CONSOLE_STREAM_CHAT, szChatMessage.c_str(), dwChatColor );
    ...
    case VK_DELETE:
      if (nCursorPos < szChatMessage.size()) szChatMessage.erase(nCursorPos, 1);
      break;
    ...
    case VK_LEFT:
      ...
      if (m_keyboardState & E_CTRL_KEY_DOWN)
      {
        ...
        while (nCursorPos > 0 && isspace(szChatMessage[nCursorPos - 1])) nCursorPos--;
        if (nCursorPos > 0)
        {
          if (isalpha(szChatMessage[nCursorPos - 1]))
            while (nCursorPos > 0 && isalpha(szChatMessage[nCursorPos - 1])) nCursorPos--;
          else
            while (nCursorPos > 0 && !isalpha(szChatMessage[nCursorPos - 1])) nCursorPos--;
        }
      }
    ...
    case VK_RIGHT:
      if (nCursorPos == szChatMessage.size()) break;
      if (m_keyboardState == E_KEYBOARD_FREE)
      {
      ...
      else if (m_keyboardState & E_CTRL_KEY_DOWN)
      {
        ...
        if (nCursorPos < szChatMessage.size())
        {
          if (isalpha(szChatMessage[nCursorPos]))
            while (nCursorPos < szChatMessage.size() && isalpha(szChatMessage[nCursorPos])) nCursorPos++;
          else
            while (nCursorPos < szChatMessage.size() && !isalpha(szChatMessage[nCursorPos])) nCursorPos++;
        }

        while (nCursorPos < szChatMessage.size() && isspace(szChatMessage[nCursorPos])) nCursorPos++;
      }
      break;
...
int CUIScreen::MessageBox(const WORD *pszText, int nType)
{
  ...
  auto pElement = dynamic_cast_ptr<IUIElement *>(pMessageBox);
  AddChild(pElement);
  SetModalFlag(true);
  //
  return 0;
