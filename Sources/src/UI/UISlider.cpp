...
constexpr int TIME_TO_SCROLL = 50;
...
int CUISlider::operator&(IDataTree &ss)
{
  ...
  if (saver.IsReading()) LoadTextureAndMap(&saver, &pSliderTexture, "Elevator_Texture", &sliderMapa, "Elevator_Maps");
  else SaveTextureAndMap(&saver, pSliderTexture, "Elevator_Texture", sliderMapa, "Elevator_Maps");
  return 0;
}
...
{
  // ScrollBar handles NOTIFY messages from children
...
void CUISlider::UpdatePosition(int nCoord)
{
  ...
  if (!bVertical)
  {
    ...
    float nNumberOfPositions = static_cast<float>(m_nMax - m_nMin) / m_nStep;
    float nSizeOfInterval = (rc.right - rc.left - m_nElevatorWidth) / (nNumberOfPositions);
    m_nPos = (nCoord - rc.left - m_nElevatorWidth / 2 + nSizeOfInterval / 2) / nSizeOfInterval;
    m_nPos *= m_nStep;
  }
  else
  {
    ...
    float nNumberOfPositions = static_cast<float>(m_nMax - m_nMin) / m_nStep;
    float nSizeOfInterval = (rc.bottom - rc.top - m_nElevatorWidth) / (nNumberOfPositions);
    m_nPos = (nCoord - rc.top - m_nElevatorWidth / 2 + nSizeOfInterval / 2) / nSizeOfInterval;
    // m_nPos = (float) ( nCoord - rc.top ) / nSizeOfInterval;
    m_nPos *= m_nStep;
  }

  if (m_nPos > m_nMax) m_nPos = m_nMax;
  else if (m_nPos < m_nMin) m_nPos = m_nMin;
...
void CUISlider::Visit(interface ISceneVisitor *pVisitor)
{
  ...
  if (m_nMin < m_nMax)
  {
    // now let's draw the elevator
    // here it is assumed that nMin <= nPosition <= nMax
    SGFXRect2 rect;
...
bool CUISlider::OnMouseMove(const CVec2 &vPos, EMouseState mouseState)
{
  ...
  {
    if (!bVertical) { UpdatePosition(vPos.x); }
    else { UpdatePosition(vPos.y); }
  }
  return true;
}
...
bool CUISlider::OnLButtonDown(const CVec2 &vPos, EMouseState mouseState)
{
  ...
  if (mouseState == E_MOUSE_FREE || !bRes) return bRes;
...
{
  if (nPos <= m_nMin) { m_nPos = m_nMin; }
  else if (nPos >= m_nMax) { m_nPos = m_nMax; }
  else m_nPos = nPos;
  NotifyPositionChanged();
}
...
void CUISlider::SetMinValue(int nMin)
{
  ...
  if (m_nPos < m_nMin) SetPosition(m_nMin);
}

void CUISlider::SetMaxValue(int nMax)
{
  m_nMax = nMax;
  if (m_nPos > m_nMax) SetPosition(m_nMax);
}
...
void CUIScrollBar::Reposition(const CTRect<float> &rcParent)
{
  ...
  // Now I’ll use the handles to change the positions for the ScrollBar components
  if (!IsVertical())
  {
...
{
  // I'm checking to see if the min or max buttons are pressed
