...
int CUITimeCounter::operator&(IDataTree &ss)
{
  ...
  if (saver.IsReading()) dwDisabledCounterColor = dwCounterColor;
  saver.Add("ColorDisabled", &dwDisabledCounterColor);
  saver.Add("BGColor", &dwBGColor);

  if (saver.IsReading()) fCurrent = fEnd;
  return 0;
}
...
bool CUITimeCounter::ProcessMessage(const SUIMessage &msg)
{
  ...
  switch (msg.nMessageCode)
  {
    case UI_SET_ANIMATION_TIME:
      ...
      {
        EnableWindow(false);// messages from the mouse will not be processed
        if (msg.nFirst == -1)
        {
          // button is disabled
          bNeedAnimate = false;
...
const DWORD CUITimeCounter::GetCounterColor() { return IsWindowEnabled() ? dwCounterColor : dwDisabledCounterColor; }
...
bool CUITimeCounter::Update(const NTimer::STime &currTime)
{
  ...
  float k = static_cast<float>(dwGameTime - dwBeginTime) / (dwRingTime - dwBeginTime);
  if (k < 0) k = 0;
