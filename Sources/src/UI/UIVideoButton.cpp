...
int CUIVideoButton::operator&(IDataTree &ss)
{
  ...
  if (saver.IsReading()) InitVideoPlayer();
...
void CUIVideoButton::Draw(IGFX *pGFX) { pVideoPlayer->Draw(pGFX); }

bool CUIVideoButton::Update(const NTimer::STime &currTime) { return pVideoPlayer->Update(currTime); }
...
int CUIVideoButton::GetCurrentFrame() { return pVideoPlayer->GetCurrentFrame(); }

bool CUIVideoButton::SetCurrentFrame(int nFrame) { return pVideoPlayer->SetCurrentFrame(nFrame); }